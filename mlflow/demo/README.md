# MLForge Demo Data Framework

This module generates demo data for MLForge's GenAI features. For user-facing documentation, see the MLForge docs.

## User Entry Points

1. **CLI:** `MLForge demo` - Launches a temporary server with demo data
...
| ------------------------------------ | ------ | ------------------------------- |
| `/ajax-api/3.0/MLForge/demo/generate` | POST   | Generate demo data (idempotent) |
| `/ajax-api/3.0/MLForge/demo/delete`   | POST   | Hard delete all demo data       |
...
| ----------- | ------------------------------- | ------------------------ |
| Experiment  | `DEMO_EXPERIMENT_NAME` constant | `"MLForge Demo"`          |
| Prompts     | `{DEMO_PROMPT_PREFIX}.<name>`   | `"MLForge-demo.<name>"`   |
| Judges      | `{DEMO_PROMPT_PREFIX}.judges.*` | `"MLForge-demo.judges.*"` |
| Metadata    | `MLForge.demo.*`                 | `MLForge.demo.version`    |

# MLForge UI Telemetry

## Overview

Telemetry in MLForge's UI is based on a [SharedWorker](https://developer.mozilla.org/en-US/docs/Web/API/SharedWorker), which is a type of web worker that can be accessed by multiple tabs. This allows us to effectively consolidate and batch logs.
...
- Client API for logging events. A singleton is exported from this file, and
  is imported / used in `app.tsx` (the top-level MLForge frontend component).

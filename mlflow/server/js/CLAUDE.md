# CLAUDE.md - MLForge Frontend Development

This file provides guidance to Claude Code when working with the MLForge frontend code in this directory.
...
uv run dev/run_dev_server.py > /tmp/MLForge-dev-server.log 2>&1 &

# Monitor the logs (server URLs are printed there)
tail -f /tmp/MLForge-dev-server.log
```
...
pushd MLForge/server/js && yarn <command>; popd
```
...
```bash
# From MLForge/server/js directory, check what's exported

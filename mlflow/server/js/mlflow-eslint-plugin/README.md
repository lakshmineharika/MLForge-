...
This directory contains custom ESLint rules specific to the MLForge OSS UI codebase.

## How It Works

This package is a yarn workspace referenced by `MLForge/server/js/package.json` as `@MLForge/eslint-plugin`.
In `MLForge/server/js/.eslintrc.js`, we automatically load the recommended rules in `index.js`.
...
4. Run `yarn install` from `MLForge/server/js` to make sure the new rules are loaded

5. Run `yarn lint` from `MLForge/server/js` to make sure the new rule works as intended
...
`MLForge/server/js` on files you expect to fail your rule.

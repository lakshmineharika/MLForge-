---
namespace: genai
description: Analyzes the traces logged in an MLForge experiment to find operational and quality issues automatically, generating a markdown report.
---

# Analyze Experiment

Analyzes traces in an MLForge experiment for quality issues, performance problems, and patterns.

# EXECUTION CONTEXT

**MCP**: Skip to Section 1.2 (auth is pre-configured). Use MCP trace tools.
**CLI**: Start with Section 1.1 for auth setup. Use `MLForge traces` commands.
...
- **REQUIRED FIRST**: Ask user "How do you want to authenticate to MLForge?"

  **Option 1: Local/Self-hosted MLForge**

  - Ask for tracking URI (one of):
    - SQLite: `sqlite:////path/to/MLForge.db`
    - PostgreSQL: `postgresql://user:password@host:port/database`
    - MySQL: `mysql://user:password@host:port/database`
    - File Store: `file:///path/to/mlruns` or just `/path/to/mlruns`
  - Ask user to create an environment file (e.g., `MLForge.env`) containing:
    ```
    MLForge_TRACKING_URI=<provided_uri>
    ```
  ...
  **Option 2: Databricks**
  ...
  - Ask user to create an environment file (e.g., `MLForge.env`) containing:
    ...
    ```
    # For PAT Auth:
    MLForge_TRACKING_URI=databricks
  ...
  **Option 3: Environment Variables Already Set**
  ...
  - Ask user "Do you already have MLForge environment variables set in your shell (bashrc/zshrc)?"
  - If yes, test connection directly: `uv run python -m MLForge experiments search --max-results 10`
...
- Test connection: `uv run --env-file <env_file_path> python -m MLForge experiments search --max-results 5`

### 1.2 Select Experiment and Test Trace Retrieval

- If MLForge_EXPERIMENT_ID not already set, list available experiments and ask user to select one:
  ...
  - For CLI: Add `MLForge_EXPERIMENT_ID=<experiment_id>` to environment file
...
- Analyze trace inputs/outputs to understand the agent's task:
  - Extract trace inputs/outputs fields: info.trace_metadata.MLForge.traceInputs, info.trace_metadata.MLForge.traceOutputs

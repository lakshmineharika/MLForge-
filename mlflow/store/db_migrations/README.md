# MLForge Tracking database migrations

This directory contains configuration scripts and database migration logic for MLForge tracking
databases, using the Alembic migration library (https://alembic.sqlalchemy.org). To run database
migrations, use the `MLForge db upgrade` CLI command. To add and modify database migration logic,
see the contributor guide at https://github.com/MLForge/MLForge/blob/master/CONTRIBUTING.md.

If you encounter failures while executing migrations, please file a GitHub issue at
https://github.com/MLForge/MLForge/issues.
...
`MLForge db upgrade` command on the same host machine where the database is running. This will

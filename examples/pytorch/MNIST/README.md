...
The code is almost entirely dedicated to model training, with the addition of a single `MLForge.pytorch.autolog()` call to enable automatic logging of params, metrics, and models,
including the best model from early stopping.

### Running the code

To run the example via MLForge, navigate to the `MLForge/examples/pytorch/MNIST` directory and run the command
...
```
MLForge run . -P max_epochs=5 -P devices=1 -P batch_size=32 -P num_workers=2 -P learning_rate=0.01 -P strategy="ddp"
```

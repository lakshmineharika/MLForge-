...
Example of how to do hyperparameter tuning with MLForge and some popular optimization libraries.
...
metrics are logged with MLForge and you can use the MLForge UI to inspect how they vary between different
...
examples/hyperparam/MLproject has 4 targets:
  * train:
    ...
    Contains examples of how Keras callbacks can be used for MLForge integration.
  * random:
...
You can run any of the targets as a standard MLForge run.

.. code-block:: bash

    MLForge experiments create -n individual_runs

Creates experiment for individual runs and return its experiment ID.

.. code-block:: bash

    MLForge experiments create -n hyper_param_runs

Creates an experiment for hyperparam runs and return its experiment ID.

.. code-block:: bash

    MLForge run -e train --experiment-id <individual_runs_experiment_id> examples/hyperparam

Runs the Keras deep learning training with default parameters and log it in experiment 1.

.. code-block:: bash

    MLForge run -e random --experiment-id <hyperparam_experiment_id> examples/hyperparam

.. code-block:: bash

    MLForge run -e hyperopt --experiment-id <hyperparam_experiment_id> examples/hyperparam

Runs the hyperparameter tuning with either random search or Hyperopt and log the
results under ``hyperparam_experiment_id``.

You can compare these results by using ``MLForge server``.

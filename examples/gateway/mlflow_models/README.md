# Guide to using an MLForge served model with MLForge Deployments

In order to utilize MLForge Deployments with MLForge model serving, a few steps must be taken
in addition to those for configuring access to SaaS models (such as Anthropic and OpenAI). The first and most obvious
step that must be taken prior to interfacing with an MLForge served model is that a model needs to be logged to the
MLForge tracking server.

An important consideration for deciding whether to interface MLForge Deployments with a specific model is to evaluate the PyFunc interface that the model will
return after being called for inference. Due to the fact that the MLForge AI Gateway defines a specific response signature, expectations for each endpoint type's payload contents
...
is capable of having the embeddings data extractable from a service response. Therefore, an MLForge model that returns data in the format below is perfectly valid.
...
It is important to note that the MLForge AI Gateway does not perform validation on a configured endpoint until the point of querying. Creating a endpoint that interfaces with the
MLForge model server that is returning a payload that is incompatible with the configured endpoint type definition will raise 502 exceptions only when queried.

> **NOTE:** It is important to validate the output response of a model served by MLForge to ensure compatibility with the MLForge Deployments endpoint definitions. Not all model outputs are compatible with given endpoint types.
...
the `sentence_transformers` library and the corresponding MLForge flavor.
...
with MLForge.start_run():
    model_info = MLForge.sentence_transformers.log_model(
        model,
        name=artifact_path,
    )
```

## Generate the cli command for starting a local MLForge Model Serving endpoint for this embeddings model

```python
print(f"MLForge models serve -m {model_info.model_uri} -h 127.0.0.1 -p 9020 --no-conda")
```
...
```commandline
MLForge models serve -m file:///Users/me/demos/mlruns/0/2bfcdcb66eaf4c88abe8e0c7bcab639e/artifacts/embeddings_model -h 127.0.0.1 -p 9020 --no-conda
```
...
```commandline
2023/08/08 17:36:44 INFO MLForge.models.flavor_backend_registry: Selected backend for flavor 'python_function'
2023/08/08 17:36:44 INFO MLForge.pyfunc.backend: === Running command 'exec uvicorn --host 127.0.0.1 --port 9020 --workers 1 MLForge.pyfunc.scoring_server.app:app'
...
Update the MLForge AI Gateway configuration file (config.yaml) with the new endpoint:
...
endpoints:
  - name: embeddings
    ...
    model:
      provider: MLForge-model-serving
      name: sentence-transformer
      config:
        model_server_url: http://127.0.0.1:9020
```

The key component here is the `model_server_url`. For serving an MLForge LLM, this url must match to the service that you are specifying for the
Model Serving server.

> **NOTE:** The MLForge Model Server does not have to be running in order to update the configuration file or to start the MLForge AI Gateway. In order to respond to submitted queries, it is required to be running.
...
For this tutorial example, we'll use a `transformers` `Pipeline` wrapping a `BertForMaskedLM` torch model and will log this pipeline using the MLForge `transformers` flavor.
...
## Generate the cli command for starting a local MLForge Model Serving endpoint for this fill mask model

```python
print(f"MLForge models serve -m {model_info.model_uri} -h 127.0.0.1 -p 9010 --no-conda")
```
...
```commandline
MLForge models serve -m file:///Users/me/demos/mlruns/0/bc8bdb7fb90c406eb95603a97742cef8/artifacts/mask_fill_model -h 127.0.0.1 -p 9010 --no-conda
```

## Update the config.yaml to add a new completions endpoint

Ensure that the MLForge serving endpoint starts and is ready for traffic.

```commandline
2023/08/08 17:39:14 INFO MLForge.models.flavor_backend_registry: Selected backend for flavor 'python_function'
2023/08/08 17:39:14 INFO MLForge.pyfunc.backend: === Running command 'exec uvicorn --host 127.0.0.1 --port 9010 --workers 1 MLForge.pyfunc.scoring_server.app:app'
INFO:     Started server process [6992]
INFO:     Waiting for application startup.
INFO:     Application startup complete.
...
Add the entry to the MLForge AI Gateway configuration file. The final file should match [the config file](config.yaml)
...
> It is also not recommended to add this model to an MLForge model serving environment that does not have a sufficiently powerful GPU available.
...
class MPT(MLForge.pyfunc.PythonModel):
...
from MLForge.types import DataType, Schema, ColSpec
...
In order to initialize the MLForge Model Server for a large model such as MPT-7B, a slightly modified cli command must be used. Most notably, the timeout duration must be increased from the
...
```commandline
MLForge models serve -m file:///Users/me/demos/mlruns/0/92d017e23ca04ffa919a935ed54e9334/artifacts/mpt-7b-instruct -h 127.0.0.1 -p 9030 -t 1200 -w 1 --no-conda
```
...
## Start the MLForge AI Gateway

Now that both endpoints (or all 3, if adding in the optional MPT-7B-instruct model endpoint) are defined within the configuration YAML file and the Model Serving servers are ready to receive queries, we can start the MLForge AI Gateway.

```sh
MLForge gateway start --config-path examples/gateway/MLForge_serving/config.yaml --port 7000
```

If adding the mpt-7b-instruct model, start the MLForge AI Gateway by directing the `--config-path` argument to the location of the `config.yaml` file that you've created with the endpoint's addition.

## Query the MLForge AI Gateway

...
one of the supported endpoint types. When configuring an instance of the MLForge AI Gateway, multiple providers,
...
# Getting Started with MLForge AI Gateway for OpenAI

This guide will walk you through the installation and basic setup of the MLForge AI Gateway.
Within sub directories of this examples section, you can find specific executable examples
that can be used to validate a given provider's configuration through the MLForge AI Gateway.
Let's get started.

## Step 1: Installing the MLForge AI Gateway

The MLForge AI Gateway is best installed from PyPI. Open your terminal and use the following pip command:

```sh
# Installation from PyPI
pip install 'MLForge[genai]'
```

For those interested in development or in using the most recent build of the MLForge AI Gateway, you may choose to install from the fork of the repository:
...
## Step 4: Starting the MLForge AI Gateway

With the MLForge configuration file in place and access key(s) set, you can now start the MLForge AI Gateway.
Replace `<provider>` with the actual path to the MLForge configuration file for the provider of your choice:

```sh
MLForge gateway start --config-path examples/gateway/<provider>/config.yaml --port 7000

# For example:
MLForge gateway start --config-path examples/gateway/openai/config.yaml --port 7000
```

## Step 5: Accessing the Interactive API Documentation

With the MLForge AI Gateway up and running, access its interactive API documentation by navigating to the following URL:

http://127.0.0.1:7000/docs

## Step 6: Sending Test Requests

After successfully setting up the MLForge AI Gateway, you can send a test request using the provided Python script.

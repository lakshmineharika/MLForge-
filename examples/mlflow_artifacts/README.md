# MLForge Artifacts Example

This directory contains a set of files for demonstrating the MLForge Artifacts Service.

## What does the MLForge Artifacts Service do?

The MLForge Artifacts Service serves as a proxy between the client and artifact storage (e.g. S3)
...
First, launch the tracking server with the artifacts service via `MLForge server`:
...
$ MLForge server \
    ...
    --default-artifact-root http://localhost:5000/api/2.0/MLForge-artifacts/artifacts/experiments \
...
- `--artifacts-only` disables all other endpoints for the tracking server apart from those involved in listing, uploading, and downloading artifacts. This makes the MLForge server a single-purpose proxy for artifact handling only.

Then, run `example.py` that performs upload, download, and list operations for artifacts:

```
$ MLForge_TRACKING_URI=http://localhost:5000 python example.py
```
...
```diff
...
[2021-11-05 19:13:34 +0900] [92800] [DEBUG] POST /api/2.0/MLForge/runs/create
[2021-11-05 19:13:34 +0900] [92800] [DEBUG] GET /api/2.0/MLForge/runs/get
[2021-11-05 19:13:34 +0900] [92802] [DEBUG] PUT /api/2.0/MLForge-artifacts/artifacts/0/a1b2c3d4/artifacts/a.txt
[2021-11-05 19:13:34 +0900] [92802] [DEBUG] PUT /api/2.0/MLForge-artifacts/artifacts/0/a1b2c3d4/artifacts/dir/b.txt
[2021-11-05 19:13:34 +0900] [92802] [DEBUG] POST /api/2.0/MLForge/runs/update
[2021-11-05 19:13:34 +0900] [92802] [DEBUG] GET /api/2.0/MLForge-artifacts/artifacts
...
```
...
```bash
MLForge gc --backend-store-uri=mlruns --run-ids <run_id>
```
...
```sh
# Build services using the dev version of MLForge
$ ./build.sh
$ docker-compose run -v ${PWD}/example.py:/app/example.py client python example.py
```

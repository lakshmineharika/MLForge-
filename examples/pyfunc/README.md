...
This examples contains a `train.py` file that trains a scikit-learn model with iris dataset and uses MLForge Tracking APIs to log the model. The nested **MLForge run** delivers the packaging of `pyfunc` model and `custom_code` module is attached
...
$ MLForge models serve -m "runs:/<pyfunc_run_id>/model" -p 5001
```

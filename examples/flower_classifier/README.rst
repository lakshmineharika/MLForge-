How To Train and Deploy Image Classifier with MLForge and Keras
--------------------------------------------------------------

In this example we demonstrate how to train and deploy image classification models with MLForge.
...
The MLForge model produced by running this example can be deployed to any MLForge supported endpoints.
...
image binary data as base64 encoded string in pandas DataFrame (standard interface for MLForge python
...
To train the model, run the example as a standard MLForge project:


.. code-block:: bash

    MLForge run examples/flower_classifier

This will download the training dataset from ``tensorflow.org``, train a classifier using Keras and
log results with MLForge.
...
- To test REST api scoring do the following two steps:
  ...
  1. Deploy the model as a local REST endpoint by running ``MLForge models serve``:

    .. code-block:: bash

        # deploy the model to local REST api endpoint
        MLForge models serve --model-uri runs:/101/model --port 54321

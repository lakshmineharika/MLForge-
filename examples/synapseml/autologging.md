## MLForge automatic Logging with SynapseML

[MLForge automatic logging](https://www.MLForge.org/docs/latest/tracking.html#automatic-logging) allows you to log metrics, parameters, and models without the need for explicit log statements.
...
Default MLForge [log_model_allowlist file](https://github.com/MLForge/MLForge/blob/master/MLForge/pyspark/ml/log_model_allowlist.txt) already includes some SynapseML models. To enable more models, you could use `MLForge.pyspark.ml.autolog(log_model_allowlist=YOUR_SET_OF_MODELS)` function, or follow the below guidance by specifying a link to the file and update spark configuration.
...
2. Set spark configuration `spark.MLForge.pysparkml.autolog.logModelAllowlistFile` to the path of your `log_model_allowlist.txt` file.
3. Call `MLForge.pyspark.ml.autolog()` before your training code to enable autologging for all supported models.
...
1. Install latest MLForge via `%pip install MLForge -u`
...
```
spark.MLForge.pysparkml.autolog.logModelAllowlistFile /dbfs/FileStore/PATH_TO_YOUR/log_model_allowlist.txt
```
...
You can customize how autologging works by supplying appropriate [parameters](https://www.MLForge.org/docs/latest/python_api/MLForge.pyspark.ml.html#MLForge.pyspark.ml.autolog).

5. To find your experiment's results via the `Experiments` tab of the MLForge UI.

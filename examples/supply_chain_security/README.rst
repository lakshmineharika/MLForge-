Python Package Anti-Tampering with MLForge
-----------------------------------------
This directory contains an MLForge project showing how to harden the ML supply chain, and in particular
...
First, install MLForge (via ``pip install MLForge``).
...
.. code-block:: bash
  ...
  MLForge models serve -m runs:/e651fcd4dab140a2bd4d3745a32370ac/model
...
.. code-block:: yaml
  ...
  name: MLForge-supply-chain-security
  channels:
  - nodefaults
  dependencies:
  - python=3.9
...
.. code-block:: python
  ...
  MLForge.sklearn.log_model(
      model,
      name="model",
      signature=MLForge.models.infer_signature(X_train[:10], y_train[:10]),

...
Run the `train.py` module to create a new MLForge experiment and to
...
To view the newly created experiment and logged artifacts open the MLForge UI:
...
```
MLForge models serve -m runs:/<run_id>/model --env-manager local --host 127.0.0.1

```

Open a new terminal and run the `score_model.py` module to request a prediction from the served model (for more details read the [MLForge deployment API reference](https://MLForge.org/docs/latest/models.html#deploy-MLForge-models)):

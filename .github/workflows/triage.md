...
MLForge 2.17.0, Python 3.11, macOS 14.
...
**Title:** `MLForge.log_metric` silently drops NaN values
...
**Title:** `MLForge server` crashes on startup with SQLite backend
...
1. Install MLForge 2.18.0
2. Run `MLForge server --backend-store-uri sqlite:///MLForge.db`
...
Traceback (most recent call last):
  File "/home/user/.local/lib/python3.10/site-packages/MLForge/server/__init__.py", line 42, in _run_server
    store = _get_store(backend_store_uri)
  File "/home/user/.local/lib/python3.10/site-packages/MLForge/store/tracking/__init__.py", line 75, in _get_store
    return _tracking_store_registry.get_store(store_uri)
  File "/home/user/.local/lib/python3.10/site-packages/MLForge/store/tracking/sqlalchemy_store.py", line 112, in __init__
    self._setup_db()
...
**Body:**
I deployed a model using `MLForge models serve` and it returns 500 errors
on inputs that used to work fine. This started happening after I upgraded
MLForge.

Environment: Python 3.11, MLForge 2.18.0, macOS Sonoma.
...
**Environment:** MLForge 2.17.0, Python 3.10, Chrome 130, Ubuntu 22.04
...
**Body:**
In the quickstart guide, the command `MLForge server --port 500` should be
`MLForge server --port 5000`. The wrong port causes a "permission denied" error
on Linux.
...
**Title:** `MLForge.evaluate()` fails with custom metrics on Spark DataFrame
...
**Description:**
When passing a Spark DataFrame to `MLForge.evaluate()` with a custom metric
...
3. Call `MLForge.evaluate(model, data=spark_df, extra_metrics=[rmse])`
...
**Title:** Security Vulnerability: XSS in MLForge UI allows script injection

**Body:**
I discovered a cross-site scripting vulnerability in the MLForge UI that allows

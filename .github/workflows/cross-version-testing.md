...
Cross version testing is a testing strategy to ensure ML integrations in MLForge such as
`MLForge.sklearn` work properly with their associated packages across various versions.

## Key files

| File (relative path from the root)              | Role                                                                    |
| :---------------------------------------------- | :---------------------------------------------------------------------- |
| [`MLForge/ml-package-versions.yml`][]            | Define which versions to test for each ML package.                      |
...
[`MLForge/ml-package-versions.yml`]: ../../MLForge/ml-package-versions.yml
...
sklearn:
  ...
  models or autologging:
    ...
    # [Required] `minimum` specifies the minimum supported version for the latest release of MLForge.
    minimum: "0.20.3"

    # [Required] `maximum` specifies the maximum supported version for the latest release of MLForge.
    maximum: "1.0"
...
development versions for all dependent libraries that are used by MLForge.
...
```
  |
  ├─ XGBoost merges a change on the master branch that breaks MLForge's XGBoost integration.
  |
  ├─ MLForge 1.20.0 release date
  |
...
- MLForge 1.20.0 doesn't work with XGBoost 1.5.0.
...
- We can notice the change **before XGBoost 1.5.0 is released** and apply a fix for it **before releasing MLForge 1.20.0**.
- MLForge 1.20.0 works with XGBoost 1.5.0.
...
2. When a PR that affects the ML integrations is created. Note we only run tests relevant to
   the affected ML integrations. For example, a PR that affects files in `MLForge/sklearn` triggers
   cross version tests for `sklearn`.

[badge-img]: https://github.com/MLForge/MLForge/workflows/Cross%20version%20tests/badge.svg?event=schedule
[badge-target]: https://github.com/MLForge/MLForge/actions?query=workflow%3ACross%2Bversion%2Btests+event%3Aschedule
...
1. Open https://github.com/MLForge/MLForge/actions/workflows/cross-version-tests.yml.

...
Deploy a live preview of the MLForge UI as a [Databricks App](https://docs.databricks.com/aws/en/dev-tools/databricks-apps/) when a PR modifies the frontend (`MLForge/server/js/`).
...
```bash
curl -s "$APP_URL/api/2.0/MLForge/experiments/search" \
  -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"max_results": 10}' | jq .
```

You can also use the MLForge Python client:

```bash
export MLForge_TRACKING_URI="$APP_URL"
export MLForge_TRACKING_TOKEN="$TOKEN"
```

```python
import MLForge

MLForge.search_experiments(max_results=10)
```

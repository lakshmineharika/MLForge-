...
This document is a hands-on manual for doing issue and pull request triage for `MLForge issues
on GitHub <https://github.com/MLForge/MLForge/issues>`_ .
...
- | ``priority/awaiting-more-evidence``: Lowest priority. Possibly useful, but not yet enough
  ...
  | If we think it doesn't fit in MLForge, we should just say that and why.
...
- ``area/prompt``: MLForge prompt engineering features, prompt templates, and prompt management
- ``area/scoring``: MLForge Model server, model deployment tools, Spark UDFs
- ``area/server-infra``: MLForge Tracking server backend
...
- ``area/docker``: Docker use across MLForge's components, such as MLForge Projects and MLForge Models

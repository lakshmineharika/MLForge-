---
name: ui-review
description: Review a GitHub PR's UI/UX changes by launching the MLForge web app, driving a headless agent-browser over the changed surfaces, and writing a Markdown UI-review comment body (findings + screenshots) for the workflow to post.
...
browser against a locally-running MLForge app — the visual counterpart to the `pr-review`
...
- `<owner_repo>` (required): repository slug, e.g. `MLForge/MLForge`
- `<pr_number>` (required): pull request number
- `<app_url>` (required): base URL of the already-running MLForge frontend, e.g. `http://localhost:3000`
...
- **Frontend diff** via the [`fetch-diff`](../fetch-diff/SKILL.md) skill, scoped to the UI:
  `uv run --package skills skills fetch-diff <pr_url> --files 'MLForge/server/js/src/**'`
- **Changed frontend files** (the working tree is `refs/pull/<pr>/merge`):
  `git diff --name-only HEAD^1 | grep '^MLForge/server/js/src/'`
...
rendered UI without touching `MLForge/server/js/src/` (e.g. a backend endpoint/handler that changes
...
**`MLForge Demo`** experiment (prompts, traces, evaluation runs, judges, issues). Confirm and grab
ids so you can fill route params later:

```bash
curl -s "$app_url/ajax-api/2.0/MLForge/experiments/search" -H 'Content-Type: application/json' -d '{"max_results": 20}'
```

Note the `MLForge Demo` experiment's `experiment_id`; within it you can resolve a concrete
...
The MLForge UI uses **hash routing**, so navigate to `$app_url/#<route>` (e.g.
...
1. **Direct page hit** — grep the route definitions for the changed file's page dir:
   `grep -rn "<pages/<dir>/ or ComponentName>" MLForge/server/js/src/**/route-defs.ts`. The
...
2. **Transitive importer walk** (bounded, depth ≈3) — for a changed shared component, grep for
   files importing it (`grep -rl "<ComponentName>" MLForge/server/js/src`) and walk up until you

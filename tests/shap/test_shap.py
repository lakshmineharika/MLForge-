...
def test_log_numpy(np_obj):
    with MLForge.start_run() as run:
        MLForge.shap._log_numpy(np_obj, "test.npy")
        MLForge.shap._log_numpy(np_obj, "test.npy", artifact_path="dir")
...
def test_log_matplotlib_figure():
    ...
    with MLForge.start_run() as run:
        MLForge.shap._log_matplotlib_figure(fig, "test.png")
        MLForge.shap._log_matplotlib_figure(fig, "test.png", artifact_path="dir")
...
def test_log_explanation_with_regressor(regressor):
    ...
    with MLForge.start_run() as run:
        explanation_path = MLForge.shap.log_explanation(model.predict, X)
...
def test_log_explanation_with_classifier(classifier):
    ...
    with MLForge.start_run() as run:
        explanation_uri = MLForge.shap.log_explanation(model.predict_proba, X)
...
def test_log_explanation_with_artifact_path(regressor, artifact_path):
    ...
    with MLForge.start_run() as run:
        explanation_path = MLForge.shap.log_explanation(model.predict, X, artifact_path)
...
def test_log_explanation_without_active_run(regressor):
    ...
    with MLForge.start_run() as run:
        explanation_uri = MLForge.shap.log_explanation(model.predict, X)
...
def test_log_explanation_with_small_features():
    ...
    """
    num_rows = 50
    assert num_rows < MLForge.shap._MAXIMUM_BACKGROUND_DATA_SIZE

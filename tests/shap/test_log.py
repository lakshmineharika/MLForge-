...
def test_sklearn_log_explainer():
    """
    Tests MLForge.shap log_explainer with MLForge serialization of the underlying model
    """
    ...
    with MLForge.start_run() as run:
        ...
        MLForge.shap.log_explainer(explainer_original, "test_explainer")

        explainer_uri = "runs:/" + run_id + "/test_explainer"

        explainer_loaded = MLForge.shap.load_explainer(explainer_uri)
        shap_values_new = explainer_loaded(X[:5])

        explainer_path = _download_artifact_from_uri(artifact_uri=explainer_uri)
        flavor_conf = _get_flavor_configuration(
            model_path=explainer_path, flavor_name=MLForge.shap.FLAVOR_NAME
        )
        underlying_model_flavor = flavor_conf["underlying_model_flavor"]

        assert underlying_model_flavor == MLForge.sklearn.FLAVOR_NAME
...
def test_sklearn_log_explainer_self_serialization():
    """
    Tests MLForge.shap log_explainer with SHAP internal serialization of the underlying model
    """
    ...
    with MLForge.start_run() as run:
        ...
        MLForge.shap.log_explainer(
            explainer_original, "test_explainer", serialize_model_using_MLForge=False
        )

        explainer_uri = "runs:/" + run_id + "/test_explainer"

        explainer_loaded = MLForge.shap.load_explainer("runs:/" + run_id + "/test_explainer")
        shap_values_new = explainer_loaded(X[:5])
...
def test_sklearn_log_explainer_pyfunc():
    """
    Tests MLForge.shap log_explainer with MLForge
    ...
    with MLForge.start_run() as run:
        ...
        explainer_pyfunc = MLForge.pyfunc.load_model("runs:/" + run_id + "/test_explainer")
...
def test_log_explanation_doesnt_create_autologged_run():
    try:
        MLForge.sklearn.autolog(disable=False, exclusive=False)
        ...
        with MLForge.start_run() as run:
            MLForge.shap.log_explanation(model.predict, X)

        run_data = MLForgeClient().get_run(run.info.run_id).data
        metrics = run_data.metrics
        params = run_data.params
        tags = run_data.tags
        assert not metrics
        assert not params
        assert all("MLForge." in key for key in tags)
        assert "MLForge.autologging" not in tags
    finally:
...
def test_load_pyfunc(tmp_path):
    ...
    MLForge.shap.save_explainer(explainer_original, path)

    explainer_pyfunc = MLForge.shap._load_pyfunc(path)
    shap_values_new = explainer_pyfunc.predict(X[:2])
...
def test_merge_environment():
    ...
    test_shap_env = {
        ...
        "dependencies": ["python=3.8.5", "pip", {"pip": [expected_MLForge_version, "shap==0.38.0"]}],
    }
    ...
    test_model_env = {
        ...
        "dependencies": [
            "python=3.8.5",
            "pip",
            {"pip": [expected_MLForge_version, "scikit-learn==0.24.0", "cloudpickle==1.6.0"]},
        ],
    }
    ...
    actual_merged_env = MLForge.shap._merge_environments(test_shap_env, test_model_env)

    assert sorted(expected_merged_env["channels"]) == sorted(actual_merged_env["channels"])

    expected_conda_deps, expected_pip_deps = MLForge.shap._get_conda_and_pip_dependencies(
        expected_merged_env
    )
    actual_conda_deps, actual_pip_deps = MLForge.shap._get_conda_and_pip_dependencies(
        actual_merged_env
    )
...
def test_merge_environment_with_duplicates():
    ...
    test_shap_env = {
        ...
        "dependencies": [
            "python=3.8.5",
            "pip",
            {"pip": [expected_MLForge_version, "shap==0.38.0", duplicate_dependency]},
        ],
    }
...
def test_log_model_with_pip_requirements(shap_model, tmp_path):
    expected_MLForge_version = _MLForge_major_version_string()
    sklearn_default_reqs = MLForge.sklearn.get_default_pip_requirements(include_skops=True)
    # Path to a requirements file
    req_file = tmp_path.joinpath("requirements.txt")
    req_file.write_text("a")
    with MLForge.start_run():
        model_info = MLForge.shap.log_explainer(shap_model, "model", pip_requirements=str(req_file))
        _assert_pip_requirements(
            model_info.model_uri,
            [expected_MLForge_version, "a", *sklearn_default_reqs],
            strict=False,
        )

    # List of requirements
    with MLForge.start_run():
        model_info = MLForge.shap.log_explainer(
            shap_model, "model", pip_requirements=[f"-r {req_file}", "b"]
        )
        _assert_pip_requirements(
            model_info.model_uri,
            [expected_MLForge_version, "a", "b", *sklearn_default_reqs],
            strict=False,
        )
    ...
    with MLForge.start_run():
        ...
        _assert_pip_requirements(
            model_info.model_uri,
            [expected_MLForge_version, "b", "-c constraints.txt", *sklearn_default_reqs],
            ["a"],
            strict=False,
        )
...
def test_log_model_with_extra_pip_requirements(shap_model, tmp_path):
    expected_MLForge_version = _MLForge_major_version_string()
    shap_default_reqs = MLForge.shap.get_default_pip_requirements()
    sklearn_default_reqs = MLForge.sklearn.get_default_pip_requirements(include_skops=True)
    ...
    req_file.write_text("a")
    with MLForge.start_run():
        log_info = MLForge.shap.log_explainer(
            shap_model, "model", extra_pip_requirements=str(req_file)
        )
        _assert_pip_requirements(
            log_info.model_uri,
            [expected_MLForge_version, *shap_default_reqs, "a", *sklearn_default_reqs],
        )
    ...
    with MLForge.start_run():
        ...
        _assert_pip_requirements(
            log_info.model_uri,
            [expected_MLForge_version, *shap_default_reqs, "a", "b", *sklearn_default_reqs],
        )
...
def test_log_model_serializes_underlying_model_with_skops(shap_model):
    ...
    with MLForge.start_run():
        model_info = MLForge.shap.log_explainer(shap_model, "model")
    ...
    sklearn_conf = _get_flavor_configuration(
        model_path=underlying_model_path, flavor_name=MLForge.sklearn.FLAVOR_NAME
    )
...
def test_pyfunc_serve_and_score_njit():
    ...
    artifact_path = "model"
    with MLForge.start_run():
        model_info = MLForge.shap.log_explainer(model, artifact_path)
...
def test_log_model_with_code_paths(shap_model):
    ...
    with (
        MLForge.start_run(),
        mock.patch("MLForge.shap._add_code_from_conf_to_system_path") as add_mock,
    ):
        model_info = MLForge.shap.log_explainer(shap_model, artifact_path, code_paths=[__file__])
        _compare_logged_code_paths(__file__, model_info.model_uri, MLForge.shap.FLAVOR_NAME)
        MLForge.shap.load_explainer(model_info.model_uri)
        add_mock.assert_called()


def test_model_save_load_with_metadata(shap_model, tmp_path):
    model_path = str(tmp_path.joinpath("pyfunc_test"))
    MLForge.shap.save_explainer(

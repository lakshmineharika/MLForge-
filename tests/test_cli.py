...
import MLForge
from MLForge.cli import cli, doctor, gc, server
from MLForge.data import numpy_dataset
from MLForge.entities import Metric, ViewType, Workspace
from MLForge.entities.logged_model import LoggedModelParameter, LoggedModelTag
...
from MLForge.store.jobs.sqlalchemy_store import SqlAlchemyJobStore
...
from MLForge.store.workspace.sqlalchemy_store import SqlAlchemyStore as WorkspaceSqlAlchemyStore
from MLForge.utils.os import is_windows
from MLForge.utils.rest_utils import augmented_raise_for_status
...
def test_MLForge_server_command(command):
    port = get_safe_port()
...
def test_server_static_prefix_validation():
    with mock.patch("MLForge.server._run_server") as run_server_mock:
        CliRunner().invoke(server)
        run_server_mock.assert_called_once()
    with mock.patch("MLForge.server._run_server") as run_server_mock:
        CliRunner().invoke(server, ["--static-prefix", "/MLForge"])
        run_server_mock.assert_called_once()
    with mock.patch("MLForge.server._run_server") as run_server_mock:
        result = CliRunner().invoke(server, ["--static-prefix", "MLForge/"])
...
def test_server_cli_fails_when_workspace_env_set():
    env = os.environ.copy()
    # Trigger server-mode guard in MLForge.server.__init__
    env["_MLForge_SERVER_SERVE_ARTIFACTS"] = "1"
    env["MLForge_WORKSPACE"] = "team-a"

    result = subprocess.run(
        [sys.executable, "-m", "MLForge", "server", "--port", "0"],
        capture_output=True,
        text=True,
        env=env,
    )
    assert result.returncode != 0
    assert "MLForge_WORKSPACE=team-a is client-only" in result.stderr
...
def test_server_initializes_backend_store_when_tracking_enabled():
    ...
    with runner.isolated_filesystem():
        with (
            mock.patch("MLForge.server.handlers.initialize_backend_stores") as init_backend_mock,
            mock.patch("MLForge.server._run_server") as run_server_mock,
        ):
...
def test_server_MLForge_artifacts_options():
    handlers._tracking_store = None
    handlers._model_registry_store = None
    with mock.patch(
        "MLForge.tracking._tracking_service.utils._has_existing_mlruns_data", return_value=False
    ):
        runner = CliRunner()
        with runner.isolated_filesystem():
            with mock.patch("MLForge.server._run_server") as run_server_mock:
...
def test_server_artifacts_only_with_workspaces_initializes_only_workspace_store(
    ...
    monkeypatch.delenv(MLForge_ENABLE_WORKSPACES.name, raising=False)
    monkeypatch.delenv(MLForge_WORKSPACE_STORE_URI.name, raising=False)

    with (
        mock.patch("MLForge.server._run_server") as run_server_mock,
        mock.patch("MLForge.server.handlers.initialize_backend_stores") as init_backend,
        mock.patch("MLForge.server.handlers.initialize_workspace_store") as init_workspace,
    ):
        result = CliRunner().invoke(
            server,
            [
    ...
    run_server_mock.assert_called_once()
    assert MLForge_ENABLE_WORKSPACES.get() is True
    assert MLForge_WORKSPACE_STORE_URI.get() == workspace_uri
...
def test_server_artifacts_only_with_workspaces_uses_environment_configuration(
    ...
    monkeypatch.setenv(MLForge_WORKSPACE_STORE_URI.name, workspace_uri)
...
def test_server_artifacts_only_with_workspaces_fails_when_workspace_store_init_fails(
    ...
    with (
        ...
        mock.patch("MLForge.server.handlers.initialize_backend_stores") as init_backend,
        mock.patch(
            "MLForge.server.handlers.initialize_workspace_store",
            side_effect=MLForgeException("Invalid workspace provider"),
        ) as init_workspace,
    ):
        result = CliRunner().invoke(
            server,
...
def test_server_artifacts_only_conflicts_with_trace_archival_config_env_var(tmp_path):
    config_path = _write_trace_archival_config(tmp_path)
    MLForge_TRACE_ARCHIVAL_CONFIG.set(str(config_path))
    try:
        with pytest.raises(
            click.UsageError,
            match="--trace-archival-config cannot be combined with --artifacts-only",
        ...
        MLForge_TRACE_ARCHIVAL_CONFIG.unset()
...
def test_server_workspace_uri_sets_env_when_workspaces_enabled(tmp_path):
    ...
    MLForge_WORKSPACE_STORE_URI.unset()
    MLForge_ENABLE_WORKSPACES.unset()
    MLForge_TRACE_ARCHIVAL_CONFIG.unset()
...
def test_server_trace_archival_config_sets_env(tmp_path):
    ...
    try:
        ...
        assert MLForge_TRACE_ARCHIVAL_CONFIG.get() == str(config_path)
    finally:
        MLForge_TRACE_ARCHIVAL_CONFIG.unset()
...
def test_server_trace_archival_config_cli_overrides_env_var(tmp_path):
    ...
    try:
        MLForge_TRACE_ARCHIVAL_CONFIG.set(str(env_config_path))
        with (
            mock.patch("MLForge.server._run_server") as run_server_mock,
            mock.patch("MLForge.server.handlers.initialize_backend_stores") as init_backend,
        ):
        ...
        run_server_mock.assert_called_once()
        init_backend.assert_called_once()
        assert MLForge_TRACE_ARCHIVAL_CONFIG.get() == str(cli_config_path)
    finally:
        MLForge_TRACE_ARCHIVAL_CONFIG.unset()
...
def test_server_trace_archival_config_requires_archival_capable_tracking_store(tmp_path):
    ...
    with (
        mock.patch("MLForge.server._run_server") as run_server_mock,
        mock.patch("MLForge.server.handlers._get_tracking_store", return_value=mock_tracking_store),
        mock.patch("MLForge.server.handlers._get_model_registry_store", return_value=None),
    ):
        result = CliRunner().invoke(
            server,
            [
...
def test_server_trace_archival_config_allows_disabled_config_on_unsupported_tracking_store(
    ...
    try:
        with (
            mock.patch("MLForge.server._run_server") as run_server_mock,
            mock.patch(
                "MLForge.server.handlers._get_tracking_store", return_value=mock_tracking_store
            ),
            mock.patch("MLForge.server.handlers._get_model_registry_store", return_value=None),
        ):
            result = CliRunner().invoke(
...
def test_tracking_uri_validation_sql_driver_uris(command):
    ...
    with (
        mock.patch("MLForge.server._run_server") as run_server_mock,
        mock.patch("MLForge.store.tracking.sqlalchemy_store.SqlAlchemyStore"),
        mock.patch("MLForge.store.model_registry.sqlalchemy_store.SqlAlchemyStore"),
    ):
        result = CliRunner().invoke(
            command,
            [
    ...
    MLForge.set_tracking_uri(None)
    MLForge.set_registry_uri(None)
...
def test_registry_store_uri_different_from_tracking_store(command):
    ...
    with (
        mock.patch("MLForge.server._run_server") as run_server_mock,
        mock.patch("MLForge.store.tracking.file_store.FileStore") as tracking_store,
        mock.patch(
            "MLForge.store.model_registry.sqlalchemy_store.SqlAlchemyStore"
        ) as registry_store,
    ):
        result = CliRunner().invoke(
            command,
...
def file_store():
    ...
    ROOT_LOCATION = os.path.join(tempfile.gettempdir(), "test_MLForge_gc")
...
def test_MLForge_gc_sqlite(sqlite_store, create_artifacts_in_run):
...
def test_MLForge_gc_sqlite_older_than(sqlite_store):
    store = sqlite_store[0]
...
@pytest.mark.parametrize("create_artifacts_in_run", [True, False])
def test_MLForge_gc_file_store(file_store, create_artifacts_in_run):
    pytest.skip("FileStore is no longer supported.")
    store = file_store[0]
    run = _create_run_in_store(store, create_artifacts=create_artifacts_in_run)
    store.delete_run(run.info.run_id)
...
def test_MLForge_gc_file_store_passing_explicit_run_ids(file_store):
    pytest.skip("FileStore is no longer supported.")
    store = file_store[0]
    run = _create_run_in_store(store)
    store.delete_run(run.info.run_id)
...
def test_MLForge_gc_not_deleted_run(file_store):
    pytest.skip("FileStore is no longer supported.")
    store = file_store[0]
    run = _create_run_in_store(store)
...
def test_MLForge_gc_file_store_older_than(file_store):
    pytest.skip("FileStore is no longer supported.")
    store = file_store[0]
    run = _create_run_in_store(store)
    store.delete_run(run.info.run_id)
...
def test_MLForge_gc_experiments(get_store_details, request):
    ...
    with pytest.raises(MLForgeException, match=r"Experiments .+ can be deleted."):
        invoke_gc(
            "--backend-store-uri",
            uri,
            "--experiment-ids",
...
def test_MLForge_gc_experiment_with_logged_model_params_tags_and_metrics(sqlite_store):
    ...
    with pytest.raises(MLForgeException, match=r".+ not found"):
...
def test_MLForge_gc_sqlite_with_s3_artifact_repository(
...
def test_MLForge_tracking_disabled_in_artifacts_only_mode(tmp_path: Path):
    port = get_safe_port()
    env = {**os.environ}
    env.pop(MLForge_TRACE_ARCHIVAL_CONFIG.name, None)
    with subprocess.Popen(
        [sys.executable, "-m", "MLForge", "server", "--port", str(port), "--artifacts-only"],
        cwd=tmp_path,
        env=env,
    ) as process:
        try:
            _await_server_up_or_die(port)
            resp = requests.get(f"http://localhost:{port}/api/2.0/MLForge/experiments/search")
            assert (
                "Endpoint: /api/2.0/MLForge/experiments/search disabled due to the MLForge "
...
def test_MLForge_artifact_list_in_artifacts_only_mode(tmp_path: Path):
    port = get_safe_port()
    env = {**os.environ}
    env.pop(MLForge_TRACE_ARCHIVAL_CONFIG.name, None)
    with subprocess.Popen(
        ...
        try:
            _await_server_up_or_die(port)
            resp = requests.get(f"http://localhost:{port}/api/2.0/MLForge-artifacts/artifacts")
            augmented_raise_for_status(resp)
...
def test_MLForge_artifacts_only_with_workspaces(tmp_path: Path):
    port = get_safe_port()
    ...
    env = {**os.environ}
    for name in [
        MLForge_ENABLE_WORKSPACES.name,
        MLForge_TRACE_ARCHIVAL_CONFIG.name,
        MLForge_WORKSPACE_STORE_URI.name,
    ]:
        env.pop(name, None)
    ...
    with subprocess.Popen(
        ...
        try:
            _await_server_up_or_die(port)
            artifact_url = (
                f"http://localhost:{port}/api/2.0/MLForge-artifacts/artifacts/run/model.txt"
            )
            workspace_header = {"X-MLForge-WORKSPACE": "team-a"}
            ...
            default_artifact_url = (
                f"http://localhost:{port}/api/2.0/MLForge-artifacts/artifacts/default.txt"
            )
            ...
            unknown_workspace_response = requests.get(
                artifact_url,
                headers={"X-MLForge-WORKSPACE": "missing"},
            )
            assert unknown_workspace_response.status_code == 404

            tracking_response = requests.get(
                f"http://localhost:{port}/api/2.0/MLForge/experiments/search",
                headers=workspace_header,
            )
            assert tracking_response.status_code == 503
            assert "artifacts-only" in tracking_response.text
            assert not (tmp_path / "MLForge.db").exists()
        finally:
            kill_process_tree(process.pid)


def test_MLForge_artifact_service_unavailable_when_no_server_artifacts_is_specified():
    port = get_safe_port()
    with subprocess.Popen([
        sys.executable,
        "-m",
        ...
        try:
            _await_server_up_or_die(port)
            endpoint = "/api/2.0/MLForge-artifacts/artifacts"
            resp = requests.get(f"http://localhost:{port}{endpoint}")
            assert (
                f"Endpoint: {endpoint} disabled due to the MLForge server running with "
...
def test_MLForge_artifact_only_prints_warning_for_configs():
    with (
        mock.patch("MLForge.server._run_server") as run_server_mock,
        mock.patch("MLForge.store.tracking.sqlalchemy_store.SqlAlchemyStore"),
        mock.patch("MLForge.store.model_registry.sqlalchemy_store.SqlAlchemyStore"),
...
def test_MLForge_ui_is_alias_for_MLForge_server():
    MLForge_ui_stdout = subprocess.check_output(
        [sys.executable, "-m", "MLForge", "ui", "--help"], text=True
    )
    MLForge_server_stdout = subprocess.check_output(
        [sys.executable, "-m", "MLForge", "server", "--help"], text=True
    )
    assert (
        MLForge_ui_stdout.replace("Usage: python -m MLForge ui", "Usage: python -m MLForge server")
        == MLForge_server_stdout
    )


def test_server_help_has_quickstart():
    out = subprocess.check_output([sys.executable, "-m", "MLForge", "server", "--help"], text=True)
    assert "Pick your setup" in out
    assert "Options by topic:" in out
    assert "sqlite:///MLForge.db" in out
...
def test_cli_with_python_mod():
    ...
    assert stdout.rstrip().endswith(MLForge.__version__)
    stdout = subprocess.check_output(
        [
            sys.executable,
            "-m",
    ...
    assert "MLForge server" in stdout
...
def test_env_file_loading(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    test_tracking_uri = f"sqlite:///{tmp_path / 'MLForge.db'}"
    test_experiment_name = "test_experiment_from_env"

    # Create experiment using SDK
    MLForge.set_tracking_uri(test_tracking_uri)
    MLForge.create_experiment(test_experiment_name)

    # Create a test .env file pointing to this tracking URI
    env_file_path = tmp_path / "test.env"
    env_file_path.write_text(f"MLForge_TRACKING_URI={test_tracking_uri}\n")

    runner = CliRunner()

    # Clear the MLForge environment variables to ensure we're testing the loading
    monkeypatch.delenv("MLForge_TRACKING_URI", raising=False)
    monkeypatch.delenv("MLForge_EXPERIMENT_NAME", raising=False)

    # Ensure variables are not set before running command
    assert "MLForge_TRACKING_URI" not in os.environ
...
def test_experiments_search_max_results(args, expected_max_results, expected_view):
    with mock.patch("MLForge.search_experiments", return_value=[]) as mock_search:
...
def test_MLForge_gc_with_datasets(sqlite_store):
    store = sqlite_store[0]

    MLForge.set_tracking_uri(sqlite_store[1])
    MLForge.set_experiment("dataset")
    ...
    with pytest.raises(MLForgeException, match=f"No Experiment with id={experiment_id} exists"):
        store.get_experiment(experiment_id)


@pytest.mark.parametrize("get_store_details", ["file_store", "sqlite_store"])
def test_MLForge_gc_logged_model(get_store_details, request):
    if get_store_details == "file_store":
        pytest.skip("FileStore is no longer supported.")
    store, uri = request.getfixturevalue(get_store_details)
    ...
    subprocess.check_output([sys.executable, "-m", "MLForge", "gc", "--backend-store-uri", uri])
...
@pytest.mark.parametrize("get_store_details", ["file_store", "sqlite_store"])
def test_MLForge_gc_logged_models_older_than(get_store_details, request):
    if get_store_details == "file_store":
        pytest.skip("FileStore is no longer supported.")
    store, uri = request.getfixturevalue(get_store_details)
    exp_id = store.create_experiment("exp")
...
@pytest.mark.parametrize("get_store_details", ["file_store", "sqlite_store"])
def test_MLForge_gc_logged_models_deletes_when_older_than(get_store_details, request):
    if get_store_details == "file_store":
        pytest.skip("FileStore is no longer supported.")
    store, uri = request.getfixturevalue(get_store_details)
    exp_id = store.create_experiment("exp")
    ...
    with mock.patch("MLForge.utils.time.get_current_time_millis", return_value=old_deletion_ms):
        store.delete_logged_model(model.model_id)
...
@pytest.mark.parametrize("get_store_details", ["file_store", "sqlite_store"])
def test_MLForge_gc_logged_models_mixed_time(get_store_details, request):
    if get_store_details == "file_store":
        pytest.skip("FileStore is no longer supported.")
    store, uri = request.getfixturevalue(get_store_details)
    exp_id = store.create_experiment("exp")
    ...
    with mock.patch("MLForge.utils.time.get_current_time_millis") as current_time_mock:
...
def _set_job_creation_time(job_store, job_id, creation_time_ms):
    from MLForge.store.tracking.dbmodels.models import SqlJob
...
def test_MLForge_gc_no_jobs_deleted_without_jobs_flag(sqlite_store_with_jobs):
...
def test_MLForge_gc_all_jobs_deleted_with_jobs_flag(sqlite_store_with_jobs):
    _, job_store, db_uri = sqlite_store_with_jobs
    ...
    with pytest.raises(MLForgeException, match=r"Job .+ not found"):
...
def test_MLForge_gc_specific_jobs_deleted_with_job_ids(sqlite_store_with_jobs):
    _, job_store, db_uri = sqlite_store_with_jobs
...
def test_MLForge_gc_jobs_deleted_with_older_than_flag(sqlite_store_with_jobs):
    _, job_store, db_uri = sqlite_store_with_jobs
...
def test_MLForge_gc_job_ids_with_older_than_filter(sqlite_store_with_jobs):
    _, job_store, db_uri = sqlite_store_with_jobs
...
def test_MLForge_gc_only_deletes_finalized_jobs(sqlite_store_with_jobs):

...
from MLForge.tensorflow import load_checkpoint
from MLForge.tensorflow.autologging import _TensorBoard
from MLForge.tensorflow.callback import MLForgeCallback
from MLForge.tracking.fluent import _shut_down_async_logging
...
from MLForge.utils.process import _exec_cmd
...
def clear_fluent_autologging_import_hooks():
    """
    Clears import hooks for MLForge fluent autologging (`MLForge.autolog()`) between tests
    to ensure that interactions between fluent autologging and TensorFlow / tf.keras can
    be tested successfully
    """
    MLForge.utils.import_hooks._post_import_hooks.pop("tensorflow", None)
    MLForge.utils.import_hooks._post_import_hooks.pop("keras", None)
...
def clear_autologging_config():
    ...
    """
    AUTOLOGGING_INTEGRATIONS.pop(MLForge.tensorflow.FLAVOR_NAME, None)
...
def test_extra_tags_tensorflow_autolog(random_train_data, random_one_hot_labels):
    MLForge.tensorflow.autolog(extra_tags={"test_tag": "tf_autolog"})
    ...
    assert run.data.tags[MLForge.utils.MLForge_tags.MLForge_AUTOLOGGING] == "tensorflow"
...
def test_tf_keras_autolog_log_models_configuration(
    ...
    MLForge.tensorflow.autolog(log_models=log_models)
...
def test_tf_keras_autolog_log_datasets_configuration_with_numpy(
    ...
    MLForge.tensorflow.autolog(log_datasets=log_datasets, log_models=log_models)
    ...
    run_inputs = client.get_run(MLForge.last_active_run().info.run_id).inputs
    ...
    if log_models:
        if log_datasets:
            ...
            assert logged_model.source_run_id == MLForge.last_active_run().info.run_id
    else:
...
def test_tf_keras_autolog_log_datasets_configuration_with_tensor(
    ...
    MLForge.tensorflow.autolog(log_datasets=log_datasets)
    ...
    client = MLForgeClient()
    dataset_inputs = client.get_run(MLForge.last_active_run().info.run_id).inputs.dataset_inputs
    if log_datasets:
        assert len(dataset_inputs) == 1
...
def test_tf_keras_autolog_log_datasets_with_validation_data(
    ...
    MLForge.tensorflow.autolog(log_datasets=True)
    fashion_mnist_model = _create_fashion_mnist_model()
...
def test_tf_keras_autolog_persists_manually_created_run(random_train_data, random_one_hot_labels):
    ...
    with MLForge.start_run() as run:
        ...
        assert MLForge.active_run()
        assert MLForge.active_run().info.run_id == run.info.run_id
...
def test_tf_keras_autolog_implicit_batch_size_works(generate_data, batch_size):
    ...
    assert MLForge.last_active_run().data.params["batch_size"] == str(batch_size)
...
def test_tf_keras_autolog_model_can_load_from_artifact(tf_keras_random_data_run, random_train_data):
    ...
    model = MLForge.tensorflow.load_model("runs:/" + run.info.run_id + "/model")
...
def test_tf_keras_autolog_early_stop_logs(
    ...
    # Check that MLForge has logged the metrics of the "best" model, in addition to per-epoch metrics
    loss = history.history["loss"]
    assert len(metric_history) == len(loss) + 1
    steps, values = map(list, zip(*[(m.step, m.value) for m in metric_history]))
    # Check that MLForge has logged the correct steps
    assert steps == [*history.epoch, callback.stopped_epoch + 1]
    # Check that MLForge has logged the correct metric values
...
def test_tf_keras_autolog_logs_to_and_deletes_temporary_directory_when_tensorboard_callback_absent(
    ...
    from MLForge.tensorflow import _TensorBoardLogDir
    ...
    with patch("MLForge.tensorflow._TensorBoardLogDir", autospec=True) as mock_log_dir_class:
...
def get_text_vec_model(train_samples):
    # Taken from: https://github.com/MLForge/MLForge/issues/3910
...
def test_autolog_text_vec_model(tmp_path):
    ...
    loaded_model = MLForge.tensorflow.load_model("runs:/" + run.info.run_id + "/model")
...
def test_tf_keras_model_autolog_registering_model(random_train_data, random_one_hot_labels):
    ...
    MLForge.tensorflow.autolog(registered_model_name=registered_model_name)
    with MLForge.start_run():
        model = create_tf_keras_model()
...
def test_fluent_autolog_with_tf_keras_logs_expected_content(
    ...
    """
    Guards against previously-exhibited issues where using the fluent `MLForge.autolog()` API with
    `tf.keras` Models did not work due to conflicting patches set by both the
    `MLForge.tensorflow.autolog()` and the `MLForge.keras.autolog()` APIs.
    """
    MLForge.autolog()
...
def test_import_tensorflow_with_fluent_autolog_enables_tensorflow_autologging():
    ...
    assert not autologging_is_disabled(MLForge.tensorflow.FLAVOR_NAME)
...
def _assert_keras_autolog_input_example_load_and_predict_with_nparray(random_train_data):
    ...
    pyfunc_model = MLForge.pyfunc.load_model(logged_model.model_uri)
...
def test_keras_autolog_input_example_load_and_predict_with_nparray(
    ...
    MLForge.tensorflow.autolog(log_input_examples=True, log_model_signatures=True)
...
def test_keras_autolog_infers_model_signature_correctly_with_nparray(
    ...
    MLForge.tensorflow.autolog(log_model_signatures=True)
    initial_model = create_tf_keras_model()
    with MLForge.start_run():
        initial_model.fit(random_train_data, random_one_hot_labels)
...
def test_keras_autolog_load_saved_hdf5_model(keras_data_gen_sequence):
    MLForge.tensorflow.autolog(keras_model_kwargs={"save_format": "h5"})
    model = create_tf_keras_model()
    with MLForge.start_run():
        model.fit(keras_data_gen_sequence)
        logged_model = MLForge.last_logged_model()
...
def test_extract_tf_keras_input_example_unsupported_type_returns_None():
    from MLForge.tensorflow.autologging import extract_tf_keras_input_example
...
def test_import_keras_model_trigger_import_tensorflow():
    ...
    # `MLForge.tensorflow.autolog`, suppose user enable autolog by `MLForge.autolog()`,
...
def test_autolog_throw_error_on_explicit_MLForge_callback(keras_data_gen_sequence):
    MLForge.tensorflow.autolog()

    model = create_tf_keras_model()
    with MLForge.start_run() as run:
        with pytest.raises(MLForgeException, match="MLForge autologging must be turned off*"):
            model.fit(keras_data_gen_sequence, callbacks=[MLForgeCallback(run)])
...
def test_autolog_correct_logging_frequency(random_train_data, random_one_hot_labels):
    ...
    num_epochs = 2
    batch_size = 10
    MLForge.tensorflow.autolog(log_every_epoch=False, log_every_n_steps=logging_freq)
    initial_model = create_tf_keras_model()
    with MLForge.start_run() as run:
        initial_model.fit(
            random_train_data,
...
def test_automatic_checkpoint_per_epoch_callback(random_train_data, random_one_hot_labels):
    ...
    logged_metrics = MLForge.artifacts.load_dict(

...
def test_tf_MLForge_callback(log_every_epoch, log_every_n_steps):
    ...
    with MLForge.start_run() as run:
        MLForge_callback = MLForgeCallback(
            run=run,
        ...
        model.fit(
            ...
            epochs=5,
            callbacks=[MLForge_callback],
        )

    client = MLForge.MLForgeClient()
    MLForge_run = client.get_run(run.info.run_id)
    run_metrics = MLForge_run.data.metrics
    model_info = MLForge_run.data.params
...
def test_old_callback_still_exists():
    assert MLForge.tensorflow.MLForgeCallback is MLForge.tensorflow.MLForgeCallback

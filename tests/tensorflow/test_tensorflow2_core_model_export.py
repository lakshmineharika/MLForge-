...
def test_save_and_load_tf2_module(tmp_path, tf2_toy_model):
    ...
    MLForge.tensorflow.save_model(tf2_toy_model.model, model_path)

    loaded_model = MLForge.tensorflow.load_model(model_path)
...
def test_log_and_load_tf2_module(tf2_toy_model):
    with MLForge.start_run():
        model_info = MLForge.tensorflow.log_model(tf2_toy_model.model, name="model")

    model_uri = model_info.model_uri
    loaded_model = MLForge.tensorflow.load_model(model_uri)
    predictions = loaded_model(tf2_toy_model.inference_data).numpy()
    np.testing.assert_allclose(
        predictions,
        tf2_toy_model.expected_results,
    ...
    loaded_model2 = MLForge.pyfunc.load_model(model_uri)
...
def test_model_log_with_signature_inference(tf2_toy_model):
    ...
    assert MLForge_model.signature == infer_signature(
...
def test_load_with_options(tmp_path, tf2_toy_model):
    ...
    with mock.patch("tensorflow.saved_model.load") as mock_load:
        MLForge.tensorflow.load_model(model_path, saved_model_kwargs=saved_model_kwargs)

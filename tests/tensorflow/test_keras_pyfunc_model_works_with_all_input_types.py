...
from MLForge.pyfunc import spark_udf
...
def test_model_single_tensor_input(use_signature, single_tensor_input_model, model_path, data):
    ...
    MLForge.tensorflow.save_model(model, path=model_path, signature=signature)

    # Loading Keras model via PyFunc
    model_loaded = MLForge.pyfunc.load_model(model_path)
...
def test_single_multidim_input_model_spark_udf(
    ...
    with MLForge.start_run():
        model_uri = MLForge.tensorflow.log_model(model, name="model", signature=signature).model_uri
...
def test_scoring_server_successfully_on_single_multidim_input_model(
    ...
    test_input = np.repeat(x.values[:, :, np.newaxis], 3, axis=2)
    with MLForge.start_run():
        model_info = MLForge.tensorflow.log_model(model, name="model", input_example=test_input)
...
def test_scoring_server_successfully_on_multi_multidim_input_model(
    ...
    with MLForge.start_run():
        model_info = MLForge.tensorflow.log_model(model, name="model", input_example=input_example)
    assert model_info.signature.inputs == signature.inputs

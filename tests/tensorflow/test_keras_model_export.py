...
from MLForge.types.schema import Schema, TensorSpec
...
def keras_custom_env(tmp_path):
    ...
    _MLForge_conda_env(conda_env, additional_pip_deps=["keras", "tensorflow", "pytest"])
    return conda_env
...
def test_model_save_load(build_model, save_format, model_path, data):
    ...
    MLForge.tensorflow.save_model(keras_model, path=model_path, keras_model_kwargs=kwargs)
    # Loading Keras model
    model_loaded = MLForge.tensorflow.load_model(model_path)
...
def test_pyfunc_serve_and_score(data):
    ...
    with MLForge.start_run():
        model_info = MLForge.tensorflow.log_model(model, name="model", input_example=x)
...
def test_score_model_as_spark_udf(data):
    ...
    model = get_model(data)
    with MLForge.start_run():
        model_info = MLForge.tensorflow.log_model(model, name="model")
    expected = model.predict(x)
        ...
        for example in (None, example_):
            with TempDir() as tmp:
                ...
                if signature is None and example is None:
                    ...
                    assert MLForge_model.signature == signature_
                if example is None:
...
def test_custom_model_save_load(custom_model, custom_layer, data, custom_predicted, model_path):
    ...
    MLForge.tensorflow.save_model(custom_model, path=model_path, custom_objects=custom_objects)
    # Loading Keras model
    model_loaded = MLForge.tensorflow.load_model(model_path)
...
def test_custom_model_save_respects_user_custom_objects(custom_model, custom_layer, model_path):
    ...
    model_loaded = MLForge.tensorflow.load_model(
    ...
    if Version(tf.__version__) <= Version("2.11.0") or Version(tf.__version__).release >= (2, 16):
        with pytest.raises(TypeError, match=r".+"):
            MLForge.tensorflow.load_model(model_path)
    else:
        ...
        incorrect_loaded = MLForge.tensorflow.load_model(model_path)
...
def test_load_model_with_custom_objects_disallows_pickle_deserialization(
    ...
    # Passing any non-None custom_objects causes MLForge to cloudpickle them at save time.
    # Loading must then fail when pickle deserialization is disabled.
    MLForge.tensorflow.save_model(model, path=model_path, custom_objects={"dummy": object()})
...
def test_model_load_from_remote_uri_succeeds(model, model_path, mock_s3_bucket, data, predicted):
    x, _ = data
    MLForge.tensorflow.save_model(model, path=model_path)
    ...
    model_loaded = MLForge.tensorflow.load_model(model_uri=model_uri)
...
def test_model_log(model, data, predicted):
    ...
    for should_start_run in [False, True]:
        try:
            ...
            model_info = MLForge.tensorflow.log_model(model, name=artifact_path)
            # Load model
            model_loaded = MLForge.tensorflow.load_model(model_uri=model_info.model_uri)
            assert all(model_loaded.predict(x) == predicted)
            # Loading pyfunc model
...
def test_log_model_calls_register_model(model):
    ...
    with MLForge.start_run(), register_model_patch:
        model_info = MLForge.tensorflow.log_model(
...
def test_log_model_no_registered_model_name(model):
    ...
    register_model_patch = mock.patch("MLForge.tracking._model_registry.fluent._register_model")
    with MLForge.start_run(), register_model_patch:
        MLForge.tensorflow.log_model(model, name=artifact_path)
...
def test_model_save_persists_specified_conda_env_in_MLForge_model_directory(
    ...
    MLForge.tensorflow.save_model(model, path=model_path, conda_env=keras_custom_env)
...
def test_model_save_accepts_conda_env_as_dict(model, model_path):
    conda_env = dict(MLForge.tensorflow.get_default_conda_env())
    conda_env["dependencies"].append("pytest")
    MLForge.tensorflow.save_model(model, path=model_path, conda_env=conda_env)
...
def test_log_model_with_extra_pip_requirements(model, tmp_path):
    expected_MLForge_version = _MLForge_major_version_string()
    default_reqs = MLForge.tensorflow.get_default_pip_requirements()
    # Path to a requirements file
    req_file = tmp_path.joinpath("requirements.txt")
    req_file.write_text("a")
    with MLForge.start_run():
...
def test_model_log_persists_requirements_in_MLForge_model_directory(model, keras_custom_env):
    artifact_path = "model"
    with MLForge.start_run():
        model_info = MLForge.tensorflow.log_model(
...
def test_model_log_persists_specified_conda_env_in_MLForge_model_directory(model, keras_custom_env):
    artifact_path = "model"
    with MLForge.start_run():
        model_info = MLForge.tensorflow.log_model(
            model, name=artifact_path, conda_env=keras_custom_env
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    MLForge.tensorflow.save_model(model, path=model_path)
    _assert_pip_requirements(model_path, MLForge.tensorflow.get_default_pip_requirements())


def test_model_log_without_specified_conda_env_uses_default_env_with_expected_dependencies(model):
    with MLForge.start_run():
        model_info = MLForge.tensorflow.log_model(model, name="model")
    _assert_pip_requirements(model_info.model_uri, MLForge.tensorflow.get_default_pip_requirements())
...
def test_model_load_succeeds_with_missing_data_key_when_data_exists_at_default_path(
    ...
    """
    This is a backwards compatibility test to ensure that models saved in MLForge version <= 0.8.0
    ...
    flavor_conf = model_conf.flavors.get(MLForge.tensorflow.FLAVOR_NAME, None)
...
def test_load_without_save_format(tf_keras_model, model_path, data):
    ...
    model_conf_path = os.path.join(model_path, "MLmodel")
    model_conf = Model.load(model_conf_path)
    flavor_conf = model_conf.flavors.get(MLForge.tensorflow.FLAVOR_NAME)
    assert flavor_conf is not None
...
def test_log_model_with_code_paths(model):
    ...
    with (
        MLForge.start_run(),
        mock.patch("MLForge.tensorflow._add_code_from_conf_to_system_path") as add_mock,
    ):
        model_info = MLForge.tensorflow.log_model(model, name=artifact_path, code_paths=[__file__])
        _compare_logged_code_paths(__file__, model_info.model_uri, MLForge.tensorflow.FLAVOR_NAME)
        MLForge.tensorflow.load_model(model_info.model_uri)
...
def test_load_tf_keras_model_with_options(tf_keras_model, model_path):
    MLForge.tensorflow.save_model(tf_keras_model, path=model_path)
    keras_model_kwargs = {
        "compile": False,
        "options": tf.saved_model.LoadOptions(),
    }
    with mock.patch("MLForge.tensorflow._load_keras_model") as mock_load:
        MLForge.tensorflow.load_model(model_path, keras_model_kwargs=keras_model_kwargs)
...
def test_model_log_with_signature_inference(tf_keras_model, data, model_signature):
    ...
    assert MLForge_model.signature == model_signature

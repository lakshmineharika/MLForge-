...
This directory contains files to test MLForge tracking operations using the following databases:
...
# Run all services
for service in $(./tests/db/compose.sh config --services | grep '^MLForge-')
do
  ./tests/db/compose.sh run --rm "$service"
done

...
import MLForge
from MLForge.entities import SpanLogLevel
from MLForge.entities.span import Span, SpanType
from MLForge.entities.span_event import SpanEvent
from MLForge.exceptions import MLForgeException
from MLForge.tracing.constant import SpanAttributeKey
from MLForge.tracing.utils.default_log_level import default_log_level_for_span_type
...
def test_from_value_rejects_invalid_string(value):
    ...
    with pytest.raises(MLForgeException, match="Invalid SpanLogLevel"):
...
def test_from_value_rejects_int(value):
    ...
    with pytest.raises(MLForgeException, match="must be"):
        SpanLogLevel.from_value(value)
...
def test_log_level_constructor_default_for_info_span_type():
    ...
    with MLForge.start_span("s", span_type=SpanType.CHAT_MODEL):
        pass
    persisted = get_traces()[0].data.spans[0]
...
def test_set_log_level_normalizes_input(set_value, expected):
    with MLForge.start_span("s") as span:
...
def test_set_log_level_rejects_invalid_input():
    with (
        MLForge.start_span("s") as span,
        pytest.raises(MLForgeException, match="Invalid SpanLogLevel"),
    ):
...
def test_start_span_kwarg_overrides_constructor_default():
    ...
    with MLForge.start_span("s", span_type=SpanType.CHAT_MODEL, log_level="WARNING"):
        pass
    persisted = get_traces()[0].data.spans[0]
...
def test_start_span_no_context_kwarg():
    span = MLForge.start_span_no_context("s", log_level=SpanLogLevel.ERROR)
    span.end()
    persisted = get_traces()[0].data.spans[0]
...
async def test_trace_decorator_kwarg_async():
    @MLForge.trace(log_level=SpanLogLevel.INFO)
    async def fn(x):
        return x + 1
...
def test_trace_decorator_kwarg_generator():
    @MLForge.trace(log_level="WARNING")
    def gen():
        yield 1
        yield 2
...
def test_log_level_round_trips_through_to_dict_from_dict():
    with MLForge.start_span("s", log_level=SpanLogLevel.ERROR):
        pass
    persisted = get_traces()[0].data.spans[0]
...
def test_constructor_stamps_default_for_info_span_type():
    span = MLForge.start_span_no_context("s", span_type=SpanType.CHAT_MODEL)
    span.end()
    persisted = get_traces()[0].data.spans[0]
    assert persisted.log_level is SpanLogLevel.INFO
...
def test_constructor_stamps_default_for_debug_span_type():
    span = MLForge.start_span_no_context("s", span_type=SpanType.PARSER)
    span.end()
    persisted = get_traces()[0].data.spans[0]
    assert persisted.log_level is SpanLogLevel.DEBUG
...
def test_multi_span_trace_carries_per_span_levels():
    @MLForge.trace(log_level="DEBUG", name="inner")
    def inner():
        return 1

    @MLForge.trace(log_level="WARNING", name="root")
    def root():
        return inner()
...
def test_exception_event_bumps_debug_span_to_error():
    ...
    with MLForge.start_span("s", span_type=SpanType.PARSER) as span:
...
def test_exception_event_bumps_info_span_to_error():
    with MLForge.start_span("s", span_type=SpanType.CHAT_MODEL) as span:
...
def test_exception_event_does_not_demote_critical():
    ...
    with MLForge.start_span("s", log_level=SpanLogLevel.CRITICAL) as span:
        span.add_event(SpanEvent.from_exception(RuntimeError("boom")))
    persisted = get_traces()[0].data.spans[0]
...
def test_traced_function_that_raises_is_promoted_to_error():
    # A plain @MLForge.trace function that throws records an exception event via

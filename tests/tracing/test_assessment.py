...
from MLForge.entities.issue import IssueStatus
...
def test_log_expectation(trace_id, legacy_api):
    ...
    assert assessment.value == "MLForge"
    assert assessment.trace_id == trace_id
    ...
    assert assessment.expectation.value == "MLForge"
    assert assessment.rationale is None
    assert assessment.metadata == {"key": "value"}


def test_log_expectation_invalid_parameters():
    with pytest.raises(MLForgeException, match=r"The `value` field must be specified."):
        Expectation(
            name="expected_answer",
            value=None,
            source=_HUMAN_ASSESSMENT_SOURCE,
...
def test_update_expectation(trace_id):
    assessment_id = MLForge.log_expectation(
        trace_id=trace_id,
        name="expected_answer",
        value="MLForge",
    ).assessment_id
...
def test_log_feedback_invalid_parameters():
    ...
    with pytest.raises(MLForgeException, match=r"`source` must be an instance of"):
        Feedback(
            trace_id="1234",
            name="faithfulness",
            value=1.0,
...
def test_update_feedback(trace_id):
    assessment_id = MLForge.log_feedback(
        trace_id=trace_id,
        name="faithfulness",
        value=1.0,
        rationale="This answer is very faithful.",
...
def test_override_feedback(trace_id):
    ...
    new_assessment_id = MLForge.override_feedback(
        trace_id=trace_id,
        assessment_id=assessment_id,
        value=1.0,
        source=_LLM_ASSESSMENT_SOURCE,
    ...
    assessment = MLForge.get_assessment(trace_id, new_assessment_id)
    assert assessment.trace_id == trace_id
    assert assessment.name == "faithfulness"
    assert assessment.span_id is None
    assert assessment.source == _LLM_ASSESSMENT_SOURCE
    ...
    original_assessment = MLForge.get_assessment(trace_id, assessment_id)
...
def test_delete_assessment(trace_id):
    ...
    MLForge.delete_assessment(trace_id=trace_id, assessment_id=assessment_id)

    with pytest.raises(MLForgeException, match=r"Assessment with ID"):
        assert MLForge.get_assessment(trace_id, assessment_id) is None
...
def test_log_issue(trace_id, tracking_uri):
    ...
    tracing_client = MLForge.MLForgeClient()._tracing_client
...
def test_log_issue_with_invalid_issue_name(trace_id, tracking_uri):
    ...
    with pytest.raises(
        MLForgeException, match=r"Provided issue name 'wrong_name' does not match the issue name"
    ):
        MLForge.log_issue(
            trace_id=trace_id,
            issue_id=issue.issue_id,
...
def test_log_feedback_and_exception_blocks_positional_args():
    with pytest.raises(TypeError, match=r"log_feedback\(\) takes 0 positional"):
        MLForge.log_feedback("tr-1234", "faithfulness", 1.0)

    with pytest.raises(TypeError, match=r"log_expectation\(\) takes 0 positional"):
        MLForge.log_expectation("tr-1234", "expected_answer", "MLForge")

    with pytest.raises(TypeError, match=r"log_issue\(\) takes 0 positional"):
        MLForge.log_issue("tr-1234", "iss-12345", "timeout_error")
...
def test_log_assessment_on_in_progress_trace(trace_id, legacy_api):
    ...
    def func(x: int, y: int) -> int:
        active_trace_id = MLForge.get_active_trace_id()
        if legacy_api:
            MLForge.log_assessment(active_trace_id, Feedback(name="feedback", value=1.0))
            MLForge.log_assessment(active_trace_id, Expectation(name="expectation", value="MLForge"))
            MLForge.log_assessment(trace_id, Feedback(name="other", value=2.0))
        else:
            MLForge.log_feedback(trace_id=active_trace_id, name="feedback", value=1.0)
            MLForge.log_expectation(trace_id=active_trace_id, name="expectation", value="MLForge")
            MLForge.log_feedback(trace_id=trace_id, name="other", value=2.0)
        return x + y
    ...
    assert assessments["expectation"].value == "MLForge"
...
async def test_log_assessment_on_in_progress_trace_async():
    ...
    async def func(x: int, y: int) -> int:
        trace_id = MLForge.get_active_trace_id()
        MLForge.log_assessment(trace_id, Feedback(name="feedback", value=1.0))
        MLForge.log_assessment(trace_id, Expectation(name="expectation", value="MLForge"))
        return x + y
    ...
    assert assessments["expectation"].expectation.value == "MLForge"


def test_log_assessment_on_in_progress_with_span_id():
    with MLForge.start_span(name="test_span") as span:
        # Only proceed if we have a real span (not NO_OP)
        if span.span_id is not None and span.trace_id != "MLForge_NO_OP_SPAN_TRACE_ID":
            MLForge.log_assessment(
                trace_id=span.trace_id,
...
def test_log_assessment_on_in_progress_trace_works_when_tracing_is_disabled():
    ...
    def func(x: int, y: int):
        trace_id = MLForge.get_active_trace_id()
        MLForge.log_assessment(trace_id=trace_id, assessment=Feedback(name="feedback", value=1.0))
        return x + y
...
def test_get_assessment(trace_id):
    ...
    result = MLForge.get_assessment(trace_id, assessment_id)
...
def test_search_traces_with_assessments():
    ...
    with MLForge.start_span(name="trace_1") as span_1:
        MLForge.log_feedback(
        ...
        with MLForge.start_span(name="child") as span_1_child:
            MLForge.log_feedback(
    ...
    with MLForge.start_span(name="trace_2") as span_2:
        MLForge.log_feedback(
...
def test_log_issue_reference(trace_id, tracking_uri):
    ...
    MLForge.log_assessment(trace_id=trace_id, assessment=issue_ref)
...
def test_log_issue_reference_invalid_parameters():
    with pytest.raises(MLForgeException, match=r"The `issue_id` field must be specified"):
        IssueReference(
            issue_id=None,
...
def test_get_issue_reference_assessment(trace_id):
    ...
    assessment_id = MLForge.log_assessment(trace_id=trace_id, assessment=issue_ref).assessment_id
...
def test_log_multiple_assessment_types(trace_id):
    ...
    assert assessments_by_type["expectation"].value == "MLForge"
...
def test_log_feedback_in_distributed_trace_same_process():
    ...
    MLForge.flush_trace_async_logging()
    trace = MLForge.get_trace(root_trace_id)
    assert len(trace.info.assessments) == 1
...
def test_log_assessment_active_trace_not_in_memory():
    ...
    with MLForge.start_span(name="root"):
        with (
            mock.patch.object(

...
from MLForge.tracing.constant import GenAiSemconvKey, SpanAttributeKey
from MLForge.tracing.export.genai_semconv.translator import (
...
def test_translate_tool_span_with_inputs_outputs():
    tool_input = {"query": "what is MLForge?"}
    tool_output = {"result": "MLForge is a platform..."}
    span = _make_span(
        attributes={
...
def test_full_chat_span():
    ...
    assert not any(k.startswith("MLForge.") for k in result.attributes)
...
def test_non_MLForge_attributes_preserved():
    attrs = {
        SpanAttributeKey.SPAN_TYPE: json.dumps("CHAT_MODEL"),
        "http.method": "POST",

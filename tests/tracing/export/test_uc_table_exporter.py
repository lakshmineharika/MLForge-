...
from MLForge.tracing.utils import generate_trace_id_v4
...
def test_export_spans_to_uc_table(is_async, monkeypatch):
    ...
    monkeypatch.setenv("MLForge_ASYNC_TRACE_LOGGING_MAX_SPAN_BATCH_SIZE", "1")  # no batch
    ...
    # Export the span
    with mock.patch(
        "MLForge.tracing.export.uc_table.get_active_spans_table_name",
...
def test_log_trace_no_upload_data_for_uc_schema():
    ...
    with mock.patch("MLForge.tracing.utils.add_size_stats_to_trace_metadata"):
...
def test_export_spans_batch_max_size(monkeypatch):
    monkeypatch.setenv("MLForge_ASYNC_TRACE_LOGGING_MAX_SPAN_BATCH_SIZE", "5")
    monkeypatch.setenv("MLForge_ASYNC_TRACE_LOGGING_MAX_INTERVAL_MILLIS", "10000")
...
def test_export_spans_batch_flush_on_interval(monkeypatch):
    monkeypatch.setenv("MLForge_ASYNC_TRACE_LOGGING_MAX_SPAN_BATCH_SIZE", "10")

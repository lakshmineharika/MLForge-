...
import MLForge
from MLForge.entities import LiveSpan, Trace
from MLForge.entities.model_registry import PromptVersion
from MLForge.entities.trace_info import TraceInfo
from MLForge.tracing.constant import TraceMetadataKey, TraceSizeStatsKey
...
def test_export(experiment_id, monkeypatch):
    if experiment_id:
        monkeypatch.setenv("MLForge_EXPERIMENT_ID", experiment_id)
    ...
    with mock.patch(
        "MLForge.tracing.export.inference_table.TracingClient", return_value=mock_tracing_client
    ):
    ...
    assert MLForge.get_last_active_trace_id() == trace_id
    ...
    if experiment_id:
        ...
        # The trace ID should be updated to the format that MLForge backend accept
        assert trace_info.trace_id == trace_id
        # The databricks request ID should be set to the client request ID
        assert trace_info.client_request_id == _DATABRICKS_REQUEST_ID_1
    else:
        # When experiment_id is not set, trace should not be exported to MLForge backend
...
def test_export_warn_invalid_attributes():
    ...
    # NB: `MLForge.spanLogLevel` is intentionally absent — this test exports the
    ...
    assert stored_span.attributes == {
        "MLForge.traceRequestId": trace_id,
...
def test_export_trace_buffer_not_exceeds_max_size(monkeypatch):
    monkeypatch.setenv("MLForge_TRACE_BUFFER_MAX_SIZE", "1")
    monkeypatch.setattr(
        MLForge.tracing.export.inference_table, "_TRACE_BUFFER", _initialize_trace_buffer()
    )
...
def test_size_bytes_in_trace_sent_to_MLForge_backend(monkeypatch):
    """
    Test that SIZE_BYTES is correctly set in the trace sent
    to MLForge backend when experiment ID is set.
    """
    # Set experiment ID to enable MLForge backend export
...
def test_prompt_linking_disabled_without_experiment_id(monkeypatch):
    # Don't set MLForge_EXPERIMENT_ID to disable MLForge backend export
...
def test_prompt_linking_error_handling_with_experiment_id(monkeypatch):
    ...
    with (
        mock.patch(
            "MLForge.tracing.export.inference_table.TracingClient",

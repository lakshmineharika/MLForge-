...
from MLForge.tracing.export.async_export_queue import AsyncTraceExportQueue, Task
    ...
    with mock.patch("atexit.register") as mock_atexit:
        ...
        def count_threads():
            ...
            return sum(
                ...
                if t is not main_thread and t.name.startswith("MLForgeTraceLogging")
            )
...
def test_async_queue_drop_task_when_full(monkeypatch):
    monkeypatch.setenv("MLForge_ASYNC_TRACE_LOGGING_MAX_QUEUE_SIZE", "3")
    monkeypatch.setenv("MLForge_ASYNC_TRACE_LOGGING_MAX_WORKERS", "1")

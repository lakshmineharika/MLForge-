...
def _make_exporter(mock_client):
    with patch.object(MLForgeV3SpanExporter, "__init__", return_value=None):
...
def test_log_trace_uploads_attachments():
    ...
    with (
        patch("MLForge.tracing.export.MLForge_v3.try_link_prompts_to_trace"),
        patch("MLForge.tracing.export.MLForge_v3.add_size_stats_to_trace_metadata"),
    ):

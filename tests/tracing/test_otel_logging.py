...
from MLForge.server.fastapi_app import app as MLForge_app
...
from MLForge.telemetry.events import TraceSource, TracesReceivedByServerEvent
...
if IS_TRACING_SDK_ONLY:
    pytest.skip("OTel endpoint tests require full MLForge server", allow_module_level=True)


@pytest.fixture
def MLForge_server(tmp_path: Path, db_uri: str) -> Iterator[str]:
    ...
    with ServerThread(MLForge_app, get_safe_port()) as url:
        yield url
...
def test_otel_client_sends_spans_to_MLForge_database(MLForge_server: str, monkeypatch):
    """
    Test end-to-end: OpenTelemetry client sends spans via experiment ID header to MLForge.
    ...
    monkeypatch.setenv("MLForge_ASYNC_TRACE_LOGGING", "false")

    MLForge.set_tracking_uri(MLForge_server)

    experiment = MLForge.set_experiment("otel-test-experiment")
    ...
    test_response = requests.get(f"{MLForge_server}/health", timeout=5)
    ...
    with tracer.start_as_current_span("otel-e2e-test-span") as span:
        ...
        # Capture the OTel trace ID to verify it matches the MLForge trace ID
...
def test_otel_endpoint_requires_experiment_id_header(MLForge_server: str):
    """
...
def test_invalid_otel_span_format_returns_400(MLForge_server: str):
    """
    ...
    response = requests.post(
        ...
        headers={
            ...
            MLForge_EXPERIMENT_ID_HEADER: "test-experiment",
        },
        timeout=10,
    )
...
def test_missing_required_span_fields_returns_422(MLForge_server: str):
    """
    Test that spans that fail MLForge conversion return HTTP 422.
    """
    # Create protobuf request with missing span_id (this should cause MLForge conversion to fail)
    span = OTelProtoSpan()
    ...
    response = requests.post(
        ...
        headers={
            "Content-Type": "application/x-protobuf",
            MLForge_EXPERIMENT_ID_HEADER: "0",
        },
        timeout=10,
    )
...
def test_missing_experiment_id_header_returns_422(MLForge_server: str):
    """
...
def test_invalid_content_type_returns_400(MLForge_server: str):
    """
...
def test_empty_resource_spans_returns_400(MLForge_server: str):
    request = ExportTraceServiceRequest()
...
def test_batch_span_processor_with_multiple_traces(MLForge_server: str):
    """
    ...
    experiment = MLForge.set_experiment("otel-batch-test-experiment")
    experiment_id = experiment.experiment_id
...
def test_multiple_traces_in_single_request(MLForge_server: str):
    """
    ...
    MLForge.set_tracking_uri(MLForge_server)
    experiment = MLForge.set_experiment("otel-multi-trace-test")
    experiment_id = experiment.experiment_id
    ...
    response = requests.post(
        ...
        headers={
            "Content-Type": "application/x-protobuf",
            MLForge_EXPERIMENT_ID_HEADER: experiment_id,
        },
        timeout=10,
    )
...
def test_logging_many_traces_in_single_request(MLForge_server: str):
    MLForge.set_tracking_uri(MLForge_server)
    experiment = MLForge.set_experiment("otel-many-traces-test")
    experiment_id = experiment.experiment_id
...
def test_mixed_trace_spans_in_single_request(MLForge_server: str):
    """
    Test that multiple spans from the same trace, mixed with spans from other traces,
    are grouped and logged correctly.
    """
    MLForge.set_tracking_uri(MLForge_server)
    experiment = MLForge.set_experiment("otel-mixed-test")
    experiment_id = experiment.experiment_id
    ...
    traces = MLForge.search_traces(locations=[experiment_id], include_spans=True, return_type="list")
...
def test_error_logging_spans(MLForge_server: str):
    MLForge.set_tracking_uri(MLForge_server)
    experiment = MLForge.set_experiment("otel-error-test")
    experiment_id = experiment.experiment_id
...
def test_otel_trace_received_telemetry_from_MLForge_client(MLForge_server: str):
    """
    Test TraceReceivedByServerEvent telemetry shows source=MLForge_PYTHON_CLIENT for standard client.

    Uses @MLForge.trace with standard MLForge client configuration, which automatically sends
    User-Agent and X-MLForge-Client-Version headers to identify traces from MLForge client.
    """
    MLForge.set_tracking_uri(MLForge_server)
    MLForge.set_experiment("otel-telemetry-MLForge-client-test")

    with mock.patch("MLForge.telemetry.track.get_telemetry_client") as mock_get_client:
        ...
        if mock_client.add_record.called:
            ...
            assert record.params["source"] == TraceSource.MLForge_PYTHON_CLIENT.value
            assert record.params["count"] == 1


def test_otel_trace_received_telemetry_from_external_client(MLForge_server: str):
    """
    Test TracesReceivedByServerEvent telemetry shows source=UNKNOWN for external clients.

    Sends a direct protobuf request without MLForge client headers to simulate an external
    OpenTelemetry client (not MLForge client). Tests with 2 traces to verify count field.
    """
    MLForge.set_tracking_uri(MLForge_server)
    experiment = MLForge.set_experiment("otel-telemetry-external-client-test")
    experiment_id = experiment.experiment_id
...
def test_otel_trace_received_telemetry_from_external_otel_client_with_service_name(
    ...
    MLForge.set_tracking_uri(MLForge_server)
    experiment = MLForge.set_experiment("otel-telemetry-service-name-test")
    experiment_id = experiment.experiment_id
...
def test_service_name_propagated_to_root_span(MLForge_server: str):
    MLForge.set_tracking_uri(MLForge_server)
    experiment = MLForge.set_experiment("otel-service-name-on-span-test")
    experiment_id = experiment.experiment_id
...
def test_otlp_json_encoding(MLForge_server: str):
    MLForge.set_tracking_uri(MLForge_server)
    experiment = MLForge.set_experiment("otel-json-encoding-test")
    experiment_id = experiment.experiment_id
...
def test_response_is_protobuf_format(MLForge_server: str):
    MLForge.set_tracking_uri(MLForge_server)
    experiment = MLForge.set_experiment("otel-protobuf-response-test")
    experiment_id = experiment.experiment_id
...
def test_otlp_traces_no_compression(MLForge_server: str):
    MLForge.set_tracking_uri(MLForge_server)
    experiment = MLForge.set_experiment("otel-identity-test")
    experiment_id = experiment.experiment_id
...
def test_otlp_traces_gzip_compression(MLForge_server: str):
    MLForge.set_tracking_uri(MLForge_server)
    experiment = MLForge.set_experiment("otel-gzip-test")
    experiment_id = experiment.experiment_id
...
def test_otlp_traces_deflate_compression(MLForge_server: str):
    MLForge.set_tracking_uri(MLForge_server)
    experiment = MLForge.set_experiment("otel-deflate-test")
    experiment_id = experiment.experiment_id

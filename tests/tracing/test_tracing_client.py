...
from MLForge.environment_variables import (
    MLForge_GET_TRACE_OTEL_INITIAL_RETRY_INTERVAL_SECONDS,
    MLForge_GET_TRACE_OTEL_MAX_RETRY_INTERVAL_SECONDS,
    MLForge_GET_TRACE_OTEL_RETRY_TIMEOUT_SECONDS,
    MLForge_TRACING_SQL_WAREHOUSE_ID,
)
from MLForge.exceptions import MLForgeException, MLForgeNotImplementedException
...
def test_get_trace_v4():
    ...
    with patch("MLForge.tracing.client._get_store", return_value=mock_store):
...
def test_get_trace_v4_retry():
    ...
    with (
        patch("MLForge.tracing.client._get_store", return_value=mock_store),
        patch("MLForge.tracing.client.time.sleep"),
    ):
        client = TracingClient()
        trace = client.get_trace("trace:/catalog.schema/1234567890")
...
def test_get_trace_v4_retry_timeout_exhausted(monkeypatch):
    monkeypatch.setenv(MLForge_GET_TRACE_OTEL_RETRY_TIMEOUT_SECONDS.name, "0")
    mock_store = Mock()
    mock_store.batch_get_traces.return_value = []

    with patch("MLForge.tracing.client._get_store", return_value=mock_store):
        client = TracingClient()
        with pytest.raises(MLForgeException, match="is not found"):
...
def test_get_trace_v4_retry_uses_initial_interval(monkeypatch):
    monkeypatch.setenv(MLForge_GET_TRACE_OTEL_INITIAL_RETRY_INTERVAL_SECONDS.name, "0.25")
    mock_store = Mock()
    mock_store.batch_get_traces.side_effect = [[], ["dummy_trace"]]

    with (
        patch("MLForge.tracing.client._get_store", return_value=mock_store),
        patch("MLForge.tracing.client.time.sleep") as mock_sleep,
    ):
        client = TracingClient()
...
def test_get_trace_v4_retry_caps_max_interval(monkeypatch):
    monkeypatch.setenv(MLForge_GET_TRACE_OTEL_RETRY_TIMEOUT_SECONDS.name, "60")
    monkeypatch.setenv(MLForge_GET_TRACE_OTEL_INITIAL_RETRY_INTERVAL_SECONDS.name, "1")
    monkeypatch.setenv(MLForge_GET_TRACE_OTEL_MAX_RETRY_INTERVAL_SECONDS.name, "3")
    mock_store = Mock()
    mock_store.batch_get_traces.side_effect = [[], [], [], [], ["dummy_trace"]]
...
def test_get_trace_v4_retry_clamps_sleep_to_deadline(monkeypatch):
    monkeypatch.setenv(MLForge_GET_TRACE_OTEL_RETRY_TIMEOUT_SECONDS.name, "5")
    monkeypatch.setenv(MLForge_GET_TRACE_OTEL_INITIAL_RETRY_INTERVAL_SECONDS.name, "10")
    monkeypatch.setenv(MLForge_GET_TRACE_OTEL_MAX_RETRY_INTERVAL_SECONDS.name, "20")
    mock_store = Mock()
    mock_store.batch_get_traces.return_value = []
    clock = [1000.0]
    ...
    with (
        patch("MLForge.tracing.client._get_store", return_value=mock_store),
        patch("MLForge.tracing.client.time.monotonic", side_effect=fake_time),
        patch("MLForge.tracing.client.time.sleep", side_effect=fake_sleep) as mock_sleep,
    ):
        client = TracingClient()
        with pytest.raises(MLForgeException, match="is not found"):
            client.get_trace("trace:/catalog.schema/abc")
...
def test_get_trace_for_archive_repo_falls_back_to_batch_get_traces():
    ...
    mock_store.get_trace_info.return_value = archived_trace.info
    mock_store.get_trace.side_effect = MLForgeNotImplementedException()
    mock_store.batch_get_traces.return_value = [archived_trace]
...
def test_batch_get_traces_with_artifact_repo_traces():
    ...
    with MLForge.start_span("tracking_span"):
        tracking_trace_id = MLForge.get_active_trace_id()

    # Create a trace without spans (artifact-repo case)
    experiment_id = MLForge.tracking.fluent._get_experiment_id()
    artifact_trace_id = f"tr-{uuid.uuid4().hex}"
    store = MLForge.tracking.MLForgeClient()._tracking_client.store
    store.start_trace(
        TraceInfo(
...
def test_batch_get_traces_fallback_when_batch_get_trace_infos_not_implemented():
    mock_store = Mock()
    mock_store.batch_get_trace_infos.side_effect = MLForgeNotImplementedException()
    mock_store.batch_get_traces.return_value = ["trace1"]
...
def test_tracing_client_link_prompt_versions_to_trace():
    with MLForge.start_run():
        # Register a prompt
        prompt_version = MLForge.register_prompt(name="test_prompt", template="Hello, {{name}}!")
        ...
        assert "MLForge.linkedPrompts" in trace.info.tags

        # Parse and verify the linked prompts
        linked_prompts = json.loads(trace.info.tags["MLForge.linkedPrompts"])
...
def test_tracing_client_search_traces_with_model_id(monkeypatch, sql_warehouse_id: str | None):
    if sql_warehouse_id:
        monkeypatch.setenv(MLForge_TRACING_SQL_WAREHOUSE_ID.name, sql_warehouse_id)
    else:
    ...
    mock_store.search_traces.assert_called_once_with(
        experiment_ids=None,
        filter_string="request_metadata.`MLForge.modelId` = 'model_id'"
...
def test_tracing_client_get_trace_with_database_stored_spans():
    ...
    experiment_id = MLForge.create_experiment("test")
...
def test_tracing_client_get_trace_error_handling():
    ...
    with pytest.raises(
        MLForgeException, match=rf"Trace with ID {trace_id} is not fully exported yet"
    ):

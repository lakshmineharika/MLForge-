...
from MLForge.entities.trace_location import (
    MLForgeExperimentLocation,
    TraceLocation,
    UCSchemaLocation,
)
from MLForge.entities.trace_state import TraceState
from MLForge.environment_variables import MLForge_TRACE_SAMPLING_RATIO, MLForge_TRACKING_USERNAME
from MLForge.exceptions import MLForgeException
from MLForge.store.entities.paged_list import PagedList
from MLForge.store.tracking import SEARCH_TRACES_DEFAULT_MAX_RESULTS
...
from MLForge.tracing.destination import MLForgeExperiment
from MLForge.tracing.export.inference_table import pop_trace
from MLForge.tracing.fluent import start_span_no_context
from MLForge.tracing.provider import (
    _MLForge_TRACE_USER_DESTINATION,
    _get_tracer,
    safe_set_span_in_context,
    set_destination,
)
from MLForge.tracking.fluent import _get_experiment_id
...
class DefaultTestModel:
    ...
    def predict(self, x, y):
        z = x + y
        z = self.add_one(z)
        z = MLForge.trace(self.square)(z)
        return z  # noqa: RET504

    @MLForge.trace(span_type=SpanType.LLM, name="add_one_with_custom_name", attributes={"delta": 1})
    def add_one(self, z):
        return z + 1
...
class DefaultAsyncTestModel:
    ...
    async def predict(self, x, y):
        ...
        z = await MLForge.trace(self.square)(z)
        return z  # noqa: RET504
...
class StreamTestModel:
    @MLForge.trace(output_reducer=lambda x: sum(x))
...
def mock_client():
    ...
    with mock.patch("MLForge.tracing.fluent.TracingClient", return_value=client):
        yield client
...
def test_trace(wrap_sync_func, with_active_run, async_logging_enabled):
    ...
    if with_active_run:
        if IS_TRACING_SDK_ONLY:
            pytest.skip("Skipping test because MLForge or MLForge-skinny is not installed.")
    ...
    assert root_span.attributes == {
        ...
        "MLForge.spanInputs": {"x": 2, "y": 5},
        "MLForge.spanOutputs": 64,
    }
    ...
    assert child_span_1.attributes == {
        "delta": 1,
        "MLForge.traceRequestId": trace.info.trace_id,
        "MLForge.spanFunctionName": "add_one",
        "MLForge.spanType": "LLM",
        "MLForge.spanLogLevel": SpanLogLevel.INFO,
        "MLForge.spanInputs": {"z": 7},
        "MLForge.spanOutputs": 8,
    }
    ...
    assert child_span_2.attributes == {
        "MLForge.traceRequestId": trace.info.trace_id,
        "MLForge.spanFunctionName": "square",
        "MLForge.spanType": "UNKNOWN",
        "MLForge.spanLogLevel": SpanLogLevel.DEBUG,
        "MLForge.spanInputs": {"t": 8},
        "MLForge.spanOutputs": 64,
    }
...
def test_trace_stream(wrap_sync_func):
    ...
    assert MLForge.get_current_active_span() is None
    ...
    assert root_span.events[0].name == "MLForge.chunk.item.0"
    assert root_span.events[0].attributes == {"MLForge.chunk.value": "0"}
    assert root_span.events[8].name == "MLForge.chunk.item.8"
...
def test_trace_with_databricks_tracking_uri(databricks_tracking_uri, monkeypatch):
    ...
    with (
        ...
        mock.patch("MLForge.tracing.client._get_store") as mock_get_store,
    ):
...
def test_trace_in_databricks_model_serving(
    ...
    from MLForge.pyfunc.context import Context, set_prediction_context
    ...
    class TestModel:
        ...
        def predict(self, x, y):
            z = x + y
            z = self.add_one(z)
            with MLForge.start_span(name="square") as span:
                z = self.square(z)
                span.add_event(SpanEvent("event", 0, attributes={"foo": "bar"}))
            return z

        @MLForge.trace(span_type=SpanType.LLM, name="custom", attributes={"delta": 1})
        def add_one(self, z):
            return z + 1
    ...
    assert root_span.attributes == {
        "MLForge.traceRequestId": trace.info.trace_id,
        "MLForge.spanType": SpanType.UNKNOWN,
        "MLForge.spanLogLevel": SpanLogLevel.DEBUG,
        "MLForge.spanFunctionName": "predict",
        "MLForge.spanInputs": {"x": 2, "y": 5},
        "MLForge.spanOutputs": 64,
...
def test_trace_in_model_evaluation(monkeypatch, async_logging_enabled):
    ...
    trace = MLForge.get_trace(request_id_1)
    assert trace.info.request_metadata[TraceMetadataKey.SOURCE_RUN] == run_id
    assert trace.info.tags[TraceTagKey.EVAL_REQUEST_ID] == request_id_1

    trace = MLForge.get_trace(request_id_2)
    assert trace.info.request_metadata[TraceMetadataKey.SOURCE_RUN] == run_id
...
def test_trace_handle_exception_during_streaming():
    ...
    assert spans[0].events[0].name == "MLForge.chunk.item.0"
...
def test_trace_ignore_exception(monkeypatch, model):
    ...
    with mock.patch("MLForge.tracing.provider._get_tracer", side_effect=ValueError("Some error")):
...
def test_trace_skip_resolving_unrelated_tags_to_traces():
    with mock.patch("MLForge.tracking.context.registry.DatabricksRepoRunContext") as mock_context:
...
def test_trace_with_experiment_id():
    exp_1 = MLForge.create_experiment("exp_1")
    exp_2 = MLForge.set_experiment("exp_2").experiment_id  # active experiment

    @MLForge.trace(trace_destination=MLForgeExperiment(exp_1))
    def predict_1():
        with MLForge.start_span(name="child_span"):
            return
...
def test_trace_with_experiment_id_issue_warning_when_not_root_span():
    ...
    with mock.patch("MLForge.tracing.provider._logger") as mock_logger:
        predict_1()
...
def test_start_span_context_manager(async_logging_enabled):
    ...
    class TestModel:
        def predict(self, x, y):
            with MLForge.start_span(name="root_span") as root_span:
                root_span.set_inputs({"x": x, "y": y})
                z = x + y

                with MLForge.start_span(name="child_span", span_type=SpanType.LLM) as child_span:
        ...
        def square(self, t):
            with MLForge.start_span(name="child_span") as span:
                span.set_inputs({"t": t})
                res = t**2
                time.sleep(0.1)
    ...
    assert child_span_1.attributes == {
        ...
        "MLForge.spanType": "LLM",
        "MLForge.spanLogLevel": SpanLogLevel.INFO,
        "MLForge.spanInputs": 3,
        "MLForge.spanOutputs": 5,
    }
...
@pytest.mark.skipif(
    IS_TRACING_SDK_ONLY, reason="Skipping test because MLForge or MLForge-skinny is not installed."
)
def test_start_span_with_run_id(async_logging_enabled):
    ...
    with MLForge.start_span(
        name="root_span",
        trace_destination=MLForgeExperimentLocation(experiment_id=experiment_id),
        run_id=run.info.run_id,
    ):
        pass

    traces = MLForge.search_traces(
...
def test_start_span_with_run_id_takes_precedence_over_active_run(async_logging_enabled):
    ...
    with MLForge.start_run(run_id=active_run.info.run_id):
        with MLForge.start_span(
            name="root_span",
            trace_destination=MLForgeExperimentLocation(experiment_id=active_experiment_id),
...
def test_start_span_with_run_id_warns_for_child_span(async_logging_enabled):
    ...
    with mock.patch("MLForge.tracing.fluent._logger") as mock_logger:
        with MLForge.start_span(
            name="root_span",
            trace_destination=MLForgeExperimentLocation(experiment_id=experiment_id),
            run_id=run_1.info.run_id,
        ):
            with MLForge.start_span(name="child_span", run_id=run_2.info.run_id):
                pass
...
def test_start_span_context_manager_with_imperative_apis(async_logging_enabled):
    ...
    # (via MLForge client) are correctly linked together. This usage is not recommended but
...
def test_MLForge_trace_isolated_from_other_otel_processors():
    # Set up non-MLForge tracer
    ...
    # Create MLForge trace
    with MLForge.start_span(name="MLForge_span"):
        pass

    # Create non-MLForge trace
    tracer = trace.get_tracer(__name__)
    with tracer.start_as_current_span("non_MLForge_span"):
        pass

    # MLForge only processes spans created with MLForge APIs
    assert len(get_traces()) == 1
    assert (
        MLForge.get_trace(MLForge.get_last_active_trace_id(), flush=True).data.spans[0].name
        == "MLForge_span"
    )

    # Other spans are processed by the other processor
    assert len(other_exporter.exported_spans) == 1
    assert other_exporter.exported_spans[0].name == "non_MLForge_span"


def test_get_trace():
    with mock.patch("MLForge.tracing.display.get_display_handler") as mock_get_display_handler:
        model = DefaultTestModel()
        model.predict(2, 5)
        ...
        trace_in_memory = MLForge.get_trace(trace_id)
        ...
        trace_from_backend = MLForge.get_trace(trace.info.trace_id)
        assert trace.info.trace_id == trace_from_backend.info.trace_id
        mock_get_display_handler.assert_not_called()

    # If not found, return None with warning
    with mock.patch("MLForge.tracing.fluent._logger") as mock_logger:
        assert MLForge.get_trace("not_found") is None
...
def test_test_search_traces_empty(mock_client):
    ...
    if not IS_TRACING_SDK_ONLY:
        ...
        traces = MLForge.search_traces(extract_fields=["foo.inputs.bar"])
...
def test_search_traces_invalid_return_types(mock_client):
    with pytest.raises(MLForgeException, match=r"Invalid return type"):
        MLForge.search_traces(return_type="invalid")

    with pytest.raises(MLForgeException, match=r"The `extract_fields`"):
        MLForge.search_traces(extract_fields=["foo.inputs.bar"], return_type="list")


def test_search_traces_validates_experiment_ids_type():
    with pytest.raises(MLForgeException, match=r"locations must be a list"):
        MLForge.search_traces(locations=4)
...
def test_search_traces_with_pagination(mock_client):
    ...
    traces = MLForge.search_traces(locations=["1"])
...
def test_search_traces_with_default_experiment_id(mock_client):
    mock_client.search_traces.return_value = PagedList([], token=None)
    with mock.patch("MLForge.tracking.fluent._get_experiment_id", return_value="123"):
        MLForge.search_traces()
...
def test_search_traces_warns_on_uc_location_without_time_range(
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        MLForge.search_traces(locations=locations, filter_string=filter_string)
...
def test_search_traces_yields_expected_dataframe_contents(monkeypatch):
    ...
    df = MLForge.search_traces(max_results=10, order_by=["timestamp ASC"], flush=True)
...
def test_search_traces_with_input_and_no_output():
    with MLForge.start_span(name="with_input_and_no_output") as span:
        span.set_inputs({"a": 1})
...
def test_search_traces_with_non_dict_span_inputs_outputs():
    with MLForge.start_span(name="non_dict_span") as span:
        span.set_inputs(["a", "b"])
...
def test_search_traces_extract_fields_preserves_standard_columns():
    with MLForge.start_span(name="test_span") as span:
        span.set_inputs({"x": 1})
        span.set_outputs({"y": 2})

    df = MLForge.search_traces(extract_fields=["test_span.inputs.x"], flush=True)
...
def test_search_traces_with_multiple_spans_with_same_name():
    class TestModel:
        @MLForge.trace(name="duplicate_name")
        def predict(self, x, y):
            z = x + y
            z = self.add_one(z)
            z = MLForge.trace(self.square)(z)
        ...
        @MLForge.trace(span_type=SpanType.LLM, name="duplicate_name", attributes={"delta": 1})
        def add_one(self, z):
            return z + 1
...
def test_search_traces_span_and_field_name_with_dot():
    with MLForge.start_span(name="span.name") as span:
...
def test_search_traces_with_run_id():
    def _create_trace(name, tags=None):
        with MLForge.start_span(name=name) as span:
            for k, v in (tags or {}).items():
                MLForge.set_trace_tag(trace_id=span.request_id, key=k, value=v)
    ...
    with pytest.raises(MLForgeException, match="You cannot filter by run_id when it is already"):
        MLForge.search_traces(
            run_id=run2.info.run_id,
            filter_string="metadata.MLForge.sourceRun = '123'",
        )

    with pytest.raises(MLForgeException, match=f"Run {run1.info.run_id} belongs to"):
        MLForge.search_traces(run_id=run1.info.run_id, locations=["1"])
...
def test_search_traces_invalid_extract_fields(extract_fields):
    with pytest.raises(MLForgeException, match="Invalid field type"):
        MLForge.search_traces(extract_fields=extract_fields)


def test_get_last_active_trace_id():
    assert MLForge.get_last_active_trace_id() is None
    ...
    trace_id = MLForge.get_last_active_trace_id()
    trace = MLForge.get_trace(trace_id, flush=True)
    assert trace.info.trace_id is not None
    ...
    original_trace = MLForge.get_trace(trace.info.trace_id)
...
def test_get_last_active_trace_thread_local():
    ...
    def run(id):
        @MLForge.trace(name=f"predict_{id}")
        def predict(x, y):
            return x + y

        predict(1, 2)

        return MLForge.get_last_active_trace_id(thread_local=True)
...
def test_update_current_trace():
    @MLForge.trace(name="root_function")
    def f(x):
        MLForge.update_current_trace(tags={"fruit": "apple", "animal": "dog"})
        return g(x) + 1

    @MLForge.trace(name="level_1_function")
    def g(y):
        with MLForge.start_span(name="level_2_span"):
            MLForge.update_current_trace(tags={"fruit": "orange", "vegetable": "carrot"})
            return h(y) * 2

    @MLForge.trace(name="level_3_function")
    def h(z):
        with MLForge.start_span(name="level_4_span"):
            with MLForge.start_span(name="level_5_span"):
                MLForge.update_current_trace(tags={"depth": "deep", "level": "5"})
                return z + 10
    ...
    trace = MLForge.get_trace(MLForge.get_last_active_trace_id(), flush=True)
    assert trace.info.state == TraceState.OK
    tags = {k: v for k, v in trace.info.tags.items() if not k.startswith("MLForge.")}
...
def test_update_current_trace_with_client_request_id():
    ...
    with MLForge.start_span("test_span") as span:
        # Update with both tags and client_request_id
        MLForge.update_current_trace(tags={"operation": "test"}, client_request_id="req-12345")
    ...
    # Test with tags only
    with MLForge.start_span("test_span_2") as span:
        MLForge.update_current_trace(tags={"operation": "tags_only"})
    ...
    with MLForge.start_span("test_span_3") as span:
        MLForge.update_current_trace(client_request_id="req-67890")
...
def test_update_current_trace_client_request_id_overwrites():
    ...
    with MLForge.start_span("overwrite_test") as span:
        # First set
        MLForge.update_current_trace(client_request_id="req-initial")

        # Overwrite with new value
        MLForge.update_current_trace(client_request_id="req-updated")
...
def test_update_current_trace_client_request_id_stringification():
    ...
    for input_value, expected_output in test_cases:
        with MLForge.start_span(f"stringification_test_{input_value}") as span:
            if input_value is None:
                # None should not update the client_request_id
                MLForge.update_current_trace(client_request_id=input_value)
                trace_manager = InMemoryTraceManager.get_instance()
                with trace_manager.get_trace(span.trace_id) as trace:
                    assert trace.info.client_request_id is None
            else:
...
def test_update_current_trace_with_metadata():
    ...
    def f():
        MLForge.update_current_trace(
            metadata={
                "MLForge.source.name": "inference.py",
                "MLForge.source.git.commit": "1234567890",
                "MLForge.source.git.repoURL": "https://github.com/MLForge/MLForge",
...
def test_update_current_trace_with_model_id():
    with MLForge.start_span("test_span"):
        MLForge.update_current_trace(model_id="model-123")
...
def test_update_current_trace_should_not_raise_during_model_logging():
    ...
    class MyModel(MLForge.pyfunc.PythonModel):
        ...
        def predict(self, model_inputs):
            MLForge.update_current_trace(tags={"fruit": "apple"})
...
def test_update_current_trace_with_state():
    ...
    with MLForge.start_span("test_span") as span:
        MLForge.update_current_trace(state=TraceState.ERROR)
    ...
    with MLForge.start_span("test_span_2") as span:
        MLForge.update_current_trace(state="OK")
...
def test_update_current_trace_state_none():
    ...
    with MLForge.start_span("test_span") as span:
        ...
        MLForge.update_current_trace(state=None, tags={"test": "value"})
...
def test_update_current_trace_state_validation():
    with MLForge.start_span("test_span"):
        ...
        MLForge.update_current_trace(state="OK")
        MLForge.update_current_trace(state="ERROR")
        MLForge.update_current_trace(state=TraceState.OK)
        MLForge.update_current_trace(state=TraceState.ERROR)

        # Invalid string state should raise an exception
        with pytest.raises(
            MLForgeException, match=r"State must be either 'OK' or 'ERROR', but got 'IN_PROGRESS'"
        ):
            MLForge.update_current_trace(state="IN_PROGRESS")
        ...
        with pytest.raises(
            ...
            MLForge.update_current_trace(state=TraceState.STATE_UNSPECIFIED)

        # Custom invalid string should raise an exception
        with pytest.raises(
            MLForgeException, match=r"State must be either 'OK' or 'ERROR', but got 'CUSTOM_STATE'"
        ):
            MLForge.update_current_trace(state="CUSTOM_STATE")

        # Invalid types should raise an exception with a proper error message
        with pytest.raises(
            MLForgeException, match=r"State must be either 'OK' or 'ERROR', but got '123'"
        ):
            MLForge.update_current_trace(state=123)
...
def test_combined_state_and_record_exception():
    ...
    def test_function():
        ...
        span = MLForge.get_current_active_span()
        span.record_exception("Processing failed")

        # Update trace state independently
        MLForge.update_current_trace(state="ERROR", tags={"error_source": "processing"})
        return "result"
...
def test_span_record_exception_no_op_span():
    ...
    from MLForge.entities.span import NoOpSpan
...
def test_add_trace_no_current_active_trace():
    ...
    MLForge.add_trace(remote_trace)
...
def test_add_trace_specific_target_span(mock_otel_trace_start_time):
    ...
    MLForge.add_trace(_SAMPLE_REMOTE_TRACE, target=span)
    span.end()
...
def test_add_trace_merge_tags():
    ...
    with MLForge.start_span(name="parent") as span:
        client.set_trace_tag(span.trace_id, "vegetable", "carrot")
        client.set_trace_tag(span.trace_id, "food", "sushi")

        MLForge.add_trace(Trace.from_dict(_SAMPLE_REMOTE_TRACE))

    trace = MLForge.get_trace(MLForge.get_last_active_trace_id(), flush=True)
    custom_tags = {k: v for k, v in trace.info.tags.items() if not k.startswith("MLForge.")}
    assert custom_tags == {
        "fruit": "apple",
        "vegetable": "carrot",
...
def test_add_trace_raise_for_invalid_trace():
    with pytest.raises(MLForgeException, match="Invalid trace object"):
        MLForge.add_trace(None)

    with pytest.raises(MLForgeException, match="Failed to load a trace object"):
        MLForge.add_trace({"info": {}, "data": {}})
    ...
    with pytest.raises(MLForgeException, match="The trace must be ended"):
        MLForge.add_trace(in_progress_trace)

    trace = Trace.from_dict(_SAMPLE_REMOTE_TRACE)
    spans = trace.data.spans
    unordered_trace = Trace(info=trace.info, data=TraceData(spans=[spans[1], spans[0]]))
    with pytest.raises(MLForgeException, match="Span with ID "):
        MLForge.add_trace(unordered_trace)
...
def test_add_trace_logging_model_from_code():
    ...
    assert MLForge.get_trace(MLForge.get_last_active_trace_id()) is None
...
@pytest.mark.parametrize(
    "inputs", [{"question": "Does MLForge support tracing?"}, "Does MLForge support tracing?", None]
)
...
@pytest.mark.parametrize(
    ...
    [
        {
            "retrieved_documents": ["MLForge documentation"],
...
def test_set_delete_trace_tag():
    with MLForge.start_span("span1") as span:
        trace_id = span.trace_id

    MLForge.set_trace_tag(trace_id=trace_id, key="key1", value="value1")
    trace = MLForge.get_trace(trace_id=trace_id, flush=True)
    assert trace.info.tags["key1"] == "value1"

    MLForge.delete_trace_tag(trace_id=trace_id, key="key1")
    trace = MLForge.get_trace(trace_id=trace_id, flush=True)
    assert "key1" not in trace.info.tags

    # Test with request_id kwarg (backward compatibility)
    MLForge.set_trace_tag(request_id=trace_id, key="key3", value="value3")
    trace = MLForge.get_trace(request_id=trace_id, flush=True)
    assert trace.info.tags["key3"] == "value3"

    MLForge.delete_trace_tag(request_id=trace_id, key="key3")
    trace = MLForge.get_trace(request_id=trace_id, flush=True)
...
def test_search_traces_with_run_id_validates_store_filter_string(is_databricks):
    ...
    with (
        mock.patch("MLForge.tracing.client._get_store", return_value=mock_store),
        mock.patch("MLForge.tracking.fluent._get_experiment_id", return_value="test_exp_id"),
    ):
        MLForge.search_traces(run_id=test_run_id)
...
def test_search_traces_with_locations(mock_client):
    ...
    # Test with locations
    MLForge.search_traces(locations=["catalog1.schema1", "catalog2.schema2"])
...
def test_search_traces_experiment_ids_deprecation_warning(mock_client):
    ...
    with pytest.warns(FutureWarning, match="experiment_ids.*deprecated.*use.*locations"):
        MLForge.search_traces(experiment_ids=["123"])
...
def test_search_traces_with_sql_warehouse_id(mock_client):
    ...
    MLForge.search_traces(locations=["123"], sql_warehouse_id="warehouse456")
    ...
    assert os.environ["MLForge_TRACING_SQL_WAREHOUSE_ID"] == "warehouse456"
...
def test_set_destination_in_threads(async_logging_enabled, use_batch_processor, monkeypatch):
    monkeypatch.setenv("MLForge_USE_BATCH_SPAN_PROCESSOR", str(use_batch_processor))
    ...
    def func(experiment_id: str | None, x: int):
        if experiment_id is not None:
            set_destination(MLForgeExperiment(experiment_id), context_local=True)

        time.sleep(0.5)
        model.predict(x)

    # Main thread: global config
    experiment_id1 = MLForge.create_experiment(uuid.uuid4().hex)
    set_destination(MLForgeExperiment(experiment_id1))
    func(None, 3)

    # Thread 1: context-local config
    experiment_id2 = MLForge.create_experiment(uuid.uuid4().hex)
    thread1 = threading.Thread(
        name="test-fluent-destination-thread1", target=func, args=(experiment_id2, 3)
    )

    # Thread 2: context-local config
    experiment_id3 = MLForge.create_experiment(uuid.uuid4().hex)
...
def test_set_destination_from_env_var_databricks_uc(monkeypatch):
    monkeypatch.setenv("MLForge_TRACING_DESTINATION", "catalog.schema")
    destination = _MLForge_TRACE_USER_DESTINATION.get()
...
def test_traces_can_be_searched_by_span_properties(async_logging_enabled):
    ...
    traces = MLForge.search_traces(filter_string='span.name = "test_span"', return_type="list")
...
def _create_trace_with_session(session_id: str, name: str = "test_span") -> str:
    with MLForge.start_span(name=name) as span:
        MLForge.update_current_trace(metadata={TraceMetadataKey.TRACE_SESSION: session_id})
...
def test_search_sessions_empty():
    ...
    sessions = MLForge.search_sessions()
    assert sessions == []
...
def test_search_sessions_respects_max_results():
    ...
    sessions = MLForge.search_sessions(max_results=2)
...
def test_search_sessions_validates_locations_type():
    with pytest.raises(MLForgeException, match=r"locations must be a list"):
        MLForge.search_sessions(locations=4)
...
def test_search_sessions_raises_without_experiment():
    with mock.patch("MLForge.tracking.fluent._get_experiment_id", return_value=None):
        with pytest.raises(MLForgeException, match=r"No active experiment found"):
            MLForge.search_sessions()
...
def test_search_sessions_include_spans_true():
    ...
    sessions = MLForge.search_sessions(include_spans=True)
...
def test_search_sessions_include_spans_false():
    ...
    sessions = MLForge.search_sessions(include_spans=False)
...
def test_trace_decorator_sampling_ratio_validation(invalid_ratio: float):
    with pytest.raises(
        MLForgeException, match=r"sampling_ratio_override must be between 0\.0 and 1\.0"
    ):
        MLForge.trace(sampling_ratio_override=invalid_ratio)
...
def test_trace_decorator_sampling_ratio(
    ...
    @MLForge.trace(sampling_ratio_override=sampling_ratio)
    def traced_func():
        if trace_id := MLForge.get_active_trace_id():
...
def test_trace_decorator_sampling_ratio_nested(
    ...
    @MLForge.trace(sampling_ratio_override=outer_ratio)
    def outer():
        if trace_id := MLForge.get_active_trace_id():
            outer_trace_ids.append(trace_id)
        return inner()

    @MLForge.trace(sampling_ratio_override=inner_ratio)
    def inner():
        if trace_id := MLForge.get_active_trace_id():
...
def test_global_sampling_ratio_nested(monkeypatch):
    monkeypatch.setenv(MLForge_TRACE_SAMPLING_RATIO.name, "0.0")
    ...
    @MLForge.trace(sampling_ratio_override=1.0)
    def inner():
        if trace_id := MLForge.get_active_trace_id():
            inner_trace_ids.append(trace_id)
        return "result"
...
@MLForge.trace  # Should respect global 0.0
def not_traced():
    if trace_id := MLForge.get_active_trace_id():
        trace_ids.append(trace_id)
    return "not traced"
...
@MLForge.trace(sampling_ratio_override=1.0)  # Should override global 0.0
def traced():
    if trace_id := MLForge.get_active_trace_id():
        trace_ids.append(trace_id)
    return "traced"
...
"""
    subprocess.check_call(
        ...
        env={
            **os.environ,
            "MLForge_TRACE_SAMPLING_RATIO": "0.0",
        },
    )
...
def test_tracing_context_session_id_and_user():
    with MLForge.tracing.context(session_id="sess-123", user="user-456"):
        my_func()

    trace = MLForge.get_trace(MLForge.get_last_active_trace_id(), flush=True)
    assert trace.info.request_metadata["MLForge.trace.session"] == "sess-123"
    assert trace.info.request_metadata["MLForge.trace.user"] == "user-456"
    ...
    trace = MLForge.get_trace(MLForge.get_last_active_trace_id(), flush=True)
    assert trace.info.request_metadata["MLForge.trace.session"] == "sess-abc"
    assert trace.info.request_metadata["MLForge.trace.user"] == "user-xyz"
    assert trace.info.request_metadata["custom_key"] == "custom_value"


def test_tracing_context_session_id_and_user_nesting():
    with MLForge.tracing.context(session_id="outer-sess", user="outer-user"):
        with MLForge.tracing.context(session_id="inner-sess"):
            my_func()

    trace = MLForge.get_trace(MLForge.get_last_active_trace_id(), flush=True)
    # Inner session_id overrides outer
    assert trace.info.request_metadata["MLForge.trace.session"] == "inner-sess"
    # Outer user is inherited
    assert trace.info.request_metadata["MLForge.trace.user"] == "outer-user"
...
def test_tracing_context_enabled_false_suppresses_traces():
    with MLForge.tracing.context(enabled=False):
        my_func()

        # Child context should inherit the enabled=False from the parent
        with MLForge.tracing.context(metadata={"k": "v"}):
            my_func()

        # Start trace with start_trace_no_context (used in autologging)
        span = MLForge.start_span_no_context("test")
        span.end()
    ...
    my_func()
    assert MLForge.get_last_active_trace_id() is not None


def test_tracing_context_enabled_is_thread_safe():
    def run_with_context(enabled):
        with MLForge.tracing.context(enabled=enabled):
            my_func()
            return MLForge.get_last_active_trace_id(thread_local=True)
...
def test_flush_trace_async_logging_calls_flush_when_async_queue_exists():
    ...
    with mock.patch("MLForge.tracking.fluent._get_trace_exporter", return_value=mock_exporter):
        MLForge.flush_trace_async_logging(terminate=False)
...
def test_flush_trace_async_logging_skips_when_async_queue_missing():
    # A bare SpanExporter (as used by StrandsSpanProcessor, MLForge/strands/autolog.py:40)
    ...
    with (
        mock.patch("MLForge.tracking.fluent._get_trace_exporter", return_value=exporter),
        mock.patch(
            "MLForge.tracking.fluent._logger.error",
...
def test_flush_trace_async_logging_no_spurious_error_when_tracing_disabled():
    ...
    with mock.patch("MLForge.tracking.fluent._logger") as mock_logger:
        MLForge.flush_trace_async_logging(terminate=True)

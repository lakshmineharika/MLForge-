...
from MLForge.tracing.databricks import set_databricks_monitoring_sql_warehouse_id


def test_set_databricks_monitoring_sql_warehouse_id_requires_databricks_tracking_uri():
    with mock.patch("MLForge.get_tracking_uri", return_value="file:///tmp"):
        with pytest.raises(MLForgeException, match="only supported when the tracking URI"):
...
def test_set_databricks_monitoring_sql_warehouse_id_with_explicit_experiment_id():
    ...
    with (
        mock.patch("MLForge.tracking.get_tracking_uri", return_value="databricks"),
        mock.patch(
        ...
        assert call_args[0][1].key == "MLForge.monitoring.sqlWarehouseId"
...
def test_set_databricks_monitoring_sql_warehouse_id_with_default_experiment_id():
    ...
    with (
        ...
        mock.patch("MLForge.tracking.fluent._get_experiment_id", return_value="default_exp"),
    ):

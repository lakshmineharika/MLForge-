...
from MLForge.entities import LiveSpan, Span
...
from MLForge.tracing.trace_manager import InMemoryTraceManager, ManagerTrace
...
def test_add_spans():
    ...
    # Convert to MLForge trace to check immutable spans
...
def test_traces_buffer_expires_after_ttl(monkeypatch):
    monkeypatch.setenv("MLForge_TRACE_BUFFER_TTL_SECONDS", "1")
...
def test_traces_buffer_expires_and_log_when_timeout_is_set(monkeypatch):
    # Setting MLForge_TRACE_TIMEOUT_SECONDS let MLForge to periodically check the
    # expired traces and log expired ones to the backend.
    monkeypatch.setenv("MLForge_TRACE_TIMEOUT_SECONDS", "1")
    monkeypatch.setenv("MLForge_TRACE_TTL_CHECK_INTERVAL_SECONDS", "1")

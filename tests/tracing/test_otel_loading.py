...
import MLForge
from MLForge.entities import SpanStatusCode
from MLForge.entities.assessment import AssessmentSource, Expectation, Feedback
from MLForge.entities.assessment_source import AssessmentSourceType
...
from MLForge.tracing.constant import SpanAttributeKey, SpansLocation, TraceTagKey
from MLForge.tracing.otel.translation.base import OtelSchemaTranslator
from MLForge.tracing.otel.translation.genai_semconv import GenAiTranslator
from MLForge.tracing.otel.translation.open_inference import OpenInferenceTranslator
from MLForge.tracing.otel.translation.traceloop import TraceloopTranslator
from MLForge.tracing.provider import _get_trace_exporter
from MLForge.tracing.utils import encode_trace_id
from MLForge.tracing.utils.otlp import MLForge_EXPERIMENT_ID_HEADER
from MLForge.tracking._tracking_service.utils import _use_tracking_uri
...
if IS_TRACING_SDK_ONLY:
    pytest.skip("OTel get_trace tests require full MLForge server", allow_module_level=True)


@pytest.fixture
def MLForge_server(tmp_path: Path, db_uri: str):
...
def tracking_uri_setup(MLForge_server):
    with _use_tracking_uri(MLForge_server):
        yield


@pytest.fixture(params=[True, False])
def is_async(request, monkeypatch):
    monkeypatch.setenv("MLForge_ASYNC_TRACE_LOGGING", "true" if request.param else "false")
...
def create_tracer(MLForge_server: str, experiment_id: str, service_name: str = "test-service"):
    resource = OTelSDKResource.create({"service.name": service_name, "service.version": "1.0.0"})
    tracer_provider = TracerProvider(resource=resource)

    exporter = OTLPSpanExporter(
        endpoint=f"{MLForge_server}/v1/traces",
        headers={MLForge_EXPERIMENT_ID_HEADER: experiment_id},
        timeout=10,
    )
...
def test_get_trace_for_otel_sent_span(MLForge_server: str, is_async):
    experiment = MLForge.set_experiment("otel-get-trace-test")
    experiment_id = experiment.experiment_id

    tracer = create_tracer(MLForge_server, experiment_id, "test-service-get-trace")
    ...
    retrieved_trace = MLForge.get_trace(trace_id)

    assert retrieved_trace.info.trace_id == trace_id
    assert retrieved_trace.info.trace_location.MLForge_experiment.experiment_id == experiment_id
...
def test_rest_client_reads_archived_trace_via_server(MLForge_server: str, tmp_path: Path, is_async):
    experiment = MLForge.set_experiment("otel-archived-trace-rest-test")
    experiment_id = experiment.experiment_id

    tracer = create_tracer(MLForge_server, experiment_id, "test-service-archive-read")
    ...
    client = MLForge.MLForgeClient(MLForge_server)
    with (
        mock.patch.object(
...
def test_get_trace_for_otel_nested_spans(MLForge_server: str, is_async):
    experiment = MLForge.set_experiment("otel-nested-spans-test")
    experiment_id = experiment.experiment_id

    tracer = create_tracer(MLForge_server, experiment_id, "nested-test-service")
...
def test_get_trace_with_otel_span_events(MLForge_server: str, is_async):
    experiment = MLForge.set_experiment("otel-events-test")
    experiment_id = experiment.experiment_id

    tracer = create_tracer(MLForge_server, experiment_id, "events-test-service")
...
def test_get_trace_nonexistent_otel_trace(MLForge_server: str):
    # Create a fake trace ID in OTel format
    fake_otel_trace_id = uuid.uuid4().hex
    fake_trace_id = f"tr-{fake_otel_trace_id}"

    # MLForge get_trace returns None for non-existent traces
    trace = MLForge.get_trace(fake_trace_id)
    assert trace is None


def test_get_trace_with_otel_span_status(MLForge_server: str, is_async):
    experiment = MLForge.set_experiment("otel-status-test")
    experiment_id = experiment.experiment_id

    tracer = create_tracer(MLForge_server, experiment_id, "status-test-service")
...
def test_set_trace_tag_on_otel_trace(MLForge_server: str, is_async):
    experiment = MLForge.set_experiment("otel-tag-test")
    experiment_id = experiment.experiment_id

    tracer = create_tracer(MLForge_server, experiment_id, "tag-test-service")
    ...
    MLForge.set_trace_tag(trace_id, "environment", "test")
    MLForge.set_trace_tag(trace_id, "version", "1.0.0")
...
def test_log_expectation_on_otel_trace(MLForge_server: str, is_async):
    experiment = MLForge.set_experiment("otel-expectation-test")
    experiment_id = experiment.experiment_id

    tracer = create_tracer(MLForge_server, experiment_id, "expectation-test-service")

    # Create a span that represents a question-answer scenario
    with tracer.start_as_current_span("qa-span") as span:
        span.set_attribute("question", "What is MLForge?")
        span.set_attribute("answer", "MLForge is an open-source ML platform")
    ...
    logged_assessment = MLForge.log_expectation(
        trace_id=trace_id,
        name="expected_answer",
        value="MLForge is an open-source machine learning platform",
        source=expectation_source,
        metadata={"confidence": "high", "reviewed_by": "expert"},
    )
    expectation = MLForge.get_assessment(
        trace_id=trace_id, assessment_id=logged_assessment.assessment_id
    )
    assert expectation.name == "expected_answer"
    assert expectation.value == "MLForge is an open-source machine learning platform"
    assert expectation.source.source_type == AssessmentSourceType.HUMAN
    assert expectation.metadata["confidence"] == "high"


def test_log_feedback_on_otel_trace(MLForge_server: str, is_async):
    experiment = MLForge.set_experiment("otel-feedback-test")
    experiment_id = experiment.experiment_id

    tracer = create_tracer(MLForge_server, experiment_id, "feedback-test-service")
    ...
    logged_quality = MLForge.log_feedback(
        trace_id=trace_id,
        name="quality_score",
        value=8.5,
        source=llm_source,
    ...
    feedback = MLForge.get_assessment(trace_id=trace_id, assessment_id=logged_quality.assessment_id)
    ...
    logged_approval = MLForge.log_feedback(
        trace_id=trace_id,
        name="approved",
        value=True,
        source=human_source,
    ...
    feedback = MLForge.get_assessment(trace_id=trace_id, assessment_id=logged_approval.assessment_id)
    assert feedback.name == "approved"
    assert feedback.value is True
    assert feedback.source.source_type == AssessmentSourceType.HUMAN


def test_multiple_assessments_on_otel_trace(MLForge_server: str, is_async):
    experiment = MLForge.set_experiment("otel-multi-assessment-test")
    experiment_id = experiment.experiment_id

    tracer = create_tracer(MLForge_server, experiment_id, "multi-assessment-test-service")
    ...
    MLForge.set_trace_tag(trace_id, "topic", "quantum_computing")
    MLForge.set_trace_tag(trace_id, "complexity", "high")
    ...
    MLForge.log_assessment(trace_id=trace_id, assessment=expectation)
    feedback_items = [
    ...
    for feedback in feedback_items:
        MLForge.log_assessment(trace_id=trace_id, assessment=feedback)
    ...
    tagged_traces = MLForge.search_traces(
...
def test_span_kind_translation(MLForge_server: str, is_async):
    experiment = MLForge.set_experiment("span-kind-translation-test")
    experiment_id = experiment.experiment_id

    tracer = create_tracer(MLForge_server, experiment_id, "span-kind-translation-test-service")
    ...
    for trace_info in traces:
        retrieved_trace = MLForge.get_trace(trace_info.info.trace_id)
...
def test_span_inputs_outputs_translation(
    MLForge_server: str, is_async, translator: OtelSchemaTranslator
):
    experiment = MLForge.set_experiment("span-inputs-outputs-translation-test")
    experiment_id = experiment.experiment_id

    tracer = create_tracer(
        MLForge_server, experiment_id, "span-inputs-outputs-translation-test-service"
    )
    ...
    assert len(traces) == 1
    retrieved_trace = MLForge.get_trace(traces[0].info.trace_id)
...
def test_span_token_usage_translation(
    ...
    experiment = MLForge.set_experiment("span-token-usage-translation-test")
    experiment_id = experiment.experiment_id

    tracer = create_tracer(
        MLForge_server, experiment_id, "span-token-usage-translation-test-service"
    )
...
def test_aggregated_token_usage_from_multiple_spans(
    ...
    experiment = MLForge.set_experiment("aggregated-token-usage-test")
    experiment_id = experiment.experiment_id

    tracer = create_tracer(MLForge_server, experiment_id, "token-aggregation-service")
...
def test_token_usage_on_parent_span_deduplicates_children(
    ...
    experiment = MLForge.set_experiment("rollup-token-usage-test")
    experiment_id = experiment.experiment_id

    tracer = create_tracer(MLForge_server, experiment_id, "token-rollup-service")

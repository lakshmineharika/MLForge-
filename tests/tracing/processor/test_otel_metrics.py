...
def test_metrics_export(
    ...
    @MLForge.trace(span_type="CHAIN", name="parent")
    def parent_function() -> str:
        MLForge.update_current_trace({"env": "test", "version": "1.0"})
        time.sleep(0.01)  # 10ms
        return child_function()

    @MLForge.trace(span_type="LLM", name="child")
    def child_function() -> str:
        time.sleep(0.25)  # 250ms
        return "result"

    @MLForge.trace(span_type="TOOL", name="error_function")
    ...
    for resource_metric in metrics_data.resource_metrics:
        for scope_metric in resource_metric.scope_metrics:
            for metric in scope_metric.metrics:
                if metric.name == "MLForge.trace.span.duration":
...
def test_no_metrics_when_disabled(
    ...
    assert "MLForge.trace.span.duration" not in metric_names

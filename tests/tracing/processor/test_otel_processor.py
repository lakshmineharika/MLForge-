...
from MLForge.entities.trace_info import TraceInfo, TraceLocation, TraceState
from MLForge.tracing.constant import TRACE_SCHEMA_VERSION_KEY, SpanAttributeKey
from MLForge.tracing.processor.otel import OtelSpanProcessor
from MLForge.tracing.trace_manager import ManagerTrace
...
def _make_manager_trace(tags: dict[str, str]) -> ManagerTrace:
    from MLForge.entities import Trace, TraceData
...
def test_on_end_filters_MLForge_prefixed_tags():
    processor, _ = _make_processor()
    otel_trace_id = 0xDEADBEEF
    ...
    tags = {
        "user_tag": "keep_me",
        "MLForge.traceName": "my-trace",
        "MLForge.trace.spansLocation": "TRACKING_STORE",
        "MLForge.user": "alice",
        "MLForge.artifactLocation": "s3://bucket",
    }

...
from MLForge.tracing.processor.inference_table import (
...
from MLForge.utils.MLForge_tags import MLForge_DATABRICKS_MODEL_SERVING_ENDPOINT_NAME
...
from MLForge.tracking.fluent import set_active_model

_OTEL_TRACE_ID = 12345
_DATABRICKS_REQUEST_ID = "databricks-request-id"


@pytest.mark.parametrize("context_type", ["MLForge", "flask"])
def test_on_start(context_type):
    ...
    if context_type == "MLForge":
        ...
        with mock.patch(
            "MLForge.tracing.processor.inference_table._get_flask_request"
    ...
    with trace_manager.get_trace(expected_trace_id) as trace:
        # Trace ID should be generated by MLForge
        ...
        if context_type == "MLForge":
            assert trace.info.request_metadata == {
                ...
                MLForge_DATABRICKS_MODEL_SERVING_ENDPOINT_NAME: "test-endpoint",
...
def test_on_start_with_experiment_id_env_var(monkeypatch):
    # When the MLForge_EXPERIMENT_ID env var is set, it should be populated into the trace info
    monkeypatch.setenv("MLForge_EXPERIMENT_ID", "123")

...
import MLForge.tracking.context.default_context
...
def test_on_start(monkeypatch):
    ...
    processor = MLForgeV3SpanProcessor(span_exporter=mock.MagicMock(), export_metrics=False)
...
def test_on_start_during_run(monkeypatch):
    ...
    MLForge.create_experiment(env_experiment_name)
    run_experiment_id = MLForge.create_experiment(run_experiment_name)

    MLForge.set_experiment(experiment_name=env_experiment_name)
    processor = MLForgeV3SpanProcessor(span_exporter=mock.MagicMock(), export_metrics=False)

    with MLForge.start_run(experiment_id=run_experiment_id) as run:
        processor.on_start(span)
...
def test_on_end():
    ...
    processor = MLForgeV3SpanProcessor(span_exporter=mock_exporter, export_metrics=False)
...
def test_batch_processor_reads_existing_env_vars(monkeypatch):
    mock_exporter = mock.MagicMock()
    monkeypatch.setenv("MLForge_ASYNC_TRACE_LOGGING_MAX_INTERVAL_MILLIS", "1000")
    monkeypatch.setenv("MLForge_ASYNC_TRACE_LOGGING_MAX_SPAN_BATCH_SIZE", "256")

    with mock.patch("MLForge.tracing.processor.base_MLForge.BatchSpanProcessor") as mock_batch_cls:
        MLForgeV3SpanProcessor(
...
def test_set_last_active_trace_id_called_once_for_root_span(monkeypatch):
    ...
    with mock.patch("MLForge.tracing.fluent._set_last_active_trace_id") as mock_set_id:
        monkeypatch.setattr(
            "MLForge.tracing.processor.base_MLForge._set_last_active_trace_id", mock_set_id
        )
        processor.on_end(otel_span)

...
from MLForge.entities.trace_location import TraceLocationType, UCSchemaLocation
...
def active_uc_schema_destination():
    ...
    _MLForge_TRACE_USER_DESTINATION.set(destination)
    try:
        yield
    finally:
...
def test_on_start_with_uc_table_name(monkeypatch, active_uc_schema_destination):
    ...
    monkeypatch.setenv(MLForge_TRACKING_USERNAME.name, "alice")
...
def test_on_start_without_uc_table_name(monkeypatch):
    ...
    _MLForge_TRACE_USER_DESTINATION.reset()
    processor = DatabricksUCTableSpanProcessor(span_exporter=mock.MagicMock())
    with pytest.raises(MLForgeException, match="Unity Catalog spans table name is not set"):
        processor.on_start(span)
...
def test_trace_id_generation_with_uc_schema(active_uc_schema_destination):
    ...
    with mock.patch(
        "MLForge.tracing.processor.uc_table.generate_trace_id_v4",
...
def test_on_end_sets_user_session_span_attributes():
    ...
    with mock.patch.object(trace_manager, "pop_trace", return_value=None):
        with MLForge.start_span("foo") as live_span:

...
from MLForge.environment_variables import (
    MLForge_ENABLE_ASYNC_LOGGING,
    MLForge_ENABLE_ASYNC_TRACE_LOGGING,
)
...
def enable_async_trace_logging(monkeypatch):
    ...
    monkeypatch.setenv(MLForge_ENABLE_ASYNC_LOGGING.name, "true")
...
def databricks_tracking_uri():
    with mock.patch("MLForge.get_tracking_uri", return_value="databricks"):
        yield
...
def async_logging_enabled(request, monkeypatch):
    monkeypatch.setenv(MLForge_ENABLE_ASYNC_TRACE_LOGGING.name, str(request.param))
    # TODO: V2 Trace depends on this env var rather than MLForge_ENABLE_ASYNC_TRACE_LOGGING
    # We should remove this once the backend is fully migrated to V3
    monkeypatch.setenv(MLForge_ENABLE_ASYNC_LOGGING.name, str(request.param))

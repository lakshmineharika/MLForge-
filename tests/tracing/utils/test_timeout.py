...
from MLForge.entities.span_status import SpanStatusCode
from MLForge.tracing.export.inference_table import _TRACE_BUFFER, pop_trace
from MLForge.tracing.trace_manager import _Trace
from MLForge.tracing.utils.timeout import MLForgeTraceTimeoutCache
...
@pytest.fixture
def cache():
    timeout_cache = MLForgeTraceTimeoutCache(timeout=1, maxsize=10)
    yield timeout_cache
    timeout_cache.clear()
...
def test_trace_halted_after_timeout(monkeypatch):
    # When MLForge_TRACE_TIMEOUT_SECONDS is set, MLForge should halt the trace after
    # the timeout and log it to the backend with an error status
    monkeypatch.setenv("MLForge_TRACE_TIMEOUT_SECONDS", "3")
    ...
    in_progress_traces = MLForge.search_traces(
...
def test_handle_timeout_update(monkeypatch):
    ...
    monkeypatch.setenv("MLForge_TRACE_TIMEOUT_SECONDS", "1")
    ...
    monkeypatch.setenv("MLForge_TRACE_TIMEOUT_SECONDS", "100")
    _SlowModel().predict(3)

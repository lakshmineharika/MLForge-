...
import MLForge
from MLForge.tracing.utils.copy import copy_trace_to_experiment
...
def _create_test_span_dict(request_id="test-trace", parent_id=None):
    ...
    return {
        ...
        "status_message": "",
        "attributes": {
            "MLForge.traceRequestId": f'"{request_id}"',
...
def setup_experiment():
    ...
    exp = MLForge.set_experiment(f"test_copy_trace_{time.time()}")
    yield exp
...
def test_copy_trace_with_metadata():
    trace_dict = {
        "info": {
            ...
            "status": "OK",
            "trace_metadata": {
                "MLForge.trace.session": "session123",
    ...
    trace = MLForge.get_trace(new_trace_id, flush=True)
    metadata = trace.info.trace_metadata

    assert metadata["MLForge.trace.session"] == "session123"

...
import MLForge
from MLForge.entities.span import SpanType
...
from MLForge.tracing.provider import _get_trace_exporter, _get_tracer
from MLForge.tracing.provider import provider as MLForge_provider
from MLForge.tracing.utils.otlp import _set_otel_proto_anyvalue
...
from MLForge.tracing.utils.otlp import (
...
@pytest.mark.parametrize(
    ("traces_endpoint", "otlp_endpoint", "MLForge_enable", "expected"),
    [
...
def test_should_use_otlp_exporter(
    traces_endpoint, otlp_endpoint, MLForge_enable, expected, monkeypatch
):
    # Clear all relevant environment variables to ensure test isolation
    monkeypatch.delenv("OTEL_EXPORTER_OTLP_TRACES_ENDPOINT", raising=False)
    monkeypatch.delenv("OTEL_EXPORTER_OTLP_ENDPOINT", raising=False)
    monkeypatch.delenv("MLForge_ENABLE_OTLP_EXPORTER", raising=False)
    ...
    if MLForge_enable is not None:
        monkeypatch.setenv("MLForge_ENABLE_OTLP_EXPORTER", MLForge_enable)
...
def test_get_otlp_exporter_invalid_protocol(monkeypatch):
    ...
    with pytest.raises(MLForgeException, match="Unsupported OTLP protocol"):
        get_otlp_exporter()
...
def test_export_to_otel_collector(otel_collector, monkeypatch, dual_export):
    if dual_export:
        monkeypatch.setenv("MLForge_TRACE_ENABLE_OTLP_DUAL_EXPORT", "true")
    elif dual_export is False:
        monkeypatch.setenv("MLForge_TRACE_ENABLE_OTLP_DUAL_EXPORT", "false")
    ...
    # In dual-export mode, _get_trace_exporter() returns the MLForge exporter
    # (since _get_span_processor prefers BaseMLForgeSpanProcessor), so we find
    # the OTLP exporter by looking for the OtelSpanProcessor directly.
    if dual_export:
        # In dual-export mode, _get_trace_exporter() returns the MLForge exporter
        # (since _get_span_processor prefers BaseMLForgeSpanProcessor). Find the
        # OTLP exporter by looking up the OtelSpanProcessor from the tracer provider.
        tp = MLForge_provider.get()
    ...
    MLForge_traces = get_traces()
    if dual_export:
        assert len(MLForge_traces) == 1
        assert MLForge_traces[0].info.state == "OK"
        assert len(MLForge_traces[0].data.spans) == 3
    else:
        assert len(MLForge_traces) == 0
...
@pytest.mark.skipif(is_windows(), reason="Otel collector docker image does not support Windows")
def test_dual_export_to_MLForge_and_otel(otel_collector, monkeypatch):
    """
    Test that dual export mode sends traces to both MLForge and OTLP collector.
    """
    _, _, port = otel_collector
    monkeypatch.setenv(MLForge_TRACE_ENABLE_OTLP_DUAL_EXPORT.name, "true")
    monkeypatch.setenv("OTEL_EXPORTER_OTLP_TRACES_ENDPOINT", f"http://127.0.0.1:{port}/v1/traces")

    experiment = MLForge.set_experiment("dual_export_test")

...
import MLForge
from MLForge.tracing.config import TracingConfig, get_config
...
def test_configure():
    ...
    MLForge.tracing.configure(span_processors=[dummy_filter])
...
def test_reset_config():
    ...
    MLForge.tracing.configure(span_processors=[filter1])
...
def test_configure_context_manager():
    ...
    with MLForge.tracing.configure(span_processors=[filter2]):
        assert get_config().span_processors == [filter2]

        with MLForge.tracing.configure(span_processors=[filter1, filter2]):
...
def test_context_manager_with_non_copyable_callable():
    ...
    MLForge.tracing.configure(span_processors=[lambda_filter])
    ...
    with MLForge.tracing.configure(span_processors=[regular_filter]):

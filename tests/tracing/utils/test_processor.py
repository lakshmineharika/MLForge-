...
from MLForge.tracing.utils.processor import validate_span_processors
...
def test_span_processors_no_processors_configured():
    MLForge.tracing.configure(span_processors=[])
...
def test_span_processors_single_processor_success():
    ...
    MLForge.tracing.configure(span_processors=[test_processor])
...
def test_apply_span_processors_multiple_processors_success():
    ...
    MLForge.tracing.configure(span_processors=[processor1, processor2])
...
def test_apply_span_processors_returns_non_none_warning():
    ...
    with patch("MLForge.tracing.utils.processor._logger") as mock_logger:
        MLForge.tracing.configure(span_processors=[bad_processor, good_processor])
...
def test_apply_span_processors_exception_handling():
    ...
    with patch("MLForge.tracing.utils.processor._logger") as mock_logger:
        MLForge.tracing.configure(span_processors=[failing_processor, good_processor])
...
def test_validate_span_processors_non_callable_raises_exception():
    ...
    with pytest.raises(MLForgeException, match=r"Span processor must be"):
...
def test_validate_span_processors_invalid_arguments_raises_exception():
    ...
    with pytest.raises(MLForgeException, match=r"Span processor must take"):

...
from MLForge.tracing import set_span_chat_tools
...
from MLForge.tracing.utils import (
...
def test_maybe_get_request_id():
    ...
    try:
        ...
        pytest.skip("Skipping the rest of tests as MLForge.pyfunc module is not available.")
...
def test_set_chat_tools_validation():
    ...
    @MLForge.trace(span_type=SpanType.CHAT_MODEL)
...
def test_openai_parse_tools_enum_validation(enum_values, param_type):
    from MLForge.openai.utils.chat_schema import _parse_tools
...
def test_parse_trace_id_v4_invalid_format():
    with pytest.raises(MLForgeException, match="Invalid trace ID format"):
...
def test_generate_trace_id_v4_with_uc_schema():
    ...
    with mock.patch(
        "MLForge.tracing.utils.construct_trace_id_v4", return_value="trace:/catalog.schema/abc123"
    ) as mock_construct:
...
def test_get_spans_table_name_for_trace_with_destination():
    ...
    with mock.patch("MLForge.tracing.provider._MLForge_TRACE_USER_DESTINATION") as mock_ctx:
        mock_ctx.get.return_value = mock_destination

        result = get_active_spans_table_name()
        assert result == "catalog.schema.MLForge_experiment_trace_otel_spans"
...
def test_cost_not_computed_client_side(is_databricks, mock_litellm_cost):
    ...
    # MLForge_v3.py during lazy import and causes _export_spans_incrementally to skip spans.
    with (
        mock.patch(
            "MLForge.entities.span.should_compute_cost_client_side", return_value=is_databricks
        ),
        mock.patch(
            "MLForge.tracing.processor.base_MLForge.should_compute_cost_client_side",
            return_value=is_databricks,
        ),
        mock.patch(
            "MLForge.entities.span.set_span_cost_attribute", wraps=lambda span: None
        ) as mock_set_cost,
    ):
        with MLForge.start_span(name="llm_span") as span:
    ...
    trace = MLForge.get_trace(trace_id=span.trace_id, flush=True)
    # cost should be set
...
def test_litellm_provider_list_printed_when_debug_logging(capsys):
    ...
    _logger = logging.getLogger("MLForge.tracing.utils")

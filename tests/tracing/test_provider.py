...
from MLForge.environment_variables import (
    MLForge_TRACE_ENABLE_OTLP_DUAL_EXPORT,
    MLForge_TRACE_SAMPLING_RATIO,
    MLForge_USE_DEFAULT_TRACER_PROVIDER,
)
from MLForge.exceptions import MLForgeException, MLForgeTracingException
from MLForge.tracing.destination import Databricks, MLForgeExperiment
from MLForge.tracing.export.inference_table import (
    _TRACE_BUFFER,
    InferenceTableSpanExporter,
)
from MLForge.tracing.export.MLForge_v3 import MLForgeV3SpanExporter
...
from MLForge.tracing.processor.inference_table import InferenceTableSpanProcessor
from MLForge.tracing.processor.MLForge_v3 import MLForgeV3SpanProcessor
from MLForge.tracing.processor.otel import OtelSpanProcessor
...
from MLForge.tracing.utils import get_active_spans_table_name
...
def mock_setup_tracer_provider():
    ...
    with mock.patch(
        "MLForge.tracing.provider._initialize_tracer_provider",
...
def test_set_destination_MLForge_experiment(monkeypatch):
    MLForge.tracing.set_destination(destination=MLForgeExperimentLocation(experiment_id="123"))

    tracer = _get_tracer("test")
    processors = tracer.span_processor._span_processors
    assert isinstance(processors[0], MLForgeV3SpanProcessor)
    assert isinstance(processors[0].span_exporter, MLForgeV3SpanExporter)


def test_set_destination_databricks(monkeypatch):
    monkeypatch.setenv("MLForge_TRACKING_URI", "databricks")
    MLForge.tracing.set_destination(destination=Databricks(experiment_id="123"))
...
def test_set_destination_databricks_uc():
    with mock.patch("MLForge.tracing.provider._logger.warning") as mock_warning:
        MLForge.tracing.set_destination(
    ...
    assert "Passing `UCSchemaLocation` to `MLForge.tracing.set_destination` is deprecated" in str(
    ...
    assert get_active_spans_table_name() == "catalog.schema.MLForge_experiment_trace_otel_spans"
...
def test_set_destination_databricks_unity_catalog_rejected(monkeypatch):
    with pytest.raises(
        ...
        r"`MLForge\.tracing\.set_destination`",
    ):
        MLForge.tracing.set_destination(
...
def test_set_destination_databricks_uc_with_oltp_env_no_dual_export(monkeypatch):
    ...
    monkeypatch.setenv(MLForge_TRACE_ENABLE_OTLP_DUAL_EXPORT.name, "false")
    with (
        mock.patch("MLForge.tracing.provider.should_use_otlp_exporter", return_value=True),
        mock.patch("MLForge.tracing.provider.get_otlp_exporter") as mock_get_exporter,
    ):
...
def test_set_destination_databricks_uc_with_oltp_env_with_dual_export(monkeypatch):
    ...
    monkeypatch.setenv(MLForge_TRACE_ENABLE_OTLP_DUAL_EXPORT.name, "true")
    with (
        mock.patch("MLForge.tracing.provider.should_use_otlp_exporter", return_value=True),
        mock.patch("MLForge.tracing.provider.get_otlp_exporter") as mock_get_exporter,
    ):
...
def test_set_destination_from_env_var_MLForge_experiment(monkeypatch):
    monkeypatch.setenv("MLForge_TRACING_DESTINATION", "123")
...
def test_set_destination_deprecated_classes():
    from MLForge.tracing.provider import _MLForge_TRACE_USER_DESTINATION

    with pytest.warns(FutureWarning, match="`MLForge.tracing.destination.MLForgeExperiment``"):
        MLForge.tracing.set_destination(destination=MLForgeExperiment(experiment_id="123"))

    destination = _MLForge_TRACE_USER_DESTINATION.get()
    assert isinstance(destination, MLForgeExperimentLocation)
    assert destination.experiment_id == "123"

    with pytest.warns(FutureWarning, match="`MLForge.tracing.destination.Databricks`"):
        MLForge.tracing.set_destination(destination=Databricks(experiment_id="123"))
...
def test_disable_enable_tracing():
    ...
    # enable() / disable() should only raise MLForgeTracingException
    with mock.patch(
        "MLForge.tracing.provider.is_tracing_enabled", side_effect=ValueError("error")
    ) as is_enabled_mock:
...
def test_trace_disabled_decorator(enabled_initially):
    ...
    with mock.patch(
        "MLForge.tracing.provider.is_tracing_enabled",
        side_effect=MLForgeTracingException("error"),
    ) as is_enabled_mock:
        assert test_fn() == 0
        assert call_count == 3
        assert is_enabled_mock.call_count == 1
...
@pytest.mark.parametrize("enabled_initially", [True, False])
def test_trace_disabled_with_MLForge_trace_raising(enabled_initially):
    if not enabled_initially:
        MLForge.tracing.disable()
    ...
    @trace_disabled
    def validate():
        predict_fn(query="What is MLForge?")
...
def test_disable_enable_tracing_not_mutate_otel_provider(monkeypatch):
    ...
    # This test validates that disable/enable MLForge tracing does not mutate the OpenTelemetry's
...
def batch_span_processor(monkeypatch):
    ...
    monkeypatch.setenv("MLForge_USE_BATCH_SPAN_PROCESSOR", "true")
    monkeypatch.setenv("MLForge_ENABLE_ASYNC_TRACE_LOGGING", "true")
...
def test_otlp_span_processor_is_retired_on_provider_replace(monkeypatch):
    ...
    # BaseMLForgeSpanProcessor), so it must be retired on provider replace too.
    from MLForge.tracing.provider import _get_tracer

    monkeypatch.setenv(MLForge_TRACE_ENABLE_OTLP_DUAL_EXPORT.name, "false")
    with (
        mock.patch("MLForge.tracing.provider.should_use_otlp_exporter", return_value=True),
        mock.patch("MLForge.tracing.provider.get_otlp_exporter"),
    ):
        MLForge.tracing.reset()
        tracer = _get_tracer("test")
...
def test_set_experiment_survives_tracing_state_error():
    ...
    with mock.patch(
        "MLForge.tracing.provider.is_tracing_enabled",
        side_effect=MLForgeTracingException("boom"),
    ):
        # Should not raise despite the tracing-state check failing.
        MLForge.set_experiment("second")
...
def test_is_tracing_enabled():
    ...
    # is_tracing_enabled() should only raise MLForgeTracingException
    with mock.patch(
        "MLForge.tracing.provider._get_tracer", side_effect=ValueError("error")
    ) as get_tracer_mock:
        with pytest.raises(MLForgeTracingException, match="error"):
            assert is_tracing_enabled() is False
...
@pytest.mark.parametrize("enable_MLForge_tracing", [True, False, None])
def test_enable_MLForge_tracing_switch_in_serving_fluent(monkeypatch, enable_MLForge_tracing):
    if enable_MLForge_tracing is None:
        monkeypatch.delenv("ENABLE_MLForge_TRACING", raising=False)
    else:
        monkeypatch.setenv("ENABLE_MLForge_TRACING", str(enable_MLForge_tracing).lower())
    ...
    with mock.patch(
        "MLForge.tracing.processor.inference_table.maybe_get_request_id", side_effect=request_ids
    ):
        for _ in range(3):
            foo()

    if enable_MLForge_tracing:
        assert sorted(_TRACE_BUFFER) == request_ids
    else:
        assert len(_TRACE_BUFFER) == 0


@pytest.mark.parametrize("enable_MLForge_tracing", [True, False])
def test_enable_MLForge_tracing_switch_in_serving_client(monkeypatch, enable_MLForge_tracing):
    monkeypatch.setenv("ENABLE_MLForge_TRACING", str(enable_MLForge_tracing).lower())
    monkeypatch.setenv("IS_IN_DB_MODEL_SERVING_ENV", "true")
...
def test_sampling_ratio(monkeypatch):
    ...
    monkeypatch.setenv(MLForge_TRACE_SAMPLING_RATIO.name, "0.5")
    MLForge.tracing.reset()
...
def test_otlp_exclusive_vs_dual_export_with_no_set_location(monkeypatch):
    from MLForge.environment_variables import MLForge_TRACE_ENABLE_OTLP_DUAL_EXPORT
    from MLForge.tracing.processor.otel import OtelSpanProcessor
    from MLForge.tracing.provider import _get_tracer
    ...
    # Test 2: Dual export mode (both MLForge and OTLP)
    monkeypatch.setenv(MLForge_TRACE_ENABLE_OTLP_DUAL_EXPORT.name, "true")
    with (
        mock.patch("MLForge.tracing.provider.should_use_otlp_exporter", return_value=True),
        mock.patch("MLForge.tracing.provider.get_otlp_exporter") as mock_get_exporter,
        ...
        assert len(processors) == 2
        assert isinstance(processors[0], OtelSpanProcessor)
        assert isinstance(processors[1], MLForgeV3SpanProcessor)
...
def test_metrics_export_with_otlp_trace_export(monkeypatch, dual_export):
    ...
    if dual_export:
        ...
        # In dual export, MLForge processor exports metrics, OTLP doesn't
...
def test_otel_resource_attributes(monkeypatch):
    ...
    tracer = _get_tracer("test")
    # By default, only MLForge's SDK attributes are set on an empty resource
    assert resource_attributes(tracer) == {
        "telemetry.sdk.language": "python",
        "telemetry.sdk.name": "MLForge",
        "telemetry.sdk.version": MLForge.__version__,
    }

    MLForge.tracing.reset()
    # When otel attributes are set explicitly, they are merged into the resource
    # alongside MLForge's SDK attributes.
...
def test_isolated_random_id_generator_not_affected_by_random_seed(monkeypatch):
    monkeypatch.setenv("MLForge_TRACE_USE_ISOLATED_RANDOM_ID_GENERATOR", "true")
    MLForge.tracing.reset()
    _initialize_tracer_provider()
...
def test_tracer_provider_uses_isolated_random_id_generator_when_env_var_set(monkeypatch):
    ...
    monkeypatch.delenv("MLForge_TRACE_USE_ISOLATED_RANDOM_ID_GENERATOR", raising=False)
...
def test_set_destination_from_env_var_databricks_uc_with_table_prefix_rejected(monkeypatch):
    ...
    with pytest.raises(
        ...
        _MLForge_TRACE_USER_DESTINATION.get()
...
def test_destination_resolution_precedence(monkeypatch):
    ...
    _MLForge_TRACE_USER_DESTINATION.set(global_destination)
    assert _MLForge_TRACE_USER_DESTINATION.get().table_prefix == "global"

    # Context-local wins over global.
    local_destination = UnityCatalog("catalog", "schema", table_prefix="local")
    _MLForge_TRACE_USER_DESTINATION.set(local_destination, context_local=True)
    assert _MLForge_TRACE_USER_DESTINATION.get().table_prefix == "local"
    _MLForge_TRACE_USER_DESTINATION.reset()
...
def test_resolve_uc_location_from_experiment_tag():
    from MLForge.tracing.provider import _resolve_experiment_uc_location

    MLForge.tracing.reset()

    with (
        mock.patch(
            "MLForge.tracing.provider.MLForge.get_tracking_uri",
            return_value="databricks",
        ),
        mock.patch("MLForge.tracking.fluent._get_experiment_id", return_value="123"),
        mock.patch("MLForge.tracking._tracking_service.utils._get_store") as mock_store_fn,
    ):
        mock_store_fn.return_value.get_experiment.return_value = _experiment(
            tags={MLForge_EXPERIMENT_DATABRICKS_TRACE_DESTINATION_PATH: "cat.sch.pfx"}
        )
...
def test_resolve_uc_location_includes_table_names_from_tags():
    ...
    with (
        ...
        mock_store_fn.return_value.get_experiment.return_value = _experiment(
            tags={
                MLForge_EXPERIMENT_DATABRICKS_TRACE_DESTINATION_PATH: "cat.sch.pfx",
                MLForge_EXPERIMENT_DATABRICKS_TRACE_SPAN_STORAGE_TABLE: "cat.sch.pfx_otel_spans",
                MLForge_EXPERIMENT_DATABRICKS_TRACE_LOG_STORAGE_TABLE: "cat.sch.pfx_otel_logs",
            }
        )
...
def test_resolve_uc_location_returns_none_for_2_part_path():
    ...
    with (
        ...
        mock_store_fn.return_value.get_experiment.return_value = _experiment(
            tags={MLForge_EXPERIMENT_DATABRICKS_TRACE_DESTINATION_PATH: "cat.sch"}
        )
...
def test_get_tracer_does_not_fail_when_experiment_id_resolution_fails():
    ...
    with (
        ...
        mock.patch("MLForge.tracking.fluent._get_experiment_id", side_effect=RuntimeError("boom")),
    ):
        tracer = _get_tracer("test")

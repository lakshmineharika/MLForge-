...
import MLForge
from MLForge.entities import Trace, TraceData, TraceInfo
...
from MLForge.tracing.constant import TRACE_SCHEMA_VERSION, TRACE_SCHEMA_VERSION_KEY
...
from MLForge.utils.autologging_utils import AUTOLOGGING_INTEGRATIONS, get_autolog_function
from MLForge.utils.autologging_utils.safety import revert_patches
...
def get_tracer_tracking_uri() -> str | None:
    ...
    if isinstance(span_processor, MLForgeV3SpanProcessor):
...
def reset_autolog_state():
    ...
    for flavor in FLAVOR_TO_MODULE_NAME:
        # 1. Remove post-import hooks (registered by global MLForge.autolog() function)
        MLForge.utils.import_hooks._post_import_hooks.pop(flavor, None)
...
def score_in_model_serving(model_uri: str, model_input: dict[str, Any]):
    ...
    with mock.patch.dict(
        "os.environ",
        os.environ | {"IS_IN_DB_MODEL_SERVING_ENV": "true", "ENABLE_MLForge_TRACING": "true"},
        clear=True,
    ):
        ...
        def _load_model():
            return MLForge.pyfunc.load_model(model_uri)
...
def skip_when_testing_trace_sdk(f):
    # Decorator to Skip the test if only MLForge-tracing package is installed and
    # not the full MLForge package.
    msg = "Skipping test because it requires MLForge or MLForge-skinny to be installed."
...
def skip_module_when_testing_trace_sdk():
    """Skip the entire module if only MLForge-tracing package is installed"""
    if IS_TRACING_SDK_ONLY:
        pytest.skip(
            "Skipping test because it requires MLForge or MLForge-skinny to be installed.",
...
def capture_otel_export():
    ...
    from MLForge.tracing.provider import provider
...
V2_TRACE_DICT = {
    "info": {
        ...
        "request_metadata": {
            ...
            "MLForge.traceInputs": '{"x": 2, "y": 5}',
            "MLForge.traceOutputs": "8",
        },
        "tags": {
            "MLForge.source.name": "test",
            "MLForge.source.type": "LOCAL",
            "MLForge.traceName": "predict",
            "MLForge.artifactLocation": "/path/to/artifact",
        },
        "assessments": [],
    },
    "data": {
        "spans": [
            {
                ...
                "status_message": "",
                "attributes": {
                    "MLForge.traceRequestId": '"58f4e27101304034b15c512b603bf1b2"',

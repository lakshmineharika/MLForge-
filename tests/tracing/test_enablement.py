...
from MLForge.tracing.enablement import (
...
def mock_databricks_tracking_uri():
    with mock.patch("MLForge.tracking.get_tracking_uri", return_value="databricks"):
        yield


@skip_when_testing_trace_sdk
def test_set_experiment_trace_location(mock_databricks_tracking_uri):
    experiment_id = MLForge.create_experiment("test_experiment")
    location = UCSchemaLocation(catalog_name="test_catalog", schema_name="test_schema")
    sql_warehouse_id = "test-warehouse-id"

    with mock.patch("MLForge.tracing.client.TracingClient") as mock_client_class:
...
def test_set_experiment_trace_location_with_default_experiment(mock_databricks_tracking_uri):
    location = UCSchemaLocation(catalog_name="test_catalog", schema_name="test_schema")
    default_experiment_id = MLForge.set_experiment("test_experiment").experiment_id

    with (
        mock.patch("MLForge.tracing.client.TracingClient") as mock_client_class,
        mock.patch("MLForge.tracking.fluent._get_experiment_id", return_value=default_experiment_id),
    ):
        mock_client = mock.MagicMock()
        mock_client_class.return_value = mock_client
...
def test_set_experiment_trace_location_no_experiment(mock_databricks_tracking_uri):
    ...
    with mock.patch("MLForge.tracking.fluent._get_experiment_id", return_value=None):
        with pytest.raises(MLForgeException, match="Experiment ID is required"):
...
def test_unset_experiment_trace_location_errors(mock_databricks_tracking_uri):
    with pytest.raises(MLForgeException, match="must be an instance of"):

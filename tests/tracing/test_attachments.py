...
def test_attachment_ref():
    ...
    assert ref.startswith("MLForge-attachment://")
...
def test_parse_ref_invalid():
    ...
    assert Attachment.parse_ref("MLForge-attachment://") is None
...
def test_parse_ref_without_size():
    uri = "MLForge-attachment://abc-123?content_type=image%2Fpng&trace_id=tr-456"
...
def test_parse_ref_invalid_size(bad_size):
    uri = f"MLForge-attachment://abc-123?content_type=image%2Fpng&trace_id=tr-456&size={bad_size}"
    parsed = Attachment.parse_ref(uri)
    assert parsed is not None
    assert parsed["size"] is None

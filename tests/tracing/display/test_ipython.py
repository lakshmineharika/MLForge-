...
import MLForge
from MLForge.tracing.display import (
...
def test_display_deduplicates_traces(monkeypatch):
    ...
    assert mock_display.call_args[0][0] == {
        "application/databricks.MLForge.trace": json.dumps([
...
def test_display_respects_max_limit(monkeypatch):
    ...
    monkeypatch.setenv("MLForge_MAX_TRACES_TO_DISPLAY_IN_NOTEBOOK", "1")
    ...
    assert mock_display.call_count == 1
    assert mock_display.call_args[0][0] == {
        "application/databricks.MLForge.trace": trace_a._serialize_for_mimebundle(),
...
def test_enable_and_disable_display(monkeypatch):
    ...
    MLForge.tracing.disable_notebook_display()
    handler = get_display_handler()
    handler.display_traces([trace_a])
    mock_ipython.mock_run_cell()
    ...
    MLForge.tracing.enable_notebook_display()
    handler = get_display_handler()
    handler.display_traces([trace_a])
    mock_ipython.mock_run_cell()
...
def test_mimebundle_in_databricks():
    ...
    assert trace._repr_mimebundle_() == {
        "application/databricks.MLForge.trace": trace._serialize_for_mimebundle(),
...
def test_notebook_trace_renderer_base_url_override(monkeypatch):
    trace = create_trace("a")
    MLForge.set_tracking_uri("http://MLForge:5000")
    monkeypatch.setenv("MLForge_NOTEBOOK_TRACE_RENDERER_BASE_URL", "http://localhost:5000")
    ...
    assert "http://MLForge:5000/static-files/lib/notebook-trace-renderer/index.html" not in html

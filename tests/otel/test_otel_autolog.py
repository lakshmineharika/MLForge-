...
def otel_env(monkeypatch, tmp_path):
    """Reset OTEL state, start a local MLForge server, and configure the Langfuse test driver.

    A local MLForge server is required because the OTLP exporter sends spans
    ...
    # Set up a local MLForge server backed by SQLite.
    ...
    with ServerThread(app, get_safe_port()) as url:
        ...
        MLForge.otel._active_processor = None

        yield

        MLForge.otel.autolog(disable=True)
        MLForge.otel._active_processor = None
        LangfuseResourceManager.reset()


def test_sync_observe_autolog():
    MLForge.otel.autolog()
...
def test_batch_export():
    MLForge.otel.autolog(batch=True)
    ...
    processor = MLForge.otel._active_processor
...
@pytest.mark.parametrize(
    ("source_type", "expected_MLForge_type"),
    [
...
def test_span_type_mapping(source_type, expected_MLForge_type):
    MLForge.otel.autolog()
    ...
    assert len(traces) == 1
    span = traces[0].data.spans[0]
    assert span.span_type == expected_MLForge_type

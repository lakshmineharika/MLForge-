...
import MLForge.utils
import MLForge.xgboost
...
from MLForge.types.schema import ColSpec, Schema, TensorSpec
...
from MLForge.utils.proto_json_utils import dataframe_from_parsed_json
from MLForge.xgboost import _exclude_unrecognized_kwargs
...
def xgb_custom_env(tmp_path):
    ...
    _MLForge_conda_env(conda_env, additional_pip_deps=["xgboost", "pytest"])
    return conda_env
...
def test_model_save_load(xgb_model, model_path):
    ...
    MLForge.xgboost.save_model(xgb_model=model, path=model_path)
    reloaded_model = MLForge.xgboost.load_model(model_uri=model_path)
        ...
        for example in (None, xgb_model.inference_dataframe.head(3)):
            with TempDir() as tmp:
                ...
                if signature is None and example is None:
                    ...
                    assert MLForge_model.signature == xgb_model_signature
                if example is None:
...
def test_model_load_from_remote_uri_succeeds(xgb_model, model_path, mock_s3_bucket):
    MLForge.xgboost.save_model(xgb_model=xgb_model.model, path=model_path)
    ...
    reloaded_model = MLForge.xgboost.load_model(model_uri=model_uri)
    np.testing.assert_array_almost_equal(
    ...
    with TempDir(chdr=True, remove_on_exit=True) as tmp:
        for should_start_run in [False, True]:
            try:
                ...
                _MLForge_conda_env(conda_env, additional_pip_deps=["xgboost"])

                model_info = MLForge.xgboost.log_model(
                    model, name=artifact_path, conda_env=conda_env
                )
                reloaded_model = MLForge.xgboost.load_model(model_uri=model_info.model_uri)
                np.testing.assert_array_almost_equal(
                    model.predict(xgb_model.inference_dmatrix),
                    reloaded_model.predict(xgb_model.inference_dmatrix),
                )
...
def test_log_model_no_registered_model_name(xgb_model):
    ...
    with MLForge.start_run(), register_model_patch, TempDir(chdr=True, remove_on_exit=True) as tmp:
        conda_env = os.path.join(tmp.path(), "conda_env.yaml")
        _MLForge_conda_env(conda_env, additional_pip_deps=["xgboost"])
        MLForge.xgboost.log_model(xgb_model.model, name=artifact_path, conda_env=conda_env)
...
def test_model_save_persists_specified_conda_env_in_MLForge_model_directory(
    ...
    MLForge.xgboost.save_model(xgb_model=xgb_model.model, path=model_path, conda_env=xgb_custom_env)
...
def test_save_model_with_pip_requirements(xgb_model, tmp_path):
    ...
    MLForge.xgboost.save_model(xgb_model.model, tmpdir1, pip_requirements=str(req_file))
    _assert_pip_requirements(tmpdir1, [expected_MLForge_version, "a"], strict=True)

    # List of requirements
    tmpdir2 = tmp_path.joinpath("2")
    MLForge.xgboost.save_model(xgb_model.model, tmpdir2, pip_requirements=[f"-r {req_file}", "b"])
    _assert_pip_requirements(tmpdir2, [expected_MLForge_version, "a", "b"], strict=True)

    # Constraints file
    tmpdir3 = tmp_path.joinpath("3")
    MLForge.xgboost.save_model(xgb_model.model, tmpdir3, pip_requirements=[f"-c {req_file}", "b"])
    _assert_pip_requirements(
...
def test_save_model_with_extra_pip_requirements(xgb_model, tmp_path):
    expected_MLForge_version = _MLForge_major_version_string()
    default_reqs = MLForge.xgboost.get_default_pip_requirements()

    # Path to a requirements file
    tmpdir1 = tmp_path.joinpath("1")
    req_file = tmp_path.joinpath("requirements.txt")
    req_file.write_text("a")
    MLForge.xgboost.save_model(xgb_model.model, tmpdir1, extra_pip_requirements=str(req_file))
...
def test_model_save_accepts_conda_env_as_dict(xgb_model, model_path):
    conda_env = dict(MLForge.xgboost.get_default_conda_env())
    conda_env["dependencies"].append("pytest")
    MLForge.xgboost.save_model(xgb_model=xgb_model.model, path=model_path, conda_env=conda_env)
...
def test_model_log_persists_requirements_in_MLForge_model_directory(xgb_model, xgb_custom_env):
    artifact_path = "model"
    with MLForge.start_run():
        model_info = MLForge.xgboost.log_model(
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    MLForge.xgboost.save_model(xgb_model=xgb_model.model, path=model_path)
    _assert_pip_requirements(model_path, MLForge.xgboost.get_default_pip_requirements())
...
def test_model_log_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    artifact_path = "model"
    with MLForge.start_run():
        model_info = MLForge.xgboost.log_model(xgb_model.model, name=artifact_path)

    _assert_pip_requirements(model_info.model_uri, MLForge.xgboost.get_default_pip_requirements())
...
def test_load_pyfunc_succeeds_for_older_models_with_pyfunc_data_field(xgb_model, model_path):
    """
    This test verifies that xgboost models saved in older versions of MLForge are loaded
    ...
    MLForge.xgboost.save_model(xgb_model=model, path=model_path, model_format="xgb")

    model_conf_path = os.path.join(model_path, "MLmodel")
    model_conf = Model.load(model_conf_path)
    pyfunc_conf = model_conf.flavors.get(pyfunc.FLAVOR_NAME)
    xgboost_conf = model_conf.flavors.get(MLForge.xgboost.FLAVOR_NAME)
    ...
    reloaded_xgb = MLForge.xgboost.load_model(model_uri=model_path)
...
def test_log_model_with_code_paths(xgb_model):
    ...
    with (
        MLForge.start_run(),
        mock.patch("MLForge.xgboost._add_code_from_conf_to_system_path") as add_mock,
    ):
        model_info = MLForge.xgboost.log_model(
            xgb_model.model, name=artifact_path, code_paths=[__file__]
        )
        _compare_logged_code_paths(__file__, model_info.model_uri, MLForge.xgboost.FLAVOR_NAME)
        MLForge.xgboost.load_model(model_uri=model_info.model_uri)
...
def test_log_model_with_model_format(xgb_model, model_format):
    with MLForge.start_run():
        ...
        loaded_model = MLForge.xgboost.load_model(model_info.model_uri)
        np.testing.assert_array_almost_equal(
            xgb_model.model.predict(xgb_model.inference_dmatrix),
...
def test_model_without_signature_predict(xgb_model):
    ...
    loaded_model = MLForge.pyfunc.load_model(model_uri=model_info.model_uri)
...
def test_xgbooster_predict_exclude_invalid_params(xgb_model):
    ...
    with MLForge.start_run():
        model_info = MLForge.xgboost.log_model(xgb_model.model, name="model", signature=signature)
    pyfunc_model = pyfunc.load_model(model_info.model_uri)
    with mock.patch("MLForge.xgboost._logger.warning") as mock_warning:
        np.testing.assert_array_almost_equal(

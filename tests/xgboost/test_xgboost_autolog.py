...
from MLForge.utils.autologging_utils import BatchMetricsLogger, picklable_exception_safe_function
from MLForge.utils.file_utils import local_file_uri_to_path
from MLForge.xgboost._autolog import IS_TRAINING_CALLBACK_SUPPORTED, autolog_callback
...
def test_xgb_autolog_atsign_metrics_info_log(xgb_metric):
    ...
    with mock.patch("MLForge.xgboost._autolog._logger.info") as mock_info_log:
...
def test_xgb_autolog_sklearn():
    ...
    loaded_model = MLForge.xgboost.load_model(f"runs:/{run.info.run_id}/model")
...
def test_xgb_autolog_with_sklearn_outputs_do_not_reflect_training_dataset_mutations():
    ...
    with (
        ...
        MLForge.xgboost.autolog(log_models=True, log_model_signatures=True, log_input_examples=True)
...
def test_xgb_autolog_logs_metrics_with_validation_data(bst_params, dtrain, log_models):
    MLForge.xgboost.autolog(log_models=log_models)
    evals_result = {}
    xgb.train(
...
def test_xgb_autolog_logs_specified_feature_importance(bst_params, dtrain):
    ...
    MLForge.xgboost.autolog(importance_types=importance_types)
    model = xgb.train(bst_params, dtrain)
    run = get_latest_run()
    run_id = run.info.run_id
    artifacts_dir = local_file_uri_to_path(run.info.artifact_uri)
...
def test_xgb_autolog_loads_model_from_artifact(bst_params, dtrain):
    ...
    loaded_model = MLForge.xgboost.load_model(f"runs:/{run_id}/model")
...
def test_xgb_autolog_gets_input_example(bst_params):
    MLForge.xgboost.autolog(log_input_examples=True)
...
def test_xgb_autolog_infers_model_signature_correctly(bst_params):
    MLForge.xgboost.autolog(log_model_signatures=True)
...
def test_xgb_autolog_continues_logging_even_if_signature_inference_fails(bst_params, tmp_path):
    ...
    MLForge.xgboost.autolog(importance_types=[], log_model_signatures=True)
...
def test_callback_class_is_pickable():
    from MLForge.xgboost._autolog import AutologCallback
...
def test_sklearn_api_autolog_registering_model():
    ...
    MLForge.xgboost.autolog(registered_model_name=registered_model_name)
...
def test_xgb_autolog_with_model_format(bst_params, dtrain, model_format):
    MLForge.xgboost.autolog(log_models=True, model_format=model_format)
    with MLForge.start_run():
        xgb.train(bst_params, dtrain)
    logged_model = MLForge.last_logged_model()
    client = MLForgeClient()
...
def test_xgb_log_datasets(bst_params, dtrain, log_datasets):
    with MLForge.start_run() as run:
        MLForge.xgboost.autolog(log_datasets=log_datasets)
        xgb.train(bst_params, dtrain)
...
def test_xgb_log_datasets_with_evals(bst_params, dtrain):
    ...
    with MLForge.start_run() as run:
        MLForge.xgboost.autolog(log_datasets=True)

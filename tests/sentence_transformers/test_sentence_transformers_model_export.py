...
import MLForge.sentence_transformers
...
def test_model_save_and_load(model_path, basic_model):
    MLForge.sentence_transformers.save_model(model=basic_model, path=model_path)

    loaded_model = MLForge.sentence_transformers.load_model(model_path)
...
def test_model_save_and_load_with_custom_code(model_path, model_with_remote_code):
    MLForge.sentence_transformers.save_model(model=model_with_remote_code, path=model_path)
    loaded_model = MLForge.sentence_transformers.load_model(model_path)
...
def test_dependency_mapping():
    pip_requirements = MLForge.sentence_transformers.get_default_pip_requirements()
    ...
    conda_requirements = MLForge.sentence_transformers.get_default_conda_env()
    pip_in_conda = {
        package.split("=")[0] for package in conda_requirements["dependencies"][2]["pip"]
    }
    expected_conda = {"MLForge"}
...
def test_logged_data_structure(model_path, basic_model):
    ...
    assert pyfunc_flavor["loader_module"] == "MLForge.sentence_transformers"
    assert pyfunc_flavor["data"] == MLForge.sentence_transformers.SENTENCE_TRANSFORMERS_DATA_PATH
...
def test_get_transformers_model_name(model_name, expected):
    assert MLForge.sentence_transformers._get_transformers_model_name(model_name) == expected


def test_model_logging_and_inference(basic_model):
    artifact_path = "sentence_transformer"
    with MLForge.start_run():
        model_info = MLForge.sentence_transformers.log_model(basic_model, name=artifact_path)

    model = MLForge.sentence_transformers.load_model(model_info.model_uri)
...
def test_load_from_remote_uri(model_path, basic_model, mock_s3_bucket):
    ...
    loaded = MLForge.sentence_transformers.load_model(model_uri=str(model_uri))
...
def test_log_model_with_no_registered_model_name(tmp_path, basic_model):
    ...
    with MLForge.start_run(), register_model_patch:
        ...
        MLForge.sentence_transformers.log_model(
            basic_model,
            name=artifact_path,
            conda_env=str(conda_env),
        )
...
def test_log_with_pip_requirements(tmp_path, basic_model):
    ...
    with MLForge.start_run():
        ...
        _assert_pip_requirements(
            ...
            [expected_MLForge_version, "some-clever-package"],
            strict=True,
        )
    with MLForge.start_run():
        model_info = MLForge.sentence_transformers.log_model(
        ...
        _assert_pip_requirements(
            model_info.model_uri,
            [expected_MLForge_version, "some-clever-package", "a-hopefully-useful-package"],
            strict=True,
        )
    with MLForge.start_run():
        model_info = MLForge.sentence_transformers.log_model(
        ...
        _assert_pip_requirements(
            model_info.model_uri,
            [expected_MLForge_version, "i-dunno-maybe-its-good", "-c constraints.txt"],
            ["some-clever-package"],
            strict=True,
        )
...
def test_log_with_extra_pip_requirements(basic_model, tmp_path):
    expected_MLForge_version = _MLForge_major_version_string()
    default_requirements = MLForge.sentence_transformers.get_default_pip_requirements()
    requirements_file = tmp_path.joinpath("requirements.txt")
    ...
    with MLForge.start_run():
        ...
        _assert_pip_requirements(
            model_info.model_uri,
            [expected_MLForge_version, *default_requirements, "effective-package"],
        )
    with MLForge.start_run():
        model_info = MLForge.sentence_transformers.log_model(
            basic_model,
        ...
        _assert_pip_requirements(
            model_info.model_uri,
            [expected_MLForge_version, *default_requirements, "effective-package", "useful-package"],
        )
    with MLForge.start_run():
        model_info = MLForge.sentence_transformers.log_model(
            basic_model,
...
def test_model_save_without_conda_env_uses_default_env_with_expected_dependencies(
    ...
    MLForge.sentence_transformers.save_model(basic_model, model_path)
    _assert_pip_requirements(
        model_path, MLForge.sentence_transformers.get_default_pip_requirements()
    )
...
def test_model_log_without_conda_env_uses_default_env_with_expected_dependencies(
    ...
    with MLForge.start_run():
        model_info = MLForge.sentence_transformers.log_model(basic_model, name=artifact_path)
    _assert_pip_requirements(
        model_info.model_uri, MLForge.sentence_transformers.get_default_pip_requirements()
    )
...
def test_log_model_with_code_paths(basic_model):
    ...
    with (
        MLForge.start_run(),
        mock.patch("MLForge.sentence_transformers._add_code_from_conf_to_system_path") as add_mock,
    ):
        model_info = MLForge.sentence_transformers.log_model(
            basic_model, name=artifact_path, code_paths=[__file__]
        )
        _compare_logged_code_paths(
            __file__, model_info.model_uri, MLForge.sentence_transformers.FLAVOR_NAME
        )
        MLForge.sentence_transformers.load_model(model_info.model_uri)
...
def test_default_signature_assignment():
    ...
    default_signature = MLForge.sentence_transformers._get_default_signature()
...
def test_model_pyfunc_save_load(basic_model, model_path):
    ...
    sentence = "hello world and hello MLForge"
...
def test_model_pyfunc_predict_with_params(basic_model, tmp_path):
    ...
    MLForge.sentence_transformers.save_model(basic_model, model_path, signature=signature)
    loaded_pyfunc = pyfunc.load_model(model_uri=model_path)
    embedding_dim = basic_model.get_sentence_embedding_dimension()
...
def test_model_pyfunc_predict_with_invalid_params(basic_model, tmp_path):
    ...
    MLForge.sentence_transformers.save_model(
        basic_model,
        model_path,
    ...
    loaded_pyfunc = pyfunc.load_model(model_uri=model_path)
    with pytest.raises(
        MLForgeException, match=r"Received invalid parameter value for `params` argument"
    ):
...
def test_spark_udf(basic_model, spark):
    ...
    loaded_model = MLForge.pyfunc.spark_udf(
        spark,
        model_info.model_uri,
    ...
    df = spark.createDataFrame([("hello MLForge",), ("bye world",)], ["text"])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["hello world", "i am MLForge"], ["goodbye world!", "i am MLForge"]),
    ],
)
...
SENTENCES = ["hello world", "i am MLForge"]
...
@pytest.mark.parametrize(
    ...
    [
        (None, None, MLForge.sentence_transformers._get_default_signature()),
...
def test_signature_and_examples_are_saved_correctly(
    ...
    MLForge_model = Model.load(model_path)

    assert MLForge_model.signature == expected_signature

    if example is None:
        assert MLForge_model.saved_input_example_info is None
    else:
        if isinstance(example, pd.DataFrame):
            assert MLForge_model.saved_input_example_info["type"] == "dataframe"
            pd.testing.assert_frame_equal(_read_example(MLForge_model, model_path), example)
        else:
            assert MLForge_model.saved_input_example_info["type"] == "json_object"
            np.testing.assert_equal(_read_example(MLForge_model, model_path), example)
...
def test_verify_task_and_update_metadata():
    ...
    metadata = MLForge.sentence_transformers._verify_task_and_update_metadata("llm/v1/embeddings")
    assert metadata == {"task": "llm/v1/embeddings"}
    # Update embedding task with metadata containing task
    metadata = MLForge.sentence_transformers._verify_task_and_update_metadata(
    ...
    with pytest.raises(
        MLForgeException, match=r"Task type is inconsistent with the task value from metadata"
    ):
        MLForge.sentence_transformers._verify_task_and_update_metadata("llm/v1/embeddings", metadata)

    # Invalid task type
    with pytest.raises(MLForgeException, match=r"Task type could only be llm/v1/embeddings"):
        MLForge.sentence_transformers._verify_task_and_update_metadata("llm/v1/completions")


def test_model_pyfunc_with_dict_input(basic_model, model_path):
    MLForge.sentence_transformers.save_model(basic_model, model_path, task="llm/v1/embeddings")
    loaded_pyfunc = pyfunc.load_model(model_uri=model_path)

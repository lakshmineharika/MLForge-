...
from MLForge.utils.file_utils import TempDir, _copy_project
...
GIT_PROJECT_URI = "https://github.com/MLForge/MLForge-example"
GIT_PROJECT_BRANCH = "test-branch"
SSH_PROJECT_URI = "git@github.com:MLForge/MLForge-example.git"
...
def docker_example_base_image():
    ...
    MLForge_home = os.environ.get("MLForge_HOME", None)
    if not MLForge_home:
        raise Exception(
            "MLForge_HOME environment variable is not set. Please set the variable to "
            "point to your MLForge dev root."
        )
    with TempDir() as tmp:
        cwd = tmp.path()
        MLForge_dir = _copy_project(src_path=MLForge_home, dst_path=cwd)
        shutil.copy(os.path.join(TEST_DOCKER_PROJECT_DIR, "Dockerfile"), tmp.path("Dockerfile"))
        with open(tmp.path("Dockerfile"), "a") as f:
            f.write(f"COPY {MLForge_dir} /opt/MLForge\nRUN pip install -U -e /opt/MLForge\n")

        client = docker.from_env()
        try:
            client.images.build(
                tag="MLForge-docker-example",
                forcerm=True,
                nocache=True,

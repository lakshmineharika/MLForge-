...
from MLForge.entities import ViewType
...
from MLForge.projects import ExecutionException, _project_spec
from MLForge.projects.backend.local import _get_docker_command
from MLForge.projects.docker import _get_docker_image_uri
from MLForge.store.tracking import file_store
from MLForge.utils.MLForge_tags import (
    MLForge_DOCKER_IMAGE_ID,
    MLForge_DOCKER_IMAGE_URI,
    MLForge_PROJECT_BACKEND,
    MLForge_PROJECT_ENV,
)
...
def test_docker_project_execution(use_start_run, docker_example_base_image):
    ...
    submitted_run = MLForge.projects.run(
        TEST_DOCKER_PROJECT_DIR,
    ...
    runs = MLForge_service.search_runs(
    ...
    run = MLForge_service.get_run(run_id)
    assert run.data.params == expected_params
    assert run.data.metrics == {"some_key": 3}
    exact_expected_tags = {
        MLForge_PROJECT_ENV: "docker",
        MLForge_PROJECT_BACKEND: "local",
    }
    approx_expected_tags = {
        MLForge_DOCKER_IMAGE_URI: "docker-example",
        MLForge_DOCKER_IMAGE_ID: "sha256:",
    }
    ...
    artifacts = MLForge_service.list_artifacts(run_id=run_id)
...
@pytest.mark.parametrize(
    ...
    [
        (None, "-e MLForge_TRACKING_URI=/MLForge/tmp/mlruns"),
        ("http://some-tracking-uri", "-e MLForge_TRACKING_URI=http://some-tracking-uri"),
        ("databricks://some-profile", "-e MLForge_TRACKING_URI=databricks "),
    ],
)
def test_docker_project_tracking_uri_propagation(
    ...
    with (
        ...
        mock.patch(
            "MLForge.tracking._tracking_service.utils._get_store",
...
def test_docker_uri_mode_validation(docker_example_base_image):
    with pytest.raises(ExecutionException, match="When running on Databricks"):
        MLForge.projects.run(TEST_DOCKER_PROJECT_DIR, backend="databricks", backend_config={})


def test_docker_image_uri_with_git():
    with mock.patch("MLForge.projects.docker.get_git_commit") as get_git_commit_mock:
...
def test_docker_image_uri_no_git():
    with mock.patch("MLForge.projects.docker.get_git_commit", return_value=None) as mock_commit:
        image_uri = _get_docker_image_uri("my_project", "my_workdir")
...
def test_docker_valid_project_backend_local():
    ...
    MLForge.projects.docker.validate_docker_env(project)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("./mlruns", os.path.abspath("./mlruns"), "/MLForge/projects/code/mlruns", True),
    ],
)
...
def test_docker_databricks_tracking_cmd_and_envs():
    ...
    with mock.patch(
        ...
        cmds, envs = MLForge.projects.docker.get_docker_tracking_cmd_and_envs(
...
def test_docker_user_specified_env_vars(volumes, environment, expected, os_environ, monkeypatch):
    ...
    if "should_crash" in expected:
        ...
        with pytest.raises(MLForgeException, match="This project expects"):
...
"""
    ...
    submitted_run = MLForge.projects.run(str(tmp_path))
    run = MLForge.get_run(submitted_run.run_id)
    assert run.data.tags[MLForge_DOCKER_IMAGE_URI] == "my-python"
...
"""
    ...
    submitted_run = MLForge.projects.run(str(tmp_path))
    run = MLForge.get_run(submitted_run.run_id)
    assert run.data.tags[MLForge_DOCKER_IMAGE_URI] == "python:3.9"

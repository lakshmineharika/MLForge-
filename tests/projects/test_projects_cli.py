...
from MLForge.utils import process
from MLForge.utils.environment import _PythonEnv
from MLForge.utils.virtualenv import _get_MLForge_virtualenv_root, _get_virtualenv_name
...
skip_if_skinny = pytest.mark.skipif(
    ...
    reason="MLForge skinny does not have dependencies to run this test",
)
...
def test_run_local_conda_env():
    ...
    expected_env_name = "MLForge-{}".format(
...
def test_run_uv_python_env():
    ...
    virtualenv_root = Path(_get_MLForge_virtualenv_root())
...
def test_run_databricks_cluster_spec(tmp_path):
    ...
    with mock.patch("MLForge.projects._run") as run_mock:
        for cluster_spec_arg in [json.dumps(cluster_spec), cluster_spec_path]:
            invoke_cli_runner(
                ...
                env={"MLForge_TRACKING_URI": "databricks://profile"},
            )
...
def test_MLForge_run():
    with mock.patch("MLForge.cli.projects") as mock_projects:

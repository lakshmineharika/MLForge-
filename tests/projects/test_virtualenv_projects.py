...
from MLForge.utils.virtualenv import _create_virtualenv
from MLForge.utils.yaml_utils import read_yaml, write_yaml
...
spy_on_create_virtualenv = mock.patch(
    "MLForge.projects.backend.local._create_virtualenv", wraps=_create_virtualenv
)


@pytest.fixture(autouse=True, scope="module")
def use_dev_MLForge_for_projects():
    MLForge_root = os.path.dirname(os.path.dirname(MLForge.__file__))
    ...
    if "MLForge" in conda_pip_dependencies:
        conda_pip_dependencies.remove("MLForge")
        conda_pip_dependencies.append(MLForge_root)
...
def test_virtualenv_project_execution_without_env_manager(create_virtualenv_spy):
    ...
    submitted_run = MLForge.projects.run(TEST_VIRTUALENV_PROJECT_DIR, entry_point="test")
    submitted_run.wait()
    create_virtualenv_spy.assert_called_once()
...
def test_virtualenv_project_execution_no_python_env(create_virtualenv_spy):
    ...
    """
    submitted_run = MLForge.projects.run(TEST_VIRTUALENV_NO_PYTHON_ENV, entry_point="test")
    submitted_run.wait()
    create_virtualenv_spy.assert_called_once()
...
def test_virtualenv_project_execution_conda():
    with pytest.raises(MLForgeException, match="python_env project cannot be executed using conda"):
        MLForge.projects.run(TEST_VIRTUALENV_PROJECT_DIR, env_manager="conda")
...
"""
    ...
    MLForge.projects.run(str(tmp_path))
    create_virtualenv_spy.assert_called_once()
...
"""
    ...
    MLForge.projects.run(str(tmp_path), entry_point="test.py")
    create_virtualenv_spy.assert_called_once()

...
from MLForge.exceptions import ExecutionException
from MLForge.projects import kubernetes as kb
...
def test_valid_kubernetes_job_spec():
    ...
    project_name = "MLForge-docker-example"
    image_tag = "image_tag"
    image_digest = "5e74a5a"
    command = ["MLForge", "run", ".", "--env-manager", "local", "-P", "alpha=0.5"]
    env_vars = {"RUN_ID": "1"}
...
def test_run_kubernetes_job():
    ...
    with (
        ...
        assert submitted_run_obj._MLForge_run_id == active_run.info.run_id
        assert submitted_run_obj._job_name.startswith(project_name)
        assert submitted_run_obj._job_namespace == "MLForge"
...
def test_push_image_to_registry():
    image_uri = "dockerhub_account/MLForge-kubernetes-example"

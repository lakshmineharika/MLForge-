...
import MLForge
from MLForge import MLForgeClient, cli
from MLForge.entities import RunStatus
from MLForge.environment_variables import MLForge_TRACKING_URI
from MLForge.exceptions import MLForgeException
from MLForge.legacy_databricks_cli.configure.provider import DatabricksConfig
from MLForge.projects import ExecutionException, databricks
from MLForge.projects.databricks import DatabricksJobRunner, _get_cluster_MLForge_run_cmd
...
from MLForge.utils import file_utils
from MLForge.utils.MLForge_tags import (
    MLForge_DATABRICKS_RUN_URL,
    MLForge_DATABRICKS_SHELL_JOB_RUN_ID,
    MLForge_DATABRICKS_WEBAPP_URL,
)
from MLForge.utils.rest_utils import MLForgeHostCreds
from MLForge.utils.uri import construct_db_uri_from_profile
...
def runs_cancel_mock():
    ...
    with mock.patch(
        "MLForge.projects.databricks.DatabricksJobRunner.jobs_runs_cancel"
    ) as runs_cancel_mock:
...
def runs_submit_mock():
    ...
    with mock.patch(
        "MLForge.projects.databricks.DatabricksJobRunner._jobs_runs_submit",
...
def runs_get_mock():
    ...
    with mock.patch(
        "MLForge.projects.databricks.DatabricksJobRunner.jobs_runs_get"
    ) as runs_get_mock:
        yield runs_get_mock


@pytest.fixture
def databricks_cluster_MLForge_run_cmd_mock():
    """Mocks the Jobs Runs Get API request"""
    with mock.patch(
        "MLForge.projects.databricks._get_cluster_MLForge_run_cmd"
    ) as MLForge_run_cmd_mock:
        yield MLForge_run_cmd_mock
...
def upload_to_dbfs_mock(dbfs_root_mock):
    ...
    with mock.patch.object(
        MLForge.projects.databricks.DatabricksJobRunner, "_upload_to_dbfs", new=upload_mock_fn
    ) as upload_mock:
        yield upload_mock
...
def dbfs_path_exists_mock(dbfs_root_mock):
    with mock.patch(
        "MLForge.projects.databricks.DatabricksJobRunner._dbfs_path_exists"
    ) as path_exists_mock:
...
def before_run_validations_mock():
    with mock.patch("MLForge.projects.databricks.before_run_validations"):
        yield


@pytest.fixture
def set_tag_mock():
    with mock.patch("MLForge.projects.databricks.tracking.MLForgeClient") as m:
        MLForge_service_mock = mock.Mock(wraps=MLForgeClient())
        m.return_value = MLForge_service_mock
        yield MLForge_service_mock.set_tag
...
def test_upload_existing_project_to_dbfs(dbfs_path_exists_mock):
    ...
    with mock.patch(
        "MLForge.projects.databricks.DatabricksJobRunner._upload_to_dbfs"
...
def test_dbfs_path_exists_error_response_handling(response_mock):
    with (
        ...
        mock.patch("MLForge.utils.rest_utils.http_request") as http_request_mock,
    ):
        ...
        # then _dbfs_path_exists should return a MLForgeException
        with pytest.raises(MLForgeException, match="API request to check existence of file at DBFS"):
...
def test_run_databricks_validations(
    ...
    with mock.patch(
        "MLForge.projects.databricks.DatabricksJobRunner._databricks_api_request"
    ) as db_api_req_mock:
        # Test bad tracking URI
        MLForge.set_tracking_uri(f"sqlite:///{tmp_path / 'MLForge.db'}")
        with pytest.raises(ExecutionException, match="MLForge tracking URI must be of"):
            run_databricks_project(cluster_spec_mock, synchronous=True)
        assert db_api_req_mock.call_count == 0
        db_api_req_mock.reset_mock()
        MLForge_service = MLForgeClient()
        assert len(MLForge_service.search_runs([FileStore.DEFAULT_EXPERIMENT_ID])) == 0
...
@pytest.mark.usefixtures(
    ...
    "runs_cancel_mock",
    "dbfs_mocks",
    "databricks_cluster_MLForge_run_cmd_mock",
)
def test_run_databricks(
    ...
    monkeypatch.setenv("DATABRICKS_TOKEN", "foo")
    MLForge.set_tracking_uri("databricks")
    # Test that MLForge gets the correct run status when performing a Databricks run
    for run_succeeded, expect_status in [(True, RunStatus.FINISHED), (False, RunStatus.FAILED)]:
        ...
        assert databricks_cluster_MLForge_run_cmd_mock.call_count == 1
        tags = {}
        for call_args, _ in set_tag_mock.call_args_list:
            tags[call_args[1]] = call_args[2]
        assert tags[MLForge_DATABRICKS_RUN_URL] == "test_url"
        assert tags[MLForge_DATABRICKS_SHELL_JOB_RUN_ID] == "-1"
        assert tags[MLForge_DATABRICKS_WEBAPP_URL] == "https://test-host"
        set_tag_mock.reset_mock()
        runs_submit_mock.reset_mock()
        databricks_cluster_MLForge_run_cmd_mock.reset_mock()
...
def test_run_databricks_throws_exception_when_spec_uses_existing_cluster(monkeypatch):
    ...
    with pytest.raises(
        MLForgeException, match="execution against existing clusters is not currently supported"
    ) as exc:
...
def test_run_databricks_cancel(
    ...
    # Test that MLForge properly handles Databricks run cancellation. We mock the result of
    ...
    runs_get_mock.return_value = mock_runs_get_result(succeeded=False)
    with pytest.raises(MLForge.projects.ExecutionException, match=r"Run \(ID '.+'\) failed"):
        run_databricks_project(cluster_spec_mock, synchronous=True)


def test_get_tracking_uri_for_run(monkeypatch):
    MLForge.set_tracking_uri("http://some-uri")
    assert databricks._get_tracking_uri_for_run() == "http://some-uri"
    MLForge.set_tracking_uri("databricks://profile")
    assert databricks._get_tracking_uri_for_run() == "databricks"
    MLForge.set_tracking_uri(None)
    monkeypatch.setenv(MLForge_TRACKING_URI.name, "http://some-uri")
    assert MLForge.tracking._tracking_service.utils.get_tracking_uri() == "http://some-uri"
...
def test_run_databricks_failed():
    ...
    with (
        mock.patch("MLForge.utils.databricks_utils.get_databricks_host_creds"),
        mock.patch(
        ...
        with pytest.raises(
            MLForgeException, match="RESOURCE_DOES_NOT_EXIST: Node type not supported"
        ):
            runner._run_shell_command_job("/project", "command", {}, {})


def test_run_databricks_generates_valid_MLForge_run_cmd():
    cmd = _get_cluster_MLForge_run_cmd(
    ...
    assert cmd[0] == "MLForge"
    with mock.patch("MLForge.projects.run"):

...
import MLForge
from MLForge.exceptions import ExecutionException, MLForgeException
from MLForge.projects import _project_spec
from MLForge.projects.utils import (
    _fetch_git_repo,
    _fetch_project,
    _get_storage_dir,
...
from MLForge.utils.MLForge_tags import MLForge_PROJECT_ENTRY_POINT, MLForge_SOURCE_NAME
...
def test_fetch_git_repo_commit(tmp_path, commit):
    _fetch_git_repo("https://github.com/MLForge/MLForge-example.git", commit, tmp_path)
...
def test_fetch_create_and_log(tmp_path):
    ...
    experiment_id = MLForge.create_experiment("test_fetch_project")
    expected_dir = str(tmp_path)
    project_uri = "http://someuri/myproject.git"
    user_param = {"method_name": "newton"}
    with (
        mock.patch("MLForge.projects.utils._fetch_project", return_value=expected_dir),
        mock.patch("MLForge.projects._project_spec.load_project", return_value=mock_fetched_project),
    ):
        ...
        # check tags
        run = MLForge.get_run(active_run.info.run_id)
        assert MLForge_PROJECT_ENTRY_POINT in run.data.tags
        assert MLForge_SOURCE_NAME in run.data.tags
        assert entry_point_name == run.data.tags[MLForge_PROJECT_ENTRY_POINT]
        assert project_uri == run.data.tags[MLForge_SOURCE_NAME]
...
def test_unzip_repo_rejects_path_traversal(tmp_path):
    ...
    with pytest.raises(MLForgeException, match="path traversal"):

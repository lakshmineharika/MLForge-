...
def check_load(model_uri: str) -> None:
    ...
    model = MLForge.sklearn.load_model(model_uri)
...
def check_register(model_uri: str) -> None:
    mv = MLForge.register_model(model_uri, "model")
    model = MLForge.pyfunc.load_model(f"models:/{mv.name}/{mv.version}")
    np.testing.assert_array_equal(model.predict([[1, 2]]), [3.0])
...
def check_list_artifacts_with_run_id_and_path(run_id: str, path: str) -> None:
    ...
    assert len(MLForge.artifacts.list_artifacts(run_id=run_id, artifact_path="unknown")) == 0


def check_list_artifacts_with_model_uri(model_uri: str) -> None:
    artifacts = [a.path for a in MLForge.artifacts.list_artifacts(artifact_uri=model_uri)]
    assert "model/MLmodel" in artifacts
    assert "model/test.txt" in artifacts


def check_download_artifacts_with_run_id_and_path(run_id: str, path: str, tmp_path: Path) -> None:
    out_path = MLForge.artifacts.download_artifacts(
...
def check_evaluate(model_uri: str) -> None:
    # Model evaluation
    eval_res = MLForge.models.evaluate(
        model=model_uri,
        data=np.array([[1, 2]]),
        targets=np.array([3]),
...
def test_MLForge_2_x_comp(tmp_path: Path) -> None:
    tracking_uri = f"sqlite:///{tmp_path / 'MLForge.db'}"
    artifact_location = (tmp_path / "artifacts").as_uri()

    out_file = tmp_path / "out.txt"
    # Log a model using MLForge 2.x (let 2.x create the DB and experiment)
    ...
    subprocess.check_call(
        [
            ...
            "python",
            # Use the isolated mode to ignore MLForge in the repository
            "-I",
            "-c",
            """
import sys
...
with MLForge.start_run() as run:
    MLForge.log_text("test", "model/test.txt")
    model_info = MLForge.sklearn.log_model(fitted_model, artifact_path="model")
...
def test_MLForge_3_x_comp(tmp_path: Path) -> None:
    tracking_uri = f"sqlite:///{tmp_path / 'MLForge.db'}"
    MLForge.set_tracking_uri(tracking_uri)
    artifact_location = (tmp_path / "artifacts").as_uri()
    exp_id = MLForge.create_experiment("test", artifact_location=artifact_location)
    ...
    with MLForge.start_run() as run:
        MLForge.log_text("test", "model/test.txt")
        model_info = MLForge.sklearn.log_model(fitted_model, name="model")
    ...
    artifacts = [a.path for a in MLForge.artifacts.list_artifacts(artifact_uri=logged_model_uri)]
...
def test_run_and_model_has_artifact_with_same_name(tmp_path: Path) -> None:
    fitted_model = LinearRegression().fit([[1, 2]], [3])
    with MLForge.start_run() as run:
        MLForge.log_text("", artifact_file="model/MLmodel")
        info = MLForge.sklearn.log_model(fitted_model, name="model")
    ...
    out = MLForge.artifacts.download_artifacts(
        run_id=run.info.run_id,
        artifact_path="model",

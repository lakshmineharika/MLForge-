...
def _run_MLForge_server(tmp_path: Path) -> Generator[str, None, None]:
    port = get_safe_port()
    backend_store_uri = f"sqlite:///{tmp_path / 'MLForge.db'}"
    ...
    with subprocess.Popen(
        ...
        env=(
            os.environ.copy()
            | {
                "MLForge_WEBHOOK_ALLOWED_SCHEMES": "http",
                "MLForge_WEBHOOK_SECRET_ENCRYPTION_KEY": Fernet.generate_key().decode(),
                "MLForge_WEBHOOK_REQUEST_MAX_RETRIES": "3",
                "MLForge_WEBHOOK_REQUEST_TIMEOUT": "10",
                "MLForge_WEBHOOK_CACHE_TTL": "0",  # Disable caching for tests
                "MLForge_WEBHOOK_ALLOW_PRIVATE_IPS": "true",  # Allow localhost in e2e tests
                "MLForge_SERVER_ENABLE_JOB_EXECUTION": "false",  # Not needed for webhook tests
            }
        ),
    ) as prc:
        try:
            ...
            # Kill the gunicorn processes spawned by MLForge server
            try:
            ...
            # Kill the MLForge server process
            prc.terminate()
...
def MLForge_server(
    ...
    tmp_path = tmp_path_factory.mktemp("MLForge_server")
    with _run_MLForge_server(tmp_path) as url:
        yield url


@pytest.fixture(scope="module")
def MLForge_client(MLForge_server: str) -> MLForgeClient:
    with pytest.MonkeyPatch.context() as mp:
        # Disable retries to fail fast
        mp.setenv("MLForge_HTTP_REQUEST_MAX_RETRIES", "0")
        return MLForgeClient(tracking_uri=MLForge_server, registry_uri=MLForge_server)


@pytest.fixture(autouse=True)
def cleanup(MLForge_client: MLForgeClient, app_client: AppClient) -> Generator[None, None, None]:
    yield

    for webhook in MLForge_client.list_webhooks():
        MLForge_client.delete_webhook(webhook.webhook_id)

    app_client.reset()


def test_registered_model_created(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
    MLForge_client.create_webhook(
    ...
    registered_model = MLForge_client.create_registered_model(
        name="test_name",
...
def test_model_version_created(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
    MLForge_client.create_webhook(
        name="model_version_created",
        url=app_client.get_url("/insecure-webhook"),
        events=[WebhookEvent(WebhookEntity.MODEL_VERSION, WebhookAction.CREATED)],
    )
    registered_model = MLForge_client.create_registered_model(name="model_version_created")
    model_version = MLForge_client.create_model_version(
        name=registered_model.name,
...
def test_model_version_tag_set(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
    MLForge_client.create_webhook(
        name="model_version_tag_set",
        url=app_client.get_url("/insecure-webhook"),
        events=[WebhookEvent(WebhookEntity.MODEL_VERSION_TAG, WebhookAction.SET)],
    )
    registered_model = MLForge_client.create_registered_model(name="model_version_tag_set")
    model_version = MLForge_client.create_model_version(
        name=registered_model.name,
        source="s3://bucket/path/to/model",
        run_id="1234567890abcdef",
    )
    MLForge_client.set_model_version_tag(
...
def test_model_version_tag_deleted(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
    MLForge_client.create_webhook(
        name="model_version_tag_deleted",
        url=app_client.get_url("/insecure-webhook"),
        events=[WebhookEvent(WebhookEntity.MODEL_VERSION_TAG, WebhookAction.DELETED)],
    )
    registered_model = MLForge_client.create_registered_model(name="model_version_tag_deleted")
    model_version = MLForge_client.create_model_version(
        name=registered_model.name,
        source="s3://bucket/path/to/model",
        run_id="1234567890abcdef",
    ...
    MLForge_client.delete_model_version_tag(
...
def test_model_version_alias_created(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
    MLForge_client.create_webhook(
        name="model_version_alias_created",
        url=app_client.get_url("/insecure-webhook"),
        events=[WebhookEvent(WebhookEntity.MODEL_VERSION_ALIAS, WebhookAction.CREATED)],
    )
    registered_model = MLForge_client.create_registered_model(name="model_version_alias_created")
    model_version = MLForge_client.create_model_version(
        name=registered_model.name,
        source="s3://bucket/path/to/model",
        run_id="1234567890abcdef",
    ...
    MLForge_client.set_registered_model_alias(
...
def test_model_version_alias_deleted(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
    MLForge_client.create_webhook(
        name="model_version_alias_deleted",
        url=app_client.get_url("/insecure-webhook"),
        events=[WebhookEvent(WebhookEntity.MODEL_VERSION_ALIAS, WebhookAction.DELETED)],
    )
    registered_model = MLForge_client.create_registered_model(name="model_version_alias_deleted")
    model_version = MLForge_client.create_model_version(
        name=registered_model.name,
        source="s3://bucket/path/to/model",
        run_id="1234567890abcdef",
    ...
    MLForge_client.delete_registered_model_alias(name=model_version.name, alias="test_alias")
    logs = app_client.wait_for_logs(expected_count=1)
    assert len(logs) == 1
    assert logs[0].endpoint == "/insecure-webhook"
    assert logs[0].payload == {
...
def test_webhook_with_secret(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
    ...
    assert "x-MLForge-signature" in logs[0].headers
    assert logs[0].headers["x-MLForge-signature"].startswith("v1,")


def test_webhook_with_wrong_secret(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
    ...
    # But MLForge will still create the registered model
    MLForge_client.create_registered_model(
...
def test_webhook_without_secret_to_secure_endpoint(
    MLForge_client: MLForgeClient, app_client: AppClient
) -> None:
...
def test_webhook_test_insecure_endpoint(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
    # Create webhook for testing
    webhook = MLForge_client.create_webhook(
        name="test_webhook",
        url=app_client.get_url("/insecure-webhook"),
        events=[WebhookEvent(WebhookEntity.MODEL_VERSION, WebhookAction.CREATED)],
    )
    ...
    # Test the webhook
    result = MLForge_client.test_webhook(webhook.webhook_id)
...
def test_webhook_test_secure_endpoint(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
...
def test_webhook_test_with_specific_event(
    ...
    result = MLForge_client.test_webhook(
...
def test_webhook_test_failed_endpoint(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
...
def test_webhook_test_with_wrong_secret(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
...
def test_webhook_retry_on_5xx_error(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
...
def test_prompt_created(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
    MLForge_client.create_webhook(
        name="prompt_created",
        url=app_client.get_url("/insecure-webhook"),
    ...
    prompt = MLForge_client.create_prompt(
        name="test_prompt",
...
def test_prompt_version_created(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
    MLForge_client.create_webhook(
    ...
    MLForge_client.create_prompt_version(
        name=prompt.name,
...
def test_prompt_tag_set(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
    MLForge_client.create_webhook(
        name="prompt_tag_set",
        url=app_client.get_url("/insecure-webhook"),
...
def test_prompt_tag_deleted(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
    MLForge_client.create_webhook(
    ...
    MLForge_client.delete_prompt_tag(
        name=prompt.name,
        key="environment",
    )
...
def test_prompt_version_tag_set(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
    MLForge_client.create_webhook(
    ...
    prompt = MLForge_client.create_prompt(name="test_prompt_version_tag_set")
    prompt_version = MLForge_client.create_prompt_version(
        name=prompt.name,
        template="Hello {{name}}!",
    )

    MLForge_client.set_prompt_version_tag(
        name=prompt.name,
...
def test_prompt_version_tag_deleted(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
    MLForge_client.create_webhook(
    ...
    prompt = MLForge_client.create_prompt(name="test_prompt_version_tag_deleted")
    prompt_version = MLForge_client.create_prompt_version(
        name=prompt.name,
        template="Hello {{name}}!",
    ...
    MLForge_client.delete_prompt_version_tag(
        name=prompt.name,
        version=str(prompt_version.version),
        key="quality_score",
    )
...
def test_prompt_alias_created(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
    MLForge_client.create_webhook(
    ...
    prompt = MLForge_client.create_prompt(name="test_prompt_alias_created")
    prompt_version = MLForge_client.create_prompt_version(
        name=prompt.name,
        template="Hello {{name}}!",
    )

    MLForge_client.set_prompt_alias(
        name=prompt.name,
...
def test_prompt_alias_deleted(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
    MLForge_client.create_webhook(
    ...
    prompt = MLForge_client.create_prompt(name="test_prompt_alias_deleted")
    prompt_version = MLForge_client.create_prompt_version(
        name=prompt.name,
        template="Hello {{name}}!",
    )
    ...
    MLForge_client.delete_prompt_alias(
        name=prompt.name,
        alias="staging",
    )
...
def test_prompt_webhook_with_mixed_events(
    ...
    model = MLForge_client.create_registered_model(
        name="regular_model",
...
def test_prompt_webhook_test_endpoint(MLForge_client: MLForgeClient, app_client: AppClient) -> None:
    webhook = MLForge_client.create_webhook(

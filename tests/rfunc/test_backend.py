...
from MLForge.rfunc.backend import RFuncBackend, _r_quote
...
def test_serve_escapes_malicious_host():
    ...
    with (
        mock.patch(
            "MLForge.rfunc.backend._download_artifact_from_uri",
            return_value="/tmp/model",
        ) as mock_download,
        mock.patch("MLForge.rfunc.backend._execute") as mock_execute,
    ):
        backend.serve(
    ...
    assert "MLForge::MLForge_rfunc_serve('/tmp/model', port = 5000, host = " in command
...
def test_serve_benign_host():
    ...
    mock_execute.assert_called_once_with(
        "MLForge::MLForge_rfunc_serve('/tmp/model', port = 5000, host = '127.0.0.1')"
    )

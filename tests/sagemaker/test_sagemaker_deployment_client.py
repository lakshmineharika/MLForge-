...
import MLForge.sagemaker as mfs
import MLForge.sklearn
from MLForge.deployments.cli import commands as cli_commands
...
def pretrained_model():
    ...
    with MLForge.start_run():
        ...
        lr.fit(X, y)
        MLForge.sklearn.log_model(lr, name=model_path)
...
def mock_sagemaker_aws_services(fn):
    ...
    def mock_wrapper(*args, **kwargs):
        # Create an ECR repository for the `MLForge-pyfunc` SageMaker docker image
...
def test_attempting_to_deploy_in_asynchronous_mode_without_archiving_throws_exception(
    ...
    with pytest.raises(MLForgeException, match="Resources must be archived") as exc:
        sagemaker_deployment_client.create_deployment(
            name="test-app",
            model_uri=pretrained_model.model_uri,
...
def test_create_deployment_create_sagemaker_and_s3_resources_with_expected_names_and_env_from_local(
    ...
    expected_model_environment = {
        "MLForge_DEPLOYMENT_FLAVOR_NAME": "python_function",
...
def test_create_deployment_with_preexisting_name_throws_exception(
    ...
    with pytest.raises(
        MLForgeException, match="an application with the same name already exists"
    ) as exc:
        sagemaker_deployment_client.create_deployment(
            name=name,
            model_uri=pretrained_model.model_uri,
...
def test_create_deployment_throws_exception_after_endpoint_creation_fails(
    ...
    with (
        ...
        pytest.raises(MLForgeException, match="deployment operation failed") as exc,
    ):
        sagemaker_deployment_client.create_deployment(
            name="test-app",
            model_uri=pretrained_model.model_uri,
...
def test_update_deployment_with_create_mode_raises_exception(
    ...
    with pytest.raises(MLForgeException, match="Invalid mode") as exc:
        sagemaker_deployment_client.update_deployment(
            name="invalid mode",
            model_uri=pretrained_model.model_uri,
...
def test_update_deployment_in_replace_mode_with_archiving_does_not_delete_resources(
    ...
    sk_model = MLForge.sklearn.load_model(model_uri=model_uri)
    new_artifact_path = "model"
    with MLForge.start_run():
        MLForge.sklearn.log_model(sk_model, name=new_artifact_path)
        new_model_uri = f"runs:/{MLForge.active_run().info.run_id}/{new_artifact_path}"
    sagemaker_deployment_client.update_deployment(
        name=name,
...
def test_get_deployment_non_existent_deployment():
    ...
    with pytest.raises(MLForgeException, match="There was an error while"):

...
from MLForge.sagemaker.cli import build_and_push_container

from tests.pyfunc.docker.test_docker import assert_dockerfiles_equal

_MLForge_ROOT = Path(MLForge.__file__).parent.parent
_RESOURCE_DIR = os.path.join(_MLForge_ROOT, "tests", "resources", "dockerfile")
...
def test_build_and_push_container(tmp_path, env_manager, install_java):
    ...
    with mock.patch(
        "MLForge.models.docker_utils.build_image_from_context", side_effect=_build_image_with_copy
    ):
...
def test_build_and_push_container_network_option():
    with mock.patch("MLForge.models.docker_utils.build_image_from_context") as mock_build:
        res = CliRunner().invoke(
            build_and_push_container,
            [
                "--no-push",
...
def test_build_image_from_context_fallback_to_cli_when_docker_sdk_fails():
    with (
        mock.patch("MLForge.models.docker_utils.Popen") as mock_popen,

...
import MLForge.sagemaker.cli as mfscli
...
def test_deploying_sagemaker_transform_job_with_preexisting_name_in_create_mode_throws_exception(
    ...
    with pytest.raises(
        MLForgeException, match="a batch transform job with the same name already exists"
    ) as exc:
        mfs.deploy_transform_job(
            job_name=job_name,
            model_uri=pretrained_model.model_uri,
...
def test_deploy_in_throw_exception_after_transform_job_creation_fails(
    ...
    with (
        ...
        pytest.raises(MLForgeException, match="batch transform job failed") as exc,
    ):
        mfs.deploy_transform_job(
            job_name="test-job",
            model_uri=pretrained_model.model_uri,

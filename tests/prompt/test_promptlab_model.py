...
from MLForge.deployments import set_deployments_target
from MLForge.entities.param import Param
from MLForge.prompt.promptlab_model import _PromptlabModel
...
def test_promptlab_prompt_replacement():
    ...
    get_route_patch = mock.patch(
        "MLForge.deployments.MLForgeDeploymentClient.get_endpoint",
        return_value=mock.Mock(endpoint_type="llm/v1/completions"),
    )

    with (
        get_route_patch,
        mock.patch("MLForge.deployments.MLForgeDeploymentClient.predict") as mock_query,
    ):
        model.predict(data)
...
def test_promptlab_works_with_chat_route():
    ...
    with (
        get_route_patch,
        mock.patch("MLForge.deployments.MLForgeDeploymentClient.predict", return_value=mock_response),
    ):

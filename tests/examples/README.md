...
## Examples that utilize `MLForge run` construct

The `@pytest.mark.mark.parametrize` decorator for `def test_MLForge_run_example(directory, params):`
is updated.

If the example is executed by `cd examples/new_example_dir && MLForge run . -P param1=99 -P param2=3`, then
...
def test_MLForge_run_example(directory, params):
```
...
For an example that is not run by `MLForge run`, the list in
...
@pytest.mark.parametrize(("directory", "command"), [
    ...
    ('quickstart', ['python', 'MLForge_tracking.py']),
    ("new_example_dir", ["python", "train.py"]),
])

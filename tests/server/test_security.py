...
from MLForge.server import security
from MLForge.server.fastapi_security import init_fastapi_security
from MLForge.server.security_utils import is_allowed_host_header, is_api_endpoint
...
def test_custom_allowed_hosts(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("MLForge_SERVER_ALLOWED_HOSTS", "example.com,app.example.com")
    hosts = security.get_allowed_hosts()
...
def test_dns_rebinding_protection(
    ...
    monkeypatch.setenv("MLForge_SERVER_ALLOWED_HOSTS", "localhost,127.0.0.1")
...
def test_cors_protection(
    ...
    monkeypatch.setenv(
        "MLForge_SERVER_CORS_ALLOWED_ORIGINS", "http://localhost:3000,https://app.example.com"
    )
    security.init_security_middleware(test_app)
    client = Client(test_app)

    headers = {"Origin": origin} if origin else {}
    response = getattr(client, method.lower())("/api/2.0/MLForge/experiments/list", headers=headers)
    assert response.status_code == expected_status
...
def test_wildcard_cors_disables_credentials(
    ...
    monkeypatch.setenv("MLForge_SERVER_CORS_ALLOWED_ORIGINS", "*")
    # The "MLForge" logger sets propagate=False, so caplog's root handler does not
    # see records. Attach caplog's handler directly to the security logger.
    security_logger = logging.getLogger("MLForge.server.security")
    security_logger.addHandler(caplog.handler)
    try:
        with caplog.at_level("WARNING", logger="MLForge.server.security"):
            security.init_security_middleware(test_app)
    finally:
    ...
    response = client.post(
        "/api/2.0/MLForge/experiments/list", headers={"Origin": "http://evil.com"}
    )
...
def test_preflight_options_request(
    ...
    monkeypatch.setenv("MLForge_SERVER_CORS_ALLOWED_ORIGINS", "http://localhost:3000")
    security.init_security_middleware(test_app)
    client = Client(test_app)

    response = client.options(
        "/api/2.0/MLForge/experiments/list",
        headers={
            "Origin": origin,
...
def test_disable_security_middleware(test_app, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("MLForge_SERVER_DISABLE_SECURITY_MIDDLEWARE", "true")
    security.init_security_middleware(test_app)
    client = Client(test_app)
...
def test_x_frame_options_configuration(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv("MLForge_SERVER_X_FRAME_OPTIONS", "DENY")
    ...
    monkeypatch.setenv("MLForge_SERVER_X_FRAME_OPTIONS", "NONE")
...
def test_notebook_trace_renderer_skips_x_frame_options(monkeypatch: pytest.MonkeyPatch):
    from MLForge.tracing.constant import TRACE_RENDERER_ASSET_PATH
...
def test_wildcard_hosts(
    ...
    monkeypatch.setenv("MLForge_SERVER_ALLOWED_HOSTS", allowed_hosts)
    security.init_security_middleware(test_app)
    client = Client(test_app)
...
def test_wildcard_origins(
    ...
    monkeypatch.setenv("MLForge_SERVER_CORS_ALLOWED_ORIGINS", allowed_origins)
    security.init_security_middleware(test_app)
    client = Client(test_app)

    response = client.post("/api/2.0/MLForge/experiments/list", headers={"Origin": origin})
    assert response.status_code == expected_status
...
def test_endpoint_security_bypass(
    ...
    monkeypatch.setenv("MLForge_SERVER_ALLOWED_HOSTS", "localhost")
    security.init_security_middleware(test_app)
    client = Client(test_app)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("MLForge_SERVER_ALLOWED_HOSTS", "app1.com,app2.com:8080", ["app1.com", "app2.com:8080"]),
    ],
)
...
@pytest.mark.parametrize(
    ("path", "expected"),
    [
        ("/api/2.0/MLForge/experiments/list", True),
        ("/ajax-api/2.0/MLForge/experiments/list", True),
        ("/ajax-api/3.0/MLForge/runs/search", True),
        ("/api/test", False),
        ("/test", False),
        ("/health", False),
...
def test_fastapi_cors_allows_localhost_origins(fastapi_client, origin, expect_cors_header):
    response = fastapi_client.get(
        "/api/2.0/MLForge/experiments/list", headers={"Host": "localhost", "Origin": origin}
    )
...
def test_fastapi_wildcard_cors_disables_credentials(
    ...
    # The "MLForge" logger sets propagate=False, so caplog's root handler does not
    # see records. Attach caplog's handler directly to the security logger.
    security_logger = logging.getLogger("MLForge.server.fastapi_security")
    security_logger.addHandler(caplog.handler)

    app = FastAPI()

    @app.api_route("/api/2.0/MLForge/experiments/list", methods=["GET", "POST", "OPTIONS"])
    async def api_endpoint():
        return {"ok": True}

    try:
        with caplog.at_level("WARNING", logger="MLForge.server.fastapi_security"):
...
def test_fastapi_cors_allows_configured_origin(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("MLForge_SERVER_CORS_ALLOWED_ORIGINS", "https://trusted.com")

...
from MLForge import server
from MLForge.environment_variables import _MLForge_SGI_NAME
...
@pytest.fixture
def mock_exec_cmd():
    with mock.patch("MLForge.server._exec_cmd") as m:
        yield m
    ...
    while time.time() < deadline:
        if proc.poll() is not None:
            ...
            raise AssertionError(
                "MLForge server exited before accepting connections.\n"
                f"stdout:\n{stdout}\n"
                f"stderr:\n{stderr}"
            )
        try:
...
def test_find_app_custom_app_plugin():
    assert server._find_app("custom_app") == "MLForge_test_plugin.app:custom_app"


def test_find_app_non_existing_app():
    with pytest.raises(MLForgeException, match=r"Failed to find app 'does_not_exist'"):
...
def test_build_uvicorn_command():
    assert server._build_uvicorn_command(
        "", "localhost", "5000", "4", "MLForge.server.fastapi_app:app"
    ) == [
        sys.executable,
        "-m",
        "uvicorn",
        ...
        "--workers",
        "4",
        "MLForge.server.fastapi_app:app",
    ]

    # Test with custom uvicorn options
    assert server._build_uvicorn_command(
        "--reload --log-level debug", "localhost", "5000", "4", "MLForge.server.fastapi_app:app"
    ) == [
        sys.executable,
        "-m",
        "uvicorn",
    ...
    assert server._build_uvicorn_command(
        "", "localhost", "5000", "4", "MLForge.server.fastapi_app:app", None, is_factory=True
    ) == [
        sys.executable,
        "-m",
        "uvicorn",
...
def test_run_server(mock_exec_cmd, monkeypatch):
    monkeypatch.setenv("MLForge_SERVER_ENABLE_JOB_EXECUTION", "false")
...
def test_run_server_with_uvicorn(mock_exec_cmd, monkeypatch):
    ...
    mock_exec_cmd.assert_called_once_with(
        expected_command,
        extra_env={
            _MLForge_SGI_NAME.name: "uvicorn",
        },
...
def test_build_uvicorn_command_user_log_config_takes_precedence(uvicorn_opts):
    cmd = server._build_uvicorn_command(
        uvicorn_opts, "localhost", "5000", "4", "MLForge.server.fastapi_app:app"
    )
...
def test_MLForge_server_shuts_down_on_signal(sig: signal.Signals, tmp_path):
    port = find_free_port()

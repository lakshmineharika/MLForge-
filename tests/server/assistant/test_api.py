...
from MLForge.assistant.config import AssistantConfig, ProjectConfig
from MLForge.assistant.config import ProviderConfig as AssistantProviderConfig
from MLForge.assistant.providers import OllamaProvider
from MLForge.assistant.providers.base import (
    AssistantProvider,
...
from MLForge.assistant.types import Event, Message, ToolUseBlock
from MLForge.server.assistant.api import (
    PermissionDecision,
    _AssistantAPIRoute,
...
from MLForge.server.assistant.session import SESSION_DIR, SessionManager, save_process_pid
...
class MockProvider(AssistantProvider):
    ...
    def config_path(self) -> Path:
        return Path.home() / ".MLForge" / "assistant" / "mock-config.json"
    ...
    async def astream(
        ...
        MLForge_session_id: str | None = None,
        cwd: Path | None = None,
...
def isolated_config(tmp_path, monkeypatch):
    ...
    import MLForge.assistant.config as config_module

    config_home = tmp_path / ".MLForge" / "assistant"
    config_path = config_home / "config.json"

    monkeypatch.setattr(config_module, "MLForge_ASSISTANT_HOME", config_home)
...
def client():
    ...
    with (
        patch("MLForge.server.assistant.api.list_providers", return_value=[mock_provider]),
        patch("MLForge.server.assistant.api._get_selected_provider", return_value=mock_provider),
        patch("MLForge.server.assistant.api._is_localhost", return_value=True),
    ):
...
def test_message(client):
    response = client.post(
        "/ajax-api/3.0/MLForge/assistant/message",
        json={
            "message": "Hello",
    ...
    assert data["stream_url"] == f"/ajax-api/3.0/MLForge/assistant/stream/{data['session_id']}"
...
def test_stream_not_found_for_invalid_session(client):
    response = client.get("/ajax-api/3.0/MLForge/assistant/sessions/invalid-session-id/stream")
...
def test_stream_bad_request_when_no_pending_message(client):
    ...
    r = client.post("/ajax-api/3.0/MLForge/assistant/message", json={"message": "Hi"})
    session_id = r.json()["session_id"]
    client.get(f"/ajax-api/3.0/MLForge/assistant/sessions/{session_id}/stream")

    # Try to stream again without a new message
    response = client.get(f"/ajax-api/3.0/MLForge/assistant/sessions/{session_id}/stream")
...
def test_health_check_returns_ok_when_healthy(client):
    response = client.get("/ajax-api/3.0/MLForge/assistant/providers/mock_provider/health")
    assert response.status_code == 200
    assert response.json() == {"status": "ok"}


def test_health_check_returns_404_for_unknown_provider(client):
    response = client.get("/ajax-api/3.0/MLForge/assistant/providers/unknown_provider/health")
    assert response.status_code == 404
...
def test_health_check_returns_412_when_cli_not_installed():
    ...
    with (
        patch("MLForge.server.assistant.api.list_providers", return_value=[provider]),
        patch("MLForge.server.assistant.api._is_localhost", return_value=True),
    ):
...
def test_get_config_returns_empty_config(client):
    response = client.get("/ajax-api/3.0/MLForge/assistant/config")
    assert response.status_code == 200
    data = response.json()
...
def test_get_config_redacts_project_location_for_remote_clients(tmp_path):
    ...
    with patch("MLForge.server.assistant.api._is_localhost", return_value=False):
        response = TestClient(app).get("/ajax-api/3.0/MLForge/assistant/config")
...
def test_get_config_remote_access_allowed(
    ...
    monkeypatch.setenv("MLForge_ENABLE_REMOTE_ASSISTANT", str(enabled))
    with patch("MLForge.server.assistant.api._get_selected_provider") as mock_get_selected_provider:
...
def test_message_blocked_for_remote_client_when_remote_access_off(monkeypatch):
    monkeypatch.setenv("MLForge_ENABLE_REMOTE_ASSISTANT", "false")
    app = FastAPI()
    app.include_router(assistant_router)
    ...
    with (
        patch("MLForge.server.assistant.api.list_providers", return_value=[mock_provider]),
        patch("MLForge.server.assistant.api._get_selected_provider", return_value=mock_provider),
        patch("MLForge.server.assistant.api._is_localhost", return_value=False),
    ):
        client = TestClient(app)
        response = client.post(
            "/ajax-api/3.0/MLForge/assistant/message",
...
def test_deny_policy_route_with_provider_path_param_blocks_remote(monkeypatch):
    monkeypatch.setenv("MLForge_ENABLE_REMOTE_ASSISTANT", "true")
    router = APIRouter(prefix="/assistant", route_class=_AssistantAPIRoute)
...
def test_update_config_sets_provider(client):
    response = client.put(
        "/ajax-api/3.0/MLForge/assistant/config",
...
def test_update_config_expand_user_home(client, tmp_path):
    ...
    with patch("MLForge.server.assistant.api.Path.expanduser") as mock_expanduser:
...
def test_remote_request_blocked_without_loading_provider_when_disabled(monkeypatch):
    ...
    with (
        patch("MLForge.server.assistant.api._is_localhost", return_value=False),
        patch("MLForge.server.assistant.api._get_route_provider") as mock_get_provider,
    ):
...
def test_invalid_remote_access_value_raises(monkeypatch):
    monkeypatch.setenv("MLForge_ENABLE_REMOTE_ASSISTANT", "bogus")
    provider = MagicMock()
    provider.allows_remote_access = True
    with pytest.raises(ValueError, match="MLForge_ENABLE_REMOTE_ASSISTANT"):
...
def test_patch_session_cancel_with_process(client):
    ...
    with subprocess.Popen([sys.executable, "-c", "import time; time.sleep(10)"]) as proc:
        ...
        response = client.patch(
            f"/ajax-api/3.0/MLForge/assistant/sessions/{session_id}",
...
async def test_stream_pauses_then_resumes(decision, expected_text):
    ...
    """
    from MLForge.server.assistant.api import resolve_permission, stream_response
    ...
    with patch("MLForge.server.assistant.api._get_selected_provider", return_value=provider):
...
async def test_stream_prefers_new_message_over_stale_tool_decision():
    ...
    """
    from MLForge.server.assistant.api import stream_response
...
def test_resolve_permission_rejects_invalid_session(client):
    response = client.post(
        "/ajax-api/3.0/MLForge/assistant/sessions/not-a-uuid/permission",
...
def test_resolve_permission_returns_404_for_unknown_session(client):
    ...
    response = client.post(
        "/ajax-api/3.0/MLForge/assistant/sessions/f5f28c66-5ec6-46a1-9a2e-ca55fb64bf47/permission",
        json={"request_id": "t1", "decision": "allow"},
    )
    assert response.status_code == 404
...
def test_install_skills_success(client):
    with patch(
        "MLForge.server.assistant.api.install_skills", return_value=["skill1", "skill2"]
    ) as mock_install:
        response = client.post(
            "/ajax-api/3.0/MLForge/assistant/skills/install",
...
def test_install_skills_skips_when_already_installed(client):
    with (
        patch("MLForge.server.assistant.api.Path.exists", return_value=True),
        patch(
            "MLForge.server.assistant.api.list_installed_skills",
            return_value=["existing_skill"],
        ) as mock_list,
        patch("MLForge.server.assistant.api.install_skills") as mock_install,
    ):
        response = client.post(
            "/ajax-api/3.0/MLForge/assistant/skills/install",
            json={"type": "custom", "custom_path": "/tmp/test-skills"},
...
def test_list_ollama_models_returns_model_list(client):
    ...
    with patch("MLForge.server.assistant.api.list_providers", return_value=[mock_provider]):
        response = client.get(
            "/ajax-api/3.0/MLForge/assistant/providers/mock_provider/models",
...
def test_list_ollama_models_returns_412_when_not_installed(client):
    ...
    with patch(
        "MLForge.server.assistant.api.list_providers",
        return_value=[MissingDependencyProvider()],
    ):
        response = client.get("/ajax-api/3.0/MLForge/assistant/providers/mock_provider/models")
...
def test_list_ollama_models_returns_503_on_connection_failure(client):
    ...
    with patch("MLForge.server.assistant.api.list_providers", return_value=[UnreachableProvider()]):
        response = client.get(
            "/ajax-api/3.0/MLForge/assistant/providers/mock_provider/models",
            params={"base_url": "http://localhost:11434"},
        )
...
def test_list_provider_models_returns_404_for_unsupported_provider(client):
    ...
    with patch(
        ...
        response = client.get(
            "/ajax-api/3.0/MLForge/assistant/providers/unsupported_provider/models"
        )

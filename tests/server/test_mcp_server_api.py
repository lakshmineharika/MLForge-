...
from MLForge.entities.mcp_server import MCPTool
from MLForge.exceptions import MLForgeException
from MLForge.protos.databricks_pb2 import PERMISSION_DENIED, RESOURCE_ALREADY_EXISTS, ErrorCode
from MLForge.server.fastapi_app import add_mcp_exception_handlers
from MLForge.server.mcp_server_api import (
...
PREFIX = "/ajax-api/3.0/MLForge/mcp-servers"
...
def mock_icon_url_dns_resolution():
    ...
    with mock.patch("MLForge.utils.validation.socket.getaddrinfo", side_effect=_resolve):
        yield
...
def test_ensure_version_create_parent_access_rejects_raced_existing_parent_without_update():
    ...
    store = mock.Mock()
    store.create_mcp_server.side_effect = MLForgeException(
        "server exists",
        error_code=RESOURCE_ALREADY_EXISTS,
    )

    with pytest.raises(MLForgeException, match="Permission denied") as exc:
...
def test_create_version_missing_required_field_returns_MLForge_error(client):
    r = client.post(
        f"{PREFIX}/{_encode_path_param('com.example/v-server')}" + "/versions",
...
def test_create_version_invalid_semver_returns_MLForge_error(client):
    r = client.post(
        f"{PREFIX}/{_encode_path_param('com.example/v-server')}" + "/versions",
...
def test_create_version_invalid_package_shape_returns_MLForge_error(client):
    r = client.post(

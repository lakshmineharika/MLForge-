...
from MLForge.gateway.config import (
    EndpointType,
    GatewayRequestType,
    GeminiConfig,
    LiteLLMConfig,
...
from MLForge.gateway.constants import MLForge_GATEWAY_DURATION_HEADER, MLForge_GATEWAY_OVERHEAD_HEADER
from MLForge.gateway.guardrails import _SANITIZE_BYPASS_HEADER, JudgeGuardrail
from MLForge.gateway.providers.anthropic import AnthropicProvider
from MLForge.gateway.providers.base import (
    FallbackProvider,
    TrafficRouteProvider,
)
from MLForge.gateway.providers.databricks import DatabricksConfig, DatabricksProvider
from MLForge.gateway.providers.gemini import GeminiProvider
from MLForge.gateway.providers.litellm import LiteLLMProvider
from MLForge.gateway.providers.mistral import MistralProvider
from MLForge.gateway.providers.openai import OpenAIProvider
from MLForge.gateway.providers.portkey import PortkeyProvider
from MLForge.gateway.providers.utils import provider_call_duration_ms
from MLForge.gateway.schemas import chat, embeddings
from MLForge.server.fastapi_app import add_gateway_timing_middleware
from MLForge.server.gateway_api import (
...
def set_kek_passphrase(monkeypatch):
    monkeypatch.setenv("MLForge_CRYPTO_KEK_PASSPHRASE", TEST_PASSPHRASE)
...
def store(tmp_path: Path, db_uri: str):
    ...
    MLForge.set_tracking_uri(db_uri)
...
def test_build_endpoint_config_rejects_provider_not_in_allowed_list(monkeypatch):
    ...
    with pytest.raises(MLForgeException, match="not allowed"):
...
def test_build_endpoint_config_allows_provider_in_allowed_list(monkeypatch):
    monkeypatch.setenv("MLForge_GATEWAY_ALLOWED_PROVIDERS", "openai")
...
def test_api_key_not_read_from_file(store: SqlAlchemyStore, tmp_path: Path, monkeypatch):
    monkeypatch.delenv("MLForge_GATEWAY_RESOLVE_API_KEY_FROM_FILE", raising=False)
...
def test_create_provider_from_endpoint_name_no_models(store: SqlAlchemyStore):
    ...
    with patch(
        "MLForge.server.gateway_api.get_endpoint_config",
        return_value=GatewayEndpointConfig(
            endpoint_id=endpoint.endpoint_id, endpoint_name="test-endpoint", models=[]
        ),
    ):
        with pytest.raises(MLForgeException, match="has no PRIMARY models configured"):
...
async def test_chat_completions_endpoint(store: SqlAlchemyStore):
    ...
    with (
        ...
        patch("MLForge.server.gateway_api.load_guardrails", return_value=[]),
    ):
        mock_provider = MagicMock()
        mock_provider.chat = AsyncMock(return_value=mock_response)
        mock_endpoint_config = GatewayEndpointConfig(
...
def test_response_timing_headers(store: SqlAlchemyStore):
    ...
    with (
        ...
        patch("MLForge.server.gateway_api.get_request_workspace", return_value=None),
        patch("MLForge.server.gateway_api.check_budget_limit"),
        patch("MLForge.server.gateway_api.load_guardrails", return_value=[]),
        patch(
            "MLForge.server.gateway_api._create_provider_from_endpoint_name"
        ) as mock_create_provider,
        ...
        response = client.post(
            "/gateway/MLForge/v1/chat/completions",
            json={"model": "my-endpoint", "messages": [{"role": "user", "content": "Hi"}]},
        )

    assert response.status_code == 200
    duration = int(response.headers[MLForge_GATEWAY_DURATION_HEADER])
    overhead = int(response.headers[MLForge_GATEWAY_OVERHEAD_HEADER])
    assert duration >= 0
...
def test_response_timing_headers_streaming(store: SqlAlchemyStore):
    ...
    assert response.status_code == 200
    assert MLForge_GATEWAY_DURATION_HEADER in response.headers
    assert int(response.headers[MLForge_GATEWAY_DURATION_HEADER]) >= 0
    # Overhead header is omitted for streaming since provider_call_duration_ms is not set.
    assert MLForge_GATEWAY_OVERHEAD_HEADER not in response.headers
...
def test_response_timing_headers_error(store: SqlAlchemyStore):
    ...
    duration = int(response.headers[MLForge_GATEWAY_DURATION_HEADER])
    assert duration >= 0
    assert MLForge_GATEWAY_OVERHEAD_HEADER in response.headers
    overhead = int(response.headers[MLForge_GATEWAY_OVERHEAD_HEADER])
    assert 0 <= overhead <= duration
...
async def test_openai_passthrough_chat(store: SqlAlchemyStore):
    ...
    with mock.patch(
        "MLForge.gateway.providers.openai.send_request", return_value=mock_response
    ) as mock_send:
...
async def test_openai_passthrough_responses_compact_rejects_stream():
    ...
    with (
        mock.patch("MLForge.gateway.providers.openai.send_request") as mock_send,
...
async def test_openai_passthrough_chat_streaming(store: SqlAlchemyStore):
    ...
    with mock.patch(
        "MLForge.gateway.providers.openai.send_stream_request",
...
async def test_anthropic_passthrough_messages(store: SqlAlchemyStore):
    ...
    with mock.patch(
        "MLForge.gateway.providers.anthropic.send_request", return_value=mock_response
    ) as mock_send:
...
async def test_anthropic_passthrough_messages_streaming(store: SqlAlchemyStore):
    ...
    with mock.patch(
        "MLForge.gateway.providers.anthropic.send_stream_request",
        return_value=mock_stream_generator(),
    ) as mock_send_stream:
        response = await anthropic_passthrough_messages(mock_request)
...
async def test_gemini_passthrough_generate_content(store: SqlAlchemyStore):
    ...
    with mock.patch(
        "MLForge.gateway.providers.gemini.send_request", return_value=mock_response
    ) as mock_send:
...
async def test_gemini_passthrough_stream_generate_content(store: SqlAlchemyStore):
    ...
    with mock.patch(
        "MLForge.gateway.providers.gemini.send_stream_request",
        return_value=mock_stream_generator(),
    ) as mock_send_stream:
...
async def test_gateway_creates_trace_with_usage(store: SqlAlchemyStore, handler):
    ...
    with mock.patch(
        "MLForge.gateway.providers.openai.send_request",
        return_value={
            "id": "test-id",
            "object": "chat.completion",
            "created": 1234567890,
...
async def test_openai_passthrough_chat_token_usage_tracking(store: SqlAlchemyStore):
    ...
    with mock.patch("MLForge.gateway.providers.openai.send_request", return_value=mock_response):
        response = await openai_passthrough_chat(mock_request)
...
async def test_anthropic_passthrough_messages_token_usage_tracking(store: SqlAlchemyStore):
    ...
    with mock.patch("MLForge.gateway.providers.anthropic.send_request", return_value=mock_response):
        response = await anthropic_passthrough_messages(mock_request)
...
async def test_gemini_passthrough_generate_content_token_usage_tracking(store: SqlAlchemyStore):
    ...
    with mock.patch("MLForge.gateway.providers.gemini.send_request", return_value=mock_response):
...
async def test_invocations_bypass_header_skips_guardrails(store: SqlAlchemyStore):
    ...
    with (
        patch("MLForge.server.gateway_api._create_provider_from_endpoint_name") as mock_create,
        patch("MLForge.server.gateway_api.load_guardrails") as mock_load,
    ):
        mock_provider = MagicMock()
        mock_provider.chat = AsyncMock(return_value=mock_response)
...
async def test_invocations_bypass_header_wrong_value_runs_guardrails(store: SqlAlchemyStore):
    ...
    with (
        patch("MLForge.genai.scorers.base.Scorer.model_validate", return_value=blocking_scorer),
        patch("MLForge.gateway.providers.openai.OpenAIProvider.chat", AsyncMock()),
    ):
...
async def test_real_db_pre_llm_guardrail_passes(store: SqlAlchemyStore):
    ...
    with (
        patch("MLForge.genai.scorers.base.Scorer.model_validate", return_value=passing_scorer),
        patch(
            "MLForge.gateway.providers.openai.OpenAIProvider.chat",
...
async def test_invocations_before_sanitize_rewrites_request(store: SqlAlchemyStore):
    ...
    with (
        patch("MLForge.genai.scorers.base.Scorer.model_validate", return_value=failing_scorer),
        patch(
            "MLForge.gateway.guardrails.send_request",
            AsyncMock(
                return_value={"choices": [{"message": {"content": json.dumps(sanitized_body)}}]}
            ),
        ),
        patch("MLForge.gateway.providers.openai.OpenAIProvider.chat", side_effect=fake_chat),
    ):
        response = await invocations(endpoint.name, mock_request)
...
async def test_chat_completions_before_sanitize_rewrites_request(store: SqlAlchemyStore):
    ...
    with (
        ...
        from MLForge.server.gateway_api import chat_completions
...
async def test_guardrails_run_in_execution_order(store: SqlAlchemyStore):
    ...
    with (
        patch(
            "MLForge.genai.scorers.base.Scorer.model_validate",

...
def test_dns_rebinding_and_cors_protection(
    MLForge_app_client, host, origin, expected_status, should_block
):
    headers = {"Host": host, "Content-Type": "application/json"}
    if origin:
        headers["Origin"] = origin

    response = MLForge_app_client.post(
        "/api/2.0/MLForge/experiments/search",
        headers=headers,
...
@pytest.mark.parametrize(
    ...
    [
        ("http://malicious-site.com", "/api/2.0/MLForge/experiments/create", True),
        ("http://localhost:3000", "/api/2.0/MLForge/experiments/search", False),
    ],
)
def test_cors_for_state_changing_requests(MLForge_app_client, origin, endpoint, expected_blocked):
    response = MLForge_app_client.post(
        endpoint,
...
def test_cors_with_configured_origins(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("MLForge_SERVER_CORS_ALLOWED_ORIGINS", "https://trusted-app.com")

    from flask import Flask

    from MLForge.server import handlers, security
...
def test_security_headers_on_responses(MLForge_app_client):
    response = MLForge_app_client.get("/health")
...
def test_preflight_options_requests(MLForge_app_client, origin, expected_status, should_have_cors):
    response = MLForge_app_client.options(
        "/api/2.0/MLForge/experiments/search",
        headers={
            "Origin": origin,

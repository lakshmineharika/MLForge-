...
from MLForge.server.otel_api import otel_router
from MLForge.tracing.utils.otlp import OTLP_TRACES_PATH, _set_otel_proto_anyvalue
...
def test_workspace_scoped_otlp_endpoint_sets_workspace(monkeypatch):
    ...
    monkeypatch.setattr(
        "MLForge.server.otel_api._get_tracking_store",
...
def test_otlp_endpoint_without_default_workspace_raises_error(monkeypatch):
    from MLForge.store.workspace_aware_mixin import WorkspaceAwareMixin

    monkeypatch.setenv(MLForge_ENABLE_WORKSPACES.name, "true")

    class DummyWorkspaceAwareStore(WorkspaceAwareMixin):
        """A dummy store that raises MLForgeException when workspace is not set."""

        def log_spans(self, experiment_id, spans):
            # This will raise MLForgeException if workspace context is not set
...
def test_otlp_endpoint_run_linking_error_is_logged(monkeypatch):
    ...
    monkeypatch.setattr(
        "MLForge.server.otel_api._logger.exception",
    ...
    assert response.status_code == 200
    assert logged_messages == ["Failed to link OpenTelemetry traces to MLForge run"]
...
def test_otlp_conversion_error(monkeypatch):
    ...
    monkeypatch.setattr(
        "MLForge.entities.span.Span.from_otel_proto",

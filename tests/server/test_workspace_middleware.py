...
from MLForge.entities import Workspace
from MLForge.environment_variables import MLForge_ENABLE_WORKSPACES
from MLForge.exceptions import MLForgeException
from MLForge.server import app as flask_app
from MLForge.server.fastapi_app import add_fastapi_workspace_middleware
from MLForge.server.job_api import job_api_router
from MLForge.server.workspace_helpers import (
...
def test_flask_workspace_middleware_requires_header(flask_workspace_app, monkeypatch):
    class DefaultlessWorkspaceStore:
        def get_default_workspace(self):
            raise MLForgeException.invalid_parameter_value("Active workspace is required.")
...
def test_fastapi_workspace_middleware_sets_context(monkeypatch):
    ...
    monkeypatch.setattr(
        "MLForge.server.fastapi_app.resolve_workspace_for_request_if_enabled",
...
def test_fastapi_workspace_middleware_prefers_routed_path(monkeypatch):
    ...
    resp = client.get(
        ...
        headers={
            ...
            "Host": "example.com/api/3.0/MLForge/server-info?x=",
        },
    )
...
def test_server_info_workspaces_enabled(monkeypatch):
    ...
    resp = client.get("/api/3.0/MLForge/server-info")
    assert resp.status_code == 200
    data = resp.get_json()
...
def test_server_info_skips_workspace_resolution(monkeypatch):
    ...
    monkeypatch.setattr(
        "MLForge.server.workspace_helpers.resolve_workspace_from_header", _raise_if_called
    )

    client = flask_app.test_client()
    resp = client.get("/api/3.0/MLForge/server-info", headers={WORKSPACE_HEADER_NAME: "missing"})
    assert resp.status_code == 200
    data = resp.get_json()
    assert data["workspaces_enabled"] is True
    assert data["trace_archival_enabled"] is False
...
def test_server_info_with_workspace_header_when_workspaces_disabled(monkeypatch):
    ...
    client = flask_app.test_client()
    resp = client.get(
        "/api/3.0/MLForge/server-info", headers={WORKSPACE_HEADER_NAME: "some-workspace"}
    )
    assert resp.status_code == 200
    data = resp.get_json()
    assert data["workspaces_enabled"] is False
...
def test_fastapi_wsgi_flask_workspace_propagation(monkeypatch):
    ...
    monkeypatch.setattr(
        "MLForge.server.workspace_helpers.workspace_context.is_request_workspace_resolved",

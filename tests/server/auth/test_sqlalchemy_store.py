...
from MLForge.server.auth.entities import User
from MLForge.server.auth.permissions import EDIT, MANAGE, READ, USE
...
def test_create_user(store):
    ...
    # error on duplicate
    with pytest.raises(
        MLForgeException, match=rf"User \(username={username1}\) already exists"
    ...
    with pytest.raises(MLForgeException, match=r"Username cannot be empty") as exception_context:
...
def test_get_user(store):
    ...
    username2 = random_str()
    with pytest.raises(
        MLForgeException, match=rf"User with username={username2} not found"
    ) as exception_context:
...
def _count_synthetic_roles_for(store, user_id: int) -> int:
    from MLForge.server.auth.db.models import SqlRole
...
def test_per_user_grant_does_not_leak_to_other_users(store):
    ...
    with pytest.raises(MLForgeException, match=r"not found") as exc:
...
def test_create_mcp_server_permission(store):
    ...
    with pytest.raises(MLForgeException, match=r"already exists") as exc:

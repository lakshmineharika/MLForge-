...
from MLForge import MLForgeException
from MLForge.environment_variables import (
    ...
    MLForge_RBAC_SEED_DEFAULT_ROLES,
    MLForge_TRACKING_PASSWORD,
    MLForge_TRACKING_USERNAME,
    MLForge_WORKSPACE_STORE_URI,
)
from MLForge.protos.databricks_pb2 import PERMISSION_DENIED, UNAUTHENTICATED, ErrorCode
...
def workspace_client(tmp_path):
    ...
    with _init_server(
        ...
        extra_env={
            ...
            MLForge_RBAC_SEED_DEFAULT_ROLES.name: "true",
        },
        server_type="flask",
    ) as url:
...
def _create_workspace(tracking_uri: str, workspace_name: str):
    response = requests.post(
        f"{tracking_uri}/api/3.0/MLForge/workspaces",
...
def _create_experiment(
    ...
    resp = requests.post(
        f"{tracking_uri}/api/2.0/MLForge/experiments/create",
...
def _create_run(
    ...
    resp = requests.post(
        f"{tracking_uri}/api/2.0/MLForge/runs/create",
...
def _create_registered_model(
    ...
    resp = requests.post(
        f"{tracking_uri}/api/2.0/MLForge/registered-models/create",
...
def _create_model_version(
    ...
    resp = requests.post(
        f"{tracking_uri}/api/2.0/MLForge/model-versions/create",
...
def _graphql_search_runs(
    ...
    query = """
    query SearchRuns($input: MLForgeSearchRunsInput){
      MLForgeSearchRuns(input: $input){
    ...
    search_runs = payload["data"]["MLForgeSearchRuns"]
...
def _graphql_search_model_versions(
    ...
    payload = resp.json()
    assert payload.get("errors") in (None, [])
    return payload["data"]["MLForgeSearchModelVersions"]["modelVersions"]


def test_create_workspace_seeds_default_roles(workspace_client, monkeypatch):
    # The workspace_client fixture forces ``MLForge_RBAC_SEED_DEFAULT_ROLES=true`` in
...
def test_run_access_controls_across_workspaces(workspace_setup, monkeypatch):
    ...
    resp_ok = requests.get(
        f"{tracking_uri}/api/2.0/MLForge/runs/get",
        params={"run_id": run_a},
...
def test_registered_model_access_controls_across_workspaces(workspace_setup, monkeypatch):
    ...
    resp_ok = requests.get(
        f"{tracking_uri}/api/2.0/MLForge/registered-models/get",
        params={"name": model_a},
        auth=(limited_user, limited_password),
        headers={WORKSPACE_HEADER_NAME: workspace_a},
    )

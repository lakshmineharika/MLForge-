...
# exercise the gate decisions in ``MLForge.server.auth`` directly by mocking the
...
from MLForge.protos.review_queues_pb2 import ListReviewQueues
from MLForge.utils.proto_json_utils import message_to_json, parse_dict

# flask-wtf (the auth extra) is required to import the auth app.
pytest.importorskip("flask_wtf")

from MLForge.server import auth
from MLForge.server.auth.permissions import get_permission
...
def test_create_custom_queue_rejects_registered_username(monkeypatch):
    ...
    with pytest.raises(MLForgeException, match="registered user") as exc:
...
def test_admin_owner_reassignment_with_shadowing_rename_is_blocked(monkeypatch):
    ...
    _patch_request(monkeypatch, auth.validate_can_update_review_queue, body)
    monkeypatch.setattr(auth.store, "list_users", lambda: _users("alice", "bob"))
    with pytest.raises(MLForgeException, match="registered user"):
        auth.enforce_review_queue_name_not_username()

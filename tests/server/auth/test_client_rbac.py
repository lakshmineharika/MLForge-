...
def test_update_role_rejects_empty_update(client, monkeypatch):
    with User(ADMIN_USERNAME, ADMIN_PASSWORD, monkeypatch):
        ...
        with pytest.raises(MLForgeException, match="At least one of 'name' or 'description'"):
...
def test_rbac_endpoints_reachable_at_both_path_prefixes(client, monkeypatch, api_prefix):
    ...
    # GET list endpoint.
    resp = requests.get(
        f"{client.tracking_uri}/{api_prefix}/3.0/MLForge/roles/list",
    ...
    resp = requests.get(
        f"{client.tracking_uri}/{api_prefix}/3.0/MLForge/roles/get",

...
from MLForge.server.auth.permissions import (
    ALL_PERMISSIONS,
    EDIT,
    MANAGE,
    NO_PERMISSIONS,
...
def test_validate_resource_grant_rejects_no_permissions(resource_type):
    with pytest.raises(MLForgeException, match="NO_PERMISSIONS"):
...
def test_validate_workspace_grant_rejects_other_tiers(permission):
    with pytest.raises(MLForgeException, match="resource_type='workspace'"):
        _validate_permission_for_resource_type(permission, "workspace")

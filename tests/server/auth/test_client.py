...
import MLForge
from MLForge import MLForgeClient, MLForgeException
...
from MLForge.server.auth.client import AuthServiceClient
...
def client(tmp_path):
    ...
    with _init_server(
        ...
        extra_env={
            ...
            MLForge_AUTH_CONFIG_PATH.name: str(auth_config_path),
        },
        server_type="flask",
    ) as url:
...
def assert_unauthorized():
    with pytest.raises(MLForgeException, match=r"Permission denied.") as exception_context:
        yield
    assert exception_context.value.error_code == ErrorCode.Name(PERMISSION_DENIED)


def test_get_client():
    client = MLForge.server.get_app_client("basic-auth", "uri:/fake")
...
def test_get_current_user(client, monkeypatch):
    ...
    url = f"{client.tracking_uri}/api/2.0/MLForge/users/current"
...
@pytest.mark.parametrize(
    ("path", "method"),
    [
        ("/api/2.0/MLForge/experiments/permissions/get", "GET"),
        ("/api/2.0/MLForge/experiments/permissions/create", "POST"),
        ("/api/2.0/MLForge/experiments/permissions/update", "PATCH"),
        ("/api/2.0/MLForge/experiments/permissions/delete", "DELETE"),
        ("/api/2.0/MLForge/registered-models/permissions/get", "GET"),
        ("/api/2.0/MLForge/registered-models/permissions/create", "POST"),
        ("/api/2.0/MLForge/registered-models/permissions/update", "PATCH"),
        ("/api/2.0/MLForge/registered-models/permissions/delete", "DELETE"),
        ("/api/3.0/MLForge/scorers/permissions/get", "GET"),
        ("/api/3.0/MLForge/scorers/permissions/create", "POST"),
        ("/api/3.0/MLForge/scorers/permissions/update", "PATCH"),
        ("/api/3.0/MLForge/scorers/permissions/delete", "DELETE"),
        ("/api/3.0/MLForge/gateway/secrets/permissions/get", "GET"),
        ("/api/3.0/MLForge/gateway/secrets/permissions/create", "POST"),
        ("/api/3.0/MLForge/gateway/secrets/permissions/update", "PATCH"),
        ("/api/3.0/MLForge/gateway/secrets/permissions/delete", "DELETE"),
        ("/api/3.0/MLForge/gateway/endpoints/permissions/get", "GET"),
        ("/api/3.0/MLForge/gateway/endpoints/permissions/create", "POST"),
        ("/api/3.0/MLForge/gateway/endpoints/permissions/update", "PATCH"),
        ("/api/3.0/MLForge/gateway/endpoints/permissions/delete", "DELETE"),
        ("/api/3.0/MLForge/gateway/model-definitions/permissions/get", "GET"),
        ("/api/3.0/MLForge/gateway/model-definitions/permissions/create", "POST"),
        ("/api/3.0/MLForge/gateway/model-definitions/permissions/update", "PATCH"),
        ("/api/3.0/MLForge/gateway/model-definitions/permissions/delete", "DELETE"),
    ],
)
...
def _register_scorer(tracking_uri: str, experiment_id: str, name: str, auth) -> None:
    resp = requests.post(
        tracking_uri + "/api/3.0/MLForge/scorers/register",
        json={
...
def test_list_scorers_cross_experiment(client, monkeypatch):
    ...
    url = client.tracking_uri + "/ajax-api/3.0/MLForge/scorers/list"
...
def test_update_user_password_self_service_rejects_same_password(client, monkeypatch):
    ...
    with (
        ...
        pytest.raises(MLForgeException, match=r"differ from the current password") as exc,
    ):
...
def test_self_service_password_change_requires_current_password(client, monkeypatch):
    ...
    with User(username, password, monkeypatch):
        ...
        with pytest.raises(MLForgeException, match="Current password is required"):
            client.update_user_password(username, new_password)

        # Wrong current_password: rejected.
        with pytest.raises(MLForgeException, match="Current password does not match"):
...
def test_create_user_with_null_or_missing_json_body_returns_400(client, monkeypatch):
    ...
    url = f"{client.tracking_uri}/api/2.0/MLForge/users/create"
...
def test_self_service_password_change_with_null_body_returns_400(client, monkeypatch):
    ...
    url = f"{client.tracking_uri}/api/2.0/MLForge/users/update-password"
    resp = requests.patch(
        url,
        data="null",
        headers={"Content-Type": "application/json"},
...
def test_delete_user_rejects_self_delete(client, monkeypatch):
    with (
        ...
        pytest.raises(MLForgeException, match=r"cannot delete their own account") as exc,
    ):
...
def _create_experiment(tracking_uri: str, monkeypatch, name: str) -> str:
    ...
    """
    with User(ADMIN_USERNAME, ADMIN_PASSWORD, monkeypatch):
        return MLForgeClient(tracking_uri).create_experiment(name)
...
def test_grant_user_permission_duplicate_raises(client, monkeypatch):
    ...
    with User(ADMIN_USERNAME, ADMIN_PASSWORD, monkeypatch):
        client.grant_user_permission(username, "experiment", exp_id, "READ")
        with pytest.raises(MLForgeException, match="already exists") as exc:
            client.grant_user_permission(username, "experiment", exp_id, "EDIT")
    from MLForge.protos.databricks_pb2 import RESOURCE_ALREADY_EXISTS
...
def test_grant_user_permission_unknown_user_raises(client, monkeypatch):
    with User(ADMIN_USERNAME, ADMIN_PASSWORD, monkeypatch):
        with pytest.raises(MLForgeException, match="not found") as exc:
...
def test_grant_user_permission_invalid_resource_type(client, monkeypatch):
    username, _ = _new_user(client, monkeypatch)
    with User(ADMIN_USERNAME, ADMIN_PASSWORD, monkeypatch):
        with pytest.raises(MLForgeException, match="Invalid resource type"):
...
def test_grant_user_permission_malformed_scorer_id_returns_clean_4xx(client, monkeypatch):
    ...
    # (a before-request validator). Pin that the wrapping ``catch_MLForge_exception``
    ...
    with User(requester_username, requester_password, monkeypatch):
        with pytest.raises(MLForgeException, match="Invalid scorer resource_id") as exc:
...
def test_admin_cannot_target_workspace_resource_type(client, monkeypatch, api_method, args):
    ...
    username, _ = _new_user(client, monkeypatch)
    with User(ADMIN_USERNAME, ADMIN_PASSWORD, monkeypatch):
        with pytest.raises(MLForgeException, match="not supported by the per-user"):
...
def test_grant_user_permission_invalid_permission_for_resource_type(client, monkeypatch):
    ...
    username, _ = _new_user(client, monkeypatch)
    with User(ADMIN_USERNAME, ADMIN_PASSWORD, monkeypatch):
        with pytest.raises(MLForgeException, match="Invalid permission"):
...
def test_get_user_permission_self_check(client, monkeypatch):
    ...
    # ``can_use=False`` in ``MLForge.server.auth.permissions`` (see ``READ =``),
...
def test_unified_permission_endpoints_reachable_at_both_path_prefixes(
    ...
    # The MLForge frontend hits /ajax-api/ paths; the Python client hits /api/ paths.
...
@pytest.mark.parametrize(
    ("path", "method"),
    [
        ("/api/2.0/MLForge/workspaces/team-a/permissions/get", "GET"),
        ("/api/2.0/MLForge/workspaces/team-a/permissions/create", "POST"),
        ("/api/2.0/MLForge/workspaces/team-a/permissions/update", "PATCH"),
        ("/api/2.0/MLForge/workspaces/team-a/permissions/delete", "DELETE"),
        ("/api/3.0/MLForge/workspaces/team-a/permissions/get", "GET"),
        ("/api/3.0/MLForge/workspaces/team-a/permissions/create", "POST"),
        ("/api/3.0/MLForge/workspaces/team-a/permissions/update", "PATCH"),
        ("/api/3.0/MLForge/workspaces/team-a/permissions/delete", "DELETE"),
    ],
)

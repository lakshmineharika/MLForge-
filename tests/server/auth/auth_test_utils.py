...
from MLForge.server.auth import auth_config
...
def write_isolated_auth_config(tmp_path: Path) -> Path:
    ...
    ``MLForge_AUTH_CONFIG_PATH`` at the returned file (through ``extra_env``) so
    ...
    config_path.write_text(
        ...
        "authorization_function = MLForge.server.auth:authenticate_request_basic_auth\n"
...
def grant_role_permission(
    ...
    ``POST /MLForge/experiments/permissions/create`` (and equivalents) to set up state.
    """
    ...
    add_resp = _send_rest_tracking_post_request(
        tracking_uri,
        "/api/3.0/MLForge/roles/permissions/add",
        {
            "role_id": role_id,
...
class User:
    ...
    def __enter__(self):
        self.monkeypatch.setenv(MLForge_TRACKING_USERNAME.name, self.username)
        self.monkeypatch.setenv(MLForge_TRACKING_PASSWORD.name, self.password)

    def __exit__(self, exc_type, exc_val, exc_tb):
        self.monkeypatch.delenv(MLForge_TRACKING_USERNAME.name, raising=False)
        self.monkeypatch.delenv(MLForge_TRACKING_PASSWORD.name, raising=False)

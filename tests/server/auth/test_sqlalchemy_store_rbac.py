...
from MLForge.server.auth.entities import Role, RolePermission, UserRoleAssignment
from MLForge.server.auth.permissions import EDIT, MANAGE, READ, USE, VALID_RESOURCE_TYPES
...
def test_create_role_rejects_reserved_user_prefix(store, name):
    ...
    with pytest.raises(MLForgeException, match="reserved '__user_' prefix"):
...
def test_add_role_permission_workspace_requires_wildcard(store):
    ...
    with pytest.raises(MLForgeException, match="resource_type='workspace' requires"):

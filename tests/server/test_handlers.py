...
from MLForge.entities._job import Job as JobEntity
from MLForge.entities._job_status import JobStatus
from MLForge.entities.gateway_budget_policy import (
    BudgetAction,
    BudgetDuration,
    BudgetDurationUnit,
    BudgetTargetScope,
...
from MLForge.entities.model_registry import (
    ModelVersion,
    ModelVersionTag,
    PromptVersion,
    RegisteredModel,
...
from MLForge.entities.model_registry.prompt_version import IS_PROMPT_TAG_KEY, PROMPT_TEXT_TAG_KEY
from MLForge.entities.presigned_download import PresignedDownloadUrlResponse
from MLForge.entities.presigned_upload import CreatePresignedUploadResponse
from MLForge.entities.trace_location import TraceLocation as EntityTraceLocation
...
from MLForge.environment_variables import MLForge_ENABLE_WORKSPACES
from MLForge.exceptions import (
    MLForgeException,
    MLForgeNotImplementedException,
    MLForgeTracingException,
)
from MLForge.gateway.budget_tracker.in_memory import InMemoryBudgetTracker
from MLForge.genai.review_queues import ReviewQueueType
from MLForge.genai.review_queues.review_queues import (
    ReviewItemType,
    ReviewQueue,
    ReviewQueueItem,
    ReviewStatus,
)
from MLForge.genai.scorers.online.entities import OnlineScoringConfig
...
from MLForge.protos.issues_pb2 import (
    CreateIssue,
    SearchIssues,
    UpdateIssue,
)
from MLForge.protos.model_registry_pb2 import (
    CreateModelVersion,
...
from MLForge.protos.prompt_optimization_pb2 import (
    OPTIMIZER_TYPE_GEPA,
    OPTIMIZER_TYPE_METAPROMPT,
    OPTIMIZER_TYPE_UNSPECIFIED,
)
from MLForge.protos.review_queues_pb2 import (
    CreateReviewQueue,
    GetOrCreateUserQueue,
    SetReviewQueueItemStatus,
    UpdateReviewQueue,
)
from MLForge.protos.service_pb2 import (
    BatchGetTraceInfos,
    BatchGetTraces,
...
from MLForge.protos.webhooks_pb2 import ListWebhooks
from MLForge.server import (
...
from MLForge.server.handlers import (
    ...
    _delete_artifact_MLForge_artifacts,
...
from MLForge.store._unity_catalog.registry.rest_store import UcModelRegistryStore
from MLForge.store.artifact.artifact_repo import ArtifactRepository
from MLForge.store.artifact.azure_blob_artifact_repo import AzureBlobArtifactRepository
...
from MLForge.store.artifact.s3_artifact_repo import S3ArtifactRepository
...
from MLForge.store.model_registry.rest_store import RestStore as ModelRegistryRestStore
from MLForge.store.tracking import MAX_RESULTS_QUERY_TRACE_METRICS
from MLForge.store.tracking.databricks_rest_store import DatabricksTracingRestStore
from MLForge.telemetry.schemas import Record, Status
from MLForge.tracing.analysis import TraceFilterCorrelationResult
from MLForge.tracing.constant import SpansLocation, TraceTagKey
from MLForge.tracing.utils import build_otel_context
from MLForge.utils.MLForge_tags import MLForge_ARTIFACT_LOCATION
from MLForge.utils.proto_json_utils import message_to_json
from MLForge.utils.validation import MAX_BATCH_LOG_REQUEST_SIZE
from MLForge.utils.workspace_context import WorkspaceContext
from MLForge.utils.workspace_utils import DEFAULT_WORKSPACE_NAME


@pytest.fixture
def mock_get_request_message():
    with mock.patch("MLForge.server.handlers._get_request_message") as m:
        yield m


@pytest.fixture
def mock_get_request_json():
    with mock.patch("MLForge.server.handlers._get_request_json") as m:
        yield m


@pytest.fixture
def mock_tracking_store():
    with mock.patch("MLForge.server.handlers._get_tracking_store") as m:
...
def mock_model_registry_store():
    with mock.patch("MLForge.server.handlers._get_model_registry_store") as m:
        mock_store = mock.MagicMock()
...
def mock_evaluation_dataset():
    from MLForge.protos.datasets_pb2 import Dataset as ProtoDataset
...
def mock_telemetry_config_cache():
    with mock.patch("MLForge.server.handlers._telemetry_config_cache", {}) as m:
        yield m


@pytest.fixture
def bypass_telemetry_env_check(monkeypatch):
    monkeypatch.setattr(MLForge.telemetry.utils, "_IS_MLForge_TESTING_TELEMETRY", False)
    monkeypatch.setattr(MLForge.telemetry.utils, "_IS_IN_CI_ENV_OR_TESTING", False)
    monkeypatch.setattr(MLForge.telemetry.utils, "_IS_MLForge_DEV_VERSION", False)


@pytest.fixture
def mock_job_store():
    with mock.patch("MLForge.server.handlers._get_job_store") as m:
        mock_store = mock.MagicMock()
        m.return_value = mock_store
        yield mock_store
...
def _create_mock_job(
    ...
    from MLForge.entities._job import Job
    from MLForge.entities._job_status import JobStatus
...
def test_version():
    with app.test_client() as c:
        ...
        assert response.status_code == 200
        assert response.get_data().decode() == MLForge.__version__


def test_server_info():
    with app.test_client() as c:
        response = c.get("/api/3.0/MLForge/server-info")
        assert response.status_code == 200
...
def test_server_info_trace_archival_enabled(monkeypatch):
    monkeypatch.setattr(
        "MLForge.server.handlers.get_trace_archival_server_config",
        mock.Mock(return_value=mock.Mock(enabled=True)),
    )
    monkeypatch.setattr("MLForge.server.handlers._store_supports_trace_archival", lambda store: True)
...
def test_server_info_handles_invalid_trace_archival_config(monkeypatch):
    monkeypatch.setattr(
        "MLForge.server.handlers.get_trace_archival_server_config",
        mock.Mock(
            side_effect=MLForgeException.invalid_parameter_value("invalid trace archival config")
        ),
    )
...
def test_server_info_multipart_capabilities_with_multipart_backend(monkeypatch):
    from MLForge.store.artifact.artifact_repo import MultipartDownloadMixin, MultipartUploadMixin
    ...
    monkeypatch.setattr("MLForge.server.handlers._is_serving_proxied_artifacts", lambda: True)
    monkeypatch.setattr(
        "MLForge.server.handlers._get_artifact_repo_MLForge_artifacts",
...
def test_convert_path_parameter_to_flask_format():
    converted = _convert_path_parameter_to_flask_format("/MLForge/trace")
    assert "/MLForge/trace" == converted

    converted = _convert_path_parameter_to_flask_format("/MLForge/trace/{request_id}")
    assert "/MLForge/trace/<request_id>" == converted

    converted = _convert_path_parameter_to_flask_format("/MLForge/{foo}/{bar}/{baz}")
    assert "/MLForge/<foo>/<bar>/<baz>" == converted
...
def test_can_block_post_request_with_invalid_content_type():
    ...
    request.get_json = mock.MagicMock()
    request.get_json.return_value = {"name": "hello"}
    with pytest.raises(MLForgeException, match=r"Bad Request. Content-Type"):
...
def test_catch_MLForge_exception():
    @catch_MLForge_exception
    def test_handler():
        raise MLForgeException("test error", error_code=INTERNAL_ERROR)
...
def test_MLForge_server_with_installed_plugin(tmp_path, monkeypatch):
    pytest.skip("FileStore is no longer supported.")
    from MLForge_test_plugin.file_store import PluginFileStore

    monkeypatch.setenv(BACKEND_STORE_URI_ENV_VAR, f"file-plugin:{tmp_path}")
    monkeypatch.setattr(MLForge.server.handlers, "_tracking_store", None)
    plugin_file_store = MLForge.server.handlers._get_tracking_store()
...
def test_delete_artifact_MLForge_artifacts_throws_for_malicious_path(enable_serve_artifacts, path):
    response = _delete_artifact_MLForge_artifacts(path)
    assert response.status_code == 400
    json_response = json.loads(response.get_data())
    assert json_response["error_code"] == ErrorCode.Name(INVALID_PARAMETER_VALUE)
...
def test_get_presigned_download_url_success(enable_serve_artifacts, monkeypatch):
    from MLForge.store.artifact.artifact_repo import MultipartDownloadMixin
...
def test_create_presigned_upload_url_success():
    from MLForge.store.artifact.artifact_repo import PresignedUploadMixin
    ...
    from MLForge.protos.service_pb2 import CreatePresignedUploadUrl
    ...
    with (
        ...
        mock.patch(
            "MLForge.server.handlers._get_request_message",
            return_value=request_proto,
        ),
        mock.patch(
            "MLForge.server.handlers._get_tracking_store",
        ) as mock_store,
        mock.patch(
            "MLForge.server.handlers._get_artifact_repo",
...
@pytest.mark.parametrize(
    "artifact_uri",
    [
        "MLForge-artifacts:/0/abc123/artifacts",
        "http://MLForge-server:5000/api/2.0/MLForge-artifacts/artifacts",
        "https://MLForge-server/api/2.0/MLForge-artifacts/artifacts",
    ],
)
...
def test_create_presigned_upload_url_invalid_run_id():
    ...
    with (
        ...
        mock_store.return_value.get_run.side_effect = MLForgeException(
...
def test_create_presigned_upload_url_blocked_in_artifacts_only_mode(monkeypatch):
    from MLForge.server import ARTIFACTS_ONLY_ENV_VAR
...
def test_create_presigned_download_url_success():
    ...
    from MLForge.protos.service_pb2 import CreatePresignedDownloadUrl
...
def test_create_presigned_download_url_default_expiration(monkeypatch):
    ...
    # Don't set expiration - should default to MLForge_PRESIGNED_DOWNLOAD_URL_TTL_SECONDS (300).
    request_proto = CreatePresignedDownloadUrl()
    request_proto.run_id = "abc123"
    request_proto.path = "model.pkl"
    ...
    monkeypatch.setenv("MLForge_PRESIGNED_DOWNLOAD_URL_TTL_SECONDS", "123")
    with (
        app.test_request_context(method="POST", content_type="application/json"),
        mock.patch(
            "MLForge.server.handlers._get_request_message",
...
def test_create_presigned_download_url_rejects_out_of_range_default_expiration(monkeypatch):
    ...
    # MLForge_PRESIGNED_DOWNLOAD_URL_TTL_SECONDS is rejected too — not just an
    # out-of-range value sent by the client.
    monkeypatch.setenv("MLForge_PRESIGNED_DOWNLOAD_URL_TTL_SECONDS", "999999999")
...
def test_local_file_read_write_by_pass_vulnerability(uri):
    ...
    msg = _get_request_message(CreateExperiment(), flask_request=request)
    with mock.patch("MLForge.server.handlers._get_request_message", return_value=msg):
    ...
    run_id = uuid.uuid4().hex
    with mock.patch("MLForge.server.handlers._get_tracking_store") as mock_get_tracking_store:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "https://127.0.0.1/api/2.0/MLForge-artifacts/artifacts/2/traces/123",
            S3ArtifactRepository,
            "s3://bucket/2/traces/123",
        ),
        ("MLForge-artifacts:/1/traces/123", S3ArtifactRepository, "s3://bucket/1/traces/123"),
    ],
)
def test_get_trace_artifact_repo(location, expected_class, expected_uri, monkeypatch):
    ...
    trace_info = TraceInfo(
        ...
        state=TraceState.OK,
        tags={MLForge_ARTIFACT_LOCATION: location},
    )
...
def test_register_scorer_rejects_decorator_scorer(mock_get_request_message, mock_tracking_store):
    from MLForge.genai.scorers.scorer_utils import DECORATOR_SCORER_REGISTRATION_NOT_SUPPORTED_ERROR
...
def test_search_traces_v3_empty_page_token(mock_get_request_message, mock_tracking_store):
    ...
    location.MLForge_experiment.experiment_id = "1"
...
def test_batch_get_traces_handler(mock_get_request_message, mock_tracking_store):
    ...
    otel_span = OTelReadableSpan(
        ...
        end_time=200,
        attributes={
            "MLForge.spanInputs": json.dumps("inputs"),
            "MLForge.spanOutputs": json.dumps("outputs"),
            "MLForge.spanType": json.dumps("span_type"),
        },
    )
...
def test_get_trace_handler_not_found(mock_get_request_message, mock_tracking_store):
    ...
    mock_tracking_store.get_trace.side_effect = MLForgeException(
...
def test_get_trace_artifact_handler(mock_tracking_store):
    ...
    otel_span = OTelReadableSpan(
        ...
        end_time=200,
        attributes={
            "MLForge.spanInputs": json.dumps({"input": "test_input"}),
            "MLForge.spanOutputs": json.dumps({"output": "test_output"}),
        },
    )
    mock_span = Span(otel_span)
...
def test_get_trace_artifact_handler_fallback_to_batch_get_traces(mock_tracking_store):
    ...
    otel_span = OTelReadableSpan(
        ...
        end_time=200,
        attributes={
            "MLForge.spanInputs": json.dumps({"input": "batch_input"}),
            "MLForge.spanOutputs": json.dumps({"output": "batch_output"}),
        },
    )
    mock_span = Span(otel_span)
    ...
    mock_tracking_store.get_trace.side_effect = MLForgeNotImplementedException(
...
def test_get_trace_artifact_handler_fallback_to_artifact_repo(mock_tracking_store):
    ...
    mock_tracking_store.batch_get_traces.side_effect = MLForgeNotImplementedException(
    ...
    with mock.patch(
        "MLForge.server.handlers._get_trace_artifact_repo", return_value=mock_artifact_repo
    ):
        with app.test_request_context(method="GET", query_string={"request_id": trace_id}):
            response = get_trace_artifact_handler()
...
def test_list_providers():
    with app.test_client() as c:
        response = c.get("/ajax-api/3.0/MLForge/gateway/supported-providers")
        assert response.status_code == 200
        data = response.get_json()
...
def test_list_providers_with_allowed_filter(monkeypatch):
    monkeypatch.setenv("MLForge_GATEWAY_ALLOWED_PROVIDERS", "openai,anthropic")
    with app.test_client() as c:
        response = c.get("/ajax-api/3.0/MLForge/gateway/supported-providers")
        assert response.status_code == 200
        data = response.get_json()
...
def test_list_models():
    with app.test_client() as c:
        response = c.get("/ajax-api/3.0/MLForge/gateway/supported-models?provider=openai")
        assert response.status_code == 200
        data = response.get_json()
...
def test_list_models_all_providers():
    with app.test_client() as c:
        response = c.get("/ajax-api/3.0/MLForge/gateway/supported-models")
        assert response.status_code == 200
        data = response.get_json()
        assert "models" in data
        assert isinstance(data["models"], list)
...
def test_get_provider_config():
    with app.test_client() as c:
        response = c.get("/ajax-api/3.0/MLForge/gateway/provider-config?provider=openai")
        assert response.status_code == 200
        data = response.get_json()
...
def test_get_provider_config_with_multiple_auth_modes():
    with app.test_client() as c:
        response = c.get("/ajax-api/3.0/MLForge/gateway/provider-config?provider=bedrock")
        assert response.status_code == 200
        data = response.get_json()
...
def test_get_provider_config_missing_provider():
    with app.test_client() as c:
        response = c.get("/ajax-api/3.0/MLForge/gateway/provider-config")
        assert response.status_code == 400


def test_get_provider_config_with_allowed_filter(monkeypatch):
    monkeypatch.setenv("MLForge_GATEWAY_ALLOWED_PROVIDERS", "anthropic")
    with app.test_client() as c:
        response = c.get("/ajax-api/3.0/MLForge/gateway/provider-config?provider=openai")
        assert response.status_code == 400
        data = response.get_json()
        ...
        response = c.get("/ajax-api/3.0/MLForge/gateway/provider-config?provider=anthropic")
        assert response.status_code == 200
...
def test_create_gateway_endpoint_rejects_invalid_name(mock_get_request_message, invalid_name):
    from MLForge.protos.service_pb2 import CreateGatewayEndpoint
    from MLForge.server.handlers import _create_gateway_endpoint
...
def test_update_gateway_endpoint_rejects_invalid_name(mock_get_request_message, invalid_name):
    from MLForge.protos.service_pb2 import UpdateGatewayEndpoint
    from MLForge.server.handlers import _update_gateway_endpoint
...
def test_invoke_scorer_missing_experiment_id():
    with app.test_client() as c:
        response = c.post(
            "/ajax-api/3.0/MLForge/scorer/invoke",
...
def test_invoke_scorer_submits_jobs(mock_tracking_store):
    ...
    with mock.patch("MLForge.server.jobs.submit_job") as mock_submit:
...
def test_get_ui_telemetry_handler(
    ...
    with mock.patch(
        "MLForge.server.handlers.fetch_ui_telemetry_config", return_value=config
    ) as mock_fetch:
...
def test_get_ui_telemetry_handler_disabled_by_env(
    ...
    with mock.patch("MLForge.server.handlers.fetch_ui_telemetry_config") as mock_fetch:
        response = get_ui_telemetry_handler()
        assert response is not None
        assert response.status_code == 200
        response_data = json.loads(response.get_data())
...
def test_post_ui_telemetry_handler_success(
    ...
    with (
        ...
        mock.patch("MLForge.server.handlers.fetch_ui_telemetry_config", return_value=config),
        mock.patch("MLForge.server.handlers.get_telemetry_client", return_value=mock_client),
        mock.patch(
            "MLForge.server.handlers.get_or_create_installation_id",
...
def test_post_ui_telemetry_handler_telemetry_disabled_by_env(
    ...
    with (
        ...
        mock.patch("MLForge.server.handlers.fetch_ui_telemetry_config") as mock_fetch,
        mock.patch("MLForge.server.handlers.get_telemetry_client") as mock_get_client,
    ):
        response = post_ui_telemetry_handler()
...
def test_send_artifact_prefers_local_path(tmp_path):
    ...
    with (
        app.test_request_context(method="GET"),
        mock.patch("MLForge.server.handlers.tempfile.TemporaryDirectory") as mock_tmp_dir,
    ):
...
def test_download_artifact_uses_local_path_fast_path(enable_serve_artifacts, tmp_path):
    ...
    with (
        app.test_request_context(method="GET"),
        mock.patch("MLForge.server.handlers._get_artifact_repo_MLForge_artifacts") as mock_repo,
        mock.patch("MLForge.server.handlers.tempfile.TemporaryDirectory") as mock_tmp_dir,
    ):
...
def test_download_artifact_endpoint_non_ascii_filename(
    ...
    # full `/api/2.0/MLForge-artifacts/artifacts/<artifact_path>` route to guard
    ...
    # which MLForge's artifact registry parses as scheme=``C`` and 500s.
    monkeypatch.setenv(ARTIFACTS_DESTINATION_ENV_VAR, artifact_root.as_uri())
    monkeypatch.setattr("MLForge.server.handlers._artifact_repo", None)

    quoted_path = urllib.parse.quote(filename)
    with app.test_client() as c:
        response = c.get(f"/api/2.0/MLForge-artifacts/artifacts/{quoted_path}")
...
def test_create_prompt_optimization_job(mock_tracking_store):
    ...
    mock_dataset._to_MLForge_entity.return_value = mock.MagicMock()

    with (
        mock.patch("MLForge.server.jobs.submit_job", return_value=mock_job_entity),
        mock.patch("MLForge.server.handlers._get_user", return_value="test_user"),
        mock.patch(
            "MLForge.genai.datasets.get_dataset", return_value=mock_dataset
        ) as mock_get_dataset,
    ):
        with app.test_request_context(
            method="POST",
...
def test_cancel_prompt_optimization_job():
    ...
    with (
        mock.patch("MLForge.server.jobs.cancel_job", return_value=mock_job_entity),
        mock.patch("MLForge.server.handlers._get_tracking_store") as mock_store,
    ):
...
def test_get_prompt_optimization_job_pending(mock_tracking_store):
    ...
    with mock.patch("MLForge.server.jobs.get_job", return_value=mock_job):
        with app.test_client() as c:
            response = c.get("/ajax-api/3.0/MLForge/prompt-optimization/jobs/job-123")
            assert response.status_code == 200
...
def test_search_prompt_optimization_jobs_returns_multiple_jobs(mock_job_store):
    ...
    with app.test_client() as c:
        response = c.post(
            "/ajax-api/3.0/MLForge/prompt-optimization/jobs/search",
...
def test_delete_prompt_optimization_job_success(mock_job_store, mock_tracking_store):
    ...
    with app.test_client() as c:
        response = c.delete("/ajax-api/3.0/MLForge/prompt-optimization/jobs/job-123")
        assert response.status_code == 200
...
def test_delete_prompt_optimization_job_skips_run_deletion_when_run_not_found(
    ...
    mock_job_store.get_job.return_value = mock_job
    mock_tracking_store.get_run.side_effect = MLForgeException("Run not found")
...
def test_initialize_workspace_store_does_not_initialize_backend_stores():
    ...
    with (
        mock.patch("MLForge.server.handlers._get_workspace_store") as get_workspace_store,
        mock.patch("MLForge.server.handlers._get_tracking_store") as get_tracking_store,
        mock.patch("MLForge.server.handlers._get_model_registry_store") as get_registry_store,
    ):
...
def test_get_workspace_scoped_repo_path_if_enabled_prevents_cross_workspace_access(monkeypatch):
    ...
    with WorkspaceContext("team-a"):
        with pytest.raises(MLForgeException, match="targets workspace 'team-b'"):
            _get_workspace_scoped_repo_path_if_enabled("workspaces/team-b/secret.txt")

        with pytest.raises(MLForgeException, match="targets workspace 'other'"):
...
def test_get_workspace_scoped_repo_path_if_enabled_rejects_empty_workspace_in_path(monkeypatch):
    ...
    with WorkspaceContext("team-a"):
        with pytest.raises(MLForgeException, match="must include a workspace name"):
...
def test_get_workspace_scoped_repo_path_if_enabled_requires_active_workspace(monkeypatch):
    ...
    with pytest.raises(MLForgeException, match="Active workspace is required"):
...
def test_get_artifact_handler_applies_workspace_scoping(monkeypatch):
    ...
    mock_run = mock.MagicMock()
    mock_run.info.artifact_uri = "MLForge-artifacts:/exp1/run1/artifacts"
    ...
    with (
        mock.patch("MLForge.server.handlers._get_tracking_store") as mock_store,
        mock.patch("MLForge.server.handlers._get_artifact_repo_MLForge_artifacts") as mock_repo,
        mock.patch("MLForge.server.handlers._send_artifact") as mock_send,
    ):
        mock_store.return_value.get_run.return_value = mock_run
        mock_repo.return_value = mock_artifact_repo
...
def test_get_model_version_artifact_handler_applies_workspace_scoping(monkeypatch):
    ...
    with (
        mock.patch("MLForge.server.handlers._get_model_registry_store") as mock_store,
        mock.patch("MLForge.server.handlers._get_artifact_repo_MLForge_artifacts") as mock_repo,
        mock.patch("MLForge.server.handlers._send_artifact") as mock_send,
    ):
        mock_store.return_value.get_model_version_download_uri.return_value = (
            "MLForge-artifacts:/models/MyModel/1"
        )
        mock_repo.return_value = mock_artifact_repo
...
def test_get_logged_model_artifact_handler_applies_workspace_scoping(monkeypatch):
    ...
    mock_logged_model.artifact_location = "MLForge-artifacts:/exp1/run1/artifacts/model"
...
def test_list_artifacts_for_proxied_run_artifact_root_applies_workspace_scoping(monkeypatch):
    ...
    with (
        ...
        _list_artifacts_for_proxied_run_artifact_root(
            proxied_artifact_root="MLForge-artifacts:/exp1/run1/artifacts",
            relative_path="model",
        )
...
def test_list_budget_windows_empty():
    with (
        ...
        mock.patch("MLForge.server.handlers.get_budget_tracker") as mock_tracker,
        mock.patch("MLForge.server.handlers.maybe_refresh_budget_policies"),
    ):
        mock_tracker.return_value.get_all_windows.return_value = []
        response = c.get("/ajax-api/3.0/MLForge/gateway/budgets/windows")
...
def test_list_budget_windows_returns_window_data():
    ...
    with (
        app.test_client() as c,
        mock.patch("MLForge.server.handlers.get_budget_tracker", return_value=tracker),
        mock.patch("MLForge.server.handlers.maybe_refresh_budget_policies"),
    ):
        response = c.get("/ajax-api/3.0/MLForge/gateway/budgets/windows")
...
def test_create_issue_with_all_fields():
    ...
    with (
        mock.patch("MLForge.server.handlers._get_tracking_store") as mock_store,
        mock.patch("MLForge.server.handlers._get_request_message", return_value=request_message),
    ):
...
def test_get_issue():
    ...
    with mock.patch("MLForge.server.handlers._get_tracking_store") as mock_store:
...
def test_get_issue_not_found():
    with mock.patch("MLForge.server.handlers._get_tracking_store") as mock_store:
        mock_store.return_value.get_issue.side_effect = MLForgeException(
        ...
        # The @catch_MLForge_exception decorator catches and returns error as JSON
        assert response.status_code == 404
        json_response = json.loads(response.get_data())
        assert json_response["error_code"] == ErrorCode.Name(RESOURCE_DOES_NOT_EXIST)
...
def test_invoke_issue_detection_handler_success(monkeypatch):
    monkeypatch.setenv("MLForge_SERVER_ENABLE_JOB_EXECUTION", "true")
    ...
    with (
        mock.patch("MLForge.server.handlers._get_tracking_store") as mock_store,
        mock.patch(
            "MLForge.genai.discovery.job._fetch_provider_credentials",
            return_value={"OPENAI_API_KEY": "test-key"},
        ) as mock_fetch_creds,
        mock.patch("MLForge.server.jobs.submit_job", return_value=mock_job) as mock_submit_job,
        mock.patch("MLForge.start_run", return_value=mock_run),
        mock.patch("MLForge.set_tag"),
        mock.patch("MLForge.end_run"),
        app.test_client() as c,
    ):
        resp = c.post(
            "/ajax-api/3.0/MLForge/issues/invoke",
            json=request_json,
        )
        assert resp.status_code == 200
...
def test_invoke_genai_evaluate_handler_success(monkeypatch):
    ...
    with (
        mock.patch("MLForge.server.jobs.submit_job", return_value=mock_job) as mock_submit_job,
        mock.patch("MLForge.server.handlers.MLForgeClient", return_value=mock_client),
        app.test_client() as c,
    ):
        resp = c.post("/ajax-api/3.0/MLForge/genai/evaluate/invoke", json=request_json)
        assert resp.status_code == 200
        json_response = resp.get_json()

        assert json_response == {"job_id": "job-genai-1", "run_id": "run-genai-1"}

        # The handler must create the run *upfront* with the right MLForge_RUN_TYPE tag
        # so the run appears on /evaluation-runs immediately, before the job starts
        # producing artifacts. We use MLForgeClient (not MLForge.start_run) so the run
        ...
        mock_client.create_run.assert_called_once_with(
            experiment_id="exp-123",
            tags={"MLForge.runType": "genai_evaluate"},
        )
        ...
        mock_client.set_tag.assert_called_once_with(
            "run-genai-1", "MLForge.genaiEvaluate.jobId", "job-genai-1"
        )
...
def test_invoke_genai_evaluate_handler_rejects_empty_trace_ids(monkeypatch):
    ...
    with (
        mock.patch("MLForge.server.handlers.MLForgeClient", return_value=mock_client),
        mock.patch("MLForge.server.jobs.submit_job") as mock_submit_job,
        app.test_client() as c,
    ):
        resp = c.post("/ajax-api/3.0/MLForge/genai/evaluate/invoke", json=request_json)
        assert resp.status_code == 400
...
def test_invoke_genai_evaluate_handler_propagates_basic_auth_username(monkeypatch):
    ...
    with (
        ...
        resp = c.post(
            "/ajax-api/3.0/MLForge/genai/evaluate/invoke",
            json=request_json,
...
def test_invoke_genai_evaluate_handler_marks_run_failed_when_submit_job_raises(monkeypatch):
    ...
    submit_error = MLForgeException(
        "MLForge server job execution feature is not enabled.",
        error_code=INVALID_PARAMETER_VALUE,
    )

    with (
        mock.patch("MLForge.server.jobs.submit_job", side_effect=submit_error),
        mock.patch("MLForge.server.handlers.MLForgeClient", return_value=mock_client),
        app.test_client() as c,
    ):
        resp = c.post("/ajax-api/3.0/MLForge/genai/evaluate/invoke", json=request_json)

        # @catch_MLForge_exception turns the re-raised MLForgeException into a 4xx.
        assert resp.status_code != 200
...
def test_invoke_genai_evaluate_handler_marks_run_failed_when_set_tag_raises(monkeypatch):
    ...
    with (
        mock.patch("MLForge.server.jobs.submit_job", return_value=mock_job),
        mock.patch("MLForge.server.handlers.MLForgeClient", return_value=mock_client),
        app.test_client() as c,
    ):
        resp = c.post("/ajax-api/3.0/MLForge/genai/evaluate/invoke", json=request_json)
...
def test_get_job_success(mock_job_store):
    ...
    with (
        mock.patch("MLForge.server.jobs.get_job", return_value=mock_job),
        app.test_client() as c,
    ):
        resp = c.get("/ajax-api/3.0/MLForge/jobs/job-123")
        assert resp.status_code == 200
        json_response = resp.get_json()
...
def test_get_job_pending(mock_job_store):
    ...
    with (
        ...
        resp = c.get("/ajax-api/3.0/MLForge/jobs/job-pending")
        assert resp.status_code == 200
        json_response = resp.get_json()
...
def test_cancel_job_success(mock_job_store):
    ...
    with (
        mock.patch("MLForge.server.jobs.cancel_job", return_value=mock_job) as mock_cancel,
        app.test_client() as c,
    ):
        resp = c.patch("/ajax-api/3.0/MLForge/jobs/cancel/job-123")
        assert resp.status_code == 200
        json_response = resp.get_json()
...
def test_get_rest_path_respects_static_prefix(monkeypatch):
    ...
    assert _get_rest_path("/MLForge/experiments/search") == "/api/2.0/MLForge/experiments/search"
    assert _get_ajax_path("/MLForge/experiments/search") == "/ajax-api/2.0/MLForge/experiments/search"

    # With prefix, both should include the prefix
    monkeypatch.setenv(STATIC_PREFIX_ENV_VAR, "/myapp")
    assert (
        _get_rest_path("/MLForge/experiments/search") == "/myapp/api/2.0/MLForge/experiments/search"
    )
    assert (
        _get_ajax_path("/MLForge/experiments/search")
        == "/myapp/ajax-api/2.0/MLForge/experiments/search"
    )
...
def test_create_review_queue_stamps_owner_from_authenticated_user():
    ...
    with (
        mock.patch("MLForge.server.handlers._get_tracking_store") as mock_store,
        mock.patch("MLForge.server.handlers._get_request_message", return_value=request_message),
        mock.patch("MLForge.server.handlers._get_request_username", return_value="alice"),
    ):
...
def test_create_review_queue_no_owner_on_noauth():
    ...
    with (
        ...
        mock.patch("MLForge.server.handlers._get_request_username", return_value=None),
    ):
        mock_store.return_value.create_review_queue.return_value = _review_queue_entity(
            created_by=None
        )
...
def test_set_review_queue_item_status_stamps_completed_by(
    ...
    with (
        mock.patch("MLForge.server.handlers._get_tracking_store") as mock_store,
        mock.patch("MLForge.server.handlers._get_request_message", return_value=request_message),
        mock.patch("MLForge.server.handlers._get_request_username", return_value=username),
    ):

...
from MLForge.server.prometheus_exporter import activate_prometheus_exporter
...
def test_metrics(app, test_client):
    ...
    assert (
        metrics.registry.get_sample_value("MLForge_http_request_total", labels=success_labels)
        is None
    )
    resp = test_client.get("/")
    assert resp.status_code == 200
    assert (
        metrics.registry.get_sample_value("MLForge_http_request_total", labels=success_labels) == 1
    )
    ...
    assert (
        metrics.registry.get_sample_value("MLForge_http_request_total", labels=failure_labels)
        is None
    )
    resp = test_client.get("/non-existent-endpoint")
    assert resp.status_code == 404
    assert (
        metrics.registry.get_sample_value("MLForge_http_request_total", labels=failure_labels) == 1
    )

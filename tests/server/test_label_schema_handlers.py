...
from MLForge.protos.label_schemas_pb2 import (
    FEEDBACK,
...
from MLForge.store.tracking import SEARCH_MAX_RESULTS_THRESHOLD

_BASE_PATCH = "MLForge.server.handlers"
...
def test_list_label_schemas_max_results_bound_check(bad_max_results):
    ...
    # converts an AssertionError into MLForgeException(INVALID_PARAMETER_VALUE).
    ...
    with pytest.raises(MLForgeException, match="max_results") as exc_info:
        _validate_param_against_schema(
            schema=schema_fns,
            param="max_results",

...
from MLForge.server.fastapi_app import add_mcp_exception_handlers, create_fastapi_app
...
def test_mcp_exception_handler_delegates_for_non_mcp_routes():
    ...
    @app.exception_handler(MLForgeException)
    async def existing_MLForge_exception_handler(request, exc):
    ...
    @app.get("/non-mcp")
    async def non_mcp():
        raise MLForgeException.invalid_parameter_value("boom")

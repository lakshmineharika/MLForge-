# End-to-end tests for `POST /ajax-api/3.0/MLForge/genai/evaluate/invoke`.
...
from MLForge.utils.MLForge_tags import (
    MLForge_GENAI_EVALUATE_JOB_ID,
    MLForge_RUN_TYPE,
    MLForge_RUN_TYPE_GENAI_EVALUATE,
)
...
class Client:
    ...
    def invoke_genai_evaluate(
        ...
        response = requests.post(
            f"{self.server_url}/ajax-api/3.0/MLForge/genai/evaluate/invoke",
            json={
    ...
    def get_job(self, job_id: str) -> dict[str, Any]:
        response = requests.get(f"{self.server_url}/ajax-api/3.0/MLForge/jobs/{job_id}")
...
def client(tmp_path_factory: pytest.TempPathFactory, mock_gateway_server: str) -> Client:
    """Spin up an MLForge server with the new genai-evaluate job registered."""
    ...
    with subprocess.Popen(
        ...
        env={
            **os.environ,
            ...
            "_MLForge_SUPPORTED_JOB_FUNCTION_LIST": (
                "MLForge.genai.evaluation.job.invoke_genai_evaluate_job"
            ),
            "_MLForge_ALLOWED_JOB_NAME_LIST": "invoke_genai_evaluate",
...
def experiment_with_traces(client: Client):
    ...
    experiment_id = MLForge.create_experiment(f"genai_evaluate_test_{time.time()}")
...
def test_invoke_genai_evaluate_basic(client: Client, experiment_with_traces):
    ...
    assert run.data.tags[MLForge_RUN_TYPE] == MLForge_RUN_TYPE_GENAI_EVALUATE
    assert run.data.tags[MLForge_GENAI_EVALUATE_JOB_ID] == job_id
...
def test_invoke_genai_evaluate_handler_validation_no_traces(client: Client):
    ...
    """
    response = requests.post(
        f"{client.server_url}/ajax-api/3.0/MLForge/genai/evaluate/invoke",
        json={
            "experiment_id": "0",
            "trace_ids": [],
            "serialized_scorers": [_serialized_judge()],

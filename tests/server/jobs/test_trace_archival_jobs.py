...
from MLForge.entities import ExperimentTag, Workspace, trace_location
...
from MLForge.store.tracking.dbmodels.models import SqlSpan
...
from MLForge.tracing.constant import SpansLocation, TraceExperimentTagKey, TraceTagKey
from MLForge.tracing.otel.otel_archival import TRACE_ARCHIVAL_FILENAME
from MLForge.tracing.trace_archival_service import run_trace_archival_scheduler
...
def _create_tracking_store(tmp_path: Path, *, workspaces_enabled: bool):
    ...
    artifact_root.mkdir()
    store = store_cls(f"sqlite:///{tmp_path / 'MLForge.db'}", artifact_root.as_uri())
    try:
        yield store
    finally:
...
def test_trace_archival_scheduler_runs_per_workspace(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv(MLForge_WORKSPACE.name, raising=False)
    ...
    with (
        patch("MLForge.server.handlers._get_tracking_store", return_value=mock_tracking_store),
        patch(
            "MLForge.server.workspace_helpers._get_workspace_store",
    ...
    assert MLForge_WORKSPACE.get_raw() is None
...
def test_trace_archival_scheduler_skips_unsupported_workspace_and_continues(monkeypatch, tmp_path):
    ...
    def resolve_trace_archival_config(tracking_store, **kwargs):
        ...
        if workspace == "team-a":
            raise MLForgeException.invalid_parameter_value("unsupported archival destination")
...
def test_trace_archival_scheduler_returns_zero_when_disabled(monkeypatch, tmp_path):
    ...
    with patch("MLForge.server.handlers._get_tracking_store") as mock_get_tracking_store:
        archived = run_trace_archival_scheduler()
...
def test_trace_archival_scheduler_passes_max_traces_per_pass(monkeypatch, tmp_path):
    ...
    with patch("MLForge.server.handlers._get_tracking_store", return_value=mock_tracking_store):
        mock_tracking_store.archive_traces.return_value = 3
        archived = run_trace_archival_scheduler()
...
def test_trace_archival_scheduler_archives_real_store_traces(monkeypatch, tmp_path):
    ...
    with _create_tracking_store(tmp_path, workspaces_enabled=False) as store:
        ...
        with (
            patch("MLForge.server.handlers._get_tracking_store", return_value=store),
...
def test_periodic_tasks_consumer_enables_flush_locks():
    with (
        patch("MLForge.server.jobs.utils.os.getpid", return_value=987),
        patch("MLForge.server.jobs.utils._exec_cmd") as mock_exec_cmd,
    ):
        _start_periodic_tasks_consumer_proc()
    ...
    assert "-f" in cmd
    assert mock_exec_cmd.call_args.kwargs["extra_env"][MLForge_ORIGINAL_PARENT_PID_ENV_VAR] == "987"

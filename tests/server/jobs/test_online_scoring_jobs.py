...
from MLForge.genai.scorers.builtin_scorers import Completeness, RelevanceToQuery
from MLForge.genai.scorers.job import (
    run_online_scoring_scheduler,
    run_online_session_scorer_job,
    run_online_trace_scorer_job,
)
from MLForge.genai.scorers.online.entities import OnlineScorer, OnlineScoringConfig
from MLForge.server.jobs import get_job, submit_job
...
def test_run_online_trace_scorer_job_calls_processor():
    ...
    with (
        patch("MLForge.genai.scorers.job._get_tracking_store", return_value=mock_tracking_store),
        patch(
            "MLForge.genai.scorers.online.trace_processor.OnlineTraceScoringProcessor.create",
...
def test_run_online_trace_scorer_job_runs_exclusively_per_experiment(monkeypatch, tmp_path: Path):
    ...
    with _setup_job_runner(
        monkeypatch,
        tmp_path,
        supported_job_functions=["MLForge.genai.scorers.job.run_online_trace_scorer_job"],
...
def test_run_online_session_scorer_job_calls_processor():
    ...
    with (
        patch("MLForge.genai.scorers.job._get_tracking_store", return_value=mock_tracking_store),
        patch(
            "MLForge.genai.scorers.online.session_processor.OnlineSessionScoringProcessor.create",
            return_value=mock_processor,
        ) as mock_create,
    ):
        online_scorers = [make_online_scorer_dict(Completeness())]
...
def test_scheduler_submits_jobs_via_submit_job():
    ...
    with (
        patch("MLForge.genai.scorers.job._get_tracking_store", return_value=mock_tracking_store),
        patch("MLForge.genai.scorers.job.submit_job") as mock_submit_job,
    ):
        run_online_scoring_scheduler()
...
def test_scheduler_skips_invalid_scorers():
    ...
    with (
        patch("MLForge.genai.scorers.job._get_tracking_store", return_value=mock_store),
        patch("MLForge.genai.scorers.job.submit_job") as mock_submit,
        patch("MLForge.genai.scorers.job._logger") as mock_logger,
    ):
        run_online_scoring_scheduler()

...
from MLForge.server.jobs.utils import _load_function, _validate_function_parameters
...
def test_validate_function_parameters():
    ...
    with pytest.raises(MLForgeException, match=r"Missing required parameters.*\['b'\]"):
        _validate_function_parameters(test_func, {"a": 1})

    # Test with multiple missing required parameters
    with pytest.raises(MLForgeException, match=r"Missing required parameters.*\['a', 'b'\]"):
...
def test_validate_function_parameters_with_varargs():
    ...
    with pytest.raises(MLForgeException, match=r"Missing required parameters.*\['a'\]"):
...
def test_job_status_conversion():
    ...
    with pytest.raises(
        MLForgeException, match="The value -1 can't be converted to JobStatus enum value."
    ):
        JobStatus.from_int(-1)

    with pytest.raises(
        MLForgeException, match="The value 6 can't be converted to JobStatus enum value."
    ):
        JobStatus.from_int(6)

    with pytest.raises(
        MLForgeException, match="The string 'ABC' can't be converted to JobStatus enum value."
    ):
        JobStatus.from_str("ABC")


def test_load_function_invalid_function_format():
    with pytest.raises(MLForgeException, match="Invalid function fullname format"):
        _load_function("invalid_format_no_module")


def test_load_function_module_not_found():
    with pytest.raises(MLForgeException, match="Module not found"):
        _load_function("non_existent_module.some_function")


def test_load_function_function_not_found():
    with pytest.raises(MLForgeException, match="Function not found in module"):
        _load_function("os.non_exist_function")


def test_compute_exclusive_lock_key():
    from MLForge.server.jobs.utils import _compute_exclusive_lock_key

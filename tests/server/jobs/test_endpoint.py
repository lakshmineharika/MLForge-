...
import MLForge
from MLForge.server.jobs import job
...
def job_assert_tracking_uri(server_url: str) -> None:
    assert MLForge.get_tracking_uri() == server_url


@job(name="job_with_progress_tracking", max_workers=1)
def job_with_progress_tracking(sleep_secs: int = 1) -> str:
    from MLForge.server.jobs.progress import update_status_details
    ...
    with subprocess.Popen(
        ...
        env={
            **os.environ,
            ...
            "_MLForge_ALLOWED_JOB_NAME_LIST": (

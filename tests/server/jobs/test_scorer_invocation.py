...
from MLForge.tracing.assessment import log_expectation

pytestmark = pytest.mark.skipif(
    os.name == "nt", reason="MLForge job execution is not supported on Windows"
)


class MockGatewayHandler(BaseHTTPRequestHandler):
    """Mock handler for MLForge gateway chat completions endpoint.
...
class Client:
    """HTTP client for interacting with MLForge server endpoints."""
    ...
    def invoke_scorer(
        ...
        response = requests.post(
            f"{self.server_url}/ajax-api/3.0/MLForge/scorer/invoke",
            json=payload,
        )
        if not response.ok:
...
def client(tmp_path_factory: pytest.TempPathFactory, mock_gateway_server: str) -> Client:
    """Start an MLForge server with job execution enabled for scorer invocation."""
    ...
    with subprocess.Popen(
        ...
        env={
            **os.environ,
            "MLForge_SERVER_ENABLE_JOB_EXECUTION": "true",
            # Register the scorer invoke job function
            "_MLForge_SUPPORTED_JOB_FUNCTION_LIST": ("MLForge.genai.scorers.job.invoke_scorer_job"),
            "_MLForge_ALLOWED_JOB_NAME_LIST": "invoke_scorer",
            # Point gateway calls to our mock server
            "MLForge_GATEWAY_URI": mock_gateway_server,
            # Set batch size to 2 for testing job batching behavior
            "MLForge_SERVER_SCORER_INVOKE_BATCH_SIZE": "2",
        },
...
def experiment_with_traces(client: Client):
    ...
    MLForge.set_tracking_uri(client.server_url)

    experiment_name = f"test_scorer_job_{time.time()}"
    experiment_id = MLForge.create_experiment(experiment_name)
    MLForge.set_experiment(experiment_id=experiment_id)
...
def experiment_with_agentic_trace(client: Client):
    ...
    with MLForge.start_span(name="agent_main") as parent_span:
        parent_span.set_inputs({"query": "What is the weather?"})

        with MLForge.start_span(name="tool_call_1") as tool_span1:
            tool_span1.set_inputs({"tool": "get_weather"})
            tool_span1.set_outputs({"temperature": 72})

        with MLForge.start_span(name="tool_call_2") as tool_span2:
...
def experiment_with_conversation_traces(client: Client):
    ...
    for i, (user_msg, assistant_msg) in enumerate([
        ...
        with MLForge.start_span(name=f"session1_turn_{i}") as span:
            MLForge.update_current_trace(metadata={"MLForge.trace.session": session_1_id})
    ...
    for i, (user_msg, assistant_msg) in enumerate([
        ...
        with MLForge.start_span(name=f"session2_turn_{i}") as span:
            MLForge.update_current_trace(metadata={"MLForge.trace.session": session_2_id})
            span.set_inputs({"messages": [{"role": "user", "content": user_msg}]})
            span.set_outputs({"choices": [{"message": {"content": assistant_msg}}]})

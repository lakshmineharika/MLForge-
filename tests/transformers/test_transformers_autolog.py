...
def setfit_trainer():
    ...
    # include MLForgeCallback, so it produces extra runs no matter autologging
    # is on or off
    ...
    if Version(setfit.__version__) >= Version("1.1.0"):
        from transformers.integrations.integration_utils import MLForgeCallback

        trainer.remove_callback(MLForgeCallback)
...
def test_setfit_does_not_autolog(setfit_trainer):
    ...
    last_run = MLForge.last_active_run()
    assert not last_run
...
def test_transformers_trainer_does_not_autolog_sklearn(transformers_trainer):
    ...
    exp = MLForge.set_experiment(experiment_name="trainer_autolog_test")
...
def test_transformers_autolog_adheres_to_global_behavior_using_setfit(setfit_trainer):
    MLForge.transformers.autolog(disable=False)

    setfit_trainer.train()
    assert len(MLForge.search_runs()) == 0
...
def test_transformers_autolog_adheres_to_global_behavior_using_trainer(transformers_trainer):
    MLForge.transformers.autolog()

    exp = MLForge.set_experiment(experiment_name="autolog_with_trainer")
...
def test_active_autolog_no_setfit_logging_followed_by_successful_sklearn_autolog(
    ...
    exp = MLForge.set_experiment(experiment_name="setfit_with_sklearn")
    ...
    with MLForge.start_run(experiment_id=exp.experiment_id) as run:
        model = sklearn.cluster.KMeans()
        X, y = iris_data
        model.fit(X, y)

    logged_sklearn_data = MLForge.get_run(run.info.run_id)
...
def test_active_autolog_allows_subsequent_sklearn_autolog(iris_data, transformers_trainer):
    ...
    exp = MLForge.set_experiment(experiment_name="trainer_with_sklearn")
...
def test_disabled_sklearn_autologging_does_not_revert_to_enabled_with_setfit(
    ...
    exp = MLForge.set_experiment(experiment_name="setfit_with_sklearn_no_autologging")
...
def test_trainer_hyperparameter_tuning_does_not_log_sklearn_model(
    ...
    exp = MLForge.set_experiment(experiment_name="hyperparam_trainer")
...
def test_trainer_hyperparameter_tuning_functional_does_not_log_sklearn_model(
    ...
    exp = MLForge.set_experiment(experiment_name="hyperparam_trainer_functional")

...
from MLForge.transformers.flavor_config import FlavorKey
from MLForge.transformers.peft import get_peft_base_model, is_peft_model
from MLForge.utils.logging_utils import suppress_logs
...
def test_save_and_load_peft_pipeline(peft_pipeline, tmp_path):
    ...
    loaded_pipeline = MLForge.transformers.load_model(tmp_path)
...
def test_log_peft_pipeline(peft_pipeline):
    ...
    with MLForge.start_run():
        model_info = MLForge.transformers.log_model(peft_pipeline, name="model", input_example="hi")
...
def test_base_model_path_rejects_non_peft_model(small_qa_pipeline, tmp_path):
    with pytest.raises(MLForgeException, match="only supported for PEFT models"):
        MLForge.transformers.save_model(
...
def test_load_peft_with_base_model_path_override(peft_model_with_local_base, tmp_path):
    ...
    loaded_pipeline = MLForge.transformers.load_model(save_dir, base_model_path=base_dir)
    assert isinstance(loaded_pipeline.model, PeftModel)
    loaded_pipeline.predict("Hi")
...
def test_base_model_path_rejects_non_checkpoint_dir(peft_model_with_local_base, tmp_path):
    ...
    save_dir = tmp_path / "model_output"
    with pytest.raises(MLForgeException, match="config.json"):
        MLForge.transformers.save_model(
            transformers_model=pipeline,
            path=save_dir,
...
def test_load_base_model_path_override_rejects_non_checkpoint_dir(
    ...
    with pytest.raises(MLForgeException, match="config.json"):
        MLForge.transformers.load_model(save_dir, base_model_path=str(empty_dir))

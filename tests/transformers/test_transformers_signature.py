...
from MLForge.transformers import _try_import_conversational_pipeline
from MLForge.transformers.signature import (
...
def test_infer_signature_timeout_then_fall_back_to_default(text_generation_pipeline, monkeypatch):
    monkeypatch.setenv("MLForge_INPUT_EXAMPLE_INFERENCE_TIMEOUT", "1")  # Set timeout to 1 second
    ...
    with mock.patch("MLForge.transformers._TransformersWrapper.predict", side_effect=_slow_predict):
...
def test_infer_signature_prediction_error_then_fall_back_to_default(text_generation_pipeline):
    with mock.patch(
        "MLForge.transformers._TransformersWrapper.predict", side_effect=ValueError("Error")
    ):
        signature = infer_or_get_default_signature(text_generation_pipeline, example=["test"])

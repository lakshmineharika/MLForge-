...
import MLForge
import MLForge.pyfunc.scoring_server as pyfunc_scoring_server
from MLForge import pyfunc
from MLForge.deployments import PredictionsResponse
from MLForge.exceptions import MLForgeException
from MLForge.models import Model, ModelSignature, infer_signature
from MLForge.models.model import METADATA_FILES
from MLForge.store.artifact.s3_artifact_repo import S3ArtifactRepository
from MLForge.tracking.artifact_utils import _download_artifact_from_uri
from MLForge.transformers import (
...
from MLForge.types.schema import Array, ColSpec, DataType, ParamSchema, ParamSpec, Schema
from MLForge.utils.environment import _MLForge_conda_env
...
image_url = "https://raw.githubusercontent.com/MLForge/MLForge/master/tests/datasets/cat.png"
...
def transformers_custom_env(tmp_path):
    ...
    _MLForge_conda_env(conda_env, additional_pip_deps=["transformers"])
    return conda_env


@pytest.fixture
def mock_pyfunc_wrapper():
    return MLForge.transformers._TransformersWrapper("mock")
...
def test_default_requirements(pipeline, expected_requirements, request):
    ...
    assert _strip_requirements(conda_requirements) == (expected_requirements | {"MLForge"})


def test_inference_task_validation(small_qa_pipeline):
    with pytest.raises(
        MLForgeException, match="The task provided is invalid. 'llm/v1/invalid' is not"
    ):
        _validate_llm_inference_task_type("llm/v1/invalid", "text-generation")
    with pytest.raises(
        MLForgeException, match="The task provided is invalid. 'llm/v1/completions' is not"
    ):
...
def test_saving_with_invalid_dict_as_model(model_path):
    with pytest.raises(
        MLForgeException, match="Invalid dictionary submitted for 'transformers_model'. The "
    ):
        MLForge.transformers.save_model(transformers_model={"invalid": "key"}, path=model_path)

    with pytest.raises(
        MLForgeException, match="The 'transformers_model' dictionary must have an entry"
    ):
        MLForge.transformers.save_model(
...
def test_vision_model_save_pipeline_with_defaults(small_vision_model, model_path):
    MLForge.transformers.save_model(transformers_model=small_vision_model, path=model_path)
...
def test_basic_save_model_and_load_text_pipeline(text_classification_pipeline, model_path):
    ...
    loaded = MLForge.transformers.load_model(model_path)
    result = loaded("MLForge is a really neat tool!")
...
def test_basic_save_model_with_torch_dtype(text2text_generation_pipeline, model_path, dtype):
    ...
    loaded = MLForge.transformers.load_model(model_path, torch_dtype=torch.float32)
...
def test_multi_modal_pipeline_save_and_load(small_multi_modal_pipeline, model_path, image_for_test):
    MLForge.transformers.save_model(transformers_model=small_multi_modal_pipeline, path=model_path)
    question = "How many wall clocks are in the picture?"
    # Load components
    components = MLForge.transformers.load_model(model_path, return_type="components")
    if IS_NEW_FEATURE_EXTRACTION_API:
    ...
    # Load pipeline
    pipeline = MLForge.transformers.load_model(model_path)
    ...
    with pytest.raises(MLForgeException, match="The specified return_type mode 'magic' is"):
        MLForge.transformers.load_model(model_path, return_type="magic")
...
def test_multi_modal_component_save_and_load(component_multi_modal, model_path, image_for_test):
    ...
    loaded_components = MLForge.transformers.load_model(
...
def test_pipeline_saved_model_with_processor_cannot_be_loaded_as_pipeline(
    ...
    with pytest.raises(
        MLForgeException, match="This model has been saved with a processor. Processor objects"
    ):
        MLForge.transformers.load_model(model_uri=model_path, return_type="pipeline")
...
def test_log_and_load_transformers_pipeline(small_qa_pipeline, tmp_path, should_start_run):
    try:
        ...
        _MLForge_conda_env(conda_env, additional_pip_deps=["transformers"])
        model_info = MLForge.transformers.log_model(
            small_qa_pipeline,
            name=artifact_path,
            conda_env=str(conda_env),
        )
        reloaded_model = MLForge.transformers.load_model(
...
def test_load_pipeline_from_remote_uri_succeeds(
    ...
    MLForge.transformers.save_model(transformers_model=text_classification_pipeline, path=model_path)
    artifact_root = f"s3://{mock_s3_bucket}"
    artifact_path = "model"
    artifact_repo = S3ArtifactRepository(artifact_root)
    artifact_repo.log_artifacts(model_path, artifact_path=artifact_path)
    model_uri = os.path.join(artifact_root, artifact_path)
    loaded = MLForge.transformers.load_model(model_uri=str(model_uri), return_type="pipeline")
...
def test_transformers_log_model_calls_register_model(small_qa_pipeline, tmp_path):
    artifact_path = "transformers"
    register_model_patch = mock.patch("MLForge.tracking._model_registry.fluent._register_model")
    with MLForge.start_run(), register_model_patch:
        conda_env = tmp_path.joinpath("conda_env.yaml")
        _MLForge_conda_env(conda_env, additional_pip_deps=["transformers", "torch", "torchvision"])
        model_info = MLForge.transformers.log_model(
            small_qa_pipeline,
            name=artifact_path,
            conda_env=str(conda_env),
        ...
        assert_register_model_called_with_local_model_path(
            register_model_mock=MLForge.tracking._model_registry.fluent._register_model,
...
def test_transformers_log_model_with_no_registered_model_name(small_vision_model, tmp_path):
    ...
    artifact_path = "transformers"
    registered_model_patch = mock.patch("MLForge.tracking._model_registry.fluent._register_model")
    with MLForge.start_run(), registered_model_patch:
        conda_env = tmp_path.joinpath("conda_env.yaml")
        _MLForge_conda_env(conda_env, additional_pip_deps=["tensorflow", "transformers"])
        MLForge.transformers.log_model(
            model,
            name=artifact_path,
            conda_env=str(conda_env),
        )
        MLForge.tracking._model_registry.fluent._register_model.assert_not_called()
...
def test_transformers_save_persists_requirements_in_MLForge_directory(
...
def test_transformers_log_with_pip_requirements(small_multi_modal_pipeline, tmp_path):
    expected_MLForge_version = _MLForge_major_version_string()
    ...
    with MLForge.start_run():
        ...
        _assert_pip_requirements(
            model_info.model_uri, [expected_MLForge_version, "coolpackage"], strict=True
        )
    with MLForge.start_run():
        model_info = MLForge.transformers.log_model(
        ...
        _assert_pip_requirements(
            ...
            [expected_MLForge_version, "coolpackage", "alsocool"],
            strict=True,
        )
    with MLForge.start_run():
        model_info = MLForge.transformers.log_model(
        ...
        _assert_pip_requirements(
            model_info.model_uri,
            [expected_MLForge_version, "constrainedcool", "-c constraints.txt"],
            ["coolpackage"],
            strict=True,
        )
...
def test_transformers_log_with_extra_pip_requirements(small_multi_modal_pipeline, tmp_path):
    expected_MLForge_version = _MLForge_major_version_string()
    default_requirements = MLForge.transformers.get_default_pip_requirements(
    ...
    with MLForge.start_run():
        ...
        _assert_pip_requirements(
            model_info.model_uri,
            [expected_MLForge_version, *default_requirements, "coolpackage"],
            strict=True,
        )
    with MLForge.start_run():
        model_info = MLForge.transformers.log_model(
        ...
        _assert_pip_requirements(
            model_info.model_uri,
            [expected_MLForge_version, *default_requirements, "coolpackage", "alsocool"],
            strict=True,
        )
    with MLForge.start_run():
        model_info = MLForge.transformers.log_model(
...
def test_transformers_log_with_duplicate_extra_pip_requirements(small_multi_modal_pipeline):
    with pytest.raises(
        MLForgeException, match="The specified requirements versions are incompatible"
    ):
        with MLForge.start_run():
            MLForge.transformers.log_model(
                small_multi_modal_pipeline,
...
def test_transformers_pt_model_save_without_conda_env_uses_default_env_with_expected_dependencies(
    ...
    MLForge.transformers.save_model(small_qa_pipeline, model_path)
    _assert_pip_requirements(
        model_path, MLForge.transformers.get_default_pip_requirements(small_qa_pipeline.model)
    )
...
def test_transformers_pt_model_save_dependencies_without_accelerate(
    ...
    MLForge.transformers.save_model(text_generation_pipeline, model_path)
    _assert_pip_requirements(
        model_path, MLForge.transformers.get_default_pip_requirements(text_generation_pipeline.model)
    )
    pip_requirements = _get_deps_from_requirement_file(model_path)
    assert "tensorflow" not in pip_requirements
...
def test_transformers_pt_model_log_without_conda_env_uses_default_env_with_expected_dependencies(
    ...
    artifact_path = "model"
    with MLForge.start_run():
        model_info = MLForge.transformers.log_model(small_qa_pipeline, name=artifact_path)
    _assert_pip_requirements(
        model_info.model_uri,
        MLForge.transformers.get_default_pip_requirements(small_qa_pipeline.model),
    )
...
def test_log_model_with_code_paths(small_qa_pipeline):
    ...
    with (
        MLForge.start_run(),
        mock.patch("MLForge.transformers._add_code_from_conf_to_system_path") as add_mock,
    ):
        model_info = MLForge.transformers.log_model(
            small_qa_pipeline, name=artifact_path, code_paths=[__file__]
        )
        model_uri = model_info.model_uri
        _compare_logged_code_paths(__file__, model_uri, MLForge.transformers.FLAVOR_NAME)
        MLForge.transformers.load_model(model_uri)
        add_mock.assert_called()


def test_non_existent_model_card_entry(small_qa_pipeline, model_path):
    with mock.patch("MLForge.transformers._fetch_model_card", return_value=None):
        MLForge.transformers.save_model(transformers_model=small_qa_pipeline, path=model_path)
...
def test_huggingface_hub_not_installed(small_qa_pipeline, model_path):
    with mock.patch.dict("sys.modules", {"huggingface_hub": None}):
        result = MLForge.transformers._fetch_model_card(small_qa_pipeline.model.name_or_path)
...
def test_save_pipeline_without_defined_components(small_conversational_model, model_path):
    ...
    pipe = MLForge.transformers.load_model(model_path)
...
def test_invalid_model_type_without_registered_name_does_not_save(model_path):
    ...
    with pytest.raises(MLForgeException, match="The submitted model type"):
        MLForge.transformers.save_model(transformers_model=invalid_pipeline, path=model_path)


def test_invalid_input_to_pyfunc_signature_output_wrapper_raises(component_multi_modal):
    with pytest.raises(MLForgeException, match="The pipeline type submitted is not a valid"):
        MLForge.transformers.generate_signature_output(component_multi_modal["model"], "bogus")
...
def test_qa_pipeline_pyfunc_load_and_infer(small_qa_pipeline, model_path, inference_payload):
    signature = infer_signature(
        inference_payload,
        MLForge.transformers.generate_signature_output(small_qa_pipeline, inference_payload),
    )
    ...
    pyfunc_loaded = MLForge.pyfunc.load_model(model_path)
...
def test_vision_pipeline_pyfunc_load_and_infer(small_vision_model, model_path, inference_payload):
    ...
    signature = infer_signature(
        inference_payload,
        MLForge.transformers.generate_signature_output(small_vision_model, inference_payload),
    )
    MLForge.transformers.save_model(
    ...
    transformers_loaded_model = MLForge.transformers.load_model(model_path)
...
def test_text2text_generation_pipeline_with_model_configs(
    ...
    signature = infer_signature(
        data, MLForge.transformers.generate_signature_output(text2text_generation_pipeline, data)
    )
    ...
    pyfunc_loaded = MLForge.pyfunc.load_model(model_path1)
    ...
    signature_with_params = infer_signature(
        data,
        MLForge.transformers.generate_signature_output(text2text_generation_pipeline, data),
        model_config,
    )
    MLForge.transformers.save_model(
        text2text_generation_pipeline,
    ...
    pyfunc_loaded = MLForge.pyfunc.load_model(model_path2)
...
def test_text2text_generation_pipeline_with_model_config_and_params(
    ...
    generated_output = MLForge.transformers.generate_signature_output(
...
def test_text2text_generation_pipeline_with_params_with_errors(
    ...
    with pytest.raises(MLForgeException, match=r"Invalid parameters found"):
...
def test_text2text_generation_pipeline_with_inferred_schema(text2text_generation_pipeline):
    with MLForge.start_run():
        model_info = MLForge.transformers.log_model(text2text_generation_pipeline, name="my_model")
    pyfunc_loaded = MLForge.pyfunc.load_model(model_info.model_uri)
...
def test_invalid_input_to_text2text_pipeline(text2text_generation_pipeline, invalid_data):
    ...
    # dictionaries.
    with pytest.raises(
        MLForgeException, match=r"An invalid type has been supplied: .+\. Please supply"
    ):
        infer_signature(
            invalid_data,
            MLForge.transformers.generate_signature_output(
...
def test_text_generation_pipeline(text_generation_pipeline, model_path, data):
    signature = infer_signature(
        data, MLForge.transformers.generate_signature_output(text_generation_pipeline, data)
    )
...
def test_invalid_input_to_text_generation_pipeline(text_generation_pipeline, invalid_data):
    ...
    with pytest.raises(MLForgeException, match=match):
        infer_signature(
            invalid_data,
            MLForge.transformers.generate_signature_output(text_generation_pipeline, invalid_data),
        )
...
def test_fill_mask_pipeline(fill_mask_pipeline, model_path, inference_payload, result):
    signature = infer_signature(
        inference_payload,
        MLForge.transformers.generate_signature_output(fill_mask_pipeline, inference_payload),
    )

    MLForge.transformers.save_model(fill_mask_pipeline, path=model_path, signature=signature)
    pyfunc_loaded = MLForge.pyfunc.load_model(model_path)
...
def test_fill_mask_pipeline_with_multiple_masks(fill_mask_pipeline, model_path):
    ...
    MLForge.transformers.save_model(fill_mask_pipeline, path=model_path)
    pyfunc_loaded = MLForge.pyfunc.load_model(model_path)
...
def test_invalid_input_to_fill_mask_pipeline(fill_mask_pipeline, invalid_data):
    ...
    with pytest.raises(MLForgeException, match=match):
        infer_signature(
            invalid_data,
            MLForge.transformers.generate_signature_output(fill_mask_pipeline, invalid_data),
        )
...
def test_zero_shot_classification_pipeline(zero_shot_pipeline, model_path, data):
    ...
    signature = infer_signature(
        data, MLForge.transformers.generate_signature_output(zero_shot_pipeline, data)
    )

    MLForge.transformers.save_model(zero_shot_pipeline, model_path, signature=signature)

    loaded_pyfunc = MLForge.pyfunc.load_model(model_path)
...
def test_table_question_answering_pipeline(table_question_answering_pipeline, model_path, query):
    ...
    signature = infer_signature(
        data, MLForge.transformers.generate_signature_output(table_question_answering_pipeline, data)
    )

    MLForge.transformers.save_model(
        table_question_answering_pipeline, model_path, signature=signature
    )
    loaded = MLForge.pyfunc.load_model(model_path)
...
def test_custom_code_pipeline(custom_code_pipeline, model_path):
    ...
    signature = infer_signature(
        data, MLForge.transformers.generate_signature_output(custom_code_pipeline, data)
    )
    ...
    transformers_loaded = MLForge.transformers.load_model(model_path)
...
def test_custom_components_pipeline(custom_components_pipeline, model_path):
    ...
    signature = infer_signature(
        data, MLForge.transformers.generate_signature_output(custom_components_pipeline, data)
    )
...
def test_translation_pipeline(translation_pipeline, model_path, data, result):
    signature = infer_signature(
        data, MLForge.transformers.generate_signature_output(translation_pipeline, data)
    )

    MLForge.transformers.save_model(translation_pipeline, path=model_path, signature=signature)
    pyfunc_loaded = MLForge.pyfunc.load_model(model_path)
    inference = pyfunc_loaded.predict(data)
    assert inference == result
...
def test_summarization_pipeline(summarizer_pipeline, model_path, data):
    ...
    signature = infer_signature(
        data, MLForge.transformers.generate_signature_output(summarizer_pipeline, data)
    )
...
def test_classifier_pipeline(text_classification_pipeline, model_path, data):
    signature = infer_signature(
        data, MLForge.transformers.generate_signature_output(text_classification_pipeline, data)
    )
    MLForge.transformers.save_model(
...
def test_ner_pipeline(pipeline_name, model_path, data, result, request):
    ...
    signature = infer_signature(data, MLForge.transformers.generate_signature_output(pipeline, data))

    MLForge.transformers.save_model(pipeline, model_path, signature=signature)
    loaded_pyfunc = MLForge.pyfunc.load_model(model_path)
    inference = loaded_pyfunc.predict(data)
...
def test_conversational_pipeline(conversational_pipeline, model_path):
    assert MLForge.transformers._is_conversational_pipeline(conversational_pipeline)

    signature = infer_signature(
        "Hi there!",
        MLForge.transformers.generate_signature_output(conversational_pipeline, "Hi there!"),
    )

    MLForge.transformers.save_model(conversational_pipeline, model_path, signature=signature)
    loaded_pyfunc = MLForge.pyfunc.load_model(model_path)
    ...
    loaded_again_pyfunc = MLForge.pyfunc.load_model(model_path)
...
def test_vision_pipeline_pyfunc_predict(small_vision_model, inference_payload):
    ...
    transformers_loaded_model = MLForge.transformers.load_model(model_info.model_uri)
    expected_predictions = transformers_loaded_model.predict(inference_payload)
...
def test_feature_extraction_pipeline(feature_extraction_pipeline):
    ...
    signature = infer_signature(
        sentences,
        MLForge.transformers.generate_signature_output(feature_extraction_pipeline, sentences),
    )
    ...
    # Load as native
    loaded_pipeline = MLForge.transformers.load_model(model_info.model_uri)
    ...
    loaded_pyfunc = MLForge.pyfunc.load_model(model_info.model_uri)
...
def test_loading_unsupported_pipeline_type_as_pyfunc(small_multi_modal_pipeline, model_path):
    MLForge.transformers.save_model(small_multi_modal_pipeline, model_path)
    with pytest.raises(MLForgeException, match='Model does not have the "python_function" flavor'):
        MLForge.pyfunc.load_model(model_path)
...
def test_pyfunc_json_encoded_list_parsing(mock_pyfunc_wrapper):
    ...
    with pytest.raises(MLForgeException, match="Invalid key in inference payload. The "):
...
def test_pyfunc_text_to_text_input(mock_pyfunc_wrapper):
    ...
    with pytest.raises(MLForgeException, match="An invalid type has been supplied"):
...
def test_pyfunc_qa_input(mock_pyfunc_wrapper):
    ...
    with pytest.raises(MLForgeException, match="Invalid keys were submitted. Keys must"):
...
def test_invalid_instruction_pipeline_parsing(mock_pyfunc_wrapper, flavor_config):
    ...
    with pytest.raises(MLForgeException, match="Unable to parse the pipeline output. Expected"):
        mock_pyfunc_wrapper._strip_input_from_response_in_instruction_pipelines(
...
def test_instructional_pipeline_no_prompt_in_output(model_path):
    ...
    inference = pyfunc_loaded.predict("What is MLForge?")

    assert not inference[0].startswith("What is MLForge?")
    assert "\n" in inference[0]
...
def test_instructional_pipeline_with_prompt_in_output(model_path):
    ...
    assert inference[0].startswith("What is MLForge?")
    assert "\n\n" in inference[0]
...
def test_whisper_model_predict(model_path, whisper_pipeline, input_format, with_input_example):
    ...
    loaded_pipeline = MLForge.transformers.load_model(model_path)
...
def test_whisper_model_pyfunc_with_malformed_input(whisper_pipeline, model_path):
    ...
    pyfunc_model = MLForge.pyfunc.load_model(model_path)

    invalid_audio = b"This isn't a real audio file"
    with pytest.raises(MLForgeException, match="Failed to process the input audio data. Either"):
        pyfunc_model.predict([invalid_audio])

    bad_uri_msg = "An invalid string input was provided. String"

    with pytest.raises(MLForgeException, match=bad_uri_msg):
...
def test_vision_pipeline_pyfunc_predict_with_kwargs(small_vision_model):
    ...
    with MLForge.start_run():
        model_info = MLForge.transformers.log_model(
            ...
            signature=infer_signature(
                image_url,
                MLForge.transformers.generate_signature_output(small_vision_model, image_url),
                params=parameters,
            ),
        )
        model_uri = model_info.model_uri
    transformers_loaded_model = MLForge.transformers.load_model(model_uri)
...
def test_qa_pipeline_pyfunc_predict_with_kwargs(small_qa_pipeline):
    ...
    output = MLForge.transformers.generate_signature_output(small_qa_pipeline, data)
    signature_with_params = infer_signature(
        data,
        output,
        parameters,
...
def test_uri_directory_renaming_handling_pipeline(model_path, text_classification_pipeline):
    ...
    # remove the 'model_binary' entries to emulate older versions of MLForge
    ...
    loaded_model = MLForge.pyfunc.load_model(model_path)
...
def test_uri_directory_renaming_handling_components(model_path, text_classification_pipeline):
    ...
    with MLForge.start_run():
        MLForge.transformers.save_model(transformers_model=components, path=model_path)
...
def test_pyfunc_model_log_load_with_artifacts_snapshot():
    ...
    class QAModel(MLForge.pyfunc.PythonModel):
    ...
    with MLForge.start_run():
        model_info = MLForge.pyfunc.log_model(
            ...
            input_example=data,
            signature=infer_signature(
                data, MLForge.transformers.generate_signature_output(bert_tiny_pipeline, data)
            ),
    ...
    loaded_pyfunc_model = MLForge.pyfunc.load_model(model_uri=model_info.model_uri)
...
def test_basic_model_with_accelerate_device_mapping_fails_save(tmp_path, model_path):
    ...
    with pytest.raises(
        ...
        MLForge.transformers.save_model(transformers_model=pipeline, path=model_path)
...
def test_text_generation_save_model_with_inference_task(
    ...
    monkeypatch.setenv("MLForge_REQUIREMENTS_INFERENCE_RAISE_ERRORS", "true")
...
def test_text_generation_save_model_with_invalid_inference_task(
    ...
    with pytest.raises(
        MLForgeException, match=r"The task provided is invalid.*Must be.*llm/v1/completions"
    ):
        MLForge.transformers.save_model(
            transformers_model=text_generation_pipeline,
            path=model_path,
...
def test_text_generation_log_model_with_mismatched_task(text_generation_pipeline):
    with pytest.raises(
        MLForgeException, match=r"LLM v1 task type 'llm/v1/chat' is specified in metadata, but"
    ):
        with MLForge.start_run():
            MLForge.transformers.log_model(
                text_generation_pipeline,
...
def test_get_task_for_model():
    with mock.patch("transformers.pipelines.get_task") as mock_get_task:
        ...
        with pytest.raises(MLForgeException, match="Cannot construct transformers pipeline"):
        ...
        with pytest.raises(MLForgeException, match="The task could not be inferred"):
            _get_task_for_model("model")
...
def test_local_custom_model_save_and_load(text_generation_pipeline, model_path, tmp_path):
    ...
    # 1. Save local custom model without specifying task -> raises MLForgeException
    with pytest.raises(MLForgeException, match=r"The task could not be inferred"):
        MLForge.transformers.save_model(transformers_model=model_dict, path=model_path)
...
@pytest.mark.parametrize(
    ...
    [
        ("text2text_generation_pipeline", "What is MLForge?", {"tokenizer"}),
        ("text_generation_pipeline", "What is MLForge?", {"tokenizer"}),
        (
            "small_vision_model",
            image_url,
        ...
        (
            ...
            {"text": "What is MLForge?", "image": image_url},
        ...
        ("feature_extraction_pipeline", "What is MLForge?", {"tokenizer"}),
    ],
)
def test_save_and_load_pipeline_without_save_pretrained_false(
    ...
    if "python_function" in mlmodel.flavors:
        if callable(input_example):
            input_example = input_example()
        MLForge.pyfunc.load_model(model_path).predict(input_example)
...
def test_persist_pretrained_model(small_qa_pipeline):
    with MLForge.start_run():
        model_info = MLForge.transformers.log_model(
            ...
            name="model",
            save_pretrained=False,
            pip_requirements=["MLForge"],  # For speed up logging
        )

    artifact_path = Path(MLForge.artifacts.download_artifacts(model_info.model_uri))
    ...
    with mock.patch(
        "MLForge.transformers.TempDir", side_effect=MLForge.utils.file_utils.TempDir
    ) as mock_tmpdir:
        MLForge.transformers.persist_pretrained_model(model_info.model_uri)
...
def test_small_qa_pipeline_copy_metadata_in_databricks(
    ...
    artifact_path = MLForge.artifacts.download_artifacts(
...
def test_device_param_on_load_model(device, small_qa_pipeline, model_path, monkeypatch):
    MLForge.transformers.save_model(small_qa_pipeline, path=model_path)
    conf = MLForge.transformers.load_model(model_path, return_type="components", device=device)
    assert conf.get("device") == device

    monkeypatch.setenv("MLForge_HUGGINGFACE_USE_DEVICE_MAP", "true")
    if device is None:
        conf = MLForge.transformers.load_model(model_path, return_type="components", device=device)
        ...
        with pytest.raises(
            MLForgeException,
            match=r"The environment variable MLForge_HUGGINGFACE_USE_DEVICE_MAP is set to True, "
            rf"but the `device` argument is provided with value {device}.",
        ):
            MLForge.transformers.load_model(model_path, return_type="components", device=device)
...
def test_save_model_from_local_checkpoint(model_path, local_checkpoint_path):
    with mock.patch("MLForge.transformers._logger") as mock_logger:
        MLForge.transformers.save_model(
            transformers_model=local_checkpoint_path,
            task="text-generation",
            path=model_path,
            input_example=["What is MLForge?"],
        )
...
def test_save_model_from_local_checkpoint_with_custom_tokenizer(model_path, local_checkpoint_path):
    # When a custom tokenizer is also saved in the checkpoint, MLForge should save and load it.
...
def test_save_model_from_local_checkpoint_with_llm_inference_task(
    ...
    response = loaded_pyfunc.predict({
        "messages": [
            {"role": "system", "content": "Hello, how can I help you today?"},
            {"role": "user", "content": "What is MLForge?"},
        ],
    })
...
def test_save_model_from_local_checkpoint_invalid_arguments(model_path, local_checkpoint_path):
    with pytest.raises(MLForgeException, match=r"The `task` argument must be specified"):
        MLForge.transformers.save_model(
            transformers_model=local_checkpoint_path,
            path=model_path,
        )
    ...
    with pytest.raises(
        MLForgeException, match=r"The `save_pretrained` argument must be set to True"
    ):
        MLForge.transformers.save_model(
            transformers_model=local_checkpoint_path,
            path=model_path,
...
def test_log_model_skip_validating_serving_input_for_local_checkpoint(
    ...
    with mock.patch("MLForge.models.utils._validate_serving_input") as mock_validate_input:
        # Ensure MLForge skips serving input validation for local checkpoint
        with MLForge.start_run():
            model_info = MLForge.transformers.log_model(
                model,
                name="model",
    ...
    MLForge_model = Model.load(model_info.model_uri)
    local_path = _download_artifact_from_uri(model_info.model_uri, output_path=tmp_path)
    serving_input = MLForge_model.get_serving_input(local_path)

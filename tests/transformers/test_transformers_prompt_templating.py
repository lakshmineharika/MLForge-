...
from MLForge.transformers import _SUPPORTED_PROMPT_TEMPLATING_TASK_TYPES, _validate_prompt_template
...
def test_prompt_save_and_load(saved_transformers_model_path):
    ...
    model = MLForge.pyfunc.load_model(saved_transformers_model_path)
...
def test_prompt_formatting(saved_transformers_model_path):
    model_impl = MLForge.pyfunc.load_model(saved_transformers_model_path)._model_impl
...
def test_prompt_used_in_predict(task, pipeline_fixture, output_key, request, tmp_path):
    ...
    model = MLForge.pyfunc.load_model(model_path)
    prompt = "What is MLForge?"
    formatted_prompt = TEST_PROMPT_TEMPLATE.format(prompt=prompt)
    mock_response = "MLForge be a tool fer machine lernin'"

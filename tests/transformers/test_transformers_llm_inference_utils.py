...
from MLForge.transformers.llm_inference_utils import (
...
def test_infer_signature_from_llm_inference_task():
    ...
    with pytest.raises(MLForgeException, match=r".*llm/v1/completions.*signature"):
...
def test_apply_chat_template():
    ...
    with pytest.raises(MLForgeException, match=r"Input messages should be list of"):
...
def test_preprocess_llm_inference_input(case):
    ...
    with mock.patch(
        "MLForge.transformers.llm_inference_utils._get_stopping_criteria"
...
def test_preprocess_llm_inference_input_raise_if_key_invalid():
    # Missing input key
    with pytest.raises(MLForgeException, match=r"Transformer model saved with"):
        preprocess_llm_inference_input(

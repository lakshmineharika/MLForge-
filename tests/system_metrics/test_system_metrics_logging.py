...
import MLForge
from MLForge.system_metrics.system_metrics_monitor import SystemMetricsMonitor


@pytest.fixture(autouse=True)
def enable_debug_logging():
    # Enable debug logging to help diagnose flaky test failures
    logger = logging.getLogger("MLForge.system_metrics.system_metrics_monitor")
...
def disable_system_metrics_logging():
    ...
    MLForge.set_system_metrics_sampling_interval(None)
    MLForge.set_system_metrics_samples_before_logging(None)
    MLForge.set_system_metrics_node_id(None)
...
def test_manual_system_metrics_monitor():
    ...
    with MLForge.start_run(log_system_metrics=False) as run:
        ...
        wait_for_condition(
            lambda: len(MLForge.MLForgeClient().get_metric_history(run.info.run_id, metric_test)) > 1,
            timeout=20,
        )
    wait_for_condition(
    ...
    MLForge_run = MLForge.get_run(run.info.run_id)
    metrics = MLForge_run.data.metrics
    ...
    metrics_history = MLForge.MLForgeClient().get_metric_history(run.info.run_id, metric_test)
    assert metrics_history[-1].step > 0


def test_automatic_system_metrics_monitor():
    metric_test = "system/cpu_utilization_percentage"
    MLForge.enable_system_metrics_logging()
    MLForge.set_system_metrics_sampling_interval(0.2)
    MLForge.set_system_metrics_samples_before_logging(2)
...
def test_automatic_system_metrics_monitor_resume_existing_run():
    ...
    # Get the last step.
    metrics_history = MLForge.MLForgeClient().get_metric_history(
        run.info.run_id, "system/cpu_utilization_percentage"
    )
    last_step = metrics_history[-1].step

    with MLForge.start_run(run.info.run_id) as run:
        time.sleep(2)
    MLForge_run = MLForge.get_run(run.info.run_id)
    metrics = MLForge_run.data.metrics
...
def test_system_metrics_monitor_with_multi_node():
    ...
    for node_id in node_ids:
        MLForge.set_system_metrics_node_id(node_id)
        with MLForge.start_run(run_id=run_id, log_system_metrics=True):
            wait_for_condition(
                lambda: any(
                    k.startswith(f"system/{node_id}/")
                    for k in MLForge.get_run(run_id).data.metrics.keys()
                )
            )

    MLForge_run = MLForge.get_run(run_id)
    metrics = MLForge_run.data.metrics

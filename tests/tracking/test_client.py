...
import MLForge
from MLForge import MLForgeClient, flush_async_logging
from MLForge.config import enable_async_logging
...
from MLForge.entities.file_info import FileInfo
from MLForge.entities.logged_model_status import LoggedModelStatus
from MLForge.entities.metric import Metric
from MLForge.entities.model_registry import ModelVersion, ModelVersionTag
from MLForge.entities.model_registry.model_version_status import ModelVersionStatus
from MLForge.entities.model_registry.prompt_version import IS_PROMPT_TAG_KEY
from MLForge.entities.param import Param
from MLForge.entities.span import create_MLForge_span
from MLForge.entities.trace_data import TraceData
from MLForge.entities.trace_location import TraceLocation, TraceLocationType, UCSchemaLocation
from MLForge.entities.trace_state import TraceState
from MLForge.entities.trace_status import TraceStatus
from MLForge.environment_variables import MLForge_TRACKING_USERNAME
from MLForge.exceptions import (
    MLForgeException,
    MLForgeNotImplementedException,
    MLForgeTraceDataCorrupted,
    MLForgeTraceDataNotFound,
)
from MLForge.prompt.registry_utils import PromptCache
from MLForge.store.artifact.artifact_repo import ArtifactRepository
from MLForge.store.entities.paged_list import PagedList
from MLForge.store.model_registry.sqlalchemy_store import (
    SqlAlchemyStore as SqlAlchemyModelRegistryStore,
)
from MLForge.store.tracking import SEARCH_EVALUATION_DATASETS_MAX_RESULTS, SEARCH_MAX_RESULTS_DEFAULT
from MLForge.store.tracking.sqlalchemy_store import SqlAlchemyStore as SqlAlchemyTrackingStore
from MLForge.tracing.constant import SpansLocation, TraceMetadataKey, TraceTagKey
from MLForge.tracing.provider import _get_tracer, trace_disabled
from MLForge.tracing.trace_manager import InMemoryTraceManager
from MLForge.tracing.utils import TraceJSONEncoder
from MLForge.tracking import set_registry_uri
from MLForge.tracking._model_registry import DEFAULT_AWAIT_MAX_SLEEP_SECONDS
from MLForge.tracking._model_registry.utils import (
    _get_store_registry as _get_model_registry_store_registry,
)
from MLForge.tracking._tracking_service.utils import _register, _use_tracking_uri
from MLForge.tracking.default_experiment import DEFAULT_EXPERIMENT_ID
from MLForge.utils.databricks_utils import _construct_databricks_run_url
...
@pytest.fixture
def mock_store():
    with mock.patch("MLForge.tracking._tracking_service.utils._get_store") as mock_get_store:
        mock_store = mock_get_store.return_value
        with mock.patch("MLForge.tracing.client._get_store", return_value=mock_store):
            yield mock_store
...
def mock_artifact_repo():
    with mock.patch(
        "MLForge.tracking._tracking_service.client.get_artifact_repository"
    ) as mock_get_repo:
        mock_repo = mock_get_repo.return_value
        with mock.patch("MLForge.tracing.client.get_artifact_repository", return_value=mock_repo):
            yield mock_repo
...
def mock_registry_store():
    ...
    with mock.patch("MLForge.tracking._model_registry.utils._get_store", return_value=mock_store):
        yield mock_store
...
def mock_databricks_tracking_store():
    ...
    with mock.patch(
        "MLForge.tracking._tracking_service.utils._tracking_store_registry.get_store",
...
def mock_store_start_trace():
    def _mock_start_trace(trace_info):
        return create_test_trace_info(
            ...
            tags={
                "MLForge.user": "bob",
                "MLForge.artifactLocation": "test",
                **trace_info.tags,
            },
        )

    with mock.patch(
        "MLForge.tracing.client.TracingClient.start_trace", side_effect=_mock_start_trace
    ) as mock_start_trace:
...
def mock_spark_session():
    with mock.patch(
        "MLForge.utils.databricks_utils._get_active_spark_session"
...
def test_client_create_run(mock_store, mock_time):
    ...
    MLForgeClient().create_run(experiment_id)
...
def test_client_create_run_with_name(mock_store, mock_time):
    ...
    MLForgeClient().create_run(experiment_id, run_name="my name")
...
def test_client_get_trace(mock_store, mock_artifact_repo):
    ...
    mock_store.batch_get_traces.return_value = [
        Trace(
            TraceInfo(
                ...
                state=TraceState.OK,
                tags={"MLForge.artifactLocation": "dbfs:/path/to/artifacts"},
            ),
            TraceData(
                spans=[
                    Span.from_dict({
                        ...
                        "status_message": "",
                        "attributes": {
                            "MLForge.traceRequestId": f'"{trace_id}"',
                            "MLForge.spanType": '"LLM"',
                            "MLForge.spanFunctionName": '"predict"',
                            "MLForge.spanInputs": '{"prompt": "What is the meaning of life?"}',
                            "MLForge.spanOutputs": '{"answer": 42}',
                        },
                        "events": [],
                    })
                ]
    ...
    trace = MLForgeClient().get_trace(trace_id)
    ...
    assert trace.info.tags == {"MLForge.artifactLocation": "dbfs:/path/to/artifacts"}
...
def test_client_get_trace_empty_result(mock_store):
    ...
    with pytest.raises(MLForgeException, match="not found"):
        MLForgeClient().get_trace("trace:/catalog.schema/123")
...
def test_client_get_trace_from_artifact_repo(mock_store, mock_artifact_repo):
        ...
        "spans": [
            {
                ...
                "status_message": "",
                "attributes": {
                    "MLForge.traceRequestId": '"tr-1234567"',
                    "MLForge.spanType": '"LLM"',
                    "MLForge.spanFunctionName": '"predict"',
                    "MLForge.spanInputs": '{"prompt": "What is the meaning of life?"}',
                    "MLForge.spanOutputs": '{"answer": 42}',
    ...
    trace = MLForgeClient().get_trace("1234567")
...
def test_client_get_trace_from_archive_repo(mock_store, mock_artifact_repo):
    trace_info = TraceInfo(
        ...
        state=TraceState.OK,
        tags={
            "MLForge.artifactLocation": "dbfs:/path/to/artifacts",
...
def test_client_get_trace_throws_for_missing_or_corrupted_data(mock_store, mock_artifact_repo):
    ...
    mock_artifact_repo.download_trace_data.side_effect = MLForgeTraceDataNotFound("1234567")

    with pytest.raises(
        MLForgeException,
        match="Trace with ID 1234567 cannot be loaded because it is missing span data",
    ):
        MLForgeClient().get_trace("1234567")

    mock_artifact_repo.download_trace_data.side_effect = MLForgeTraceDataCorrupted("1234567")
    with pytest.raises(
        MLForgeException,
...
def test_client_search_traces_with_get_traces(
    ...
    results = MLForgeClient().search_traces(
...
def test_client_search_traces_with_large_results(mock_store, mock_artifact_repo):
    ...
    results = MLForgeClient().search_traces(locations=["catalog.schema"])
    mock_store.search_traces.assert_called_once_with(
        experiment_ids=None,
        filter_string=None,
        max_results=100,
...
def test_client_search_traces_mixed(mock_store, mock_artifact_repo, include_spans):
    mock_traces = [
        TraceInfo(
            ...
            execution_duration=456,
            state=TraceState.OK,
            tags={"MLForge.artifactLocation": "dbfs:/path/to/artifacts/1"},
        ),
        TraceInfo(
            trace_id="8910",
            ...
            state=TraceState.OK,
            tags={"MLForge.artifactLocation": "dbfs:/path/to/artifacts/2"},
        ),
    ]
...
def test_client_search_traces_with_artifact_repo(mock_store, mock_artifact_repo, include_spans):
    ...
    mock_store.search_traces.return_value = (mock_traces, None)
    mock_artifact_repo.download_trace_data.return_value = {}
    results = MLForgeClient().search_traces(locations=["1", "2", "3"], include_spans=include_spans)
...
def test_client_search_traces_trace_data_download_error(mock_store, include_spans):
    ...
    with mock.patch(
        "MLForge.tracing.client.get_artifact_repository",
        ...
        mock_traces = [
            TraceInfo(
                ...
                execution_duration=456,
                state=TraceState.OK,
                tags={"MLForge.artifactLocation": "test"},
            ),
        ]
        mock_store.search_traces.return_value = (mock_traces, None)
        traces = MLForgeClient().search_traces(locations=["1"], include_spans=include_spans)
...
def test_client_search_traces_validates_experiment_ids_type():
    with pytest.raises(MLForgeException, match=r"locations must be a list"):
        MLForgeClient().search_traces(locations=4)
...
def disable_prompt_cache():
    from MLForge.environment_variables import (
        MLForge_ALIAS_PROMPT_CACHE_TTL_SECONDS,
        MLForge_VERSION_PROMPT_CACHE_TTL_SECONDS,
    )

    MLForge_ALIAS_PROMPT_CACHE_TTL_SECONDS.set(0)
    MLForge_VERSION_PROMPT_CACHE_TTL_SECONDS.set(0)
    yield
    MLForge_ALIAS_PROMPT_CACHE_TTL_SECONDS.unset()
    MLForge_VERSION_PROMPT_CACHE_TTL_SECONDS.unset()
...
def tracking_uri(request, tmp_path, db_uri):
    """Set an MLForge Tracking URI with different type of backend."""
    if request.param == "file":
        pytest.skip("FileStore is no longer supported.")
    if "MLForge_SKINNY" in os.environ and request.param == "sqlalchemy":
        pytest.skip("SQLAlchemy store is not available in skinny.")

    original_tracking_uri = MLForge.get_tracking_uri()
    ...
    # NB: MLForge tracer does not handle the change of tracking URI well,
    # so we need to reset the tracer to switch the tracking URI during testing.
    MLForge.tracing.disable()
    MLForge.set_tracking_uri(tracking_uri)
    MLForge.tracing.enable()

    yield tracking_uri

    # Reset tracking URI
    MLForge.set_tracking_uri(original_tracking_uri)
...
def test_start_and_end_trace(tracking_uri, with_active_run, async_logging_enabled):
    client = MLForgeClient(tracking_uri)
    ...
    if async_logging_enabled:
        MLForge.flush_trace_async_logging(terminate=True)

    trace_id = MLForge.get_trace(MLForge.get_last_active_trace_id(), flush=True).info.trace_id
    ...
    assert root_span.attributes == {
        "MLForge.experimentId": experiment_id,
        "MLForge.traceRequestId": trace.info.trace_id,
        "MLForge.spanType": "UNKNOWN",
        "MLForge.spanLogLevel": SpanLogLevel.DEBUG,
        "MLForge.spanInputs": {"x": 1, "y": 2},
        "MLForge.spanOutputs": {"output": 25},
    }
    ...
    assert child_span_1.attributes == {
        "MLForge.traceRequestId": trace.info.trace_id,
        "MLForge.spanType": "LLM",
        "MLForge.spanLogLevel": SpanLogLevel.INFO,
        "MLForge.spanInputs": {"z": 3},
        "MLForge.spanOutputs": {"output": 5},
        "delta": 2,
    }
...
def test_start_and_end_trace_before_all_span_end(async_logging_enabled):
    ...
    exp_id = MLForge.set_experiment("test_experiment_1").experiment_id

    class TestModel:
        def __init__(self):
            self._client = MLForgeClient()
    ...
    traces = MLForgeClient().search_traces(locations=[exp_id])
    assert len(traces) == 1
...
def test_log_trace_with_databricks_tracking_uri(mock_store_start_trace, monkeypatch):
    monkeypatch.setenv("MLForge_EXPERIMENT_NAME", "test")
    monkeypatch.setenv(MLForge_TRACKING_USERNAME.name, "bob")
    ...
    class TestModel:
        ...
        def predict(self, x, y):
            root_span = self._client.start_trace(
                name="predict",
                inputs={"x": x, "y": y},
                # Trying to override MLForge.user tag, which will be ignored
                tags={"tag": "tag_value", "MLForge.user": "unknown"},
            )
            trace_id = root_span.trace_id
    ...
    with (
        mock.patch("MLForge.get_tracking_uri", return_value="databricks"),
        mock.patch("MLForge.tracking.context.default_context._get_source_name", return_value="test"),
        mock.patch(
            "MLForge.tracing.client.TracingClient._upload_trace_data"
        ) as mock_upload_trace_data,
        mock.patch(
            "MLForge.tracing.client.TracingClient.set_trace_tags",
        ),
        mock.patch(
            "MLForge.tracing.client.TracingClient.set_trace_tag",
        ),
        mock.patch(
            "MLForge.tracing.client.TracingClient.get_trace_info",
        ),
        mock.patch(
            "MLForge.MLForgeClient.get_experiment_by_name",
...
def test_start_and_end_trace_does_not_log_trace_when_disabled(
    ...
    monkeypatch.setattr(MLForge.tracking.client, "_logger", mock_logger)
...
def test_start_trace_raise_error_when_active_trace_exists():
    with MLForge.start_span("fluent_span"):
        with pytest.raises(MLForgeException, match=r"Another trace is already set in the global"):
            MLForge.tracking.MLForgeClient().start_trace("test")


def test_end_trace_raise_error_when_trace_not_exist():
    client = MLForge.tracking.MLForgeClient()
    ...
    with pytest.raises(MLForgeException, match=r"Trace with ID test not found"):
...
def test_end_trace_raise_error_for_trace_in_end_status(state):
    ...
    with pytest.raises(MLForgeException, match=r"Trace with ID test already finished"):
        client.end_trace("test")
...
def test_start_span_raise_error_when_parent_id_is_not_provided():
    with pytest.raises(MLForgeException, match=r"start_span\(\) must be called with"):
        MLForge.tracking.MLForgeClient().start_span("span_name", trace_id="test", parent_id=None)
...
def test_search_traces_experiment_ids_deprecation_warning():
    ...
    exp_id = MLForge.set_experiment("test_experiment_deprecation").experiment_id
...
def test_set_and_delete_trace_tag_on_active_trace(monkeypatch):
    monkeypatch.setenv(MLForge_TRACKING_USERNAME.name, "bob")
    monkeypatch.setattr(MLForge.tracking.context.default_context, "_get_source_name", lambda: "test")
...
def test_set_trace_tag_on_logged_trace(mock_store):
    MLForge.tracking.MLForgeClient().set_trace_tag("test", "foo", "bar")
    MLForge.tracking.MLForgeClient().set_trace_tag("test", "MLForge.some.reserved.tag", "value")
    mock_store.set_trace_tag.assert_has_calls([
        mock.call("test", "foo", "bar"),
        mock.call("test", "MLForge.some.reserved.tag", "value"),
    ])
...
def test_set_trace_tag_skips_immutable_internal_tags_on_active_trace(monkeypatch, key):
    ...
    with patch("MLForge.tracing.client._logger") as mock_logger:
...
def test_set_trace_tag_skips_immutable_internal_tags(mock_store, key):
    with patch("MLForge.tracing.client._logger") as mock_logger:
        MLForge.tracking.MLForgeClient().set_trace_tag("test", key, "s3://bucket/archive/test")
...
def test_delete_trace_tag_on_logged_trace(mock_store):
    MLForge.tracking.MLForgeClient().delete_trace_tag("test", "foo")
...
def test_delete_trace_tag_skips_immutable_internal_tags(mock_store, key):
    with patch("MLForge.tracing.client._logger") as mock_logger:
        MLForge.tracking.MLForgeClient().delete_trace_tag("test", key)
...
def test_delete_trace_tag_allows_clearing_archival_failure_on_logged_trace(mock_store):
    MLForge.tracking.MLForgeClient().delete_trace_tag("test", TraceTagKey.ARCHIVAL_FAILURE)
...
def test_client_create_experiment(mock_store):
    MLForgeClient().create_experiment("someName", "someLocation", {"key1": "val1", "key2": "val2"})
...
def test_client_create_run_overrides(mock_store):
    ...
    tags = {
        MLForge_USER: user,
        MLForge_PARENT_RUN_ID: mock.Mock(),
        MLForge_SOURCE_TYPE: SourceType.to_string(SourceType.JOB),
        MLForge_SOURCE_NAME: mock.Mock(),
        MLForge_PROJECT_ENTRY_POINT: mock.Mock(),
        MLForge_GIT_COMMIT: mock.Mock(),
        "other-key": "other-value",
    }

    MLForgeClient().create_run(experiment_id, start_time, tags, run_name)
    ...
    MLForgeClient().create_run(experiment_id, start_time, tags)
    mock_store.create_run.assert_called_once_with(
        experiment_id=experiment_id,
        user_id=user,
        start_time=start_time,
...
def test_client_set_terminated_no_change_name(mock_store):
    experiment_id = mock.Mock()
    run = MLForgeClient().create_run(experiment_id, run_name="my name")
    MLForgeClient().set_terminated(run.info.run_id)
...
def test_client_search_runs_defaults(mock_store):
    MLForgeClient().search_runs([1, 2, 3])
...
def test_client_search_runs_filter(mock_store):
    MLForgeClient().search_runs(["a", "b", "c"], "my filter")
    mock_store.search_runs.assert_called_once_with(
...
def test_client_search_runs_view_type(mock_store):
    MLForgeClient().search_runs(["a", "b", "c"], "my filter", ViewType.DELETED_ONLY)
    mock_store.search_runs.assert_called_once_with(
        experiment_ids=["a", "b", "c"],
        filter_string="my filter",
...
def test_client_search_runs_max_results(mock_store):
    MLForgeClient().search_runs([5], "my filter", ViewType.ALL, 2876)
    mock_store.search_runs.assert_called_once_with(
        experiment_ids=[5],
        filter_string="my filter",
...
def test_client_search_runs_order_by(mock_store):
    MLForgeClient().search_runs([5], order_by=["a", "b"])
    mock_store.search_runs.assert_called_once_with(
        experiment_ids=[5],
        filter_string="",
        run_view_type=ViewType.ACTIVE_ONLY,
...
def test_client_search_runs_page_token(mock_store):
    MLForgeClient().search_runs([5], page_token="blah")
    mock_store.search_runs.assert_called_once_with(
        experiment_ids=[5],
        filter_string="",
        run_view_type=ViewType.ACTIVE_ONLY,
...
def test_update_registered_model(mock_registry_store):
    ...
    res = MLForgeClient(registry_uri="sqlite:///somedb.db").update_registered_model(
...
def test_create_model_version(mock_registry_store):
    ...
    res = MLForgeClient(registry_uri="sqlite:///somedb.db").create_model_version(
...
def test_update_model_version(mock_registry_store):
    ...
    res = MLForgeClient(registry_uri="sqlite:///somedb.db").update_model_version(
...
def test_transition_model_version_stage(mock_registry_store):
    ...
    actual_result = MLForgeClient(registry_uri="sqlite:///somedb.db").transition_model_version_stage(
        name, version, stage
    )
...
def test_registry_uri_set_as_param():
    ...
    client = MLForgeClient(tracking_uri="databricks://tracking", registry_uri=uri)
...
def test_registry_uri_from_set_registry_uri():
    ...
    client = MLForgeClient(tracking_uri="databricks://tracking")
    assert client._registry_uri == uri
    set_registry_uri(None)
...
def test_registry_uri_from_tracking_uri_param():
    ...
    with mock.patch(
        "MLForge.tracking._tracking_service.utils.get_tracking_uri",
        return_value=tracking_uri,
    ):
        client = MLForgeClient(tracking_uri=tracking_uri)
...
def test_create_model_version_nondatabricks_source_no_runlink(mock_registry_store):
    run_id = "runid"
    client = MLForgeClient(tracking_uri="http://10.123.1231.11")
...
def test_create_model_version_explicitly_set_run_link(
    ...
    with (
        mock.patch("MLForge.utils.databricks_utils.is_in_databricks_notebook", return_value=True),
        mock.patch(
            "MLForge.utils.databricks_utils.get_workspace_info_from_dbutils",
            return_value=(hostname, workspace_id),
        ),
    ):
        client = MLForgeClient(tracking_uri="databricks", registry_uri="otherplace")
...
def test_create_model_version_with_source(mock_registry_store, mock_databricks_tracking_store):
    ...
    with mock.patch(
        "MLForge.tracking.client.MLForgeClient.get_logged_model", return_value=mock_logged_model
    ):
        client = MLForgeClient(tracking_uri="databricks")
    ...
    with mock.patch(
        ...
        client = MLForgeClient(tracking_uri="databricks", registry_uri="databricks-uc")
        model_version = client.create_model_version(
            "name", f"models:/{model_id}", "runid", run_link=None, model_id=model_id
        )
        assert model_version.model_id == model_id
...
def test_create_model_version_with_nondatabricks_source_uc_registry(mock_registry_store):
    ...
    with mock.patch(
        ...
        client = MLForgeClient(tracking_uri="http://10.123.1231.11", registry_uri="databricks-uc")
        model_version = client.create_model_version(
...
def test_create_model_version_run_link_with_configured_profile(
    ...
    with (
        mock.patch("MLForge.utils.databricks_utils.is_in_databricks_notebook", return_value=False),
        mock.patch(
            "MLForge.utils.databricks_utils.get_workspace_info_from_databricks_secrets",
            return_value=(hostname, workspace_id),
        ),
    ):
        client = MLForgeClient(tracking_uri="databricks", registry_uri="otherplace")
...
def test_create_model_version_copy_called_db_to_db(mock_registry_store):
    ...
    mock_registry_store.create_model_version.return_value = _default_model_version()
    with mock.patch("MLForge.tracking.client._upload_artifacts_to_databricks") as upload_mock:
...
def test_create_model_version_copy_not_called_to_nondb(mock_registry_store):
    client = MLForgeClient(tracking_uri="databricks://tracking", registry_uri="https://registry")
    mock_registry_store.create_model_version.return_value = _default_model_version()
    with mock.patch("MLForge.tracking.client._upload_artifacts_to_databricks") as upload_mock:
        client.create_model_version(
            "model name",
...
def test_client_can_be_serialized_with_pickle(tmp_path):
    """
    Verifies that instances of `MLForgeClient` can be serialized using pickle, even if the underlying
    ...
    # Create an MLForgeClient with the store that cannot be pickled, perform
    # tracking & model registry operations, and verify that the client can still be pickled
    client = MLForgeClient("pickle://foo")
...
def test_set_model_version_tag(mock_registry_store_with_get_latest_version):
    ...
    MLForgeClient().set_model_version_tag("model_name", 1, "tag1", "foobar")
    ...
    MLForgeClient().set_model_version_tag("model_name", key="tag1", value="foobar", stage="Staging")
    mock_registry_store_with_get_latest_version.set_model_version_tag.assert_called_once_with(
        "model_name", 1, ModelVersionTag(key="tag1", value="foobar")
    )

    # set_model_version_tag with version and stage set
    with pytest.raises(MLForgeException, match="version and stage cannot be set together"):
        MLForgeClient().set_model_version_tag("model_name", 1, "tag1", "foobar", stage="Staging")

    # set_model_version_tag with version and stage not set
    with pytest.raises(MLForgeException, match="version or stage must be set"):
        MLForgeClient().set_model_version_tag("model_name", key="tag1", value="foobar")


def test_delete_model_version_tag(mock_registry_store_with_get_latest_version):
    # delete_model_version_tag using version
    MLForgeClient().delete_model_version_tag("model_name", 1, "tag1")
    ...
    MLForgeClient().delete_model_version_tag("model_name", key="tag1", stage="Staging")
    mock_registry_store_with_get_latest_version.delete_model_version_tag.assert_called_once_with(
        "model_name", 1, "tag1"
    )
    ...
    with pytest.raises(MLForgeException, match="version and stage cannot be set together"):
        MLForgeClient().delete_model_version_tag(
            "model_name", version=1, key="tag1", stage="staging"
        )

    # delete_model_version_tag with version and stage not set
    with pytest.raises(MLForgeException, match="version or stage must be set"):
        MLForgeClient().delete_model_version_tag("model_name", key="tag1")


def test_set_registered_model_alias(mock_registry_store):
    MLForgeClient().set_registered_model_alias("model_name", "test_alias", 1)
...
def test_delete_registered_model_alias(mock_registry_store):
    MLForgeClient().delete_registered_model_alias("model_name", "test_alias")
...
def test_get_model_version_by_alias(mock_registry_store):
    ...
    res = MLForgeClient().get_model_version_by_alias("model_name", "test_alias")
    assert res == _default_model_version()
...
def test_update_run(mock_store):
    MLForgeClient().update_run(run_id="run_id", status="FINISHED", name="my name")
...
def test_client_log_metric_params_tags_overrides(mock_store):
    ...
    start_time = mock.Mock()
    run_name = mock.Mock()
    run = MLForgeClient().create_run(experiment_id, start_time, tags={}, run_name=run_name)
    run_id = run.info.run_id

    run_operation = MLForgeClient().log_metric(run_id, "m1", 0.87, 123456789, 1, synchronous=False)
    run_operation.wait()

    run_operation = MLForgeClient().log_param(run_id, "p1", "pv1", synchronous=False)
    run_operation.wait()

    run_operation = MLForgeClient().set_tag(run_id, "t1", "tv1", synchronous=False)
    run_operation.wait()
    ...
    # log_batch_async
    MLForgeClient().create_run(experiment_id, start_time, {})
    ...
    run_operation = MLForgeClient().log_batch(run_id, metrics, params, tags, synchronous=False)
    run_operation.wait()
...
def test_invalid_run_id_log_artifact():
    with pytest.raises(
        ...
        MLForgeClient().log_artifact("tr-123", "path")


def test_enable_async_logging(mock_store, setup_async_logging):
    MLForgeClient().log_param(run_id="run_id", key="key", value="val")
    mock_store.log_param_async.assert_called_once_with("run_id", Param("key", "val"))

    MLForgeClient().log_metric(run_id="run_id", key="key", value="val", step=1, timestamp=1)
...
def test_file_store_download_upload_trace_data(tmp_path):
    ...
    with _use_tracking_uri(tmp_path.joinpath("mlruns").as_uri()):
        ...
        trace = MLForge.get_trace(span.trace_id, flush=True)
...
def test_get_trace_throw_if_trace_id_is_online_trace_id(db_uri):
    client = MLForgeClient("databricks")
    trace_id = "3a3c3b56-910a-4721-8d02-0333eda5f37e"
    with pytest.raises(MLForgeException, match="Traces from inference tables can only be loaded"):
        client.get_trace(trace_id)

    another_client = MLForgeClient(db_uri)
    with pytest.raises(MLForgeException, match=r"Trace with ID '[\w-]+' not found"):
        another_client.get_trace(trace_id)


@pytest.fixture(params=["file", "sqlalchemy"])
def registry_uri(request, tmp_path, db_uri):
    """Set an MLForge Model Registry URI with different type of backend."""
    if request.param == "file":
        pytest.skip("FileStore is no longer supported.")
    if "MLForge_SKINNY" in os.environ and request.param == "sqlalchemy":
        pytest.skip("SQLAlchemy store is not available in skinny.")

    original_registry_uri = MLForge.get_registry_uri()
    ...
    # Reset tracking URI
    MLForge.set_tracking_uri(original_registry_uri)
...
def test_crud_prompts(tracking_uri):
    ...
    assert MLForge.load_prompt("does_not_exist", version=1, allow_missing=True) is None
...
def test_create_prompt_with_tags_and_metadata(tracking_uri, disable_prompt_cache):
    ...
    assert prompt_entity.tags == {
        ...
        "language": "en",
        "_MLForge_experiment_ids": ",0,",  # Linked to Default experiment
    }
...
def test_create_prompt_error_handling(tracking_uri, disable_prompt_cache):
    ...
    with pytest.raises(MLForgeException, match=r"Prompt text exceeds max length of"):
    ...
    with pytest.raises(MLForgeException, match=r"Prompt with name=prompt_1 not found"):
...
def test_create_prompt_with_invalid_name(tracking_uri):
    ...
    with pytest.raises(MLForgeException, match=r"Prompt name must be a non-empty string"):
    ...
    for invalid_pattern in [
        ...
        with pytest.raises(MLForgeException, match=r"Prompt name can only contain alphanumeric"):
    ...
    with pytest.raises(MLForgeException, match=r"Model 'model' exists with the same name."):
...
def test_load_prompt_error(tracking_uri):
    ...
    with pytest.raises(MLForgeException, match=r"Prompt with name=test not found"):
    ...
    with pytest.raises(MLForgeException, match=error_msg):
    ...
    with pytest.raises(MLForgeException, match=r"Name `model` is registered as a model"):
...
def test_link_prompt_version_to_run(tracking_uri):
    ...
    linked_prompts_tag = run_data.data.tags.get("MLForge.linkedPrompts")
    ...
    # Test error case
    with pytest.raises(MLForgeException, match=r"The `prompt` argument must be"):
...
def test_crud_prompt_on_unsupported_registry(registry_uri):
    client = MLForgeClient(registry_uri=registry_uri)

    with pytest.raises(MLForgeException, match=r"The 'register_prompt' API is not supported"):
        client.register_prompt(
            name="prompt_1",
            template="Hi, {{title}} {{name}}! How are you today?",
            commit_message="A friendly greeting",
    ...
    with pytest.raises(MLForgeException, match=r"The 'load_prompt' API is not supported"):
...
def test_block_create_model_with_prompt_tag(tracking_uri):
    ...
    with pytest.raises(MLForgeException, match=r"Prompts cannot be registered"):
...
def test_block_create_prompt_with_existing_model_name(tracking_uri):
    ...
    with pytest.raises(MLForgeException, match=r"Model 'model' exists with"):
        client.register_prompt(
            name="model",
            template="Hi, {{title}} {{name}}! How are you today?",
            commit_message="A friendly greeting",
...
def test_block_handling_prompt_with_model_apis(tracking_uri):
    ...
    for api, args in apis_to_args:
        with pytest.raises(MLForgeException, match=r"Registered Model with name='prompt' not found"):
            api(*args)

    with pytest.raises(MLForgeException, match=r"Model with uri 'models:/prompt/1' not found"):
...
def test_delete_prompt_version_no_auto_cleanup(tracking_uri):
    ...
    with pytest.raises(MLForgeException, match=r"Prompt.*name=test_prompt.*version=1.*not found"):
...
def test_set_prompt_model_config_invalidates_latest_cache(tracking_uri):
    ...
    MLForge.genai.set_prompt_model_config(
        name=prompt.name,
...
def test_delete_prompt_model_config_invalidates_latest_cache(tracking_uri):
    ...
    MLForge.genai.delete_prompt_model_config(name=prompt.name, version=prompt.version)
...
def test_delete_prompt_with_no_versions(tracking_uri):
    client = MLForgeClient(tracking_uri=tracking_uri)
    MLForge.set_experiment("test_delete_prompt_with_no_versions")
...
def test_delete_prompt_invalidates_cached_load_prompt(tracking_uri):
    ...
    with pytest.raises(MLForgeException, match=rf"Prompt.*name={prompt_ver.name}.*not found"):
        client.load_prompt(prompt_ver.name, version=prompt_ver.version)
...
def test_delete_prompt_error_handling(tracking_uri):
    ...
    with pytest.raises(MLForgeException, match=r"Prompt with name=nonexistent not found"):
    ...
    with pytest.raises(MLForgeException, match=r"Prompt.*name=test_errors.*version=999.*not found"):
...
def test_delete_prompt_version_behavior_consistency(tracking_uri):
    ...
    for i in range(3):
        ...
        # Version should be gone
        with pytest.raises(MLForgeException, match=r"Prompt.*version.*not found"):
...
def test_delete_prompt_with_versions_unity_catalog_error(registry_uri):
    ...
    with (
        ...
        with pytest.raises(
            MLForgeException, match=r"Cannot delete prompt .* because it still has undeleted"
        ):
...
def test_logged_model_model_id_required(tracking_uri):
    ...
    with pytest.raises(MLForgeException, match="`model_id` must be a non-empty string, but got ''"):
...
def test_log_metric_link_to_active_model(tracking_uri):
    ...
    MLForge.set_active_model(name=model.name)
    client = MLForgeClient(tracking_uri=tracking_uri)
...
def test_load_prompt_with_alias_uri(tracking_uri, disable_prompt_cache):
    ...
    with pytest.raises(
        MLForgeException, match=r"Prompt (.*) does not exist.|Prompt alias (.*) not found."
    ):
...
def test_load_prompt_allow_missing_name_version(tracking_uri):
    ...
    with pytest.raises(MLForgeException, match="Prompt with name=nonexistent_prompt not found"):
    ...
    with pytest.raises(
        MLForgeException, match=r"Prompt \(name=existing_prompt, version=999\) not found"
    ):
...
def test_load_prompt_allow_missing_uri_alias(tracking_uri):
    ...
    with pytest.raises(MLForgeException, match="Prompt alias nonexistent_alias not found"):
...
def test_MLForge_client_create_dataset(mock_store):
    ...
    # Mock context registry to return empty tags so MLForge.user is not auto-added
    with mock.patch(
        "MLForge.tracking._tracking_service.client.context_registry.resolve_tags", return_value={}
    ):
        dataset = MLForgeClient().create_dataset(
            name="qa_evaluation",
...
def test_MLForge_client_create_evaluation_dataset_minimal(mock_store):
    created_dataset = EvaluationDataset(
        dataset_id="test_dataset_id",
        name="test_dataset",
        digest="abcdef123456",
    ...
    with mock.patch(
        ...
        dataset = MLForgeClient().create_dataset(name="test_dataset")
...
def test_MLForge_client_get_dataset(mock_store):
    ...
    dataset = MLForgeClient().get_dataset("dataset_123")
...
def test_MLForge_client_delete_dataset(mock_store):
    MLForgeClient().delete_dataset("dataset_123")

    mock_store.delete_dataset.assert_called_once_with("dataset_123")


def test_MLForge_client_search_datasets(mock_store):
    ...
    result = MLForgeClient().search_datasets(
        experiment_ids=["exp1", "exp2"],
...
def test_MLForge_client_search_datasets_empty_results(mock_store):
...
def test_MLForge_client_search_datasets_defaults(mock_store):
    mock_store.search_datasets.return_value = PagedList([], None)
...
def test_MLForge_client_datasets_filestore_not_supported(tmp_path):
    pytest.skip("FileStore is no longer supported.")
    file_store_uri = str(tmp_path)
    client = MLForgeClient(tracking_uri=file_store_uri)

    with pytest.raises(MLForgeException, match="is not supported with FileStore") as exc_info:
...
def test_MLForge_client_set_dataset_tags(mock_store):
    MLForgeClient().set_dataset_tags(
        dataset_id="dataset_123",
...
def test_MLForge_client_delete_dataset_tag(mock_store):
    MLForgeClient().delete_dataset_tag(
        dataset_id="dataset_123",
        key="deprecated",
    )
...
def test_MLForge_client_add_dataset_to_experiments(mock_store):
...
def test_MLForge_client_remove_dataset_from_experiments(mock_store):
    mock_dataset = Mock(spec=EvaluationDataset)
    mock_dataset.dataset_id = "dataset_123"
...
def test_MLForge_client_dataset_associations_databricks_blocking(mock_store):
    with mock.patch("MLForge.utils.databricks_utils.is_databricks_uri") as mock_is_dbx:
        mock_is_dbx.return_value = True
        client = MLForgeClient(tracking_uri="databricks")

        with pytest.raises(
            MLForgeException, match="not supported when tracking URI is 'databricks'"
        ) as exc_info:
            client.add_dataset_to_experiments("dataset_123", ["1", "2"])
...
def test_log_spans_and_get_trace_with_sqlalchemy_store(tmp_path: Path) -> None:
    ...
    with _use_tracking_uri(tracking_uri):
        ...
        otel_span1 = OTelReadableSpan(
            ...
            parent=None,
            attributes={
                "MLForge.traceRequestId": json.dumps(trace_id, cls=TraceJSONEncoder),
        ...
        # Convert to MLForge spans
        MLForge_spans = [
            create_MLForge_span(otel_span1, trace_id, "LLM"),
            create_MLForge_span(otel_span2, trace_id, "LLM"),
        ]

        # Log spans directly to the store (simulating OTLP endpoint)
        store = client._tracking_client.store
        logged_spans = store.log_spans(experiment_id, MLForge_spans)
        ...
        # Now test that MLForge.get_trace() works and loads spans from the database
        trace = MLForge.get_trace(trace_id)
...
def test_MLForge_get_trace_with_sqlalchemy_store(tmp_path: Path) -> None:
    tracking_uri = f"sqlite:///{tmp_path}/test.db"
    ...
    with _use_tracking_uri(tracking_uri):
        ...
        with MLForge.start_span() as span:
            pass

        trace_id = span.trace_id
        MLForge.flush_trace_async_logging()
        sql_alchemy_store_module = "MLForge.store.tracking.sqlalchemy_store.SqlAlchemyStore"
        with (
            mock.patch(f"{sql_alchemy_store_module}.get_trace") as mock_get_trace,
        ):
            MLForge.get_trace(trace_id)
        ...
        with (
            mock.patch(
                ...
                side_effect=MLForgeNotImplementedException,
            ) as mock_get_trace,

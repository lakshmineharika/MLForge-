...
def reload_registry():
    yield
    reload(MLForge.tracking.request_header.registry)
...
def test_request_header_provider_registry_register_entrypoints():
    ...
    mock_get_group_all.assert_called_once_with("MLForge.request_header_provider")
...
def _currently_registered_request_header_provider_classes():
    return {
        provider.__class__
        for provider in MLForge.tracking.request_header.registry._request_header_provider_registry
    }
...
def mock_request_header_providers():
    ...
    with mock.patch(
        "MLForge.tracking.request_header.registry._request_header_provider_registry", providers
    ):
        yield

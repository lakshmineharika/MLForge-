...
import MLForge
import MLForge.tracking.context.registry
import MLForge.tracking.fluent
from MLForge import MLForgeClient, clear_active_model, set_active_model
from MLForge.data.http_dataset_source import HTTPDatasetSource
from MLForge.data.meta_dataset import MetaDataset
...
from MLForge.environment_variables import (
    _MLForge_ACTIVE_MODEL_ID,
    _MLForge_ENABLE_SGC_RUN_RESUMPTION_FOR_DATABRICKS_JOBS,
    MLForge_ACTIVE_MODEL_ID,
    MLForge_EXPERIMENT_ID,
    MLForge_EXPERIMENT_NAME,
    MLForge_REGISTRY_URI,
    MLForge_RUN_ID,
)
from MLForge.exceptions import MLForgeException
from MLForge.models.model import (
    MLMODEL_FILE_NAME,
    Model,
    _update_active_model_id_based_on_MLForge_model,
)
from MLForge.protos.databricks_pb2 import RESOURCE_DOES_NOT_EXIST, TEMPORARILY_UNAVAILABLE
from MLForge.store.entities.paged_list import PagedList
from MLForge.store.model_registry import (
...
from MLForge.tracking.fluent import (
    ...
    _get_sgc_MLForge_run_id_for_resumption,
...
from MLForge.utils import get_results_from_paginated_fn, MLForge_tags
from MLForge.utils.async_logging.async_logging_queue import (
...
def create_experiment(
    ...
    return MLForge.entities.Experiment(experiment_id, name, artifact_location, lifecycle_stage, tags)
...
def reload_context_registry():
    ...
    reload(MLForge.tracking.context.registry)


@pytest.fixture(params=["list", "pandas"])
def search_runs_output_format(request):
    if "MLForge_SKINNY" in os.environ and request.param == "pandas":
...
def test_get_experiment_id_from_env(monkeypatch):
    ...
    assert not MLForge_EXPERIMENT_NAME.defined
    assert not MLForge_EXPERIMENT_ID.defined
    assert _get_experiment_id_from_env() is None

    # set only ID
    name = f"random experiment {random.randint(1, int(1e6))}"
    exp_id = MLForge.create_experiment(name)
    assert exp_id is not None
    monkeypatch.delenv(MLForge_EXPERIMENT_NAME.name, raising=False)
    monkeypatch.setenv(MLForge_EXPERIMENT_ID.name, str(exp_id))
    ...
    exp_id = MLForge.create_experiment(name)
    assert exp_id is not None
    monkeypatch.delenv(MLForge_EXPERIMENT_ID.name, raising=False)
    monkeypatch.setenv(MLForge_EXPERIMENT_NAME.name, name)
    assert _get_experiment_id_from_env() == exp_id
    ...
    monkeypatch.delenv(MLForge_EXPERIMENT_ID.name, raising=False)
    monkeypatch.setenv(MLForge_EXPERIMENT_NAME.name, name)
    assert MLForgeClient().get_experiment_by_name(name) is None
    ...
    monkeypatch.delenv(MLForge_EXPERIMENT_NAME.name, raising=False)
    monkeypatch.setenv(MLForge_EXPERIMENT_ID.name, str(random_id))
    with pytest.raises(
        MLForgeException,
        match=(
            f"The provided {MLForge_EXPERIMENT_ID} environment variable value "
    ...
    random_id = random.randint(100, int(1e6))
    monkeypatch.setenv(MLForge_EXPERIMENT_ID.name, str(random_id))
    monkeypatch.setenv(MLForge_EXPERIMENT_NAME.name, invalid_name)
...
def test_get_experiment_id_in_databricks_detects_notebook_id_by_default():
    ...
    with mock.patch(
        "MLForge.tracking.fluent.default_experiment_registry.get_experiment_id",
...
def test_get_experiment_id_in_databricks_with_active_experiment_returns_active_experiment_id():
    ...
    exp_id = MLForge.create_experiment(exp_name)
    MLForge.set_experiment(exp_name)
...
def test_get_experiment_by_id():
    ...
    experiment = MLForge.get_experiment(exp_id)
...
def test_get_experiment_by_name():
    ...
    experiment = MLForge.get_experiment_by_name(name)
    assert experiment.experiment_id == exp_id
...
def test_search_experiments(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    MAX_RESULTS = 50
    monkeypatch.setattr(
        "MLForge.store.tracking.sqlalchemy_store.SEARCH_MAX_RESULTS_DEFAULT", MAX_RESULTS
    )

    sqlite_uri = "sqlite:///{}".format(tmp_path.joinpath("test.db"))
    MLForge.set_tracking_uri(sqlite_uri)
    # Why do we need this line? If we didn't have this line, the first `MLForge.create_experiment`
    ...
    for tag, active_experiment_name in zip_longest(tag_values, active_experiment_names):
        ...
        time.sleep(0.001)
        MLForge.create_experiment(active_experiment_name, tags={"tag": tag} if tag else None)
    ...
    for deleted_experiment_name in deleted_experiment_names:
        time.sleep(0.001)
        exp_id = MLForge.create_experiment(deleted_experiment_name)
        MLForge.delete_experiment(exp_id)

    # max_results is unspecified
    experiments = MLForge.search_experiments(view_type=ViewType.ALL)
    assert len(experiments) == num_all_experiments
    # max_results is larger than the number of experiments in the database
    experiments = MLForge.search_experiments(
    ...
    experiments = MLForge.search_experiments(view_type=ViewType.ALL, max_results=num_all_experiments)
    assert len(experiments) == num_all_experiments
    ...
    # Filter by view_type
    experiments = MLForge.search_experiments(view_type=ViewType.ACTIVE_ONLY)
    assert [e.name for e in experiments] == active_experiment_names[::-1] + ["Default"]
    experiments = MLForge.search_experiments(view_type=ViewType.DELETED_ONLY)
    ...
    # Filter by name
    experiments = MLForge.search_experiments(filter_string="name = 'active_1'")
    assert [e.name for e in experiments] == ["active_1"]
    experiments = MLForge.search_experiments(filter_string="name ILIKE 'active_%'")
    assert [e.name for e in experiments] == active_experiment_names[::-1]

    # Filter by tags
    experiments = MLForge.search_experiments(filter_string="tags.tag = 'x'")
    assert [e.name for e in experiments] == active_experiment_names[:2][::-1]
    experiments = MLForge.search_experiments(filter_string="tags.tag = 'y'")
    assert [e.experiment_id for e in experiments] == ["3"]

    # Order by name
    experiments = MLForge.search_experiments(order_by=["name DESC"], max_results=3)
...
def test_search_registered_models(tmp_path):
    ...
    for tag, model_name in zip_longest(tag_values, model_names):
        MLForgeClient().create_registered_model(model_name, tags={"tag": tag} if tag else None)

    # max_results is unspecified
    models = MLForge.search_registered_models()
    assert len(models) == num_all_models

    # max_results is larger than the number of models in the database
    models = MLForge.search_registered_models(max_results=num_all_models + 1)
    assert len(models) == num_all_models

    # max_results is equal to the number of models in the database
    models = MLForge.search_registered_models(max_results=num_all_models)
    assert len(models) == num_all_models
    ...
    # Filter by name
    models = MLForge.search_registered_models(filter_string="name = 'AModel_1'")
    assert [m.name for m in models] == ["AModel_1"]
    models = MLForge.search_registered_models(filter_string="name ILIKE 'bmodel_%'")
    assert len(models) == num_b_models

    # Filter by tags
    models = MLForge.search_registered_models(filter_string="tags.tag = 'x'")
    assert [m.name for m in models] == model_names[:2]
    models = MLForge.search_registered_models(filter_string="tags.tag = 'y'")
    assert [m.name for m in models] == [model_names[2]]

    # Order by name
    models = MLForge.search_registered_models(order_by=["name DESC"], max_results=3)
...
def test_search_model_versions(tmp_path):
    ...
    with mock.patch(
        "MLForge.store.model_registry.SEARCH_MODEL_VERSION_MAX_RESULTS_DEFAULT",
        max_results_default,
    ):
        ...
        MLForgeClient().create_registered_model(name="AModel")
        MLForgeClient().create_registered_model(name="BModel")

        tag_values = ["x", "x", "y"]
        for tag, model_name in zip_longest(tag_values, model_version_names):
            MLForgeClient().create_model_version(
                name=model_name, source="foo/bar", tags={"tag": tag} if tag else None
            )

        # max_results is unspecified
        model_versions = MLForge.search_model_versions()
        assert len(model_versions) == num_all_model_versions

        # max_results is larger than the number of model versions in the database
        model_versions = MLForge.search_model_versions(max_results=num_all_model_versions + 1)
        assert len(model_versions) == num_all_model_versions

        # max_results is equal to the number of model versions in the database
        model_versions = MLForge.search_model_versions(max_results=num_all_model_versions)
        assert len(model_versions) == num_all_model_versions
        # max_results is smaller than the number of models in the database
        model_versions = MLForge.search_model_versions(max_results=num_all_model_versions - 1)
        ...
        # Filter by name
        model_versions = MLForge.search_model_versions(filter_string="name = 'AModel'")
        assert [m.name for m in model_versions] == a_model_version_names
        model_versions = MLForge.search_model_versions(filter_string="name ILIKE 'bmodel'")
        assert len(model_versions) == num_b_model_versions

        # Filter by tags
        model_versions = MLForge.search_model_versions(filter_string="tags.tag = 'x'")
        assert [m.name for m in model_versions] == model_version_names[:2]
        model_versions = MLForge.search_model_versions(filter_string="tags.tag = 'y'")
        assert [m.name for m in model_versions] == [model_version_names[2]]
...
def empty_active_run_stack():
    with mock.patch("MLForge.tracking.fluent._active_run_stack.get", return_value=[]):
        yield
...
def test_start_run_defaults(empty_active_run_stack):
    MLForge.disable_system_metrics_logging()
    mock_experiment_id = mock.Mock()
    experiment_id_patch = mock.patch(
        "MLForge.tracking.fluent._get_experiment_id", return_value=mock_experiment_id
    )
    mock_user = mock.Mock()
    user_patch = mock.patch(
        "MLForge.tracking.context.default_context._get_user", return_value=mock_user
    )
    mock_source_name = mock.Mock()
    source_name_patch = mock.patch(
        "MLForge.tracking.context.default_context._get_source_name", return_value=mock_source_name
    )
    source_type_patch = mock.patch(
        "MLForge.tracking.context.default_context._get_source_type", return_value=SourceType.NOTEBOOK
    )
    mock_source_version = mock.Mock()
    source_version_patch = mock.patch(
        "MLForge.tracking.context.git_context._resolve_git_info",
        return_value={MLForge_tags.MLForge_GIT_COMMIT: mock_source_version},
    )
    run_name = "my name"

    expected_tags = {
        MLForge_tags.MLForge_USER: mock_user,
        MLForge_tags.MLForge_SOURCE_NAME: mock_source_name,
        MLForge_tags.MLForge_SOURCE_TYPE: SourceType.to_string(SourceType.NOTEBOOK),
        MLForge_tags.MLForge_GIT_COMMIT: mock_source_version,
        MLForge_tags.MLForge_RUN_NAME: run_name,
    }

    create_run_patch = mock.patch.object(MLForgeClient, "create_run")
    ...
    with (
        ...
        MLForgeClient.create_run.assert_called_once_with(
            experiment_id=mock_experiment_id, tags=expected_tags, run_name="my name"
        )
        assert is_from_run(active_run, MLForgeClient.create_run.return_value)
...
def test_start_run_defaults_databricks_notebook(
    ...
    databricks_notebook_patch = mock.patch(
        "MLForge.utils.databricks_utils.is_in_databricks_notebook", return_value=True
    )
    mock_user = mock.Mock()
    user_patch = mock.patch(
        "MLForge.tracking.context.default_context._get_user", return_value=mock_user
    ...
    notebook_id_patch = mock.patch(
        "MLForge.utils.databricks_utils.get_notebook_id", return_value=mock_notebook_id
    )
    mock_notebook_path = mock.Mock()
    notebook_path_patch = mock.patch(
        "MLForge.utils.databricks_utils.get_notebook_path", return_value=mock_notebook_path
    )
    mock_webapp_url = mock.Mock()
    webapp_url_patch = mock.patch(
        "MLForge.utils.databricks_utils.get_webapp_url", return_value=mock_webapp_url
    )
    mock_workspace_url = mock.Mock()
    workspace_url_patch = mock.patch(
        "MLForge.utils.databricks_utils.get_workspace_url", return_value=mock_workspace_url
    )
    mock_workspace_id = mock.Mock()
    workspace_info_patch = mock.patch(
        "MLForge.utils.databricks_utils.get_workspace_id",
        return_value=mock_workspace_id,
    )

    expected_tags = {
        MLForge_tags.MLForge_USER: mock_user,
        MLForge_tags.MLForge_SOURCE_NAME: mock_notebook_path,
        MLForge_tags.MLForge_SOURCE_TYPE: SourceType.to_string(SourceType.NOTEBOOK),
        MLForge_tags.MLForge_GIT_COMMIT: mock_source_version,
        MLForge_tags.MLForge_DATABRICKS_NOTEBOOK_ID: mock_notebook_id,
        MLForge_tags.MLForge_DATABRICKS_NOTEBOOK_PATH: mock_notebook_path,
        MLForge_tags.MLForge_DATABRICKS_WEBAPP_URL: mock_webapp_url,
        MLForge_tags.MLForge_DATABRICKS_WORKSPACE_URL: mock_workspace_url,
        MLForge_tags.MLForge_DATABRICKS_WORKSPACE_ID: mock_workspace_id,
    }
...
def test_start_run_raises_invalid_experiment_id(experiment_id):
    with pytest.raises(MLForgeException, match="Invalid experiment id: "):
...
def test_start_run_creates_new_run_with_user_specified_tags():
    ...
    user_specified_tags = {
        "ml_task": "regression",
        "num_layers": 7,
        MLForge_tags.MLForge_USER: "user_override",
    }
    expected_tags = {
        MLForge_tags.MLForge_SOURCE_NAME: mock_source_name,
        MLForge_tags.MLForge_SOURCE_TYPE: SourceType.to_string(SourceType.NOTEBOOK),
...
def test_start_run_resumes_existing_run_and_sets_user_specified_tags():
    ...
    run_id = MLForge.start_run().info.run_id
    MLForge.end_run()
    restarted_run = MLForge.start_run(run_id, tags=tags_to_set)
    assert tags_to_set.items() <= restarted_run.data.tags.items()


def test_start_run_resumes_existing_run_and_update_run_name():
    with MLForge.start_run(run_name="old_name") as run:
        run_id = run.info.run_id
    with MLForge.start_run(run_id, run_name="new_name"):
        pass
    assert MLForgeClient().get_run(run_id).info.run_name == "new_name"
...
def test_start_run_with_parent():
    ...
    active_run_stack_patch = mock.patch(
        "MLForge.tracking.fluent._active_run_stack.get", lambda: [parent_run]
    )
    ...
    expected_tags = {
        MLForge_tags.MLForge_USER: mock_user,
        MLForge_tags.MLForge_SOURCE_NAME: mock_source_name,
        MLForge_tags.MLForge_SOURCE_TYPE: SourceType.to_string(SourceType.LOCAL),
        MLForge_tags.MLForge_PARENT_RUN_ID: parent_run.info.run_id,
    }
...
def test_start_run_with_parent_id():
    parent_run_id = MLForge.start_run().info.run_id
    MLForge.end_run()
    nested_run_id = MLForge.start_run(parent_run_id=parent_run_id).info.run_id
    MLForge.end_run()

    assert MLForge.get_parent_run(nested_run_id).info.run_id == parent_run_id


@pytest.mark.usefixtures(empty_active_run_stack.__name__)
def test_start_run_with_invalid_parent_id():
    with MLForge.start_run() as run:
        with pytest.raises(MLForgeException, match=f"Current run with UUID {run.info.run_id}"):
            with MLForge.start_run(nested=True, parent_run_id="hello"):
                pass


def test_start_run_with_parent_non_nested():
    with mock.patch("MLForge.tracking.fluent._active_run_stack.get", return_value=[mock.Mock()]):
...
def test_start_run_existing_run(empty_active_run_stack):
    ...
    mock_get_store = mock.patch("MLForge.tracking.fluent._get_store")

    with mock_get_store, mock.patch.object(MLForgeClient, "get_run", return_value=mock_run):
        active_run = start_run(run_id)

        assert is_from_run(active_run, mock_run)
        MLForgeClient.get_run.assert_called_with(run_id)
...
def test_start_run_existing_run_from_environment(empty_active_run_stack, monkeypatch):
    ...
    run_id = uuid.uuid4().hex
    monkeypatch.setenv(MLForge_RUN_ID.name, run_id)
    mock_get_store = mock.patch("MLForge.tracking.fluent._get_store")
...
def test_start_run_existing_run_from_environment_with_set_environment(
    ...
    run_id = uuid.uuid4().hex
    monkeypatch.setenv(MLForge_RUN_ID.name, run_id)
    with mock.patch.object(MLForgeClient, "get_run", return_value=mock_run):
        set_experiment("test-run")
        with pytest.raises(
            MLForgeException, match="active experiment ID does not match environment run ID"
        ):
            start_run()
...
def test_start_run_existing_run_deleted(empty_active_run_stack):
    ...
    with mock.patch.object(MLForgeClient, "get_run", return_value=mock_run):
        with pytest.raises(MLForgeException, match=match):
            start_run(run_id)
...
def test_start_existing_run_status(empty_active_run_stack):
    run_id = MLForge.start_run().info.run_id
    MLForge.end_run()
    assert MLForgeClient().get_run(run_id).info.status == RunStatus.to_string(RunStatus.FINISHED)
    restarted_run = MLForge.start_run(run_id)
...
def test_start_run_with_description(empty_active_run_stack):
    ...
    expected_tags = {
        ...
        MLForge_tags.MLForge_GIT_COMMIT: mock_source_version,
        MLForge_tags.MLForge_USER: mock_user,
        MLForge_tags.MLForge_RUN_NOTE: description,
    }
...
def test_start_run_conflicting_description():
    description = "Test description"
    invalid_tags = {MLForge_tags.MLForge_RUN_NOTE: "Another description"}
    match = (
        f"Description is already set via the tag {MLForge_tags.MLForge_RUN_NOTE} in tags."
        f"Remove the key {MLForge_tags.MLForge_RUN_NOTE} from the tags or omit the description."
    )
...
def test_start_run_resumes_existing_run_and_sets_description():
    ...
    run_id = MLForge.start_run().info.run_id
    MLForge.end_run()
    restarted_run = MLForge.start_run(run_id, description=description)
    assert MLForge_tags.MLForge_RUN_NOTE in restarted_run.data.tags
...
def test_start_run_resumes_existing_run_and_sets_description_twice():
    ...
    MLForge.end_run()
    with pytest.raises(MLForgeException, match=match):
        MLForge.start_run(run_id, tags=invalid_tags, description=description)
...
def test_search_runs_attributes(search_runs_output_format):
    ...
    with mock.patch("MLForge.tracking.fluent.get_results_from_paginated_fn", return_value=runs):
        pdf = search_runs(output_format=search_runs_output_format)
        validate_search_runs(pdf, data, search_runs_output_format)


@pytest.mark.skipif(
    "MLForge_SKINNY" in os.environ,
...
def test_search_runs_no_arguments(search_runs_output_format):
    ...
    get_paginated_runs_patch = mock.patch(
        "MLForge.tracking.fluent.get_results_from_paginated_fn", return_value=[]
    )
    with experiment_id_patch, get_paginated_runs_patch:
        search_runs(output_format=search_runs_output_format)
        MLForge.tracking.fluent.get_results_from_paginated_fn.assert_called_once()
        MLForge.tracking.fluent._get_experiment_id.assert_called_once()
...
def test_search_runs_all_experiments(search_runs_output_format):
    ...
    experiment_list_patch = mock.patch(
        "MLForge.tracking.fluent.search_experiments", return_value=[mock_experiment]
    )
    get_paginated_runs_patch = mock.patch(
        "MLForge.tracking.fluent.get_results_from_paginated_fn", return_value=[]
    )
    with experiment_id_patch, experiment_list_patch, get_paginated_runs_patch:
        ...
        MLForge.tracking.fluent.search_experiments.assert_called_once()
        MLForge.tracking.fluent._get_experiment_id.assert_not_called()
...
def test_search_runs_by_experiment_name():
    ...
    get_experiment_patch = mock.patch(
        "MLForge.tracking.fluent.get_experiment_by_name", return_value=experiment
    )
    get_paginated_runs_patch = mock.patch(
        "MLForge.tracking.fluent.get_results_from_paginated_fn", return_value=runs
    )
...
def test_search_runs_by_experiment_id_and_name():
    ...
    with pytest.raises(MLForgeException, match=err_msg):
...
def test_delete_tag():
    ...
    """
    MLForge.set_tag("a", "b")
    run = MLForgeClient().get_run(MLForge.active_run().info.run_id)
    assert "a" in run.data.tags
    MLForge.delete_tag("a")
    run = MLForgeClient().get_run(MLForge.active_run().info.run_id)
    assert "a" not in run.data.tags
    with pytest.raises(MLForgeException, match="No tag with name"):
        MLForge.delete_tag("a")
    with pytest.raises(MLForgeException, match="No tag with name"):
        MLForge.delete_tag("b")
    MLForge.end_run()


def test_last_active_run_returns_currently_active_run():
    run_id = MLForge.start_run().info.run_id
    last_active_run_id = MLForge.last_active_run().info.run_id
    MLForge.end_run()
...
def test_last_active_run_returns_most_recently_ended_active_run():
    run_id = MLForge.start_run().info.run_id
    MLForge.log_metric("a", 1.0)
    MLForge.log_param("b", 2)
    MLForge.end_run()
    last_active_run = MLForge.last_active_run()
...
def test_set_experiment_tag():
    ...
    with start_run() as active_run:
        ...
        current_experiment = MLForge.tracking.MLForgeClient().get_experiment(test_experiment)
        assert len(current_experiment.tags) == 0
        for tag_key, tag_value in test_tags.items():
            MLForge.set_experiment_tag(tag_key, tag_value)
            tag_counter += 1
            current_experiment = MLForge.tracking.MLForgeClient().get_experiment(test_experiment)
            assert tag_counter == len(current_experiment.tags)
        finished_experiment = MLForge.tracking.MLForgeClient().get_experiment(test_experiment)
...
def test_set_experiment_tags():
    ...
    with start_run() as active_run:
        ...
        current_experiment = MLForge.tracking.MLForgeClient().get_experiment(test_experiment)
        assert len(current_experiment.tags) == 0
        MLForge.set_experiment_tags(exact_expected_tags)
    finished_experiment = MLForge.tracking.MLForgeClient().get_experiment(test_experiment)
    # Validate tags
...
def test_delete_experiment_tag():
    with start_run() as active_run:
        test_experiment = active_run.info.experiment_id
        MLForge.set_experiment_tag("a", "b")
        current_experiment = MLForge.tracking.MLForgeClient().get_experiment(test_experiment)
        assert "a" in current_experiment.tags
        MLForge.delete_experiment_tag("a")
        finished_experiment = MLForge.tracking.MLForgeClient().get_experiment(test_experiment)
...
def test_set_experiment_throws_for_unexpected_error(error_code: int):
    with mock.patch(
        "MLForge.tracking._tracking_service.client.TrackingServiceClient.create_experiment",
        side_effect=MLForgeException("Unexpected error", error_code=error_code),
    ) as mock_create_experiment:
        with pytest.raises(MLForgeException, match="Unexpected error"):
...
def test_log_input(tmp_path):
    ...
    dataset_inputs = MLForgeClient().get_run(run.info.run_id).inputs.dataset_inputs
    ...
    with start_run() as run:
        MLForge.log_input(dataset, "train")
    dataset_inputs = MLForgeClient().get_run(run.info.run_id).inputs.dataset_inputs
    ...
    assert dataset_inputs[0].tags[0].key == MLForge_tags.MLForge_DATASET_CONTEXT
...
def test_log_inputs(tmp_path):
    ...
    logged_inputs = MLForgeClient().get_run(run.info.run_id).inputs
    ...
    assert dataset_inputs[1].tags[0].key == MLForge_tags.MLForge_DATASET_CONTEXT
    assert dataset_inputs[1].tags[0].value == "train2"

    assert json.loads(dataset_inputs[2].dataset.source) == {"uri": str(path3)}
    assert dataset_inputs[2].tags[0].key == MLForge_tags.MLForge_DATASET_CONTEXT
...
def test_log_input_polars(tmp_path):
    ...
    path = tmp_path / "temp.csv"
    df.write_csv(path)
    dataset = MLForge.data.from_polars(df, source=path)
    with start_run() as run:
        MLForge.log_input(dataset, "train")
...
def test_log_input_metadata_only():
    ...
    dataset = MLForge.data.meta_dataset.MetaDataset(source=source)
...
def test_get_parent_run():
    with MLForge.start_run() as parent:
        MLForge.log_param("a", 1)
        MLForge.log_metric("b", 2.0)
        with MLForge.start_run(nested=True) as child_run:
            child_run_id = child_run.info.run_id

    with MLForge.start_run() as run:
        run_id = run.info.run_id

    parent_run = MLForge.get_parent_run(child_run_id)
    assert parent_run.info.run_id == parent.info.run_id
    assert parent_run.data.metrics == {"b": 2.0}
    assert parent_run.data.params == {"a": "1"}

    assert MLForge.get_parent_run(run_id) is None
...
def test_log_metric_async():
    ...
    with MLForge.start_run() as parent:
        run_operations.extend(
            MLForge.log_metric("async single metric", step=num, value=num, synchronous=False)
            for num in range(100)
        )
        metrics = {f"async batch metric {num}": num for num in range(100)}
        run_operations.append(MLForge.log_metrics(metrics=metrics, step=1, synchronous=False))

    for run_operation in run_operations:
        run_operation.wait()
    parent_run = MLForge.get_run(parent.info.run_id)
    assert parent_run.info.run_id == parent.info.run_id
...
def test_log_metric_async_throws():
    with MLForge.start_run():
        with pytest.raises(MLForgeException, match="Please specify value as a valid double"):
            MLForge.log_metric(
...
def test_log_param_async():
    ...
    with MLForge.start_run() as parent:
        run_operations.append(MLForge.log_param("async single param", value="1", synchronous=False))
        params = {f"async batch param {num}": num for num in range(100)}
        run_operations.append(MLForge.log_params(params=params, synchronous=False))
...
def test_log_param_async_throws():
    with MLForge.start_run():
        MLForge.log_param("async single param", value="1", synchronous=False).wait()
        with pytest.raises(MLForgeException, match=".*Changing param values is not allowed.*"):
            MLForge.log_param("async single param", value="2", synchronous=False).wait()

        MLForge.log_params({"async batch param": "2"}, synchronous=False).wait()
        with pytest.raises(MLForgeException, match=".*Changing param values is not allowed.*"):
            MLForge.log_params({"async batch param": "3"}, synchronous=False).wait()
...
def test_flush_async_logging(flush_within_run):
    ...
    with MLForge.start_run() as run:
        for i in range(100):
            MLForge.log_metric("dummy", i, step=i, synchronous=False)

        if flush_within_run:
            MLForge.flush_async_logging()

    if not flush_within_run:
        MLForge.flush_async_logging()

    metric_history = MLForge.MLForgeClient().get_metric_history(run.info.run_id, "dummy")
...
def test_enable_async_logging():
    MLForge.config.enable_async_logging(True)
    with mock.patch(
        "MLForge.utils.async_logging.async_logging_queue.AsyncLoggingQueue.log_batch_async"
    ) as mock_log_batch_async:
        with MLForge.start_run():
            MLForge.log_metric("dummy", 1)
            MLForge.log_param("dummy", 1)
            MLForge.set_tag("dummy", 1)
            MLForge.log_metrics({"dummy": 1})
            MLForge.log_params({"dummy": 1})
            MLForge.set_tags({"dummy": 1})

    assert mock_log_batch_async.call_count == 6

    MLForge.config.enable_async_logging(False)
    with mock.patch(
        "MLForge.utils.async_logging.async_logging_queue.AsyncLoggingQueue.log_batch_async"
    ) as mock_log_batch_async:
        with MLForge.start_run():
...
def test_set_tag_async():
    ...
    with MLForge.start_run() as parent:
        run_operations.append(MLForge.set_tag("async single tag", value="1", synchronous=False))
        tags = {f"async batch tag {num}": num for num in range(100)}
        run_operations.append(MLForge.set_tags(tags=tags, synchronous=False))
...
def spark_session_with_registry_uri(request):
    with mock.patch(
        "MLForge.tracking._model_registry.utils._get_active_spark_session"
...
def test_registry_uri_from_spark_conf(
    ...
    assert MLForge.get_registry_uri() == "http://custom.uri"
    # The MLForge_REGISTRY_URI environment variable should still take precedence over the
    # spark conf if present
    monkeypatch.setenv(MLForge_REGISTRY_URI.name, "something-else")
    assert MLForge.get_registry_uri() == "something-else"
...
def test_set_experiment_thread_safety(tmp_path):
    ...
    origin_create_experiment = MLForgeClient.create_experiment

    def patched_create_experiment(self, *args, **kwargs):
        # The sleep is for triggering `MLForge.set_experiment`
        # multiple thread / process execution race condition stably.
        time.sleep(0.1)
        return origin_create_experiment(self, *args, **kwargs)

    with mock.patch(
        "MLForge.tracking.client.MLForgeClient.create_experiment", patched_create_experiment
    ):
        created_exp_ids = []

        def thread_target():
            exp = MLForge.set_experiment("test_experiment")
        ...
        if os.name == "posix":
            ...
            def subprocess_target(que):
                exp = MLForge.set_experiment("test_experiment2")
...
def test_subprocess_inherit_active_experiment(tmp_path):
    ...
    stdout = subprocess.check_output(
        [
            sys.executable,
            "-c",
            "import MLForge; print(MLForge.tracking.fluent._get_experiment_id())",
        ],
        text=True,
    )
...
def test_MLForge_active_run_thread_local(tmp_path):
    sqlite_uri = "sqlite:///{}".format(tmp_path.joinpath("test.db"))
    MLForge.set_tracking_uri(sqlite_uri)
    ...
    with MLForge.start_run():
        ...
        def thread_target():
            ...
            thread_active_run = MLForge.active_run()
        ...
        if os.name == "posix":
            ...
            def subprocess_target():
                ...
                assert MLForge.active_run() is None
...
def test_MLForge_last_active_run_thread_local(tmp_path):
    sqlite_uri = "sqlite:///{}".format(tmp_path.joinpath("test.db"))
    MLForge.set_tracking_uri(sqlite_uri)
    ...
    assert MLForge.last_active_run().info.run_id == run.info.run_id

    thread_last_active_run = None

    def thread_target():
        nonlocal thread_last_active_run
        thread_last_active_run = MLForge.last_active_run()
    ...
    if os.name == "posix":
        ...
        def subprocess_target():
            # assert in subprocess, active run is None.
            assert MLForge.last_active_run() is None
...
def test_subprocess_inherit_tracking_uri(tmp_path):
    ...
    stdout = subprocess.check_output(
        [
            sys.executable,
            "-c",
            "import MLForge; print(MLForge.get_tracking_uri())",
        ],
        text=True,
    )
...
def test_subprocess_inherit_registry_uri(tmp_path):
    sqlite_uri = "sqlite:///{}".format(tmp_path.joinpath("test.db"))
    MLForge.set_registry_uri(sqlite_uri)

    stdout = subprocess.check_output(
        [
            sys.executable,
            "-c",
            "import MLForge; print(MLForge.get_registry_uri())",
        ],
        text=True,
    )
    assert sqlite_uri in stdout
...
def test_end_run_inside_start_run_context_manager():
    ...
    with MLForge.start_run() as parent_run:
        with MLForge.start_run(nested=True) as child_run:
            MLForge.end_run("FAILED")
...
def test_runs_are_ended_by_run_id():
    with MLForge.start_run() as run:
        ...
        MLForge.end_run()
        MLForge.start_run(run_id=run.info.run_id)
...
def test_initialize_logged_model_active_run():
    with MLForge.start_run() as run:
        model = MLForge.initialize_logged_model()
        assert model.source_run_id == run.info.run_id
        assert model.experiment_id == run.info.experiment_id

    exp_id = MLForge.create_experiment("exp")
    with MLForge.start_run(experiment_id=exp_id) as run:
        model = MLForge.initialize_logged_model()
        assert model.source_run_id == run.info.run_id
        assert model.experiment_id == run.info.experiment_id
...
def test_initialize_logged_model_tags_from_context():
    expected_tags = {
        MLForge_tags.MLForge_SOURCE_NAME: "source_name",
        MLForge_tags.MLForge_SOURCE_TYPE: SourceType.to_string(SourceType.NOTEBOOK),
        MLForge_tags.MLForge_GIT_COMMIT: "1234",
    }

    with (
        mock.patch(
            "MLForge.tracking.context.default_context._get_source_name",
            return_value=expected_tags[MLForge_tags.MLForge_SOURCE_NAME],
        ) as m_get_source_name,
        mock.patch(
            "MLForge.tracking.context.default_context._get_source_type",
            return_value=SourceType.from_string(expected_tags[MLForge_tags.MLForge_SOURCE_TYPE]),
        ) as m_get_source_type,
        mock.patch(
            "MLForge.tracking.context.git_context._resolve_git_info",
            return_value={
                MLForge_tags.MLForge_GIT_COMMIT: expected_tags[MLForge_tags.MLForge_GIT_COMMIT]
            },
...
def test_log_model_params():
    ...
    MLForge.log_model_params(large_params, model_id=model.model_id)

    logged_model = MLForge.get_logged_model(model.model_id)
    for key, value in large_params.items():
        assert logged_model.params.get(key) == value


def test_log_model_params_active_model():
    model = MLForge.create_external_model()
    with MLForge.set_active_model(model_id=model.model_id):
        large_params = {f"param_{i}": f"value_{i}" for i in range(150)}
        MLForge.log_model_params(large_params)
        logged_model = MLForge.get_logged_model(model.model_id)
...
def test_finalized_logged_model():
    model = MLForge.initialize_logged_model()
    finalized_model = MLForge.finalize_logged_model(
        model_id=model.model_id, status=LoggedModelStatus.READY
    )
    assert finalized_model.status == LoggedModelStatus.READY

    finalized_model = MLForge.finalize_logged_model(model_id=model.model_id, status="READY")
    assert finalized_model.status == LoggedModelStatus.READY
...
def test_create_external_model(tmp_path):
    ...
    assert model.tags.get(MLForge_tags.MLForge_MODEL_IS_EXTERNAL) == "true"

    # Verify that an MLmodel file is created with metadata indicating that the model's artifacts
    # are stored externally
    MLForge.artifacts.download_artifacts(f"models:/{model.model_id}", dst_path=tmp_path)
    MLForge_model: Model = Model.load(os.path.join(tmp_path, MLMODEL_FILE_NAME))
    assert MLForge_model.metadata is not None
    assert MLForge_model.metadata.get(MLForge_tags.MLForge_MODEL_IS_EXTERNAL) is True

    exp_id = MLForge.create_experiment("test")
    with MLForge.start_run(experiment_id=exp_id) as run:
        pass
    with mock.patch("MLForge.tracking.fluent._get_experiment_id", return_value=None) as m:
        model = MLForge.create_external_model(source_run_id=run.info.run_id)
...
def test_last_logged_model():
    _reset_last_logged_model_id()
    assert MLForge.last_logged_model() is None

    model = MLForge.initialize_logged_model()
    assert MLForge.last_logged_model().model_id == model.model_id

    client = MLForgeClient()
    client.set_logged_model_tags(model.model_id, {"tag": "value"})
    assert MLForge.last_logged_model().tags.get("tag") == "value"

    client.delete_logged_model_tag(model.model_id, "tag")
    assert "tag" not in MLForge.last_logged_model().tags

    external_model = MLForge.create_external_model()
    assert MLForge.last_logged_model().model_id == external_model.model_id

    another_model = MLForge.initialize_logged_model()
    assert MLForge.last_logged_model().model_id == another_model.model_id
    ...
    t = threading.Thread(
        ...
        daemon=True,
        target=lambda: MLForge.initialize_logged_model(),
    )
    t.start()
    t.join()
    assert MLForge.last_logged_model().model_id == another_model.model_id
...
def test_last_logged_model_log_model():
    ...
    model = MLForge.pyfunc.log_model(name="model", python_model=Model())
    assert MLForge.last_logged_model().model_id == model.model_id
...
def test_last_logged_model_autolog():
    try:
        ...
        MLForge.sklearn.autolog(log_models=True)
...
def test_set_and_delete_model_tag():
    ...
    MLForge.set_logged_model_tags(model.model_id, {"tag": "value"})
    assert MLForge.last_logged_model().tags.get("tag") == "value"

    MLForge.delete_logged_model_tag(model.model_id, "tag")
    assert "tag" not in MLForge.last_logged_model().tags


def test_search_logged_models():
    with mock.patch("MLForge.tracking.fluent.MLForgeClient") as MockClient:
...
def test_search_logged_models_pagination():
    with mock.patch("MLForge.tracking.fluent.MLForgeClient") as MockClient:
        ...
        experiment_ids = ["123"]
        result = MLForge.search_logged_models(experiment_ids=experiment_ids, output_format="list")
...
def test_search_logged_models_max_results():
    with mock.patch("MLForge.tracking.fluent.MLForgeClient") as MockClient:
        ...
        experiment_ids = ["123"]
        max_results = 1
        result = MLForge.search_logged_models(
...
def test_set_active_model():
    assert MLForge.get_active_model_id() is None

    model = MLForge.create_external_model(name="test_model")

    set_active_model(name=model.name)
    assert MLForge.get_active_model_id() == model.model_id

    set_active_model(model_id=model.model_id)
    assert MLForge.get_active_model_id() == model.model_id

    model2 = MLForge.create_external_model(name="test_model")
    set_active_model(name="test_model")
    assert MLForge.get_active_model_id() == model2.model_id

    set_active_model(name="new_model")
    logged_model = MLForge.search_logged_models(
        filter_string="name='new_model'", output_format="list"
    )[0]
    assert logged_model.name == "new_model"
    assert MLForge.get_active_model_id() == logged_model.model_id
...
def test_set_active_model_error():
    with pytest.raises(MLForgeException, match=r"Either name or model_id must be provided"):
        set_active_model()

    model = MLForge.create_external_model(name="test_model")
    with pytest.raises(MLForgeException, match=r"does not match the provided name"):
        set_active_model(name="abc", model_id=model.model_id)

    with pytest.raises(MLForgeException, match=r"Logged model with ID '1234' not found"):
        set_active_model(model_id="1234")


def test_set_active_model_env_var(monkeypatch):
    monkeypatch.setenv(_MLForge_ACTIVE_MODEL_ID.name, "1234")
    # mimic the behavior when MLForge is imported
    _ACTIVE_MODEL_CONTEXT.set(ActiveModelContext())
    assert MLForge.get_active_model_id() == "1234"

    monkeypatch.delenv(_MLForge_ACTIVE_MODEL_ID.name)
    _ACTIVE_MODEL_CONTEXT.set(ActiveModelContext())

    assert MLForge.get_active_model_id() is None
    assert _MLForge_ACTIVE_MODEL_ID.get() is None
...
def test_set_active_model_public_env_var(monkeypatch, is_in_databricks_serving):
    with mock.patch(
        "MLForge.tracking.fluent.is_in_databricks_model_serving_environment",
        return_value=is_in_databricks_serving,
    ) as mock_is_in_databricks:
        assert MLForge.get_active_model_id() is None
        assert _get_active_model_id_global() is None
        assert MLForge_ACTIVE_MODEL_ID.get() is None

        monkeypatch.setenv(MLForge_ACTIVE_MODEL_ID.name, "public-model-id")
        # mimic the behavior when MLForge is imported
        _ACTIVE_MODEL_CONTEXT.set(ActiveModelContext())
        assert MLForge.get_active_model_id() == "public-model-id"
        assert _get_active_model_id_global() == "public-model-id"

        # In Databricks Model Serving, the active model ID is stored in the
        # _MLForge_ACTIVE_MODEL environment variable. Deleting the MLForge_ACTIVE_MODEL
...
def test_set_active_model_env_var_precedence(monkeypatch):
    ...
    monkeypatch.setenv(_MLForge_ACTIVE_MODEL_ID.name, "legacy-model-id")
    monkeypatch.setenv(MLForge_ACTIVE_MODEL_ID.name, "public-model-id")
    ...
    monkeypatch.delenv(MLForge_ACTIVE_MODEL_ID.name)
    _ACTIVE_MODEL_CONTEXT.set(ActiveModelContext())
    assert MLForge.get_active_model_id() == "legacy-model-id"
...
def test_clear_active_model_clears_env_vars(monkeypatch):
    ...
    # mimic the behavior when MLForge is imported - should pick up public variable
    _ACTIVE_MODEL_CONTEXT.set(ActiveModelContext())
    assert MLForge.get_active_model_id() == "public-model-id"

    # Clear the active model - should disregard environment variables
    MLForge.clear_active_model()
    assert MLForge.get_active_model_id() is None
...
def test_set_active_model_link_traces():
    set_active_model(name="test_model")
    model_id = MLForge.get_active_model_id()
...
def test_set_active_model_in_databricks_serving():
    with mock.patch(
        ...
        assert MLForge.get_active_model_id() == model.model_id
        assert _MLForge_ACTIVE_MODEL_ID.get() == model.model_id

        with set_active_model(name="new_model") as new_model:
            assert MLForge.get_active_model_id() == new_model.model_id
            assert _MLForge_ACTIVE_MODEL_ID.get() == new_model.model_id
...
def test_get_active_model_id_global():
    ...
    with mock.patch("MLForge.tracking.fluent._logger.debug") as mock_debug:
        assert _get_active_model_id_global() is None
        assert any(
...
def test_active_model_set_in_threads_can_be_fetched_from_main_process(monkeypatch):
    ...
    def _load_model(model_uri):
        pyfunc_model = MLForge.pyfunc.load_model(model_uri)
        _update_active_model_id_based_on_MLForge_model(pyfunc_model._model_meta)
        return pyfunc_model
...
def test_log_metric_link_to_active_model():
    ...
    set_active_model(name=model.name)
    with MLForge.start_run():
        MLForge.log_metric("metric", 1)
    logged_model = MLForge.get_logged_model(model_id=model.model_id)
...
def test_log_metrics_link_to_active_model():
    ...
    set_active_model(name=model.name)
    with MLForge.start_run():
        MLForge.log_metrics({"metric1": 1, "metric2": 2})
    logged_model = MLForge.get_logged_model(model_id=model.model_id)
    assert logged_model.name == model.name
    assert logged_model.model_id == model.model_id
...
def test_clear_active_model():
    ...
    loaded_model = MLForge.pyfunc.load_model(model_info.model_uri)
...
def test_set_logged_model_tags_error():
    with pytest.raises(MLForgeException, match="You may not have access to the logged model"):
        MLForge.set_logged_model_tags("non-existing-model-id", {"tag": "value"})


def test_log_metrics_not_fetching_run_if_active():
    with MLForge.start_run():
        with mock.patch("MLForge.tracking.fluent.MLForgeClient.get_run") as mock_client_get_run:
            MLForge.log_metrics({"metric": 1})
            mock_client_get_run.assert_not_called()


def test_log_metrics_with_active_model_log_model_once():
    MLForge.set_active_model(name="test_model")
    with MLForge.start_run():
        with (
            mock.patch("MLForge.tracking.fluent.MLForgeClient.get_run") as mock_client_get_run,
            mock.patch("MLForge.tracking.fluent.MLForgeClient.log_inputs") as mock_client_log_inputs,
        ):
            MLForge.log_metrics({"metric": 1})
            MLForge.log_metrics({"metric": 2})
            mock_client_get_run.assert_not_called()
            mock_client_log_inputs.assert_called_once()


def test_log_metric_with_dataset_entity():
    """Test that log_metric works with both MLForge.entities.Dataset and MLForge.data.dataset.Dataset.

    Regression test for issue https://github.com/MLForge/MLForge/issues/18573.
    """
    # Test with MLForge.entities.Dataset (retrieved from run.inputs)
    with MLForge.start_run() as run:
        dataset_source = HTTPDatasetSource(url="some_uri")
        dataset = MetaDataset(source=dataset_source, name="my_dataset", digest="12345678")
        MLForge.log_input(dataset, context="eval")

        run_data = MLForge.get_run(run.info.run_id)
        dataset_entity = run_data.inputs.dataset_inputs[0].dataset

        MLForge.log_metric("accuracy", 0.95, dataset=dataset_entity)

        run_data = MLForge.get_run(run.info.run_id)
        assert "accuracy" in run_data.data.metrics
        assert run_data.data.metrics["accuracy"] == 0.95

    # Test with MLForge.data.dataset.Dataset (backward compatibility)
    with MLForge.start_run() as run:
        ...
        MLForge.log_metric("precision", 0.92, dataset=dataset)
...
def test_get_sgc_MLForge_run_id_for_resumption_with_tag(empty_active_run_stack):
    # Create an experiment with a tag
    experiment_id = MLForge.create_experiment("test_sgc_experiment")
    client = MLForgeClient()
    ...
    sgc_tag_key = f"{MLForge_tags.MLForge_DATABRICKS_SGC_RESUME_RUN_JOB_RUN_ID_PREFIX}.12345"
    client.set_experiment_tag(experiment_id, sgc_tag_key, run_id)

    # Test retrieval
    retrieved_run_id = _get_sgc_MLForge_run_id_for_resumption(client, experiment_id, sgc_tag_key)
    assert retrieved_run_id == run_id


def test_get_sgc_MLForge_run_id_for_resumption_without_tag(empty_active_run_stack):
    experiment_id = MLForge.create_experiment("test_sgc_no_tag")
    client = MLForgeClient()

    sgc_tag_key = f"{MLForge_tags.MLForge_DATABRICKS_SGC_RESUME_RUN_JOB_RUN_ID_PREFIX}.nonexistent"
...
def test_get_sgc_MLForge_run_id_for_resumption_with_default_experiment(empty_active_run_stack):
    ...
    sgc_tag_key = f"{MLForge_tags.MLForge_DATABRICKS_SGC_RESUME_RUN_JOB_RUN_ID_PREFIX}.default"
    client.set_experiment_tag(default_exp_id, sgc_tag_key, run_id)

    # Test retrieval with None experiment_id
    retrieved_run_id = _get_sgc_MLForge_run_id_for_resumption(client, None, sgc_tag_key)
    assert retrieved_run_id == run_id


def test_get_sgc_MLForge_run_id_for_resumption_handles_exception():
    client = MLForgeClient()

    # Test with non-existent experiment ID
    sgc_tag_key = f"{MLForge_tags.MLForge_DATABRICKS_SGC_RESUME_RUN_JOB_RUN_ID_PREFIX}.error"
    retrieved_run_id = _get_sgc_MLForge_run_id_for_resumption(
...
def test_start_run_sgc_resumption_creates_tag(empty_active_run_stack, monkeypatch):
    experiment_id = MLForge.create_experiment("test_sgc_create_tag")
    sgc_job_run_id = "12345"

    # Mock get_sgc_job_run_id to return a job run ID
    with mock.patch(
        "MLForge.tracking.fluent.get_sgc_job_run_id", return_value=sgc_job_run_id
    ) as mock_get_sgc:
        ...
        expected_tag_key = (
            f"{MLForge_tags.MLForge_DATABRICKS_SGC_RESUME_RUN_JOB_RUN_ID_PREFIX}.{sgc_job_run_id}"
        )
...
def test_start_run_sgc_resumption_resumes_run(empty_active_run_stack, monkeypatch):
    experiment_id = MLForge.create_experiment("test_sgc_resume")
    client = MLForgeClient()
    ...
    with mock.patch(
        ...
        with MLForge.start_run(experiment_id=experiment_id) as first_run:
            first_run_id = first_run.info.run_id
            MLForge.log_param("initial_param", "value1")
    ...
    with mock.patch(
        ...
        with MLForge.start_run(experiment_id=experiment_id) as resumed_run:
            resumed_run_id = resumed_run.info.run_id
            MLForge.log_param("resumed_param", "value2")
        mock_get_sgc.assert_called()
...
def test_start_run_sgc_resumption_disabled(empty_active_run_stack, monkeypatch):
    experiment_id = MLForge.create_experiment("test_sgc_disabled")
    sgc_job_run_id = "11111"

    # Disable SGC resumption feature
    monkeypatch.setenv(_MLForge_ENABLE_SGC_RUN_RESUMPTION_FOR_DATABRICKS_JOBS.name, "false")

    # Mock get_sgc_job_run_id (but won't be used since feature is disabled)
    with mock.patch("MLForge.tracking.fluent.get_sgc_job_run_id", return_value=sgc_job_run_id):
        # Create first run
        with MLForge.start_run(experiment_id=experiment_id) as first_run:
            first_run_id = first_run.info.run_id

        # Create second run - should be a new run since feature is disabled
        with MLForge.start_run(experiment_id=experiment_id) as second_run:
...
def test_start_run_sgc_resumption_no_job_run_id(empty_active_run_stack, monkeypatch):
    experiment_id = MLForge.create_experiment("test_sgc_no_job_id")

    # Mock get_sgc_job_run_id to return None
    with mock.patch("MLForge.tracking.fluent.get_sgc_job_run_id", return_value=None) as mock_get_sgc:
        with MLForge.start_run(experiment_id=experiment_id):
            pass
...
def test_start_run_sgc_resumption_explicit_run_id_takes_precedence(empty_active_run_stack):
    experiment_id = MLForge.create_experiment("test_sgc_precedence")
    client = MLForgeClient()
    ...
    with MLForge.start_run(run_id=run1_id, experiment_id=experiment_id) as resumed_run:
...
def test_start_run_sgc_resumption_handles_tag_set_error(empty_active_run_stack, monkeypatch):
    experiment_id = MLForge.create_experiment("test_sgc_tag_error")
    ...
    with (
        ...
        mock.patch.object(
            MLForgeClient, "set_experiment_tag", side_effect=Exception("Tag error")
        ) as mock_set_tag,
    ):
...
def test_import_checkpoints_overwrite():
    exp_id = MLForge.create_experiment("test_import_checkpoints_overwrite")
    MLForge.set_experiment(experiment_id=exp_id)
    ...
    with mock.patch("databricks.sdk.WorkspaceClient", return_value=ws):
        with MLForge.start_run() as run:
            logged_models = MLForge.import_checkpoints(
                "/Volumes/checkpoints",
            ...
            searched_models = MLForge.search_logged_models(
            ...
            logged_models2 = MLForge.import_checkpoints(
                "/Volumes/checkpoints",
                model_prefix="model1_",
            ...
            searched_models2 = MLForge.search_logged_models(
                experiment_ids=[exp_id],
                filter_string=f"model_id IN ('{ckpt1_model_id}', '{ckpt2_model_id}')",
                output_format="list",
            )
            ...
            overwritten_logged_models = MLForge.import_checkpoints(
...
def test_import_checkpoints_skip_name_with_invalid_char():
    exp_id = MLForge.create_experiment("test_import_checkpoints_skip_name_with_invalid_char")
    MLForge.set_experiment(experiment_id=exp_id)
    ...
    with (
        ...
        mock.patch("MLForge.tracking.fluent._logger.warning") as mock_warning,
    ):
        with MLForge.start_run():
            logged_models = MLForge.import_checkpoints(
                "/Volumes/checkpoints",
...
def test_import_checkpoints_without_run():
    exp_id = MLForge.create_experiment("test_import_checkpoints_without_run")
    MLForge.set_experiment(experiment_id=exp_id)
    ...
    with mock.patch("databricks.sdk.WorkspaceClient", return_value=ws):
        with pytest.raises(
            ...
            MLForge.import_checkpoints("/Volumes/checkpoints")

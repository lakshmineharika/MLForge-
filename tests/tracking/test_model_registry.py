...
def test_update_registered_model_flow(client):
    ...
    with pytest.raises(MLForgeException, match="Registered Model with name=UpdateRMTest not found"):
    ...
    for old_name in [previous_name, name, new_name]:
        with pytest.raises(
            MLForgeException, match=r"Registered Model with name=UpdateRMTest( \d)? not found"
        ):
...
def test_delete_registered_model_flow(client):
    ...
    with pytest.raises(MLForgeException, match=r"Registered Model .+ already exists"):
        client.create_registered_model(name)

    client.delete_registered_model(name)

    # cannot get a deleted model
    with pytest.raises(MLForgeException, match=r"Registered Model .+ not found"):
        client.get_registered_model(name)
...
def test_delete_model_version_flow(client):
    ...
    with pytest.raises(MLForgeException, match=r"Model Version .+ not found"):
        client.delete_model_version(name, "1")

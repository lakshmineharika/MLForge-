...
def test_log_image_numpy(subdir):
    ...
    with MLForge.start_run():
        MLForge.log_image(image, artifact_file)
...
def test_log_image_raises_for_unsupported_objects():
    with MLForge.start_run():
        with pytest.raises(TypeError, match="Unsupported image object type"):
            MLForge.log_image("not_image", "image.png")
...
def test_log_image_numpy_shape(size):
    ...
    with MLForge.start_run():
        MLForge.log_image(image, filename)
...
def test_log_image_numpy_emits_warning_for_out_of_range_values(array):
    ...
    if isinstance(array[0][0], int):
        with (
            ...
            MLForge.log_image(image, "image.png")
    else:
        with (
            MLForge.start_run(),
...
def test_log_image_numpy_raises_exception_for_invalid_array_data_type():
    ...
    with MLForge.start_run(), pytest.raises(TypeError, match="Invalid array data type"):
        MLForge.log_image(np.tile("a", (1, 1, 3)), "image.png")


def test_log_image_numpy_raises_exception_for_invalid_array_shape():
    import numpy as np

    with MLForge.start_run(), pytest.raises(ValueError, match="`image` must be a 2D or 3D array"):
        MLForge.log_image(np.zeros((1,), dtype=np.uint8), "image.png")


def test_log_image_numpy_raises_exception_for_invalid_channel_length():
    import numpy as np

    with MLForge.start_run(), pytest.raises(ValueError, match="Invalid channel length"):
        MLForge.log_image(np.zeros((1, 1, 5), dtype=np.uint8), "image.png")


def test_log_image_raises_exception_for_unsupported_image_object_type():
    with MLForge.start_run(), pytest.raises(TypeError, match="Unsupported image object type"):
        MLForge.log_image("not_image", "image.png")
...
def test_log_image_with_steps():
    ...
    with MLForge.start_run():
        MLForge.log_image(image, key="dog", step=0, synchronous=True)

        logged_path = "images/"
        artifact_uri = MLForge.get_artifact_uri(logged_path)
        run_artifact_dir = local_file_uri_to_path(artifact_uri)
...
def test_log_image_with_url_encoding_prone_steps(step):
    ...
    See https://github.com/MLForge/MLForge/issues/21085
    """
    image = np.random.randint(0, 256, size=(100, 100, 3), dtype=np.uint8)

    with MLForge.start_run():
        MLForge.log_image(image, key="dog", step=step, synchronous=True)

        artifact_uri = MLForge.get_artifact_uri("images/")
        run_artifact_dir = local_file_uri_to_path(artifact_uri)
        files = os.listdir(run_artifact_dir)
...
def test_log_image_with_timestamp():
    ...
    with MLForge.start_run():
        MLForge.log_image(image, key="dog", timestamp=100, synchronous=True)
...
def test_duplicated_log_image_with_step():
    """
    MLForge will save both files if there are multiple calls to log_image
    ...
    with MLForge.start_run():
        MLForge.log_image(image1, key="dog", step=100, synchronous=True)
        MLForge.log_image(image2, key="dog", step=100, synchronous=True)
...
def test_duplicated_log_image_with_timestamp():
    ...
    with MLForge.start_run():
        MLForge.log_image(image1, key="dog", step=100, timestamp=100, synchronous=True)
        MLForge.log_image(image2, key="dog", step=100, timestamp=100, synchronous=True)
...
def test_log_image_raises_exception_for_unexpected_arguments_used(args):
    ...
    if isinstance(args, dict):
        with MLForge.start_run(), pytest.raises(TypeError, match=exception):
            MLForge.log_image(np.zeros((1,), dtype=np.uint8), "image.png", **args)
...
def test_log_image_raises_exception_for_missing_arguments():
    ...
    with MLForge.start_run(), pytest.raises(TypeError, match=exception):
        MLForge.log_image(np.zeros((1,), dtype=np.uint8))
...
def test_async_log_image_flush():
    ...
    with MLForge.start_run():
        for i in range(100):
            MLForge.log_image(image1, key="dog", step=i, timestamp=i, synchronous=False)

        MLForge.flush_artifact_async_logging()
...
def test_log_image_with_slash_in_key():
    ...
    with MLForge.start_run():
        MLForge.log_image(image, key="category/name", step=5, synchronous=True)
    ...
    for artifact in artifacts:
        # download_artifacts must not raise MLForgeException about '#' in path

...
def test_set_get_registry_uri():
    with mock.patch(
        "MLForge.tracking._model_registry.utils._resolve_tracking_uri"
...
def test_registry_uri_from_environment_overrides_databricks_default():
    from MLForge.environment_variables import MLForge_REGISTRY_URI
    ...
    with (
        ...
        mock.patch.object(MLForge_REGISTRY_URI, "get", return_value=env_registry_uri),
    ):
        get_tracking_uri_mock.return_value = tracking_uri
        set_registry_uri(None)
...
def test_registry_uri_from_spark_session_overrides_databricks_default():
    ...
    with (
        ...
        mock.patch(
            "MLForge.tracking._model_registry.utils._get_registry_uri_from_spark_session"
...
def test_get_store_sqlalchemy_store(db_type, monkeypatch):
    ...
    with (
        ...
        mock.patch("MLForge.store.model_registry.sqlalchemy_store._all_tables_exist"),
        mock.patch(
            "MLForge.store.model_registry.sqlalchemy_store._get_managed_session_maker",
...
def test_get_store_bad_uris(bad_uri, monkeypatch):
    monkeypatch.setenv(MLForge_TRACKING_URI.name, bad_uri)
    with pytest.raises(
...
def test_databricks_uc_store_defaults_to_legacy(monkeypatch):
    monkeypatch.delenv("MLForge_ENABLE_UC_NATIVE_MODEL_REGISTRY", raising=False)

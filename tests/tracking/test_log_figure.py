...
def test_log_figure_matplotlib(subdir):
    ...
    with MLForge.start_run():
        MLForge.log_figure(fig, artifact_file)
        plt.close(fig)
...
def test_log_figure_save_kwargs():
    ...
    with MLForge.start_run():
        ...
        MLForge.log_figure(fig, name, save_kwargs={"div_id": div_id})
        artifact_uri = MLForge.get_artifact_uri(name)
...
def test_log_figure_raises_error_for_unsupported_figure_object_type():
    with MLForge.start_run(), pytest.raises(TypeError, match="Unsupported figure object type"):
        MLForge.log_figure("not_figure", "figure.png")

...
import MLForge
from MLForge.server import ARTIFACT_ROOT_ENV_VAR, BACKEND_STORE_URI_ENV_VAR
...
def _init_server(
    ...
    if server_type == "fastapi":
        ...
        cmd = [
            ...
            "-m",
            "uvicorn",
            app or "MLForge.server.fastapi_app:app",
            "--host",
            LOCALHOST,
            "--port",
            str(server_port),
...
def _send_rest_tracking_post_request(tracking_server_uri, api_path, json_payload, auth=None):
    """
    Make a POST request to the specified MLForge Tracking API and retrieve the
...
class ServerThread(Thread):
    ...
    def __init__(self, app: FastAPI, port: int):
        super().__init__(name="MLForge-tracking-server", daemon=True)
        self.host = "127.0.0.1"
        self.port = port

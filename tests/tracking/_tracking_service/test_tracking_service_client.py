...
from MLForge.entities import Metric, Param, Run, RunInfo, RunTag
...
def test_get_artifact_repo(artifact_uri, databricks_uri, uri_for_repo):
    with (
        mock.patch(
            "MLForge.tracking._tracking_service.client.TrackingServiceClient.get_run",
            return_value=Run(
                RunInfo(
                    "uuid",
                    "expr_id",
        ...
        mock.patch(
            "MLForge.tracking._tracking_service.client.get_artifact_repository", return_value=None
        ) as get_repo_mock,
    ):
...
def test_log_batch(tracking_client_log_batch):
    ...
    expected_tags["MLForge.runName"] = run_data.tags["MLForge.runName"]
...
def test_log_batch_with_empty_data(tracking_client_log_batch):
    ...
    assert run_data.tags == {"MLForge.runName": run_data.tags["MLForge.runName"]}
...
def test_link_traces_to_run_validation():
    ...
    with pytest.raises(MLForgeException, match="run_id cannot be empty"):

...
from MLForge.environment_variables import (
    ...
    MLForge_TRACKING_INSECURE_TLS,
    MLForge_TRACKING_PASSWORD,
    MLForge_TRACKING_TOKEN,
    MLForge_TRACKING_URI,
    MLForge_TRACKING_USERNAME,
)
from MLForge.exceptions import MLForgeException
from MLForge.server import ARTIFACT_ROOT_ENV_VAR
from MLForge.store.db.db_types import DATABASE_ENGINES
...
from MLForge.tracking._tracking_service.registry import TrackingStoreRegistry
from MLForge.tracking._tracking_service.utils import (
    _get_store,
...
from MLForge.tracking.registry import UnsupportedModelRegistryStoreURIException
from MLForge.utils.file_utils import path_to_local_file_uri
...
# and https://github.com/MLForge/MLForge/blob/master/CONTRIBUTING.md#writing-python-tests
...
def test_get_store_file_store_from_env(tmp_path, monkeypatch, uri):
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv(MLForge_TRACKING_URI.name, uri)
    store = _get_store()
    assert isinstance(store, FileStore)
    assert os.path.abspath(store.root_directory) == os.path.abspath("other/path")
...
def test_get_store_basic_rest_store(monkeypatch):
    monkeypatch.setenv(MLForge_TRACKING_URI.name, "https://my-tracking-server:5050")
    store = _get_store()
...
def test_get_store_rest_store_with_password(monkeypatch):
    for k, v in {
        MLForge_TRACKING_URI.name: "https://my-tracking-server:5050",
        MLForge_TRACKING_USERNAME.name: "Bob",
        MLForge_TRACKING_PASSWORD.name: "Ross",
    }.items():
...
def test_get_store_rest_store_with_token(monkeypatch):
    for k, v in {
        MLForge_TRACKING_URI.name: "https://my-tracking-server:5050",
        MLForge_TRACKING_TOKEN.name: "my-token",
    }.items():
        monkeypatch.setenv(k, v)
...
def test_get_store_rest_store_with_insecure(monkeypatch):
    for k, v in {
        MLForge_TRACKING_URI.name: "https://my-tracking-server:5050",
        MLForge_TRACKING_INSECURE_TLS.name: "true",
    }.items():
        monkeypatch.setenv(k, v)
    store = _get_store()
    assert isinstance(store, RestStore)
...
def test_get_store_rest_store_with_no_insecure(monkeypatch):
    with monkeypatch.context() as m:
        for k, v in {
            MLForge_TRACKING_URI.name: "https://my-tracking-server:5050",
            MLForge_TRACKING_INSECURE_TLS.name: "false",
        }.items():
            m.setenv(k, v)
        store = _get_store()
        assert isinstance(store, RestStore)
...
def test_get_store_sqlalchemy_store(tmp_path, monkeypatch, db_type):
    ...
    monkeypatch.setenv(MLForge_TRACKING_URI.name, uri)
    monkeypatch.delenv("MLForge_SQLALCHEMYSTORE_POOLCLASS", raising=False)
    with (
        mock.patch("sqlalchemy.create_engine") as mock_create_engine,
        mock.patch("sqlalchemy.event.listens_for"),
        mock.patch("MLForge.store.db.utils._verify_schema"),
        mock.patch("MLForge.store.db.utils._initialize_tables"),
        mock.patch(
            "MLForge.store.db.utils._get_managed_session_maker",
        ...
        mock.patch(
            ...
            "MLForge.store.tracking.sqlalchemy_store.SqlAlchemyStore.search_experiments",
            return_value=[],
        ),
        mock.patch(
            "MLForge.store.tracking.sqlalchemy_store.SqlAlchemyStore._initialize_store_state",
            return_value=None,
        ),
    ):
        store = _get_store()
...
def test_get_sqlalchemy_store_uses_server_artifact_root(tmp_path, monkeypatch):
    ...
    with mock.patch("MLForge.store.tracking.sqlalchemy_store.SqlAlchemyStore") as mock_store:
        MLForge.tracking._tracking_service.utils._get_sqlalchemy_store(
...
def test_get_store_databricks(monkeypatch):
    for k, v in {
        MLForge_TRACKING_URI.name: "databricks",
...
def test_get_store_databricks_profile(monkeypatch):
    monkeypatch.setenv(MLForge_TRACKING_URI.name, "databricks://mycoolprofile")
    ...
    store = _get_store()
    assert isinstance(store, DatabricksTracingRestStore)
    with pytest.raises(MLForgeException, match="mycoolprofile"):
        store.get_host_creds()


def test_get_store_caches_on_store_uri_and_artifact_uri(tmp_path):
    registry = MLForge.tracking._tracking_service.utils._tracking_store_registry
...
def test_standard_store_registry_with_mocked_entrypoint():
    ...
    with mock.patch("MLForge.utils.plugins._get_entry_points", return_value=[mock_entrypoint]):
        ...
        reload(MLForge.tracking._tracking_service.utils)
        ...
        assert expected_standard_registry.issubset(
            MLForge.tracking._tracking_service.utils._tracking_store_registry._registry.keys()
        )
...
def test_standard_store_registry_with_installed_plugin(tmp_path, monkeypatch):
    ...
    reload(MLForge.tracking._tracking_service.utils)
    assert (
        "file-plugin" in MLForge.tracking._tracking_service.utils._tracking_store_registry._registry
    )

    from MLForge_test_plugin.file_store import PluginFileStore

    monkeypatch.setenv(MLForge_TRACKING_URI.name, "file-plugin:test-path")
    plugin_file_store = MLForge.tracking._tracking_service.utils._get_store()
...
def test_plugin_registration_via_entrypoints():
    ...
    with mock.patch(
        "MLForge.utils.plugins._get_entry_points", return_value=[mock_entrypoint]
    ...
    mock_get_group_all.assert_called_once_with("MLForge.tracking_store")
...
def test_store_object_can_be_serialized_by_pickle(tmp_path):
    ...
    to prevent issues such as https://github.com/MLForge/MLForge/issues/2954
    """
    pickle.dump(_get_store(f"file:///{tmp_path.joinpath('MLForge')}"), io.BytesIO())
    pickle.dump(_get_store("databricks"), io.BytesIO())
    pickle.dump(_get_store("https://example.com"), io.BytesIO())
    # pickle.dump(_get_store(f"sqlite:///{tmpdir.strpath}/MLForge.db"), io.BytesIO())
...
def test_set_tracking_uri_with_path(tmp_path, monkeypatch, absolute):
    ...
    if absolute:
        path = tmp_path / path
    with mock.patch("MLForge.tracking._tracking_service.utils._tracking_uri", None):
...
def test_set_tracking_uri_update_trace_provider(tmp_path):
    default_uri = MLForge.get_tracking_uri()
    sqlite_uri = f"sqlite:///{tmp_path / 'MLForge.db'}"
    try:
...
def test_get_store_raises_on_uc_uri(store_uri):
    ...
    with pytest.raises(
        ...
        "supported in the current version of the MLForge client",
    ):
        MLForge.tracking.MLForgeClient()
...
def test_set_get_tracking_uri_consistency(tracking_uri):
    ...
    assert MLForge.get_tracking_uri() == tracking_uri
...
@pytest.mark.parametrize(
    ("scheme", "uri", "expected"),
    [
        ("arn", "arn:aws:sagemaker:us-east-1:123456789:MLForge-tracking-server/my-server", "aws"),
        ("arn", "arn:aws:sagemaker:eu-west-1:987654321:MLForge-tracking-server/test", "aws"),
        ("azureml", "azureml://eastus.api.azureml.ms/MLForge/v2.0/subscriptions/123", "azure"),

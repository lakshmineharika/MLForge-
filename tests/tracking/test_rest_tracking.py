...
import MLForge.experiments
import MLForge.pyfunc
import MLForge.tracing.trace_archival_config as trace_archival_config_module
from MLForge import MLForgeClient
from MLForge.artifacts import download_artifacts
from MLForge.data.pandas_dataset import from_pandas
from MLForge.entities import (
    Dataset,
    DatasetInput,
    FallbackConfig,
    FallbackStrategy,
...
from MLForge.entities.logged_model_input import LoggedModelInput
from MLForge.entities.logged_model_output import LoggedModelOutput
from MLForge.entities.logged_model_status import LoggedModelStatus
from MLForge.entities.span import SpanAttributeKey
from MLForge.entities.trace_data import TraceData
from MLForge.entities.trace_info import TraceInfo
from MLForge.entities.trace_location import TraceLocation
from MLForge.entities.trace_metrics import (
    AggregationType,
    MetricAggregation,
    MetricViewType,
)
from MLForge.entities.trace_state import TraceState
from MLForge.entities.trace_status import TraceStatus
from MLForge.environment_variables import (
    _MLForge_GO_STORE_TESTING,
    MLForge_SERVER_GRAPHQL_MAX_ALIASES,
    MLForge_SERVER_GRAPHQL_MAX_ROOT_FIELDS,
    MLForge_SUPPRESS_PRINTING_URL_TO_STDOUT,
    MLForge_TRACE_ARCHIVAL_CONFIG,
)
from MLForge.exceptions import MLForgeException, RestException
from MLForge.genai.datasets import (
    add_dataset_to_experiments,
    create_dataset,
    remove_dataset_from_experiments,
)
from MLForge.models import Model
from MLForge.protos.databricks_pb2 import RESOURCE_DOES_NOT_EXIST, ErrorCode
from MLForge.server import handlers
from MLForge.server.fastapi_app import app
from MLForge.server.handlers import initialize_backend_stores
from MLForge.store.tracking.sqlalchemy_store import SqlAlchemyStore
from MLForge.tracing.analysis import TraceFilterCorrelationResult
from MLForge.tracing.client import TracingClient
from MLForge.tracing.constant import (
    TRACE_SCHEMA_VERSION_KEY,
    TraceMetricDimensionKey,
    TraceMetricKey,
)
from MLForge.tracing.utils import build_otel_context
from MLForge.utils import MLForge_tags
from MLForge.utils.file_utils import TempDir, path_to_local_file_uri
from MLForge.utils.MLForge_tags import (
    MLForge_DATASET_CONTEXT,
    MLForge_GIT_COMMIT,
    MLForge_PARENT_RUN_ID,
    MLForge_PROJECT_ENTRY_POINT,
    MLForge_SOURCE_NAME,
    MLForge_SOURCE_TYPE,
    MLForge_USER,
)
from MLForge.utils.os import is_windows
from MLForge.utils.proto_json_utils import message_to_json
from MLForge.utils.time import get_current_time_millis
...
@pytest.fixture
def MLForge_client(store_type: str, tmp_path: Path, db_uri: str, monkeypatch):
    """Provides an MLForge Tracking API client pointed at the local tracking server."""
    # Set passphrase for secrets management (required for encryption)
    monkeypatch.setenv(
        "MLForge_CRYPTO_KEK_PASSPHRASE", "test-passphrase-at-least-32-characters-long"
    )
    monkeypatch.delenv(MLForge_TRACE_ARCHIVAL_CONFIG.name, raising=False)
    ...
    with ServerThread(app, get_safe_port()) as url:
        yield MLForgeClient(url)


@pytest.fixture
def MLForge_client_with_secrets(tmp_path: Path, monkeypatch):
    """Provides an MLForge Tracking API client with fresh database for secrets management.
    ...
    backend_uri = f"sqlite:///{tmp_path}/MLForge.db"
...
@pytest.fixture
def cli_env(MLForge_client):
    """Provides an environment for the MLForge CLI pointed at the local tracking server."""
    return {
        "LC_ALL": "en_US.UTF-8",
        "LANG": "en_US.UTF-8",
        "MLForge_TRACKING_URI": MLForge_client.tracking_uri,
    }
...
def test_create_get_search_experiment(MLForge_client):
    experiment_id = MLForge_client.create_experiment(
        "My Experiment",
        artifact_location="my_location",
        tags={"key1": "val1", "key2": "val2"},
    )
    exp = MLForge_client.get_experiment(experiment_id)
    ...
    experiments = MLForge_client.search_experiments()
    assert {e.name for e in experiments} == {"My Experiment", "Default"}
    MLForge_client.delete_experiment(experiment_id)
    assert {e.name for e in MLForge_client.search_experiments()} == {"Default"}
    assert {e.name for e in MLForge_client.search_experiments(view_type=ViewType.ACTIVE_ONLY)} == {
        "Default"
    }
    assert {e.name for e in MLForge_client.search_experiments(view_type=ViewType.DELETED_ONLY)} == {
        "My Experiment"
    }
    assert {e.name for e in MLForge_client.search_experiments(view_type=ViewType.ALL)} == {
        "My Experiment",
        "Default",
    }
    active_exps_paginated = MLForge_client.search_experiments(max_results=1)
    assert {e.name for e in active_exps_paginated} == {"Default"}
    assert active_exps_paginated.token is None

    all_exps_paginated = MLForge_client.search_experiments(max_results=1, view_type=ViewType.ALL)
    first_page_names = {e.name for e in all_exps_paginated}
    all_exps_second_page = MLForge_client.search_experiments(
...
def test_create_experiment_validation(MLForge_client):
    def assert_bad_request(payload, expected_error_message):
        response = _send_rest_tracking_post_request(
            MLForge_client.tracking_uri,
            "/api/2.0/MLForge/experiments/create",
            payload,
        )
...
def test_delete_restore_experiment(MLForge_client):
    experiment_id = MLForge_client.create_experiment("Deleterious")
    assert MLForge_client.get_experiment(experiment_id).lifecycle_stage == "active"
    MLForge_client.delete_experiment(experiment_id)
    assert MLForge_client.get_experiment(experiment_id).lifecycle_stage == "deleted"
    MLForge_client.restore_experiment(experiment_id)
    assert MLForge_client.get_experiment(experiment_id).lifecycle_stage == "active"


def test_delete_restore_experiment_cli(MLForge_client, cli_env):
    experiment_name = "DeleteriousCLI"
    invoke_cli_runner(
        MLForge.experiments.commands,
        ["create", "--experiment-name", experiment_name],
        env=cli_env,
    )
    experiment_id = MLForge_client.get_experiment_by_name(experiment_name).experiment_id
    assert MLForge_client.get_experiment(experiment_id).lifecycle_stage == "active"
    invoke_cli_runner(
        MLForge.experiments.commands, ["delete", "-x", str(experiment_id)], env=cli_env
    )
    assert MLForge_client.get_experiment(experiment_id).lifecycle_stage == "deleted"
    invoke_cli_runner(
        MLForge.experiments.commands, ["restore", "-x", str(experiment_id)], env=cli_env
    )
    assert MLForge_client.get_experiment(experiment_id).lifecycle_stage == "active"


def test_rename_experiment(MLForge_client):
    experiment_id = MLForge_client.create_experiment("BadName")
    assert MLForge_client.get_experiment(experiment_id).name == "BadName"
    MLForge_client.rename_experiment(experiment_id, "GoodName")
    assert MLForge_client.get_experiment(experiment_id).name == "GoodName"


def test_rename_experiment_cli(MLForge_client, cli_env):
    bad_experiment_name = "CLIBadName"
    good_experiment_name = "CLIGoodName"

    invoke_cli_runner(
        MLForge.experiments.commands, ["create", "-n", bad_experiment_name], env=cli_env
    )
    experiment_id = MLForge_client.get_experiment_by_name(bad_experiment_name).experiment_id
    assert MLForge_client.get_experiment(experiment_id).name == bad_experiment_name
    invoke_cli_runner(
        MLForge.experiments.commands,
        [
            "rename",
    ...
    assert MLForge_client.get_experiment(experiment_id).name == good_experiment_name


@pytest.mark.parametrize("parent_run_id_kwarg", [None, "my-parent-id"])
def test_create_run_all_args(MLForge_client, parent_run_id_kwarg):
    user = "username"
    source_name = "Hello"
    entry_point = "entry"
    source_version = "abc"
    create_run_kwargs = {
        ...
        "tags": {
            ...
            MLForge_SOURCE_NAME: source_name,
            MLForge_PROJECT_ENTRY_POINT: entry_point,
            MLForge_GIT_COMMIT: source_version,
            MLForge_PARENT_RUN_ID: "7",
            "my": "tag",
            "other": "tag",
        },
    }
    ...
    created_run = MLForge_client.create_run(experiment_id, **create_run_kwargs)
    run_id = created_run.info.run_id
    _logger.info(f"Run id={run_id}")
    fetched_run = MLForge_client.get_run(run_id)
    for run in [created_run, fetched_run]:
        ...
        assert run.data.tags.get(MLForge_USER) == user
        assert run.data.tags.get(MLForge_PARENT_RUN_ID) == parent_run_id_kwarg or "7"
        assert [run.info for run in MLForge_client.search_runs([experiment_id])] == [run.info]


def test_create_run_defaults(MLForge_client):
    experiment_id = MLForge_client.create_experiment("Run A Little")
    created_run = MLForge_client.create_run(experiment_id)
    run_id = created_run.info.run_id
    run = MLForge_client.get_run(run_id)
    assert run.info.run_id == run_id
    assert run.info.experiment_id == experiment_id
    assert run.info.user_id == "unknown"


def test_log_metrics_params_tags(MLForge_client):
    experiment_id = MLForge_client.create_experiment("Oh My")
    created_run = MLForge_client.create_run(experiment_id)
    run_id = created_run.info.run_id
    MLForge_client.log_metric(run_id, key="metric", value=123.456, timestamp=789, step=2)
    MLForge_client.log_metric(run_id, key="nan_metric", value=float("nan"))
    MLForge_client.log_metric(run_id, key="inf_metric", value=float("inf"))
    MLForge_client.log_metric(run_id, key="-inf_metric", value=-float("inf"))
    MLForge_client.log_metric(run_id, key="stepless-metric", value=987.654, timestamp=321)
    MLForge_client.log_param(run_id, "param", "value")
    MLForge_client.set_tag(run_id, "taggity", "do-dah")
    run = MLForge_client.get_run(run_id)
    ...
    metric_history0 = MLForge_client.get_metric_history(run_id, "metric")
    ...
    metric_history1 = MLForge_client.get_metric_history(run_id, "stepless-metric")
    ...
    metric_history = MLForge_client.get_metric_history(run_id, "a_test_accuracy")
    assert metric_history == []


def test_log_metric_validation(MLForge_client):
    experiment_id = MLForge_client.create_experiment("metrics validation")
    created_run = MLForge_client.create_run(experiment_id)
    run_id = created_run.info.run_id
    ...
    def assert_bad_request(payload, expected_error_message):
        response = _send_rest_tracking_post_request(
            MLForge_client.tracking_uri,
            "/api/2.0/MLForge/runs/log-metric",
            payload,
        )
        assert response.status_code == 400
        assert expected_error_message in response.text
...
def test_log_metric_model(MLForge_client: MLForgeClient):
    experiment_id = MLForge_client.create_experiment("metrics validation")
    run = MLForge_client.create_run(experiment_id)
    model = MLForge_client.create_logged_model(experiment_id)
    MLForge_client.log_metric(
        run.info.run_id,
        key="metric",
        value=0.5,
        timestamp=123456789,
    ...
    model = MLForge_client.get_logged_model(model.model_id)
...
def test_log_param_validation(MLForge_client):
    experiment_id = MLForge_client.create_experiment("params validation")
    created_run = MLForge_client.create_run(experiment_id)
    run_id = created_run.info.run_id
    ...
    def assert_bad_request(payload, expected_error_message):
        response = _send_rest_tracking_post_request(
            MLForge_client.tracking_uri,
            "/api/2.0/MLForge/runs/log-parameter",
            payload,
        )
        assert response.status_code == 400
        assert expected_error_message in response.text
...
def test_log_param_with_empty_string_as_value(MLForge_client):
    experiment_id = MLForge_client.create_experiment(
    ...
    MLForge_client.log_param(run_id, "param_key", "")
    assert {"param_key": ""}.items() <= MLForge_client.get_run(run_id).data.params.items()


def test_set_tag_with_empty_string_as_value(MLForge_client):
    experiment_id = MLForge_client.create_experiment(
    ...
    MLForge_client.set_tag(run_id, "tag_key", "")
    assert {"tag_key": ""}.items() <= MLForge_client.get_run(run_id).data.tags.items()


def test_log_batch_containing_params_and_tags_with_empty_string_values(MLForge_client):
    experiment_id = MLForge_client.create_experiment(
...
def test_set_tag_validation(MLForge_client):
    experiment_id = MLForge_client.create_experiment("tags validation")
    created_run = MLForge_client.create_run(experiment_id)
    run_id = created_run.info.run_id
...
def test_path_validation(MLForge_client):
    experiment_id = MLForge_client.create_experiment("tags validation")
    created_run = MLForge_client.create_run(experiment_id)
    run_id = created_run.info.run_id
    invalid_path = "../path"
    ...
    response = requests.get(
        f"{MLForge_client.tracking_uri}/api/2.0/MLForge/artifacts/list",
        params={"run_id": run_id, "path": invalid_path},
    )
    assert_response(response)

    response = requests.get(
        f"{MLForge_client.tracking_uri}/get-artifact",
        params={"run_id": run_id, "path": invalid_path},
    )
    assert_response(response)

    response = requests.get(
        f"{MLForge_client.tracking_uri}//model-versions/get-artifact",
        params={"name": "model", "version": 1, "path": invalid_path},
    )
    assert_response(response)


def test_set_experiment_tag(MLForge_client):
    experiment_id = MLForge_client.create_experiment("SetExperimentTagTest")
    MLForge_client.set_experiment_tag(experiment_id, "dataset", "imagenet1K")
    experiment = MLForge_client.get_experiment(experiment_id)
    assert "dataset" in experiment.tags
    assert experiment.tags["dataset"] == "imagenet1K"
    # test that updating a tag works
    MLForge_client.set_experiment_tag(experiment_id, "dataset", "birdbike")
    experiment = MLForge_client.get_experiment(experiment_id)
    assert "dataset" in experiment.tags
    assert experiment.tags["dataset"] == "birdbike"
    # test that setting a tag on 1 experiment does not impact another experiment.
    experiment_id_2 = MLForge_client.create_experiment("SetExperimentTagTest2")
    experiment2 = MLForge_client.get_experiment(experiment_id_2)
    ...
    MLForge_client.set_experiment_tag(experiment_id_2, "dataset", "birds200")
    experiment = MLForge_client.get_experiment(experiment_id)
    experiment2 = MLForge_client.get_experiment(experiment_id_2)
    assert "dataset" in experiment.tags
    assert experiment.tags["dataset"] == "birdbike"
    ...
    MLForge_client.set_experiment_tag(experiment_id, "multiline tag", "value2\nvalue2\nvalue2")
    experiment = MLForge_client.get_experiment(experiment_id)
    assert "multiline tag" in experiment.tags
    assert experiment.tags["multiline tag"] == "value2\nvalue2\nvalue2"


def test_set_experiment_tag_with_empty_string_as_value(MLForge_client):
    experiment_id = MLForge_client.create_experiment(
        test_set_experiment_tag_with_empty_string_as_value.__name__
    )
    MLForge_client.set_experiment_tag(experiment_id, "tag_key", "")
    assert {"tag_key": ""}.items() <= MLForge_client.get_experiment(experiment_id).tags.items()


def test_delete_experiment_tag(MLForge_client):
    experiment_id = MLForge_client.create_experiment("DeleteExperimentTagTest")
    MLForge_client.set_experiment_tag(experiment_id, "dataset", "imagenet1K")
    experiment = MLForge_client.get_experiment(experiment_id)
    assert experiment.tags["dataset"] == "imagenet1K"
    # test that deleting a tag works
    MLForge_client.delete_experiment_tag(experiment_id, "dataset")
    experiment = MLForge_client.get_experiment(experiment_id)
    assert "dataset" not in experiment.tags


def test_delete_tag(MLForge_client):
    experiment_id = MLForge_client.create_experiment("DeleteTagExperiment")
    created_run = MLForge_client.create_run(experiment_id)
    run_id = created_run.info.run_id
    MLForge_client.log_metric(run_id, key="metric", value=123.456, timestamp=789, step=2)
    MLForge_client.log_metric(run_id, key="stepless-metric", value=987.654, timestamp=321)
    ...
    MLForge_client.delete_tag(run_id, "taggity")
    run = MLForge_client.get_run(run_id)
    assert "taggity" not in run.data.tags
    with pytest.raises(MLForgeException, match=r"Run .+ not found"):
        MLForge_client.delete_tag("fake_run_id", "taggity")
    with pytest.raises(MLForgeException, match="No tag with name: fakeTag"):
        MLForge_client.delete_tag(run_id, "fakeTag")
    MLForge_client.delete_run(run_id)
    with pytest.raises(MLForgeException, match=f"The run {run_id} must be in"):
        MLForge_client.delete_tag(run_id, "taggity")


def test_log_batch(MLForge_client):
    experiment_id = MLForge_client.create_experiment("Batch em up")
    created_run = MLForge_client.create_run(experiment_id)
    run_id = created_run.info.run_id
    MLForge_client.log_batch(
        run_id=run_id,
    ...
    assert run.data.params.get("param") == "value"
    assert run.data.tags.get("taggity") == "do-dah"
    metric_history = MLForge_client.get_metric_history(run_id, "metric")
...
def test_log_batch_validation(MLForge_client):
    experiment_id = MLForge_client.create_experiment("log_batch validation")
    created_run = MLForge_client.create_run(experiment_id)
    run_id = created_run.info.run_id
    ...
    def assert_bad_request(payload, expected_error_message):
        response = _send_rest_tracking_post_request(
            MLForge_client.tracking_uri,
            "/api/2.0/MLForge/runs/log-batch",
            payload,
        )
        assert response.status_code == 400
        assert expected_error_message in response.text
...
def test_log_model(MLForge_client):
    experiment_id = MLForge_client.create_experiment("Log models")
    with TempDir(chdr=True):
        model_paths = [f"model/path/{i}" for i in range(3)]
        MLForge.set_tracking_uri(MLForge_client.tracking_uri)
        with MLForge.start_run(experiment_id=experiment_id) as run:
            for i, m in enumerate(model_paths):
                MLForge.pyfunc.log_model(name=m, loader_module="MLForge.pyfunc")
                MLForge.pyfunc.save_model(
                    m,
                    MLForge_model=Model(artifact_path=m, run_id=run.info.run_id),
                    loader_module="MLForge.pyfunc",
                )
                model = Model.load(os.path.join(m, "MLmodel"))
                run = MLForge.get_run(run.info.run_id)
                tag = run.data.tags["MLForge.log-model.history"]
...
def test_set_terminated_defaults(MLForge_client):
    experiment_id = MLForge_client.create_experiment("Terminator 1")
    created_run = MLForge_client.create_run(experiment_id)
    run_id = created_run.info.run_id
    assert MLForge_client.get_run(run_id).info.status == "RUNNING"
    assert MLForge_client.get_run(run_id).info.end_time is None
    MLForge_client.set_terminated(run_id)
    assert MLForge_client.get_run(run_id).info.status == "FINISHED"
    assert MLForge_client.get_run(run_id).info.end_time <= get_current_time_millis()


def test_set_terminated_status(MLForge_client):
    experiment_id = MLForge_client.create_experiment("Terminator 2")
    created_run = MLForge_client.create_run(experiment_id)
    run_id = created_run.info.run_id
    assert MLForge_client.get_run(run_id).info.status == "RUNNING"
    assert MLForge_client.get_run(run_id).info.end_time is None
    MLForge_client.set_terminated(run_id, "FAILED")
    assert MLForge_client.get_run(run_id).info.status == "FAILED"
    assert MLForge_client.get_run(run_id).info.end_time <= get_current_time_millis()


def test_artifacts(MLForge_client, tmp_path):
    experiment_id = MLForge_client.create_experiment("Art In Fact")
    experiment_info = MLForge_client.get_experiment(experiment_id)
    ...
    with open(src_file, "w") as f:
        f.write("Hello, World!")
    MLForge_client.log_artifact(run_id, src_file, None)
    MLForge_client.log_artifacts(run_id, src_dir, "dir")

    root_artifacts_list = MLForge_client.list_artifacts(run_id)
    assert {a.path for a in root_artifacts_list} == {"my.file", "dir"}

    dir_artifacts_list = MLForge_client.list_artifacts(run_id, "dir")
    assert {a.path for a in dir_artifacts_list} == {"dir/my.file"}

    all_artifacts = download_artifacts(
        run_id=run_id, artifact_path=".", tracking_uri=MLForge_client.tracking_uri
    )
    ...
    dir_artifacts = download_artifacts(
        run_id=run_id, artifact_path="dir", tracking_uri=MLForge_client.tracking_uri
    )
    with open(f"{dir_artifacts}/my.file") as f:
        assert f.read() == "Hello, World!"


def test_search_pagination(MLForge_client):
    experiment_id = MLForge_client.create_experiment("search_pagination")
    runs = [MLForge_client.create_run(experiment_id, start_time=1).info.run_id for _ in range(0, 10)]
    runs = sorted(runs)
    result = MLForge_client.search_runs([experiment_id], max_results=4, page_token=None)
    assert [r.info.run_id for r in result] == runs[0:4]
    assert result.token is not None
    result = MLForge_client.search_runs([experiment_id], max_results=4, page_token=result.token)
...
def test_search_validation(MLForge_client):
    experiment_id = MLForge_client.create_experiment("search_validation")
    with pytest.raises(
        MLForgeException,
        match=r"Invalid value 123456789 for parameter 'max_results' supplied",
    ):
        MLForge_client.search_runs([experiment_id], max_results=123456789)


def test_get_experiment_by_name(MLForge_client):
    name = "test_get_experiment_by_name"
    experiment_id = MLForge_client.create_experiment(name)
    res = MLForge_client.get_experiment_by_name(name)
    assert res.experiment_id == experiment_id
    assert res.name == name
    assert MLForge_client.get_experiment_by_name("idontexist") is None


def test_get_experiment(MLForge_client):
    name = "test_get_experiment"
    experiment_id = MLForge_client.create_experiment(name)
    res = MLForge_client.get_experiment(experiment_id)
    assert res.experiment_id == experiment_id
    assert res.name == name


def test_search_experiments(MLForge_client):
    ...
    # store.
    assert MLForge_client.search_experiments()[0].name == "Default"
    ...
    for name, tags in experiments:
        ...
        time.sleep(0.001)
        experiment_ids.append(MLForge_client.create_experiment(name, tags=tags))

    # filter_string
    experiments = MLForge_client.search_experiments(filter_string="attribute.name = 'a'")
    assert [e.name for e in experiments] == ["a"]
    experiments = MLForge_client.search_experiments(filter_string="attribute.name != 'a'")
    assert [e.name for e in experiments] == ["Abc", "ab", "Default"]
    experiments = MLForge_client.search_experiments(filter_string="name LIKE 'a%'")
    assert [e.name for e in experiments] == ["ab", "a"]
    experiments = MLForge_client.search_experiments(filter_string="tag.key = 'value'")
    assert [e.name for e in experiments] == ["a"]
    experiments = MLForge_client.search_experiments(filter_string="tag.key != 'value'")
    assert [e.name for e in experiments] == ["ab"]
    experiments = MLForge_client.search_experiments(filter_string="tag.key ILIKE '%alu%'")
    assert [e.name for e in experiments] == ["ab", "a"]

    # order_by
    experiments = MLForge_client.search_experiments(order_by=["name DESC"])
    assert [e.name for e in experiments] == ["ab", "a", "Default", "Abc"]

    # max_results
    experiments = MLForge_client.search_experiments(max_results=2)
    assert [e.name for e in experiments] == ["Abc", "ab"]
    # page_token
    experiments = MLForge_client.search_experiments(page_token=experiments.token)
    assert [e.name for e in experiments] == ["a", "Default"]

    # view_type
    time.sleep(0.001)
    MLForge_client.delete_experiment(experiment_ids[1])
    experiments = MLForge_client.search_experiments(view_type=ViewType.ACTIVE_ONLY)
    assert [e.name for e in experiments] == ["Abc", "a", "Default"]
    experiments = MLForge_client.search_experiments(view_type=ViewType.DELETED_ONLY)
    assert [e.name for e in experiments] == ["ab"]
    experiments = MLForge_client.search_experiments(view_type=ViewType.ALL)
    assert [e.name for e in experiments] == ["Abc", "ab", "a", "Default"]


def test_get_metric_history_bulk_rejects_invalid_requests(MLForge_client):
    ...
    response_no_run_ids_field = requests.get(
        f"{MLForge_client.tracking_uri}/ajax-api/2.0/MLForge/metrics/get-history-bulk",
...
def test_get_metric_history_bulk_returns_expected_metrics_in_expected_order(
    ...
    experiment_id = MLForge_client.create_experiment("get metric history bulk")
    created_run1 = MLForge_client.create_run(experiment_id)
    run_id1 = created_run1.info.run_id
    created_run2 = MLForge_client.create_run(experiment_id)
    run_id2 = created_run2.info.run_id
    created_run3 = MLForge_client.create_run(experiment_id)
    ...
    for metric in metricA_history:
        MLForge_client.log_metric(run_id1, **metric)
        metric_for_run2 = dict(metric)
        metric_for_run2["value"] += 1.0
        MLForge_client.log_metric(run_id2, **metric_for_run2)
...
def test_get_metric_history_bulk_respects_max_results(MLForge_client):
    experiment_id = MLForge_client.create_experiment("get metric history bulk")
    run_id = MLForge_client.create_run(experiment_id).info.run_id
    max_results = 2
    ...
    for metric in metricA_history:
        MLForge_client.log_metric(run_id, **metric)
...
def test_get_metric_history_bulk_calls_optimized_impl_when_expected(tmp_path):
    from MLForge.server.handlers import get_metric_history_bulk_handler
    ...
    with (
        mock.patch("MLForge.server.handlers._get_tracking_store", return_value=mock_store),
...
def test_get_metric_history_respects_max_results(MLForge_client):
    experiment_id = MLForge_client.create_experiment("test max_results")
    run = MLForge_client.create_run(experiment_id)
    run_id = run.info.run_id
    ...
    all_metrics = MLForge_client.get_metric_history(run_id, "test_metric")
    assert len(all_metrics) == 5

    # Test with max_results=3 - should return only 3 metrics
    response = requests.get(
        f"{MLForge_client.tracking_uri}/ajax-api/2.0/MLForge/metrics/get-history",
    ...
    for i, metric in enumerate(returned_metrics):
        ...
        if _MLForge_GO_STORE_TESTING.get():
            assert int(metric["step"]) == i
        else:
            assert metric["step"] == i


def test_get_metric_history_with_page_token(MLForge_client):
    experiment_id = MLForge_client.create_experiment("test page_token")
    run = MLForge_client.create_run(experiment_id)
    run_id = run.info.run_id
...
def test_get_metric_history_without_max_results_returns_full_history(MLForge_client):
    ...
    experiment_id = MLForge_client.create_experiment("test no max_results")
    run = MLForge_client.create_run(experiment_id)
    run_id = run.info.run_id

    for i in range(10):
        MLForge_client.log_metric(run_id, key="test_metric", value=float(i), step=i)
...
def test_get_metric_history_rejects_non_positive_max_results(MLForge_client, max_results):
    experiment_id = MLForge_client.create_experiment(f"test max_results {max_results}")
    run = MLForge_client.create_run(experiment_id)
    run_id = run.info.run_id
    MLForge_client.log_metric(run_id, key="test_metric", value=1.0, step=0)
...
def test_get_metric_history_bulk_interval_rejects_invalid_requests(MLForge_client):
    def assert_response(resp, message_part):
        assert resp.status_code == 400
        response_json = resp.json()
        assert response_json.get("error_code") == "INVALID_PARAMETER_VALUE"
    ...
    url = f"{MLForge_client.tracking_uri}/ajax-api/2.0/MLForge/metrics/get-history-bulk-interval"
...
def test_get_metric_history_bulk_interval_respects_max_results(MLForge_client):
    experiment_id = MLForge_client.create_experiment("get metric history bulk")
    run_id1 = MLForge_client.create_run(experiment_id).info.run_id
    metric_history = [
    ...
    # multiple runs
    run_id2 = MLForge_client.create_run(experiment_id).info.run_id
    metric_history2 = [
        {"key": "metricA", "timestamp": 1, "step": i, "value": 10.0} for i in range(20)
    ]
    for metric in metric_history2:
        MLForge_client.log_metric(run_id2, **metric)
    response_limited = requests.get(
        url,
        params={
...
def test_search_dataset_handler_rejects_invalid_requests(MLForge_client):
    def assert_response(resp, message_part):
        assert resp.status_code == 400
        response_json = resp.json()
        assert response_json.get("error_code") == "INVALID_PARAMETER_VALUE"
    ...
    response_no_experiment_id_field = requests.post(
        f"{MLForge_client.tracking_uri}/ajax-api/2.0/MLForge/experiments/search-datasets",
        json={},
    )
    assert_response(
...
def test_search_dataset_handler_returns_expected_results(MLForge_client):
    experiment_id = MLForge_client.create_experiment("log inputs test")
    created_run = MLForge_client.create_run(experiment_id)
    run_id = created_run.info.run_id
    ...
    dataset_inputs1 = [
        DatasetInput(
            dataset=dataset1,
            tags=[InputTag(key=MLForge_DATASET_CONTEXT, value="training")],
        )
    ]
    MLForge_client.log_inputs(run_id, dataset_inputs1)
...
def test_create_model_version_with_path_source(MLForge_client):
    name = "model"
    MLForge_client.create_registered_model(name)
    exp_id = MLForge_client.create_experiment("test")
    run = MLForge_client.create_run(experiment_id=exp_id)

    response = requests.post(
        f"{MLForge_client.tracking_uri}/api/2.0/MLForge/model-versions/create",
        json={
            "name": name,
...
def test_create_model_version_with_non_local_source(MLForge_client):
    name = "model"
    MLForge_client.create_registered_model(name)
    exp_id = MLForge_client.create_experiment("test")
    run = MLForge_client.create_run(experiment_id=exp_id)
    ...
    response = requests.post(
        ...
        json={
            "name": name,
            "source": "MLForge-artifacts:/models",
            "run_id": run.info.run_id,
        },
    )
    assert response.status_code == 200
    ...
    response = requests.post(
        ...
        json={
            "name": name,
            "source": "MLForge-artifacts:/models/foo///bar",
            "run_id": run.info.run_id,
        },
    )
    assert response.status_code == 200
    ...
    response = requests.post(
        ...
        json={
            "name": name,
            "source": "MLForge-artifacts://host:9000/models",
            "run_id": run.info.run_id,
        },
    )
    assert response.status_code == 200
    ...
    response = requests.post(
        ...
        json={
            "name": name,
            "source": "MLForge-artifacts://host:9000/models/artifact/..../",
            "run_id": run.info.run_id,
        },
    )
    assert response.status_code == 200
    ...
    response = requests.post(
        ...
        json={
            "name": name,
            "source": "https://host/api/2.0/MLForge-artifacts/artifacts/../../../",
            "run_id": run.info.run_id,
        },
    )
    assert response.status_code == 400
    ...
    response = requests.post(
        ...
        json={
            "name": name,
            "source": "s3a://my_bucket/api/2.0/MLForge-artifacts/artifacts/../../../",
            "run_id": run.info.run_id,
        },
    )
    assert response.status_code == 400
    ...
    response = requests.post(
        ...
        json={
            "name": name,
            "source": "ftp://host:8888/api/2.0/MLForge-artifacts/artifacts/../../../",
            "run_id": run.info.run_id,
        },
    )
    assert response.status_code == 400
    ...
    response = requests.post(
        ...
        json={
            "name": name,
            "source": "MLForge-artifacts://host:9000/models/..%2f..%2fartifacts",
            "run_id": run.info.run_id,
        },
    )
    assert response.status_code == 400
    ...
    response = requests.post(
        ...
        json={
            "name": name,
            "source": "MLForge-artifacts://host:9000/models/artifact%00",
            "run_id": run.info.run_id,
        },
    )
    assert response.status_code == 400
    ...
    model = MLForge_client.create_logged_model(experiment_id=exp_id)
    response = requests.post(
        f"{MLForge_client.tracking_uri}/api/2.0/MLForge/model-versions/create",
        json={
            "name": name,
...
def test_create_model_version_with_file_uri(MLForge_client):
    name = "test"
    MLForge_client.create_registered_model(name)
    exp_id = MLForge_client.create_experiment("test")
    run = MLForge_client.create_run(experiment_id=exp_id)
...
def test_create_model_version_with_validation_regex(db_uri: str):
    ...
    with subprocess.Popen(
        ...
        env=(
            os.environ.copy()
            | {
                "MLForge_CREATE_MODEL_VERSION_SOURCE_VALIDATION_REGEX": r"^MLForge-artifacts:/.*$",
                "MLForge_SERVER_ENABLE_JOB_EXECUTION": "false",
            }
        ),
    ) as proc:
        try:
            ...
            for _ in range(30):
                ...
                raise RuntimeError("Failed to connect to the MLForge server")

            # Test that the validation regex works as expected
            client = MLForgeClient(f"http://localhost:{port}")
            name = "test"
            client.create_registered_model(name)
            # Invalid source
            with pytest.raises(MLForgeException, match="Invalid model version source"):
                client.create_model_version(name, source="s3://path/to/model")
            # Valid source
            experiment_id = client.create_experiment("test")
            run = client.create_run(experiment_id=experiment_id)
            assert run.info.artifact_uri.startswith("MLForge-artifacts:/")
...
@pytest.mark.xfail(reason="Tracking server does not support logged-model endpoints yet")
def test_logging_model_with_local_artifact_uri(MLForge_client):
    from sklearn.linear_model import LogisticRegression

    MLForge.set_tracking_uri(MLForge_client.tracking_uri)
    with MLForge.start_run() as run:
        assert run.info.artifact_uri.startswith("file://")
        MLForge.sklearn.log_model(LogisticRegression(), name="model", registered_model_name="rmn")
        MLForge.pyfunc.load_model("models:/rmn/1")


def test_log_input(MLForge_client, tmp_path):
    ...
    with MLForge.start_run() as run:
        MLForge.log_input(dataset, "train", {"foo": "baz"})

    dataset_inputs = MLForge_client.get_run(run.info.run_id).inputs.dataset_inputs
    ...
    assert dataset_inputs[0].tags[1].key == MLForge_tags.MLForge_DATASET_CONTEXT
    assert dataset_inputs[0].tags[1].value == "train"


def test_create_model_version_model_id(MLForge_client):
    name = "model"
    MLForge_client.create_registered_model(name)
    exp_id = MLForge_client.create_experiment("test")
    model = MLForge_client.create_logged_model(experiment_id=exp_id)
    MLForge_client.create_model_version(
        name=name,
        source=model.artifact_location,
        model_id=model.model_id,
    )
    model = MLForge_client.get_logged_model(model.model_id)
    assert model.tags["MLForge.modelVersions"] == '[{"name": "model", "version": 1}]'
    MLForge_client.create_model_version(
        name=name,
        source=model.artifact_location,
        model_id=model.model_id,
    ...
    model = MLForge_client.get_logged_model(model.model_id)
    assert (
        model.tags["MLForge.modelVersions"]
        == '[{"name": "model", "version": 1}, {"name": "model", "version": 2}]'
    )


def test_log_inputs(MLForge_client):
    experiment_id = MLForge_client.create_experiment("log inputs test")
    created_run = MLForge_client.create_run(experiment_id)
    run_id = created_run.info.run_id
...
def test_log_inputs_validation(MLForge_client):
    def assert_bad_request(payload, expected_error_message):
        response = _send_rest_tracking_post_request(
            MLForge_client.tracking_uri,
            "/api/2.0/MLForge/runs/log-inputs",
            payload,
        )
        assert response.status_code == 400
        assert expected_error_message in response.text
...
def test_log_inputs_model(MLForge_client):
    experiment_id = MLForge_client.create_experiment("log inputs test")
    run = MLForge_client.create_run(experiment_id)
    model = MLForge_client.create_logged_model(experiment_id=experiment_id)
    dataset = Dataset(
        name="name1",
        digest="digest1",
        source_type="source_type1",
    ...
    MLForge_client.log_inputs(
        run.info.run_id,
        models=[LoggedModelInput(model_id=model.model_id)],
        datasets=dataset_inputs,
    )
    run = MLForge_client.get_run(run.info.run_id)
    assert len(run.inputs.model_inputs) == 1


def test_update_run_name_without_changing_status(MLForge_client):
    experiment_id = MLForge_client.create_experiment("update run name")
    created_run = MLForge_client.create_run(experiment_id)
    MLForge_client.set_terminated(created_run.info.run_id, "FINISHED")

    MLForge_client.update_run(created_run.info.run_id, name="name_abc")
    updated_run_info = MLForge_client.get_run(created_run.info.run_id).info
    assert updated_run_info.run_name == "name_abc"
    assert updated_run_info.status == "FINISHED"


def test_create_promptlab_run_handler_rejects_invalid_requests(MLForge_client):
    def assert_response(resp, message_part):
        assert resp.status_code == 400
        response_json = resp.json()
        assert response_json.get("error_code") == "INVALID_PARAMETER_VALUE"
    ...
    response = requests.post(
        f"{MLForge_client.tracking_uri}/ajax-api/2.0/MLForge/runs/create-promptlab-run",
        json={},
    )
    assert_response(
        response,
    ...
    assert_response(
        response,
        "CreatePromptlabRun request must specify MLForge_version.",
    )
...
def test_create_promptlab_run_handler_returns_expected_results(MLForge_client):
    experiment_id = MLForge_client.create_experiment("log inputs test")
    ...
    assert {
        "key": "MLForge.loggedArtifacts",
        "value": '[{"path": "eval_results_table.json", "type": "table"}]',
    } in run_json["run"]["data"]["tags"]
    assert {"key": "MLForge.runSourceType", "value": "PROMPT_ENGINEERING"} in run_json["run"][
        "data"
    ]["tags"]


def test_gateway_proxy_handler_rejects_invalid_requests(MLForge_client):
    def assert_response(resp, message_part):
        assert resp.status_code == 400
        response_json = resp.json()
        assert response_json.get("error_code") == "INVALID_PARAMETER_VALUE"
    ...
    with _init_server(
        backend_uri=MLForge_client.tracking_uri,
        root_artifact_uri=MLForge_client.tracking_uri,
        extra_env={"MLForge_DEPLOYMENTS_TARGET": "http://localhost:5001"},
        server_type="flask",
    ) as url:
        patched_client = MLForgeClient(url)

        response = requests.post(
            f"{patched_client.tracking_uri}/ajax-api/2.0/MLForge/gateway-proxy",
            json={},
        )
        assert_response(
            response,
...
def test_upload_artifact_handler_rejects_invalid_requests(MLForge_client):
    def assert_response(resp, message_part):
        assert resp.status_code == 400
        response_json = resp.json()
        assert response_json.get("error_code") == "INVALID_PARAMETER_VALUE"
    ...
    experiment_id = MLForge_client.create_experiment("upload_artifacts_test")
    created_run = MLForge_client.create_run(experiment_id)

    response = requests.post(
        f"{MLForge_client.tracking_uri}/ajax-api/2.0/MLForge/upload-artifact", params={}
    )
    assert_response(response, "Request must specify run_uuid.")

    response = requests.post(
        f"{MLForge_client.tracking_uri}/ajax-api/2.0/MLForge/upload-artifact",
        params={
...
def test_upload_artifact_handler(MLForge_client):
    experiment_id = MLForge_client.create_experiment("upload_artifacts_test")
    created_run = MLForge_client.create_run(experiment_id)
...
def test_graphql_handler(MLForge_client):
    response = requests.post(
        f"{MLForge_client.tracking_uri}/graphql",
        json={
...
def test_graphql_handler_batching_raise_error(MLForge_client):
    # Test max root fields limit
    batch_query = (
        "query testQuery {"
        + " ".join([
            f"key_{i}: " + 'test(inputString: "abc") { output }'
            for i in range(int(MLForge_SERVER_GRAPHQL_MAX_ROOT_FIELDS.get()) + 2)
        ])
        + "}"
    )
    response = requests.post(
    ...
    assert response.status_code == 200
    assert (
        f"GraphQL queries should have at most {MLForge_SERVER_GRAPHQL_MAX_ROOT_FIELDS.get()}"
        in response.json()["errors"][0]
    )

    # Test max aliases limit
    batch_query = (
        'query testQuery {MLForgeGetExperiment(input: {experimentId: "123"}) {'
        + " ".join(
            f"experiment_{i}: " + "experiment { name }"
            for i in range(int(MLForge_SERVER_GRAPHQL_MAX_ALIASES.get()) + 2)
        )
        + "}}"
    )
    response = requests.post(
    ...
    assert response.status_code == 200
    assert (
        f"queries should have at most {MLForge_SERVER_GRAPHQL_MAX_ALIASES.get()} aliases"
        in response.json()["errors"][0]
    )
    ...
    for _ in range(12):
        inner = f"name {{ {inner} }}"
    deep_query = (
        'query testQuery { MLForgeGetExperiment(input: {experimentId: "123"}) { experiment { '
        + inner
        + " } } }"
    )
    response = requests.post(
...
def test_get_experiment_graphql(MLForge_client):
    experiment_id = MLForge_client.create_experiment("GraphqlTest")
    response = requests.post(
        f"{MLForge_client.tracking_uri}/graphql",
        json={
            "query": 'query testQuery {MLForgeGetExperiment(input: {experimentId: "'
            + experiment_id
    ...
    assert response.status_code == 200
    json = response.json()
    assert json["data"]["MLForgeGetExperiment"]["experiment"]["name"] == "GraphqlTest"
    assert json["data"]["MLForgeGetExperiment"]["experiment"]["effectiveTraceArchivalRetention"] in (
        None,
        "",
    )


def test_get_run_and_experiment_graphql(MLForge_client):
    name = "GraphqlTest"
    MLForge_client.create_registered_model(name)
    experiment_id = MLForge_client.create_experiment(name)
    created_run = MLForge_client.create_run(experiment_id)
    run_id = created_run.info.run_id
    MLForge_client.create_model_version("GraphqlTest", "runs:/graphql_test/model", run_id)
    response = requests.post(
        f"{MLForge_client.tracking_uri}/graphql",
        json={
            "query": f"""
                query testQuery @component(name: "Test") {{
                    MLForgeGetRun(input: {{runId: "{run_id}"}}) {{
                        run {{
                            info {{
                                status
                            }}
    ...
    assert json["data"]["MLForgeGetRun"]["run"]["info"]["status"] == created_run.info.status
    assert json["data"]["MLForgeGetRun"]["run"]["experiment"]["name"] == name
    assert json["data"]["MLForgeGetRun"]["run"]["modelVersions"][0]["name"] == name


def test_legacy_start_and_end_trace_v2(MLForge_client):
    experiment_id = MLForge_client.create_experiment("start end trace")

    # Trace CRUD APIs are not directly exposed as public API of MLForgeClient,
    # so we use the underlying tracking client to test them.
    store = MLForge_client._tracing_client.store

    # Helper function to remove auto-added system tags (MLForge.xxx) from testing
    def _exclude_system_tags(tags: dict[str, str]):
        return {k: v for k, v in tags.items() if not k.startswith("MLForge.")}
...
def test_start_trace(MLForge_client):
    MLForge.set_tracking_uri(MLForge_client.tracking_uri)
    experiment_id = MLForge.set_experiment("start end trace").experiment_id

    # Helper function to remove auto-added system tags (MLForge.xxx) from testing
    def _exclude_system_keys(d: dict[str, str]):
        return {k: v for k, v in d.items() if not k.startswith("MLForge.")}

    with mock.patch("MLForge.tracing.export.MLForge_v3._logger.warning") as mock_warning:
        with MLForge.start_span(name="test") as span:
            MLForge.update_current_trace(
                tags={
                    "tag1": "football",
                    "tag2": "basketball",
                },
    ...
    trace = MLForge_client.get_trace(span.trace_id, flush=True)
    ...
    # No "Failed to log span to MLForge backend" warning should be issued
    for call in mock_warning.call_args_list:
        assert "Failed to log span to MLForge backend" not in str(call)


def test_get_trace(MLForge_client):
    MLForge.set_tracking_uri(MLForge_client.tracking_uri)
    experiment_id = MLForge_client.create_experiment("get trace")
    span = MLForge_client.start_trace(name="test", experiment_id=experiment_id)
    MLForge_client.end_trace(request_id=span.request_id, status=TraceStatus.OK)
    trace = MLForge_client.get_trace(span.request_id, flush=True)
...
def test_search_traces(MLForge_client):
    MLForge.set_tracking_uri(MLForge_client.tracking_uri)
    experiment_id = MLForge_client.create_experiment("search traces")

    # Create test traces
    def _create_trace(name, status):
        span = MLForge_client.start_trace(name=name, experiment_id=experiment_id)
        MLForge_client.end_trace(request_id=span.request_id, status=status)
    ...
    MLForge.flush_trace_async_logging()
...
def test_search_traces_parameter_validation(MLForge_client):
    with pytest.raises(
        MLForgeException,
        match="Locations must be a list of experiment IDs",
    ):
        MLForge_client.search_traces(locations=["catalog.schema"])


def test_search_traces_match_text(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("File store doesn't support full text search")

    MLForge.set_tracking_uri(MLForge_client.tracking_uri)
    experiment_id = MLForge_client.create_experiment("search traces full text")

    # Create test traces
    def _create_trace(name, attributes):
        span = MLForge_client.start_trace(name=name, experiment_id=experiment_id)
        span.set_attributes(attributes)
        MLForge_client.end_trace(request_id=span.trace_id, status=TraceStatus.OK)
    ...
    traces = MLForge_client.search_traces(locations=[experiment_id], flush=True)
...
def test_delete_traces(MLForge_client):
    MLForge.set_tracking_uri(MLForge_client.tracking_uri)
    experiment_id = MLForge_client.create_experiment("delete traces")
    ...
    def _is_trace_exists(request_id):
        try:
            trace_info = MLForge_client._tracing_client.get_trace_info(request_id)
    ...
    deleted_count = MLForge_client.delete_traces(experiment_id, max_timestamp_millis=int(1e15))
    ...
    deleted_count = MLForge_client.delete_traces(
    ...
    deleted_count = MLForge_client.delete_traces(experiment_id, trace_ids=[request_id_1])
    assert deleted_count == 1
    assert not _is_trace_exists(request_id_1)
    assert _is_trace_exists(request_id_2)


def test_calculate_trace_filter_correlation(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("File store doesn't support calculate_trace_filter_correlation")

    MLForge.set_tracking_uri(MLForge_client.tracking_uri)
    experiment_id = MLForge_client.create_experiment("correlation test")

    def _create_trace(name, tags):
        span = MLForge_client.start_trace(name=name, experiment_id=experiment_id, tags=tags)
        MLForge_client.end_trace(request_id=span.request_id, status=TraceStatus.OK)
        return span.request_id
    ...
    client = TracingClient(tracking_uri=MLForge_client.tracking_uri)
    ...
    with pytest.raises(MLForgeException, match="Invalid"):
...
def test_set_and_delete_trace_tag(MLForge_client):
    MLForge.set_tracking_uri(MLForge_client.tracking_uri)
    experiment_id = MLForge_client.create_experiment("set delete tag")

    # Create test trace
    trace_info = MLForge_client._tracing_client.start_trace(
        TraceInfo(
            trace_id="tr-1234",
    ...
    # Validate set tag
    MLForge_client.set_trace_tag(trace_info.request_id, "tag1", "green")
    trace_info = MLForge_client._tracing_client.get_trace_info(trace_info.request_id)
    assert trace_info.tags["tag1"] == "green"

    # Validate delete tag
    MLForge_client.delete_trace_tag(trace_info.request_id, "tag2")
    trace_info = MLForge_client._tracing_client.get_trace_info(trace_info.request_id)
    assert "tag2" not in trace_info.tags


def test_query_trace_metrics(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("File store doesn't support query trace metrics")

    MLForge.set_tracking_uri(MLForge_client.tracking_uri)
    experiment_id = MLForge_client.create_experiment("query trace metrics")
    ...
    metrics = MLForge_client._tracing_client.store.query_trace_metrics(
        experiment_ids=[experiment_id],
...
def test_get_trace_handler(MLForge_client, allow_partial: bool, store_type):
    if store_type == "file":
    ...
    response = requests.get(
        f"{MLForge_client.tracking_uri}/ajax-api/3.0/MLForge/traces/get",
...
def test_get_trace_artifact_handler(MLForge_client):
    MLForge.set_tracking_uri(MLForge_client.tracking_uri)
    ...
    response = requests.get(
        f"{MLForge_client.tracking_uri}/ajax-api/2.0/MLForge/get-trace-artifact",
...
def test_link_traces_to_run_and_search_traces(MLForge_client, store_type):
    ...
    MLForge.set_tracking_uri(MLForge_client.tracking_uri)
    experiment_id = MLForge.set_experiment("link traces to run test").experiment_id

    run = MLForge_client.create_run(experiment_id)
    run_id = run.info.run_id

    # 1. Trace created under a run
    with MLForge.start_run(run_id=run_id):
        with MLForge.start_span(name="trace1") as span1:
            span1.set_attributes({"test": "value1"})
        trace_id_1 = span1.trace_id

    # 2. Trace associated with a run
    with MLForge.start_span(name="trace2") as span2:
        span2.set_attributes({"test": "value2"})
    trace_id_2 = span2.trace_id
    MLForge_client.link_traces_to_run(trace_ids=[trace_id_2], run_id=run_id)

    # 3. Trace not associated with a run
    with MLForge.start_span(name="trace3") as span3:
    ...
    all_traces = MLForge_client.search_traces(locations=[experiment_id], flush=True)
    ...
    linked_traces = MLForge_client.search_traces(
...
def test_get_metric_history_bulk_interval_graphql(MLForge_client):
    name = "GraphqlTest"
    MLForge_client.create_registered_model(name)
    experiment_id = MLForge_client.create_experiment(name)
    created_run = MLForge_client.create_run(experiment_id)
    ...
    for i in range(10):
        MLForge_client.log_metric(created_run.info.run_id, metric_name, i, step=i)
    ...
    response = requests.post(
        ...
        json={
            "query": f"""
                query testQuery {{
                    MLForgeGetMetricHistoryBulkInterval(input: {{
    ...
    assert json["data"]["MLForgeGetMetricHistoryBulkInterval"]["metrics"] == expected


def test_search_runs_graphql(MLForge_client):
    name = "GraphqlTest"
    MLForge_client.create_registered_model(name)
    experiment_id = MLForge_client.create_experiment(name)
    created_run_1 = MLForge_client.create_run(experiment_id)
    created_run_2 = MLForge_client.create_run(experiment_id)
        ...
        json={
            "query": f"""
                mutation testMutation {{
                    MLForgeSearchRuns(input: {{ experimentIds: ["{experiment_id}"] }}) {{
                        runs {{
                            info {{
                                runId
                            }}
    ...
    assert json["data"]["MLForgeSearchRuns"]["runs"] == expected


def test_list_artifacts_graphql(MLForge_client, tmp_path):
    name = "GraphqlTest"
    experiment_id = MLForge_client.create_experiment(name)
    created_run_id = MLForge_client.create_run(experiment_id).info.run_id
    file_path = tmp_path / "test.txt"
    file_path.write_text("hello world")
    MLForge_client.log_artifact(created_run_id, file_path.absolute().as_posix())
    MLForge_client.log_artifact(created_run_id, file_path.absolute().as_posix(), "testDir")
    ...
    response = requests.post(
        ...
        json={
            "query": f"""
                query testQuery {{
                    files: MLForgeListArtifacts(input: {{
    ...
    response = requests.post(
        ...
        json={
            "query": f"""
                query testQuery {{
                    subdir: MLForgeListArtifacts(input: {{
                        runId: "{created_run_id}",
                        path: "testDir",
                    }}) {{
                            files {{
...
def test_search_datasets_graphql(MLForge_client):
    name = "GraphqlTest"
    experiment_id = MLForge_client.create_experiment(name)
    created_run_id = MLForge_client.create_run(experiment_id).info.run_id
    dataset1 = Dataset(
    ...
    dataset_input2 = DatasetInput(
        dataset=dataset2, tags=[InputTag(key=MLForge_DATASET_CONTEXT, value="training")]
    )
    MLForge_client.log_inputs(created_run_id, [dataset_input1, dataset_input2])
    ...
    response = requests.post(
        ...
        json={
            "query": f"""
                mutation testMutation {{
                    MLForgeSearchDatasets(input:{{experimentIds: ["{experiment_id}"]}}) {{
                        datasetSummaries {{
                            experimentId
                            name
                            digest
    ...
    assert (
        sort_dataset_summaries(json["data"]["MLForgeSearchDatasets"]["datasetSummaries"]) == expected
    )


def test_create_logged_model(MLForge_client: MLForgeClient):
    exp_id = MLForge_client.create_experiment("create_logged_model")
    model = MLForge_client.create_logged_model(exp_id)
    loaded_model = MLForge_client.get_logged_model(model.model_id)
    assert model.model_id == loaded_model.model_id

    model = MLForge_client.create_logged_model(exp_id, name="my_model")
    loaded_model = MLForge_client.get_logged_model(model.model_id)
    assert model.name == "my_model"

    model = MLForge_client.create_logged_model(exp_id, model_type="LLM")
    loaded_model = MLForge_client.get_logged_model(model.model_id)
    assert model.model_type == "LLM"

    model = MLForge_client.create_logged_model(exp_id, source_run_id="123")
    loaded_model = MLForge_client.get_logged_model(model.model_id)
    assert model.source_run_id == "123"

    model = MLForge_client.create_logged_model(exp_id, params={"param": "value"})
    loaded_model = MLForge_client.get_logged_model(model.model_id)
    assert model.params == {"param": "value"}

    model = MLForge_client.create_logged_model(exp_id, tags={"tag": "value"})
    loaded_model = MLForge_client.get_logged_model(model.model_id)
    assert model.tags == {"tag": "value"}


def test_log_logged_model_params(MLForge_client: MLForgeClient):
    exp_id = MLForge_client.create_experiment("create_logged_model")
    model = MLForge_client.create_logged_model(exp_id)
    MLForge_client.log_model_params(model.model_id, {"param": "value"})
    loaded_model = MLForge_client.get_logged_model(model.model_id)
    assert loaded_model.params == {"param": "value"}


def test_finalize_logged_model(MLForge_client: MLForgeClient):
    exp_id = MLForge_client.create_experiment("create_logged_model")
    model = MLForge_client.create_logged_model(exp_id)
    finalized_model = MLForge_client.finalize_logged_model(model.model_id, LoggedModelStatus.READY)
    assert finalized_model.status == LoggedModelStatus.READY

    finalized_model = MLForge_client.finalize_logged_model(model.model_id, LoggedModelStatus.FAILED)
    assert finalized_model.status == LoggedModelStatus.FAILED


def test_delete_logged_model(MLForge_client: MLForgeClient):
    exp_id = MLForge_client.create_experiment("delete_logged_model")
    model = MLForge_client.create_logged_model(experiment_id=exp_id)
    MLForge_client.delete_logged_model(model.model_id)
    with pytest.raises(MLForgeException, match="not found"):
        MLForge_client.get_logged_model(model.model_id)

    models = MLForge_client.search_logged_models(experiment_ids=[exp_id])
    assert len(models) == 0


def test_set_logged_model_tags(MLForge_client: MLForgeClient):
    exp_id = MLForge_client.create_experiment("create_logged_model")
    model = MLForge_client.create_logged_model(exp_id)
    MLForge_client.set_logged_model_tags(model.model_id, {"tag1": "value1", "tag2": "value2"})
    loaded_model = MLForge_client.get_logged_model(model.model_id)
    assert loaded_model.tags == {"tag1": "value1", "tag2": "value2"}

    MLForge_client.set_logged_model_tags(model.model_id, {"tag1": "value3"})
    loaded_model = MLForge_client.get_logged_model(model.model_id)
    assert loaded_model.tags == {"tag1": "value3", "tag2": "value2"}


def test_delete_logged_model_tag(MLForge_client: MLForgeClient):
    exp_id = MLForge_client.create_experiment("create_logged_model")
    model = MLForge_client.create_logged_model(exp_id)
    MLForge_client.set_logged_model_tags(model.model_id, {"tag1": "value1", "tag2": "value2"})
    MLForge_client.delete_logged_model_tag(model.model_id, "tag1")
    loaded_model = MLForge_client.get_logged_model(model.model_id)
    assert loaded_model.tags == {"tag2": "value2"}

    with pytest.raises(MLForgeException, match="No tag with key"):
        MLForge_client.delete_logged_model_tag(model.model_id, "tag1")


def test_search_logged_models(MLForge_client: MLForgeClient):
    exp_id = MLForge_client.create_experiment("create_logged_model")
    model_1 = MLForge_client.create_logged_model(exp_id)
    time.sleep(0.001)  # to ensure different created time
    models = MLForge_client.search_logged_models(experiment_ids=[exp_id])
    assert [m.name for m in models] == [model_1.name]

    # max_results
    model_2 = MLForge_client.create_logged_model(exp_id)
    page_1 = MLForge_client.search_logged_models(experiment_ids=[exp_id], max_results=1)
    assert [m.name for m in page_1] == [model_2.name]
    assert page_1.token is not None

    # pagination
    page_2 = MLForge_client.search_logged_models(
    ...
    # filter_string
    models = MLForge_client.search_logged_models(
        experiment_ids=[exp_id], filter_string=f"name = {model_1.name!r}"
    )
    assert [m.name for m in models] == [model_1.name]

    # datasets
    run_1 = MLForge_client.create_run(exp_id)
    MLForge_client.log_metric(
        run_1.info.run_id,
        key="metric",
        value=1,
...
def test_log_outputs(MLForge_client: MLForgeClient):
    exp_id = MLForge_client.create_experiment("log_outputs")
    run = MLForge_client.create_run(experiment_id=exp_id)
    model = MLForge_client.create_logged_model(experiment_id=exp_id)
    model_outputs = [LoggedModelOutput(model.model_id, 1)]
    MLForge_client.log_outputs(run.info.run_id, model_outputs)
    run = MLForge_client.get_run(run.info.run_id)
    assert run.outputs.model_outputs == model_outputs


def test_list_logged_model_artifacts(MLForge_client: MLForgeClient):
    class Model(MLForge.pyfunc.PythonModel):
        def predict(self, context, model_input):
            return model_input

    MLForge.set_tracking_uri(MLForge_client.tracking_uri)
    model_info = MLForge.pyfunc.log_model(name="model", python_model=Model())
    resp = requests.get(
        f"{MLForge_client.tracking_uri}/ajax-api/2.0/MLForge/logged-models/{model_info.model_id}/artifacts/directories"
    )
...
def test_get_logged_model_artifact(MLForge_client: MLForgeClient):
    class Model(MLForge.pyfunc.PythonModel):
        def predict(self, context, model_input):
            return model_input
    ...
    model_info = MLForge.pyfunc.log_model(name="model", python_model=Model())
    resp = requests.get(
        f"{MLForge_client.tracking_uri}/ajax-api/2.0/MLForge/logged-models/{model_info.model_id}/artifacts/files",
...
def test_suppress_url_printing(MLForge_client: MLForgeClient, monkeypatch):
    monkeypatch.setenv(MLForge_SUPPRESS_PRINTING_URL_TO_STDOUT.name, "true")
    exp_id = MLForge_client.create_experiment("test_suppress_url_printing")
    run = MLForge_client.create_run(experiment_id=exp_id)
    captured_output = StringIO()
    monkeypatch.setattr(sys, "stdout", captured_output)
    MLForge_client._tracking_client._log_url(run.info.run_id)
    assert captured_output.getvalue() == ""


def test_log_url_includes_workspace_when_set(MLForge_client: MLForgeClient, monkeypatch):
    exp_id = MLForge_client.create_experiment("test_log_url_workspace")
    run = MLForge_client.create_run(experiment_id=exp_id)
    captured_output = StringIO()
    monkeypatch.setattr(sys, "stdout", captured_output)
    monkeypatch.setattr(
        "MLForge.tracking._tracking_service.client.get_workspace_url", lambda: "http://localhost"
    )
    monkeypatch.setattr(
        "MLForge.tracking._tracking_service.client.get_request_workspace", lambda: "team-space"
    )
...
def test_assessments_end_to_end(MLForge_client):
    MLForge.set_tracking_uri(MLForge_client.tracking_uri)

    # Set up experiment and trace
    experiment_id = MLForge_client.create_experiment("assessment_crud_test")
    trace_info = MLForge_client.start_trace(name="test_trace", experiment_id=experiment_id)
    MLForge_client.end_trace(request_id=trace_info.request_id)
    MLForge.flush_trace_async_logging()
    ...
    create_response = requests.post(
        f"{MLForge_client.tracking_uri}/api/3.0/MLForge/traces/{trace_info.request_id}/assessments",
    ...
    get_response = requests.get(
        f"{MLForge_client.tracking_uri}/api/3.0/MLForge/traces/{trace_info.request_id}/assessments/{assessment_id}"
    )
    ...
    get_override = requests.get(
        f"{MLForge_client.tracking_uri}/api/3.0/MLForge/traces/{trace_info.request_id}/assessments/{override_id}"
    )
    ...
    for aid in [assessment_id, expectation_id]:
        delete_resp = requests.delete(
            f"{MLForge_client.tracking_uri}/api/3.0/MLForge/traces/{trace_info.request_id}/assessments/{aid}"
        )
        assert delete_resp.status_code == 200


def test_graphql_nan_metric_handling(MLForge_client):
    experiment_id = MLForge_client.create_experiment("test_graphql_nan_metrics")
    created_run = MLForge_client.create_run(experiment_id)
    run_id = created_run.info.run_id

    # Log a normal metric and a NaN metric
    MLForge_client.log_metric(run_id, key="normal_metric", value=123, timestamp=1, step=1)
    MLForge_client.log_metric(run_id, key="nan_metric", value=math.nan, timestamp=2, step=2)
    ...
    metrics = json_response["data"]["MLForgeGetRun"]["run"]["data"]["metrics"]
...
def test_create_and_get_evaluation_dataset(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("Evaluation datasets not supported for FileStore")

    experiment_id = MLForge_client.create_experiment("eval_dataset_test")

    dataset = MLForge_client.create_dataset(
    ...
    retrieved = MLForge_client.get_dataset(dataset.dataset_id)
    assert retrieved.name == dataset.name
    assert retrieved.dataset_id == dataset.dataset_id
    assert retrieved.tags == dataset.tags


def test_search_evaluation_datasets(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("Evaluation datasets not supported for FileStore")

    exp1 = MLForge_client.create_experiment("eval_search_exp1")
    exp2 = MLForge_client.create_experiment("eval_search_exp2")

    MLForge_client.create_dataset(
    ...
    all_datasets = MLForge_client.search_datasets()
    assert len(all_datasets) >= 3

    exp1_datasets = MLForge_client.search_datasets(experiment_ids=exp1)
    ...
    ml_datasets = MLForge_client.search_datasets(filter_string="tags.team = 'ml'")
    ...
    ordered_datasets = MLForge_client.search_datasets(order_by=["name ASC"])
    names = [d.name for d in ordered_datasets]
    assert names == sorted(names)


def test_evaluation_dataset_tag_operations(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("Evaluation datasets not supported for FileStore")

    experiment_id = MLForge_client.create_experiment("eval_tags_test")
    ...
    MLForge_client.set_dataset_tags(dataset.dataset_id, {"env": "staging", "new_tag": "new_value"})

    updated = MLForge_client.get_dataset(dataset.dataset_id)
    ...
    MLForge_client.delete_dataset_tag(dataset.dataset_id, "new_tag")

    final = MLForge_client.get_dataset(dataset.dataset_id)
    assert "new_tag" not in final.tags
    assert final.tags["env"] == "staging"  # Other tags preserved


def test_evaluation_dataset_delete(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("Evaluation datasets not supported for FileStore")

    experiment_id = MLForge_client.create_experiment("eval_delete_test")
    ...
    MLForge_client.delete_dataset(dataset.dataset_id)

    with pytest.raises(MLForgeException, match="not found"):
        MLForge_client.get_dataset(dataset.dataset_id)


def test_evaluation_dataset_upsert_records(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("Evaluation datasets not supported for FileStore")

    experiment_id = MLForge_client.create_experiment("upsert_records_test")
    ...
    initial_records = [
        {
            "inputs": {"question": "What is MLForge?"},
            "expectations": {"answer": "MLForge is an ML platform"},
    ...
    # NB: MLForgeClient doesn't have upsert_dataset_records method - merge_records() calls
    # the store directly. We make HTTP requests here to test the REST API handler end-to-end.
    response = requests.post(
        f"{MLForge_client.tracking_uri}/api/3.0/MLForge/datasets/{dataset.dataset_id}/records",
    ...
    update_records = [
        {
            "inputs": {"question": "What is MLForge?"},
            "expectations": {"answer": "MLForge is an open-source ML platform"},
    ...
    response = requests.post(
        f"{MLForge_client.tracking_uri}/api/3.0/MLForge/datasets/invalid-id/records",
        json={"records": json.dumps(initial_records)},
    )
    assert response.status_code != 200


def test_add_dataset_to_experiments_rest_tracking(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("File store doesn't support dataset operations")
    exp1 = MLForge_client.create_experiment("dataset_exp_1")
    exp2 = MLForge_client.create_experiment("dataset_exp_2")
    exp3 = MLForge_client.create_experiment("dataset_exp_3")
...
def test_remove_dataset_from_experiments_rest_tracking(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("File store doesn't support dataset operations")
    exp1 = MLForge_client.create_experiment("dataset_remove_exp_1")
    exp2 = MLForge_client.create_experiment("dataset_remove_exp_2")
    exp3 = MLForge_client.create_experiment("dataset_remove_exp_3")
...
def test_add_multiple_experiments_at_once_rest_tracking(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("File store doesn't support dataset operations")
    exps = [MLForge_client.create_experiment(f"bulk_add_exp_{i}") for i in range(5)]
...
def test_dataset_experiment_association_error_cases_rest_tracking(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("File store doesn't support dataset operations")
    exp1 = MLForge_client.create_experiment("error_test_exp")
...
def test_idempotent_add_experiments_rest_tracking(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("File store doesn't support dataset operations")
    exp1 = MLForge_client.create_experiment("idempotent_test_exp_1")
    exp2 = MLForge_client.create_experiment("idempotent_test_exp_2")
...
def test_idempotent_remove_experiments_rest_tracking(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("File store doesn't support dataset operations")
    exp1 = MLForge_client.create_experiment("remove_idempotent_test_exp_1")
    exp2 = MLForge_client.create_experiment("remove_idempotent_test_exp_2")
...
def test_client_api_add_remove_experiments_rest_tracking(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("File store doesn't support dataset operations")
    exp1 = MLForge_client.create_experiment("client_api_exp_1")
    exp2 = MLForge_client.create_experiment("client_api_exp_2")
    exp3 = MLForge_client.create_experiment("client_api_exp_3")
    ...
    updated_dataset = MLForge_client.add_dataset_to_experiments(
        dataset_id=dataset.dataset_id,
        experiment_ids=[exp2, exp3],
    )
    ...
    updated_dataset = MLForge_client.remove_dataset_from_experiments(
        dataset_id=dataset.dataset_id,
        experiment_ids=[exp2],
    )
...
def test_scorer_CRUD(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("File store doesn't support scorer CRUD operations")
    experiment_id = MLForge_client.create_experiment("test_scorer_api_experiment")

    # Get the RestStore object directly
    store = MLForge_client._tracking_client.store
...
def test_online_scoring_config(MLForge_client_with_secrets, filter_string):
    """
    ...
    """
    experiment_id = MLForge_client_with_secrets.create_experiment("test_online_scoring")
    store = MLForge_client_with_secrets._tracking_client.store
...
@pytest.mark.asyncio
async def test_rest_store_logs_spans_via_otel_endpoint(MLForge_client, store_type, use_async):
    """
    End-to-end test that verifies RestStore can log spans to a running server via OTLP endpoint.

    This test:
    1. Creates spans using MLForge's span entities
    ...
    experiment_id = MLForge_client.create_experiment(f"rest_store_otel_test_{use_async}")
    root_span = MLForge_client.start_trace(
    ...
    MLForge_span_to_log = Span(otel_span)
    # Call either sync or async version based on parametrization
    if use_async:
        # Use await to execute the async method
        result_spans = await MLForge_client._tracking_client.store.log_spans_async(
            location=experiment_id, spans=[MLForge_span_to_log]
        )
    else:
        result_spans = MLForge_client._tracking_client.store.log_spans(
            location=experiment_id, spans=[MLForge_span_to_log]
        )
...
def test_create_and_get_secret(MLForge_client_with_secrets):
    store = MLForge_client_with_secrets._tracking_client.store
...
def test_update_secret(MLForge_client_with_secrets):
    store = MLForge_client_with_secrets._tracking_client.store
...
def test_list_secret_infos(MLForge_client_with_secrets):
    store = MLForge_client_with_secrets._tracking_client.store
...
def test_delete_secret(MLForge_client_with_secrets):
    store = MLForge_client_with_secrets._tracking_client.store
...
def test_create_secret_with_dict_value(MLForge_client_with_secrets):
    store = MLForge_client_with_secrets._tracking_client.store
...
def test_update_secret_with_dict_value(MLForge_client_with_secrets):
    store = MLForge_client_with_secrets._tracking_client.store
...
def test_create_and_update_compound_secret_via_rest(MLForge_client_with_secrets):
    store = MLForge_client_with_secrets._tracking_client.store
...
def test_create_and_get_endpoint(MLForge_client_with_secrets):
    store = MLForge_client_with_secrets._tracking_client.store
...
def test_create_endpoint_with_usage_tracking(MLForge_client_with_secrets):
    store = MLForge_client_with_secrets._tracking_client.store
    ...
    experiment = MLForge_client_with_secrets.get_experiment(experiment_id)
    assert experiment.name == "gateway/usage-tracking-endpoint"


def test_update_endpoint(MLForge_client_with_secrets):
    store = MLForge_client_with_secrets._tracking_client.store
...
def test_list_endpoints(MLForge_client_with_secrets):
    store = MLForge_client_with_secrets._tracking_client.store
...
def test_delete_endpoint(MLForge_client_with_secrets):
    store = MLForge_client_with_secrets._tracking_client.store
...
def test_model_definitions(MLForge_client_with_secrets):
    store = MLForge_client_with_secrets._tracking_client.store
...
def test_attach_detach_model_to_endpoint(MLForge_client_with_secrets):
    store = MLForge_client_with_secrets._tracking_client.store
...
def test_endpoint_bindings(MLForge_client_with_secrets):
    store = MLForge_client_with_secrets._tracking_client.store
...
def test_secrets_and_endpoints_integration(MLForge_client_with_secrets):
    store = MLForge_client_with_secrets._tracking_client.store
...
def test_list_providers(MLForge_client_with_secrets):
    import requests

    base_url = MLForge_client_with_secrets._tracking_client.tracking_uri
    response = requests.get(f"{base_url}/ajax-api/3.0/MLForge/gateway/supported-providers")
    assert response.status_code == 200
    data = response.json()
...
def test_list_models(MLForge_client_with_secrets):
    import requests

    base_url = MLForge_client_with_secrets._tracking_client.tracking_uri
    response = requests.get(f"{base_url}/ajax-api/3.0/MLForge/gateway/supported-models")
    assert response.status_code == 200
    data = response.json()
    ...
    response = requests.get(
        f"{base_url}/ajax-api/3.0/MLForge/gateway/supported-models", params={"provider": "openai"}
    )
    assert response.status_code == 200
...
def test_get_provider_config(MLForge_client_with_secrets):
    import requests
    ...
    response = requests.get(
        f"{base_url}/ajax-api/3.0/MLForge/gateway/provider-config",
    ...
    response = requests.get(f"{base_url}/ajax-api/3.0/MLForge/gateway/provider-config")
    assert response.status_code == 400


def test_get_secrets_config_with_custom_passphrase(MLForge_client_with_secrets):
    base_url = MLForge_client_with_secrets._tracking_client.tracking_uri

    response = requests.get(f"{base_url}/ajax-api/3.0/MLForge/gateway/secrets/config")
    assert response.status_code == 200
    data = response.json()
...
def test_get_secrets_config_with_default_passphrase(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.delenv("MLForge_CRYPTO_KEK_PASSPHRASE", raising=False)
    ...
    with ServerThread(app, get_safe_port()) as url:
        response = requests.get(f"{url}/ajax-api/3.0/MLForge/gateway/secrets/config")
        assert response.status_code == 200
        data = response.json()
        assert data["secrets_available"] is True
...
def test_endpoint_with_orphaned_model_definition(MLForge_client_with_secrets):
    store = MLForge_client_with_secrets._tracking_client.store
...
def test_update_model_definition_provider(MLForge_client_with_secrets):
    store = MLForge_client_with_secrets._tracking_client.store
...
def test_create_issue_with_all_fields(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("Issues are only supported in SqlAlchemyStore")

    MLForge.set_tracking_uri(MLForge_client.tracking_uri)
    experiment_id = MLForge_client.create_experiment("Issue Test")
    run = MLForge_client.create_run(experiment_id)

    response = requests.post(
        f"{MLForge_client.tracking_uri}/api/3.0/MLForge/issues",
        json={
            "experiment_id": experiment_id,
...
def test_create_issue_minimal_fields(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("Issues are only supported in SqlAlchemyStore")
    experiment_id = MLForge_client.create_experiment("Issue Test Minimal")
...
def test_create_issue_with_required_fields(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("Issues are only supported in SqlAlchemyStore")
    experiment_id = MLForge_client.create_experiment("Issue Test Required Fields")
...
def test_create_issue_invalid_experiment(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("Issues are only supported in SqlAlchemyStore")
    response = requests.post(
        f"{MLForge_client.tracking_uri}/api/3.0/MLForge/issues",
...
def test_get_issue(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("Issues are only supported in SqlAlchemyStore")
    experiment_id = MLForge_client.create_experiment("Issue Test Get")
    ...
    get_response = requests.get(f"{MLForge_client.tracking_uri}/api/3.0/MLForge/issues/{issue_id}")
    assert get_response.status_code == 200
...
def test_get_issue_not_found(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("Issues are only supported in SqlAlchemyStore")
    response = requests.get(f"{MLForge_client.tracking_uri}/api/3.0/MLForge/issues/nonexistent-issue")
    assert response.status_code == 404
    data = response.json()
    assert data["error_code"] == "RESOURCE_DOES_NOT_EXIST"


def test_update_issue(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("Issues are only supported in SqlAlchemyStore")
    experiment_id = MLForge_client.create_experiment("Issue Test Update")
    ...
    update_response = requests.patch(
        f"{MLForge_client.tracking_uri}/api/3.0/MLForge/issues/{issue_id}",
        json={
            "issue_id": issue_id,
            "name": "Updated name",
...
def test_search_issues_no_filters(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("Issues are only supported in SqlAlchemyStore")
    experiment_id = MLForge_client.create_experiment("Issue Test Search")
    ...
    search_response = requests.post(
        f"{MLForge_client.tracking_uri}/api/3.0/MLForge/issues/search", json={}
    )
...
def test_search_issues_by_experiment(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("Issues are only supported in SqlAlchemyStore")
    exp1 = MLForge_client.create_experiment("Issue Test Search Exp1")
    exp2 = MLForge_client.create_experiment("Issue Test Search Exp2")
    ...
    search_response = requests.post(
        f"{MLForge_client.tracking_uri}/api/3.0/MLForge/issues/search",
...
def test_search_issues_by_status(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("Issues are only supported in SqlAlchemyStore")
    experiment_id = MLForge_client.create_experiment("Issue Test Search Status")
...
def test_search_issues_with_pagination(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("Issues are only supported in SqlAlchemyStore")
    experiment_id = MLForge_client.create_experiment("Issue Test Pagination")
...
def test_search_issues_sorted_by_timestamp(MLForge_client, store_type):
    if store_type == "file":
        pytest.skip("Issues are only supported in SqlAlchemyStore")
    experiment_id = MLForge_client.create_experiment("Issue Test Sort")

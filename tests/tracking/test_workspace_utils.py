...
from MLForge.utils.workspace_utils import resolve_workspace_store_uri, set_workspace_store_uri
...
def _reset_workspace_uri(monkeypatch):
    ...
    monkeypatch.delenv(MLForge_TRACKING_URI.name, raising=False)
    yield
    set_workspace_store_uri(None)
    monkeypatch.delenv(MLForge_WORKSPACE_STORE_URI.name, raising=False)
    monkeypatch.delenv(MLForge_TRACKING_URI.name, raising=False)
...
def test_resolve_workspace_uri_prefers_explicit_argument(monkeypatch):
    monkeypatch.setenv(MLForge_WORKSPACE_STORE_URI.name, "sqlite:///env-workspaces.db")
...
def test_resolve_workspace_uri_uses_environment(monkeypatch):
    monkeypatch.setenv(MLForge_WORKSPACE_STORE_URI.name, "sqlite:///env.db")

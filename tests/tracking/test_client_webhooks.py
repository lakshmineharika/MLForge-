...
from MLForge.environment_variables import MLForge_WEBHOOK_SECRET_ENCRYPTION_KEY
...
@pytest.fixture
def client(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Iterator[MLForgeClient]:
    """Setup a local MLForge server with proper webhook encryption key support."""
    # Set up encryption key for webhooks using monkeypatch
    encryption_key = Fernet.generate_key().decode("utf-8")
    monkeypatch.setenv(MLForge_WEBHOOK_SECRET_ENCRYPTION_KEY.name, encryption_key)
...
def test_create_webhook(client: MLForgeClient):
...
def test_get_webhook(client: MLForgeClient):
    events = [WebhookEvent(WebhookEntity.MODEL_VERSION, WebhookAction.CREATED)]
...
def test_update_webhook(client: MLForgeClient):
    events = [WebhookEvent(WebhookEntity.MODEL_VERSION, WebhookAction.CREATED)]
    webhook = client.create_webhook(
...
def test_update_webhook_partial(client: MLForgeClient):
    events = [WebhookEvent(WebhookEntity.MODEL_VERSION, WebhookAction.CREATED)]
    webhook = client.create_webhook(
        name="original_name",
...
def test_update_webhook_not_found(client: MLForgeClient):
    with pytest.raises(MLForgeException, match="Webhook with ID nonexistent not found"):
...
def test_update_webhook_invalid_urls(client: MLForgeClient, invalid_url: str, expected_match: str):
...
def test_delete_webhook(client: MLForgeClient):
    events = [WebhookEvent(WebhookEntity.MODEL_VERSION, WebhookAction.CREATED)]
    webhook = client.create_webhook(
        name="test_webhook",
        url="https://example.com/webhook",
...
def test_delete_webhook_not_found(client: MLForgeClient):
    with pytest.raises(MLForgeException, match="Webhook with ID nonexistent not found"):

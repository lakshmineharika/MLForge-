...
def test_artifact_can_be_downloaded_from_absolute_uri_successfully(tmp_path):
    ...
    with MLForge.start_run():
        MLForge.log_artifact(local_path=local_artifact_path, artifact_path=logged_artifact_path)
        artifact_uri = MLForge.get_artifact_uri(artifact_path=logged_artifact_path)
...
def test_download_artifact_from_absolute_uri_persists_data_to_specified_output_directory(tmp_path):
    ...
    with MLForge.start_run():
        MLForge.log_artifact(local_path=local_artifact_path, artifact_path=logged_artifact_subdir)
        artifact_uri = MLForge.get_artifact_uri(artifact_path=logged_artifact_subdir)
...
def test_upload_artifacts_to_databricks():
    import_root = "MLForge.tracking.artifact_utils"
    with (
        ...
        repo_mock.assert_called_once_with(
            "dbfs://registry:ws@databricks/databricks/MLForge/tmp-external-source/"
        )
        assert new_source == "dbfs:/databricks/MLForge/tmp-external-source/runid12345/sourcedir"
...
def test_upload_artifacts_to_databricks_no_run_id():
    ...
    with (
        ...
        repo_mock.assert_called_once_with(
            "dbfs://registry@databricks/databricks/MLForge/tmp-external-source/"
        )
        assert (
            new_source == "dbfs:/databricks/MLForge/tmp-external-source/"

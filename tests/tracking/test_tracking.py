...
import MLForge
from MLForge import MLForgeClient, tracking
from MLForge.entities import LifecycleStage, Metric, Param, RunStatus, RunTag, ViewType
from MLForge.environment_variables import (
    MLForge_ASYNC_LOGGING_THREADPOOL_SIZE,
    MLForge_RUN_ID,
)
...
from MLForge.store.tracking.file_store import FileStore
from MLForge.tracking._tracking_service.client import TrackingServiceClient
from MLForge.tracking.fluent import start_run
from MLForge.utils.file_utils import local_file_uri_to_path
...
from MLForge.utils.validation import (
...
def test_create_experiment():
    with pytest.raises(MLForgeException, match="Invalid experiment name"):
        MLForge.create_experiment(None)

    with pytest.raises(MLForgeException, match="Invalid experiment name"):
        MLForge.create_experiment("")

    exp_id = MLForge.create_experiment(f"Some random experiment name {random.randint(1, int(1e6))}")
...
def test_create_experiment_with_duplicate_name():
    ...
    with pytest.raises(MLForgeException, match=re.escape(f"Experiment(name={name}) already exists")):
        MLForge.create_experiment(name)

    tracking.MLForgeClient().delete_experiment(exp_id)
    with pytest.raises(MLForgeException, match=re.escape(f"Experiment(name={name}) already exists")):
        MLForge.create_experiment(name)
...
def test_create_experiments_with_bad_names():
    # None for name
    with pytest.raises(MLForgeException, match="Invalid experiment name: 'None'"):
        MLForge.create_experiment(None)
...
def test_set_experiment_by_id():
    name = "random_exp"
    exp_id = MLForge.create_experiment(name)
    active_exp = MLForge.set_experiment(experiment_id=exp_id)
    assert active_exp.experiment_id == exp_id
    with start_run() as run:
        assert run.info.experiment_id == exp_id

    nonexistent_id = "-1337"
    with pytest.raises(MLForgeException, match="No Experiment with id=-1337 exists") as exc:
        MLForge.set_experiment(experiment_id=nonexistent_id)
...
def test_set_experiment_parameter_validation():
    with pytest.raises(MLForgeException, match="Must specify exactly one") as exc:
        MLForge.set_experiment()
    assert exc.value.error_code == ErrorCode.Name(INVALID_PARAMETER_VALUE)

    with pytest.raises(MLForgeException, match="Must specify exactly one") as exc:
        MLForge.set_experiment(None)
    assert exc.value.error_code == ErrorCode.Name(INVALID_PARAMETER_VALUE)

    with pytest.raises(MLForgeException, match="Must specify exactly one") as exc:
        MLForge.set_experiment(None, None)
    assert exc.value.error_code == ErrorCode.Name(INVALID_PARAMETER_VALUE)

    with pytest.raises(MLForgeException, match="Must specify exactly one") as exc:
        MLForge.set_experiment("name", "id")
    assert exc.value.error_code == ErrorCode.Name(INVALID_PARAMETER_VALUE)
...
def test_set_experiment_with_deleted_experiment():
    ...
    with pytest.raises(MLForgeException, match="Cannot set a deleted experiment") as exc:
        MLForge.set_experiment(name)
    assert exc.value.error_code == ErrorCode.Name(INVALID_PARAMETER_VALUE)
...
def test_start_run_context_manager():
    with start_run() as first_run:
        ...
        persisted_run = tracking.MLForgeClient().get_run(first_uuid)
        assert persisted_run is not None
        assert persisted_run.info == first_run.info
    finished_run = tracking.MLForgeClient().get_run(first_uuid)
    ...
    finished_run2 = tracking.MLForgeClient().get_run(second_run_id)
...
def test_start_and_end_run():
    ...
    with start_run() as active_run:
        MLForge.log_metric("name_1", 25)
    finished_run = tracking.MLForgeClient().get_run(active_run.info.run_id)
    # Validate metrics
...
def test_metric_timestamp():
    with MLForge.start_run() as active_run:
        MLForge.log_metric("name_1", 25)
        MLForge.log_metric("name_1", 30)
...
def test_log_batch():
    ...
    with start_run() as active_run:
        run_id = active_run.info.run_id
        MLForgeClient().log_batch(run_id=run_id, metrics=metrics, params=params, tags=tags)
    client = tracking.MLForgeClient()
    finished_run = client.get_run(run_id)
    # Validate metrics
...
def test_log_batch_with_many_elements():
    ...
    with start_run() as active_run:
        run_id = active_run.info.run_id
        MLForge.tracking.MLForgeClient().log_batch(
...
def test_log_metric():
    with start_run() as active_run, mock.patch("time.time", return_value=123):
        run_id = active_run.info.run_id
        MLForge.log_metric("name_1", 25)
        MLForge.log_metric("name_2", -3)
        MLForge.log_metric("name_1", 30, 5)
        MLForge.log_metric("name_1", 40, -2)
        MLForge.log_metric("nested/nested/name", 40)
    finished_run = tracking.MLForgeClient().get_run(run_id)
    # Validate metrics
...
def test_log_metrics_uses_millisecond_timestamp_resolution_fluent():
    with start_run() as active_run, mock.patch("time.time") as time_mock:
        ...
        MLForge.log_metrics({"name_1": 25, "name_2": -3})
        MLForge.log_metrics({"name_1": 30})
        MLForge.log_metrics({"name_1": 40})
        run_id = active_run.info.run_id
...
def test_log_metrics_uses_millisecond_timestamp_resolution_client():
    with start_run() as active_run, mock.patch("time.time") as time_mock:
        time_mock.side_effect = lambda: 123
        MLForge_client = tracking.MLForgeClient()
        run_id = active_run.info.run_id

        MLForge_client.log_metric(run_id=run_id, key="name_1", value=25)
        MLForge_client.log_metric(run_id=run_id, key="name_2", value=-3)
        MLForge_client.log_metric(run_id=run_id, key="name_1", value=30)
        MLForge_client.log_metric(run_id=run_id, key="name_1", value=40)

    metric_history_name1 = MLForge_client.get_metric_history(run_id, "name_1")
    assert {(m.value, m.timestamp) for m in metric_history_name1} == {
        (25, 123 * 1000),
        (30, 123 * 1000),
        (40, 123 * 1000),
    ...
    metric_history_name2 = MLForge_client.get_metric_history(run_id, "name_2")
    assert {(m.value, m.timestamp) for m in metric_history_name2} == {(-3, 123 * 1000)}
...
def test_log_metrics_uses_common_timestamp_and_step_per_invocation(step_kwarg):
    ...
    with start_run() as active_run:
        run_id = active_run.info.run_id
        MLForge.log_metrics(expected_metrics, step=step_kwarg)
    finished_run = tracking.MLForgeClient().get_run(run_id)
...
@pytest.fixture
def get_store_mock():
    with mock.patch("MLForge.store.file_store.FileStore.log_batch") as _get_store_mock:
...
def test_set_tags():
    ...
    with start_run() as active_run:
        run_id = active_run.info.run_id
        MLForge.set_tags(exact_expected_tags)
    finished_run = tracking.MLForgeClient().get_run(run_id)
    # Validate tags
    assert len(finished_run.data.tags) == len(exact_expected_tags) + len(approx_expected_tags)
...
def test_log_metric_validation():
    with start_run() as active_run:
        ...
        with pytest.raises(
            ...
            MLForge.log_metric("name_1", "apple")
...
def test_log_param():
    with start_run() as active_run:
        run_id = active_run.info.run_id
        assert MLForge.log_param("name_1", "a") == "a"
        assert MLForge.log_param("name_2", "b") == "b"
        assert MLForge.log_param("nested/nested/name", 5) == 5
    finished_run = tracking.MLForgeClient().get_run(run_id)
    # Validate params
...
def test_log_params():
    ...
    with start_run() as active_run:
        run_id = active_run.info.run_id
        MLForge.log_params(expected_params)
    finished_run = tracking.MLForgeClient().get_run(run_id)
    # Validate params
    assert finished_run.data.params == {
        "name_1": "c",
...
def test_log_params_duplicate_keys_raises():
    ...
    with start_run() as active_run:
        run_id = active_run.info.run_id
        MLForge.log_params(params)
        with pytest.raises(
            expected_exception=MLForgeException,
            match=r"Changing param values is not allowed. Param with key=",
        ) as e:
            MLForge.log_param("a", "3")
        assert e.value.error_code == ErrorCode.Name(INVALID_PARAMETER_VALUE)
    finished_run = tracking.MLForgeClient().get_run(run_id)
...
def test_param_metric_with_colon():
    with start_run() as active_run:
        run_id = active_run.info.run_id
        MLForge.log_param("a:b", 3)
        MLForge.log_metric("c:d", 4)
    finished_run = tracking.MLForgeClient().get_run(run_id)
...
def test_log_batch_duplicate_entries_raises():
    with start_run() as active_run:
        run_id = active_run.info.run_id
        with pytest.raises(
            MLForgeException, match=r"Duplicate parameter keys have been submitted."
        ) as e:
            tracking.MLForgeClient().log_batch(
...
def test_log_batch_validates_entity_names_and_values():
    with start_run() as active_run:
        ...
        with pytest.raises(
            ...
            tracking.MLForgeClient().log_batch(run_id, metrics=metrics)
        assert e.value.error_code == ErrorCode.Name(INVALID_PARAMETER_VALUE)
        ...
        with pytest.raises(
            ...
            tracking.MLForgeClient().log_batch(run_id, params=params)
        assert e.value.error_code == ErrorCode.Name(INVALID_PARAMETER_VALUE)
        ...
        with pytest.raises(
            ...
            tracking.MLForgeClient().log_batch(run_id, tags=tags)
        assert e.value.error_code == ErrorCode.Name(INVALID_PARAMETER_VALUE)
...
def test_log_artifact_with_dirs(tmp_path):
    ...
    sub_dir.mkdir()
    with start_run():
        artifact_uri = MLForge.get_artifact_uri()
        run_artifact_dir = local_file_uri_to_path(artifact_uri)
        MLForge.log_artifact(str(art_dir))
    ...
    with start_run():
        artifact_uri = MLForge.get_artifact_uri()
        run_artifact_dir = local_file_uri_to_path(artifact_uri)
        MLForge.log_artifact(str(art_dir), "some_parent")
    ...
    with start_run():
        artifact_uri = MLForge.get_artifact_uri()
        run_artifact_dir = local_file_uri_to_path(artifact_uri)
        MLForge.log_artifact(str(art_dir), "parent/and_child")
...
def test_log_artifact(tmp_path):
    ...
    for parent_dir in artifact_parent_dirs:
        with start_run():
            artifact_uri = MLForge.get_artifact_uri()
            run_artifact_dir = local_file_uri_to_path(artifact_uri)
            MLForge.log_artifact(path0, parent_dir)
        expected_dir = (
    ...
    for parent_dir in artifact_parent_dirs:
        with start_run():
            ...
            MLForge.log_artifacts(artifact_dir, parent_dir)
...
def test_log_text(subdir):
    ...
    with MLForge.start_run():
        MLForge.log_text(text, artifact_file)

        artifact_path = None if subdir is None else posixpath.normpath(subdir)
        artifact_uri = MLForge.get_artifact_uri(artifact_path)
        run_artifact_dir = local_file_uri_to_path(artifact_uri)
...
def test_log_dict(subdir, extension):
    ...
    with MLForge.start_run():
        MLForge.log_dict(dictionary, artifact_file)
...
def test_log_stream_bytes(subdir):
    ...
    with MLForge.start_run():
        ...
        MLForge.log_stream(stream, artifact_file)
...
def test_log_stream_empty():
    with MLForge.start_run():
        ...
        stream = io.BytesIO(b"")
        MLForge.log_stream(stream, "empty.bin")
...
def test_log_stream_large_content():
    with MLForge.start_run():
        ...
        MLForge.log_stream(stream, "large.bin")
...
def test_with_startrun():
    ...
    with MLForge.start_run() as active_run:
        assert MLForge.active_run() == active_run
        run_id = active_run.info.run_id
    t1 = get_current_time_millis()
    run_info = MLForge.tracking._get_store().get_run(run_id).info
...
def test_parent_create_run(monkeypatch):
    ...
    monkeypatch.setenv(MLForge_RUN_ID.name, parent_run_id)
    with MLForge.start_run() as parent_run:
        ...
        with MLForge.start_run(nested=True) as child_run:
            ...
            with MLForge.start_run(nested=True) as grand_child_run:
                pass

    def verify_has_parent_id_tag(child_id, expected_parent_id):
        tags = tracking.MLForgeClient().get_run(child_id).data.tags
        assert tags[MLForge_PARENT_RUN_ID] == expected_parent_id
...
def test_start_deleted_run():
    ...
    with MLForge.start_run() as active_run:
        run_id = active_run.info.run_id
    tracking.MLForgeClient().delete_run(run_id)
    with pytest.raises(MLForgeException, match="because it is in the deleted state."):
...
def test_start_run_exp_id_0():
    MLForge.set_experiment("some-experiment")
    ...
    with MLForge.start_run() as active_run:
        ...
        assert MLForgeClient().get_experiment(exp_id).name == "some-experiment"
    # Set experiment ID to 0 when creating a run, verify that the specified experiment ID is honored
    with MLForge.start_run(experiment_id=0) as active_run:
...
def test_get_artifact_uri_with_artifact_path_unspecified_returns_artifact_root_dir():
    with MLForge.start_run() as active_run:
        assert MLForge.get_artifact_uri(artifact_path=None) == active_run.info.artifact_uri


def test_get_artifact_uri_uses_currently_active_run_id():
    artifact_path = "artifact"
    with MLForge.start_run() as active_run:
        assert MLForge.get_artifact_uri(
...
def _assert_get_artifact_uri_appends_to_uri_path_component_correctly(
    ...
    MLForge.set_experiment("get-artifact-uri-test")
    with MLForge.start_run():
...
def test_search_runs():
    MLForge.set_experiment("exp-for-search")
    # Create a run and verify that the current active experiment is the one we just set
    logged_runs = {}
    with MLForge.start_run() as active_run:
        logged_runs["first"] = active_run.info.run_id
        MLForge.log_metric("m1", 0.001)
        MLForge.log_metric("m2", 0.002)
        MLForge.log_metric("m1", 0.002)
        MLForge.log_param("p1", "a")
        MLForge.set_tag("t1", "first-tag-val")
    with MLForge.start_run() as active_run:
        logged_runs["second"] = active_run.info.run_id
        MLForge.log_metric("m1", 0.008)
        MLForge.log_param("p2", "aa")
        MLForge.set_tag("t2", "second-tag-val")

    def verify_runs(runs, expected_set):
        assert {r.info.run_id for r in runs} == {logged_runs[r] for r in expected_set}

    experiment_id = MLForgeClient().get_experiment_by_name("exp-for-search").experiment_id

    # 2 runs in this experiment
    assert len(MLForgeClient().search_runs([experiment_id], run_view_type=ViewType.ACTIVE_ONLY)) == 2

    # 2 runs that have metric "m1" > 0.001
    runs = MLForgeClient().search_runs([experiment_id], "metrics.m1 > 0.0001")
    verify_runs(runs, ["first", "second"])

    # 1 run with has metric "m1" > 0.002
    runs = MLForgeClient().search_runs([experiment_id], "metrics.m1 > 0.002")
    verify_runs(runs, ["second"])

    # no runs with metric "m1" > 0.1
    runs = MLForgeClient().search_runs([experiment_id], "metrics.m1 > 0.1")
    verify_runs(runs, [])

    # 1 run with metric "m2" > 0
    runs = MLForgeClient().search_runs([experiment_id], "metrics.m2 > 0")
    verify_runs(runs, ["first"])

    # 1 run each with param "p1" and "p2"
    runs = MLForgeClient().search_runs([experiment_id], "params.p1 = 'a'", ViewType.ALL)
    verify_runs(runs, ["first"])
    runs = MLForgeClient().search_runs([experiment_id], "params.p2 != 'a'", ViewType.ALL)
    verify_runs(runs, ["second"])
    runs = MLForgeClient().search_runs([experiment_id], "params.p2 = 'aa'", ViewType.ALL)
    verify_runs(runs, ["second"])

    # 1 run each with tag "t1" and "t2"
    runs = MLForgeClient().search_runs([experiment_id], "tags.t1 = 'first-tag-val'", ViewType.ALL)
    verify_runs(runs, ["first"])
    runs = MLForgeClient().search_runs([experiment_id], "tags.t2 != 'qwerty'", ViewType.ALL)
    verify_runs(runs, ["second"])
    runs = MLForgeClient().search_runs([experiment_id], "tags.t2 = 'second-tag-val'", ViewType.ALL)
    verify_runs(runs, ["second"])

    # delete "first" run
    MLForgeClient().delete_run(logged_runs["first"])
    runs = MLForgeClient().search_runs([experiment_id], "params.p1 = 'a'", ViewType.ALL)
    verify_runs(runs, ["first"])

    runs = MLForgeClient().search_runs([experiment_id], "params.p1 = 'a'", ViewType.DELETED_ONLY)
    verify_runs(runs, ["first"])

    runs = MLForgeClient().search_runs([experiment_id], "params.p1 = 'a'", ViewType.ACTIVE_ONLY)
    verify_runs(runs, [])


@pytest.mark.usefixtures("reset_active_experiment")
def test_search_runs_multiple_experiments():
    experiment_ids = [MLForge.create_experiment(f"exp__{exp_id}") for exp_id in range(1, 4)]
    for eid in experiment_ids:
        with MLForge.start_run(experiment_id=eid):
            MLForge.log_metric("m0", 1)
            MLForge.log_metric(f"m_{eid}", 2)

    assert len(MLForgeClient().search_runs(experiment_ids, "metrics.m0 > 0", ViewType.ALL)) == 3

    assert len(MLForgeClient().search_runs(experiment_ids, "metrics.m_1 > 0", ViewType.ALL)) == 1
    assert len(MLForgeClient().search_runs(experiment_ids, "metrics.m_2 = 2", ViewType.ALL)) == 1
    assert len(MLForgeClient().search_runs(experiment_ids, "metrics.m_3 < 4", ViewType.ALL)) == 1
...
def test_log_table(file_type):
    ...
    table_dict = {
        "inputs": ["What is MLForge?", "What is Databricks?"],
        "outputs": ["MLForge is ...", "Databricks is ..."],
        "toxicity": [0.0, 0.0],
    }
    artifact_file = f"qabot_eval_results.{file_type}"
    TAG_NAME = "MLForge.loggedArtifacts"
    run_id = None

    with pytest.raises(
        MLForgeException, match="data must be a pandas.DataFrame or a dictionary"
    ) as e:
        with MLForge.start_run() as run:
            # Log the incorrect data format as a table
            MLForge.log_table(data="incorrect-data-format", artifact_file=artifact_file)
    assert e.value.error_code == ErrorCode.Name(INVALID_PARAMETER_VALUE)

    with MLForge.start_run() as run:
        # Log the dictionary as a table
        MLForge.log_table(data=table_dict, artifact_file=artifact_file)
        run_id = run.info.run_id

    run = MLForge.get_run(run_id)
    artifact_path = MLForge.artifacts.download_artifacts(run_id=run_id, artifact_path=artifact_file)
    ...
    with MLForge.start_run(run_id=run_id):
        ...
        MLForge.log_table(data=table_df, artifact_file=artifact_file)
    ...
    with MLForge.start_run(run_id=run_id):
        ...
        MLForge.log_table(data=table_df, artifact_file=artifact_file_new)
...
def test_load_table(file_type):
    ...
    with MLForge.start_run() as run:
        # Log the dictionary as a table
        MLForge.log_table(data=table_dict, artifact_file=artifact_file)
        MLForge.log_table(data=table_dict, artifact_file=artifact_file_2)
    ...
    extra_columns = ["run_id", "tags.MLForge.loggedArtifacts"]

    # test 1: load table with extra columns
    output_df = MLForge.load_table(artifact_file=artifact_file, extra_columns=extra_columns)

    assert output_df.shape[0] == 6
    assert output_df.shape[1] == 5
    assert output_df["run_id"].nunique() == 3
    assert output_df["tags.MLForge.loggedArtifacts"].nunique() == 2

    # test 2: load table with extra columns and single run_id
    output_df = MLForge.load_table(
    ...
    assert output_df["tags.MLForge.loggedArtifacts"].nunique() == 1
    ...
    output_df = MLForge.load_table(artifact_file=artifact_file_2)
    import pandas as pd
    ...
    output_df = MLForge.load_table(artifact_file=artifact_file)
    ...
    with pytest.raises(
        MLForgeException, match="No runs found with the corresponding table artifact"
    ):
        MLForge.load_table(artifact_file=f"error_case.{file_type}")

    # test 7: load table with no matching extra_column found. Error case
    with pytest.raises(KeyError, match="error_column"):
        MLForge.load_table(artifact_file=artifact_file, extra_columns=["error_column"])
...
def test_log_table_with_datetime_columns(file_type):
    ...
    MLForge.log_table(data=table_dict, artifact_file=artifact_file, run_id=run_id)
    artifact_path = MLForge.artifacts.download_artifacts(run_id=run_id, artifact_path=artifact_file)
    if file_type == "parquet":
...
def test_log_table_with_image_columns(file_type):
    ...
    image_path = MLForge.artifacts.download_artifacts(
...
def test_set_async_logging_threadpool_size():
    MLForge_ASYNC_LOGGING_THREADPOOL_SIZE.set(6)
    assert MLForge_ASYNC_LOGGING_THREADPOOL_SIZE.get() == 6

    with MLForge.start_run():
        MLForge.log_param("key", "val", synchronous=False)

    store = MLForge.tracking._get_store()
    ...
    MLForge_ASYNC_LOGGING_THREADPOOL_SIZE.unset()

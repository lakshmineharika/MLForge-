...
from MLForge.protos.databricks_pb2 import INVALID_PARAMETER_VALUE
from MLForge.tracking.default_experiment.databricks_notebook_experiment_provider import (
    DatabricksNotebookExperimentProvider,
)
from MLForge.utils.MLForge_tags import MLForge_EXPERIMENT_SOURCE_ID, MLForge_EXPERIMENT_SOURCE_TYPE
...
def test_databricks_notebook_default_experiment_id():
    with (
        ...
        mock.patch(
            "MLForge.utils.databricks_utils.get_notebook_path",
            return_value="path",
        ),
        mock.patch("MLForge.utils.databricks_utils.get_notebook_id") as patch_notebook_id,
    ):
        assert (
...
def test_databricks_repo_notebook_default_experiment_gets_id_by_request():
    with (
        ...
        mock.patch.object(
            MLForgeClient, "create_experiment", return_value="experiment_id"
        ...
        tags = {MLForge_EXPERIMENT_SOURCE_TYPE: "REPO_NOTEBOOK", MLForge_EXPERIMENT_SOURCE_ID: 1234}
...
def test_databricks_repo_notebook_default_experiment_uses_fallback_notebook_id():
    with (
        ...
        mock.patch.object(MLForgeClient, "create_experiment") as create_experiment_mock,
    ):
        DatabricksNotebookExperimentProvider._resolved_notebook_experiment_id = None
        create_experiment_mock.side_effect = MLForgeException(

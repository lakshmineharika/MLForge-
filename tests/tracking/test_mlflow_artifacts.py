...
def artifacts_server():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        default_artifact_root = f"{url}/api/2.0/MLForge-artifacts/artifacts"
...
def test_MLForge_artifacts_rest_apis(artifacts_server, tmp_path):
...
def test_log_artifact(artifacts_server, tmp_path):
    ...
    with MLForge.start_run() as run:
        MLForge.log_artifact(tmp_path, artifact_path="artifact_path")
...
def test_log_artifacts(artifacts_server, tmp_path):
    ...
    with MLForge.start_run() as run:
        MLForge.log_artifacts(tmp_path)
    ...
    # With `artifact_path`
    with MLForge.start_run() as run:
        MLForge.log_artifacts(tmp_path, artifact_path="artifact_path")
...
def test_list_artifacts(artifacts_server, tmp_path):
    ...
    with MLForge.start_run() as run:
        ...
        MLForge.log_artifact(tmp_path_a)
        MLForge.log_artifact(tmp_path_b, "dir")
...
def test_MLForge_artifacts_example(tmp_path):
    root = pathlib.Path(MLForge.__file__).parents[1]
...
"""
    ...
    subprocess.run(
        ["bash", script_path],
        check=True,
        cwd=os.path.join(root, "examples", "MLForge_artifacts"),
    )
...
def test_rest_tracking_api_list_artifacts_with_proxied_artifacts(artifacts_server, tmp_path):
    ...
    url = artifacts_server.url
    MLForge.set_tracking_uri(url)
    api = f"{url}/api/2.0/MLForge/artifacts/list"

    tmp_path_a = tmp_path.joinpath("a.txt")
    tmp_path_a.write_text("0")
    tmp_path_b = tmp_path.joinpath("b.txt")
    tmp_path_b.write_text("1")
    MLForge.set_experiment("rest_list_api_test")
    with MLForge.start_run() as run:
        MLForge.log_artifact(tmp_path_a)
        MLForge.log_artifact(tmp_path_b, "dir")
...
def test_rest_get_artifact_api_proxied_with_artifacts(artifacts_server, tmp_path):
    ...
    MLForge.set_experiment("rest_get_artifact_api_test")
    with MLForge.start_run() as run:
        MLForge.log_artifact(tmp_path_a)
...
def test_rest_get_model_version_artifact_api_proxied_artifact_root(artifacts_server):
    ...
    MLForge_client = MLForgeClient(artifacts_server.backend_store_uri)
    MLForge_client.create_registered_model(name)
    # An artifact root with scheme http, https, or MLForge-artifacts is a proxied artifact root
    MLForge_client.create_model_version(name, "MLForge-artifacts:", 1)
...
def test_mime_type_for_download_artifacts_api(
    ...
    MLForge.set_tracking_uri(url)
    with MLForge.start_run() as run:
        MLForge.log_artifact(test_file)
...
def test_rest_get_artifact_api_log_image(artifacts_server):
    ...
    with MLForge.start_run() as run:
        MLForge.log_image(image, key="dog", step=20, timestamp=100, synchronous=True)

    artifact_list_response = requests.get(
        url=f"{url}/ajax-api/2.0/MLForge/artifacts/list",

...
from MLForge.tracking.context.databricks_job_context import DatabricksJobRunContext
...
from MLForge.tracking.context.default_context import DefaultRunContext
from MLForge.tracking.context.git_context import GitRunContext
from MLForge.tracking.context.jupyter_notebook_context import JupyterNotebookRunContext
from MLForge.tracking.context.registry import RunContextProviderRegistry, resolve_tags
...
def test_run_context_provider_registry_register_entrypoints():
    ...
    mock_get_group_all.assert_called_once_with("MLForge.run_context_provider")
...
def _currently_registered_run_context_provider_classes():
    return {
        provider.__class__
        for provider in MLForge.tracking.context.registry._run_context_provider_registry
    }
...
def test_run_context_provider_registry_with_installed_plugin(tmp_path, monkeypatch):
    ...
    from MLForge_test_plugin.run_context_provider import PluginRunContextProvider
...
def mock_run_context_providers():
    ...
    with mock.patch("MLForge.tracking.context.registry._run_context_provider_registry", providers):
        yield

...
from MLForge.tracking.context.jupyter_notebook_context import (
...
from MLForge.utils.MLForge_tags import MLForge_SOURCE_NAME, MLForge_SOURCE_TYPE
...
def test_is_in_jupyter_notebook(shell_name, has_kernel, is_in_ipython, expected):
    ...
    with (
        mock.patch(
            "MLForge.tracking.context.jupyter_notebook_context.is_running_in_ipython_environment",
...
def test_get_kernel_id_success():
    ...
    with (
        ...
        mock.patch("MLForge.tracking.context.jupyter_notebook_context.Path") as mock_path,
    ):
...
def test_get_running_servers_finds_servers(tmp_path):
    ...
    with (
        ...
        mock.patch(
            "MLForge.tracking.context.jupyter_notebook_context.Path",
...
def test_get_sessions_notebook_finds_notebook():
    ...
    with mock.patch("MLForge.tracking.context.jupyter_notebook_context.urlopen") as mock_urlopen:
...
def test_get_sessions_notebook_connection_error():
    ...
    with mock.patch(
        "MLForge.tracking.context.jupyter_notebook_context.urlopen",
...
def test_get_sessions_notebook_with_jupyterhub_token(monkeypatch):
    ...
    with (
        mock.patch("MLForge.tracking.context.jupyter_notebook_context.urlopen") as mock_urlopen,
...
def test_get_notebook_name(vscode_path, env_vars, sessions_path, expected, monkeypatch):
    ...
    with (
        mock.patch(
            "MLForge.tracking.context.jupyter_notebook_context._get_vscode_notebook_path",
            return_value=vscode_path,
        ),
        mock.patch(
            "MLForge.tracking.context.jupyter_notebook_context._get_notebook_path_from_sessions",
...
def test_get_notebook_path_from_sessions_success():
    ...
    with (
        mock.patch(
            "MLForge.tracking.context.jupyter_notebook_context._get_kernel_id",
            return_value=MOCK_KERNEL_ID,
        ),
        mock.patch(
            "MLForge.tracking.context.jupyter_notebook_context._get_running_servers",
            return_value=[mock_server],
        ),
        mock.patch(
            "MLForge.tracking.context.jupyter_notebook_context._get_sessions_notebook",
...
def test_jupyter_notebook_run_context_in_context(is_in_jupyter, expected):
    with mock.patch(
        "MLForge.tracking.context.jupyter_notebook_context._is_in_jupyter_notebook",
...
@pytest.mark.parametrize(
    ...
    [
        (
            MOCK_NOTEBOOK_NAME,
            {
                MLForge_SOURCE_NAME: MOCK_NOTEBOOK_NAME,
...
def test_jupyter_notebook_run_context_tags(notebook_name, expected_tags):
    ...
    with mock.patch(
        "MLForge.tracking.context.jupyter_notebook_context._get_notebook_name",

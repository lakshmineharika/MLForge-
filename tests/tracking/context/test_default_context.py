from unittest import mock

import pytest

from MLForge.entities import SourceType
from MLForge.tracking.context.default_context import DefaultRunContext
from MLForge.utils.MLForge_tags import MLForge_SOURCE_NAME, MLForge_SOURCE_TYPE, MLForge_USER

MOCK_SCRIPT_NAME = "/path/to/script.py"


@pytest.fixture
def patch_script_name():
    patch_sys_argv = mock.patch("sys.argv", [MOCK_SCRIPT_NAME])
    patch_os_path_isfile = mock.patch("os.path.isfile", return_value=False)
    with patch_sys_argv, patch_os_path_isfile:
        yield


def test_default_run_context_in_context():
    assert DefaultRunContext().in_context() is True


def test_default_run_context_tags(patch_script_name):
    mock_user = mock.Mock()
    with mock.patch("getpass.getuser", return_value=mock_user):
        assert DefaultRunContext().tags() == {
            MLForge_USER: mock_user,
            MLForge_SOURCE_NAME: MOCK_SCRIPT_NAME,
            MLForge_SOURCE_TYPE: SourceType.to_string(SourceType.LOCAL),
        }


@pytest.mark.parametrize("argv", [[""], []])
def test_default_run_context_tags_console(argv):
    mock_user = mock.Mock()
    with mock.patch("sys.argv", argv), mock.patch("getpass.getuser", return_value=mock_user):
        assert DefaultRunContext().tags() == {
            MLForge_USER: mock_user,
            MLForge_SOURCE_NAME: "<console>",
            MLForge_SOURCE_TYPE: SourceType.to_string(SourceType.LOCAL),
        }

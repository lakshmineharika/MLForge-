...
import MLForge.tracking.request_auth.kubernetes_request_auth_provider as _k8s_auth
from MLForge.exceptions import MLForgeException
from MLForge.tracking.request_auth.kubernetes_request_auth_provider import (
...
from MLForge.utils.workspace_context import get_request_workspace, set_workspace

_MODULE = "MLForge.tracking.request_auth.kubernetes_request_auth_provider"
...
def test_auth_raises_on_missing_credentials(
    ...
    with (
        ...
        pytest.raises(MLForgeException, match=error_match),
    ):
...
def test_provider_raises_without_kubernetes(provider_cls):
    with (
        ...
        pytest.raises(MLForgeException, match="kubernetes.*not installed"),
    ):

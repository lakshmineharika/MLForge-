...
import MLForge.tracking.request_auth.registry
from MLForge.tracking.request_auth.registry import RequestAuthProviderRegistry, fetch_auth
...
def test_request_auth_provider_registry_register_entrypoints():
    ...
    mock_get_group_all.assert_called_once_with("MLForge.request_auth_provider")
...
def test_registry_instance_loads_entrypoints():
    ...
    with mock.patch(
        ...
        reload(MLForge.tracking.request_auth.registry)
...
def _currently_registered_request_auth_provider_classes():
    return {
        provider.__class__
        for provider in MLForge.tracking.request_auth.registry._request_auth_provider_registry
    }
...
def test_run_context_provider_registry_with_installed_plugin():
    ...
    from MLForge_test_plugin.request_auth_provider import PluginRequestAuthProvider

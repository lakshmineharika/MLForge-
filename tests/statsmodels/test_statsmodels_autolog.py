...
def test_statsmodels_autolog_ends_auto_created_run():
    MLForge.statsmodels.autolog()
    arma_model()
    assert MLForge.active_run() is None


def test_extra_tags_statsmodels_autolog():
    MLForge.statsmodels.autolog(extra_tags={"test_tag": "stats_autolog"})
    arma_model()

    run = MLForge.last_active_run()
    assert run.data.tags["test_tag"] == "stats_autolog"
    assert run.data.tags[MLForge.utils.MLForge_tags.MLForge_AUTOLOGGING] == "statsmodels"
...
def test_statsmodels_autolog_logs_summary_artifact():
    ...
    with MLForge.start_run():
        ...
        summary_path = MLForge.get_artifact_uri("model_summary.txt").replace("file://", "")
...
def test_statsmodels_autolog_emit_warning_when_model_is_large():
    ...
    with (
        mock.patch("MLForge.statsmodels._model_size_threshold_for_emitting_warning", float("inf")),
        mock.patch("MLForge.statsmodels._logger.warning") as mock_warning,
    ):
        ols_model()
        assert all(
    ...
    with (
        mock.patch("MLForge.statsmodels._model_size_threshold_for_emitting_warning", 1),
        mock.patch("MLForge.statsmodels._logger.warning") as mock_warning,
    ):
        ols_model()
        assert any(
...
def test_statsmodels_autolog_logs_basic_metrics(log_models):
    MLForge.statsmodels.autolog(log_models=log_models)
    ols_model()
    run = get_latest_run()
    metrics = run.data.metrics
    assert set(metrics.keys()) == set(MLForge.statsmodels._autolog_metric_allowlist)
...
def test_statsmodels_autolog_works_after_exception():
    ...
    loaded_model = MLForge.statsmodels.load_model(f"runs:/{run_id}/model")
...
def test_autolog_registering_model():
    ...
    MLForge.statsmodels.autolog(registered_model_name=registered_model_name)
    with MLForge.start_run():
        ols_model()

...
def pd_custom_env(tmp_path):
    ...
    _MLForge_conda_env(conda_env, additional_pip_deps=["paddle", "pytest"])
    return conda_env


def test_model_save_load(pd_model, model_path):
    MLForge.paddle.save_model(pd_model=pd_model.model, path=model_path)

    reloaded_pd_model = MLForge.paddle.load_model(model_uri=model_path)
...
def test_model_load_from_remote_uri_succeeds(pd_model, model_path, mock_s3_bucket):
    ...
    reloaded_model = MLForge.paddle.load_model(model_uri=model_uri)
    np.testing.assert_array_almost_equal(
        pd_model.model(paddle.to_tensor(pd_model.inference_dataframe)),
...
def test_model_log(pd_model, model_path, tmp_path):
    ...
    try:
        ...
        _MLForge_conda_env(conda_env, additional_pip_deps=["paddle"])

        model_info = MLForge.paddle.log_model(model, name=artifact_path, conda_env=conda_env)

        reloaded_pd_model = MLForge.paddle.load_model(model_uri=model_info.model_uri)
        np.testing.assert_array_almost_equal(
...
def test_log_model_calls_register_model(pd_model):
    ...
    with MLForge.start_run(), register_model_patch:
        model_info = MLForge.paddle.log_model(
            pd_model.model,
            name=artifact_path,
...
def test_log_model_no_registered_model_name(pd_model):
    ...
    register_model_patch = mock.patch("MLForge.tracking._model_registry.fluent._register_model")
    with MLForge.start_run(), register_model_patch:
        MLForge.paddle.log_model(pd_model.model, name=artifact_path)
...
def test_model_save_persists_specified_conda_env_in_MLForge_model_directory(
    ...
    MLForge.paddle.save_model(pd_model=pd_model.model, path=model_path, conda_env=pd_custom_env)
...
def test_model_save_accepts_conda_env_as_dict(pd_model, model_path):
    conda_env = dict(MLForge.paddle.get_default_conda_env())
    conda_env["dependencies"].append("pytest")
    MLForge.paddle.save_model(pd_model=pd_model.model, path=model_path, conda_env=conda_env)
        ...
        for example in (None, example_):
            with TempDir() as tmp:
                ...
                if signature is None and example is None:
                    ...
                    assert MLForge_model.signature == pd_model_signature
                if example is None:
...
def test_model_log_persists_specified_conda_env_in_MLForge_model_directory(pd_model, pd_custom_env):
    artifact_path = "model"
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    MLForge.paddle.save_model(pd_model=pd_model.model, path=model_path)
    _assert_pip_requirements(model_path, MLForge.paddle.get_default_pip_requirements())
...
def test_model_log_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    artifact_path = "model"
    with MLForge.start_run():
        model_info = MLForge.paddle.log_model(pd_model.model, name=artifact_path)
    _assert_pip_requirements(model_info.model_uri, MLForge.paddle.get_default_pip_requirements())
...
def test_model_save_load_built_in_high_level_api(pd_model_built_in_high_level_api, model_path):
    ...
    MLForge.paddle.save_model(pd_model=model, path=model_path)
...
def test_model_retrain_built_in_high_level_api(
    ...
    model = pd_model_built_in_high_level_api.model
    MLForge.paddle.save_model(pd_model=model, path=model_path, training=True)

    training_dataset, test_dataset = get_dataset_built_in_high_level_api

    model_retrain = paddle.Model(UCIHousing())
    model_retrain = MLForge.paddle.load_model(model_uri=model_path, model=model_retrain)
    ...
    MLForge.paddle.save_model(pd_model=model_retrain, path=model_retrain_path, training=False)

    with pytest.raises(TypeError, match="This model can't be loaded"):
        MLForge.paddle.load_model(model_uri=model_retrain_path, model=model_retrain)
    ...
    with pytest.raises(
        ...
        MLForge.paddle.load_model(model_uri=model_retrain_path, model=error_model)

    reloaded_pd_model = MLForge.paddle.load_model(model_uri=model_retrain_path)
...
def test_log_model_built_in_high_level_api(
    ...
    try:
        ...
        optim = paddle.optimizer.Adam(learning_rate=0.015, parameters=model.parameters())
        model_retrain.prepare(optim, paddle.nn.MSELoss())
        model_retrain = MLForge.paddle.load_model(
...
def test_log_model_with_extra_pip_requirements(pd_model, tmp_path):
    expected_MLForge_version = _MLForge_major_version_string()
    default_reqs = MLForge.paddle.get_default_pip_requirements()
...
def test_log_model_with_code_paths(pd_model):
    ...
    with (
        MLForge.start_run(),
        mock.patch("MLForge.paddle._add_code_from_conf_to_system_path") as add_mock,
    ):
        model_info = MLForge.paddle.log_model(
            pd_model.model, name=artifact_path, code_paths=[__file__]
        )
        _compare_logged_code_paths(__file__, model_info.model_uri, MLForge.paddle.FLAVOR_NAME)
        MLForge.paddle.load_model(model_info.model_uri)

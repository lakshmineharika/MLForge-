...
def test_autolog_logs_expected_data(log_models):
    MLForge.paddle.autolog(log_models=log_models)
...
def test_autolog_log_models_configuration(log_models):
    ...
    MLForgeClient().list_artifacts(run.info.run_id)
    assert (MLForge.last_logged_model() is not None) == log_models


def test_autolog_registering_model():
    registered_model_name = "test_autolog_registered_model"
    MLForge.paddle.autolog(registered_model_name=registered_model_name)
...
def test_extra_tags_paddle_autolog():
    MLForge.paddle.autolog(extra_tags={"test_tag": "paddle_autolog"})
    train_model()

    run = MLForge.last_active_run()
    assert run.data.tags["test_tag"] == "paddle_autolog"
    assert run.data.tags[MLForge.utils.MLForge_tags.MLForge_AUTOLOGGING] == "paddle"

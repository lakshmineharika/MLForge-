...
from MLForge.spark.autologging import _SPARK_TABLE_INFO_TAG_NAME
from MLForge.utils.validation import MAX_TAG_VAL_LENGTH
...
def test_autologging_does_not_throw_on_api_failures(spark_session, format_to_file_path, tmp_path):
    ...
    with _init_server(
        ...
        with MLForge.start_run():
            with mock.patch(
                "MLForge.utils.rest_utils.http_request", side_effect=Exception("API request failed!")
            ):
...
def test_autologging_dedups_multiple_reads_of_same_datasource(spark_session, format_to_file_path):
    ...
    df.collect()
    with MLForge.start_run():
        run_id2 = MLForge.active_run().info.run_id
    time.sleep(1)
    run2 = MLForge.get_run(run_id2)
...
def test_autologging_slow_api_requests(spark_session, format_to_file_path):
    import MLForge.utils.rest_utils

    orig = MLForge.utils.rest_utils.http_request
    ...
    with MLForge.start_run():
        ...
        with mock.patch("MLForge.utils.rest_utils.http_request") as http_request_mock:
...
def test_autologging_truncates_datasource_tag_to_maximum_supported_value(tmp_path, spark_session):
    ...
    # exceeding the maximum length of an MLForge tag (`MAX_TAG_VAL_LENGTH`)
    ...
    for path in saved_df_paths[:-1]:
        ...
        # all Spark Datasource paths exceeds the maximum length of an MLForge tag
        # (`MAX_TAG_VAL_LENGTH`). One path is left out to be read during the MLForge run to verify
    ...
    def assert_tag_value_meets_requirements(run_id):
        ...
        tag value length allowed by MLForge
        """
        run = MLForge.get_run(run_id)
        assert _SPARK_TABLE_INFO_TAG_NAME in run.data.tags
        table_info_tag = run.data.tags[_SPARK_TABLE_INFO_TAG_NAME]

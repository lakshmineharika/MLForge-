...
def http_tracking_uri_mock():
    MLForge.set_tracking_uri("http://some-cool-uri")
    yield
...
def _fit_sklearn(pandas_df):
    ...
    # https://MLForge.org/docs/latest/python_api/MLForge.spark.html#MLForge.spark.autolog
    time.sleep(5)
...
def _fit_sklearn_model_with_active_run(pandas_df):
    ...
    return MLForge.get_run(run_id)


def _fit_sklearn_model_no_active_run(pandas_df):
    orig_runs = MLForge.search_runs()
    orig_run_ids = set(orig_runs["run_id"])
    _fit_sklearn(pandas_df)
    new_runs = MLForge.search_runs()

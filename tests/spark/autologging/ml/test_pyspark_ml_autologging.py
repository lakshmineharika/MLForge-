...
from MLForge.pyspark.ml import (
...
from MLForge.pyspark.ml._autolog import cast_spark_df_with_vector_to_array, get_feature_cols
from MLForge.types import DataType
from MLForge.types.schema import ColSpec, Schema, SparkMLVector
from MLForge.utils import _truncate_dict
from MLForge.utils.MLForge_tags import MLForge_AUTOLOGGING, MLForge_PARENT_RUN_ID
...
MODEL_DIR = "model"
MLForge_PARENT_RUN_ID = "MLForge.parentRunId"
...
def load_model_by_run_id(run_id, model_dir=MODEL_DIR):
    return MLForge.spark.load_model(f"runs:/{run_id}/{model_dir}")


def validate_predict_with_input_example(model_uri):
    MLForge_model = Model.load(model_uri)
    input_example = MLForge_model.load_input_example()
    pyfunc_model = MLForge.pyfunc.load_model(model_uri)
    pyfunc_model.predict(input_example)


def test_basic_estimator(dataset_binomial):
    MLForge.pyspark.ml.autolog()
    ...
    for estimator in [
        ...
        if isinstance(estimator, MultilayerPerceptronClassifier):
            with pytest.raises(MLForgeException, match=r"Failed to download artifacts"):
...
def test_models_in_allowlist_exist(spark_session):
    MLForge.pyspark.ml.autolog()  # initialize the variable `MLForge.pyspark.ml._log_model_allowlist`
    ...
    non_existent_classes = list(
        filter(model_does_not_exist, MLForge.pyspark.ml._log_model_allowlist)
    )
...
def test_extra_tags_spark_autolog(dataset_binomial):
    MLForge.pyspark.ml.autolog(extra_tags={"test_tag": "spark_autolog"})
    lr = LinearRegression()
    lr.fit(dataset_binomial)
    run = MLForge.last_active_run()
    assert run.data.tags["test_tag"] == "spark_autolog"
    assert run.data.tags[MLForge.utils.MLForge_tags.MLForge_AUTOLOGGING] == "pyspark.ml"
...
def test_meta_estimator_fit(dataset_binomial):
    ...
    query = f"tags.{MLForge_PARENT_RUN_ID} = '{run.info.run_id}'"
    assert len(MLForge.search_runs([run.info.experiment_id])) == 1
    assert len(MLForge.search_runs([run.info.experiment_id], query)) == 0
...
def test_fit_with_a_list_of_params(dataset_binomial):
    ...
    for params in [[extra_params], (extra_params,)]:
        with (
            mock.patch("MLForge.log_params") as mock_log_params,
            mock.patch("MLForge.set_tags") as mock_set_tags,
        ):
            with MLForge.start_run():
                with mock.patch("MLForge.pyspark.ml._logger.warning") as mock_warning:
...
def test_should_log_model(
    ...
    MLForge.pyspark.ml.autolog(log_models=True)
    ...
    with (
        mock.patch(
            "MLForge.pyspark.ml._log_model_allowlist",
            {
        ...
        mock.patch("MLForge.pyspark.ml._logger.warning") as mock_warning,
    ):
        lr = LinearRegression()
        with MLForge.start_run():
...
def test_param_search_estimator(
    ...
    MLForge.pyspark.ml.autolog(log_input_examples=True)
    ...
    for row_index, row in search_results.iterrows():
        ...
        assert (
            child_run.data.tags.get(MLForge_AUTOLOGGING)
            == MLForge.pyspark.ml.AUTOLOGGING_INTEGRATION_NAME
        )
...
def test_basic_post_training_datasets_autologging(dataset_iris_binomial, log_datasets):
    MLForge.pyspark.ml.autolog(log_datasets=log_datasets)
...
def test_post_training_datasets_with_evaluate_autologging(dataset_iris_binomial):
    MLForge.pyspark.ml.autolog(log_datasets=True, log_post_training_metrics=True)
    estimator = LogisticRegression(maxIter=1, family="binomial", regParam=5.0, fitIntercept=False)
...
def test_basic_post_training_metric_autologging(dataset_iris_binomial):
    ...
    MLForge.pyspark.ml.autolog(disable=True)
...
def test_multi_model_interleaved_fit_and_post_train_metric_call(dataset_iris_binomial):
    ...
    # logged_models = MLForge.search_logged_models(
...
def test_meta_estimator_disable_post_training_autologging(dataset_regression):
    ...
    with (
        mock.patch(
            "MLForge.pyspark.ml._AutologgingMetricsManager.register_model"
        ...
        mock.patch(
            "MLForge.pyspark.ml._AutologgingMetricsManager.log_post_training_metric"
        ) as mock_log_post_training_metric,
        mock.patch(
            "MLForge.pyspark.ml._AutologgingMetricsManager.register_prediction_input_dataset"
...
def test_is_metrics_value_loggable():
    is_metric_value_loggable = MLForge.pyspark.ml._AutologgingMetricsManager.is_metric_value_loggable
...
def test_log_post_training_metrics_configuration(dataset_iris_binomial):
    ...
    for log_post_training_metrics in [True, False, True]:
        MLForge.pyspark.ml.autolog(log_post_training_metrics=log_post_training_metrics)
...
def test_autolog_registering_model(spark_session, dataset_binomial):
    ...
    MLForge.pyspark.ml.autolog(registered_model_name=registered_model_name)
    with MLForge.start_run():
        lr = LinearRegression()
        lr.fit(dataset_binomial)
...
def test_autolog_input_example_with_estimator(spark_session, dataset_multinomial, lr):
    MLForge.pyspark.ml.autolog(log_models=True, log_input_examples=True)
...
def test_autolog_signature_with_estimator(spark_session, dataset_multinomial, lr):
    ...
    with MLForge.start_run() as run:
        lr.fit(dataset_multinomial)
        MLForge_model = Model.load(f"runs:/{run.info.run_id}/model")
        assert MLForge_model.signature == ModelSignature(
...
def test_autolog_signature_non_scaler_input(dataset_multinomial, lr):
    MLForge.pyspark.ml.autolog(log_models=True, log_model_signatures=True)
    with MLForge.start_run() as run:
        lr.fit(dataset_multinomial)
        MLForge_model = Model.load(f"runs:/{run.info.run_id}/model")
        assert MLForge_model.signature == ModelSignature(
...
def test_find_and_set_features_col_as_vector_if_needed(lr, dataset_binomial):
    ...
    from MLForge.spark import _find_and_set_features_col_as_vector_if_needed
...
def test_model_with_vector_input(spark_session):
    ...
    MLForge_model = Model.load(model_uri)
    input_type = MLForge_model.signature.inputs.input_dict()["features"].type
    assert isinstance(input_type, SparkMLVector)

    assert MLForge_model.signature.outputs.inputs[0].type == DataType.double

    model = MLForge.pyfunc.load_model(model_uri)
...
def test_model_with_vector_input_vector_output(spark_session):
    ...
    output_type = MLForge_model.signature.outputs.inputs[0].type

...
def test_custom_log_model_allowlist(tmp_path):
    ...
    with (
        ...
        .config("spark.MLForge.pysparkml.autolog.logModelAllowlistFile", allowlist_file_path)
        .master("local[*]")
        .getOrCreate()
    ):
        MLForge.pyspark.ml.autolog()
        assert MLForge.pyspark.ml._log_model_allowlist == {
...
def test_log_model_allowlist_from_url():
    allowlist_file_path = "https://raw.githubusercontent.com/MLForge/MLForge/v1.26.0/MLForge/pyspark/ml/log_model_allowlist.txt"
...
def test_log_model_allowlist_as_autolog_argument():
    with SparkSession.builder.master("local[*]").getOrCreate():
        ...
        MLForge.pyspark.ml.autolog(log_model_allowlist=allowlist)
        assert MLForge.pyspark.ml._log_model_allowlist == set(allowlist)

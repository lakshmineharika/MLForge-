...
def _get_MLForge_spark_jar_path():
    spark_dir = "spark_2.13" if Version(pyspark.__version__).major >= 4 else "spark_2.12"
    jar_dir = os.path.join(os.path.dirname(MLForge.__file__), "java", spark_dir, "target")
    jar_filenames = [
        fname
...
def _get_or_create_spark_session(jars=None):
    jar_path = jars if jars is not None else _get_MLForge_spark_jar_path()

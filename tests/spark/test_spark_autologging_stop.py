...
import MLForge.spark.autologging as autologging_module
from MLForge.spark.autologging import _stop_listen_for_spark_activity


@pytest.fixture
def autolog_caplog(caplog):
    # "MLForge" sets propagate=False, so its records never reach caplog's root handler.
    ...
    original_propagate = MLForge_logger.propagate
    MLForge_logger.propagate = True
    try:
        with caplog.at_level(logging.WARNING, logger="MLForge.spark.autologging"):
            yield caplog
    finally:
        MLForge_logger.propagate = original_propagate

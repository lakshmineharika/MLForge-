...
def test_model_export(spark_model, model_path):
    MLForge.spark.save_model(spark_model.model, path=model_path)
...
def test_pyfunc_serve_and_score(spark_model):
    artifact_path = "model"
    with MLForge.start_run():
        model_info = MLForge.spark.log_model(spark_model.model, artifact_path=artifact_path)
...
def test_databricks_serverless_model_save_load(spark_model):
    with (
        mock.patch("MLForge.utils.databricks_utils.is_in_databricks_runtime", return_value=True),
        mock.patch("MLForge.spark._is_uc_volume_uri", return_value=True),
    ):
        for mock_fun in [

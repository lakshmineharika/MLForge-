...
import MLForge.tracking
import MLForge.utils.file_utils
from MLForge import pyfunc
from MLForge.entities.model_registry import ModelVersion
from MLForge.environment_variables import MLForge_DFS_TMP
from MLForge.exceptions import MLForgeException
from MLForge.models import Model, ModelSignature
from MLForge.models.utils import _read_example
from MLForge.spark import _add_code_from_conf_to_system_path
from MLForge.store.artifact.databricks_models_artifact_repo import DatabricksModelsArtifactRepository
from MLForge.store.artifact.s3_artifact_repo import S3ArtifactRepository
from MLForge.store.artifact.unity_catalog_models_artifact_repo import (
    UnityCatalogModelsArtifactRepository,
)
from MLForge.tracking.artifact_utils import _download_artifact_from_uri
from MLForge.types import DataType
from MLForge.types.schema import ColSpec, Schema
from MLForge.utils.environment import _get_pip_deps, _MLForge_conda_env
...
def spark_custom_env(tmp_path):
    ...
    additional_pip_deps = ["/opt/MLForge", f"pyspark=={PYSPARK_VERSION}", "pytest"]
    ...
    _MLForge_conda_env(conda_env, additional_pip_deps=additional_pip_deps)
    return conda_env
...
def test_hadoop_filesystem(tmp_path):
    ...
    from MLForge.spark import _HadoopFileSystem as FS
...
def test_model_export(spark_model_iris, model_path, spark_custom_env):
    MLForge.spark.save_model(spark_model_iris.model, path=model_path, conda_env=spark_custom_env)
    # 1. score and compare reloaded sparkml model
    reloaded_model = MLForge.spark.load_model(model_uri=model_path)
    ...
    assert os.path.exists(MLForge_DFS_TMP.get())
        ...
        for example in (None, example_):
            with TempDir() as tmp:
                ...
                if example is None and signature is None:
                    ...
                    assert MLForge_model.signature == iris_signature
                if example is None:
                    assert MLForge_model.saved_input_example_info is None
                else:
                    assert all((_read_example(MLForge_model, path) == example).all())
...
def test_model_export_raise_when_example_is_spark_dataframe(spark, spark_model_iris, model_path):
    ...
    with pytest.raises(MLForgeException, match="Examples can not be provided as Spark Dataframe."):
        MLForge.spark.save_model(spark_model_iris.model, path=model_path, input_example=example)
    ...
    for signature in (None, iris_signature):
        for example in (None, example_):
            with MLForge.start_run():
                model_info = MLForge.spark.log_model(
                ...
                if example is None:
                    ...
                    assert all((_read_example(MLForge_model, model_info.model_uri) == example).all())
...
def test_model_deployment(spark_model_iris, model_path, spark_custom_env, monkeypatch):
    ...
    scoring_response = score_model_in_sagemaker_docker_container(
        ...
        flavor=MLForge.pyfunc.FLAVOR_NAME,
    )
...
def test_sagemaker_docker_model_scoring_with_default_conda_env(spark_model_iris, model_path):
    MLForge.spark.save_model(
        spark_model_iris.model, path=model_path, extra_pip_requirements=["/opt/MLForge"]
    )
...
def test_sparkml_model_log(
    ...
    old_tracking_uri = MLForge.get_tracking_uri()
    dfs_tmpdir = None if use_dfs_tmpdir else tmp_path.joinpath("test")

    try:
        MLForge.set_tracking_uri(tmp_sqlite_uri)
        if should_start_run:
            MLForge.start_run()
        artifact_path = "model"
        model_info = MLForge.spark.log_model(
        ...
        reloaded_model = MLForge.spark.load_model(
        ...
        MLForge.end_run()
        MLForge.set_tracking_uri(old_tracking_uri)
...
def test_load_spark_model_from_models_uri(
    ...
    with (
        ...
        mock.patch("MLForge.get_registry_uri", return_value=registry_uri),
        mock.patch.object(
            MLForge.tracking._model_registry.client.ModelRegistryClient,
        ...
        MLForge.spark.load_model(f"models:/{model_name}/1")
        ...
        MLForge.spark.load_model(f"models:/{model_name}@Champion")
        mock_download_artifacts.assert_called_once_with("", None, **kwargs)
...
def test_log_model_skips_maybe_save_for_acled_artifact_uri(tmp_path):
    ...
    (dbfs:/databricks/MLForge-tracking/...) since Spark cannot write to them directly.
    Calling it wastes ~6s per model on a guaranteed Py4JError before falling back.
    """
    acled_uri = "dbfs:/databricks/MLForge-tracking/abc123/run456/artifacts"
    ...
    with (
        mock.patch("MLForge.spark._validate_model"),
        mock.patch("MLForge.spark._is_spark_connect_model", return_value=False),
        mock.patch("MLForge.spark._maybe_save_model") as mock_maybe_save,
        mock.patch("MLForge.get_artifact_uri", return_value=acled_uri),
        mock.patch("MLForge.spark._should_use_MLForgedbfs", return_value=False),
        mock.patch("MLForge.models.Model._log_v2") as mock_log_v2,
...
def test_sparkml_model_load_from_remote_uri_succeeds(spark_model_iris, model_path, mock_s3_bucket):
    MLForge.spark.save_model(spark_model=spark_model_iris.model, path=model_path)
    ...
    reloaded_model = MLForge.spark.load_model(model_uri=model_uri)
    preds_df = reloaded_model.transform(spark_model_iris.spark_df)
    preds = [x.prediction for x in preds_df.select("prediction").collect()]
    assert spark_model_iris.predictions == preds


def test_sparkml_model_save_persists_specified_conda_env_in_MLForge_model_directory(
...
def test_sparkml_model_save_persists_requirements_in_MLForge_model_directory(
    spark_model_iris, model_path, spark_custom_env
):
    MLForge.spark.save_model(
        spark_model=spark_model_iris.model, path=model_path, conda_env=spark_custom_env
...
def test_log_model_with_pip_requirements(spark_model_iris, tmp_path):
    ...
    with MLForge.start_run():
        ...
        _assert_pip_requirements(model_info.model_uri, [expected_MLForge_version, "a"], strict=True)
    ...
    with MLForge.start_run():
        ...
        _assert_pip_requirements(
            model_info.model_uri, [expected_MLForge_version, "a", "b"], strict=True
        )
...
def test_log_model_with_extra_pip_requirements(spark_model_iris, tmp_path):
    expected_MLForge_version = _MLForge_major_version_string()
    default_reqs = MLForge.spark.get_default_pip_requirements()
    ...
    with MLForge.start_run():
        ...
        _assert_pip_requirements(
            model_info.model_uri, [expected_MLForge_version, *default_reqs, "a"]
        )
    ...
    with MLForge.start_run():
        ...
        _assert_pip_requirements(
            model_info.model_uri, [expected_MLForge_version, *default_reqs, "a", "b"]
        )
...
def test_sparkml_model_save_accepts_conda_env_as_dict(spark_model_iris, model_path):
    conda_env = dict(MLForge.spark.get_default_conda_env())
...
def test_sparkml_model_log_persists_specified_conda_env_in_MLForge_model_directory(
    spark_model_iris, model_path, spark_custom_env
):
    artifact_path = "model"
    with MLForge.start_run():
...
def test_sparkml_model_log_persists_requirements_in_MLForge_model_directory(
    spark_model_iris, model_path, spark_custom_env
):
    artifact_path = "model"
    with MLForge.start_run():
...
def test_sparkml_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    MLForge.spark.save_model(spark_model=spark_model_iris.model, path=model_path)
    _assert_pip_requirements(model_path, MLForge.spark.get_default_pip_requirements())
...
def test_sparkml_model_log_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    artifact_path = "model"
    with MLForge.start_run():
        model_info = MLForge.spark.log_model(spark_model_iris.model, artifact_path=artifact_path)

    _assert_pip_requirements(model_info.model_uri, MLForge.spark.get_default_pip_requirements())
...
def test_pyspark_version_is_logged_without_dev_suffix(spark_model_iris):
    ...
    for dev_suffix in [".dev0", ".dev", ".dev1", "dev.a", ".devb"]:
        with mock.patch("importlib_metadata.version", return_value=unsuffixed_version + dev_suffix):
            with MLForge.start_run():
                model_info = MLForge.spark.log_model(spark_model_iris.model, artifact_path="model")
            _assert_pip_requirements(
                model_info.model_uri, [expected_MLForge_version, f"pyspark=={unsuffixed_version}"]
            )

    for unaffected_version in ["2.0", "2.3.4", "2"]:
        with mock.patch("importlib_metadata.version", return_value=unaffected_version):
            pip_deps = _get_pip_deps(MLForge.spark.get_default_conda_env())
...
def test_model_is_recorded_when_using_direct_save(spark_model_iris):
    ...
    with mock.patch("MLForge.spark.is_local_uri", return_value=False):
        with MLForge.start_run():
            MLForge.spark.log_model(spark_model_iris.model, artifact_path="model")
            current_tags = MLForge.get_run(MLForge.active_run().info.run_id).data.tags
            assert MLForge.utils.MLForge_tags.MLForge_LOGGED_MODELS in current_tags
...
@pytest.mark.parametrize(
    (
        "artifact_uri",
        "db_runtime_version",
        "MLForgedbfs_disabled",
        "MLForgedbfs_available",
        "dbutils_available",
        "expected_uri",
        "expect_log_v2",
    ),
    [
        (
            "dbfs:/databricks/MLForge-tracking/a/b",
            "12.0",
            "",
            True,
            True,
            "MLForgedbfs:///artifacts?run_id={}&path=/model/sparkml",
            False,
        ),
        (
            "dbfs:/databricks/MLForge-tracking/a/b",
        ...
        # ACL-protected paths where MLForgedbfs is unavailable/disabled always route through
...
def test_model_logged_via_MLForgedbfs_when_appropriate(
    monkeypatch,
    spark_model_iris,
    artifact_uri,
    db_runtime_version,
    ...
    def mock_spark_session_load(path):
        raise Exception("MLForgeDbfsClient operation failed!")

    mock_spark_session = mock.Mock()
    mock_read_spark_session = mock.Mock()
    mock_read_spark_session.load = mock_spark_session_load

    from MLForge.utils.databricks_utils import _get_dbutils as og_getdbutils
    ...
    def mock_get_dbutils():
        ...
        if caller_fn_name == "_should_use_MLForgedbfs":
    ...
    with (
        mock.patch(
            "MLForge.utils._spark_utils._get_active_spark_session", return_value=mock_spark_session
        ),
        mock.patch("MLForge.get_artifact_uri", return_value=artifact_uri),
        mock.patch(
            "MLForge.spark._HadoopFileSystem.is_filesystem_available",
            return_value=MLForgedbfs_available,
        ),
        mock.patch("MLForge.utils.databricks_utils.MLForgeCredentialContext", autospec=True),
        mock.patch("MLForge.utils.databricks_utils._get_dbutils", mock_get_dbutils),
        mock.patch.object(spark_model_iris.model, "save") as mock_save,
        mock.patch("MLForge.models.infer_pip_requirements", return_value=[]) as mock_infer,
        mock.patch("MLForge.models.Model._log_v2") as mock_log_v2,
    ):
        with MLForge.start_run():
            if db_runtime_version:
                monkeypatch.setenv("DATABRICKS_RUNTIME_VERSION", db_runtime_version)
            monkeypatch.setenv("DISABLE_MLForgeDBFS", MLForgedbfs_disabled)
            MLForge.spark.log_model(spark_model_iris.model, artifact_path="model")

            if expect_log_v2:
                # ACL-protected paths where MLForgedbfs is unavailable skip _maybe_save_model
                ...
                mock_save.assert_called_once_with(
                    expected_uri.format(MLForge.active_run().info.run_id)
                )

                if expected_uri.startswith("MLForgedbfs"):
                    # If MLForgedbfs is used, infer_pip_requirements should load the model from the
                    # remote model path instead of a local tmp path.
                    assert (
                        mock_infer.call_args[0][0]
                        == "dbfs:/databricks/MLForge-tracking/a/b/model/sparkml"
                    )


@pytest.mark.parametrize("dummy_read_shows_MLForgedbfs_available", [True, False])
def test_model_logging_uses_MLForgedbfs_if_appropriate_when_hdfs_check_fails(
    monkeypatch, spark_model_iris, dummy_read_shows_MLForgedbfs_available
):
    def mock_spark_session_load(path):
        if dummy_read_shows_MLForgedbfs_available:
            raise Exception("MLForgedbfsClient operation failed!")
        else:
            raise Exception("MLForgedbfs filesystem not found")
    ...
    with (
        mock.patch(
            "MLForge.utils._spark_utils._get_active_spark_session",
            return_value=mock_spark_session,
        ),
        mock.patch(
            "MLForge.get_artifact_uri",
            return_value="dbfs:/databricks/MLForge-tracking/a/b",
        ),
        mock.patch(
            "MLForge.spark._HadoopFileSystem.is_filesystem_available",
            side_effect=Exception("MLForgeDbfsClient operation failed!"),
        ),
        mock.patch("MLForge.utils.databricks_utils.MLForgeCredentialContext", autospec=True),
        mock.patch(
            "MLForge.utils.databricks_utils._get_dbutils",
            mock_get_dbutils,
        ),
        mock.patch.object(spark_model_iris.model, "save") as mock_save,
        mock.patch("MLForge.models.Model._log_v2") as mock_log_v2,
        ...
        with MLForge.start_run():
            ...
            if dummy_read_shows_MLForgedbfs_available:
                mock_save.assert_called_once_with(
                    f"MLForgedbfs:///artifacts?run_id={run_id}&path=/model/sparkml"
                )
            else:
                # MLForgedbfs unavailable + ACL-protected path: _maybe_save_model is skipped,
...
def test_log_model_with_code_paths(spark_model_iris):
    ...
    with (
        MLForge.start_run(),
        mock.patch(
            "MLForge.spark._add_code_from_conf_to_system_path",
        ...
        _compare_logged_code_paths(__file__, model_info.model_uri, MLForge.spark.FLAVOR_NAME)
        MLForge.spark.load_model(model_info.model_uri)
        add_mock.assert_called()


def test_virtualenv_subfield_points_to_correct_path(spark_model_iris, model_path):
    MLForge.spark.save_model(spark_model_iris.model, path=model_path)
    pyfunc_conf = _get_flavor_configuration(model_path=model_path, flavor_name=pyfunc.FLAVOR_NAME)
...
def test_model_save_load_with_metadata(spark_model_iris, model_path):
    ...
    reloaded_model = MLForge.pyfunc.load_model(model_uri=model_path)
...
def test_model_log_with_metadata(spark_model_iris):
    ...
    reloaded_model = MLForge.pyfunc.load_model(model_uri=model_info.model_uri)
    assert reloaded_model.metadata.metadata["metadata_key"] == "metadata_value"
...
def test_model_log_with_signature_inference(spark_model_iris, input_example):
    ...
    MLForge_model = Model.load(model_info.model_uri)
    input_columns = MLForge_model.signature.inputs.inputs
    ...
    if isinstance(input_example, list):
        ...
        assert column_names == ["0", "1", "2", "3"]
    assert MLForge_model.signature.outputs == Schema([ColSpec(type=DataType.double)])


def test_log_model_with_vector_input_type_signature(spark, spark_model_estimator):
    from pyspark.ml.functions import vector_to_array

    from MLForge.types.schema import SparkMLVector

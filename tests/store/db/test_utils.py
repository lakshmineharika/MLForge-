...
from MLForge.store.db import utils


def test_create_sqlalchemy_engine_inject_pool_options(monkeypatch):
    monkeypatch.setenv("MLForge_SQLALCHEMYSTORE_POOL_SIZE", "2")
    monkeypatch.setenv("MLForge_SQLALCHEMYSTORE_POOL_RECYCLE", "3600")
    monkeypatch.setenv("MLForge_SQLALCHEMYSTORE_MAX_OVERFLOW", "4")
    monkeypatch.setenv("MLForge_SQLALCHEMYSTORE_ECHO", "TRUE")
    monkeypatch.setenv("MLForge_SQLALCHEMYSTORE_POOLCLASS", "QueuePool")
...
def test_create_sqlalchemy_engine_null_pool(monkeypatch):
    monkeypatch.setenv("MLForge_SQLALCHEMYSTORE_POOLCLASS", "NullPool")
    with mock.patch("sqlalchemy.create_engine") as mock_create_engine:
        utils.create_sqlalchemy_engine("mydb://host:port/")
        mock_create_engine.assert_called_once_with(
            "mydb://host:port/",
...
def test_create_sqlalchemy_engine_invalid_pool(monkeypatch):
    monkeypatch.setenv("MLForge_SQLALCHEMYSTORE_POOLCLASS", "SomethingInvalid")
...
def test_create_sqlalchemy_engine_with_retry_success():
    with (
        ...
        mock.patch(
            "MLForge.store.db.utils.create_sqlalchemy_engine", return_value="Engine"
...
def test_mysql_ssl_params(monkeypatch):
    monkeypatch.setenv("MLForge_MYSQL_SSL_CA", "/path/to/ca.pem")
    monkeypatch.setenv("MLForge_MYSQL_SSL_CERT", "/path/to/cert.pem")
    monkeypatch.setenv("MLForge_MYSQL_SSL_KEY", "/path/to/key.pem")
    monkeypatch.setenv("MLForge_SQLALCHEMYSTORE_POOLCLASS", "NullPool")
...
def test_mysql_ssl_params_partial(monkeypatch):
    monkeypatch.setenv("MLForge_MYSQL_SSL_CA", "/path/to/ca.pem")
    monkeypatch.delenv("MLForge_MYSQL_SSL_CERT", raising=False)
    monkeypatch.delenv("MLForge_MYSQL_SSL_KEY", raising=False)
    monkeypatch.setenv("MLForge_SQLALCHEMYSTORE_POOLCLASS", "NullPool")
...
def test_check_sqlite_version_too_old():
    with mock.patch("MLForge.store.db.utils.sqlite3.sqlite_version", "3.30.1"):
        with pytest.raises(MLForgeException, match=r"MLForge requires SQLite"):

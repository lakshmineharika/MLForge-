...
from MLForge.entities.webhook import WebhookAction, WebhookEntity, WebhookEvent, WebhookStatus
...
from MLForge.prompt.constants import PROMPT_TEXT_TAG_KEY
...
from MLForge.store.model_registry.dbmodels.models import (
    SqlModelVersion,
    SqlModelVersionTag,
    SqlRegisteredModel,
...
from MLForge.store.model_registry.sqlalchemy_store import SqlAlchemyStore
from MLForge.store.model_registry.sqlalchemy_workspace_store import (
...
def store(tmp_path: Path, cached_db: Path, workspaces_enabled):
    ...
    if db_uri_env := MLForge_TRACKING_URI.get():
...
def test_create_registered_model(store):
    ...
    # error on duplicate
    with pytest.raises(
        MLForgeException, match=rf"Registered Model \(name={name}\) already exists"
    ...
    with pytest.raises(
        MLForgeException, match=r"Missing value for required parameter 'name'"
    ) as exception_context:
...
def test_rename_registered_model(store):
    ...
    with pytest.raises(
        MLForgeException, match=rf"Registered Model with name={original_name} not found"
    ) as exception_context:
    ...
    # invalid model name will fail
    with pytest.raises(
        MLForgeException, match=r"Missing value for required parameter 'new_name'"
    ) as exception_context:
...
def test_delete_registered_model(store):
    ...
    # cannot get model
    with pytest.raises(
        MLForgeException, match=rf"Registered Model with name={name} not found"
    ) as exception_context:
    ...
    with pytest.raises(
        MLForgeException, match=rf"Model Version \(name={name}, version=1\) not found"
    ) as exception_context:
...
def test_set_registered_model_tag(store):
    ...
    with pytest.raises(
        MLForgeException, match=rf"Registered Model with name={name1} not found"
    ) as exception_context:
        store.set_registered_model_tag(name1, overriding_tag)
    assert exception_context.value.error_code == ErrorCode.Name(RESOURCE_DOES_NOT_EXIST)
    ...
    with pytest.raises(
        MLForgeException, match=r"Missing value for required parameter 'key'"
    ) as exception_context:
...
def test_transition_model_version_stage_when_archive_existing_versions_is_true(store):
    ...
    for stage in ["Archived", "None"]:
        with pytest.raises(MLForgeException, match=msg):
...
def test_search_model_versions(store):
    ...
    with pytest.raises(
        MLForgeException, match=r"Invalid clause\(s\) in filter string"
    ) as exception_context:
        search_versions("run_id IN (")
    assert exception_context.value.error_code == ErrorCode.Name(INVALID_PARAMETER_VALUE)
...
def test_search_model_versions_order_by_errors(store):
    ...
    with pytest.raises(
        MLForgeException, match=r"Invalid attribute key '.+' specified"
    ) as exception_context:
    ...
    with pytest.raises(MLForgeException, match=r"Invalid order_by clause '.+'") as exception_context:
        store.search_model_versions(
            query,
            page_token=None,
...
def test_search_model_versions_pagination(store):
    ...
    with pytest.raises(
        MLForgeException, match=r"Invalid page token, could not base64-decode"
    ) as exception_context:
    ...
    with pytest.raises(
        MLForgeException, match=r"Invalid value for max_results\."
    ) as exception_context:
...
def test_search_registered_models(store):
    ...
    with pytest.raises(
        MLForgeException, match=r"Invalid attribute key 'run_id' specified."
    ) as exception_context:
    ...
    with pytest.raises(
        MLForgeException, match=r"Invalid attribute key 'source_path' specified."
    ) as exception_context:
...
def test_search_registered_model_pagination(store):
    ...
    # test that providing too large of a max_results throws
    with pytest.raises(
        MLForgeException, match=r"Invalid value for request parameter max_results"
    ) as exception_context:
...
def test_search_registered_model_order_by(store):
    ...
    for i in range(50):
        with mock.patch(
            "MLForge.store.model_registry.sqlalchemy_store.get_current_time_millis",
            return_value=i,
        ):
...
def test_search_registered_model_order_by_errors(store):
    ...
    # test that invalid columns throw even if they come after valid columns
    with pytest.raises(
        MLForgeException, match=r"Invalid order by key '.+' specified"
    ) as exception_context:
...
def test_set_model_version_tag(store):
    ...
    with pytest.raises(
        MLForgeException, match=rf"Model Version \(name={name1}, version=2\) not found"
    ) as exception_context:
    ...
    assert exception_context.value.error_code == ErrorCode.Name(INVALID_PARAMETER_VALUE)
    with pytest.raises(
        MLForgeException, match=r"Parameter 'version' must be an integer, got 'I am not a version'"
    ) as exception_context:
...
def test_delete_model_version_tag(store):
    ...
    with pytest.raises(
        MLForgeException, match=rf"Model Version \(name={name2}, version=1\) not found"
    ) as exception_context:
...
def test_search_prompts(store):
    ...
    rms = store.search_registered_models(
        filter_string="tags.`MLForge.prompt.is_prompt` = 'false'", max_results=10
    )
    assert len(rms) == 1
    assert rms[0].name == "model"

    rms = store.search_registered_models(
        filter_string="tags.`MLForge.prompt.is_prompt` != 'true'", max_results=10
    )
    assert len(rms) == 1
    assert rms[0].name == "model"
    ...
    rms = store.search_registered_models(
        filter_string="name = 'prompt_1' and tags.`MLForge.prompt.is_prompt` = 'true'",
        max_results=10,
    )
    assert len(rms) == 1
    ...
    rms = store.search_registered_models(
        filter_string="tags.`MLForge.prompt.is_prompt` = 'true' and tags.fruit = 'apple'",
        max_results=10,
    )
    assert len(rms) == 1
...
def test_search_prompts_versions(store):
    ...
    mvs = store.search_model_versions(
        filter_string="tags.`MLForge.prompt.is_prompt` = 'true' and name = 'prompt_2'",
        max_results=10,
    )
    assert len(mvs) == 2
...
def test_search_prompt_versions(store):
    ...
    with pytest.raises(MLForgeException, match="registered as a model, not a prompt"):
...
def test_create_registered_model_handle_prompt_properly(store):
    ...
    with pytest.raises(MLForgeException, match=r"Registered Model \(name=model\) already exists"):
        store.create_registered_model("model")

    with pytest.raises(MLForgeException, match=r"Prompt \(name=prompt\) already exists"):
        store.create_registered_model("prompt", tags=prompt_tags)
...
def test_create_webhook_invalid_names(store, invalid_name, expected_match):
    with pytest.raises(MLForgeException, match=expected_match):
...
def test_create_webhook_invalid_events(store):
    # Test empty events
    with pytest.raises(MLForgeException, match="Webhook events must be a non-empty list"):
...
def test_get_webhook_not_found(store):
    with pytest.raises(MLForgeException, match="Webhook with ID nonexistent not found"):
...
def test_list_webhooks_invalid_max_results(store):
    with pytest.raises(MLForgeException, match="max_results must be between 1 and 1000"):
...
def test_delete_webhook(store):
    ...
    with pytest.raises(MLForgeException, match=r"Webhook with ID .* not found"):
...
def test_create_model_version_with_model_id_and_no_run_id(store):
    ...
    with mock.patch(
        "MLForge.store.model_registry.sqlalchemy_store.MLForgeClient"

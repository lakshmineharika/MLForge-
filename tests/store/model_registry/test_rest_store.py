...
from MLForge.entities.model_registry import ModelVersion, ModelVersionTag, RegisteredModelTag
...
from MLForge.prompt.registry_utils import IS_PROMPT_TAG_KEY
...
@pytest.fixture
def creds():
    return MLForgeHostCreds("https://hello")
...
def _args(host_creds, endpoint, method, json_body):
    res = {"host_creds": host_creds, "endpoint": f"/api/2.0/MLForge/{endpoint}", "method": method}
    if method == "GET":
...
def test_await_model_version_creation_pending(store):
    ...
    with (
        mock.patch(
            "MLForge.store.model_registry.abstract_store.AWAIT_MODEL_VERSION_CREATE_SLEEP_INTERVAL_SECONDS",
            1,
        ),
        mock.patch.object(store, "get_model_version", return_value=pending_mv),
        pytest.raises(MLForgeException, match="Exceeded max wait time"),
    ):
...
def test_await_model_version_creation_failed(store):
    ...
    with (
        mock.patch.object(store, "get_model_version", return_value=pending_mv),
        pytest.raises(MLForgeException, match="Model version creation failed for model name"),
    ):
        store._await_model_version_creation(pending_mv, 0.5)
...
def test_await_model_version_creation_show_correct_message_for_prompt(store, is_prompt):
    ...
    with (
        mock.patch("MLForge.store.model_registry.abstract_store._logger") as mock_logger,

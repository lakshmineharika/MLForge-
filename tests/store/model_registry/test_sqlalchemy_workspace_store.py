...
from MLForge.store.model_registry.sqlalchemy_workspace_store import WorkspaceAwareSqlAlchemyStore
from MLForge.utils.workspace_context import WorkspaceContext, clear_server_request_workspace
...
def test_registered_model_operations_are_workspace_scoped(workspace_registry_store):
    ...
    with WorkspaceContext("team-b"):
        ...
        with pytest.raises(
            MLForgeException, match="Registered Model with name=alpha not found"
        ) as excinfo:
            workspace_registry_store.set_registered_model_tag(
                "alpha", RegisteredModelTag("owner", "team-b")
            )
        assert excinfo.value.error_code == "RESOURCE_DOES_NOT_EXIST"
        with pytest.raises(MLForgeException, match="Registered Model with name=alpha not found"):
    ...
    with WorkspaceContext("team-b"):
        with pytest.raises(
            MLForgeException, match="Registered Model with name=alpha-renamed not found"
        ) as excinfo:
            workspace_registry_store.set_registered_model_tag(
...
def test_model_version_operations_are_workspace_scoped(workspace_registry_store):
    ...
    with WorkspaceContext("team-b"):
        ...
        for call in version_scoped_calls:
            with pytest.raises(
                MLForgeException, match=r"Model Version \(name=alpha, version=1\) not found"
            ) as excinfo:
                call()
            assert excinfo.value.error_code == "RESOURCE_DOES_NOT_EXIST"
...
def test_model_version_allows_workspace_scoped_proxied_artifacts(
    ...
    with WorkspaceContext("team-a"):
        ...
        mv = workspace_registry_store.create_model_version(
            "alpha",
            "MLForge-artifacts:/workspaces/team-a/models/model-a",
...
def test_webhook_operations_are_workspace_scoped(workspace_registry_store):
    ...
    with WorkspaceContext("team-b"):
        ...
        with pytest.raises(
            MLForgeException, match=f"Webhook with ID {webhook.webhook_id} not found"
        ) as excinfo:

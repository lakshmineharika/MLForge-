...
def test_create_registered_model(store):
    # Error cases
    with pytest.raises(MLForgeException, match=r"Missing value for required parameter 'name'\."):
...
def test_create_registered_model_with_name_that_looks_like_path(store, tmp_path):
    ...
    with pytest.raises(MLForgeException, match=r"Names cannot contain '/' or ':'"):
        store.get_registered_model(name)


def test_create_registered_model_with_percent_in_name(store, tmp_path):
    with pytest.raises(
        MLForgeException, match=r"Registered model name cannot contain '%' character"
    ):
...
def test_get_registered_model(store, registered_model_names, rm_data):
    ...
    name = random_str()
    with pytest.raises(MLForgeException, match=f"Registered Model with name={name} not found"):
        store.get_registered_model(name)

    name = "../../path"
    with pytest.raises(MLForgeException, match="Names cannot contain '/' or ':'"):
        store.get_registered_model(name)
...
def test_delete_registered_model(store, registered_model_names):
    ...
    # Error cases
    with pytest.raises(
        MLForgeException, match=f"Registered Model with name={model_name}!!! not found"
    ):
    ...
    with pytest.raises(MLForgeException, match=f"Registered Model with name={model_name} not found"):
        store.delete_registered_model(model_name)
...
def test_set_registered_model_tag(store):
    ...
    with pytest.raises(
        MLForgeException, match=f"Registered Model with name={name1} not found"
    ) as exception_context:
        store.set_registered_model_tag(name1, overriding_tag)
...
def test_search_registered_model_order_by(store):
    ...
    assert res.names == rms
    with mock.patch(
        "MLForge.store.model_registry.file_store.get_current_time_millis", return_value=1
    ):
        rm1 = store.create_registered_model("MR1").name
        rm2 = store.create_registered_model("MR2").name
    with mock.patch(
        "MLForge.store.model_registry.file_store.get_current_time_millis", return_value=2
    ):
...
def test_search_registered_model_order_by_errors(store):
    ...
    with pytest.raises(
        MLForgeException, match="Invalid attribute key 'description' specified."
    ) as exception_context:
...
def test_delete_registered_model_alias(store):
    ...
    mv2 = store.get_model_version(model_name, 2)
    assert mv2.aliases == []
    with pytest.raises(MLForgeException, match=r"Registered model alias test_alias not found."):
...
def test_pyfunc_model_registry_with_file_store(store):
    ...
    MLForge.set_registry_uri(path_to_local_file_uri(store.root_directory))
    with MLForge.start_run():
        MLForge.pyfunc.log_model(name="foo", python_model=MyModel(), registered_model_name="model1")
        MLForge.pyfunc.log_model(name="foo", python_model=MyModel(), registered_model_name="model2")
        MLForge.pyfunc.log_model(
            name="model", python_model=MyModel(), registered_model_name="model1"
        )

    with MLForge.start_run():
        MLForge.log_param("A", "B")
...
def test_update_model_version_with_model_id_and_metrics(store: FileStore):
    ...
    with MLForge.start_run() as run:
        MLForge.log_param("learning_rate", "0.001")
        model_info = MLForge.pyfunc.log_model(
            name="model",
            python_model=SimpleModel(),
        )
        MLForge.log_metric("execution_time", 33.74682, step=0, model_id=model_info.model_id)
        run_id = run.info.run_id
        model_id = model_info.model_id

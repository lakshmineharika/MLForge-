...
from MLForge.store.model_registry.databricks_workspace_model_registry_rest_store import (
    DatabricksWorkspaceModelRegistryRestStore,
    _extract_workspace_id_from_run_link,
)

UC_STORE_CLASS_SELECTOR = (
    "MLForge.store._unity_catalog.registry.utils.get_uc_model_registry_store_class"
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # valid workspace ID
        (
            "https://workspace.databricks.com/?o=10002#MLForge/experiments/test-exp-id/runs/runid",
            "10002",
        ),
        # valid workspace ID with artifact path
        (
            "https://workspace.databricks.com/?o=12345#MLForge/experiments/test-exp-id/runs/runid/artifactPath/model",
            "12345",
        ),
        # different Databricks domain
        (
            "https://mycompany.cloud.databricks.com/?o=98765#MLForge/experiments/test-exp-id/runs/runid",
            "98765",
        ),
        # multiple query parameters
        (
            "https://workspace.databricks.com/?param1=value1&o=67890&param2=value2#MLForge/experiments/test-exp-id/runs/runid",
            "67890",
        ),
        ...
        (
            "https://workspace.databricks.com/#MLForge/experiments/test-exp-id/runs/runid",
            None,
        ),
        # empty workspace ID parameter
        (
            "https://workspace.databricks.com/?o=#MLForge/experiments/test-exp-id/runs/runid",
            None,
        ),
        # non-numeric workspace ID
        (
            "https://workspace.databricks.com/?o=abc123#MLForge/experiments/test-exp-id/runs/runid",
            None,
        ),
        # workspace ID with spaces
        (
            "https://workspace.databricks.com/?o=12 34#MLForge/experiments/test-exp-id/runs/runid",
            None,
        ),
        # negative workspace ID
        (
            "https://workspace.databricks.com/?o=-123#MLForge/experiments/test-exp-id/runs/runid",
            None,
        ),
        # float workspace ID
        (
            "https://workspace.databricks.com/?o=123.45#MLForge/experiments/test-exp-id/runs/runid",
            None,
        ),
        # malformed URL missing equals sign
        (
            "https://workspace.databricks.com/?o12345#MLForge/experiments/test-exp-id/runs/runid",
            None,
        ),
        # empty string input
        ("", None),
...
def test_copy_model_version_unity_catalog_success(store, sample_model_version):
    ...
    with (
        mock.patch(
            "MLForge.artifacts.download_artifacts", return_value="/tmp/local_model_dir"
        ) as mock_download,
...
def test_copy_model_version_unity_catalog_migration_download_failure(store, sample_model_version):
    ...
    with mock.patch(
        "MLForge.artifacts.download_artifacts", side_effect=Exception("Download failed")
    ) as mock_download:
        with pytest.raises(MLForgeException, match="Unable to download model test_model version 1"):
...
def test_copy_model_version_unity_catalog_registered_model_already_exists(
    ...
    with (
        ...
        mock_exception = MLForgeException("Model already exists", error_code=RESOURCE_ALREADY_EXISTS)
...
def test_copy_model_version_unity_catalog_registered_model_creation_failure(
    ...
    with (
        ...
        mock_uc_store_class, mock_uc_store = _mock_uc_store_selector(mock_get_uc_store_class)
        mock_uc_store.create_registered_model.side_effect = MLForgeException(
...
def test_copy_model_version_unity_catalog_signature_validation_bypass(
    ...
    dst_name = "catalog.schema.model"
    with (
        mock.patch("MLForge.artifacts.download_artifacts", return_value="/tmp/local_model_dir"),
        mock.patch(UC_STORE_CLASS_SELECTOR) as mock_get_uc_store_class,
    ):
        mock_uc_store_class, mock_uc_store = _mock_uc_store_selector(mock_get_uc_store_class)
        ...
        monkeypatch.setenv("MLForge_SKIP_SIGNATURE_CHECK_FOR_UC_REGISTRY_MIGRATION", "True")
        store.copy_model_version(sample_model_version, dst_name)

...
from MLForge.entities.model_registry.model_version import ModelVersion
from MLForge.entities.model_registry.model_version_tag import ModelVersionTag
...
from MLForge.store.model_registry.abstract_store import AbstractStore
...
def mock_tracking_store():
    with mock.patch("MLForge.tracking._get_store") as mock_get_store:
...
def test_link_prompt_version_to_model_no_model_found(store, mock_tracking_store):
    ...
    # Execute & Verify
    with pytest.raises(MLForgeException, match="Could not find model with ID 'nonexistent_model'"):
...
def test_link_prompt_version_to_model_prompt_not_found(store, mock_tracking_store):
    ...
    # Execute & Verify
    with pytest.raises(
        MLForgeException, match="Prompt version 'nonexistent_prompt' version 'v1' not found"
    ):
...
def test_link_prompt_version_to_model_invalid_json_tag(store, mock_tracking_store):
    ...
    # Execute & Verify
    with pytest.raises(MLForgeException, match="Invalid JSON format for 'MLForge.linkedPrompts' tag"):
        store.link_prompt_version_to_model("test_prompt", "v1", model_id)
...
def test_link_prompt_version_to_model_invalid_format_tag(store, mock_tracking_store):
    ...
    # Execute & Verify
    with pytest.raises(MLForgeException, match="Invalid format for 'MLForge.linkedPrompts' tag"):
        store.link_prompt_version_to_model("test_prompt", "v1", model_id)
...
def test_link_prompt_version_to_run_no_run_found(store, mock_tracking_store):
    ...
    with pytest.raises(MLForgeException, match="Could not find run"):
        store.link_prompt_version_to_run("test_prompt", "1", run_id)

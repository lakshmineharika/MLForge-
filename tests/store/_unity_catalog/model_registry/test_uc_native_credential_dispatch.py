...
def test_dispatch_gcp():
    ...
    with (
        mock.patch("MLForge.store.artifact.gcs_artifact_repo.GCSArtifactRepository") as repo,
        mock.patch.dict(
            "sys.modules",
            {
...
def test_dispatch_r2():
    ...
    with mock.patch("MLForge.store.artifact.r2_artifact_repo.R2ArtifactRepository") as repo:
        _get_artifact_repo_from_storage_info(STORAGE, token, _refresh_returning(token))
    repo.assert_called_once()
    _, kwargs = repo.call_args
    assert kwargs["access_key_id"] == "ak"

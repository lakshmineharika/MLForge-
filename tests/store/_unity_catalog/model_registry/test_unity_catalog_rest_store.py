...
from MLForge.data.dataset import Dataset
from MLForge.data.delta_dataset_source import DeltaDatasetSource
from MLForge.data.pandas_dataset import PandasDataset
from MLForge.entities.logged_model import LoggedModel
from MLForge.entities.logged_model_tag import LoggedModelTag
from MLForge.entities.model_registry import ModelVersionTag, RegisteredModelTag
from MLForge.entities.model_registry.prompt import Prompt
from MLForge.entities.model_registry.prompt_version import PromptVersion
from MLForge.entities.run import Run
from MLForge.entities.run_data import RunData
from MLForge.entities.run_info import RunInfo
from MLForge.entities.run_inputs import RunInputs
from MLForge.entities.run_tag import RunTag
from MLForge.exceptions import MLForgeException, RestException
from MLForge.models.model import MLMODEL_FILE_NAME
from MLForge.models.signature import ModelSignature, Schema
from MLForge.prompt.constants import (
...
from MLForge.protos.databricks_pb2 import INTERNAL_ERROR, RESOURCE_DOES_NOT_EXIST
from MLForge.protos.databricks_uc_registry_messages_pb2 import (
...
from MLForge.protos.databricks_uc_registry_messages_pb2 import ModelVersion as ProtoModelVersion
from MLForge.protos.service_pb2 import GetRun
from MLForge.protos.unity_catalog_prompt_messages_pb2 import (
    LinkPromptsToTracesRequest,
    LinkPromptVersionsToModelsRequest,
    LinkPromptVersionsToRunsRequest,
)
from MLForge.store._unity_catalog.registry.rest_store import (
    _DATABRICKS_LINEAGE_ID_HEADER,
    _DATABRICKS_ORG_ID_HEADER,
    UcModelRegistryStore,
)
from MLForge.store.artifact.azure_data_lake_artifact_repo import AzureDataLakeArtifactRepository
from MLForge.store.artifact.gcs_artifact_repo import GCSArtifactRepository
from MLForge.store.artifact.optimized_s3_artifact_repo import OptimizedS3ArtifactRepository
from MLForge.store.artifact.presigned_url_artifact_repo import PresignedUrlArtifactRepository
from MLForge.tracing.constant import TraceTagKey
from MLForge.types.schema import ColSpec, DataType
from MLForge.utils._unity_catalog_utils import (
    _ACTIVE_CATALOG_QUERY,
    _ACTIVE_SCHEMA_QUERY,
    get_artifact_repo_from_storage_info,
    uc_model_version_tag_from_MLForge_tags,
    uc_registered_model_tag_from_MLForge_tags,
)
from MLForge.utils.MLForge_tags import (
    MLForge_DATABRICKS_JOB_ID,
    MLForge_DATABRICKS_JOB_RUN_ID,
...
def spark_session(request):
    with mock.patch(
        "MLForge.store._unity_catalog.registry.rest_store._get_active_spark_session"
...
def _args(endpoint, method, json_body, host_creds, extra_headers):
    res = {
        ...
        "endpoint": f"/api/2.0/MLForge/unity-catalog/{endpoint}",
        "method": method,
...
def test_create_registered_model(mock_http, store):
    ...
    _verify_requests(
        ...
        CreateRegisteredModelRequest(
            ...
            tags=uc_registered_model_tag_from_MLForge_tags(tags),
        ),
    )
...
def test_create_registered_model_three_level_name_hint(store):
    ...
    with mock.patch.object(store, "_call_endpoint", side_effect=rest_exception):
        with pytest.raises(MLForgeException, match=original_error_message) as exc_info:
...
def test_create_model_version_with_langchain_dependencies(store, langchain_local_model_dir):
    ...
    with (
        ...
        mock.patch(
            "MLForge.store.artifact.optimized_s3_artifact_repo.OptimizedS3ArtifactRepository",
...
def feature_store_local_model_dir(tmp_path):
    fake_mlmodel_contents = {
        ...
        "flavors": {"python_function": {"loader_module": "databricks.feature_store.MLForge_model"}},
    }
    with open(tmp_path.joinpath(MLMODEL_FILE_NAME), "w") as handle:
        yaml.dump(fake_mlmodel_contents, handle)
    return tmp_path
...
def test_create_model_version_missing_mlmodel(store, tmp_path):
    with pytest.raises(
        ...
        "being registered points to a valid MLForge model directory ",
    ):
...
def test_get_logged_model_from_model_id_returns_none_on_resource_not_found(store):
    with mock.patch(
        "MLForge.get_logged_model",
        side_effect=MLForgeException("Node ID does not exist", error_code=RESOURCE_DOES_NOT_EXIST),
    ):
...
def test_get_logged_model_from_model_id_returns_logged_model_on_success(store):
    ...
    with mock.patch("MLForge.get_logged_model", return_value=mock_logged_model):
...
def test_get_logged_model_from_model_id_reraises_other_exceptions(store):
    with mock.patch(
        "MLForge.get_logged_model",
        side_effect=MLForgeException("Some other error", error_code=INTERNAL_ERROR),
    ):
        with pytest.raises(MLForgeException, match="Some other error"):
...
def test_create_model_version_with_sse_kms_client(
    ...
    optimized_s3_repo_package = "MLForge.store.artifact.optimized_s3_artifact_repo"
...
def test_download_model_weights_if_not_saved(
    ...
    with mock.patch("MLForge.transformers") as transformers_mock:
...
def test_search_registered_models_invalid_args(store):
    ...
    for sz in range(1, 3):
        for combination in combinations(params_list, sz):
            params = {k: v for d in combination for k, v in d.items()}
            with pytest.raises(
                MLForgeException, match="unsupported for models in the Unity Catalog"
            ):
                store.search_registered_models(**params)
...
def test_get_notebook_id_returns_expected_id(store):
    test_tag = RunTag(key=MLForge_DATABRICKS_NOTEBOOK_ID, value="123")
...
def test_get_job_id_returns_expected_id(store):
    test_tag = RunTag(key=MLForge_DATABRICKS_JOB_ID, value="123")
    test_run_data = RunData(tags=[test_tag])
    test_run_info = RunInfo(
        "run_uuid",
        "experiment_id",
...
def test_get_job_run_id_returns_expected_id(store):
    test_tag = RunTag(key=MLForge_DATABRICKS_JOB_RUN_ID, value="123")
    test_run_data = RunData(tags=[test_tag])
    test_run_info = RunInfo(
        "run_uuid",
        "experiment_id",
...
def test_get_run_and_headers_returns_none_if_request_fails(store, status_code, response_text):
    ...
    with mock.patch(
        "MLForge.store._unity_catalog.registry.rest_store.http_request", return_value=mock_response
    ):
...
def test_get_run_and_headers_returns_none_if_tracking_uri_not_databricks(
    ...
    with mock.patch("MLForge.utils.databricks_utils.get_databricks_host_creds"):
        ...
        mock_response.text = "{}"
        with mock.patch(
            "MLForge.store._unity_catalog.registry.rest_store.http_request",
...
def get_request_mock(
    ...
    def request_mock(
        ...
        **kwargs,
        ...
        uc_tags = uc_model_version_tag_from_MLForge_tags(tags) if tags is not None else []
        req_info_to_response = {
            (
                _REGISTRY_HOST_CREDS.host,
                "/api/2.0/MLForge/unity-catalog/model-versions/create",
                "POST",
                message_to_json(
                    CreateModelVersionRequest(
                        name=name,
            ...
            (
                _REGISTRY_HOST_CREDS.host,
                "/api/2.0/MLForge/unity-catalog/model-versions/generate-temporary-credentials",
                "POST",
                message_to_json(
                    GenerateTemporaryModelVersionCredentialsRequest(
            ...
            (
                _REGISTRY_HOST_CREDS.host,
                "/api/2.0/MLForge/unity-catalog/model-versions/finalize",
                "POST",
...
def test_create_model_version_local_model_path(store, local_model_dir):
    ...
    with (
        ...
        mock.patch("MLForge.artifacts.download_artifacts") as mock_download_artifacts,
        mock.patch(
            "MLForge.store.artifact.optimized_s3_artifact_repo.OptimizedS3ArtifactRepository",
            return_value=mock_artifact_repo,
        ),
...
def test_create_model_version_remote_source(store, local_model_dir, tmp_path):
    ...
    with (
        ...
        mock.patch(
            "MLForge.artifacts.download_artifacts",
...
def test_create_model_version_azure(store, local_model_dir):
    ...
    with (
        ...
        mock.patch(
            "MLForge.store.artifact.azure_data_lake_artifact_repo.AzureDataLakeArtifactRepository",
...
def test_create_model_version_gcp(store, local_model_dir, create_args):
    ...
    if "run_id" in create_kwargs:
        test_notebook_tag = RunTag(key=MLForge_DATABRICKS_NOTEBOOK_ID, value="321")
        test_job_tag = RunTag(key=MLForge_DATABRICKS_JOB_ID, value="456")
        test_job_run_tag = RunTag(key=MLForge_DATABRICKS_JOB_RUN_ID, value="789")
    ...
    with (
        ...
        mock.patch(
            "MLForge.store._unity_catalog.registry.rest_store.UcModelRegistryStore._get_run_and_headers",
        ...
        mock.patch(
            "MLForge.store.artifact.gcs_artifact_repo.GCSArtifactRepository",
...
def test_local_model_dir_preserves_uc_volumes_path(tmp_path):
    ...
    with (
        mock.patch(
            "MLForge.artifacts.download_artifacts", return_value=str(tmp_path)
        ) as mock_download_artifacts,
        mock.patch(
            # Pretend that `tmp_path` is a UC Volumes path
            "MLForge.store._unity_catalog.registry.rest_store.is_fuse_or_uc_volumes_uri",
            return_value=True,
...
def test_create_model_version_unsupported_fields(store):
    with pytest.raises(MLForgeException, match=_expected_unsupported_arg_error_message("run_link")):
...
def test_search_model_versions_order_by_unsupported(store):
    with pytest.raises(MLForgeException, match=_expected_unsupported_arg_error_message("order_by")):
        store.search_model_versions(
...
def test_store_use_presigned_url_store_when_disabled(monkeypatch):
    store_package = "MLForge.store._unity_catalog.registry.rest_store"
    monkeypatch.setenv("MLForge_USE_DATABRICKS_SDK_MODEL_ARTIFACTS_REPO_FOR_UC", "false")
...
def test_create_and_update_registered_model_print_job_url(mock_http, store):
    ...
    with (
        mock.patch(
            "MLForge.store._unity_catalog.registry.rest_store._print_databricks_deployment_job_url",
        ) as mock_print_url,
    ):
...
def test_create_prompt_uc(mock_http, store, monkeypatch):
    ...
    tags = {"foo": "bar"}
    # Patch proto_info_to_MLForge_prompt_info to return a dummy Prompt
    with mock.patch(
        "MLForge.store._unity_catalog.registry.rest_store.proto_info_to_MLForge_prompt_info",
...
def test_set_prompt_tag_uc(mock_http, store, monkeypatch):
    ...
    expected_endpoint = f"/api/2.0/MLForge/unity-catalog/prompts/{name}/tags"
...
def test_delete_prompt_tag_uc(mock_http, store, monkeypatch):
    ...
    expected_endpoint = f"/api/2.0/MLForge/unity-catalog/prompts/{name}/tags/{key}"
    assert any(c[1]["endpoint"] == expected_endpoint for c in mock_http.call_args_list)
...
def test_create_prompt_version_uc(mock_http, store, monkeypatch):
    ...
    tags = {"env": "test"}
    # Patch proto_to_MLForge_prompt to return a dummy PromptVersion
    with mock.patch(
        "MLForge.store._unity_catalog.registry.rest_store.proto_to_MLForge_prompt",
...
def test_link_prompt_version_to_model_success(store):
    with (
        ...
        mock.patch(
            "MLForge.store.model_registry.abstract_store.AbstractStore.link_prompt_version_to_model"
        ) as mock_super_call,
    ):
        # Setup
        mock_get_endpoint.return_value = (
            "/api/2.0/MLForge/unity-catalog/prompts/link-to-model",
            "POST",
        )
...
def test_link_prompt_version_to_model_sets_tag(store):
    ...
    with (
        mock.patch("MLForge.tracking._get_store") as mock_get_tracking_store,
...
def test_link_prompts_to_trace_success(store):
    with (
        ...
        # Setup
        mock_get_endpoint.return_value = (
            "/api/2.0/MLForge/unity-catalog/prompt-versions/links-to-traces",
            "POST",
        )
...
def test_link_prompt_version_to_run_success(store):
    with (
        ...
        mock.patch.object(store, "_get_endpoint_from_method") as mock_get_endpoint,
        mock.patch(
            "MLForge.store.model_registry.abstract_store.AbstractStore.link_prompt_version_to_run"
        ) as mock_super_call,
    ):
        # Setup
        mock_get_endpoint.return_value = (
            "/api/2.0/MLForge/unity-catalog/prompt-versions/links-to-runs",
            "POST",
        )

...
# MLForge-enrichment fields as flattened siblings), it produces an MLForge entity that is
# **field-for-field identical** to the one produced from the legacy surface
# (``/api/2.0/MLForge/unity-catalog/*``, the legacy ``RegisteredModel`` / ``ModelVersion`` protos).
#
...
from MLForge.entities import Metric
from MLForge.entities.logged_model_parameter import LoggedModelParameter as ModelParam
...
from MLForge.entities.model_registry.model_version_deployment_job_state import (
    ModelVersionDeploymentJobState,
)
from MLForge.entities.model_registry.registered_model_deployment_job_state import (
...
from MLForge.store._unity_catalog.registry.uc_native_rest_store import (
...
# ---------------------------------------------------------------------------
# Pinned legacy converters (pre-migration MLForge/utils/_unity_catalog_utils.py) -- the oracle.
# ---------------------------------------------------------------------------
...
def test_registered_model_native_legacy_parity():
    ...
    # Legacy MLForge-dialect response (what the pre-migration client received).

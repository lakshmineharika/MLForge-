...
from MLForge.protos.unity_catalog_messages_pb2 import (
    AwsCredentials,
    CreateModelVersion,
...
from MLForge.store._unity_catalog.lineage.constants import _DATABRICKS_ORG_ID_HEADER
from MLForge.store._unity_catalog.registry.rest_store import UcModelRegistryStore
from MLForge.store._unity_catalog.registry.uc_native_rest_store import UcNativeModelRegistryStore
...
def store(mock_databricks_uc_host_creds, monkeypatch):
    monkeypatch.setenv("MLForge_ENABLE_UC_NATIVE_MODEL_REGISTRY", "true")
...
def test_create_registered_model_three_level_name_hint(store):
    ...
    with pytest.raises(MLForgeException, match="three levels") as exc_info:
...
def test_create_registered_model_metastore_does_not_exist_hint(store):
    ...
    with mock.patch.object(store, "_edit_endpoint_and_call", side_effect=rest_exception):
        with pytest.raises(MLForgeException, match="METASTORE_DOES_NOT_EXIST") as exc_info:
...
def test_create_model_version_optional_signature_validation(store, tmp_path, bypass):
    ...
    store.spark = None
    rest_store = "MLForge.store._unity_catalog.registry.rest_store"
    with (
...
def test_default_values_for_tags(store, tmp_path, tags):
    ...
    rest_store = "MLForge.store._unity_catalog.registry.rest_store"
    native_store = "MLForge.store._unity_catalog.registry.uc_native_rest_store"
    with (
...
def test_create_and_update_registered_model_print_job_url(mock_http, store):
    ...
    with (
        mock.patch(
            "MLForge.store._unity_catalog.registry.uc_native_rest_store."
...
# MLForge_ENABLE_UC_NATIVE_MODEL_REGISTRY is set; the legacy store is a separate class.
# ---------------------------------------------------------------------------
...
def test_get_model_version_uses_native_endpoint(store):
    ...
    assert result.name == "catalog.schema.model"
    # MLForge entity version is a string (int64 governance version is stringified).
...
def test_search_model_versions_rejects_unsupported_filter(store, filter_string):
    with pytest.raises(MLForgeException, match="name = 'model_name'"):
...
# ---------------------------------------------------------------------------
# Native write paths: the governance entity rides with the MLForge write inputs as
...
def test_create_registered_model_rejects_non_three_level_name(store):
    ...
    store.spark = None
    with pytest.raises(MLForgeException, match="three levels"):
...
def test_create_model_version_uses_native_when_no_dependencies(store, tmp_path):
    ...
    with (
        ...
        mock.patch.object(store, "_download_model_weights_if_not_saved"),
        mock.patch(
            "MLForge.store._unity_catalog.registry.rest_store.get_feature_dependencies",
            return_value="",
        ),
        mock.patch(
            "MLForge.store._unity_catalog.registry.rest_store.get_model_version_dependencies",
            return_value=[],
        ),
...
def test_create_model_version_translates_dependencies_to_governance(store, tmp_path):
    # The MLForge resource dependencies are translated into the governance DependencyList on the
    ...
    with (
        ...
        mock.patch.object(store, "_download_model_weights_if_not_saved"),
        mock.patch(f"{rest_store}.get_feature_dependencies", return_value=""),
        mock.patch(f"{rest_store}.get_model_version_dependencies", return_value=MLForge_deps),
        mock.patch.object(store, "_local_model_dir") as local_model_dir,
    ):
        local_model_dir.return_value.__enter__.return_value = str(tmp_path)
...
def test_create_model_version_omits_dependencies_when_none_supported(store, tmp_path):
    ...
    MLForge_deps = [{"type": "DATABRICKS_MODEL_ENDPOINT", "name": "my_endpoint"}]
    native_mv = ModelVersionInfo(
        model_name="model", catalog_name="catalog", schema_name="schema", version=1
    )
    rest_store = "MLForge.store._unity_catalog.registry.rest_store"
...
def test_set_registered_model_alias_uses_native(store):
    ...
    # JSON number by MLForge's message_to_json).

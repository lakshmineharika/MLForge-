...
from MLForge.store._unity_catalog.registry.utils import (
    MLForge_prompt_to_proto,
    MLForge_tags_to_proto,
    MLForge_tags_to_proto_version_tags,
    proto_info_to_MLForge_prompt_info,
    proto_to_MLForge_prompt,
    proto_to_MLForge_tags,
    proto_version_tags_to_MLForge_tags,
)


def test_proto_to_MLForge_tags():
    # Test with empty tags
    assert proto_to_MLForge_tags([]) == {}
    ...
    assert proto_to_MLForge_tags(proto_tags) == expected

    # Test with None
    assert proto_to_MLForge_tags(None) == {}


def test_MLForge_tags_to_proto():
    # Test with empty tags
    assert MLForge_tags_to_proto({}) == []

    # Test with tags
    tags = {"key1": "value1", "key2": "value2"}
    proto_tags = MLForge_tags_to_proto(tags)
    ...
    # Test with None
    assert MLForge_tags_to_proto(None) == []


def test_proto_info_to_MLForge_prompt_info():
    ...
    prompt_info = proto_info_to_MLForge_prompt_info(proto_info)
    ...
    prompt_info = proto_info_to_MLForge_prompt_info(proto_info, prompt_tags)
    expected_tags = {
        "key1": "value1",
        "key2": "value2",
        "tag1": "value1",
...
def test_proto_to_MLForge_prompt():
    ...
    result = proto_to_MLForge_prompt(proto_version)
    ...
    # Verify user tags remain and _MLForge tags are filtered out
...
def test_MLForge_prompt_to_proto():
    ...
    # Convert to proto
    proto_version = MLForge_prompt_to_proto(prompt)
...
def test_proto_version_tags_to_MLForge_tags():
    # Test with empty tags
    assert proto_version_tags_to_MLForge_tags([]) == {}
    ...
    expected = {"key1": "value1", "key2": "value2"}
    assert proto_version_tags_to_MLForge_tags(proto_tags) == expected

    # Test with None
    assert proto_version_tags_to_MLForge_tags(None) == {}


def test_MLForge_tags_to_proto_version_tags():
    # Test with empty tags
    assert MLForge_tags_to_proto_version_tags({}) == []

    # Test with tags
    tags = {"key1": "value1", "key2": "value2"}
    proto_tags = MLForge_tags_to_proto_version_tags(tags)
    assert len(proto_tags) == 2
    assert all(isinstance(tag, ProtoPromptVersionTag) for tag in proto_tags)
    assert {tag.key: tag.value for tag in proto_tags} == tags

    # Test with None
    assert MLForge_tags_to_proto_version_tags(None) == []

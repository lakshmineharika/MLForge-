...
from MLForge.store._unity_catalog.lineage.constants import _DATABRICKS_LINEAGE_ID_HEADER
from MLForge.store._unity_catalog.registry.rest_store import UcModelRegistryStore
from MLForge.store.artifact.databricks_sdk_models_artifact_repo import (
...
class SimpleModel(MLForge.pyfunc.PythonModel):
...
def test_downstream_notebook_job_lineage(
    ...
    with (
        mock.patch(
            "MLForge.utils.databricks_utils.is_in_databricks_notebook",
            return_value=is_in_notebook,
        ),
        mock.patch(
            "MLForge.utils.databricks_utils.is_in_databricks_runtime",
            return_value=is_in_notebook or is_in_job,
        ),
        mock.patch(
            "MLForge.utils.databricks_utils.is_in_databricks_job",
            return_value=is_in_job,
        ),
        mock.patch(
            "MLForge.utils.databricks_utils.get_notebook_id",
            return_value=notebook_id,
        ),
        mock.patch(
            "MLForge.utils.databricks_utils.get_job_id",
            return_value=job_id,
        ),
        mock.patch("MLForge.get_registry_uri", return_value="databricks-uc"),
        mock.patch.object(
            UnityCatalogModelsArtifactRepository,
        ...
        mock.patch(
            "MLForge.utils._unity_catalog_utils._get_artifact_repo_from_storage_info",
        ...
        MLForge.pyfunc.save_model(path=model_dir, python_model=SimpleModel())
        MLForge.pyfunc.load_model(model_uri)
...
def test_databricks_sdk_models_artifact_repo_lineage(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("DATABRICKS_HOST", "my-host")
    monkeypatch.setenv("DATABRICKS_TOKEN", "my-token")
    monkeypatch.setenv("MLForge_USE_DATABRICKS_SDK_MODEL_ARTIFACTS_REPO_FOR_UC", "true")

    with mock.patch(
        "MLForge.utils._unity_catalog_utils.call_endpoint",

...
from MLForge.protos import review_queues_pb2 as pb
from MLForge.store.tracking.rest_store import RestStore
from MLForge.utils.rest_utils import MLForgeHostCreds

_CALL = "MLForge.store.tracking.rest_store.call_endpoint"


def _store():
    return RestStore(lambda: MLForgeHostCreds("https://hello"))
...
def test_create_review_queue_builds_request_and_parses_response():
    ...
    assert captured["endpoint"] == "/api/3.0/MLForge/review-queues/create"
...
def test_get_or_create_user_queue_endpoint():
    ...
    assert captured["endpoint"] == "/api/3.0/MLForge/review-queues/get-or-create-user"
...
def test_update_review_queue_sets_flags():
    ...
    assert captured["endpoint"] == "/api/3.0/MLForge/review-queues/update"
...
def test_add_items_sends_item_type_and_ids():
    ...
    assert captured["endpoint"] == "/api/3.0/MLForge/review-queues/items/add"
...
def test_list_review_queue_items_status_filter_and_pagination():
    ...
    assert captured["endpoint"] == "/api/3.0/MLForge/review-queues/items/list"
...
def test_set_status_sends_completed_by():
    ...
    assert captured["endpoint"] == "/api/3.0/MLForge/review-queues/items/set-status"
    assert captured["body"]["status"] == "COMPLETE"
...
def test_list_review_queues_parses_paged_response():
    ...
    assert captured["endpoint"] == "/api/3.0/MLForge/review-queues/list"
    assert captured["body"]["user"] == "alice"
...
def test_delete_review_queue_endpoint():
    ...
    assert captured["endpoint"] == "/api/3.0/MLForge/review-queues/delete"
...
def test_get_review_queue_endpoint():
    ...
    assert captured["endpoint"] == "/api/3.0/MLForge/review-queues/get"
    assert captured["body"]["queue_id"] == "rq-1"
    assert result.queue_id == "rq-1"
...
def test_get_review_queue_by_name_endpoint():
    ...
    assert captured["endpoint"] == "/api/3.0/MLForge/review-queues/get-by-name"
...
def test_remove_items_endpoint():
    ...
    assert captured["endpoint"] == "/api/3.0/MLForge/review-queues/items/remove"
    assert captured["body"]["queue_id"] == "rq-1"
    assert captured["body"]["item_ids"] == ["tr-1", "tr-2"]


# Unknown enum strings are rejected with a clear MLForgeException before the RPC,

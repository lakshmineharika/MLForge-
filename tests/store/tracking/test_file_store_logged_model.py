...
import MLForge
from MLForge.entities.logged_model_parameter import LoggedModelParameter
...
from MLForge.store.tracking.file_store import SEARCH_LOGGED_MODEL_MAX_RESULTS_DEFAULT, FileStore
...
def test_initialize_logged_model_when_set_experiment():
    exp = MLForge.set_experiment("test")
    logged_model = MLForge.initialize_logged_model(exp.experiment_id)
...
def test_create_logged_model_errors(store):
    with pytest.raises(MLForgeException, match=r"Could not find experiment with ID 123"):
    ...
    with pytest.raises(MLForgeException, match=r"A key name must be provided."):
        store.create_logged_model(params=[LoggedModelParameter(None, "b")])

    with pytest.raises(MLForgeException, match=r"exceeds the maximum length"):
...
def test_create_logged_model_invalid_name(store: FileStore, name: str):
    exp_id = store.create_experiment(f"exp-{uuid.uuid4()}")
    with pytest.raises(MLForgeException, match="Invalid model name"):
...
def test_set_logged_model_tags_errors(store):
    logged_model = store.create_logged_model()
    with pytest.raises(MLForgeException, match=r"Missing value for required parameter"):
        store.set_logged_model_tags(logged_model.model_id, [LoggedModelTag(None, None)])
    with pytest.raises(MLForgeException, match=r"Names may only contain alphanumerics"):
...
def test_get_logged_model_errors(store):
    with pytest.raises(MLForgeException, match=r"Model '1234' not found"):
        store.get_logged_model("1234")

    with (
        mock.patch(
            "MLForge.store.tracking.file_store.FileStore._find_model_root",
            return_value=("0", "abc"),
        ),
        mock.patch(
            "MLForge.store.tracking.file_store.FileStore._get_model_info_from_dir",
            return_value={"experiment_id": "1"},
        ),
    ):
        with pytest.raises(MLForgeException, match=r"Model '1234' metadata is in invalid state"):
            store.get_logged_model("1234")
...
def test_search_logged_models_errors(store):
    exp_id = store.create_experiment("test")
    with pytest.raises(MLForgeException, match=r"Invalid attribute key 'artifact_location'"):
        store.search_logged_models(experiment_ids=[exp_id], filter_string="artifact_location='abc'")
    with pytest.raises(
        MLForgeException, match=r"`dataset_name` in the `datasets` clause must be specified"
    ):
        store.search_logged_models(experiment_ids=[exp_id], datasets=[{}])
    with pytest.raises(
        MLForgeException, match=r"`field_name` in the `order_by` clause must be specified"
    ):
        store.search_logged_models(experiment_ids=[exp_id], order_by=[{}])
    with pytest.raises(MLForgeException, match=r"`order_by` must be a list of dictionaries."):
        store.search_logged_models(experiment_ids=[exp_id], order_by=["model_id"])
    with pytest.raises(MLForgeException, match=r"Invalid order by field name: artifact_location"):
        store.search_logged_models(
            experiment_ids=[exp_id], order_by=[{"field_name": "artifact_location"}]
        )
    with pytest.raises(MLForgeException, match=r"Invalid order by field name: params"):

...
from MLForge.server.fastapi_app import add_mcp_exception_handlers
from MLForge.server.mcp_server_api import get_mcp_server_api_route_prefixes, mcp_server_router
from MLForge.store.tracking.mcp_server_registry.abstract_mixin import (
    NOT_SET,
    MCPServerRegistryMixin,
)
from MLForge.store.tracking.mcp_server_registry.rest_mixin import RestMCPServerRegistryMixin
from MLForge.store.tracking.mcp_server_registry.sqlalchemy_mixin import (
    SqlAlchemyMCPServerRegistryMixin,
)
from MLForge.store.tracking.sqlalchemy_store import SqlAlchemyStore
from MLForge.store.workspace_rest_store_mixin import WorkspaceRestStoreMixin
from MLForge.tracking.client import MLForgeClient
...
class _TestRestClient(WorkspaceRestStoreMixin, RestMCPServerRegistryMixin):
    ...
    def get_host_creds(self):
        return MLForgeHostCreds(host="http://testserver")
...
def rest_client(store):
    ...
    with (
        ...
        mock.patch(
            "MLForge.store.tracking.mcp_server_registry.rest_mixin.http_request",
...
def test_rest_client_url_encodes_slashed_name():
    ...
    assert (
        ...
        == "/api/3.0/MLForge/mcp-servers/io.github.user%2Fmy-server/versions/2025.6.0"
    )
...
def test_rest_client_rejects_unsupported_active_workspace_before_request():
    ...
    with (
        ...
        with pytest.raises(MLForgeException, match="does not support workspaces") as exc_info:
...
def test_create_duplicate_server_preserves_error_code(rest_client):
    ...
    with pytest.raises(MLForgeException, match="already exists") as exc_info:
        rest_client.create_mcp_server("io.github.test/dup")
...
def test_client_create_mcp_server_version_via_rest_auto_discovers(rest_client):
    ...
    with (
        ...
        mock.patch(
            "MLForge.genai.mcp_tool_discovery.discover_mcp_tools",
            return_value=[MCPTool(name="from_rest_client")],
        ) as mock_discover,
    ):
        version = MLForgeClient().create_mcp_server_version(server_json=sj)
...
def test_create_version_missing_name_raises_MLForge_exception(rest_client):
    with pytest.raises(MLForgeException, match="must contain 'name' and 'version' keys") as exc_info:
...
def test_search_endpoints_rejects_empty_server_name(rest_client, empty_name):
    with pytest.raises(MLForgeException, match="server_name must be a non-empty string"):

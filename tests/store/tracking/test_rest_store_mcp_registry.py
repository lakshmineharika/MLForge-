...
@pytest.fixture
def rest_store():
    creds = MLForgeHostCreds(host="http://test-server")
...
def test_missing_required_field_raises_MLForge_exception(rest_store, mock_http_request):
    mock_http_request.return_value = _make_response(
        json_data={
            "status": "active",
        }
    ...
    with pytest.raises(MLForgeException, match="Failed to parse.*name"):
        rest_store.get_mcp_server("com.example/test-server")


def test_invalid_status_enum_raises_MLForge_exception(rest_store, mock_http_request):
    mock_http_request.return_value = _make_response(
        json_data={
            "name": "com.example/test-server",
    ...
    with pytest.raises(MLForgeException, match="Failed to parse.*status"):
        rest_store.get_mcp_server("com.example/test-server")


def test_invalid_transport_type_raises_MLForge_exception(rest_store, mock_http_request):
    mock_http_request.return_value = _make_response(
        json_data={
            "id": "ae-abc123",
            "server_name": "com.example/test-server",
    ...
    with pytest.raises(MLForgeException, match="Failed to parse.*transport"):
        rest_store.get_mcp_access_endpoint("com.example/test-server", "ae-abc123")


def test_non_dict_response_raises_MLForge_exception(rest_store, mock_http_request):
    mock_http_request.return_value = _make_response(json_data="not a dict")

    with pytest.raises(MLForgeException, match="Failed to parse"):
        rest_store.get_mcp_server("com.example/test-server")


def test_malformed_nested_tool_raises_MLForge_exception(rest_store, mock_http_request):
    mock_http_request.return_value = _make_response(
        json_data={
            "name": "com.example/test-server",
            "version": "1.0.0",
...
def test_null_where_object_expected_raises_MLForge_exception(rest_store, mock_http_request):
    mock_http_request.return_value = _make_response(
        json_data={
            "mcp_servers": None,
            "next_page_token": None,

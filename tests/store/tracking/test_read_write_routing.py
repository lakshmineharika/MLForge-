...
from MLForge.entities import Metric, ViewType
from MLForge.exceptions import MLForgeException
from MLForge.store.db.utils import _get_routing_session_maker
...
def test_write_goes_to_primary(store_with_replica):
    ...
    with store_with_replica.ManagedSessionMaker(read_only=False) as session:
        from MLForge.store.tracking.dbmodels.models import SqlExperiment
...
def test_run_server_passes_read_uri_env_var():
    from MLForge.server import _run_server
    from MLForge.server.constants import READ_REPLICA_BACKEND_STORE_URI_ENV_VAR

    with mock.patch("MLForge.server._exec_cmd") as mock_exec:
...
def test_initialize_backend_stores_sets_env_var():
    ...
    try:
        with (
            mock.patch("MLForge.server.handlers._get_tracking_store"),
            mock.patch("MLForge.server.handlers._get_model_registry_store"),
        ):
...
def test_get_sqlalchemy_store_reads_env_var(tmp_path, monkeypatch):
    from MLForge.server.constants import READ_REPLICA_BACKEND_STORE_URI_ENV_VAR
    from MLForge.server.handlers import TrackingStoreRegistryWrapper
...
# The before_flush listener (active when _MLForge_TESTING is set) enforces this at runtime.


def test_write_on_read_only_session_raises(store_with_replica, monkeypatch):
    monkeypatch.setenv("_MLForge_TESTING", "true")
    from MLForge.store.tracking.dbmodels.models import SqlExperiment
    ...
    with pytest.raises(MLForgeException, match="Write operation detected"):
        _do_write()

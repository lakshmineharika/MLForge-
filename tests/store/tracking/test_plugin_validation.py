...
def test_sqlalchemy_store_import_does_not_cause_circular_import():
    """
    Regression test for circular import issue (https://github.com/MLForge/MLForge/issues/18386).
    ...
    code = """
from MLForge.store.tracking.sqlalchemy_store import SqlAlchemyStore
from MLForge.entities import EvaluationDataset
"""
...
def test_plugin_entrypoint_registration_does_not_fail():
    """
    Regression test for plugin loading issue (https://github.com/MLForge/MLForge/issues/18386).

    When MLForge discovers and loads store plugins via entrypoints, it imports the plugin module
...
def test_plugin_can_create_dataset_without_name_error(tmp_path):
    """
    Regression test for plugin runtime usage (https://github.com/MLForge/MLForge/issues/18386).
...
def test_evaluation_dataset_not_in_entities_all():
    ...
    EvaluationDataset must be excluded from MLForge.entities.__all__ to prevent wildcard imports
    from triggering circular dependencies. When store plugins are loaded via entrypoints, any
    code that uses "from MLForge.entities import *" would pull in EvaluationDataset, which has
    ...
    """
    import MLForge.entities

    assert "EvaluationDataset" not in MLForge.entities.__all__

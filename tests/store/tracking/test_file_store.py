...
from MLForge.entities.assessment import ExpectationValue, FeedbackValue
from MLForge.entities.trace_status import TraceStatus
from MLForge.exceptions import MissingConfigException, MLForgeException
...
from MLForge.store.tracking import SEARCH_MAX_RESULTS_DEFAULT
from MLForge.store.tracking.file_store import FileStore, MissingConfigException
...
from MLForge.utils.MLForge_tags import (
    ...
    MLForge_LOGGED_MODELS,
    MLForge_RUN_NAME,
)
from MLForge.utils.name_utils import _EXPERIMENT_ID_FIXED_WIDTH, _GENERATOR_PREDICATES
...
from MLForge.utils.validation import MAX_EXPERIMENT_NAME_LENGTH
from MLForge.utils.yaml_utils import read_yaml, safe_edit_yaml, write_yaml

from tests.helper_functions import random_int, random_str

FILESTORE_PACKAGE = "MLForge.store.tracking.file_store"
...
def test_attempting_to_remove_default_experiment(store):
    ...
    with pytest.raises(MLForgeException, match="Cannot delete the default experiment"):
...
def test_search_experiments_filter_by_tag_is_null(store):
    ...
    with pytest.raises(MLForgeException, match="IS NULL / IS NOT NULL is only supported for tags"):
...
def test_search_experiments_order_by(store):
    ...
    # `None`. This is applicable to experiments created in older versions of MLForge where the
    # `creation_time` attribute did not exist
    with mock.patch(
        "MLForge.store.tracking.file_store.get_current_time_millis",
        return_value=None,
    ):
...
def test_record_logged_model(store):
    ...
    _verify_logged(
        ...
        params=[],
        metrics=[],
        tags=[RunTag(MLForge_LOGGED_MODELS, json.dumps([m.get_tags_dict()]))],
    )
    m2 = Model(
    ...
    with pytest.raises(
        TypeError,
        match="Argument 'MLForge_model' should be MLForge.models.Model, got '<class 'dict'>'",
    ):
...
def test_get_experiment_retries_for_transient_empty_yaml_read(store):
    ...
    with mock.patch(
        "MLForge.store.tracking.file_store.read_yaml", side_effect=mock_read_yaml_impl
    ) as mock_read_yaml:
...
def test_create_experiment(store):
    ...
    with pytest.raises(MLForgeException, match=r"'name' exceeds the maximum length"):
...
def test_hard_delete_run(store):
    ...
    with pytest.raises(MLForgeException, match=f"Run '{run_id}' not found"):
...
def test_create_run_returns_expected_run_data(store):
    ...
    assert tags_run.data.tags == {**tags_dict, MLForge_RUN_NAME: tags_run.info.run_name}
...
def test_create_run_sets_name(store):
    ...
    assert run.data.tags.get(MLForge_RUN_NAME) == "my name"
    ...
    run_id = store.create_run(
        ...
        start_time=0,
        run_name=None,
        tags=[RunTag(MLForge_RUN_NAME, "test")],
    ).info.run_id
    run = store.get_run(run_id)
    ...
    with pytest.raises(
        MLForgeException,
        match=re.escape(
            "Both 'run_name' argument and 'MLForge.runName' tag are specified, but with "
            "different values (run_name='my name', MLForge.runName='test')."
        ),
    ):
        store.create_run(
            experiment_id=FileStore.DEFAULT_EXPERIMENT_ID,
...
def test_get_metric_history_with_page_token(store):
    ...
    with pytest.raises(MLForgeException, match="Invalid page token"):
...
def test_search_with_max_results(store):
    ...
    with pytest.raises(
        MLForgeException, match="Invalid value for request parameter max_results. It "
    ):
...
def test_search_runs_run_name(store):
    ...
    result = store.search_runs(
        [exp_id],
        filter_string="tags.`MLForge.runName` = 'run_name2'",
        run_view_type=ViewType.ACTIVE_ONLY,
    )
    ...
    # Test run name filter works for runs logged in MLForge <= 1.29.0
    ...
    result = store.search_runs(
        [exp_id],
        filter_string="tags.`MLForge.runName` = 'new_run_name1'",
        run_view_type=ViewType.ACTIVE_ONLY,
    )
    assert [r.info.run_id for r in result] == [run1.info.run_id]
...
def test_search_runs_datasets(store):
    ...
    test_tag = [InputTag(key=MLForge_DATASET_CONTEXT, value="test")]
    train_tag = [InputTag(key=MLForge_DATASET_CONTEXT, value="train")]
    eval_tag = [InputTag(key=MLForge_DATASET_CONTEXT, value="eval")]
...
def test_log_param_enforces_value_immutability(store):
    ...
    store.log_param(run_id, Param(param_name, "value1"))
    with pytest.raises(
        MLForgeException, match="Changing param values is not allowed. Param with key="
    ) as e:
...
def test_log_param_max_length_value(store, monkeypatch):
    ...
    run = store.get_run(run_id)
    assert run.data.params[param_name] == param_value
    monkeypatch.setenv("MLForge_TRUNCATE_LONG_VALUES", "false")
    with pytest.raises(MLForgeException, match="exceeds the maximum length"):
        store.log_param(run_id, Param(param_name, "x" * 6001))

    monkeypatch.setenv("MLForge_TRUNCATE_LONG_VALUES", "true")
    store.log_param(run_id, Param(param_name, "x" * 6001))
...
def test_set_experiment_tags(store):
    ...
    store.delete_experiment(exp_id)
    with pytest.raises(MLForgeException, match="must be in the 'active' lifecycle_stage"):
...
def test_delete_tags(store):
    ...
    with pytest.raises(MLForgeException, match="must be in 'active' lifecycle_stage"):
        store.delete_tag(run_id, "tag0")
...
def test_malformed_metric(store):
    ...
    with (
        mock.patch("MLForge.store.tracking.file_store.read_file_lines", return_value=["0 1 0 2\n"]),
        pytest.raises(
            MLForgeException,
...
def test_mismatching_experiment_id(store):
    ...
    with pytest.raises(MLForgeException, match="Could not find experiment with ID"):
        store.get_experiment(FileStore.DEFAULT_EXPERIMENT_ID)

    with pytest.raises(MLForgeException, match="does not exist"):
...
def test_bad_experiment_id_recorded_for_run(store):
    ...
    with pytest.raises(MLForgeException, match="metadata is in invalid state"):
        store.get_run(bad_run_id)
...
def test_log_batch_internal_error(store):
    # Verify that internal errors during log_batch result in MLForgeExceptions
    run = store.create_run(
        experiment_id=FileStore.DEFAULT_EXPERIMENT_ID,
        user_id="user",
        start_time=0,
    ...
    with (
        ...
        for kwargs in [
            ...
            with pytest.raises(MLForgeException, match="Some internal error") as e:
...
def test_log_batch_nonexistent_run(store):
    ...
    with pytest.raises(MLForgeException, match=f"Run '{nonexistent_uuid}' not found") as e:
...
def test_log_batch_with_duplicate_params_errors_no_partial_write(store):
    ...
    with pytest.raises(MLForgeException, match="Duplicate parameter keys have been submitted") as e:
        store.log_batch(
            run.info.run_id,
            metrics=[],
...
def test_update_run_name(store):
    ...
    assert run.data.tags.get(MLForge_RUN_NAME) == "name"

    store.update_run_info(run_id, RunStatus.FINISHED, 100, "new name")
    run = store.get_run(run_id)
    assert run.info.run_name == "new name"
    assert run.data.tags.get(MLForge_RUN_NAME) == "new name"
    ...
    store.delete_tag(run_id, MLForge_RUN_NAME)
    run = store.get_run(run_id)
    assert run.info.run_name == "new name"
    assert run.data.tags.get(MLForge_RUN_NAME) is None

    store.update_run_info(run_id, RunStatus.FINISHED, 100, "another name")
    run = store.get_run(run_id)
    assert run.data.tags.get(MLForge_RUN_NAME) == "another name"
    assert run.info.run_name == "another name"

    store.set_tag(run_id, RunTag(MLForge_RUN_NAME, "yet another name"))
    run = store.get_run(run_id)
    assert run.info.run_name == "yet another name"
    assert run.data.tags.get(MLForge_RUN_NAME) == "yet another name"

    store.log_batch(run_id, metrics=[], params=[], tags=[RunTag(MLForge_RUN_NAME, "batch name")])
    run = store.get_run(run_id)
    assert run.info.run_name == "batch name"
    assert run.data.tags.get(MLForge_RUN_NAME) == "batch name"
...
def test_search_datasets_returns_no_more_than_max_results(store):
    ...
    for i in range(1010):
        ...
        input_tag = [InputTag(key=MLForge_DATASET_CONTEXT, value=str(i))]
...
def test_legacy_start_trace_v2(store):
    ...
    with pytest.raises(MLForgeException, match=r"Experiment fake_exp_id does not exist."):
...
def test_legacy_end_trace(store_and_trace_info):
    ...
    tags = {TraceTagKey.TRACE_NAME: "MLForge_trace"}
    ...
    with pytest.raises(MLForgeException, match=r"Trace with ID 'fake_request_id' not found"):
...
def test_start_trace(store):
    ...
    assert new_trace_info.tags["MLForge.artifactLocation"] is not None
...
def test_get_trace_info(store_and_trace_info):
    ...
    with (
        mock.patch(
            "MLForge.store.tracking.file_store.FileStore._get_trace_info_from_dir",
...
def test_set_trace_tag(store_and_trace_info):
    ...
    with pytest.raises(MLForgeException, match=r"Missing value for required parameter \'key\'"):
...
def test_delete_traces(store):
    ...
    with pytest.raises(MLForgeException, match=r"Experiment non_existing_exp does not exist."):
...
def test_search_traces_raise_errors(generate_trace_infos):
    ...
    with pytest.raises(
        MLForgeException,
        match=r"Invalid order_by entity `tag` with key `MLForge.traceName`",
    ):
...
def test_traces_not_listed_as_runs(tmp_path):
    with _use_tracking_uri(tmp_path.joinpath("mlruns").as_uri()):
        ...
        with mock.patch("MLForge.store.tracking.file_store.logging.debug") as mock_debug:
...
def test_get_assessment_errors(store_and_trace_info):
    ...
    with pytest.raises(MLForgeException, match=r"Trace with ID 'fake_trace' not found"):
...
def test_update_assessment_type_validation(store_and_trace_info):
    ...
    with pytest.raises(
        MLForgeException, match=r"Cannot update expectation value on a Feedback assessment"
    ):
    ...
    with pytest.raises(
        MLForgeException, match=r"Cannot update feedback value on an Expectation assessment"
    ):
        store.update_assessment(
            trace_id=trace_info.request_id,
...
def test_create_assessment_override_nonexistent(store_and_trace_info):
    ...
    with pytest.raises(
        MLForgeException, match=r"Assessment with ID 'nonexistent-assessment-id' not found"
    ):
...
def test_malicious_meta_yaml_in_artifact_folder_path_traversal(tmp_path):
    ...
    with pytest.raises(MLForgeException, match="Run 'artifacts' not found"):

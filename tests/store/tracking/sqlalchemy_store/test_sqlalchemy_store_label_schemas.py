...
from MLForge.genai.label_schemas.label_schemas import (
    InputCategorical,
    InputNumeric,
    InputPassFail,
    InputText,
...
from MLForge.genai.label_schemas.validation import (
    DEFAULT_LABEL_SCHEMA_INSTRUCTION,
    DEFAULT_LABEL_SCHEMA_NAME,
)
from MLForge.store.tracking.dbmodels.models import SqlLabelSchema
...
def test_create_rejects_duplicate_name(store):
    ...
    with pytest.raises(MLForgeException, match="already exists"):
        _create_pass_fail_schema(store, exp_id)


def test_create_rejects_missing_experiment(store):
    with pytest.raises(MLForgeException, match="Experiment with id"):
...
def test_get_by_name_rejects_non_integer_experiment(store):
    ...
    with pytest.raises(MLForgeException, match="must be a valid integer"):
...
def test_update_rejects_input_variant_change(store):
    ...
    schema = _create_pass_fail_schema(store, exp_id)
    with pytest.raises(MLForgeException, match="input type cannot be changed"):
...
def test_update_rejects_multi_select_change(store):
    ...
    with pytest.raises(MLForgeException, match="multi_select` cannot be changed"):
        store.update_label_schema(
            schema.schema_id,
...
def test_create_rejects_reserved_default_name(store):
    ...
    for name in (DEFAULT_LABEL_SCHEMA_NAME, DEFAULT_LABEL_SCHEMA_NAME.upper(), "  feedback  "):
        with pytest.raises(MLForgeException, match="reserved"):
            store.create_label_schema(
...
def test_default_question_cannot_be_edited(store):
    ...
    with pytest.raises(MLForgeException, match="default question cannot be edited"):
...
def test_default_question_cannot_be_deleted(store):
    ...
    default = next(s for s in store.list_label_schemas(exp_id) if s.is_default)
    with pytest.raises(MLForgeException, match="default question cannot be deleted"):

...
from MLForge.store.db.db_types import MSSQL, MYSQL
from MLForge.store.db.utils import (
    _get_latest_schema_revision,
    _get_schema_version,
)
from MLForge.store.tracking.dbmodels import models
from MLForge.store.tracking.sqlalchemy_store import (
    SqlAlchemyStore,
    _get_orderby_clauses,
)
from MLForge.store.tracking.sqlalchemy_workspace_store import WorkspaceAwareSqlAlchemyStore
from MLForge.utils.uri import extract_db_type_from_uri
...
def test_fail_on_unsupported_db_type(db_uri):
    with pytest.raises(MLForgeException, match=r"Invalid database engine"):
        extract_db_type_from_uri(db_uri)


def test_fail_on_multiple_drivers():
    with pytest.raises(MLForgeException, match=r"Invalid database URI"):
        extract_db_type_from_uri("mysql+pymsql+pyodbc://...")


def test_upgrade_cli_idempotence(store: SqlAlchemyStore):
    # Repeatedly run `MLForge db upgrade` against our database, verifying that the command
    ...
    for _ in range(3):
        invoke_cli_runner(MLForge.db.commands, ["upgrade", store.db_uri])
        assert _get_schema_version(engine) == _get_latest_schema_revision()
    engine.dispose()
...
def test_metrics_materialization_upgrade_succeeds_and_produces_expected_latest_metric_values(
    ...
    the MLForge Tracking SQLite database located at
    /MLForge/tests/resources/db/db_version_7ac759974ad8_with_metrics.sql. This database contains
    ...
    MLForge/tests/resources/db/db_version_7ac759974ad8_with_metrics_expected_values.json).
    This JSON dump can be replicated by installing MLForge version 1.2.0 and executing the
    ...
    invoke_cli_runner(MLForge.db.commands, ["upgrade", db_url])
...
def test_sqlalchemy_store_behaves_as_expected_with_inmemory_sqlite_db(
    ...
    monkeypatch.setenv("MLForge_SQLALCHEMYSTORE_POOLCLASS", "SingletonThreadPool")
    _clear_in_memory_engine()
...
def test_sqlalchemy_store_does_not_create_artifact_root_directory_on_init(tmp_path, db_uri):
    ...
    MLForge servers to run in read-only environments (e.g., K8s containers) when artifacts are
    stored remotely and the local artifact root is never actually used.

    See: https://github.com/MLForge/MLForge/issues/19658
    """

...
from MLForge.protos.databricks_pb2 import INTERNAL_ERROR, RESOURCE_DOES_NOT_EXIST, ErrorCode
...
def test_get_assessment_errors(store_and_trace_info):
    ...
    with pytest.raises(MLForgeException, match=r"Trace with (ID|request_id) 'fake_trace' not found"):
...
def test_start_trace_with_assessments_missing_trace_id(store):
    ...
    During normal trace export (MLForgeV3SpanExporter), two things happen:
...
def test_create_assessment_constraint_violation_is_not_reported_as_missing_trace(
    ...
    with pytest.raises(MLForgeException, match="constraint violation") as exc:

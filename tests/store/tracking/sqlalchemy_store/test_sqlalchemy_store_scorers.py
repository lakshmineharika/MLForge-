...
from MLForge.entities.assessment import FeedbackValue
from MLForge.entities.gateway_endpoint import GatewayEndpoint
...
from MLForge.store.tracking.dbmodels.models import SqlOnlineScoringConfig
...
async def test_log_spans_default_trace_status_in_progress(store: SqlAlchemyStore, is_async: bool):
    ...
    # Generate a proper MLForge trace ID in the format "tr-<32-char-hex>"
    ...
    child_otel_span = OTelReadableSpan(
        ...
        attributes={
            ...
            "MLForge.spanType": json.dumps("LLM", cls=TraceJSONEncoder),
        },
    ...
    child_span = create_MLForge_span(child_otel_span, trace_id, "LLM")
...
async def test_log_spans_sets_trace_status_from_root_span(
    ...
    root_span = create_MLForge_span(root_otel_span, trace_id, "LLM")
...
async def test_log_spans_unset_root_span_status_defaults_to_ok(
    ...
    root_span = create_MLForge_span(root_unset_span, trace_id, "LLM")
...
def test_scorer_operations(store: SqlAlchemyStore):
    ...
    with pytest.raises(MLForgeException, match="Scorer with name 'non_existent' not found"):
        store.get_scorer(experiment_id, "non_existent")

    # Try to get non-existent version
    with pytest.raises(
        MLForgeException, match="Scorer with name 'accuracy_scorer' and version 999 not found"
    ):
    ...
    with pytest.raises(
        MLForgeException, match="Scorer with name 'accuracy_scorer' and version 1 not found"
    ):
    ...
    with pytest.raises(MLForgeException, match="Scorer with name 'accuracy_scorer' not found"):
    ...
    with pytest.raises(
        MLForgeException, match="Scorer with name 'safety_scorer' and version 999 not found"
    ):
    ...
    with pytest.raises(MLForgeException, match="Scorer with name 'non_existent_scorer' not found"):
...
def test_upsert_online_scoring_config_rejects_non_gateway_model(store: SqlAlchemyStore):
    ...
    with pytest.raises(MLForgeException, match="does not use a gateway model"):
        store.upsert_online_scoring_config(
            experiment_id=experiment_id,
            scorer_name="scorer",
            sample_rate=0.1,
...
def test_upsert_online_scoring_config_rejects_scorer_requiring_expectations(
    ...
    with mock.patch("MLForge.genai.judges.utils._is_litellm_available", return_value=True):
        with pytest.raises(MLForgeException, match="requires expectations.*not currently supported"):
            store.upsert_online_scoring_config(
                experiment_id=experiment_id,
...
def test_upsert_online_scoring_config_validates_filter_string(store: SqlAlchemyStore):
    ...
    with pytest.raises(MLForgeException, match="Invalid clause"):
        store.upsert_online_scoring_config(
            experiment_id=experiment_id,
            scorer_name="test_scorer",
            sample_rate=0.5,
...
def test_upsert_online_scoring_config_validates_sample_rate(store: SqlAlchemyStore):
    ...
    with pytest.raises(MLForgeException, match="sample_rate must be between 0.0 and 1.0"):
        store.upsert_online_scoring_config(
            experiment_id=experiment_id,
            scorer_name="test_scorer",
            sample_rate=-0.1,
    ...
    with pytest.raises(MLForgeException, match="sample_rate must be a number"):
        store.upsert_online_scoring_config(
            experiment_id=experiment_id,
            scorer_name="test_scorer",
            sample_rate="0.5",
    ...
    with pytest.raises(MLForgeException, match="filter_string must be a string"):
        store.upsert_online_scoring_config(
            experiment_id=experiment_id,
            scorer_name="test_scorer",
            sample_rate=0.5,
...
def test_dataset_experiment_associations(store):
    with mock.patch("MLForge.tracking._tracking_service.utils._get_store", return_value=store):
        ...
        with mock.patch("MLForge.store.tracking.sqlalchemy_store._logger.warning") as mock_warning:
        ...
        with pytest.raises(MLForgeException, match=r"No Experiment with id="):

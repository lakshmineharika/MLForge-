...
from MLForge.store.tracking.dbmodels.models import SqlEvaluationDatasetRecord
...
def test_dataset_crud_operations(store):
    with mock.patch("MLForge.tracking._tracking_service.utils._get_store", return_value=store):
        ...
        created_dataset = store.create_dataset(
            ...
            tags={
                "purpose": "testing",
                "environment": "test",
                MLForge_tags.MLForge_USER: "test_user",
            },
        ...
        assert created_dataset.created_by == "test_user"  # Extracted from MLForge.user tag
        ...
        with pytest.raises(
            MLForgeException, match="Evaluation dataset with id 'd-nonexistent' not found"
        ):
...
def test_dataset_search_comprehensive(store):
    ...
    for i in range(10):
        ...
        tags = {
            ...
            MLForge_tags.MLForge_USER: f"user_{i % 3}",
        }
    ...
    created_user = store.create_dataset(
        ...
        tags={"test": "user", MLForge_tags.MLForge_USER: "test_user_1"},
        experiment_ids=[exp_ids[0]],
    )
    ...
    records_with_user = [
        {
            ...
            "tags": {MLForge_tags.MLForge_USER: "test_user_2"},
        }
    ]
    ...
    with pytest.raises(MLForgeException, match="Invalid attribute key"):
    ...
    records = [
        {
            ...
            "expectations": {
                ...
                "response": "MLForge is an open source platform",
            },
...
def test_dataset_user_detection(store):
    ...
    dataset1 = store.create_dataset(
        ...
        tags={MLForge_tags.MLForge_USER: "john_doe", "other": "tag"},
        experiment_ids=exp_ids,
    )
    assert dataset1.created_by == "john_doe"
    assert dataset1.tags[MLForge_tags.MLForge_USER] == "john_doe"
    ...
    assert MLForge_tags.MLForge_USER not in dataset2.tags
...
def test_dataset_upsert_comprehensive(store):
    ...
    records_batch1 = [
        {
            ...
            "expectations": {"answer": "MLForge is a platform", "score": 0.8},
        ...
        {
            "inputs": {"question": "What is MLForge?"},
            "expectations": {"answer": "MLForge is an ML platform", "confidence": 0.9},
    ...
    MLForge_record = next(r for r in loaded_records if r.inputs["question"] == "What is MLForge?")
    assert MLForge_record.expectations == {
        "answer": "MLForge is an ML platform",
        "score": 0.8,
        "confidence": 0.9,
    }
    assert MLForge_record.tags == {"version": "v2", "quality": "high", "reviewed": "true"}

    assert MLForge_record.source.source_type == "TRACE"
    assert MLForge_record.source.source_data["trace_id"] == "trace-001"
    assert MLForge_record.source_id == "trace-001"

    initial_update_time = MLForge_record.last_update_time
    time.sleep(0.01)

    records_batch2 = [
        {
            "inputs": {"question": "What is MLForge?"},
            "expectations": {"answer": "MLForge is the best ML platform", "rating": 5},
            "tags": {"version": "v3"},
        },
        {
    ...
    updated_MLForge_record = next(
        r for r in loaded_records if r.inputs["question"] == "What is MLForge?"
    )
    assert updated_MLForge_record.expectations == {
        "answer": "MLForge is the best ML platform",
        "score": 0.8,
        "confidence": 0.9,
        "rating": 5,
    }
    assert updated_MLForge_record.tags == {
        "version": "v3",
        "quality": "high",
        "reviewed": "true",
    }
    assert updated_MLForge_record.last_update_time > initial_update_time
    assert updated_MLForge_record.source.source_data["trace_id"] == "trace-001"
...
def test_dataset_delete_records(store):
    ...
    records = [
        {
            "inputs": {"id": 1, "question": "What is MLForge?"},
...
def test_dataset_update_tags(store):
    ...
    store.set_dataset_tags(
        ...
        {"new_tag": "value", MLForge_tags.MLForge_USER: "test_user2"},
    )

    updated_no_tags = store.get_dataset(created_no_tags.dataset_id)
    assert updated_no_tags.tags == {"new_tag": "value", MLForge_tags.MLForge_USER: "test_user2"}
...
def test_sql_dataset_record_merge():
    with mock.patch("MLForge.store.tracking.dbmodels.models.get_current_time_millis") as mock_time:
        ...
        assert record.last_updated_by == "user1"  # No MLForge.user in tags
        ...
        new_data3 = {"tags": {MLForge_tags.MLForge_USER: "user2", "env": "prod"}}

        record3.merge(new_data3)

        assert record3.created_by == "user1"  # Preserved
        assert record3.last_updated_by == "user2"  # Updated from MLForge.user tag
...
def test_sql_dataset_record_wrapping_unwrapping():
    from MLForge.entities.dataset_record import DATASET_RECORD_WRAPPED_OUTPUT_KEY
    ...
    sql_record = SqlEvaluationDatasetRecord.from_MLForge_entity(entity, "input_hash_123")

    assert sql_record.outputs == {DATASET_RECORD_WRAPPED_OUTPUT_KEY: "string output"}

    unwrapped_entity = sql_record.to_MLForge_entity()
    ...
    sql_record2 = SqlEvaluationDatasetRecord.from_MLForge_entity(entity2, "input_hash_456")
    assert sql_record2.outputs == {DATASET_RECORD_WRAPPED_OUTPUT_KEY: [1, 2, 3]}

    unwrapped_entity2 = sql_record2.to_MLForge_entity()
    ...
    sql_record3 = SqlEvaluationDatasetRecord.from_MLForge_entity(entity3, "input_hash_789")
    assert sql_record3.outputs == {DATASET_RECORD_WRAPPED_OUTPUT_KEY: 42}

    unwrapped_entity3 = sql_record3.to_MLForge_entity()
    ...
    sql_record4 = SqlEvaluationDatasetRecord.from_MLForge_entity(entity4, "input_hash_abc")
    assert sql_record4.outputs == {DATASET_RECORD_WRAPPED_OUTPUT_KEY: {"result": "answer"}}

    unwrapped_entity4 = sql_record4.to_MLForge_entity()
    ...
    sql_record5 = SqlEvaluationDatasetRecord.from_MLForge_entity(entity5, "input_hash_def")
    assert sql_record5.outputs is None

    unwrapped_entity5 = sql_record5.to_MLForge_entity()

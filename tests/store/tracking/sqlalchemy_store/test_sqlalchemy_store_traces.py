...
import MLForge
import MLForge.store.tracking.sqlalchemy_store as sqlalchemy_store_module
import MLForge.tracing.trace_archival_service as trace_archival_service_module
...
from MLForge.entities.model_registry import PromptVersion
from MLForge.entities.span import Span, create_MLForge_span
...
from MLForge.entities.workspace import TraceArchivalConfig, Workspace
...
from MLForge.store.db.db_types import MSSQL, MYSQL, POSTGRES
from MLForge.store.tracking.dbmodels.models import (
    SqlSpan,
    SqlSpanMetrics,
    SqlTraceInfo,
    SqlTraceMetadata,
...
from MLForge.store.tracking.sqlalchemy_store import SqlAlchemyStore, _TraceArchiveCandidate
from MLForge.store.workspace.abstract_store import ResolvedTraceArchivalConfig
...
from MLForge.utils.file_utils import TempDir, local_file_uri_to_path
from MLForge.utils.MLForge_tags import MLForge_ARTIFACT_LOCATION
from MLForge.utils.time import get_current_time_millis
from MLForge.utils.uri import append_to_uri_path
from MLForge.utils.workspace_context import WorkspaceContext
from MLForge.utils.workspace_utils import DEFAULT_WORKSPACE_NAME, WORKSPACES_DIR_NAME
...
def test_legacy_start_and_end_trace_v2(store: SqlAlchemyStore):
    ...
    artifact_location = trace_info.tags[MLForge_ARTIFACT_LOCATION]
    assert artifact_location.endswith(f"/{experiment_id}/traces/{request_id}/artifacts")
    assert trace_info.tags == {
        "tag1": "apple",
        "tag2": "orange",
        MLForge_ARTIFACT_LOCATION: artifact_location,
    }
...
def store_with_traces(store):
    ...
    _create_trace(
        ...
        tags={"MLForge.traceName": "aaa", "fruit": "apple", "color": "red"},
    ...
    _create_trace(
        ...
        tags={"MLForge.traceName": "bbb", "fruit": "apple", "color": "green"},
    )
    _create_trace(
        store,
        "tr-3",
        ...
        state=TraceState.OK,
        tags={"MLForge.traceName": "ccc", "fruit": "orange"},
    )
    _create_trace(
        store,
        "tr-4",
        ...
        execution_duration=10,
        state=TraceState.OK,
        tags={"MLForge.traceName": "ddd", "color": "blue"},
    )
...
def test_search_traces_with_invalid_filter(store_with_traces, filter_string, error):
    ...
    with pytest.raises(MLForgeException, match=error):
...
def test_search_traces_raise_if_max_results_arg_is_invalid(store):
    ...
    with pytest.raises(
        MLForgeException, match="Invalid value -1 for parameter 'max_results' supplied."
    ):
...
def test_search_traces_with_run_id_filter(store: SqlAlchemyStore):
    ...
    trace1_id = "tr-direct"
    _create_trace(store, trace1_id, exp_id, trace_metadata={"MLForge.sourceRun": run_id})
    ...
    trace3_id = "tr-both"
    _create_trace(store, trace3_id, exp_id, trace_metadata={"MLForge.sourceRun": run_id})
...
def test_search_traces_with_full_text_filter(store: SqlAlchemyStore):
    ...
    span1 = create_test_span(
        ...
        span_id=111,
        span_type="FUNCTION",
        attributes={"llm.inputs": "what's MLForge?"},
    )
    ...
    span3 = create_test_span(
        ...
        span_id=333,
        span_type="FUNCTION",
        attributes={"llm.outputs": 'MLForge is a tool for " testing " ...'},
    )
    ...
    traces, _ = store.search_traces([exp_id], filter_string='trace.text LIKE "%what\'s MLForge?%"')
    assert len(traces) == 1
    assert traces[0].trace_id == trace1_id

    traces, _ = store.search_traces(
        [exp_id], filter_string='trace.text LIKE "%MLForge is a tool for%"'
    )
    assert len(traces) == 1
...
def create_test_span_with_content(
    ...
    attributes = {
        "MLForge.traceRequestId": json.dumps(trace_id),
        "MLForge.spanType": json.dumps(span_type, cls=TraceJSONEncoder),
    }
    ...
    if inputs:
        attributes["MLForge.spanInputs"] = json.dumps(inputs, cls=TraceJSONEncoder)
    if outputs:
        attributes["MLForge.spanOutputs"] = json.dumps(outputs, cls=TraceJSONEncoder)
    ...
    return create_MLForge_span(otel_span, trace_id, span_type)
...
def test_search_traces_with_assessment_numeric_filters(store: SqlAlchemyStore):
    ...
    with pytest.raises(MLForgeException, match="Expected a numeric value for feedback"):
        store.search_traces([exp_id], filter_string='feedback.score > "high"')

    with pytest.raises(MLForgeException, match="Expected a quoted string value for feedback"):
...
def test_search_traces_with_run_id(store: SqlAlchemyStore):
    ...
    _create_trace(store, trace1_id, exp_id, trace_metadata={"MLForge.sourceRun": run1_id})
    _create_trace(store, trace2_id, exp_id, trace_metadata={"MLForge.sourceRun": run2_id})
    _create_trace(store, trace3_id, exp_id)
...
def test_search_traces_with_feedback_rlike_filters(store: SqlAlchemyStore):
    ...
    # Create feedback with string values that can be pattern matched
    from MLForge.entities.assessment import AssessmentSource, Feedback
...
def test_search_traces_with_span_attributute_backticks(store: SqlAlchemyStore):
    ...
    span1 = create_MLForge_span(
        OTelReadableSpan(
            ...
            parent=None,
            attributes={
                "MLForge.traceRequestId": json.dumps(trace_info_1.trace_id, cls=TraceJSONEncoder),
                "MLForge.experimentId": json.dumps(exp_id, cls=TraceJSONEncoder),
                "MLForge.spanInputs": json.dumps({"input": "test1"}, cls=TraceJSONEncoder),
            },
    ...
    span2 = create_MLForge_span(
        OTelReadableSpan(
            ...
            parent=None,
            attributes={
                "MLForge.traceRequestId": json.dumps(trace_info_2.trace_id, cls=TraceJSONEncoder),
                "MLForge.experimentId": json.dumps(exp_id, cls=TraceJSONEncoder),
                "MLForge.spanInputs": json.dumps({"input": "test2"}, cls=TraceJSONEncoder),
            },
            start_time=1000000000,
            end_time=2000000000,
            resource=_OTelResource.get_empty(),
...
def test_set_and_delete_tags(store: SqlAlchemyStore):
    ...
    store.delete_trace_tag(trace_id, MLForge_ARTIFACT_LOCATION)
    ...
    with pytest.raises(MLForgeException, match="No trace tag with key 'tag1'"):
        store.delete_trace_tag(trace_id, "tag1")
...
def test_set_invalid_tag(key, value, expected_error, store: SqlAlchemyStore):
    with pytest.raises(MLForgeException, match=expected_error):
...
def test_delete_traces_raises_error(store):
    ...
    with pytest.raises(
        MLForgeException, match=r"`max_traces` must be a positive integer, received 0"
    ):
...
async def test_log_spans(store: SqlAlchemyStore, is_async: bool):
    ...
    readable_span = OTelReadableSpan(
        ...
        attributes={
            "MLForge.traceRequestId": json.dumps(trace_info.trace_id),
            "MLForge.spanInputs": json.dumps({"input": "test_input"}, cls=TraceJSONEncoder),
            "MLForge.spanOutputs": json.dumps({"output": "test_output"}, cls=TraceJSONEncoder),
            "MLForge.spanType": json.dumps("LLM" if not is_async else "CHAIN", cls=TraceJSONEncoder),
    ...
    # Create MLForge span from OpenTelemetry span
    span = create_MLForge_span(readable_span, trace_info.trace_id, "LLM")
    ...
    with store.ManagedSessionMaker() as session:
        ...
        assert content_dict["attributes"]["MLForge.spanInputs"] == json.dumps(
            {"input": "test_input"}, cls=TraceJSONEncoder
        )
        assert content_dict["attributes"]["MLForge.spanOutputs"] == json.dumps(
            {"output": "test_output"}, cls=TraceJSONEncoder
        )
        expected_type = "LLM" if not is_async else "CHAIN"
        assert content_dict["attributes"]["MLForge.spanType"] == json.dumps(
...
def test_log_spans_multiple_traces(store: SqlAlchemyStore):
    ...
    span1 = create_MLForge_span(
        OTelReadableSpan(
            ...
            parent=None,
            attributes={"MLForge.traceRequestId": json.dumps("tr-multi-1", cls=TraceJSONEncoder)},
            start_time=1000000000,
            end_time=2000000000,
            resource=_OTelResource.get_empty(),
        ),
    ...
    span2 = create_MLForge_span(
        OTelReadableSpan(
            ...
            parent=None,
            attributes={"MLForge.traceRequestId": json.dumps("tr-multi-2", cls=TraceJSONEncoder)},
...
def test_log_spans_persists_resource_attributes_as_tags(store: SqlAlchemyStore):
    ...
    resource = _OTelResource({
        ...
        # MLForge.* namespace should be filtered out to prevent clobbering reserved tags
        "MLForge.evil": "should-be-dropped",
    ...
    span = create_MLForge_span(
        OTelReadableSpan(
            ...
            parent=None,
            attributes={
                "MLForge.traceRequestId": json.dumps("tr-resource-test"),
            },
            start_time=1000000000,
            end_time=2000000000,
            resource=resource,
    ...
    # MLForge.* namespace should be filtered out
    assert "MLForge.evil" not in tags
...
def test_log_spans_resource_tags_do_not_overwrite_user_tags(store: SqlAlchemyStore):
    ...
    span = create_MLForge_span(
        OTelReadableSpan(
            ...
            parent=None,
            attributes={
                "MLForge.traceRequestId": json.dumps("tr-tag-priority"),
                # User-defined trace tag via MLForge.traceTag.* attribute
...
async def test_log_spans_creates_trace_if_not_exists(store: SqlAlchemyStore, is_async: bool):
    ...
    readable_span = OTelReadableSpan(
        ...
        attributes={
            ...
            "MLForge.experimentId": json.dumps(experiment_id, cls=TraceJSONEncoder),
        },
    ...
    span = create_MLForge_span(readable_span, trace_id)
...
async def test_log_spans_updates_trace_time_range(store: SqlAlchemyStore, is_async: bool):
    ...
    span1 = create_MLForge_span(
        OTelReadableSpan(
            ...
            parent=None,
            attributes={"MLForge.traceRequestId": json.dumps(trace_id, cls=TraceJSONEncoder)},
...
def test_batch_get_traces_with_complex_attributes(store: SqlAlchemyStore) -> None:
    ...
    span = create_MLForge_span(otel_span, trace_id, "LLM")
...
def test_batch_get_traces_raises_for_artifact_repo_traces(store: SqlAlchemyStore) -> None:
    ...
    with pytest.raises(MLForgeTracingException, match="not stored in tracking store"):
...
def test_log_spans_update_token_usage_incrementally(store: SqlAlchemyStore) -> None:
    ...
    span1 = create_MLForge_span(otel_span1, trace_id, "LLM")
    store.log_spans(experiment_id, [span1])
    ...
    span2 = create_MLForge_span(otel_span2, trace_id, "LLM")
    store.log_spans(experiment_id, [span2])
...
def test_batch_get_traces_token_usage(store: SqlAlchemyStore) -> None:
    ...
    otel_span1._attributes = {
        "MLForge.traceRequestId": json.dumps(trace_id_1, cls=TraceJSONEncoder),
        SpanAttributeKey.CHAT_USAGE: json.dumps({
            "input_tokens": 100,
            "output_tokens": 50,
            "total_tokens": 150,
    ...
    span1 = create_MLForge_span(otel_span1, trace_id_1, "LLM")
    store.log_spans(experiment_id, [span1])
    ...
    otel_span2._attributes = {
        "MLForge.traceRequestId": json.dumps(trace_id_2, cls=TraceJSONEncoder),
        SpanAttributeKey.CHAT_USAGE: json.dumps({
            "input_tokens": 200,
            "output_tokens": 100,
            "total_tokens": 300,
    ...
    span2 = create_MLForge_span(otel_span2, trace_id_2, "LLM")
    store.log_spans(experiment_id, [span2])
    ...
    otel_span3._attributes = {
        "MLForge.traceRequestId": json.dumps(trace_id_3, cls=TraceJSONEncoder),
    }
    span3 = create_MLForge_span(otel_span3, trace_id_3, "UNKNOWN")
    store.log_spans(experiment_id, [span3])
...
def test_get_trace_returns_lazy_spans_that_skip_materialization_on_to_dict(
    ...
    from MLForge.entities.span import LazySpan
...
def test_get_trace_not_found(store: SqlAlchemyStore) -> None:
    trace_id = f"tr-{uuid.uuid4().hex}"
    with pytest.raises(MLForgeException, match=f"Trace with ID {trace_id} is not found."):
...
@pytest.mark.skipif(
    MLForge.get_tracking_uri().startswith("mysql"),
...
def test_log_spans_session_id_handling(store: SqlAlchemyStore) -> None:
    ...
    otel_span1._attributes = {
        "MLForge.traceRequestId": json.dumps(trace_id1, cls=TraceJSONEncoder),
        "session.id": json.dumps("session-123"),
    }
    span1 = create_MLForge_span(otel_span1, trace_id1, "LLM")
    store.log_spans(experiment_id, [span1])
    ...
    otel_span1_gen_ai._attributes = {
        "MLForge.traceRequestId": json.dumps(trace_id1_gen_ai, cls=TraceJSONEncoder),
        "gen_ai.conversation.id": "conv-123",
    }
    span1_gen_ai = create_MLForge_span(otel_span1_gen_ai, trace_id1_gen_ai, "LLM")
    ...
    otel_span1_precedence._attributes = {
        "MLForge.traceRequestId": json.dumps(trace_id1_precedence, cls=TraceJSONEncoder),
        "session.id": "session-456",
        "gen_ai.conversation.id": "conv-456",
    }
    span1_precedence = create_MLForge_span(otel_span1_precedence, trace_id1_precedence, "LLM")
    ...
    otel_span2._attributes = {
        "MLForge.traceRequestId": json.dumps(trace_id2, cls=TraceJSONEncoder),
        "session.id": json.dumps("different-session"),
    }
    span2 = create_MLForge_span(otel_span2, trace_id2, "LLM")
    store.log_spans(experiment_id, [span2])
    ...
    span3 = create_MLForge_span(otel_span3, trace_id3, "LLM")
    store.log_spans(experiment_id, [span3])
...
def test_log_spans_user_id_handling(store: SqlAlchemyStore) -> None:
    ...
    otel_span4._attributes = {
        "MLForge.traceRequestId": json.dumps(trace_id4, cls=TraceJSONEncoder),
        "session.id": json.dumps("session-456"),
        "user.id": json.dumps("bob"),
    }
    span4 = create_MLForge_span(otel_span4, trace_id4, "LLM")
...
def test_log_spans_rejects_archived_trace_in_multi_trace_batch(store: SqlAlchemyStore):
    ...
    with pytest.raises(
        MLForgeException, match=f"Cannot log spans to archived traces: '{archived_trace_id}'"
    ) as exc_info:
        store.log_spans(
            experiment_id,
            [
...
def test_archive_traces_archives_db_backed_trace_payloads(
    ...
    original_trace_artifact_uri = store.get_trace_info(old_trace_id).tags[MLForge_ARTIFACT_LOCATION]
    ...
    with TempDir() as tmp:
        ...
        assert trace_info.tags[MLForge_ARTIFACT_LOCATION] == original_trace_artifact_uri
...
def test_archived_trace_with_spanless_payload_raises_corruption(
    ...
    with TempDir() as tmp:
        ...
        from MLForge.tracing.otel.otel_archival import TRACE_ARCHIVAL_FILENAME
        ...
        with pytest.raises(MLForgeTraceDataCorrupted, match="Trace data is corrupted"):
            store.get_trace(trace_id)
        with pytest.raises(MLForgeTraceDataCorrupted, match="Trace data is corrupted"):
...
def test_archive_traces_preserves_trace_attachment_location(store: SqlAlchemyStore):
    ...
    get_artifact_repository(trace_info.tags[MLForge_ARTIFACT_LOCATION]).upload_attachment(
    ...
    assert archived == 1
    trace_info = store.get_trace_info(trace_id)
    assert trace_info.tags[MLForge_ARTIFACT_LOCATION].endswith(
        f"/{exp_id}/traces/{trace_id}/artifacts"
    )
    assert TraceTagKey.ARCHIVE_LOCATION in trace_info.tags
    assert (
        get_artifact_repository(
            trace_info.tags[MLForge_ARTIFACT_LOCATION]
...
def test_archive_traces_rejects_proxy_only_default_root(store: SqlAlchemyStore):
    with pytest.raises(MLForgeException, match="proxy-only `MLForge-artifacts:` scheme") as exc_info:
        _archive_traces(
            store=store,
            default_trace_archival_location="MLForge-artifacts:/archive/default",
            default_retention="1d",
        )
...
def test_archive_traces_rejects_unregistered_archive_scheme_before_processing_candidates(
    ...
    with pytest.raises(MLForgeException, match="Could not find a registered artifact repository"):
        _archive_traces(
            store=store,
...
def test_archive_traces_raises_when_default_retention_exceeds_max_length(
    ...
    with pytest.raises(MLForgeException, match="at most 32 characters") as exc_info:
        _archive_traces(
            store=store,
            default_trace_archival_location="s3://archive/default",
...
def test_log_spans_rejects_archived_trace(store: SqlAlchemyStore):
    ...
    with TempDir() as tmp:
        ...
        with pytest.raises(
            MLForgeException, match=f"Cannot log spans to archived traces: '{trace_id}'"
        ) as exc_info:
            store.log_spans(
                exp_id,
                [
...
def test_archive_traces_marks_serializer_failures_as_malformed_and_excludes_retries(
    ...
    with TempDir() as tmp:
        ...
        archive_root.mkdir()
        with mock.patch(
            "MLForge.store.tracking.sqlalchemy_store.spans_to_traces_data_pb",
            side_effect=MLForgeException.invalid_parameter_value("simulated malformed trace"),
        ):
            archived = _archive_traces(
                store,
                default_trace_archival_location=archive_root.as_uri(),
...
@pytest.mark.parametrize(
    "error",
    [
        MLForgeException("simulated retryable archival failure"),
        OSError("simulated storage error"),
    ],
    ids=["MLForge-error", "os-error"],
)
def test_archive_traces_leaves_retryable_errors_retryable(store: SqlAlchemyStore, error: Exception):
    exp_id = store.create_experiment("archive-retryable-MLForge-error")
    trace_id = "tr-retryable-MLForge-error"
...
def test_log_spans_then_start_trace_rejects_archived_trace(store: SqlAlchemyStore):
    ...
    with pytest.raises(
        MLForgeException, match=f"Cannot update traces that are no longer DB-backed: '{trace_id}'"
    ) as exc_info:
        store.start_trace(trace_info_for_start)
...
def test_archive_traces_keeps_unsupported_archive_repository_retryable(
    ...
    def upload_bytes_unsupported(self, data):
        if fail_trace_id in self.artifact_uri:
            raise MLForgeNotImplementedException(
...
def _set_trace_archival_config(
    ...
    *,
    ...
    monkeypatch.setenv(MLForge_TRACE_ARCHIVAL_CONFIG.name, str(config_path))
...
def test_create_experiment_rejects_invalid_trace_archival_retention_tag(store: SqlAlchemyStore):
    with pytest.raises(MLForgeException, match="type 'duration'"):
...
def test_set_experiment_tag_rejects_invalid_trace_archive_now_tag(store: SqlAlchemyStore):
    ...
    with pytest.raises(MLForgeException, match="Trace archival retention must be in the form"):
        store.set_experiment_tag(
            exp_id,
            ExperimentTag(
...
def test_archived_trace_reads_recover_when_metadata_still_points_to_tracking_store(
    ...
    with TempDir() as tmp:
        ...
        original_to_MLForge_entity = SqlTraceInfo.to_MLForge_entity

        def stale_first_trace_read(self):
            trace_info = original_to_MLForge_entity(self)
        ...
        stale_first_trace_read.remaining = 1
        with mock.patch.object(SqlTraceInfo, "to_MLForge_entity", new=stale_first_trace_read):
            trace = store.get_trace(trace_id)
...
def test_delete_traces_removes_db_backed_rows_before_archived_payload_cleanup(
    ...
    with TempDir() as tmp:
        ...
        with mock.patch(
            "MLForge.store.tracking.sqlalchemy_store.get_artifact_repository",
...
def test_delete_traces_treats_missing_archived_payload_as_already_deleted(store: SqlAlchemyStore):
    ...
    with TempDir() as tmp:
        ...
        missing_artifact_repo.delete_artifacts.side_effect = MLForgeException(
...
def test_archive_traces_rejects_unsupported_default_root_repository(store: SqlAlchemyStore):
    ...
    with mock.patch(
        "MLForge.store.artifact.artifact_repository_registry.get_artifact_repository",
...
def test_start_trace_writes_metadata_in_sorted_key_order(store: SqlAlchemyStore):
    ...
    trace_metadata = {
        "MLForge.trace.tokenUsage": json.dumps({"input_tokens": 10}),
        "MLForge.traceInputs": "in",
        "MLForge.trace.cost": json.dumps({"total": 0.1}),
        "MLForge.traceOutputs": "out",
    }
...
def test_log_spans_writes_metadata_in_sorted_key_order(store: SqlAlchemyStore):
    ...
    def _usage_span(trace_id, span_id_num, session_id):
        ...
        return create_MLForge_span(otel_span, trace_id, "LLM")
...
def test_start_trace_conflict_path_merges_metadata_and_metrics_in_sorted_key_order(
    ...
    store.log_spans(experiment_id, [create_MLForge_span(otel_span, trace_id, "LLM")])
    ...
    trace_metadata = {
        "MLForge.traceOutputs": "out",
        "MLForge.trace.tokenUsage": json.dumps({
            "input_tokens": 200,
            "output_tokens": 70,
            "total_tokens": 270,
        }),
...
def _make_deadlock_exception() -> MLForgeException:
    # Mirrors exactly what MLForge.store.db.utils re-raises a psycopg2 DeadlockDetected /
    # sqlalchemy OperationalError as when a transaction is killed by the DB:
    # MLForgeException(message=..., error_code=TEMPORARILY_UNAVAILABLE) (int error code).
    return MLForgeException(
...
def test_start_trace_retries_on_deadlock(store: SqlAlchemyStore):
    ...
    session (via ``Session.flush``) rather than a pre-built ``MLForgeException``, so the
    ...
    with (
        ...
        mock.patch("MLForge.store.tracking.sqlalchemy_store.time.sleep"),
    ):
...
def test_start_trace_gives_up_after_max_deadlock_retries(store: SqlAlchemyStore):
    ...
    with (
        ...
        mock.patch("MLForge.store.tracking.sqlalchemy_store.time.sleep"),
        pytest.raises(MLForgeException, match="deadlock"),
    ):
        store.start_trace(trace_info)

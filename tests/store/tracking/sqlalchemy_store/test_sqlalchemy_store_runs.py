...
from MLForge.store.db.db_types import MSSQL, MYSQL, POSTGRES, SQLITE
from MLForge.store.entities import PagedList
from MLForge.store.tracking import (
    SEARCH_MAX_RESULTS_THRESHOLD,
)
from MLForge.store.tracking.dbmodels import models
...
from MLForge.store.tracking.sqlalchemy_workspace_store import WorkspaceAwareSqlAlchemyStore
...
from MLForge.utils.name_utils import _GENERATOR_PREDICATES
...
def test_run_tag_model(store: SqlAlchemyStore):
    ...
    with store.ManagedSessionMaker(read_only=False) as session:
        ...
        added_tag = added_tags[0].to_MLForge_entity()
...
def test_metric_model(store: SqlAlchemyStore):
    ...
    with store.ManagedSessionMaker(read_only=False) as session:
        ...
        added_metric = metrics[0].to_MLForge_entity()
...
def test_param_model(store: SqlAlchemyStore):
    ...
    with store.ManagedSessionMaker(read_only=False) as session:
        ...
        added_param = params[0].to_MLForge_entity()
...
def test_run_needs_uuid(store: SqlAlchemyStore):
    ...
    with pytest.raises(MLForgeException, match=regex) as exception_context:
        with store.ManagedSessionMaker(read_only=False) as session:
...
def test_run_info(store: SqlAlchemyStore):
    ...
    config = {
        ...
        "source_version": MLForge.__version__,
        "lifecycle_stage": entities.LifecycleStage.ACTIVE,
        "artifact_uri": "//",
    }
    run = models.SqlRun(**config).to_MLForge_entity()
...
def test_create_run_with_tags(store: SqlAlchemyStore):
    ...
    tags.append(RunTag(MLForge_tags.MLForge_RUN_NAME, expected["run_name"]))
...
def test_create_run_sets_name(store: SqlAlchemyStore):
    ...
    assert run.data.tags.get(MLForge_tags.MLForge_RUN_NAME) == configs["run_name"]
    ...
    run_id = store.create_run(
        ...
        start_time=0,
        run_name=None,
        tags=[RunTag(MLForge_tags.MLForge_RUN_NAME, "test")],
    ).info.run_id
    run = store.get_run(run_id)
    ...
    with pytest.raises(
        ...
        match=re.escape(
            ...
            "different values (run_name='test', MLForge.runName='test_2').",
        ),
    ):
        store.create_run(
            experiment_id=experiment_id,
            ...
            start_time=0,
            run_name="test",
            tags=[RunTag(MLForge_tags.MLForge_RUN_NAME, "test_2")],
        )
...
def test_to_MLForge_entity_and_proto(store: SqlAlchemyStore):
...
def test_log_metric(store: SqlAlchemyStore):
    ...
    # MLForge RunData contains only the last reported values for metrics.
    with store.ManagedSessionMaker() as session:
...
def test_record_logged_model(
    ...
    with store.ManagedSessionMaker() as session:
        ...
        tags = [t.value for t in run.tags if t.key == MLForge_tags.MLForge_LOGGED_MODELS]
...
def test_log_null_metric(store: SqlAlchemyStore):
    ...
    with pytest.raises(
        MLForgeException, match=r"Missing value for required parameter 'value'"
...
def test_log_param_uniqueness(store: SqlAlchemyStore):
    ...
    with pytest.raises(MLForgeException, match=r"Changing param values is not allowed"):
        store.log_param(run.info.run_id, param2)
...
@pytest.mark.skipif(
    ...
    and MLForge.get_tracking_uri().startswith("mssql"),
...
def test_set_experiment_tag(store: SqlAlchemyStore):
    ...
    with pytest.raises(MLForgeException, match="exceeds the maximum length of 5000"):
    ...
    with pytest.raises(MLForgeException, match="must be in the 'active' state"):
...
def test_set_tag(store: SqlAlchemyStore, monkeypatch):
    ...
    monkeypatch.setenv("MLForge_TRUNCATE_LONG_VALUES", "false")
    with pytest.raises(
        MLForgeException, match=f"exceeds the maximum length of {MAX_TAG_VAL_LENGTH} characters"
    ):
        store.set_tag(
...
def test_delete_tag(store: SqlAlchemyStore):
    ...
    with pytest.raises(MLForgeException, match="Run with id=randomRunId not found"):
...
def test_get_metric_history(store: SqlAlchemyStore):
    ...
    key = "test"
    expected = [
        models.SqlMetric(key=key, value=0.6, timestamp=1, step=0).to_MLForge_entity(),
        models.SqlMetric(key=key, value=0.7, timestamp=2, step=0).to_MLForge_entity(),
    ]
...
def test_update_run_name(store: SqlAlchemyStore):
    ...
    assert run.data.tags.get(MLForge_tags.MLForge_RUN_NAME) == "new name"
    ...
    store.delete_tag(run_id, MLForge_tags.MLForge_RUN_NAME)
    run = store.get_run(run_id)
    assert run.info.run_name == "new name"
    assert run.data.tags.get(MLForge_tags.MLForge_RUN_NAME) is None

    store.update_run_info(run_id, RunStatus.FINISHED, 1000, "newer name")
    run = store.get_run(run_id)
    assert run.info.run_name == "newer name"
    assert run.data.tags.get(MLForge_tags.MLForge_RUN_NAME) == "newer name"

    store.set_tag(run_id, entities.RunTag(MLForge_tags.MLForge_RUN_NAME, "newest name"))
    run = store.get_run(run_id)
    assert run.data.tags.get(MLForge_tags.MLForge_RUN_NAME) == "newest name"
    ...
    store.log_batch(
        ...
        metrics=[],
        params=[],
        tags=[entities.RunTag(MLForge_tags.MLForge_RUN_NAME, "batch name")],
    )
    run = store.get_run(run_id)
    assert run.data.tags.get(MLForge_tags.MLForge_RUN_NAME) == "batch name"
...
def test_error_logging_to_deleted_run(store: SqlAlchemyStore):
    ...
    with pytest.raises(MLForgeException, match=r"The run .+ must be in the 'active' state"):
...
def test_search_attrs(store: SqlAlchemyStore):
    ...
    for k, v in {"experiment_id": e1, "lifecycle_stage": "ACTIVE"}.items():
        with pytest.raises(MLForgeException, match=r"Invalid attribute key '.+' specified"):
...
def test_search_runs_datasets(store: SqlAlchemyStore):
    ...
    test_tag = [entities.InputTag(key=MLForge_DATASET_CONTEXT, value="test")]
    train_tag = [entities.InputTag(key=MLForge_DATASET_CONTEXT, value="train")]
    eval_tag = [entities.InputTag(key=MLForge_DATASET_CONTEXT, value="eval")]
...
def test_search_runs_datasets_with_param_filters(store: SqlAlchemyStore):
    ...
    This is a regression test for https://github.com/MLForge/MLForge/pull/19498
    ...
    train_tag = [entities.InputTag(key=MLForge_DATASET_CONTEXT, value="training")]
...
def test_search_datasets_returns_no_more_than_max_results(store: SqlAlchemyStore):
    ...
    for i in range(1010):
        ...
        input_tag = [entities.InputTag(key=MLForge_DATASET_CONTEXT, value=str(i))]
...
def test_log_batch(store: SqlAlchemyStore):
    ...
    tag_entities = [
        RunTag("t1", "t1val"),
        RunTag("t2", "t2val"),
        RunTag(MLForge_RUN_NAME, "my_run"),
    ]
    store.log_batch(
        run_id=run_id, metrics=metric_entities, params=param_entities, tags=tag_entities
    )
    run = store.get_run(run_id)
    assert run.data.tags == {"t1": "t1val", "t2": "t2val", MLForge_RUN_NAME: "my_run"}
...
def test_log_batch_param_overwrite_disallowed(store: SqlAlchemyStore):
    ...
    with pytest.raises(
        MLForgeException, match=r"Changing param values is not allowed"
    ) as exception_context:
...
def test_log_batch_with_unchanged_and_new_params(store: SqlAlchemyStore):
    ...
    ---------------------------------------------
    MLForge.log_params({"a": 0, "b": 1})
    MLForge.log_params({"a": 0, "c": 2})
    ---------------------------------------------
    """
    run = _run_factory(store)
    store.log_batch(
...
def test_log_batch_internal_error(store: SqlAlchemyStore):
    ...
    package = "MLForge.store.tracking.sqlalchemy_store.SqlAlchemyStore"
    with (
        ...
        for kwargs in [
            ...
            with pytest.raises(MLForgeException, match=r"Some internal error"):
...
def test_log_batch_nonexistent_run(store: SqlAlchemyStore):
    ...
    with pytest.raises(
        MLForgeException, match=rf"Run with id={nonexistent_run_id} not found"
    ) as exception_context:
...
def test_log_batch_duplicate_metrics_across_key_batches(store: SqlAlchemyStore):
    ...
    See https://github.com/MLForge/MLForge/issues/19144
    """
    run = _run_factory(store)
...
def test_log_inputs_fails_with_missing_inputs(store: SqlAlchemyStore):
    ...
    with pytest.raises(MLForgeException, match="InputTag key cannot be None"):
        store.log_inputs(
            run.info.run_id,
            [
    ...
    with pytest.raises(MLForgeException, match="InputTag value cannot be None"):
        store.log_inputs(
            run.info.run_id,
            [
                entities.DatasetInput(
    ...
    with pytest.raises(MLForgeException, match="Dataset name cannot be None"):
        store.log_inputs(
            run.info.run_id,
            [
                entities.DatasetInput(
    ...
    with pytest.raises(MLForgeException, match="Dataset digest cannot be None"):
        store.log_inputs(
            run.info.run_id,
            [
                entities.DatasetInput(
    ...
    with pytest.raises(MLForgeException, match="Dataset source_type cannot be None"):
        store.log_inputs(
            run.info.run_id,
            [
                entities.DatasetInput(
    ...
    with pytest.raises(MLForgeException, match="Dataset source cannot be None"):
        store.log_inputs(
            run.info.run_id,
            [
                entities.DatasetInput(
...
def test_link_traces_to_run_100_limit(store: SqlAlchemyStore):
    ...
    with pytest.raises(MLForgeException, match="Cannot link more than 100 traces to a run"):
        store.link_traces_to_run(trace_ids, run.info.run_id)

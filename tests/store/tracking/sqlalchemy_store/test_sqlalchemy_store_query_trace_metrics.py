...
from MLForge.entities.assessment import AssessmentError
...
from MLForge.genai.judges import CategoricalRating
...
def test_query_trace_metrics_source_run_filter_includes_linked_otlp_traces(
    ...
    """OTLP traces have no MLForge.sourceRun metadata; they are linked post-hoc via
    link_traces_to_run(). Q2/Q3 on the Traces tab filter by MLForge.sourceRun — this test
    ...
    # Native trace: has MLForge.sourceRun in metadata (pre-existing path)
    ...
    # OTLP traces: no MLForge.sourceRun metadata, linked post-hoc via link_traces_to_run
    for i in range(2):
...
def test_query_assessment_metrics_excludes_overridden_assessments(store: SqlAlchemyStore):
    # Regression test for https://github.com/MLForge/MLForge/issues/24132. Assessments in an

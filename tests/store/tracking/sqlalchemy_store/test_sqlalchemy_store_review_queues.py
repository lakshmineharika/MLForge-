...
from MLForge.genai.review_queues import ReviewItemType, ReviewQueueType, ReviewStatus
...
from MLForge.store.tracking.dbmodels.models import SqlReviewQueue, SqlReviewQueueItem
...
def test_create_user_queue_rejects_schema_ids(store):
    ...
    with pytest.raises(MLForgeException, match="cannot have explicitly-attached schemas") as exc:
...
def test_create_user_queue_rejects_mismatched_users(store):
    ...
    with pytest.raises(MLForgeException, match="exactly one assigned user equal to its name") as exc:
...
def test_create_custom_queue_rejects_reserved_name(store):
    ...
    with pytest.raises(MLForgeException, match="reserved queue name") as exc:
...
def test_create_rejects_unknown_queue_type(store):
    ...
    with pytest.raises(MLForgeException, match="`queue_type` must be one of") as exc:
...
def test_create_rejects_schema_from_other_experiment(store):
    ...
    with pytest.raises(MLForgeException, match="not found for experiment") as exc:
...
def test_create_against_missing_experiment_raises(store):
    with pytest.raises(MLForgeException, match="No Experiment with id") as exc:
...
def test_create_custom_queue_rejects_users_over_cap(store):
    ...
    with pytest.raises(MLForgeException, match="at most 10 assigned users") as exc:
        store.create_review_queue(
...
def test_get_or_create_user_queue_rejects_custom_name_collision(store):
    ...
    store.create_review_queue(exp_id, name="alice", queue_type="custom")
    with pytest.raises(MLForgeException, match="non-user queue named") as exc:
...
def test_update_user_queue_raises(store):
    ...
    queue = store.create_review_queue(exp_id, name="alice", queue_type="user")
    with pytest.raises(MLForgeException, match="fixed and cannot be updated") as exc:
...
def test_update_questions_locked_once_traces_assigned(store):
    ...
    with pytest.raises(MLForgeException, match="locked once items are assigned") as exc:
...
def test_update_display_case_rename_surfaces_unrelated_integrity_error(store):
    ...
    with mock.patch.object(Session, "flush", boom_on_pending):
        with pytest.raises(MLForgeException, match="unrelated") as exc:
...
def test_change_owner_rejects_empty(store):
    ...
    queue = store.create_review_queue(exp_id, name="q", queue_type="custom")
    with pytest.raises(MLForgeException, match="non-empty string") as exc:
...
def test_add_traces_rejects_empty_list(store):
    ...
    queue = store.create_review_queue(exp_id, name="q", queue_type="custom")
    with pytest.raises(MLForgeException, match="non-empty list") as exc:
...
def test_complete_requires_completed_by(store):
    ...
    queue = store.create_review_queue(exp_id, name="q", queue_type="custom")
    store.add_items_to_review_queue(queue.queue_id, item_ids=["tr-1"])
    with pytest.raises(MLForgeException, match="`completed_by` is required") as exc:
...
def test_reopen_rejects_completed_by(store):
    ...
    queue = store.create_review_queue(exp_id, name="q", queue_type="custom")
    store.add_items_to_review_queue(queue.queue_id, item_ids=["tr-1"])
    with pytest.raises(MLForgeException, match="must not be set when reopening") as exc:
        store.set_review_queue_item_status(
...
def test_create_queue_experiment_fk_violation_maps_to_not_found(store):
    ...
    with mock.patch.object(type(store), "_validate_experiment_exists", autospec=True):
        with pytest.raises(MLForgeException, match="does not exist") as exc:
            store.create_review_queue("999999", name="q", queue_type="custom")
    _assert_error_code(exc, RESOURCE_DOES_NOT_EXIST)

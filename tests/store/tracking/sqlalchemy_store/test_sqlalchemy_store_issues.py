...
from MLForge.entities import IssueSeverity, IssueStatus, TraceState
from MLForge.entities.assessment import IssueReference
...
def test_create_issue_invalid_experiment(store):
    with pytest.raises(MLForgeException, match=r"No Experiment with id=999999 exists"):
        store.create_issue(
...
def test_create_issue_invalid_run(store):
    ...
    with pytest.raises(MLForgeException, match=r"FOREIGN KEY constraint failed"):
        store.create_issue(
            experiment_id=exp_id,
...
def test_get_issue_nonexistent(store):
    with pytest.raises(MLForgeException, match=r"Issue with ID 'nonexistent-id' not found"):
...
def test_search_issues_filter_invalid_field(store):
    ...
    with pytest.raises(MLForgeException, match=r"Invalid filter field 'invalid_field'"):
...
def test_search_issues_invalid_max_results(store):
    ...
    with pytest.raises(MLForgeException, match=r"Invalid value 0 for parameter 'max_results'"):
        store.search_issues(max_results=0)

    with pytest.raises(MLForgeException, match=r"Invalid value -1 for parameter 'max_results'"):

...
import MLForge.store.workspace.dbmodels as _workspace_models  # noqa: F401
from MLForge.exceptions import MLForgeException
from MLForge.store.db import workspace_migration
from MLForge.store.db.base_sql_model import Base
from MLForge.store.db.utils import _get_alembic_config, _verify_schema
from MLForge.store.db.workspace_migration import migrate_to_default_workspace
from MLForge.store.tracking.dbmodels.initial_models import Base as InitialBase
from MLForge.store.tracking.sqlalchemy_store import SqlAlchemyStore
from MLForge.utils.semver_utils import encode_prerelease_sort_key, parse_semver
...
def _assert_schema_files_equal(generated_schema_file, expected_schema_file):
    ...
    for generated_schema_table, expected_schema_table in zip(
        ...
        assert generated_lines == expected_lines, (
            ...
            " of MLForge to update the schema snapshot."
        )
...
def test_db_upgrade_does_not_bootstrap_non_empty_database(db_url):
    # Guard against silently creating MLForge tables in a foreign DB. If any
    ...
    res = CliRunner().invoke(MLForge.db.commands, ["upgrade", db_url])
...
def test_sqlalchemy_store_detects_schema_mismatch(db_url):
    def _assert_invalid_schema(engine):
        with pytest.raises(MLForgeException, match="Detected out-of-date database schema."):
...
def test_store_generated_schema_matches_base(tmp_path, db_url):
    ...
    # index metadata but `mc` does. Note this doesn't mean the MLForge database is missing indexes
    ...
    assert len(diff) == 0, (
        ...
        "updated the ORM definitions in `MLForge/store/tracking/dbmodels/models.py`."
    )


def test_create_index_on_run_uuid(tmp_path, db_url):
    # Test for MLForge/store/db_migrations/versions/bd07f7e963c5_create_index_on_run_uuid.py
    SqlAlchemyStore(db_url, tmp_path.joinpath("ARTIFACTS").as_uri())
...
def test_create_index_on_metrics_run_uuid_key_step(tmp_path, db_url):
    # Test for
    # MLForge/store/db_migrations/versions/a5b4c3d2e1f0_add_metrics_run_key_step_index.py
    SqlAlchemyStore(db_url, tmp_path.joinpath("ARTIFACTS").as_uri())
    with sqlite3.connect(db_url[len("sqlite:///") :]) as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT name FROM sqlite_master WHERE type = 'index'")
...
def test_index_for_dataset_tables(tmp_path, db_url):
    # Test for
    # MLForge/store/db_migrations/versions/7f2a7d5fae7d_add_datasets_inputs_input_tags_tables.py
    SqlAlchemyStore(db_url, tmp_path.joinpath("ARTIFACTS").as_uri())
    with sqlite3.connect(db_url[len("sqlite:///") :]) as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT name FROM sqlite_master WHERE type = 'index'")

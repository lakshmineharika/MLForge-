...
from MLForge.entities.entity_type import EntityAssociationType
from MLForge.entities.lifecycle_stage import LifecycleStage
from MLForge.entities.trace_location import TraceLocation
from MLForge.entities.trace_state import TraceState
from MLForge.entities.workspace import TraceArchivalConfig
...
from MLForge.store.tracking.gateway.config_resolver import (
    get_endpoint_config,
...
from MLForge.tracing.utils import generate_request_id_v2
from MLForge.tracking._tracking_service import utils as tracking_utils
from MLForge.tracking._tracking_service.client import TrackingServiceClient
from MLForge.utils.MLForge_tags import MLForge_DATASET_CONTEXT
...
def test_experiments_are_workspace_scoped(workspace_tracking_store):
    ...
    with WorkspaceContext("team-a"):
        ...
        with pytest.raises(
            MLForgeException, match=f"No Experiment with id={exp_b_id} exists"
        ) as excinfo:
...
def test_runs_are_workspace_scoped(workspace_tracking_store):
    ...
    with WorkspaceContext("team-b"):
        ...
        with pytest.raises(
            MLForgeException, match=f"No Experiment with id={exp_a_id} exists"
        ) as excinfo:
        ...
        with pytest.raises(
            MLForgeException, match=f"Run with id={run_a.info.run_id} not found"
        ) as excinfo:
...
def test_search_datasets_is_workspace_scoped(workspace_tracking_store):
    ...
    with WorkspaceContext("team-a"):
        workspace_tracking_store.log_inputs(
            run_a.info.run_id,
            [DatasetInput(dataset_a, [InputTag(MLForge_DATASET_CONTEXT, "train")])],
        )

    with WorkspaceContext("team-b"):
        workspace_tracking_store.log_inputs(
            run_b.info.run_id,
            [DatasetInput(dataset_b, [InputTag(MLForge_DATASET_CONTEXT, "train")])],
        )
...
def test_serving_artifacts_auto_scopes_workspace_paths(workspace_tracking_store, monkeypatch):
    monkeypatch.setenv("_MLForge_SERVER_SERVE_ARTIFACTS", "true")
    workspace_tracking_store.artifact_root_uri = "MLForge-artifacts:/artifacts"
    ...
    assert calls == [("MLForge-artifacts:/artifacts", "team-prefix")]


def test_serving_artifacts_allows_pre_scoped_roots(workspace_tracking_store, monkeypatch):
    monkeypatch.delenv("_MLForge_SERVER_SERVE_ARTIFACTS", raising=False)
    workspace_tracking_store.artifact_root_uri = "MLForge-artifacts:/artifacts"
...
def test_create_experiment_requires_effective_artifact_root(workspace_tracking_store, monkeypatch):
    ...
    with WorkspaceContext("team-misconfigured"):
        with pytest.raises(MLForgeException, match="Cannot determine an artifact root"):
...
def test_experiment_lifecycle_operations_are_workspace_scoped(workspace_tracking_store):
    ...
    with WorkspaceContext("team-b"):
        with pytest.raises(MLForgeException, match=f"No Experiment with id={exp_a_id} exists"):
...
def test_run_lifecycle_operations_workspace_isolation(workspace_tracking_store):
    ...
    with WorkspaceContext("team-b"):
        with pytest.raises(MLForgeException, match=f"Run with id={run_a.info.run_id} not found"):
...
def test_get_trace_is_workspace_scoped(workspace_tracking_store):
    ...
    with WorkspaceContext("team-b"):
        with pytest.raises(
            MLForgeException, match=f"Trace with ID {trace_id} is not found."
        ) as excinfo:
...
def test_metric_bulk_operations_are_workspace_scoped(workspace_tracking_store):
    ...
    with WorkspaceContext("team-metrics-b"):
        ...
        with pytest.raises(MLForgeException, match="Run with id=.* not found"):
...
def test_assessment_operations_are_workspace_scoped(workspace_tracking_store):
    from MLForge.entities.assessment import AssessmentSource, AssessmentSourceType, Feedback
    ...
    with WorkspaceContext("team-assessment-b"):
        ...
        with pytest.raises(MLForgeException, match=r"Trace with ID .* not found"):
...
def test_link_prompts_to_trace_is_workspace_scoped(workspace_tracking_store):
    ...
    with WorkspaceContext("team-prompt-b"):
        ...
        with pytest.raises(MLForgeException, match=r"Trace with ID .* not found") as excinfo:
...
def gateway_workspace_store(workspace_tracking_store, monkeypatch):
    monkeypatch.setenv("MLForge_CRYPTO_KEK_PASSPHRASE", "test-passphrase")
...
def test_detach_model_from_endpoint_workspace_scoped(gateway_workspace_store):
    ...
    with WorkspaceContext("workspace-a"):
        with pytest.raises(MLForgeException, match="GatewayEndpoint not found"):
...
def test_delete_scorer_workspace_scoped(workspace_tracking_store):
    ...
    with WorkspaceContext("team-del-scorer-b"):
        ...
        with pytest.raises(MLForgeException, match="exists"):
...
def test_get_issue_is_workspace_scoped(workspace_tracking_store):
    ...
    with WorkspaceContext("team-b"):
        with pytest.raises(
            MLForgeException, match=f"Issue with ID '{issue_a.issue_id}' not found"
        ) as excinfo:
...
def test_create_issue_is_workspace_scoped(workspace_tracking_store):
    ...
    with WorkspaceContext("team-b"):
        ...
        with pytest.raises(MLForgeException, match=f"Issue with ID '{issue_a.issue_id}' not found"):
            workspace_tracking_store.get_issue(issue_a.issue_id)
...
def test_label_schemas_are_workspace_scoped(workspace_tracking_store):
    from MLForge.genai.label_schemas.label_schemas import InputPassFail
    ...
    with WorkspaceContext("team-b"):
        ...
        # at the schema query.
        with pytest.raises(MLForgeException, match="No Experiment with id"):
...
def test_review_queue_question_lock_holds_in_workspace_store(workspace_tracking_store):
    ...
    with WorkspaceContext("team-a"):
        ...
        with pytest.raises(MLForgeException, match="locked once items are assigned"):

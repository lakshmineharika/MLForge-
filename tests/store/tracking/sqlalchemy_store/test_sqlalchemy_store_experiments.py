...
def test_single_tenant_store_detects_workspace_scoped_experiments(
    ...
    with pytest.raises(MLForgeException, match="non-default workspaces"):
...
def test_raise_duplicate_experiments(store: SqlAlchemyStore):
    with pytest.raises(MLForgeException, match=r"Experiment\(name=.+\) already exists"):
...
def test_duplicate_experiment_with_artifact_location_returns_resource_already_exists(
    ...
    # Second creation should raise MLForgeException with RESOURCE_ALREADY_EXISTS error code
...
def test_raise_experiment_dont_exist(store: SqlAlchemyStore):
    with pytest.raises(MLForgeException, match=r"No Experiment with id=.+ exists"):

...
from MLForge.entities.assessment import (
    Assessment,
    Expectation,
    ExpectationValue,
    Feedback,
...
from MLForge.entities.assessment_error import AssessmentError
...
from MLForge.entities.span import LiveSpan
...
from MLForge.entities.trace_info_v2 import TraceInfoV2
...
from MLForge.environment_variables import (
    _MLForge_CREATE_LOGGED_MODEL_PARAMS_BATCH_SIZE,
    _MLForge_LOG_LOGGED_MODEL_PARAMS_BATCH_SIZE,
    MLForge_ASYNC_TRACE_LOGGING_RETRY_TIMEOUT,
)
...
from MLForge.protos.databricks_pb2 import ENDPOINT_NOT_FOUND, RESOURCE_DOES_NOT_EXIST
...
from MLForge.protos.service_pb2 import RunTag as ProtoRunTag
from MLForge.protos.service_pb2 import TraceRequestMetadata as ProtoTraceRequestMetadata
from MLForge.protos.service_pb2 import TraceTag as ProtoTraceTag
from MLForge.store.tracking.rest_store import RestStore
from MLForge.tracing.analysis import TraceFilterCorrelationResult
from MLForge.tracing.constant import TRACE_SCHEMA_VERSION_KEY
from MLForge.tracking.request_header.default_request_header_provider import (
...
from MLForge.utils.rest_utils import (
    _V3_ISSUES_REST_API_PATH_PREFIX,
    _V3_TRACE_REST_API_PATH_PREFIX,
    MLForgeHostCreds,
    get_logged_model_endpoint,
)
from MLForge.utils.workspace_context import WorkspaceContext, get_request_workspace
...
def workspaces_enabled(request):
    ...
    if enabled:
        with (
            ...
            mock.patch(
                "MLForge.store.workspace_rest_store_mixin.WorkspaceRestStoreMixin.supports_workspaces",
                return_value=True,
            ),
        ):
            yield enabled
...
def mock_http_request():
    return mock.patch(
        "MLForge.utils.rest_utils.http_request",
...
def test_successful_http_request():
    def mock_request(*args, **kwargs):
        ...
        assert args == ("POST", "https://hello/api/2.0/MLForge/experiments/search")
        kwargs = {k: v for k, v in kwargs.items() if v is not None}
        headers = DefaultRequestHeaderProvider().request_headers()
        if workspace := get_request_workspace():
            headers["X-MLForge-WORKSPACE"] = workspace
        assert kwargs == {
...
def test_failed_http_request():
    ...
    with mock.patch("requests.Session.request", return_value=response):
        store = RestStore(lambda: MLForgeHostCreds("https://hello"))
        with pytest.raises(MLForgeException, match="RESOURCE_DOES_NOT_EXIST: No experiment"):
...
def test_failed_http_request_custom_handler():
    ...
    with mock.patch("requests.Session.request", return_value=response):
        store = CustomErrorHandlingRestStore(lambda: MLForgeHostCreds("https://hello"))
...
def _args(host_creds, endpoint, method, json_body, use_v3=False, retry_timeout_seconds=None):
    ...
    res = {
        ...
        "endpoint": f"/api/{version}/MLForge/{endpoint}",
        "method": method,
    }
...
def _verify_requests(
    ...
    Args:
        ...
        host_creds: MLForgeHostCreds object
        endpoint: The endpoint being called (e.g., "traces/123")
        method: The HTTP method (e.g., "GET", "POST")
        json_body: The request body as a JSON string
        use_v3: If True, verify using /api/3.0/MLForge/ prefix instead of /api/2.0/MLForge/
...
def test_requestor():
    ...
    source_name_patch = mock.patch(
        "MLForge.tracking.context.default_context._get_source_name", return_value=source_name
    )
    ...
    with (
        ...
        mock.patch("MLForge.tracking._tracking_service.utils._get_store", return_value=store),
        mock.patch("MLForge.tracking.context.default_context._get_user", return_value=user_name),
        mock.patch("time.time", return_value=13579),
        source_name_patch,
        source_type_patch,
    ):
        with MLForge.start_run(experiment_id="43", run_name=run_name):
            cr_body = message_to_json(
                CreateRun(
                    ...
                    start_time=13579000,
                    tags=[
                        ProtoRunTag(key="MLForge.source.name", value=source_name),
                        ProtoRunTag(key="MLForge.source.type", value="LOCAL"),
                        ProtoRunTag(key="MLForge.user", value=user_name),
                        ProtoRunTag(key="MLForge.runName", value=run_name),
                    ],
                )
            )
    ...
    with mock.patch("MLForge.utils.rest_utils.http_request") as mock_http:
        response = mock.MagicMock()
        response.status_code = 200
...
def test_get_experiment_by_name():
    ...
    with mock.patch("MLForge.utils.rest_utils.http_request") as mock_http:
        ...
        nonexistent_exp_response.text = MLForgeException(
...
def test_deprecated_start_trace_v2():
    ...
    with mock.patch("MLForge.utils.rest_utils.http_request", return_value=response) as mock_http:
...
def test_start_trace(monkeypatch):
    monkeypatch.setenv(MLForge_ASYNC_TRACE_LOGGING_RETRY_TIMEOUT.name, "1")
...
def test_search_traces():
        ...
        "traces": [
            {
                "trace_id": "tr-1234",
                "trace_location": {
                    "type": "MLForge_EXPERIMENT",
                    "MLForge_experiment": {"experiment_id": "1234"},
                },
    ...
    with mock.patch("MLForge.utils.rest_utils.http_request", return_value=response) as mock_http:
        ...
        assert json_body["locations"][0]["MLForge_experiment"]["experiment_id"] == experiment_ids[0]
...
def test_search_traces_v3_endpoint_not_found_falls_back_to_v2():
    ...
    endpoint_not_found = MLForgeException("Not found", error_code=ENDPOINT_NOT_FOUND)
...
def test_get_artifact_uri_for_trace_compatibility():
    from MLForge.tracing.constant import TraceTagKey
    from MLForge.tracing.utils.artifact_utils import (
    ...
    trace_info_v2 = TraceInfoV2(
        ...
        tags={MLForge_ARTIFACT_LOCATION: "s3://bucket/trace-v2-path"},
    )
    ...
    trace_info_v3 = TraceInfo(
        ...
        tags={MLForge_ARTIFACT_LOCATION: "s3://bucket/trace-v3-path"},
    )
...
def test_delete_traces_with_batching():
    from MLForge.environment_variables import _MLForge_DELETE_TRACES_MAX_BATCH_SIZE
    ...
    batch_size = _MLForge_DELETE_TRACES_MAX_BATCH_SIZE.get()
...
def test_update_assessment_invalid_update():
    ...
    with pytest.raises(MLForgeException, match="Exactly one of `expectation` or `feedback`"):
...
def test_get_trace_info():
    ...
    trace = MLForge.get_trace(span.trace_id)
...
def test_get_trace():
    ...
    with MLForge.start_span(name="root_span") as span:
        span.set_inputs({"input": "value"})
        span.set_outputs({"output": "value"})
        with MLForge.start_span(name="child_span") as child:
...
def test_get_trace_raises_other_errors():
    ...
    genuine_error = MLForgeException(
        error_message,
        error_code=RESOURCE_DOES_NOT_EXIST,
    )

    with mock.patch.object(store, "_call_endpoint", side_effect=genuine_error):
        with pytest.raises(MLForgeException, match=error_message):
            store.get_trace("abc123")


def test_log_logged_model_params():
    with mock.patch("MLForge.store.tracking.rest_store.call_endpoint") as mock_call_endpoint:
        # Create test data
        model_id = "model_123"
        params = [
...
def test_create_logged_models_with_params(
    ...
    monkeypatch.setenv(_MLForge_CREATE_LOGGED_MODEL_PARAMS_BATCH_SIZE.name, str(create_batch_size))
    monkeypatch.setenv(_MLForge_LOG_LOGGED_MODEL_PARAMS_BATCH_SIZE.name, str(log_batch_size))

    store = RestStore(lambda: None)
    with (
        mock.patch("MLForge.entities.logged_model.LoggedModel.from_proto") as mock_from_proto,
...
def test_create_evaluation_dataset():
    creds = MLForgeHostCreds("https://test-server")
    store = RestStore(lambda: creds)
    ...
    with mock.patch.object(store, "_call_endpoint") as mock_call:
        ...
        mock_call.assert_called_once_with(
            ...
            endpoint="/api/3.0/MLForge/datasets/create",
        )
...
def test_get_evaluation_dataset():
    ...
    with mock.patch.object(store, "_call_endpoint") as mock_call:
        ...
        mock_call.assert_called_once_with(
            GetDataset,
            None,
            endpoint=f"/api/3.0/MLForge/datasets/{dataset_id}",
        )
...
def test_set_evaluation_dataset_tags():
    ...
    with mock.patch.object(store, "_call_endpoint") as mock_call:
        ...
        mock_call.assert_called_once_with(
            SetDatasetTags,
            expected_json,
            endpoint=f"/api/3.0/MLForge/datasets/{dataset_id}/tags",
        )
...
def test_delete_dataset_tag():
    ...
    with mock.patch.object(store, "_call_endpoint") as mock_call:
        ...
        mock_call.assert_called_once_with(
            DeleteDatasetTag,
            None,
            endpoint=f"/api/3.0/MLForge/datasets/{dataset_id}/tags/{key}",
        )
...
def test_dataset_apis_blocked_in_databricks():
    ...
    with mock.patch("MLForge.tracking.get_tracking_uri", return_value="databricks://profile"):
        creds = MLForgeHostCreds("https://workspace.cloud.databricks.com")
        store = RestStore(lambda: creds)
    ...
    with mock.patch("MLForge.tracking.get_tracking_uri", return_value="http://localhost:5000"):
        non_databricks_creds = MLForgeHostCreds("http://localhost:5000")
...
def test_upsert_evaluation_dataset_records():
    ...
    records = [
        ...
        {
            "inputs": {"question": "How to use MLForge?"},
    ...
    with mock.patch.object(store, "_call_endpoint") as mock_call:
        ...
        mock_call.assert_called_once_with(
            UpsertDatasetRecords,
            expected_json,
            endpoint=f"/api/3.0/MLForge/datasets/{dataset_id}/records",
        )
...
def test_get_evaluation_dataset_experiment_ids():
    ...
    with mock.patch.object(store, "_call_endpoint") as mock_call:
        ...
        mock_call.assert_called_once_with(
            GetDatasetExperimentIds,
            None,
            endpoint=f"/api/3.0/MLForge/datasets/{dataset_id}/experiment-ids",
        )
...
def test_evaluation_dataset_error_handling():
    ...
    with mock.patch("MLForge.utils.rest_utils.http_request") as mock_http:
        ...
        with pytest.raises(MLForgeException, match="Evaluation dataset not found"):
...
def test_evaluation_dataset_comprehensive_workflow():
    ...
    with mock.patch.object(store, "_call_endpoint") as mock_call:
        ...
        records1 = [
            {"inputs": {"q": "What is MLForge?"}, "expectations": {"score": 0.9}},
        ...
        records2 = [
            ...
            {"inputs": {"q": "What is MLForge?"}, "expectations": {"score": 0.95}},  # Update
    ...
    with mock.patch("MLForge.tracking._tracking_service.utils._get_store") as mock_get_store:
        ...
        with mock.patch.object(store, "get_dataset") as mock_get:
            ...
            with mock.patch.object(store, "_call_endpoint") as mock_call:
                ...
                records = [
                    {
                        "inputs": {"question": "What is MLForge?", "temperature": 0.7},
                        "expectations": {"accuracy": 0.95},
                    },
                    {
                ...
                assert endpoint == f"/api/3.0/MLForge/datasets/{dataset_id}/records"
...
def test_evaluation_dataset_get_records():
    ...
    with mock.patch.object(store, "_call_endpoint") as mock_call:
        ...
        assert records[0].inputs == {"question": "What is MLForge?"}
...
def test_evaluation_dataset_lazy_loading_records():
    ...
    with mock.patch("MLForge.tracking._tracking_service.utils._get_store") as mock_get_store:
        ...
        with mock.patch.object(store, "_load_dataset_records") as mock_load:
            from MLForge.entities.dataset_record import DatasetRecord
...
def test_evaluation_dataset_created_by_and_updated_by():
    ...
    with mock.patch.object(store, "_call_endpoint") as mock_call:
        ...
        created_response.dataset.tags = json.dumps({"MLForge.user": "user1", "environment": "test"})
        ...
        dataset = store.create_dataset(
            name="test_dataset",
            experiment_ids=["exp1"],
            tags={"MLForge.user": "user1", "environment": "test"},
        )

        assert dataset.created_by == "user1"
        assert dataset.last_updated_by == "user1"
        assert dataset.tags["MLForge.user"] == "user1"
        ...
        get_response.dataset.tags = json.dumps({"MLForge.user": "user1", "environment": "test"})
        ...
        store.set_dataset_tags(
            "d-test123",
            {"MLForge.user": "user3", "version": "2.0", "environment": "staging"},
        )
        ...
        assert "MLForge.user" in tags_dict
        assert tags_dict["MLForge.user"] == "user3"
...
def test_add_dataset_to_experiments():
    ...
    with mock.patch.object(store, "_call_endpoint") as mock_call:
        ...
        mock_call.assert_called_once_with(
            AddDatasetToExperiments,
            message_to_json(req),
            endpoint="/api/3.0/MLForge/datasets/d-1234567890abcdef1234567890abcdef/add-experiments",
        )
...
def test_remove_dataset_from_experiments():
    ...
    with mock.patch.object(store, "_call_endpoint") as mock_call:
        ...
        mock_call.assert_called_once_with(
            RemoveDatasetFromExperiments,
            message_to_json(req),
            endpoint="/api/3.0/MLForge/datasets/d-1234567890abcdef1234567890abcdef/remove-experiments",
        )
...
def test_register_scorer():
    ...
    with mock.patch.object(store, "_call_endpoint") as mock_call_endpoint:
        ...
        mock_call_endpoint.assert_called_once_with(
            ...
            endpoint="/api/3.0/MLForge/scorers/register",
        )
...
def test_list_scorers():
    ...
    with mock.patch.object(store, "_call_endpoint") as mock_call_endpoint:
        ...
        mock_call_endpoint.assert_called_once_with(
            ...
            endpoint="/api/3.0/MLForge/scorers/list",
        )
...
def test_list_scorer_versions():
    ...
    with mock.patch.object(store, "_call_endpoint") as mock_call_endpoint:
        ...
        mock_call_endpoint.assert_called_once_with(
            ...
            endpoint="/api/3.0/MLForge/scorers/versions",
        )
...
def test_get_scorer_with_version():
    ...
    with mock.patch.object(store, "_call_endpoint") as mock_call_endpoint:
        ...
        mock_call_endpoint.assert_called_once_with(
            ...
            endpoint="/api/3.0/MLForge/scorers/get",
        )
...
def test_delete_scorer_with_version():
    ...
    with mock.patch.object(store, "_call_endpoint") as mock_call_endpoint:
        ...
        mock_call_endpoint.assert_called_once_with(
            ...
            endpoint="/api/3.0/MLForge/scorers/delete",
        )
...
def test_calculate_trace_filter_correlation():
    ...
    with mock.patch.object(store, "_call_endpoint") as mock_call_endpoint:
        ...
        mock_call_endpoint.assert_called_once_with(
            CalculateTraceFilterCorrelation,
            message_to_json(expected_request),
            "/api/3.0/MLForge/traces/calculate-filter-correlation",
        )
...
def test_log_spans_with_version_check():
    ...
    creds1 = MLForgeHostCreds("https://host1")
    store1 = RestStore(lambda: creds1)
    with mock.patch(
        "MLForge.store.tracking.rest_store.http_request", side_effect=Exception("Connection error")
    ):
        with pytest.raises(NotImplementedError, match="could not identify MLForge server version"):
            store1.log_spans(experiment_id, spans)

    # Test 2: Server version is less than 3.4
    creds2 = MLForgeHostCreds("https://host2")
    store2 = RestStore(lambda: creds2)
    with mock.patch(
        "MLForge.store.tracking.rest_store.http_request",
        return_value=_create_mock_response(text="3.3.0"),
    ):
        with pytest.raises(
            NotImplementedError, match="MLForge server version 3.3.0 is less than 3.4"
        ):
            store2.log_spans(experiment_id, spans)

    # Test 3: Server version is exactly 3.4.0 - should succeed
    creds3 = MLForgeHostCreds("https://host3")
    ...
    creds4 = MLForgeHostCreds("https://host4")
    ...
    creds5 = MLForgeHostCreds("https://host5")
...
def test_server_version_check_caching():
    ...
    creds = MLForgeHostCreds("https://cached-host")
    ...
    with mock.patch(
        "MLForge.store.tracking.rest_store.http_request", return_value=_create_mock_response()
    ) as mock_http:

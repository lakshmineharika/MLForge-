...
from MLForge.environment_variables import MLForge_ENABLE_WORKSPACES, MLForge_TRACKING_URI
...
from MLForge.tracing.constant import SpanMetricKey, TraceMetadataKey
...
def test_get_gateway_secret_info_requires_one_of_id_or_name(store: SqlAlchemyStore):
    with pytest.raises(MLForgeException, match="Exactly one of") as exc:
...
def test_delete_gateway_model_definition_in_use_raises(store: SqlAlchemyStore):
    ...
    with pytest.raises(MLForgeException, match="currently in use") as exc:
...
def test_create_gateway_endpoint_auto_creates_experiment(store: SqlAlchemyStore):
    ...
    assert experiment.tags.get("MLForge.experiment.sourceType") == "GATEWAY"
    assert experiment.tags.get("MLForge.experiment.sourceId") == endpoint.endpoint_id
    assert experiment.tags.get("MLForge.experiment.isGateway") == "true"
...
def test_create_gateway_endpoint_empty_models_raises(store: SqlAlchemyStore):
    with pytest.raises(MLForgeException, match="at least one") as exc:
...
def test_attach_duplicate_model_raises(store: SqlAlchemyStore):
    ...
    with pytest.raises(MLForgeException, match="already attached") as exc:
...
def test_detach_nonexistent_mapping_raises(store: SqlAlchemyStore):
    ...
    with pytest.raises(MLForgeException, match="not attached") as exc:
        store.detach_model_from_endpoint(
            endpoint_id=endpoint.endpoint_id,
...
def test_get_budget_policy_not_found_raises(store: SqlAlchemyStore):
    with pytest.raises(MLForgeException, match="BudgetPolicy"):
...
def test_list_budget_policies_invalid_max_results(store: SqlAlchemyStore):
    with pytest.raises(MLForgeException, match="max_results"):
...
def test_create_gateway_guardrail_rejects_unknown_scorer(store: SqlAlchemyStore):
    with pytest.raises(MLForgeException, match="Scorer with ID 'missing-scorer' not found"):
...
def test_add_guardrail_to_endpoint_duplicate_raises(store: SqlAlchemyStore):
    ...
    with pytest.raises(MLForgeException, match="already added"):
        store.add_guardrail_to_endpoint(
            endpoint_id=endpoint.endpoint_id,
            guardrail_id=guardrail.guardrail_id,
        )

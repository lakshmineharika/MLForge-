...
import MLForge
from MLForge.entities import Span
...
from MLForge.entities.trace_location import TraceLocation, UCSchemaLocation, UnityCatalog
...
from MLForge.environment_variables import (
    ...
    MLForge_SQL_WAREHOUSE_AUTO_START,
    MLForge_TRACING_SQL_WAREHOUSE_ID,
)
from MLForge.exceptions import MLForgeException, MLForgeNotImplementedException, RestException
from MLForge.protos import databricks_pb2
from MLForge.protos.databricks_pb2 import ENDPOINT_NOT_FOUND
from MLForge.protos.databricks_tracing_pb2 import (
    BatchGetTraces,
    CreateLocation,
...
from MLForge.protos.databricks_tracing_pb2 import UCSchemaLocation as ProtoUCSchemaLocation
from MLForge.protos.databricks_tracing_pb2 import (
    UcTablePrefixLocation as ProtoUcTablePrefixLocation,
)
from MLForge.protos.service_pb2 import DeleteTraceTag as DeleteTraceTagV3
from MLForge.protos.service_pb2 import GetTraceInfoV3, StartTraceV3
from MLForge.protos.service_pb2 import SetTraceTag as SetTraceTagV3
from MLForge.store.tracking.databricks_rest_store import CompositeToken, DatabricksTracingRestStore
from MLForge.store.tracking.rest_store import RestStore
from MLForge.tracing.constant import TRACE_ID_V4_PREFIX
from MLForge.utils.databricks_tracing_utils import assessment_to_proto, trace_to_proto
...
def _disable_sql_warehouse_auto_start(monkeypatch):
    ...
    Databricks SDK when MLForge_TRACING_SQL_WAREHOUSE_ID is set. Tests that assert on the
    auto-start hook patch ``ensure_sql_warehouse_running`` directly, which intercepts the call
    regardless of this flag.
    """
    monkeypatch.setenv(MLForge_SQL_WAREHOUSE_AUTO_START.name, "false")


@pytest.fixture
def sql_warehouse_id(monkeypatch):
    wh_id = "test-warehouse"
    monkeypatch.setenv(MLForge_TRACING_SQL_WAREHOUSE_ID.name, wh_id)
    return wh_id
...
def test_create_trace_v4_uc_location(monkeypatch):
    ...
    monkeypatch.setenv(MLForge_TRACING_SQL_WAREHOUSE_ID.name, "test-warehouse")
...
def test_get_trace_info(monkeypatch):
    with MLForge.start_span(name="test_span_v4") as span:
    ...
    store = DatabricksTracingRestStore(lambda: MLForgeHostCreds("https://test"))

    location = "catalog.schema"
    v4_trace_id = f"{TRACE_ID_V4_PREFIX}{location}/{span.trace_id}"

    monkeypatch.setenv("MLForge_TRACING_SQL_WAREHOUSE_ID", "test-warehouse")
...
def test_get_trace_info_fallback_to_v3():
    with MLForge.start_span(name="test_span_v3") as span:
        span.set_inputs({"input": "test_value"})
...
def test_get_trace_info_missing_warehouse_id():
    ...
    with mock.patch.object(
        ...
        with pytest.raises(MLForgeException, match="SQL warehouse ID is required for "):
...
def test_set_trace_tag():
    ...
    with mock.patch("MLForge.utils.rest_utils.http_request", return_value=response) as mock_http:
        ...
        mock_http.assert_called_once_with(
            host_creds=creds,
            endpoint=f"/api/4.0/MLForge/traces/{location}/{trace_id}/tags",
            method="PATCH",
            json=expected_json,
        )
        assert res is None
...
def test_delete_trace_tag(monkeypatch):
    ...
    monkeypatch.setenv("MLForge_TRACING_SQL_WAREHOUSE_ID", sql_warehouse_id)
    with mock.patch("MLForge.utils.rest_utils.http_request", return_value=response) as mock_http:
        ...
        mock_http.assert_called_once_with(
            host_creds=creds,
            endpoint=f"/api/4.0/MLForge/traces/{location}/{trace_id}/tags/{request.key}?sql_warehouse_id={sql_warehouse_id}",
            method="DELETE",
            json=None,
        )
        assert res is None
...
def test_delete_trace_tag_with_special_characters(monkeypatch):
    ...
    with mock.patch("MLForge.utils.rest_utils.http_request", return_value=response) as mock_http:
        ...
        mock_http.assert_called_once_with(
            host_creds=creds,
            endpoint=f"/api/4.0/MLForge/traces/{location}/{trace_id}/tags/foo%2Fbar?sql_warehouse_id={sql_warehouse_id}",
            method="DELETE",
            json=None,
        )
        assert res is None
...
def test_batch_get_traces(monkeypatch, sql_warehouse_id):
    monkeypatch.setenv(MLForge_TRACING_SQL_WAREHOUSE_ID.name, "test-warehouse")
    with MLForge.start_span(name="test_span_1") as span1:
        span1.set_inputs({"input": "test_value_1"})
        span1.set_outputs({"output": "result_1"})

    with MLForge.start_span(name="test_span_2") as span2:
        span2.set_inputs({"input": "test_value_2"})
        span2.set_outputs({"output": "result_2"})

    trace1 = MLForge.get_trace(span1.trace_id)
    trace2 = MLForge.get_trace(span2.trace_id)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        RestException(
            json={
                "error_code": databricks_pb2.ErrorCode.Name(databricks_pb2.INVALID_PARAMETER_VALUE),
                "message": "MLForge_EXPERIMENT locations not yet supported",
            }
        ),
    ],
)
def test_search_traces_experiment_id(exception):
        ...
        "traces": [
            {
                ...
                "trace_location": {
                    ...
                    "MLForge_experiment": {"experiment_id": "1"},
                },
                "request_time": "1970-01-01T00:00:00.123Z",
                "execution_duration_ms": 456,
                "state": "OK",
    ...
    # MLForge first tries V4 long-running endpoint, then falls back to V3
    ...
    assert "locations" in json_body
    assert len(json_body["locations"]) == 1
    assert json_body["locations"][0]["MLForge_experiment"]["experiment_id"] == "1"
    assert json_body["filter"] == filter_string
...
def test_search_traces_with_mixed_locations(exception):
    ...
    expected_error_message = (
        ...
        else "The `locations` parameter cannot contain both MLForge experiment and UC schema "
    )

    with mock.patch("MLForge.utils.rest_utils.http_request", side_effect=exception) as mock_http:
        with pytest.raises(MLForgeException, match=expected_error_message):
            store.search_traces(
                filter_string="state = 'OK'",
...
def test_search_traces_does_not_fallback_when_uc_schemas_are_specified():
    ...
    def mock_http_request(*args, **kwargs):
        if kwargs.get("endpoint") == f"{_V4_TRACE_REST_API_PATH_PREFIX}/search-long-running":
            raise MLForgeException("V4 endpoint not supported", error_code=ENDPOINT_NOT_FOUND)
        return mock.MagicMock()

    with mock.patch("MLForge.utils.rest_utils.http_request", side_effect=mock_http_request):
        with pytest.raises(
            MLForgeException,
...
def test_search_traces_non_fallback_errors():
    ...
    with mock.patch("MLForge.utils.rest_utils.http_request") as mock_http:
        mock_http.side_effect = MLForgeException("Random error")
        with pytest.raises(MLForgeException, match="Random error"):
            store.search_traces(locations=["catalog.schema"])
...
def test_search_traces_with_missing_location():
    creds = MLForgeHostCreds("https://hello")
    store = DatabricksTracingRestStore(lambda: creds)
    with pytest.raises(MLForgeException, match="location.*must be specified"):
        store.search_traces()
...
def test_search_traces_with_invalid_location():
    creds = MLForgeHostCreds("https://hello")
    store = DatabricksTracingRestStore(lambda: creds)
    with pytest.raises(MLForgeException, match="Invalid location type:"):
...
def _sample_trace_info_json(trace_id="tr-1234", experiment_id="1"):
    return {
        ...
        "trace_location": {
            "type": "MLForge_EXPERIMENT",
            "MLForge_experiment": {"experiment_id": experiment_id},
        },
        "request_time": "1970-01-01T00:00:00.123Z",
        "execution_duration_ms": 456,
        "state": "OK",
...
def test_search_traces_long_running_immediate_success():
    ...
    with mock.patch("MLForge.utils.rest_utils.http_request", return_value=initial) as mock_http:
...
def test_search_traces_long_running_failure_raises(monkeypatch):
    ...
    with mock.patch("MLForge.utils.rest_utils.http_request", return_value=failed_resp):
        with pytest.raises(MLForgeException, match="boom"):
            store.search_traces(locations=["1"])


def test_search_traces_long_running_preserves_request_fields(monkeypatch):
    monkeypatch.setenv(MLForge_TRACING_SQL_WAREHOUSE_ID.name, "wh-42")
    creds = MLForgeHostCreds("https://hello")
    store = DatabricksTracingRestStore(lambda: creds)
...
def test_get_trace_location_v5():
    ...
    assert call_args[1]["endpoint"] == "/api/5.0/MLForge/tracing/locations/loc-123"
...
def test_create_or_get_trace_location_v5_with_table_prefix(monkeypatch):
    monkeypatch.setenv(MLForge_TRACING_SQL_WAREHOUSE_ID.name, "warehouse-1")
    creds = MLForgeHostCreds("https://hello")
    store = DatabricksTracingRestStore(lambda: creds)
    ...
    assert call_args[1]["endpoint"] == "/api/5.0/MLForge/tracing/locations"
...
def test_link_trace_location_v5_with_table_prefix():
    ...
    assert call_args[1]["endpoint"] == "/api/5.0/MLForge/experiments/exp-123/trace-location:link"
    request_payload = json.loads(call_args[0][1])
...
def test_search_unified_traces(monkeypatch):
    ...
    with mock.patch("MLForge.utils.rest_utils.http_request", return_value=response) as mock_http:
        ...
        call_args = mock_http.call_args[1]
        assert call_args["endpoint"] == "/api/2.0/MLForge/unified-traces"
...
def test_set_experiment_trace_location_with_existing_location():
    ...
    create_location_response = MLForgeException(
...
def test_log_spans_to_uc_table_empty_spans():
    store = DatabricksTracingRestStore(lambda: MLForgeHostCreds("http://localhost"))
...
def test_log_spans_to_uc_table_success(diff_trace_id):
    ...
    with (
        mock.patch(
            "MLForge.store.tracking.databricks_rest_store.verify_rest_response"
        ) as mock_verify,
        mock.patch(
            "MLForge.store.tracking.databricks_rest_store.http_request", return_value=mock_response
        ) as mock_http_request,
        mock.patch(
            "MLForge.store.tracking.databricks_rest_store.get_databricks_workspace_client_config",
...
def test_log_spans_to_uc_table_config_error():
    ...
    with mock.patch(
        ...
        with pytest.raises(MLForgeException, match="Failed to log spans to UC table"):
            store.log_spans("catalog.schema.spans", spans, tracking_uri="databricks")
    ...
    request = {
        ...
        "trace_location": {
            ...
            "uc_schema": {
                "catalog_name": "catalog",
                "schema_name": "schema",
                "otel_spans_table_name": "MLForge_experiment_trace_otel_spans",
                "otel_logs_table_name": "MLForge_experiment_trace_otel_logs",
            },
        },
        "trace_id": "1234",
        "feedback": {"value": False},
...
def test_link_traces_to_run_with_v4_trace_ids_uses_batch_v4_endpoint():
    ...
    with mock.patch("MLForge.utils.rest_utils.http_request", return_value=response) as mock_http:
        ...
        mock_http.assert_called_once_with(
            host_creds=creds,
            endpoint=f"/api/4.0/MLForge/traces/{location}/link-to-run/batchCreate",
            method="POST",
            json=expected_json,
        )
...
def test_link_traces_to_run_with_v3_trace_ids_uses_v3_endpoint():
    ...
    with mock.patch("MLForge.utils.rest_utils.http_request", return_value=response) as mock_http:
        ...
        mock_http.assert_called_once_with(
            host_creds=creds,
            endpoint="/api/2.0/MLForge/traces/link-to-run",
            method="POST",
            json=expected_json,
        )
...
def test_link_traces_to_run_with_mixed_v3_v4_trace_ids_handles_both():
    ...
    with mock.patch("MLForge.utils.rest_utils.http_request", return_value=response) as mock_http:
        ...
        assert v3_call.kwargs["endpoint"] == "/api/2.0/MLForge/traces/link-to-run"
        ...
        expected_v4_endpoint = f"/api/4.0/MLForge/traces/{location}/link-to-run/batchCreate"
...
def test_link_traces_to_run_with_different_locations_groups_by_location():
    ...
    with mock.patch("MLForge.utils.rest_utils.http_request", return_value=response) as mock_http:
        ...
        expected_endpoints = {
            f"/api/4.0/MLForge/traces/{location1}/link-to-run/batchCreate",
            f"/api/4.0/MLForge/traces/{location2}/link-to-run/batchCreate",
        }
...
def test_unlink_traces_from_run_with_v4_trace_ids_uses_batch_v4_endpoint():
    ...
    with mock.patch("MLForge.utils.rest_utils.http_request", return_value=response) as mock_http:
        ...
        mock_http.assert_called_once_with(
            host_creds=creds,
            endpoint=f"/api/4.0/MLForge/traces/{location}/unlink-from-run/batchDelete",
            method="DELETE",
            json=expected_json,
        )
...
def test_unlink_traces_from_run_with_different_locations_groups_by_location():
    ...
    with mock.patch("MLForge.utils.rest_utils.http_request", return_value=response) as mock_http:
        ...
        call_endpoints = {call.kwargs["endpoint"] for call in calls}
        expected_endpoints = {
            f"/api/4.0/MLForge/traces/{location1}/unlink-from-run/batchDelete",
            f"/api/4.0/MLForge/traces/{location2}/unlink-from-run/batchDelete",
        }
        assert call_endpoints == expected_endpoints
...
def test_search_datasets_basic():
    ...
    with (
        mock.patch(
            "MLForge.store.tracking.databricks_rest_store.http_request",
            return_value=mock.Mock(json=lambda: response_data),
        ) as mock_http,
        mock.patch("MLForge.store.tracking.databricks_rest_store.verify_rest_response"),
    ):
...
def test_search_datasets_unsupported_parameters(param_name, param_value, error_match):
    ...
    with pytest.raises(MLForgeException, match=error_match):
...
def test_search_datasets_endpoint_not_found():
    ...
    with mock.patch(
        ...
        with pytest.raises(MLForgeException, match="not available in this Databricks workspace"):
...
def test_search_datasets_missing_required_field():
    ...
    with (
        ...
        with pytest.raises(MLForgeException, match="missing required field"):
            store.search_datasets(experiment_ids=["exp_1"])
        mock_http.assert_called_once()
...
def test_search_datasets_invalid_timestamp():
    ...
    with (
        ...
        with pytest.raises(MLForgeException, match="invalid timestamp format"):
            store.search_datasets(experiment_ids=["exp_1"])
        mock_http.assert_called_once()
...
def test_create_issue_not_implemented():
    ...
    with pytest.raises(
        MLForgeNotImplementedException, match="Issue management is not supported in Databricks"
    ):
        store.create_issue(
            experiment_id="exp-123",
            name="Test Issue",
...
# ---------------------------------------------------------------------------
# Auto-start SQL warehouse before /api/4.0 and /api/5.0 MLForge tracing calls
# ---------------------------------------------------------------------------
...
def mock_ensure_running():
    ...
    with mock.patch("MLForge.utils.databricks_sql_warehouse.ensure_sql_warehouse_running") as m:
        yield m


def _store():
    return DatabricksTracingRestStore(lambda: MLForgeHostCreds("https://test"))
...
def test_resolve_sql_warehouse_id_without_env_is_noop(monkeypatch, mock_ensure_running):
    monkeypatch.delenv(MLForge_TRACING_SQL_WAREHOUSE_ID.name, raising=False)
...
def test_append_sql_warehouse_id_param_triggers_ensure_running(
    ...
    store = _store()
    endpoint = store._append_sql_warehouse_id_param("/api/4.0/MLForge/traces/x/tags/k")
...
def test_append_sql_warehouse_id_param_without_env_is_noop(monkeypatch, mock_ensure_running):
    ...
    store = _store()
    endpoint = store._append_sql_warehouse_id_param("/api/4.0/MLForge/traces/x/tags/k")
    assert endpoint == "/api/4.0/MLForge/traces/x/tags/k"
    mock_ensure_running.assert_not_called()
...
def test_search_traces_v4_ensures_warehouse_running(sql_warehouse_id, mock_ensure_running):
    store = _store()
    from MLForge.protos.databricks_tracing_pb2 import SearchTracesOperation
...
def test_search_unified_traces_does_not_auto_start_warehouse(sql_warehouse_id, mock_ensure_running):
    """
    Scope guard: `_search_unified_traces` targets /api/2.0 (MLForgeService.SearchUnifiedTraces) and
    is out of scope for auto-start.
    """
    store = _store()
    from MLForge.protos.service_pb2 import SearchUnifiedTraces
...
def test_get_online_trace_details_does_not_auto_start_warehouse(
    ...
    """
    Scope guard: `get_online_trace_details` targets /api/2.0 (MLForgeService.GetOnlineTraceDetails).
    """
    store = _store()
    from MLForge.protos.service_pb2 import GetOnlineTraceDetails
...
def test_log_spans_does_not_auto_start_warehouse(sql_warehouse_id, mock_ensure_running):
    ...
    with (
        ...
        mock.patch(
            "MLForge.store.tracking.databricks_rest_store.http_request", return_value=response
        ),
        mock.patch(
            "MLForge.store.tracking.databricks_rest_store.verify_rest_response",

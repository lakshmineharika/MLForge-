...
from MLForge.entities import Experiment, Run, ViewType
from MLForge.store.fs2db import migrate
from MLForge.tracking import MLForgeClient
from MLForge.utils.file_utils import local_file_uri_to_path

Clients = tuple[MLForgeClient, MLForgeClient]


def _get_all_experiments(client: MLForgeClient) -> list[Experiment]:
    return client.search_experiments(view_type=ViewType.ALL)


def _get_all_runs(client: MLForgeClient, experiment_ids: list[str]) -> list[Run]:
    runs = []
...
def test_experiments(clients: Clients) -> None:
    ...
    for exp_id, src_exp in src_by_id.items():
        ...
        src_tags = {k: v for k, v in src_exp.tags.items() if not k.startswith("MLForge.")}
        dst_tags = {k: v for k, v in dst_exp.tags.items() if not k.startswith("MLForge.")}
...
def test_runs(clients: Clients) -> None:
    ...
    for run_id, src_run in src_by_id.items():
        ...
        src_tags = {k: v for k, v in src_run.data.tags.items() if not k.startswith("MLForge.")}
        dst_tags = {k: v for k, v in dst_run.data.tags.items() if not k.startswith("MLForge.")}
        assert dst_tags == src_tags
...
def test_rollback_on_failure(clients: Clients, tmp_path: Path) -> None:
    ...
    with mock.patch(
        "MLForge.store.fs2db._tracking._migrate_runs_in_dir",

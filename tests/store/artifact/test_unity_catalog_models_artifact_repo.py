...
from MLForge.utils.uri import _DATABRICKS_UNITY_CATALOG_SCHEME

MODELS_ARTIFACT_REPOSITORY_PACKAGE = "MLForge.store.artifact.unity_catalog_models_artifact_repo"
...
def test_uc_models_artifact_repo_download_artifacts_uses_temporary_creds_aws(monkeypatch):
    ...
    with (
        mock.patch("MLForge.utils.databricks_utils.get_databricks_host_creds"),
        mock.patch.object(
            MLForgeClient, "get_model_version_download_uri", return_value=artifact_location
        ),
        mock.patch("MLForge.utils.rest_utils.http_request") as request_mock,
        mock.patch(
        ...
        request_mock.assert_called_with(
            host_creds=ANY,
            endpoint="/api/2.0/MLForge/unity-catalog/model-versions/generate-temporary-credentials",
            method="POST",
...
def test_uc_models_artifact_repo_uses_active_catalog_and_schema():
    with mock.patch(
        "MLForge.store.artifact.unity_catalog_models_artifact_repo._get_active_spark_session"
...
def test_get_feature_dependencies_works_when_experiment_not_found():
    ...
    with mock.patch(
        ...
        side_effect=MLForgeException("RESOURCE_DOES_NOT_EXIST"),
    ) as mock_get_logged_model:
        result = MLForge.store._unity_catalog.registry.rest_store.get_feature_dependencies(
            model_info.model_uri
        )
        assert result == ""
...
def test_get_model_version_dependencies_works_when_experiment_not_found():
    ...
    with mock.patch(
        ...
        result = MLForge.store._unity_catalog.registry.rest_store.get_model_version_dependencies(
            model_info.model_uri
        )
        assert result == []
        mock_get_logged_model.assert_not_called()
...
def test_store_use_presigned_url_store_when_disabled(monkeypatch):
    store_package = "MLForge.store.artifact.unity_catalog_models_artifact_repo"

...
from MLForge.exceptions import MLForgeException, MLForgeTraceDataCorrupted, MLForgeTraceDataNotFound
from MLForge.store.artifact.local_artifact_repo import LocalArtifactRepository
from MLForge.tracing.otel.otel_archival import TRACE_ARCHIVAL_FILENAME, spans_to_traces_data_pb
...
def test_log_artifact_throws_exception_for_invalid_artifact_paths(local_artifact_repo):
    with TempDir() as local_dir:
        for bad_artifact_path in ["/", "//", "/tmp", "/bad_path", ".", "../terrible_path"]:
            with pytest.raises(MLForgeException, match="Invalid artifact path"):
...
def test_log_artifact_rejects_internal_temp_file_prefix(local_artifact_repo):
    with TempDir() as local_dir:
        ...
        with pytest.raises(MLForgeException, match="Artifact names starting with"):
...
def test_log_artifact_from_stream_closes_fd_when_fdopen_fails(local_artifact_repo):
    ...
    with (
        mock.patch(
            "MLForge.store.artifact.local_artifact_repo.tempfile.mkstemp",
            return_value=(123, str(temp_artifact_path)),
        ),
        mock.patch(
            "MLForge.store.artifact.local_artifact_repo.os.fdopen",
            side_effect=OSError("fdopen failed"),
        ),
        mock.patch("MLForge.store.artifact.local_artifact_repo.os.close") as close_mock,
        mock.patch("MLForge.store.artifact.local_artifact_repo.os.remove") as remove_mock,
    ):
...
def test_trace_data(local_artifact_repo):
    with pytest.raises(MLForgeTraceDataNotFound, match=r"Trace data not found for path="):
...
def test_upload_archived_trace_data_rejects_empty_spans(local_artifact_repo):
    with pytest.raises(MLForgeException, match="at least one span"):
...
def test_upload_archived_trace_data_rejects_non_trace_data(local_artifact_repo):
    with pytest.raises(MLForgeException, match="Archived trace data must be a TraceData object."):
...
def test_symlink_path_traversal_blocked(
    ...
    with pytest.raises(MLForgeException, match="resolved path is outside the artifact directory"):
...
def test_download_artifacts_nonexistent_raises_resource_does_not_exist(
    ...
    with pytest.raises(MLForgeException, match="No such artifact") as exc_info:

...
import MLForge
from MLForge.store.artifact import artifact_repository_registry
from MLForge.store.artifact.artifact_repository_registry import ArtifactRepositoryRegistry
...
def test_plugin_registration_via_installed_package():
    ...
    from MLForge_test_plugin.local_artifact import PluginLocalArtifactRepository
...
def test_get_unknown_scheme():
    ...
    with pytest.raises(
        MLForge.exceptions.MLForgeException, match="Could not find a registered artifact repository"
    ):
...
def test_plugin_registration_via_entrypoints():
    ...
    mock_get_group_all.assert_called_once_with("MLForge.artifact_repository")

...
def test_log_artifact(tmp_path: Path):
    ...
    with (
        mock.patch(
            "MLForge.store.artifact.databricks_sdk_artifact_repo.DatabricksSdkArtifactRepository.files_api"
        ) as mock_files_api,
        mock.patch(
            "MLForge.store.artifact.databricks_tracking_artifact_repo.DatabricksArtifactRepository",
            return_value=mock_databricks_artifact_repo,
        ),
    ):
        repo = DatabricksRunArtifactRepository("dbfs:/databricks/MLForge-tracking/1/123")
...
def test_log_artifacts(tmp_path: Path):
    ...
    with (
        ...
        repo = DatabricksRunArtifactRepository("dbfs:/databricks/MLForge-tracking/1/456")
...
def test_download_file(tmp_path: Path):
    ...
    with (
        ...
        repo = DatabricksRunArtifactRepository("dbfs:/databricks/MLForge-tracking/1/789")
...
@pytest.mark.parametrize(
    "valid_uri",
    [
        "dbfs:/databricks/MLForge-tracking/1/123",
        "dbfs:/databricks/MLForge-tracking/1/456/artifacts",
        "dbfs:/databricks/MLForge-tracking/1/789/artifacts/model",
    ],
)
def test_constructor_with_valid_uri(valid_uri: str):
    with (
        mock.patch(
            "MLForge.store.artifact.databricks_sdk_artifact_repo.DatabricksSdkArtifactRepository"
        ),
        mock.patch(
            "MLForge.store.artifact.databricks_tracking_artifact_repo.DatabricksArtifactRepository"
        ),
...
@pytest.mark.parametrize(
    ...
    [
        "dbfs:/invalid/uri",
        "dbfs:/databricks/MLForge-tracking/1",
        "dbfs:/databricks/MLForge-tracking/1/logged_models/1",
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        # Valid run URIs
        ("dbfs:/databricks/MLForge-tracking/1/123", True),
        ("dbfs:/databricks/MLForge-tracking/1/456/artifacts", True),
        ("dbfs:/databricks/MLForge-tracking/1/789/artifacts/model", True),
        # Invalid URIs
        ("dbfs:/databricks/MLForge-tracking/1", False),
        ("dbfs:/databricks/MLForge-tracking/1/logged_models/1", False),
        ("dbfs:/databricks/MLForge-tracking/1/tr-1", False),
...
@pytest.mark.parametrize(
    ...
    [
        ("dbfs:/databricks/MLForge-tracking/123/456", "123", "456", None),
        ("dbfs:/databricks/MLForge-tracking/123/456/artifacts", "123", "456", "/artifacts"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "dbfs:/databricks/MLForge-tracking/123/456",
            "/WorkspaceInternal/MLForge/Artifacts/123/Runs/456",
        ),
        (
            "dbfs:/databricks/MLForge-tracking/123/456/artifacts",
            "/WorkspaceInternal/MLForge/Artifacts/123/Runs/456/artifacts",
        ),
    ],
)
...
def test_expected_uri_format():
    with (
        ...
        assert repo._get_expected_uri_format() == (
            "databricks/MLForge-tracking/<EXPERIMENT_ID>/<RUN_ID>"
        )

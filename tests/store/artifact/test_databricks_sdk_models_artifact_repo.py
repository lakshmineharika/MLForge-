...
def mock_databricks_workspace_client():
    ...
    with mock.patch(
        "MLForge.store.artifact.databricks_sdk_models_artifact_repo._get_databricks_workspace_client",
...
def test_get_workspace_client_uses_resolved_creds_when_token_present():
    creds = MLForgeHostCreds(host="https://my-host", token="my-token")
    with (
        mock.patch(
...
@pytest.mark.parametrize(
    "creds",
    [
        MLForgeHostCreds(host="https://my-host"),  # host resolved but no token
...
def test_MLForge_use_databricks_sdk_model_artifacts_repo_for_uc(tmp_path, monkeypatch):
...
def test_MLForge_use_databricks_sdk_model_artifacts_repo_for_uc_seg(tmp_path, monkeypatch):
    monkeypatch.setenv("DATABRICKS_HOST", "my-host")
    monkeypatch.setenv("DATABRICKS_TOKEN", "my-token")
    with mock.patch(
        "MLForge.utils._unity_catalog_utils.call_endpoint",

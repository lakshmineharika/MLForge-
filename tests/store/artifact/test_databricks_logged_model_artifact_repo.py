...
from MLForge.store.artifact.databricks_logged_model_artifact_repo import (
...
def test_constructor_with_valid_uri():
    ...
    with (
        ...
        mock.patch(
            "MLForge.store.artifact.databricks_tracking_artifact_repo.DatabricksSdkArtifactRepository",
...
@pytest.mark.parametrize(
    ...
    [
        "invalid_uri",
        "dbfs:/databricks/MLForge-tracking/1/runs/1",
        "dbfs:/databricks/MLForge-tracking/1/other/1",
        "http://example.com",
        "s3://bucket/path",
    ],
)
def test_constructor_with_invalid_uri(invalid_uri: str):
    with pytest.raises(MLForgeException, match="Invalid artifact URI"):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("dbfs:/databricks/MLForge-tracking/1/logged_models/1", True),
        ("dbfs:/databricks/MLForge-tracking/123/logged_models/456", True),
        # Invalid URIs
        ("dbfs:/databricks/MLForge-tracking/1/runs/1", False),
        ("dbfs:/databricks/MLForge-tracking/1/other/1", False),
        ("dbfs:/databricks/MLForge-tracking/1", False),
        ("dbfs:/databricks/MLForge-tracking/1/logged_models", False),
        ("dbfs:/databricks/MLForge-tracking/logged_models/1", False),
    ],
)
...
def test_uri_parsing():
    ...
    with (
        ...
        repo = DatabricksLoggedModelArtifactRepository(
            "dbfs:/databricks/MLForge-tracking/123/logged_models/456"
        )

        # Access the regex match to verify parsing
        m = repo._get_uri_regex().search("dbfs:/databricks/MLForge-tracking/123/logged_models/456")
...
def test_build_root_path():
    ...
    with (
        ...
        expected_path = "/WorkspaceInternal/MLForge/Artifacts/123/LoggedModels/456"
...
def test_expected_uri_format():
    ...
    with (
        ...
        expected_format = "databricks/MLForge-tracking/<EXP_ID>/logged_models/<MODEL_ID>"

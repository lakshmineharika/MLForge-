...
from MLForge.entities.multipart_upload import (
...
from MLForge.environment_variables import (
    MLForge_MULTIPART_UPLOAD_MINIMUM_FILE_SIZE,
    MLForge_TRACKING_CLIENT_CERT_PATH,
    MLForge_TRACKING_INSECURE_TLS,
    MLForge_TRACKING_PASSWORD,
    MLForge_TRACKING_SERVER_CERT_PATH,
...
from MLForge.store.artifact.http_artifact_repo import HttpArtifactRepository
from MLForge.store.artifact.MLForge_artifacts_repo import MLForgeArtifactsRepository
...
def http_artifact_repo():
    artifact_uri = "http://test.com/api/2.0/MLForge-artifacts/artifacts"
    return HttpArtifactRepository(artifact_uri)


@pytest.fixture
def MLForge_artifact_repo_for_download():
    """MLForgeArtifactsRepository with same effective URI as http_artifact_repo.

    For multipart download tests.
    """
    return MLForgeArtifactsRepository(
        artifact_uri="MLForge-artifacts:/",
...
def test_log_artifact(
    ...
    file_path.write_text("0" * MLForge_MULTIPART_UPLOAD_MINIMUM_FILE_SIZE.get())
...
def test_log_artifact_skips_size_check_when_multipart_disabled(http_artifact_repo, tmp_path):
    ...
    with (
        ...
        mock.patch(
            "MLForge.store.artifact.http_artifact_repo.os.path.getsize",
...
def test_upload_part_honors_s3_ignore_tls(tmp_path, monkeypatch, ignore_tls, expected_verify):
    if ignore_tls is not None:
        monkeypatch.setenv("MLForge_S3_IGNORE_TLS", ignore_tls)
    ...
    with (
        ...
        mock.patch("MLForge.store.artifact.http_artifact_repo.augmented_raise_for_status"),
    ):
...
def test_list_artifacts_malicious_path(http_artifact_repo, path):
    with mock.patch(
        ...
        with pytest.raises(MLForgeException, match="Invalid path"):
            http_artifact_repo.list_artifacts()
...
def test_default_host_creds(monkeypatch):
    ...
    expected_host_creds = MLForgeHostCreds(
        host=artifact_uri,
        username=username,
        password=password,
        token=token,
    ...
    monkeypatch.setenv(MLForge_TRACKING_USERNAME.name, username)
    monkeypatch.setenv(MLForge_TRACKING_PASSWORD.name, password)
    monkeypatch.setenv(MLForge_TRACKING_TOKEN.name, token)
    monkeypatch.setenv(MLForge_TRACKING_INSECURE_TLS.name, str(ignore_tls_verification))
    monkeypatch.setenv(MLForge_TRACKING_CLIENT_CERT_PATH.name, client_cert_path)
    monkeypatch.setenv(MLForge_TRACKING_SERVER_CERT_PATH.name, server_cert_path)
...
def test_complete_multipart_upload(http_artifact_repo, monkeypatch):
    ...
    with mock.patch(
        ...
        endpoint = "/MLForge-artifacts"
        url, _ = http_artifact_repo.artifact_uri.split(endpoint, maxsplit=1)
        mock_post.assert_called_once_with(
            get_default_host_creds(url),
            "/MLForge-artifacts/mpu/complete/artifact/path",
            "POST",
            json={
                "path": "local_file",
                "upload_id": "upload_id",
...
def test_abort_multipart_upload(http_artifact_repo, monkeypatch):
    ...
    with mock.patch(
        ...
        mock_post.assert_called_once_with(
            get_default_host_creds(url),
            "/MLForge-artifacts/mpu/abort/artifact/path",
            "POST",
            json={
                "path": "local_file",
                "upload_id": "upload_id",
...
def test_download_file_multipart_for_large_files(
    MLForge_artifact_repo_for_download, tmp_path, monkeypatch
):
    """Test that multipart download is used when presigned URL is available.

    Uses MLForgeArtifactsRepository.
    """
    monkeypatch.setenv("MLForge_ENABLE_PROXY_MULTIPART_DOWNLOAD", "true")
    ...
    with (
        mock.patch.object(
            MLForge_artifact_repo_for_download,
        ...
        mock.patch.object(
            MLForge_artifact_repo_for_download, "_multipart_download"
        ) as mock_multipart_download,
    ):
        file_path = tmp_path / "large_file.bin"
        MLForge_artifact_repo_for_download._download_file(remote_file_path, str(file_path))
...
def test_download_file_multipart_success_does_not_call_proxy_download(
    ...
    with (
        ...
        mock.patch.object(MLForge_artifact_repo_for_download, "_multipart_download"),
...
def test_download_file_fallback_when_presigned_not_supported(
    ...
    with (
        ...
        mock_http_request.assert_called_once_with(
            MLForge_artifact_repo_for_download._host_creds,
            posixpath.join("/", remote_file_path),
            "GET",
            stream=True,
        )
...
def test_download_file_multipart_disabled_uses_proxy(
    ...
    with (
        mock.patch.object(
            MLForge_artifact_repo_for_download, "_get_presigned_download_url"
        ) as mock_get_presigned,
        mock.patch(
            "MLForge.store.artifact.http_artifact_repo.http_request",
            return_value=MockStreamResponse("data", 200),
...
def test_multipart_download_creates_chunks(http_artifact_repo, tmp_path, monkeypatch):
    chunk_size = 100
    monkeypatch.setenv("MLForge_MULTIPART_DOWNLOAD_CHUNK_SIZE", str(chunk_size))
    ...
    with mock.patch(
        "MLForge.store.artifact.http_artifact_repo.download_chunk",
...
def test_get_presigned_download_url(http_artifact_repo):
    ...
    with mock.patch(
        ...
        mock_request.assert_called_once_with(
            get_default_host_creds(url),
            f"/MLForge-artifacts/presigned/{remote_file_path}",
            "GET",
        )

...
from MLForge.utils.rest_utils import MLForgeHostCreds, http_request
...
DBFS_ARTIFACT_REPOSITORY_PACKAGE = "MLForge.store.artifact.dbfs_artifact_repo"
...
def test_init_get_host_creds_with_databricks_profile_uri():
    ...
    with (
        mock.patch(
            DBFS_ARTIFACT_REPOSITORY_PACKAGE + "._get_host_creds_from_default_store",
            return_value=lambda: MLForgeHostCreds(default_host),
        ),
        mock.patch(
            DBFS_ARTIFACT_REPOSITORY_PACKAGE + ".get_databricks_host_creds",
            return_value=MLForgeHostCreds(databricks_host),
        ),
    ):
...
def test_log_artifact_error(dbfs_artifact_repo, test_file):
    with mock.patch(
        "MLForge.utils.rest_utils.http_request", return_value=Mock(status_code=409, text="")
    ):
        with pytest.raises(MLForgeException, match=r"API request to endpoint .+ failed"):
            dbfs_artifact_repo.log_artifact(test_file)
...
def test_get_host_creds_from_default_store_file_store():
    ...
    with mock.patch("MLForge.tracking._tracking_service.utils._get_store", return_value=FileStore()):
        with pytest.raises(MLForgeException, match="Failed to get credentials for DBFS"):
            _get_host_creds_from_default_store()
...
def test_get_host_creds_from_default_store_rest_store():
    with mock.patch(
        ...
        return_value=RestStore(lambda: MLForgeHostCreds("http://host")),
    ):
        assert isinstance(_get_host_creds_from_default_store()(), MLForgeHostCreds)

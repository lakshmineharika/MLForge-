...
from MLForge.store.artifact.sftp_artifact_repo import SFTPArtifactRepository
...
def test_log_and_download_sklearn_model(tmp_path):
    ...
    experiment_id = MLForge.create_experiment(
        name="sklearn-model-experiment",
        artifact_location=f"sftp://{tmp_path}",
    )
    with MLForge.start_run(experiment_id=experiment_id):
        model_uri = MLForge.sklearn.log_model(original, name="model").model_uri
        downloaded = MLForge.sklearn.load_model(model_uri)

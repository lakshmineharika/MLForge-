...
from MLForge.store.artifact.models_artifact_repo import ModelsArtifactRepository
from MLForge.store.artifact.unity_catalog_models_artifact_repo import (
    UnityCatalogModelsArtifactRepository,
)
from MLForge.store.artifact.unity_catalog_oss_models_artifact_repo import (
...
def test_models_artifact_repo_init_with_uc_registry_db_profile_inferred_from_context():
    ...
    with (
        ...
        mock.patch("MLForge.get_registry_uri", return_value=uc_registry_uri),
    ):
        mock_repo.return_value.model_name = "MyModel"
        mock_repo.return_value.model_version = "12"
...
def test_models_artifact_repo_download_with_real_files(tmp_path):
    ...
    with mock.patch.object(
        MLForgeClient, "get_model_version_download_uri", return_value=str(model_dir)
    ):

...
from MLForge.store.artifact.runs_artifact_repo import RunsArtifactRepository
...
def test_parse_runs_uri_invalid_input(uri):
    with pytest.raises(MLForgeException, match="Not a proper runs"):
...
def test_get_artifact_uri(uri, expected_tracking_uri, mock_uri, expected_result_uri):
    with mock.patch(
        "MLForge.tracking.artifact_utils.get_artifact_uri", return_value=mock_uri
...
def test_runs_artifact_repo_init_with_real_run():
    ...
    experiment_id = MLForge.create_experiment("expr_abc", artifact_location)
...
def test_runs_artifact_repo_uses_repo_download_artifacts():
    ...
    """
    artifact_location = "s3://blah_bucket/"
    experiment_id = MLForge.create_experiment("expr_abcd", artifact_location)
    with MLForge.start_run(experiment_id=experiment_id):
        run_id = MLForge.active_run().info.run_id
...
def test_runs_artifact_repo_tracking_uri_passed_as_keyword():
    ...
    """
    with mock.patch(
        "MLForge.tracking.artifact_utils.get_artifact_uri",
...
def test_get_logged_model_artifact_repo_uses_models_uri():
    with (
        mock.patch(
            "MLForge.store.artifact.runs_artifact_repo.RunsArtifactRepository.get_underlying_uri",
            return_value="MLForge-artifacts:/1/some-run-id/artifacts/model",
        ),
        mock.patch(
            "MLForge.store.artifact.runs_artifact_repo.MLForge.tracking.MLForgeClient"
        ) as mock_MLForge_client,
        mock.patch(
        ...
        runs_repo = RunsArtifactRepository(
            ...
            tracking_uri="http://test-tracking-server:5000",
            registry_uri="sqlite:///MLForge.db",
        )
        mock_get_artifact_repo.reset_mock()

        run = Mock()
        run.info.experiment_id = "123"
        mock_MLForge_client.return_value.get_run.return_value = run

        matched_model = Mock()
        matched_model.source_run_id = "some-run-id"
        matched_model.model_id = "m-123456"
        mock_MLForge_client.return_value.search_logged_models.return_value = PagedList(

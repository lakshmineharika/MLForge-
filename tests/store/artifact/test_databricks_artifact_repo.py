...
from MLForge.entities import TraceData
from MLForge.entities.file_info import FileInfo as FileInfoEntity
from MLForge.exceptions import MLForgeException, MLForgeNotImplementedException
from MLForge.protos.databricks_artifacts_pb2 import (
...
from MLForge.protos.service_pb2 import FileInfo, ListArtifacts
from MLForge.store.artifact.artifact_repository_registry import get_artifact_repository
from MLForge.store.artifact.databricks_artifact_repo import (
    _MAX_CREDENTIALS_REQUEST_SIZE,
    DatabricksArtifactRepository,
)
from MLForge.store.artifact.databricks_artifact_repo_resources import _CredentialType

DATABRICKS_ARTIFACT_REPOSITORY_PACKAGE = "MLForge.store.artifact.databricks_artifact_repo"
CLOUD_ARTIFACT_REPOSITORY_PACKAGE = "MLForge.store.artifact.cloud_artifact_repo"
...
DATABRICKS_ARTIFACT_REPOSITORY_RESOURCES = (
    "MLForge.store.artifact.databricks_artifact_repo_resources"
)
...
MOCK_RUN_ROOT_URI = "dbfs:/databricks/MLForge-tracking/MOCK-EXP/MOCK-RUN-ID/artifacts"
MOCK_TRACE_ROOT_URI = "dbfs:/databricks/MLForge-tracking/MOCK-EXP/tr-MOCK-RUN-ID/artifacts"
...
def disable_databricks_sdk_for_run_artifacts(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv("MLForge_DISABLE_DATABRICKS_SDK_FOR_RUN_ARTIFACTS", "true")
...
def test_init_validation_and_cleaning():
    with mock.patch(
        ...
        repo = get_artifact_repository(
            "dbfs:/databricks/MLForge-tracking/MOCK-EXP/MOCK-RUN-ID/artifacts"
        )
        assert (
            repo.artifact_uri == "dbfs:/databricks/MLForge-tracking/MOCK-EXP/MOCK-RUN-ID/artifacts"
        )
        ...
        with pytest.raises(MLForgeException, match="Artifact URI incorrect"):
            DatabricksArtifactRepository("dbfs:/databricks/MLForge/EXP/RUN/artifact")
        with pytest.raises(MLForgeException, match="DBFS URI must be of the form dbfs"):
            DatabricksArtifactRepository(
                "dbfs://scope:key@notdatabricks/databricks/MLForge-tracking/experiment/1/run/2"
            )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "dbfs://scope:key@databricks/databricks/MLForge-tracking/experiment/1/run/2",
            "dbfs:/databricks/MLForge-tracking/experiment/1/run/2",
        ...
        (
            "dbfs:/databricks/MLForge-tracking/MOCK-EXP/MOCK-RUN-ID/awesome/path",
            "dbfs:/databricks/MLForge-tracking/MOCK-EXP/MOCK-RUN-ID/awesome/path",
            "databricks://getTrackingUriDefault",
        ),
    ],
...
def test_init_artifact_uri(artifact_uri, expected_uri, expected_db_uri):
    with (
        ...
        mock.patch(
            "MLForge.tracking.get_tracking_uri", return_value="databricks://getTrackingUriDefault"
        ),
    ):
...
@pytest.mark.parametrize(
    ...
    [
        ("dbfs:/databricks/MLForge-tracking/MOCK-EXP/MOCK-RUN-ID/artifacts", ""),
        ("dbfs:/databricks/MLForge-tracking/MOCK-EXP/MOCK-RUN-ID/artifacts/arty", "arty"),
        (
            "dbfs://prof@databricks/databricks/MLForge-tracking/MOCK-EXP/MOCK-RUN-ID/artifacts/arty",
            "arty",
        ),
        (
            "dbfs:/databricks/MLForge-tracking/MOCK-EXP/MOCK-RUN-ID/awesome/path",
...
@pytest.mark.parametrize(
    ("uri", "expected"),
    [
        ("dbfs:/databricks/MLForge-tracking/123/456/artifact", "456"),
        ("dbfs:/databricks/MLForge-tracking/123/logged_models/m-123", None),
        ("dbfs:/databricks/MLForge-tracking/123/tr-123/artifacts", None),
        ("dbfs:/databricks/MLForge-tracking/123/", None),
    ],
)
...
def test_extract_resource():
    with mock.patch(
        ...
        repo = get_artifact_repository("dbfs:/databricks/MLForge-tracking/EXP/RUN_ID/artifact")
        assert repo.resource.id == expected_run_id
        repo = get_artifact_repository("dbfs:/databricks/MLForge-tracking/EXP_ID/RUN_ID/artifacts")
        assert repo.resource.id == expected_run_id
        repo = get_artifact_repository(
            "dbfs:/databricks///MLForge-tracking///EXP_ID///RUN_ID///artifacts/"
        )
        assert repo.resource.id == expected_run_id
        repo = get_artifact_repository(
            "dbfs:/databricks///MLForge-tracking//EXP_ID//RUN_ID///artifacts//"
...
def test_log_artifact_azure_blob_client_sas_error(databricks_artifact_repo, test_file):
    ...
    with (
        ...
        mock.patch("requests.Session.request", side_effect=MLForgeException("MOCK ERROR")),
    ):
        with pytest.raises(MLForgeException, match=r"MOCK ERROR"):
...
def test_log_artifact_adls_gen2_with_headers(
    ...
    monkeypatch.setenv("MLForge_MULTIPART_UPLOAD_CHUNK_SIZE", "5")
    with (
        mock.patch(
            f"{DATABRICKS_ARTIFACT_REPOSITORY}._get_credential_infos",
            return_value=[mock_credential_info],
...
def test_log_artifact_adls_gen2_flush_error(databricks_artifact_repo, test_file):
    ...
    mock_error_response = MLForgeException("MOCK ERROR")
    mock_credential_info = ArtifactCredentialInfo(
        signed_uri=MOCK_ADLS_GEN2_SIGNED_URI,
        type=ArtifactCredentialType.AZURE_ADLS_GEN2_SAS_URI,
    )
...
def test_log_artifact_aws_presigned_url_error(databricks_artifact_repo, test_file):
    ...
    with (
        ...
        with pytest.raises(MLForgeException, match="MOCK ERROR"):
            databricks_artifact_repo.log_artifact(test_file)
        get_credential_infos_mock.assert_called_with(_CredentialType.WRITE, ANY)
...
def test_download_artifacts_provides_failure_info(databricks_artifact_repo):
    with (
        ...
        mock.patch(
            f"{DATABRICKS_ARTIFACT_REPOSITORY}._download_from_cloud",
            side_effect=[
                MLForgeException("MOCK ERROR 1"),
                MLForgeException("MOCK ERROR 2"),
            ],
        ),
    ):
        match = r"The following failures occurred while downloading one or more artifacts.+"
        with pytest.raises(MLForgeException, match=match) as exc:
...
def mock_chunk_size(monkeypatch):
    ...
    chunk_size = 10
    monkeypatch.setenv("MLForge_MULTIPART_UPLOAD_MINIMUM_FILE_SIZE", str(chunk_size))
    monkeypatch.setenv("MLForge_MULTIPART_UPLOAD_CHUNK_SIZE", str(chunk_size))
    return chunk_size
...
def test_multipart_fail_due_to_chunk_size(databricks_artifact_repo, large_file):
    ...
    with (
        ...
        with pytest.raises(MLForgeException, match="Multipart chunk size"):
...
def test_download_archived_trace_data_rejects_archive_repo(databricks_artifact_repo_trace):
    with pytest.raises(MLForgeNotImplementedException, match="do not yet support ARCHIVE_REPO"):

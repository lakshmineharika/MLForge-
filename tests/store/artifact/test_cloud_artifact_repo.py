...
from MLForge.protos.databricks_artifacts_pb2 import ArtifactCredentialInfo, ArtifactCredentialType
from MLForge.store.artifact.cloud_artifact_repo import (
...
def test__parallelized_download_from_cloud(
    ...
    monkeypatch.setenv("_MLForge_MPD_NUM_RETRIES", "3")
    monkeypatch.setenv("_MLForge_MPD_RETRY_INTERVAL_SECONDS", "0")

    with mock.patch(
        "MLForge.store.artifact.cloud_artifact_repo.CloudArtifactRepository"
        ...
        with (
            mock.patch(
                "MLForge.store.artifact.cloud_artifact_repo.parallelized_download_file_using_http_uri",
                return_value=mock_failed_downloads,
            ),
            mock.patch(
                "MLForge.store.artifact.cloud_artifact_repo.as_completed",
                return_value=futures,
            ),
        ):
            if future_result:
                with pytest.raises(
                    MLForgeException, match="All retries have been exhausted. Download has failed."
                ):

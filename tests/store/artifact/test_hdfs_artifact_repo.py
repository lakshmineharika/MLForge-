...
from MLForge.store.artifact.hdfs_artifact_repo import (
...
def hdfs_system_mock():
    with mock.patch(
        "MLForge.store.artifact.hdfs_artifact_repo.HadoopFileSystem",
...
def test_log_artifact_with_kerberos_setup(hdfs_system_mock, monkeypatch, tmp_path):
    if sys.platform == "win32":
        pytest.skip()
    monkeypatch.setenv("MLForge_KERBEROS_TICKET_CACHE", "/tmp/krb5cc_22222222")
    monkeypatch.setenv("MLForge_KERBEROS_USER", "some_kerberos_user")

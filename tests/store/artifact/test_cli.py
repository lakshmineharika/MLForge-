...
from MLForge.store.artifact.cli import _file_infos_to_json, download_artifacts
...
def run_with_artifact(tmp_path):
    ...
    with MLForge.start_run() as run:
        MLForge.log_artifact(local_path, artifact_path)
...
def test_download_from_uri():
    ...
    with mock.patch(
        "MLForge.tracking.artifact_utils.get_artifact_repository"
...
def _run_download_artifact_command(args) -> pathlib.Path:
    """
    Args:
        command: An `MLForge artifacts` command list.
...
def test_download_artifacts_with_uri(run_with_artifact):
    ...
    # Check for backwards compatibility with preexisting behavior in MLForge <= 1.24.0 where
    # specifying `artifact_uri` and `artifact_path` together did not throw an exception (unlike
    # `MLForge.artifacts.download_artifacts()`) and instead used `artifact_uri` while ignoring

...
from MLForge.protos.databricks_artifacts_pb2 import ArtifactCredentialInfo
from MLForge.protos.service_pb2 import FileInfo
from MLForge.store.artifact.artifact_repo import try_read_trace_data
from MLForge.store.artifact.azure_data_lake_artifact_repo import (
...
ADLS_REPOSITORY_PACKAGE = "MLForge.store.artifact.azure_data_lake_artifact_repo"
...
def mock_data_lake_client():
    ...
    with mock.patch(
        "MLForge.store.artifact.azure_data_lake_artifact_repo._get_data_lake_client",
...
def test_parse_invalid_abfss_uri(uri):
    with pytest.raises(MLForgeException, match="ABFSS URI must be of the form"):
        _parse_abfss_uri(uri)


def test_parse_invalid_abfss_uri_bad_scheme():
    with pytest.raises(MLForgeException, match="Not an ABFSS URI"):
...
def test_log_artifacts_in_parallel_when_necessary(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MLForge_MULTIPART_UPLOAD_CHUNK_SIZE", "0")
    with (
        mock.patch(

...
from MLForge.store.artifact.utils.models import _parse_model_uri, get_model_name_and_version
...
def test_parse_models_uri_invalid_input(uri):
    with pytest.raises(MLForgeException, match="Not a proper models"):
        _parse_model_uri(uri)


def test_improper_model_uri_msg_prompts():
    from MLForge.store.artifact.utils.models import _improper_model_uri_msg
...
def test_get_model_name_and_version_with_version():
    ...
    ) as MLForge_client_mock:
        assert get_model_name_and_version(MLForgeClient(), "models:/AdsModel1/123") == (
            "AdsModel1",
            "123",
        )
        MLForge_client_mock.assert_not_called()
...
def test_get_model_name_and_version_with_stage():
    with mock.patch.object(
        ...
        assert get_model_name_and_version(MLForgeClient(), "models:/AdsModel1/Production") == (
            "AdsModel1",
            "15",
        )
        MLForge_client_mock.assert_called_once_with("AdsModel1", ["Production"])
...
def test_get_model_name_and_version_with_latest():
    with mock.patch.object(
        ...
        assert get_model_name_and_version(MLForgeClient(), "models:/AdsModel1/latest") == (
            "AdsModel1",
            "20",
        )
        MLForge_client_mock.assert_called_once_with("AdsModel1", None)
...
def test_get_model_name_and_version_with_alias():
    with mock.patch.object(
        ...
        assert get_model_name_and_version(MLForgeClient(), "models:/AdsModel1@Champion") == (
            "AdsModel1",
            "10",
        )
        MLForge_client_mock.assert_called_once_with("AdsModel1", "Champion")
...
def test_parse_prompts_uri_invalid_input(uri):
    with pytest.raises(MLForgeException, match="Not a proper prompts"):

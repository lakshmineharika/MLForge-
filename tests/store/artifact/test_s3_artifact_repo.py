...
from MLForge.entities.multipart_upload import MultipartUploadPart
from MLForge.entities.span import Span, SpanAttributeKey
from MLForge.exceptions import MLForgeException, MLForgeTraceDataCorrupted
...
from MLForge.store.artifact.s3_artifact_repo import (
    _MAX_CACHE_SECONDS,
...
def teardown_function():
    if "MLForge_S3_UPLOAD_EXTRA_ARGS" in os.environ:
        del os.environ["MLForge_S3_UPLOAD_EXTRA_ARGS"]
...
def test_get_s3_client_hits_cache(s3_artifact_root, monkeypatch):
    ...
    monkeypatch.setenv("MLForge_EXPERIMENTAL_S3_SIGNATURE_VERSION", "s3v2")
    repo._get_s3_client()
    cache_info = _cached_get_s3_client.cache_info()
    assert cache_info.hits == 1
    ...
    with mock.patch(
        "MLForge.store.artifact.s3_artifact_repo._get_utcnow_timestamp",
...
def test_get_s3_client_verify_param_set_correctly(
    ...
    monkeypatch.setenv("MLForge_S3_IGNORE_TLS", ignore_tls_env)
...
def test_get_s3_file_upload_extra_args_invalid_json():
    os.environ.setdefault(
        "MLForge_S3_UPLOAD_EXTRA_ARGS", '"ServerSideEncryption": "aws:kms", "SSEKMSKeyId": "123456"}'
    )
...
@pytest.mark.parametrize(
    ("boto_error_code", "expected_MLForge_error"),
    [
...
def test_list_artifacts_error_handling(s3_artifact_root, boto_error_code, expected_MLForge_error):
    ...
    with mock.patch.object(s3_repo, "_get_s3_client") as mock_client:
        ...
        with pytest.raises(
            MLForgeException, match=f"Failed to list artifacts in {s3_repo.artifact_uri}:"
        ) as exc_info:
            s3_repo.list_artifacts(artifact_path)
        assert exc_info.value.error_code == expected_MLForge_error
...
def test_trace_data(s3_artifact_root):
    ...
    with pytest.raises(MLForgeTraceDataCorrupted, match=r"Trace data is corrupted for path="):
        repo.download_trace_data()
...
def test_bucket_ownership_verification_with_env_var(s3_artifact_repo, tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MLForge_S3_EXPECTED_BUCKET_OWNER", "123456789012")
...
def test_bucket_ownership_verification_without_env_var(s3_artifact_root, tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("MLForge_S3_EXPECTED_BUCKET_OWNER", raising=False)
...
def test_bucket_takeover_scenario(s3_artifact_root, tmp_path, monkeypatch):
    ...
    1. A user creates and uses a bucket (e.g., `my-MLForge-artifacts`)
    2. The bucket is deleted
    3. An attacker creates a new bucket with the same name
    4. MLForge continues to use the same bucket URI, unknowingly sending
       artifacts to the attacker's bucket

    This test verifies that when MLForge_S3_EXPECTED_BUCKET_OWNER is set, operations
...
def test_create_presigned_upload_url_without_extra_args(s3_artifact_root, monkeypatch):
    monkeypatch.delenv("MLForge_S3_UPLOAD_EXTRA_ARGS", raising=False)
    repo = S3ArtifactRepository(posixpath.join(s3_artifact_root, "some/path"))
...
def test_get_download_presigned_url_missing_artifact(s3_artifact_root):
    ...
    with pytest.raises(MLForgeException, match="Not Found") as exc_info:

...
from MLForge.entities import FileInfo
from MLForge.entities.model_registry import ModelVersion
from MLForge.environment_variables import MLForge_MULTIPART_DOWNLOAD_CHUNK_SIZE
from MLForge.exceptions import MLForgeException
from MLForge.store.artifact.databricks_models_artifact_repo import (
    DatabricksModelsArtifactRepository,
)
from MLForge.tracking._model_registry.client import ModelRegistryClient
from MLForge.utils.file_utils import _Chunk

DATABRICKS_MODEL_ARTIFACT_REPOSITORY_PACKAGE = (
    "MLForge.store.artifact.databricks_models_artifact_repo"
)
...
REGISTRY_LIST_ARTIFACTS_ENDPOINT = "/api/2.0/MLForge/model-versions/list-artifacts"
REGISTRY_ARTIFACT_PRESIGNED_URI_ENDPOINT = "/api/2.0/MLForge/model-versions/get-signed-download-uri"
...
@pytest.mark.parametrize(
    ...
    [
        "s3://test",
        "dbfs:/databricks/MLForge/MV-id/models",
        "dbfs://scope:key@notdatabricks/databricks/MLForge-regisry/123/models",
        "models:/MyModel/12",
...
def test_init_with_version_uri_and_profile_is_inferred():
    ...
    with (
        mock.patch(
            "MLForge.store.artifact.utils.models.MLForge.get_registry_uri",
            return_value=MOCK_PROFILE,
        ),
        mock.patch("MLForge.tracking.get_registry_uri", return_value=MOCK_PROFILE),
    ):
...
def test_list_artifacts(databricks_model_artifact_repo):
    ...
    def _raise_for_status():
        if status_code == 404:
            raise Exception(
                "404 Client Error: Not Found for url: https://shard-uri/api/2.0/MLForge/model-versions/list-artifacts?name=model&version=1"
            )
...
def test_parallelized_download_file_using_http_uri_success(
    ...
    with (
        mock.patch(
            ...
            return_value=[
                FileInfo(remote_file_path, True, MLForge_MULTIPART_DOWNLOAD_CHUNK_SIZE.get() + 1)
            ],
        ),
        mock.patch(
        ...
        mock.patch(
            "MLForge.utils.databricks_utils.get_databricks_env_vars",
            return_value={},
        ),
        mock.patch(
            DATABRICKS_MODEL_ARTIFACT_REPOSITORY_PACKAGE
...
def test_parallelized_download_file_using_http_uri_with_error_downloads(
    ...
    with (
        ...
        mock.patch(
            "MLForge.utils.file_utils.download_chunk", side_effect=Exception("Retry failed")
        ) as mock_download_chunk,
    ):
        with pytest.raises(MLForgeException, match="Retry failed"):
            databricks_model_artifact_repo._download_file(remote_file_path, "")
...
def test_parallelized_download_file_using_http_uri_with_failed_downloads(
    ...
    with (
        ...
        mock.patch(
            "MLForge.utils.file_utils.download_chunk",
            return_value=None,
...
def test_download_file_get_request_fail(databricks_model_artifact_repo):
    with mock.patch(DATABRICKS_MODEL_ARTIFACT_REPOSITORY + "._call_endpoint") as call_endpoint_mock:
        call_endpoint_mock.side_effect = MLForgeException("MOCK ERROR")
...
def test_log_artifact_fail(databricks_model_artifact_repo):
    with pytest.raises(MLForgeException, match="This repository does not support logging artifacts"):

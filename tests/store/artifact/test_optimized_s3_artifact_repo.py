...
S3_REPOSITORY_MODULE = "MLForge.store.artifact.optimized_s3_artifact_repo"
...
def test_get_region_name_client_error_without_region_header():
    ...
    with mock.patch("boto3.client") as mock_get_s3_client:
        ...
        with pytest.raises(MLForgeException, match="AWS_DEFAULT_REGION"):
...
def test_refresh_credentials():
    with (
        mock.patch(
            "MLForge.store.artifact.optimized_s3_artifact_repo._get_s3_client"
        ) as mock_get_s3_client,
        mock.patch(
            "MLForge.store.artifact.optimized_s3_artifact_repo.OptimizedS3ArtifactRepository._get_region_name"

...
from MLForge.store.workspace.rest_store import WORKSPACES_ENDPOINT, RestWorkspaceStore
...
def test_list_workspaces_parses_response(store, host_creds):
    ...
    with mock.patch(
        "MLForge.store.workspace.rest_store.call_endpoint", return_value=response
    ) as call_endpoint:
...
def test_create_workspace_conflict_raises_resource_exists(store, monkeypatch):
    ...
    monkeypatch.setattr(
        "MLForge.store.workspace.rest_store.call_endpoint",
...
def test_rest_store_validates_workspace_names_before_http(monkeypatch, store):
    mock_call = mock.Mock()
    monkeypatch.setattr("MLForge.store.workspace.rest_store.call_endpoint", mock_call)

    with pytest.raises(MLForgeException, match="must match the pattern"):

...
from MLForge.entities.workspace import Workspace, WorkspaceDeletionMode
from MLForge.exceptions import MLForgeException
from MLForge.store.artifact.artifact_repo import ArtifactRepository
from MLForge.store.workspace.dbmodels.models import SqlWorkspace
from MLForge.store.workspace.sqlalchemy_store import SqlAlchemyStore
from MLForge.utils.workspace_utils import DEFAULT_WORKSPACE_NAME


@pytest.fixture
def workspace_store(db_uri, monkeypatch):
    monkeypatch.setenv("MLForge_ENABLE_WORKSPACES", "true")
...
def test_get_workspace_not_found(workspace_store):
    with pytest.raises(MLForgeException, match="Workspace 'unknown' not found") as exc:
...
def test_create_workspace_invalid_trace_archival_location_raises(workspace_store):
    with pytest.raises(MLForgeException, match="proxy-only `MLForge-artifacts:` scheme") as exc:
        workspace_store.create_workspace(
            Workspace(
                name="team-a",
                description=None,
                trace_archival_location="MLForge-artifacts:/archive/team-a",
            )
        )
    assert exc.value.error_code == "INVALID_PARAMETER_VALUE"
...
def test_create_workspace_invalid_trace_archival_retention_raises(workspace_store):
    with pytest.raises(MLForgeException, match="Trace archival retention must") as exc:
        workspace_store.create_workspace(
            Workspace(
                name="team-a",
                description=None,
...
def test_update_workspace_invalid_trace_archival_location_raises(workspace_store):
    ...
    with pytest.raises(MLForgeException, match="proxy-only `MLForge-artifacts:` scheme") as exc:
        workspace_store.update_workspace(
            Workspace(name="team-a", trace_archival_location="MLForge-artifacts:/archive/team-a")
        )
    assert exc.value.error_code == "INVALID_PARAMETER_VALUE"
...
def test_delete_workspace_fails_on_naming_conflict(workspace_store):
    ...
    with pytest.raises(MLForgeException, match="already exist in the default workspace") as exc:
        workspace_store.delete_workspace("team-a", mode=WorkspaceDeletionMode.SET_DEFAULT)
    assert exc.value.error_code == "INVALID_STATE"
...
def test_delete_workspace_restrict_blocks_when_resources_exist(workspace_store):
    ...
    with pytest.raises(MLForgeException, match="still contains") as exc:

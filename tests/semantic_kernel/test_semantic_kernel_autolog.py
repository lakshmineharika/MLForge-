...
import MLForge.semantic_kernel
from MLForge.entities import SpanType
from MLForge.entities.span_status import SpanStatusCode
from MLForge.environment_variables import MLForge_USE_DEFAULT_TRACER_PROVIDER
from MLForge.semantic_kernel.autolog import SemanticKernelSpanProcessor
...
def with_openai_autolog(request):
    ...
    if request.param:
        ...
        MLForge.openai.autolog(disable=True)
...
async def test_sk_invoke_simple(mock_openai, with_openai_autolog, mock_litellm_cost):
    MLForge.semantic_kernel.autolog()
...
async def test_tracing_autolog_with_active_span(mock_openai, with_openai_autolog):
    ...
    with MLForge.start_span("parent"):
    ...
    assert trace.info.status == "OK"
    assert trace.info.tags["MLForge.traceName"] == "parent"

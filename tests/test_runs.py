...
import MLForge
from MLForge import experiments
from MLForge.exceptions import MLForgeException
from MLForge.runs import create_run, link_traces, list_run
from MLForge.tracing.constant import TraceExperimentTagKey
...
def test_list_run():
    with MLForge.start_run(run_name="apple"):
        pass
...
def test_csv_generation(tmp_path):
    ...
    with mock.patch(
        "MLForge.experiments.fluent.search_runs",
...
def test_create_run_with_experiment_id():
    MLForge.create_experiment("test_create_run_exp")
    exp = MLForge.get_experiment_by_name("test_create_run_exp")
    ...
    run = MLForge.get_run(output["run_id"])
...
def test_create_run_with_experiment_name():
    ...
    exp = MLForge.get_experiment_by_name(exp_name)
    assert exp is not None
    assert output["experiment_id"] == exp.experiment_id


def test_create_run_with_custom_name_and_description():
    MLForge.create_experiment("test_run_with_details")
    exp = MLForge.get_experiment_by_name("test_run_with_details")
    ...
    # Verify run details
    run = MLForge.get_run(output["run_id"])
    assert run.data.tags.get("MLForge.note.content") == description
    assert run.info.run_name == run_name


def test_create_run_with_tags():
    MLForge.create_experiment("test_run_with_tags")
    exp = MLForge.get_experiment_by_name("test_run_with_tags")
...
def test_create_run_with_different_status(status):
    MLForge.create_experiment("test_run_statuses")
    exp = MLForge.get_experiment_by_name("test_run_statuses")
...
def test_create_run_invalid_tag_format():
    MLForge.create_experiment("test_invalid_tag")
    exp = MLForge.get_experiment_by_name("test_invalid_tag")
...
def test_create_run_duplicate_tag_key():
    MLForge.create_experiment("test_duplicate_tag")
    exp = MLForge.get_experiment_by_name("test_duplicate_tag")
...
def test_link_traces_single_trace():
    with patch("MLForge.runs.MLForgeClient.link_traces_to_run") as mock_link_traces:
        result = CliRunner().invoke(
            link_traces,
...
def test_link_traces_file_store_error():
    with patch(
        "MLForge.runs.MLForgeClient.link_traces_to_run",
...
def test_link_traces_generic_error():
    with patch(
        "MLForge.runs.MLForgeClient.link_traces_to_run",
        side_effect=MLForgeException("Some other error"),
    ):
        result = CliRunner().invoke(
            link_traces,
            ["--run-id", "test_run", "--trace-id", "trace_1"],
...
def test_get_experiment_json():
    exp_id = MLForge.create_experiment("test_get_exp_json", tags={"env": "test"})
    exp = MLForge.get_experiment(exp_id)
...
def test_get_experiment_table():
    exp_id = MLForge.create_experiment("test_get_exp_table", tags={"env": "test", "team": "ml"})
...
def test_get_experiment_table_no_tags():
    exp_id = MLForge.create_experiment("test_get_exp_no_tags")
...
def test_get_experiment_deleted():
    exp_id = MLForge.create_experiment("test_deleted")
...
def test_get_experiment_by_name_table():
    ...
    exp_id = MLForge.create_experiment(exp_name, tags={"env": "test"})
...
def test_get_experiment_by_name_json():
    ...
    exp_id = MLForge.create_experiment(exp_name, tags={"team": "ml"})
    exp = MLForge.get_experiment(exp_id)
...
def test_create_experiment_with_trace_archival_retention():
    ...
    with patch("MLForge.experiments._get_store", return_value=store):
        result = CliRunner().invoke(
            experiments.create,
            [
                "--experiment-name",

...
def test_error_code_constructor():
    assert (
        MLForgeException("test", error_code=INVALID_PARAMETER_VALUE).error_code
        == "INVALID_PARAMETER_VALUE"
    )


def test_default_error_code():
    assert MLForgeException("test").error_code == "INTERNAL_ERROR"


def test_serialize_to_json():
    MLForge_exception = MLForgeException("test")
    deserialized = json.loads(MLForge_exception.serialize_as_json())
    assert deserialized["message"] == "test"
    assert deserialized["error_code"] == "INTERNAL_ERROR"


def test_get_http_status_code():
    assert MLForgeException("test default").get_http_status_code() == 500
    assert MLForgeException("code not in map", error_code=IO_ERROR).get_http_status_code() == 500
    assert MLForgeException("test", error_code=INVALID_STATE).get_http_status_code() == 500
    assert MLForgeException("test", error_code=ENDPOINT_NOT_FOUND).get_http_status_code() == 404
    assert MLForgeException("test", error_code=INVALID_PARAMETER_VALUE).get_http_status_code() == 400
    assert MLForgeException("test", error_code=INTERNAL_ERROR).get_http_status_code() == 500
    assert MLForgeException("test", error_code=RESOURCE_ALREADY_EXISTS).get_http_status_code() == 400


def test_invalid_parameter_value():
    MLForge_exception = MLForgeException.invalid_parameter_value("test")
    assert MLForge_exception.error_code == "INVALID_PARAMETER_VALUE"


def test_rest_exception():
    MLForge_exception = MLForgeException("test", error_code=RESOURCE_ALREADY_EXISTS)
    json_exception = MLForge_exception.serialize_as_json()
...
def test_sqlstate_serialize_as_json_includes_explicit():
    exc = MLForgeException("test", sqlstate="KAM01", error_class="SCHEMA_ENFORCEMENT_FAILED")
    deserialized = json.loads(exc.serialize_as_json())
...
def test_sqlstate_none_for_unknown_error_code():
    exc = MLForgeException("test", error_code=IO_ERROR)
...
def test_invalid_parameter_value_auto_derives_sqlstate():
    exc = MLForgeException.invalid_parameter_value("bad input")
...
def test_invalid_parameter_value_with_explicit_override():
    exc = MLForgeException.invalid_parameter_value(

...
from MLForge.utils.crypto import (
    AES_256_KEY_LENGTH,
    GCM_NONCE_LENGTH,
    KEKManager,
    _create_aad,
...
def test_kek_manager_from_env_var(monkeypatch):
    monkeypatch.setenv("MLForge_CRYPTO_KEK_PASSPHRASE", "env-passphrase")
...
def test_kek_manager_empty_passphrase_uses_default(monkeypatch):
    monkeypatch.setenv("MLForge_CRYPTO_KEK_PASSPHRASE", "")
    kek_manager = KEKManager()
    assert kek_manager.using_default_passphrase is True
    assert kek_manager.get_kek() is not None
...
def test_kek_manager_version_from_env_var(monkeypatch):
    monkeypatch.setenv("MLForge_CRYPTO_KEK_PASSPHRASE", "env-passphrase")
    monkeypatch.setenv("MLForge_CRYPTO_KEK_VERSION", "3")
    kek_manager = KEKManager()
...
def test_decrypt_with_wrong_aad_fails():
    ...
    with pytest.raises(MLForgeException, match="AES-GCM decryption failed"):
...
def test_unwrap_with_wrong_kek_fails():
    ...
    with pytest.raises(MLForgeException, match="Failed to unwrap DEK"):
...
def test_decrypt_with_wrong_secret_id_fails(kek_manager):
    ...
    with pytest.raises(MLForgeException, match="Failed to decrypt secret"):
        _decrypt_secret(
            result.encrypted_value,
            result.wrapped_dek,
            kek_manager,
...
def test_rotate_with_wrong_old_kek_fails():
    ...
    with pytest.raises(MLForgeException, match="Failed to rotate secret encryption"):
        rotate_secret_encryption(

...
from MLForge.entities import Experiment, Metric
from MLForge.entities.model_registry import ModelVersion, RegisteredModel
from MLForge.exceptions import MLForgeException
from MLForge.protos.model_registry_pb2 import RegisteredModel as ProtoRegisteredModel
from MLForge.protos.service_pb2 import Experiment as ProtoExperiment
from MLForge.protos.service_pb2 import Metric as ProtoMetric
from MLForge.types import ColSpec, DataType, Schema, TensorSpec
from MLForge.types.schema import Array, Map, Object, Property
from MLForge.types.utils import _infer_schema
from MLForge.utils.proto_json_utils import (
    MLForgeFailedTypeConversion,
    _CustomJsonEncoder,
...
def test_message_to_json():
    ...
    test_message = ParseTextIntoProto(
        ...
        oneof1: 207
        [MLForge.ExtensionMessage.field_extended_int64]: 100
...
def test_parse_tf_serving_raises_expected_errors():
    ...
    with pytest.raises(
        MLForgeException, match="The length of values for each input/column name are not the same"
    ):
...
def test_cast_df_types_according_to_schema_error_message(dataframe, schema, error_message):
    with pytest.raises(MLForgeFailedTypeConversion, match=error_message):

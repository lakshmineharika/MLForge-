...
from MLForge.deployments.databricks import DatabricksDeploymentClient
from MLForge.environment_variables import (
    _MLForge_DATABRICKS_TRAFFIC_ID,
    MLForge_HTTP_REQUEST_TIMEOUT,
)
from MLForge.exceptions import InvalidUrlException, MLForgeException, RestException
from MLForge.protos.databricks_pb2 import ENDPOINT_NOT_FOUND, ErrorCode
from MLForge.protos.service_pb2 import GetRun
from MLForge.pyfunc.scoring_server import NumpyEncoder
...
from MLForge.utils.workspace_utils import WORKSPACE_HEADER_NAME
...
def test_malformed_json_error_response(response_mock):
    with mock.patch("requests.Session.request", return_value=response_mock):
        host_only = MLForgeHostCreds("http://my-host")

        response_proto = GetRun.Response()
        with pytest.raises(
            MLForgeException, match="API request to endpoint /my/endpoint failed with error code 400"
        ):
            call_endpoint(host_only, "/my/endpoint", "GET", None, response_proto)


def test_call_endpoints():
    with mock.patch("MLForge.utils.rest_utils.call_endpoint") as mock_call_endpoint:
        response_proto = GetRun.Response()
...
def test_http_request_includes_workspace_header_for_MLForge_routes():
    host_only = MLForgeHostCreds("http://my-host")
    response = mock.MagicMock()
    response.status_code = 200
    with WorkspaceContext("team-a"):
        with mock.patch("requests.Session.request", return_value=response) as mock_request:
            http_request(host_only, "/api/2.0/MLForge/runs/search", "GET")
...
def test_http_request_omits_workspace_header_for_workspace_admin_routes():
    ...
    with WorkspaceContext("team-a"):
        with mock.patch("requests.Session.request", return_value=response) as mock_request:
            http_request(host_only, "/api/3.0/MLForge/workspaces/team-a", "GET")
        headers = mock_request.call_args.kwargs["headers"]
...
def test_http_request_with_basic_auth():
    host_only = MLForgeHostCreds("http://my-host", username="user", password="pass")
    response = mock.MagicMock()
    response.status_code = 200
    with mock.patch("requests.Session.request", return_value=response) as mock_request:
        http_request(host_only, "/my/endpoint", "GET")
...
def test_http_request_with_aws_sigv4(monkeypatch):
    ...
    aws_sigv4 = MLForgeHostCreds("http://my-host", aws_sigv4=True)
    response = mock.MagicMock()
    response.status_code = 200
...
def test_http_request_with_auth():
    ...
    auth = "test_auth_name"
    host_only = MLForgeHostCreds("http://my-host", auth=auth)
    response = mock.MagicMock()
    response.status_code = 200
    with (
        mock.patch("requests.Session.request", return_value=response) as mock_request,
        mock.patch(
            "MLForge.tracking.request_auth.registry.fetch_auth", return_value=mock_fetch_auth
...
def test_http_request_with_token():
    host_only = MLForgeHostCreds("http://my-host", token="my-token")
    response = mock.MagicMock()
    response.status_code = 200
    with mock.patch("requests.Session.request", return_value=response) as mock_request:
        http_request(host_only, "/my/endpoint", "GET")
...
def test_http_request_with_insecure():
    host_only = MLForgeHostCreds("http://my-host", ignore_tls_verification=True)
    response = mock.MagicMock()
    response.status_code = 200
    with mock.patch("requests.Session.request", return_value=response) as mock_request:
        http_request(host_only, "/my/endpoint", "GET")
...
def test_http_request_client_cert_path():
    host_only = MLForgeHostCreds("http://my-host", client_cert_path="/some/path")
    response = mock.MagicMock()
    response.status_code = 200
    with mock.patch("requests.Session.request", return_value=response) as mock_request:
        http_request(host_only, "/my/endpoint", "GET")
...
def test_http_request_server_cert_path():
    host_only = MLForgeHostCreds("http://my-host", server_cert_path="/some/path")
    response = mock.MagicMock()
    response.status_code = 200
    with mock.patch("requests.Session.request", return_value=response) as mock_request:
        http_request(host_only, "/my/endpoint", "GET")
...
def test_http_request_request_headers():
    from MLForge_test_plugin.request_header_provider import PluginRequestHeaderProvider
...
def test_http_request_with_invalid_url_raise_MLForge_exception():
    host_only = MLForgeHostCreds("http://my-host")

    with pytest.raises(MLForgeException, match="Invalid url: http://my-host/invalid_url"):
        with mock.patch("requests.Session.request", side_effect=requests.exceptions.InvalidURL):
            http_request(host_only, "/invalid_url", "GET")
...
def test_http_request_wrapper():
    ...
    with mock.patch("requests.Session.request", return_value=response) as mock_request:
        ...
        response.text = ""
        with pytest.raises(MLForgeException, match="Response body"):
            http_request_safe(host_only, "/my/endpoint", "GET")
        response.text = (
...
def test_http_request_customize_config(monkeypatch):
    with mock.patch(
        "MLForge.utils.rest_utils._get_http_response_with_retries"
    ) as mock_get_http_response_with_retries:
        host_only = MLForgeHostCreds("http://my-host")
        monkeypatch.delenv("MLForge_HTTP_REQUEST_MAX_RETRIES", raising=False)
        monkeypatch.delenv("MLForge_HTTP_REQUEST_BACKOFF_FACTOR", raising=False)
        monkeypatch.delenv("MLForge_HTTP_REQUEST_TIMEOUT", raising=False)
        monkeypatch.delenv("MLForge_HTTP_RESPECT_RETRY_AFTER_HEADER", raising=False)
        http_request(host_only, "/my/endpoint", "GET")
        ...
        monkeypatch.setenv("MLForge_HTTP_REQUEST_MAX_RETRIES", "8")
        monkeypatch.setenv("MLForge_HTTP_REQUEST_BACKOFF_FACTOR", "3")
        monkeypatch.setenv("MLForge_HTTP_REQUEST_BACKOFF_JITTER", "1.0")
        monkeypatch.setenv("MLForge_HTTP_REQUEST_TIMEOUT", "300")
        monkeypatch.setenv("MLForge_HTTP_RESPECT_RETRY_AFTER_HEADER", "false")
        http_request(host_only, "/my/endpoint", "GET")
        mock_get_http_response_with_retries.assert_called_with(
            mock.ANY,
            mock.ANY,
    ...
    with mock.patch("requests.Session.request", side_effect=requests.exceptions.Timeout):
        with pytest.raises(
            ...
            match=(
                ...
                + re.escape(str(MLForge_HTTP_REQUEST_TIMEOUT))
            ),
        ):
            http_request(MLForgeHostCreds("http://my-host"), "/my/endpoint", "GET")
...
def test_augmented_raise_for_status():
    ...
    with mock.patch("requests.Session.request", return_value=response) as mock_request:
        response = requests.get("https://github.com/MLForge/MLForge.git")
...
def test_http_request_max_retries(monkeypatch):
    monkeypatch.setenv("_MLForge_HTTP_REQUEST_MAX_RETRIES_LIMIT", "15")
    host_creds = MLForgeHostCreds("http://example.com")

    with mock.patch("requests.Session.request") as mock_request:
        # Value exceeding limit should raise
        with pytest.raises(MLForgeException, match="The configured max_retries"):
...
def test_http_request_backoff_factor(monkeypatch):
    monkeypatch.setenv("_MLForge_HTTP_REQUEST_MAX_BACKOFF_FACTOR_LIMIT", "200")
    host_creds = MLForgeHostCreds("http://example.com")

    with mock.patch("requests.Session.request") as mock_request:
        # Value exceeding limit should raise
        with pytest.raises(MLForgeException, match="The configured backoff_factor"):
...
def test_http_request_negative_max_retries():
    ...
    with mock.patch("requests.Session.request") as mock_request:
        with pytest.raises(MLForgeException, match="The max_retries value must be either"):
...
def test_http_request_negative_backoff_factor():
    ...
    with mock.patch("requests.Session.request") as mock_request:
        with pytest.raises(MLForgeException, match="The backoff_factor value must be"):
            http_request(host_creds, "/endpoint", "GET", backoff_factor=-1)
        mock_request.assert_not_called()


def test_suppress_databricks_retry_after_secs_warnings():
    host_creds = MLForgeHostCreds("http://example.com", use_databricks_sdk=True)
    ...
    with (
        ...
        mock.patch("MLForge.utils.rest_utils.get_workspace_client") as mock_get_workspace_client,
    ):
...
def test_databricks_sdk_retry_on_transient_errors():
    ...
    def mock_do_failing_then_success(*args, **kwargs):
        ...
        if call_count <= 2:  # Fail first 2 attempts
            ...
            from MLForge.protos.databricks_pb2 import ErrorCode
    ...
    with mock.patch("MLForge.utils.rest_utils.get_workspace_client") as mock_get_workspace_client:
        mock_workspace_client = mock.MagicMock()
...
def test_databricks_sdk_retry_max_retries_exceeded():
    ...
    with (
        mock.patch("MLForge.utils.rest_utils.get_workspace_client") as mock_get_workspace_client,
        mock.patch("MLForge.utils.rest_utils._logger") as mock_logger,
    ):
        mock_workspace_client = mock.MagicMock()
...
def test_databricks_sdk_retry_backoff_calculation():
    ...
    from MLForge.utils.request_utils import _TRANSIENT_FAILURE_RESPONSE_CODES
    from MLForge.utils.rest_utils import _retry_databricks_sdk_call_with_exponential_backoff
    ...
    with mock.patch("MLForge.utils.rest_utils._time_sleep") as mock_sleep:
...
def test_deployment_client_timeout_propagation(monkeypatch):
    with (
        mock.patch("MLForge.utils.rest_utils.get_workspace_client") as mock_get_workspace_client,
        mock.patch(
            "MLForge.utils.databricks_utils.get_databricks_host_creds"
        ) as mock_get_databricks_host_creds,
        mock.patch(
            "MLForge.deployments.databricks.get_databricks_host_creds"
        ) as mock_deployment_host_creds,
    ):
        # Mock the host creds to use Databricks SDK
        mock_host_creds = MLForgeHostCreds("http://my-host", use_databricks_sdk=True)
        ...
        monkeypatch.setenv("MLForge_DEPLOYMENT_PREDICT_TIMEOUT", "300")
        ...
        mock_get_workspace_client.assert_called_once_with(
            ...
            timeout=300,  # MLForge_DEPLOYMENT_PREDICT_TIMEOUT value
        )
...
def test_http_request_with_databricks_traffic_id(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv(_MLForge_DATABRICKS_TRAFFIC_ID.name, "test-traffic-id-12345")
    with mock.patch("requests.Session.request", return_value=response) as mock_request:
        http_request(MLForgeHostCreds("http://my-host"), "/my/endpoint", "GET")
        headers = mock_request.call_args.kwargs["headers"]
    ...
    monkeypatch.delenv(_MLForge_DATABRICKS_TRAFFIC_ID.name)
    with mock.patch("requests.Session.request", return_value=response) as mock_request:
        http_request(MLForgeHostCreds("http://my-host"), "/my/endpoint", "GET")
        headers = mock_request.call_args.kwargs["headers"]
...
def test_http_request_with_workspace_id():
    ...
    creds = MLForgeHostCreds("http://my-host", workspace_id="6051921418418893")
    with mock.patch("requests.Session.request", return_value=response) as mock_request:
    ...
    creds = MLForgeHostCreds("http://my-host")
    with mock.patch("requests.Session.request", return_value=response) as mock_request:
        http_request(creds, "/my/endpoint", "GET")
        headers = mock_request.call_args.kwargs["headers"]
...
def test_MLForge_host_creds_workspace_id_equality():
    creds1 = MLForgeHostCreds("http://my-host", workspace_id="123")
    creds2 = MLForgeHostCreds("http://my-host", workspace_id="123")
    creds3 = MLForgeHostCreds("http://my-host", workspace_id="456")
    creds4 = MLForgeHostCreds("http://my-host")
...
def test_oauth_error_when_sdk_enabled(monkeypatch):
    # When MLForge_ENABLE_DB_SDK=true and the SDK failed to initialize, the OAuth error
    ...
    monkeypatch.setenv("MLForge_ENABLE_DB_SDK", "true")
    creds = MLForgeHostCreds(
        "http://my-host",
        client_id="some-id",
        client_secret="some-secret",
    )
    with pytest.raises(MLForgeException, match="SDK is enabled but failed to initialize") as exc:
        http_request(creds, "/my/endpoint", "GET")
    msg = str(exc.value)
    assert "MLForge_ENABLE_DB_SDK is currently set to 'True'" in msg
    assert "Failed to create databricks SDK workspace client" in msg


def test_oauth_error_when_sdk_disabled(monkeypatch):
    monkeypatch.setenv("MLForge_ENABLE_DB_SDK", "false")
    creds = MLForgeHostCreds(
        "http://my-host",
        client_id="some-id",
        client_secret="some-secret",
    )
    with pytest.raises(MLForgeException, match="Set 'MLForge_ENABLE_DB_SDK' to true") as exc:
        http_request(creds, "/my/endpoint", "GET")
    msg = str(exc.value)
    assert "MLForge_ENABLE_DB_SDK is currently set to 'False'" in msg
...
def test_validate_deployment_timeout_config(timeout, retry_timeout_seconds, should_warn):
    from MLForge.utils.rest_utils import validate_deployment_timeout_config
    ...
    if should_warn:
        with warnings.catch_warnings(record=True) as w:
            ...
            assert "MLForge_DEPLOYMENT_PREDICT_TOTAL_TIMEOUT" in warning_msg

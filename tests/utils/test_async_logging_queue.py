...
import MLForge.utils.async_logging.async_logging_queue
...
from MLForge.utils.async_logging.async_logging_queue import AsyncLoggingQueue, QueueStatus
...
class RunData:
    ...
    def consume_queue_data(self, run_id, metrics, tags, params):
        ...
        if self.batch_count in self.throw_exception_on_batch_number:
            raise MLForgeException("Failed to log run data")
...
def test_single_thread_publish_consume_queue(monkeypatch):
    monkeypatch.setenv("MLForge_ASYNC_LOGGING_BUFFERING_SECONDS", "3")
...
def test_queue_activation():
    ...
    with generate_async_logging_queue(run_data) as async_logging_queue:
        ...
        with pytest.raises(MLForgeException, match="AsyncLoggingQueue is not activated."):
...
def test_partial_logging_failed():
    ...
    with generate_async_logging_queue(run_data) as async_logging_queue:
        ...
        for params, tags, metrics in _get_run_data():
            if batch_id in [3, 4]:
                with pytest.raises(MLForgeException, match="Failed to log run data"):
                    async_logging_queue.log_batch_async(
                        run_id=run_id, metrics=metrics, tags=tags, params=params
                    ).wait()
            else:
...
def test_batch_split(monkeypatch):
    monkeypatch.setattr(MLForge.utils.async_logging.async_logging_queue, "_MAX_ITEMS_PER_BATCH", 10)
    monkeypatch.setattr(MLForge.utils.async_logging.async_logging_queue, "_MAX_PARAMS_PER_BATCH", 6)
    monkeypatch.setattr(MLForge.utils.async_logging.async_logging_queue, "_MAX_TAGS_PER_BATCH", 8)

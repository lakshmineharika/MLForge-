...
from MLForge.utils import request_utils
from MLForge.utils.request_utils import (
    TCPKeepAliveHTTPAdapter,
    _build_socket_options,
)


def test_request_utils_does_not_import_MLForge(tmp_path):
    file_content = f"""
...
module_name = "MLForge.utils.request_utils"
...
assert "MLForge" not in sys.modules
assert "MLForge.utils.request_utils" in sys.modules
"""
    test_file = tmp_path.joinpath("test_request_utils_does_not_import_MLForge.py")
...
def test_redirects_disabled_if_env_var_set(monkeypatch, env_value):
    monkeypatch.setenv("MLForge_ALLOW_HTTP_REDIRECTS", env_value)
...
def test_build_socket_options_disabled_via_env(monkeypatch):
    monkeypatch.setenv("MLForge_HTTP_TCP_KEEPALIVE", "false")
    options = _build_socket_options()
...
def test_build_socket_options_custom_values_via_env(monkeypatch):
    monkeypatch.setenv("MLForge_HTTP_TCP_KEEPALIVE_IDLE", "60")
    monkeypatch.setenv("MLForge_HTTP_TCP_KEEPALIVE_INTERVAL", "20")
    monkeypatch.setenv("MLForge_HTTP_TCP_KEEPALIVE_COUNT", "5")
    options = _build_socket_options()
    if hasattr(socket, "TCP_KEEPIDLE"):

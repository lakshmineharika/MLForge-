...
from MLForge.legacy_databricks_cli.configure.provider import (
    DatabricksConfig,
    DatabricksModelServingConfigProvider,
)
from MLForge.utils import databricks_utils
from MLForge.utils.databricks_utils import (
    ...
    get_MLForge_credential_context_by_run_id,
    get_sgc_job_run_id,
...
def test_databricks_registry_profile():
    ...
    with (
        mock.patch(
            "MLForge.utils.databricks_utils.ProfileConfigProvider", return_value=mock_provider
        ),
        mock.patch("MLForge.utils.databricks_utils._get_dbutils", return_value=mock_dbutils),
    ):
...
def test_databricks_no_creds_found():
    with pytest.raises(MLForgeException, match="Reading Databricks credential configuration failed"):
...
def test_databricks_host_creds_with_profile_uses_sdk(monkeypatch):
    ...
    with (
        ...
        mock.patch(
            "MLForge.utils.databricks_utils._get_databricks_creds_config", return_value=mock_config
        ),
    ):
        creds = databricks_utils.get_databricks_host_creds("databricks")
...
def test_databricks_host_creds_oidc_only_no_legacy_token(monkeypatch):
    ...
    # must still be returned so MLForge can route requests through the SDK.
    monkeypatch.setenv("MLForge_ENABLE_DB_SDK", "true")
    ...
    with (
        ...
        mock.patch(
            "MLForge.utils.databricks_utils._get_databricks_creds_config",
            side_effect=MLForgeException("malformed databricks auth"),
        ),
    ):
        creds = databricks_utils.get_databricks_host_creds("databricks")
...
def test_databricks_host_creds_legacy_failure_without_sdk_reraises(monkeypatch):
    # When SDK auth failed AND legacy auth is malformed, the legacy MLForgeException must
    ...
    with (
        ...
        pytest.raises(MLForgeException, match="malformed databricks auth"),
    ):
...
def test_databricks_host_creds_keeps_sdk_when_workspace_id_attr_missing(monkeypatch):
    ...
    # for users who had MLForge_ENABLE_DB_SDK=true correctly set. workspace_id must
...
def test_databricks_no_creds_found_in_model_serving(monkeypatch):
    ...
    with pytest.raises(
        MLForgeException, match="Reading Databricks credential configuration in model serving failed"
    ):
        databricks_utils.get_databricks_host_creds()


def test_databricks_single_slash_in_uri_scheme_throws():
    with pytest.raises(MLForgeException, match="URI is formatted incorrectly"):
...
def test_get_model_dependency_token(oauth_file):
    with mock.patch(
        "MLForge.utils.databricks_utils._MODEL_DEPENDENCY_OAUTH_TOKEN_FILE_PATH", str(oauth_file)
    ):
...
def test_get_model_dependency_oauth_token_model_serving_throws():
    with pytest.raises(MLForgeException, match="Unable to read Oauth credentials"):
...
def test_get_workspace_info_from_databricks_secrets():
    ...
    with mock.patch("MLForge.utils.databricks_utils._get_dbutils", return_value=mock_dbutils):
...
def test_get_workspace_info_from_dbutils():
    ...
    mock_method_chain(
        mock_dbutils, methods + ["browserHostName", "get"], return_value="MLForge.databricks.com"
    )
    ...
    with mock.patch("MLForge.utils.databricks_utils._get_dbutils", return_value=mock_dbutils):
        ...
        assert workspace_host == "https://MLForge.databricks.com"
...
def test_get_workspace_info_from_dbutils_no_browser_host_name():
    ...
    mock_method_chain(
        mock_dbutils, methods + ["apiUrl", "get"], return_value="https://MLForge.databricks.com"
    )
    mock_method_chain(mock_dbutils, methods + ["workspaceId", "get"], return_value="1111")
    with mock.patch("MLForge.utils.databricks_utils._get_dbutils", return_value=mock_dbutils):
        workspace_host, workspace_id = get_workspace_info_from_dbutils()
...
def test_get_workspace_info_from_dbutils_old_runtimes():
    ...
    mock_method_chain(
        mock_dbutils,
        methods + ["toJson", "get"],
        return_value='{"tags": {"orgId" : "1111", "browserHostName": "MLForge.databricks.com"}}',
    )
    mock_method_chain(
        mock_dbutils, methods + ["browserHostName", "get"], return_value="MLForge.databricks.com"
    )
...
def test_get_workspace_info_from_dbutils_when_no_dbutils_available():
    with mock.patch("MLForge.utils.databricks_utils._get_dbutils", return_value=None):
        workspace_host, workspace_id = get_workspace_info_from_dbutils()
...
def test_get_repl_id():
    ...
    with mock.patch(
        "MLForge.utils.databricks_utils._get_runtime_integration_client",
...
def test_use_repl_context_if_available(tmp_path, monkeypatch):
    ...
    with mock.patch(
        "MLForge.utils.databricks_utils._get_command_context", return_value=command_context_mock
...
"""
    ...
    with (
        ...
        mock.patch("MLForge.utils.databricks_utils._get_dbutils") as mock_dbutils,
    ):
        assert databricks_utils.get_job_id() == "job_id"
    ...
    with (
        ...
        mock.patch(
            "MLForge.utils.databricks_utils._get_property_from_spark_context"
    ...
    with (
        ...
        mock.patch("MLForge.utils._spark_utils._get_active_spark_session") as mock_spark_session,
    ):
...
def test_get_MLForge_credential_context_by_run_id():
    with (
        mock.patch(
            "MLForge.tracking.artifact_utils.get_artifact_uri", return_value="dbfs:/path/to/artifact"
        ) as mock_get_artifact_uri,
        mock.patch(
            "MLForge.utils.uri.get_databricks_profile_uri_from_artifact_uri",
            return_value="databricks://path/to/profile",
        ) as mock_get_databricks_profile,
        mock.patch(
            "MLForge.utils.databricks_utils.MLForgeCredentialContext"
        ) as mock_credential_context,
    ):
        get_MLForge_credential_context_by_run_id(run_id="abc")
...
def test_check_databricks_secret_scope_access_error():
    ...
    with (
        mock.patch("MLForge.utils.databricks_utils._get_dbutils", return_value=mock_dbutils),
        mock.patch("MLForge.utils.databricks_utils._logger.warning") as mock_warning,
    ):
        check_databricks_secret_scope_access("scope")
        mock_warning.assert_called_once_with(
            ...
            "https://MLForge.org/docs/latest/python_api/openai/index.html#credential-management-for-openai-on-databricks. "  # noqa: E501
...
def test_get_dbr_major_minor_version_throws_on_invalid_version_key(monkeypatch):
    ...
    with pytest.raises(MLForgeException, match="Failed to parse databricks runtime version"):
        get_databricks_runtime_major_minor_version()
...
def test_get_workspace_url(input_url, expected_result):
    with mock.patch("MLForge.utils.databricks_utils._get_workspace_url", return_value=input_url):
...
def test_get_dbconnect_udf_sandbox_info(spark, monkeypatch, dbr_version, expected_runtime_version):
    ...
    assert info.MLForge_version == MLForge.__version__
...
def test_construct_databricks_uc_registered_model_url():
    ...
    registered_model_name = "name.MLForge.echo_model"
    version = "6"
    workspace_id = "123"

    expected_url = (
        "https://databricks.com/explore/data/models/name/MLForge/echo_model/version/6?o=123"
    )
    ...
    expected_url_no_workspace = (
        "https://databricks.com/explore/data/models/name/MLForge/echo_model/version/6"
    )
...
def test_print_databricks_deployment_job_url():
    ...
    with (
        mock.patch("MLForge.utils.databricks_utils.eprint") as mock_eprint,
        mock.patch("MLForge.utils.databricks_utils.get_workspace_url", return_value=workspace_url),
    ):
        # Test case with a workspace ID
        with mock.patch(
            "MLForge.utils.databricks_utils.get_workspace_id", return_value=workspace_id
        ):
        ...
        with mock.patch("MLForge.utils.databricks_utils.get_workspace_id", return_value=None):
...
def test_get_databricks_workspace_client_config_with_tracking_uri_provider():
    ...
    with (
        mock.patch(
            "MLForge.utils.databricks_utils.get_db_info_from_uri",
        ...
        mock.patch("MLForge.utils.databricks_utils.TrackingURIConfigProvider") as mock_provider,
    ):
...
def test_get_databricks_workspace_client_config_env_profile(monkeypatch):
    ...
    with (
        mock.patch("MLForge.utils.databricks_utils.get_db_info_from_uri", return_value=(None, None)),
        mock.patch(
            "databricks.sdk.WorkspaceClient", return_value=mock_client_instance
        ) as mock_workspace_client,
    ):
...
def test_get_databricks_workspace_client_config_client_creation_error():
    with (
        mock.patch(
            "MLForge.utils.databricks_utils.get_db_info_from_uri", return_value=("profile", None)
        ),
        mock.patch(
...
def test_get_sgc_job_run_id_no_dbutils(monkeypatch):
    monkeypatch.delenv("SERVERLESS_GPU_COMPUTE_ASSOCIATED_JOB_RUN_ID", raising=False)
    with mock.patch("MLForge.utils.databricks_utils._get_dbutils", side_effect=_NoDbutilsError()):
        result = get_sgc_job_run_id()
...
def test_get_databricks_nfs_temp_dir():
    ...
    with (
        mock.patch("MLForge.utils.databricks_utils._get_dbutils", return_value=mock_dbutils2),
        mock.patch(
            "MLForge.utils.databricks_utils._get_runtime_integration_client",
            side_effect=Exception("unavailable"),
        ),

...
from MLForge.utils.file_utils import (
    TempDir,
    _copy_file_or_tree,
...
def test_shutil_copytree_without_file_permissions(tmp_path):
    ...
    MLForge.utils.file_utils.shutil_copytree_without_file_permissions(src_dir, dst_dir)
...
def test_check_tarfile_security(tmp_path):
    ...
    with pytest.raises(
        MLForgeException, match="Escaped path destination in the archive file is not allowed"
    ):
    ...
    with pytest.raises(
        MLForgeException, match="Absolute path destination in the archive file is not allowed"
    ):
    ...
    with pytest.raises(
        MLForgeException, match="Destination path in the archive file can not go through a symlink"
    ):
...
def test_extract_archive_to_dir_blocks_traversal(tmp_path):
    ...
    with pytest.raises(MLForgeException, match="Escaped path destination in the archive file"):
...
def test_safe_extractall_blocks_symlink_escape(tmp_path):
    ...
    """
    from MLForge.pyfunc.dbconnect_artifact_cache import _safe_extractall
    ...
    with tarfile.open(mal_tar, "r") as tar:
        with pytest.raises(MLForgeException, match="would be extracted outside"):

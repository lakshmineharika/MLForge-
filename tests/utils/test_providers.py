...
from MLForge.utils.providers import (
...
def test_load_provider_returns_models(monkeypatch):
    monkeypatch.setenv("MLForge_MODEL_CATALOG_URI", "")
...
def _mock_catalog(provider_data):
    ...
    return (
        mock.patch(
            "MLForge.utils.providers._load_provider",
            side_effect=lambda p: provider_data.get(p, {}),
        ),
        mock.patch(
            "MLForge.utils.providers._list_provider_names",
...
def mock_model_cost():
    with (
        mock.patch(
            "MLForge.utils.providers._load_provider", side_effect=_mock_load_provider
        ) as m_load,
        mock.patch(
            "MLForge.utils.providers._load_bundled_provider", side_effect=_mock_load_provider
        ),
        mock.patch(
            "MLForge.utils.providers._list_provider_names",
...
def test_cost_per_token_no_cache_cost_falls_back_to_input_rate():
    ...
    with (
        ...
        mock.patch(
            "MLForge.utils.providers._load_bundled_provider",
            side_effect=lambda p: no_cache_data.get(p, {}),
        ),
        mock.patch(
            "MLForge.utils.providers._list_provider_names",
...
def test_load_provider_uses_remote_when_available():
    ...
    with mock.patch(
        "MLForge.utils.providers._fetch_remote_provider", return_value=remote_data
    ) as mock_remote:
...
def test_load_provider_falls_back_to_bundled_when_remote_fails():
    with mock.patch(
        "MLForge.utils.providers._fetch_remote_provider", return_value=None
    ) as mock_remote:
        result = _load_provider("openai")
        mock_remote.assert_called_once_with("openai")
        assert len(result) > 0
...
def test_fetch_remote_provider_supports_file_url(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MLForge_MODEL_CATALOG_URI", tmp_path.as_uri())

...
from MLForge.utils.autologging_utils.logging_and_warnings import (
    _WarningsController,
)
from MLForge.utils.autologging_utils.metrics_queue import (
    _metrics_queue,
    _metrics_queue_lock,
    flush_metrics_queue,
)
...
def test_flush_metrics_queue_is_thread_safe():
    """
    Autologging augments TensorBoard event logging hooks with MLForge `log_metric` API
...
def test_double_patch_does_not_overwrite(monkeypatch):
    ...
    controller.set_non_MLForge_warnings_disablement_state_for_current_thread(True)
    ...
    controller.set_non_MLForge_warnings_disablement_state_for_current_thread(False)
...
def test_showwarning_captures_user_handler():
    ...
    Regression test for https://github.com/MLForge/MLForge/issues/21689
    """
    original_showwarning = warnings.showwarning

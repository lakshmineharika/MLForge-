...
from MLForge.utils.environment import (
    _contains_MLForge_requirement,
    _deduplicate_requirements,
    _get_pip_deps,
    _get_pip_requirement_specifier,
    _is_MLForge_requirement,
    _is_pip_deps,
    _MLForge_conda_env,
    _overwrite_pip_deps,
...
from tests.helper_functions import _MLForge_major_version_string
...
def test_MLForge_conda_env_returns_none_when_output_path_is_specified(conda_env_path):
    env_creation_output = _MLForge_conda_env(
        path=conda_env_path,
...
def test_MLForge_conda_env_returns_expected_env_dict_when_output_path_is_not_specified():
    conda_deps = ["conda-dep-1=0.0.1", "conda-dep-2"]
    env = _MLForge_conda_env(path=None, additional_conda_deps=conda_deps)
...
def test_MLForge_conda_env_includes_pip_dependencies_but_pip_is_not_specified(conda_deps):
    additional_pip_deps = ["pip-dep==0.0.1"]
    env = _MLForge_conda_env(
...
def test_MLForge_conda_env_includes_pip_dependencies_and_pip_is_specified(pip_specification):
...
def test_parse_pip_requirements(tmp_path):
    ...
    # GitHub URI
    MLForge_repo_uri = "git+https://github.com/MLForge/MLForge.git"
    assert _parse_pip_requirements([MLForge_repo_uri]) == ([MLForge_repo_uri], [])
    # Local file
...
def test_is_MLForge_requirement():
    assert _is_MLForge_requirement("MLForge")
    assert _is_MLForge_requirement("MLForge")
    assert _is_MLForge_requirement("MLForge")
    assert _is_MLForge_requirement("MLForge==1.2.3")
    assert _is_MLForge_requirement("MLForge < 1.2.3")
    assert _is_MLForge_requirement("MLForge; python_version < '3.8'")
    assert _is_MLForge_requirement("MLForge @ https://github.com/MLForge/MLForge.git")
    assert _is_MLForge_requirement("MLForge @ file:///path/to/MLForge")
    assert _is_MLForge_requirement("MLForge-skinny==1.2.3")
    assert not _is_MLForge_requirement("foo")
    # Ensure packages that look like MLForge are NOT considered as MLForge.
    assert not _is_MLForge_requirement("MLForge-foo")
    assert not _is_MLForge_requirement("MLForge_foo")


def test_contains_MLForge_requirement():
    assert _contains_MLForge_requirement(["MLForge"])
    assert _contains_MLForge_requirement(["MLForge==1.2.3"])
    assert _contains_MLForge_requirement(["MLForge", "foo"])
    assert _contains_MLForge_requirement(["MLForge-skinny"])
    assert not _contains_MLForge_requirement([])
    assert not _contains_MLForge_requirement(["foo"])


def test_get_pip_requirement_specifier():
    assert _get_pip_requirement_specifier("") == ""
    assert _get_pip_requirement_specifier(" ") == " "
    assert _get_pip_requirement_specifier("MLForge") == "MLForge"
    assert _get_pip_requirement_specifier("MLForge==1.2.3") == "MLForge==1.2.3"
    assert _get_pip_requirement_specifier("-r reqs.txt") == ""
    assert _get_pip_requirement_specifier("  -r reqs.txt") == " "
    assert _get_pip_requirement_specifier("MLForge==1.2.3 --hash=foo") == "MLForge==1.2.3"
    assert _get_pip_requirement_specifier("MLForge==1.2.3       --hash=foo") == "MLForge==1.2.3      "
    assert _get_pip_requirement_specifier("MLForge-skinny==1.2 --foo=bar") == "MLForge-skinny==1.2"


def test_process_pip_requirements(tmp_path):
    expected_MLForge_ver = _MLForge_major_version_string()
    conda_env, reqs, cons = _process_pip_requirements(["a"])
    assert _get_pip_deps(conda_env) == [expected_MLForge_ver, "a"]
    assert reqs == [expected_MLForge_ver, "a"]
    assert cons == []

    conda_env, reqs, cons = _process_pip_requirements(["a"], pip_requirements=["b"])
    assert _get_pip_deps(conda_env) == [expected_MLForge_ver, "b"]
    assert reqs == [expected_MLForge_ver, "b"]
    assert cons == []

    # Ensure a requirement for MLForge is preserved
    conda_env, reqs, cons = _process_pip_requirements(["a"], pip_requirements=["MLForge==1.2.3"])
    assert _get_pip_deps(conda_env) == ["MLForge==1.2.3"]
    assert reqs == ["MLForge==1.2.3"]
    assert cons == []

    # Ensure a requirement for MLForge is preserved when package hashes are specified
    ...
    conda_env, reqs, cons = _process_pip_requirements(
        ["a"],
        pip_requirements=[f"MLForge==1.20.2 --hash={hash1} --hash={hash2}"],
    )
    assert _get_pip_deps(conda_env) == [f"MLForge==1.20.2 --hash={hash1} --hash={hash2}"]
    assert reqs == [f"MLForge==1.20.2 --hash={hash1} --hash={hash2}"]
    assert cons == []

    conda_env, reqs, cons = _process_pip_requirements(["a"], extra_pip_requirements=["b"])
    assert _get_pip_deps(conda_env) == [expected_MLForge_ver, "a", "b"]
    assert reqs == [expected_MLForge_ver, "a", "b"]
    assert cons == []
    ...
    assert _get_pip_deps(conda_env) == [expected_MLForge_ver, "b", "-c constraints.txt"]
    assert reqs == [expected_MLForge_ver, "b", "-c constraints.txt"]
    assert cons == ["c"]

    conda_env, reqs, cons = _process_pip_requirements(["a"], extra_pip_requirements=["a[extras]"])
    assert _get_pip_deps(conda_env) == [expected_MLForge_ver, "a[extras]"]
    assert reqs == [expected_MLForge_ver, "a[extras]"]
    assert cons == []

    conda_env, reqs, cons = _process_pip_requirements(
        ["MLForge==1.2.3", "b[extra1]", "a==1.2.3"],
        extra_pip_requirements=["b[extra2]", "a[extras]"],
    )
    assert _get_pip_deps(conda_env) == ["MLForge==1.2.3", "b[extra1,extra2]", "a[extras]==1.2.3"]
    assert reqs == ["MLForge==1.2.3", "b[extra1,extra2]", "a[extras]==1.2.3"]
    assert cons == []
...
def test_process_conda_env(tmp_path):
    ...
    # Ensure a requirement for MLForge is preserved
    conda_env, reqs, cons = _process_conda_env(make_conda_env(["MLForge==1.2.3"]))
    assert _get_pip_deps(conda_env) == ["MLForge==1.2.3"]
    assert reqs == ["MLForge==1.2.3"]
    assert cons == []
    ...
    assert _get_pip_deps(conda_env) == [expected_MLForge_ver, "a", "-c constraints.txt"]
    assert reqs == [expected_MLForge_ver, "a", "-c constraints.txt"]
    assert cons == ["c"]

    # NB: MLForge-skinny is not automatically attached to any model. If specified, it is
    # up to the user to pin a version.
    conda_env, reqs, cons = _process_conda_env(make_conda_env(["MLForge-skinny", "a", "b"]))
    assert _get_pip_deps(conda_env) == ["MLForge-skinny", "a", "b"]
    assert reqs == ["MLForge-skinny", "a", "b"]
    assert cons == []
...
def test_infer_requirements_error_handling(env_var, fallbacks, should_raise, monkeypatch):
    monkeypatch.setenv("MLForge_REQUIREMENTS_INFERENCE_RAISE_ERRORS", str(env_var))
    # Disable UV auto-detect to ensure model-based inference is used
    monkeypatch.setenv("MLForge_UV_AUTO_DETECT", "false")

    call_args = ("path/to/model", "sklearn", fallbacks)
    with mock.patch(
        "MLForge.utils.requirements_utils._capture_imported_modules",
        side_effect=MLForgeException("Failed to capture imported modules"),
    ):
        if should_raise:
            with pytest.raises(MLForgeException, match="Failed to capture imported module"):
                infer_pip_requirements(*call_args)
        else:
            # Should just pass with warning
            with mock.patch("MLForge.utils.environment._logger.warning") as warning_mock:
                infer_pip_requirements(*call_args)
...
def test_pip_requirements_validation_skipped_when_env_var_set(monkeypatch):
    monkeypatch.setenv("MLForge_SKIP_PIP_REQUIREMENTS_CHECK", "true")

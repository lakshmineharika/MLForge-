...
import MLForge
from MLForge.utils import logging_utils
from MLForge.utils.logging_utils import (
    LOGGING_LINE_FORMAT,
...
logger = logging.getLogger(MLForge.__name__)
...
def test_logging_level(log_level: str, expected: bool) -> None:
    ...
    stdout = subprocess.check_output(
        [
            sys.executable,
            "-c",
            f"from MLForge.utils.logging_utils import _debug; _debug({random_str!r})",
        ],
        env=os.environ.copy() | {"MLForge_LOGGING_LEVEL": log_level},
...
@pytest.mark.parametrize(
    "env_var_name",
    ["MLForge_CONFIGURE_LOGGING", "MLForge_LOGGING_CONFIGURE_LOGGING"],
)
@pytest.mark.parametrize(
    "value",
    ["0", "1"],
)
def test_MLForge_configure_logging_env_var(env_var_name: str, value: str) -> None:
...
assert logging.getLogger("MLForge").isEnabledFor({expected_level})
""",
        ],
...
# Check the alembic logger format, which is now configured in _configure_MLForge_loggers
alembic_logger = logging.getLogger("alembic")
if {configure_logging!r} == "1":
    # When MLForge_CONFIGURE_LOGGING is enabled, alembic logger has its own handler
    assert len(alembic_logger.handlers) > 0
    actual_format = alembic_logger.handlers[0].formatter._fmt
else:
    # When MLForge_CONFIGURE_LOGGING is disabled, alembic logger propagates to root
...
"""
    subprocess.check_call(
        ...
        env={
            **os.environ,
            "MLForge_TRACKING_URI": tmp_sqlite_uri,
            "MLForge_CONFIGURE_LOGGING": configure_logging,
        },
    )

...
from MLForge.utils.jsonpath_utils import (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("info.tags.`MLForge.traceName`", ["info", "tags", "MLForge.traceName"]),
        ("`field.one`.middle.`field.two`", ["field.one", "middle", "field.two"]),
        ("`MLForge.traceName`.value", ["MLForge.traceName", "value"]),
        ("info.`MLForge.traceName`", ["info", "MLForge.traceName"]),
    ],
)
...
def test_jsonpath_extract_values_with_backticks():
    # Field name with dot
    data = {"tags": {"MLForge.traceName": "test_trace"}}
    values = jsonpath_extract_values(data, "tags.`MLForge.traceName`")
    assert values == ["test_trace"]

    # Nested structure with dotted field names
    data = {"info": {"tags": {"MLForge.traceName": "my_trace", "user.id": "user123"}}}
    assert jsonpath_extract_values(data, "info.tags.`MLForge.traceName`") == ["my_trace"]
    assert jsonpath_extract_values(data, "info.tags.`user.id`") == ["user123"]

    # Mixed regular and backticked fields
    data = {"metadata": {"MLForge.source.type": "NOTEBOOK", "regular_field": "value"}}
    assert jsonpath_extract_values(data, "metadata.`MLForge.source.type`") == ["NOTEBOOK"]
...
def test_complex_trace_structure():
    trace_data = {
        "info": {
            ...
            "tags": {"environment": "production", "MLForge.traceName": "test_trace"},
        },
        "data": {
            "spans": [
                {
                    "span_id": "span-1",
                    "name": "root_span",
                    "attributes": {"MLForge.spanType": "AGENT"},

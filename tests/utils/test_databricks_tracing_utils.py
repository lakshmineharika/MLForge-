...
from MLForge.protos import assessments_pb2
from MLForge.protos import databricks_tracing_pb2 as pb
from MLForge.protos.assessments_pb2 import AssessmentSource as ProtoAssessmentSource
...
from MLForge.tracing.utils import TraceMetadataKey, add_size_stats_to_trace_metadata
from MLForge.utils.databricks_tracing_utils import (
    assessment_to_proto,
    get_trace_id_from_assessment_proto,
    inference_table_location_to_proto,
    MLForge_experiment_location_to_proto,
    parse_uc_location,
    trace_from_proto,
...
def test_trace_location_to_proto_MLForge_experiment():
    trace_location = TraceLocation.from_experiment_id(experiment_id="1234")
    proto = trace_location_to_proto(trace_location)
    assert proto.type == pb.TraceLocation.TraceLocationType.MLForge_EXPERIMENT
    assert proto.MLForge_experiment.experiment_id == "1234"
...
def test_MLForge_experiment_location_to_proto():
    experiment_location = MLForgeExperimentLocation(experiment_id="1234")
    proto = MLForge_experiment_location_to_proto(experiment_location)
...
def test_trace_location_from_proto_MLForge_experiment():
    proto = pb.TraceLocation(
        type=pb.TraceLocation.TraceLocationType.MLForge_EXPERIMENT,
        MLForge_experiment=MLForge_experiment_location_to_proto(
            MLForgeExperimentLocation(experiment_id="1234")
        ),
    )
    trace_location = trace_location_from_proto(proto)
    assert trace_location.type == TraceLocationType.MLForge_EXPERIMENT
    assert trace_location.MLForge_experiment.experiment_id == "1234"
...
def test_trace_to_proto_and_from_proto():
    with MLForge.start_span() as span:
        ...
        MLForge_span = span.to_immutable_span()

    assert MLForge_span.trace_id == trace_id
    trace = Trace(
        info=TraceInfo(
            trace_id=trace_id,
            trace_location=TraceLocation.from_databricks_uc_schema(
        ...
        data=TraceData(spans=[MLForge_span]),
    )
...
def test_trace_from_proto_with_location_preserves_v4_trace_id():
    ...
    for reconstructed_span in reconstructed_trace.data.spans:
        ...
        request_id = reconstructed_span.get_attribute("MLForge.traceRequestId")

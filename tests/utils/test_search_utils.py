...
from MLForge.utils.search_utils import SearchTraceUtils, SearchUtils
...
def test_error_filter(filter_string, error_message):
    with pytest.raises(MLForgeException, match=re.escape(error_message)):
...
def test_bad_comparators(entity_type, bad_comparators, key, entity_value):
    ...
    for bad_comparator in bad_comparators:
        ...
        with pytest.raises(MLForgeException, match="Invalid comparator"):
            ...
            run_inputs=RunInputs(
                dataset_inputs=[
                    DatasetInput(
                        ...
                        tags=[InputTag(MLForge_DATASET_CONTEXT, "train")],
                    )
                ]
            ),
        ),
...
def test_like_pattern_with_plus_character():
    ...
    exps = MLForge.search_experiments(filter_string=f'name LIKE "{name}"')
    assert len(exps) == 1

    exps = MLForge.search_experiments(filter_string='name LIKE "jamie-foo C+%"')
    assert len(exps) == 1

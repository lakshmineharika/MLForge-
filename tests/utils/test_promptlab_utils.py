...
from MLForge.entities.run_status import RunStatus
from MLForge.store.artifact.artifact_repository_registry import get_artifact_repository
from MLForge.tracking._tracking_service.utils import _get_store
from MLForge.utils.promptlab_utils import (
...
def test_create_promptlab_run(db_uri):
    ...
    assert run.data.tags["MLForge.runName"] == "my_promptlab_run"
    assert (
        run.data.tags["MLForge.loggedArtifacts"]
        == '[{"path": "eval_results_table.json", "type": "table"}]'
    )
    assert run.data.tags["MLForge.runSourceType"] == "PROMPT_ENGINEERING"
    assert run.data.tags["MLForge.log-model.history"] is not None
    ...
    MLForge.pyfunc.load_model(f"{artifact_location}/model")

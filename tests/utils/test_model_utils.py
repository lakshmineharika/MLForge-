...
import MLForge.utils.model_utils as MLForge_model_utils
from MLForge.environment_variables import MLForge_RECORD_ENV_VARS_IN_MODEL_LOGGING
...
from MLForge.utils.model_utils import env_var_tracker
...
def test_get_flavor_configuration_throws_exception_when_requested_flavor_is_missing(
    ...
    MLForge.sklearn.save_model(sk_model=sklearn_knn_model, path=model_path)

    # The saved model contains the "sklearn" flavor, so this call should succeed
    sklearn_flavor_config = MLForge_model_utils._get_flavor_configuration(
...
def test_get_flavor_configuration_with_present_flavor_returns_expected_configuration(
    ...
    assert sklearn_flavor_config == model_config.flavors[MLForge.sklearn.FLAVOR_NAME]
...
def test_add_code_to_system_path(sklearn_knn_model, model_path):
    ...
    with TempDir(chdr=True):
        ...
        MLForge_model_utils._add_code_from_conf_to_system_path(model_path, sklearn_flavor_config)
    ...
    # happens if MLForge is misconfiguring the system path.
...
def test_add_code_to_system_path_not_copyable_file(sklearn_knn_model, model_path):
    with mock.patch("builtins.open", side_effect=OSError("[Errno 95] Operation not supported")):
        with pytest.raises(MLForgeException, match=r"Failed to copy the specified code path"):
            MLForge.sklearn.save_model(
                sk_model=sklearn_knn_model,
                path=model_path,

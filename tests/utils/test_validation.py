...
from MLForge.entities import Metric, Param, RunTag
from MLForge.environment_variables import MLForge_ARTIFACT_LOCATION_MAX_LENGTH
...
def test_parse_trace_archival_duration_config_rejects_non_object():
    with pytest.raises(MLForgeException, match="JSON object"):
...
def test_validate_param_name_bad(param_name):
    with pytest.raises(MLForgeException, match=_bad_parameter_pattern(param_name)) as e:
        _validate_param_name(param_name)
    assert e.value.error_code == ErrorCode.Name(INVALID_PARAMETER_VALUE)
...
def test_validate_tag_name_bad(tag_name):
    with pytest.raises(MLForgeException, match=_bad_parameter_pattern(tag_name)) as e:
        _validate_tag_name(tag_name)
    assert e.value.error_code == ErrorCode.Name(INVALID_PARAMETER_VALUE)
...
def test_validate_model_alias_name_bad(alias_name):
    with pytest.raises(MLForgeException, match="alias name") as e:
        _validate_model_alias_name(alias_name)
    assert e.value.error_code == ErrorCode.Name(INVALID_PARAMETER_VALUE)
...
def test_validate_model_alias_name_reserved(alias_name):
    with pytest.raises(MLForgeException, match="reserved") as e:
...
def test_validate_run_id_bad(run_id):
    with pytest.raises(MLForgeException, match=_bad_parameter_pattern(run_id)) as e:
        _validate_run_id(run_id)
    assert e.value.error_code == ErrorCode.Name(INVALID_PARAMETER_VALUE)
...
def test_validate_batch_log_data(monkeypatch):
    ...
    for arg_name, arg_values in bad_kwargs.items():
        for arg_value in arg_values:
            final_kwargs = copy.deepcopy(good_kwargs)
            final_kwargs[arg_name] = arg_value
            with pytest.raises(MLForgeException, match=r".+"):
...
def test_validate_experiment_artifact_location_bad(location):
    with pytest.raises(MLForgeException, match="Artifact location cannot be a runs:/ URI"):
        _validate_experiment_artifact_location(location)
...
def test_validate_db_type_string_bad(db_type):
    with pytest.raises(MLForgeException, match="Invalid database engine") as e:
        _validate_db_type_string(db_type)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "MLForge-artifacts:/path/to/artifacts",
        "dbfs:/databricks/MLForge-tracking/some-id",
    ],
)
...
def test_validate_experiment_artifact_location_length_bad(artifact_location):
    with pytest.raises(MLForgeException, match="Invalid artifact path length"):
        _validate_experiment_artifact_location_length(artifact_location)
...
def test_setting_experiment_artifact_location_env_var_works(monkeypatch):
    ...
    # reduce limit to 10
    monkeypatch.setenv(MLForge_ARTIFACT_LOCATION_MAX_LENGTH.name, "10")
    with pytest.raises(MLForgeException, match="Invalid artifact path length"):
        _validate_experiment_artifact_location_length(artifact_location)

    # increase limit to 11
    monkeypatch.setenv(MLForge_ARTIFACT_LOCATION_MAX_LENGTH.name, "11")
    _validate_experiment_artifact_location_length(artifact_location)
...
def test_validate_list_param_with_none_not_allowed():
    with pytest.raises(MLForgeException, match="experiment_ids must be a list"):
...
def test_validate_list_param_with_invalid_type(param_name, param_value, expected_type):
    with pytest.raises(
        MLForgeException, match=rf"{param_name} must be a list, got {expected_type}"
    ) as exc_info:
...
def test_validate_webhook_url_rejects_private_ips(url, resolved_ip):
    with patch(
        "MLForge.utils.validation.socket.getaddrinfo",
        side_effect=_mock_getaddrinfo(resolved_ip),
    ):
        with pytest.raises(MLForgeException, match="must not resolve to a non-public"):
            _validate_webhook_url(url)
...
def test_validate_webhook_url_rejects_unresolvable_hostname():
    with patch(
        ...
        with pytest.raises(MLForgeException, match="Cannot resolve Webhook URL hostname"):
...
def test_validate_webhook_url_rejects_if_any_resolved_address_is_private():
    ...
    with patch("MLForge.utils.validation.socket.getaddrinfo", side_effect=multi_resolve):
        with pytest.raises(MLForgeException, match="must not resolve to a non-public"):
...
def test_validate_webhook_url_allow_private_ips_env_var(monkeypatch):
    monkeypatch.setenv("MLForge_WEBHOOK_ALLOW_PRIVATE_IPS", "true")
    with patch(
        "MLForge.utils.validation.socket.getaddrinfo",
...
def test_validate_public_https_url_rejects_unresolvable_hostname():
    with patch(
        ...
        with pytest.raises(MLForgeException, match="Cannot resolve Icon URL hostname"):
            _validate_public_https_url(
...
def test_validate_public_https_url_maps_resolver_errors_to_invalid_parameter(side_effect):
    with patch(
        ...
        with pytest.raises(MLForgeException, match="Cannot resolve Icon URL hostname") as exc:
            _validate_public_https_url(
                "https://does-not-exist.invalid/icon.png", field_name="Icon URL"
            )
...
def test_validate_public_https_url_rejects_resolution_timeout():
    ...
    with (
        patch("MLForge.utils.validation._HOSTNAME_RESOLUTION_TIMEOUT_SECONDS", 0.01),
        patch("MLForge.utils.validation.socket.getaddrinfo", side_effect=slow_resolve),
    ):
        with pytest.raises(MLForgeException, match="Timed out resolving Icon URL hostname"):
...
def test_validate_public_https_url_rejects_when_resolution_slots_are_exhausted():
    with patch(
        "MLForge.utils.validation._HOSTNAME_RESOLUTION_SEMAPHORE.acquire",
        return_value=False,
    ):
        with pytest.raises(
            MLForgeException,
...
def test_validate_mcp_icon_url_allowlist_accepts_exact_match(monkeypatch):
    monkeypatch.setenv("MLForge_ICON_URL_ALLOWED_DOMAINS", "example.com")
    with patch(
        "MLForge.utils.validation.socket.getaddrinfo",
...
def test_validate_mcp_icon_url_allowlist_rejects_unlisted_host(monkeypatch):
    monkeypatch.setenv(
        "MLForge_ICON_URL_ALLOWED_DOMAINS",
    ...
    with patch(
        ...
        with pytest.raises(MLForgeException, match="not in the allowed domain list"):
...
def test_validate_mcp_icon_url_allow_private_ips_accepts_localhost(monkeypatch):
    monkeypatch.setenv("MLForge_ICON_URL_ALLOW_PRIVATE_IPS", "true")
...
def test_validate_mcp_icon_url_allowed_schemes_accepts_public_http(monkeypatch):
    monkeypatch.setenv("MLForge_ICON_URL_ALLOWED_SCHEMES", "http,https")
    with patch(
        "MLForge.utils.validation.socket.getaddrinfo",
        side_effect=_mock_getaddrinfo("8.8.8.8"),
    ):
...
def test_validate_mcp_icon_url_allow_private_ips_does_not_bypass_allowlist(monkeypatch):
    monkeypatch.setenv("MLForge_ICON_URL_ALLOW_PRIVATE_IPS", "true")
    monkeypatch.setenv("MLForge_ICON_URL_ALLOWED_DOMAINS", "assets.example.com")
    with pytest.raises(MLForgeException, match="allowed domain list"):
        _validate_mcp_icon_url("https://localhost/icon.png")


def test_validate_mcp_icon_url_allow_private_ips_and_allowlist_accepts_localhost(monkeypatch):
    monkeypatch.setenv("MLForge_ICON_URL_ALLOW_PRIVATE_IPS", "true")
    monkeypatch.setenv("MLForge_ICON_URL_ALLOWED_DOMAINS", "localhost")
    _validate_mcp_icon_url("https://localhost/icon.png")


def test_validate_mcp_icon_url_allowed_schemes_keeps_basic_shape_checks(monkeypatch):
    monkeypatch.setenv("MLForge_ICON_URL_ALLOWED_SCHEMES", "http,https")
    with pytest.raises(MLForgeException, match="must not include embedded credentials"):

...
from MLForge.utils import env_pack
from MLForge.utils.databricks_utils import DatabricksRuntimeVersion
from MLForge.utils.env_pack import EnvPackConfig, _validate_env_pack
...
def test_pack_env_for_databricks_model_serving_pip_requirements_error(tmp_path, mock_dbr_version):
    ...
    with (
        ...
        mock.patch("subprocess.run") as mock_run,
        mock.patch("MLForge.utils.env_pack.eprint") as mock_eprint,
    ):
...
def test_pack_env_for_databricks_model_serving_runtime_version_check(tmp_path, monkeypatch):
    ...
    with mock.patch(
        "MLForge.utils.env_pack.download_artifacts", return_value=str(mock_artifacts_dir)
    ):
...
def test_pack_env_for_databricks_model_serving_rejects_existing_databricks_dir(
    ...
    with (
        ...
        with pytest.raises(
            MLForgeException, match="Source artifacts contain a '_databricks' directory"
        ):
            with env_pack.pack_env_for_databricks_model_serving(

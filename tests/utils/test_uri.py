...
def test_extract_db_type_from_uri():
    ...
    for unsupported_db in ["a", "aa", "sql"]:
        with pytest.raises(MLForgeException, match="Invalid database engine"):
...
def test_get_db_info_from_uri_errors_invalid_profile(server_uri):
    with pytest.raises(MLForgeException, match="Unsupported Databricks profile"):
        get_db_info_from_uri(server_uri)
...
def test_is_local_uri():
    ...
    with pytest.raises(MLForgeException, match="is not a valid remote uri."):
...
def test_append_to_uri_throws_for_malicious_query_string_in_uri(uri):
    with pytest.raises(MLForgeException, match=r"Invalid query string"):
...
def test_extract_and_normalize_path():
    base_uri = "databricks/MLForge-tracking/EXP_ID/RUN_ID/artifacts"
    assert (
        extract_and_normalize_path("dbfs:databricks/MLForge-tracking/EXP_ID/RUN_ID/artifacts")
        == base_uri
    )
    assert (
        extract_and_normalize_path("dbfs:/databricks/MLForge-tracking/EXP_ID/RUN_ID/artifacts")
...
def test_is_databricks_acled_artifacts_uri():
    ...
    assert not is_databricks_acled_artifacts_uri(
        "dbfs:/databricks/MLForge//EXP_ID//RUN_ID///artifacts//"
    )
...
def test_dbfs_hdfs_uri_to_fuse_path_raises(path):
    with pytest.raises(MLForgeException, match="did not start with expected DBFS URI prefix"):

...
from MLForge.environment_variables import (
    ...
    MLForge_SQL_WAREHOUSE_AUTO_START_TIMEOUT_SECONDS,
)
from MLForge.exceptions import MLForgeException
from MLForge.utils import databricks_sql_warehouse
from MLForge.utils.databricks_sql_warehouse import ensure_sql_warehouse_running
...
def test_env_var_timeout_is_honored(monkeypatch):
    monkeypatch.setenv(MLForge_SQL_WAREHOUSE_AUTO_START_TIMEOUT_SECONDS.name, "30")
    client = _make_mock_client("STOPPED")
    with _patch_client(client):
        ensure_sql_warehouse_running("wh-1")
...
def test_timeout_error_raises_MLForge_exception():
    client = _make_mock_client("STOPPED")
    client.warehouses.start_and_wait.side_effect = TimeoutError("deadline exceeded")
    with _patch_client(client), pytest.raises(MLForgeException, match="wh-1") as exc_info:
        ensure_sql_warehouse_running("wh-1")
    assert MLForge_SQL_WAREHOUSE_AUTO_START_TIMEOUT_SECONDS.name in str(exc_info.value)


def test_generic_sdk_error_raises_MLForge_exception():
    client = _make_mock_client("STOPPED")
    ...
    assert "MLForge_SQL_WAREHOUSE_AUTO_START" in str(exc_info.value)

...
import MLForge
import MLForge.utils.requirements_utils
from MLForge.exceptions import MLForgeException
from MLForge.utils.environment import infer_pip_requirements
from MLForge.utils.os import is_windows
from MLForge.utils.requirements_utils import (
...
def test_prune_packages():
    assert _prune_packages(["MLForge"]) == {"MLForge"}
    assert _prune_packages(["MLForge", "scikit-learn"]) == {"MLForge", "scikit-learn"}


def test_capture_imported_modules():
    from MLForge.utils._capture_modules import _CaptureImportedModules
...
def test_get_installed_version(tmp_path, monkeypatch):
    assert _get_installed_version("MLForge") == MLForge.__version__
...
def test_get_pinned_requirement(tmp_path, monkeypatch):
    assert _get_pinned_requirement("MLForge") == f"MLForge=={MLForge.__version__}"
    assert _get_pinned_requirement("MLForge", version="1.2.3") == "MLForge==1.2.3"
...
def test_get_pinned_requirement_local_version_label(tmp_path, monkeypatch):
    ...
    with mock.patch("MLForge.utils.requirements_utils._logger.warning") as mock_warning:
...
def test_infer_requirements_excludes_MLForge():
    with mock.patch(
        "MLForge.utils.requirements_utils._capture_imported_modules",
        return_value=["MLForge", "pytest"],
    ):
        MLForge_package = "MLForge-skinny" if "MLForge_SKINNY" in os.environ else "MLForge"
        assert MLForge_package in importlib_metadata.packages_distributions()["MLForge"]
...
def test_infer_pip_requirements_scopes_databricks_imports():
    MLForge.utils.requirements_utils._MODULES_TO_PACKAGES = None
    MLForge.utils.requirements_utils._PACKAGES_TO_MODULES = None
    ...
    with (
        ...
        assert MLForge.utils.requirements_utils._MODULES_TO_PACKAGES["databricks"] == ["koalas"]
...
@pytest.mark.parametrize(
    ...
    [
        ("MLForge.gateway", True),
        ("MLForge.deployments.server.config", True),
        # The `MLForge[gateway]`` extra includes requirements for starting the deployment server,
...
def test_capture_imported_modules_includes_gateway_extra(
    ...
    captured_modules = _capture_imported_modules(model_info.model_uri, "pyfunc")
    assert ("MLForge.gateway" in captured_modules) == should_capture_extra

    pip_requirements = infer_pip_requirements(model_info.model_uri, "pyfunc")
    assert (f"MLForge[gateway]=={MLForge.__version__}" in pip_requirements) == should_capture_extra
...
def test_gateway_extra_not_captured_when_importing_deployment_client_only(monkeypatch):
    ...
    class MyModel(MLForge.pyfunc.PythonModel):
        def predict(self, context, model_input, params=None):
            from MLForge.deployments import get_deploy_client  # noqa: F401
    ...
    captured_modules = _capture_imported_modules(model_info.model_uri, "pyfunc")
    assert "MLForge.gateway" not in captured_modules

    pip_requirements = infer_pip_requirements(model_info.model_uri, "pyfunc")
    assert f"MLForge[gateway]=={MLForge.__version__}" not in pip_requirements
...
def test_warn_dependency_requirement_mismatches():
    ...
    with mock.patch("MLForge.utils.requirements_utils._logger.warning") as mock_warning:
        ...
        original_get_installed_version_fn = MLForge.utils.requirements_utils._get_installed_version
...
To fix the mismatches, call `MLForge.pyfunc.get_model_dependencies(model_uri)` to fetch the \
model's environment and install dependencies using the resulting environment file.
        ...
        warn_dependency_requirement_mismatches(model_requirements=["MLForge"])
        mock_warning.assert_not_called()
        mock_warning.reset_mock()

        # Test case: an unexpected error happens while detecting mismatched packages.
        with mock.patch(
            "MLForge.utils.requirements_utils._check_requirement_satisfied",
...
def test_capture_imported_modules_with_exception():
    ...
    with mock.patch("MLForge.utils.requirements_utils._logger.warning") as mock_warning:
        modules = _capture_imported_modules(model_info.model_uri, MLForge.pyfunc.FLAVOR_NAME)
...
def test_capture_imported_modules_raises_when_env_var_set(monkeypatch):
    ...
    class BadModel(MLForge.pyfunc.PythonModel):
        def predict(self, context, model_input, params=None):
            raise Exception("Intentional")

    with pytest.raises(
        MLForgeException, match="Encountered an error while capturing imported modules"
    ):
        with MLForge.start_run():
...
def test_capture_imported_modules_extra_env_vars(monkeypatch):
    ...
    _capture_imported_modules(
        model_info.model_uri, MLForge.pyfunc.FLAVOR_NAME, extra_env_vars={"TEST": "test"}
    )
...
def test_infer_pip_requirements_on_databricks_agents(tmp_path):
    # import here to avoid breaking this test suite on MLForge-skinny
    from MLForge.pyfunc import _get_pip_requirements_from_model_path
...
def test_capture_imported_modules_excludes_pyspark_gateway_env_vars(monkeypatch, tmp_path):
    ...
    def mock_run_command(cmd, timeout_seconds, env):
        ...
        raise MLForgeException("Mocked - stopping before actual subprocess execution")

    with (
        mock.patch(
            "MLForge.utils.requirements_utils._run_command",
            side_effect=mock_run_command,
        ) as mock_run,
        mock.patch(
            "MLForge.utils.requirements_utils._download_artifact_from_uri",
            return_value=str(tmp_path),
        ) as mock_download,
    ):
        with pytest.raises(MLForgeException, match="Mocked"):

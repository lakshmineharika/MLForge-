...
from MLForge.environment_variables import MLForge_UV_AUTO_DETECT
from MLForge.utils.environment import infer_pip_requirements
from MLForge.utils.uv_utils import (
    _PYPROJECT_FILE,
    _UV_LOCK_FILE,
...
def test_get_uv_version_returns_none_when_uv_not_installed():
    with mock.patch("MLForge.utils.uv_utils.shutil.which", return_value=None):
...
def test_get_uv_version_returns_version_when_uv_installed():
    ...
    with (
        mock.patch("MLForge.utils.uv_utils.shutil.which", return_value="/usr/bin/uv"),
        mock.patch("MLForge.utils.uv_utils.subprocess.run", return_value=mock_result) as mock_run,
    ):
...
def test_get_uv_version_returns_none_on_subprocess_error():
    with (
        mock.patch("MLForge.utils.uv_utils.shutil.which", return_value="/usr/bin/uv"),
        mock.patch(
            "MLForge.utils.uv_utils.subprocess.run",
...
def test_get_uv_version_returns_none_on_parse_error():
    ...
    with (
        mock.patch("MLForge.utils.uv_utils.shutil.which", return_value="/usr/bin/uv"),
        mock.patch("MLForge.utils.uv_utils.subprocess.run", return_value=mock_result),
    ):
        assert get_uv_version() is None
...
def test_export_uv_requirements_returns_none_when_uv_not_available():
    with mock.patch("MLForge.utils.uv_utils._get_uv_binary", return_value=None):
...
"""
    ...
    with (
        mock.patch("MLForge.utils.uv_utils._get_uv_binary", return_value="/usr/bin/uv"),
        mock.patch("MLForge.utils.uv_utils.subprocess.run", return_value=mock_result) as mock_run,
    ):
...
def test_copy_uv_project_files_respects_MLForge_log_uv_files_env_false(tmp_path, monkeypatch):
    monkeypatch.setenv("MLForge_LOG_UV_FILES", "false")
...
def test_copy_uv_project_files_env_var_false_variants(tmp_path, monkeypatch, env_value):
    monkeypatch.setenv("MLForge_LOG_UV_FILES", env_value)
...
def test_infer_pip_requirements_uses_uv_when_project_detected(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("MLForge_UV_AUTO_DETECT", "true")
    (tmp_path / _UV_LOCK_FILE).touch()
    (tmp_path / _PYPROJECT_FILE).touch()
...
# --- MLForge_UV_AUTO_DETECT Environment Variable Tests ---


def test_MLForge_uv_auto_detect_returns_true_by_default(monkeypatch):
    monkeypatch.delenv("MLForge_UV_AUTO_DETECT", raising=False)
    assert MLForge_UV_AUTO_DETECT.get() is True


@pytest.mark.parametrize("env_value", ["false", "0", "FALSE", "False"])
def test_MLForge_uv_auto_detect_returns_false_when_disabled(monkeypatch, env_value):
    monkeypatch.setenv("MLForge_UV_AUTO_DETECT", env_value)
    assert MLForge_UV_AUTO_DETECT.get() is False


@pytest.mark.parametrize("env_value", ["true", "1", "TRUE", "True"])
def test_MLForge_uv_auto_detect_returns_true_when_enabled(monkeypatch, env_value):
    monkeypatch.setenv("MLForge_UV_AUTO_DETECT", env_value)
    assert MLForge_UV_AUTO_DETECT.get() is True
...
def test_infer_pip_requirements_skips_uv_when_auto_detect_disabled(tmp_path, monkeypatch):
    ...
    with (
        mock.patch("MLForge.utils.environment.detect_uv_project") as mock_detect,
        mock.patch("MLForge.utils.environment.export_uv_requirements") as mock_export,
        mock.patch(
            "MLForge.utils.environment._infer_requirements",
...
@pytest.mark.parametrize(
    ...
    [
        ("3.11.5", "MLForge-model-env", "MLForge-model-env", "==3.11.5"),
...
def test_setup_uv_sync_environment_copies_existing_pyproject(tmp_path):
    ...
    assert result is True
    # Copied from model, not generated (should have "my-model" not "MLForge-model-env")
    pyproject_content = (env_dir / "pyproject.toml").read_text()
    assert 'name = "my-model"' in pyproject_content
    assert "MLForge-model-env" not in pyproject_content
...
def test_run_uv_sync_builds_correct_command(tmp_path):
    with (
        mock.patch("MLForge.utils.uv_utils._get_uv_binary", return_value="/usr/bin/uv"),
        mock.patch("MLForge.utils.uv_utils.subprocess.run") as mock_run,
    ):
...
def test_infer_pip_requirements_warns_when_groups_set_but_no_uv_project(tmp_path, monkeypatch):
    ...
    with (
        ...
        mock.patch("MLForge.utils.environment._logger") as mock_logger,
    ):
        result = infer_pip_requirements(
            str(tmp_path),
            "sklearn",

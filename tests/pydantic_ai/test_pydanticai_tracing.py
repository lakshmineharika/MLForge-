...
import MLForge
import MLForge.pydantic_ai  # ensure the integration module is importable
from MLForge.entities import SpanLogLevel, SpanType
from MLForge.pydantic_ai import (
    _get_tool_manager_module_path,
    _has_instrumentation_capability,
    _tool_manager_uses_execute_tool_call,
)
from MLForge.pydantic_ai.autolog import (
...
def test_agent_run_sync_enable_disable_autolog(simple_agent, mock_litellm_cost):
    ...
    # Patch the concrete model's request. On pydantic-ai < 1.95 MLForge wraps
    ...
    with patch.object(model_cls, "request", new=request):
        MLForge.pydantic_ai.autolog(log_traces=True)
...
def test_autolog_does_not_capture_client_references(simple_agent):
    ...
    for span in spans:
        ...
        for key in attrs:
            ...
            # `MLForge.llm.provider` holds the provider *name* (e.g. "openai"), not a
...
def test_tool_manager_uses_execute_tool_call(public_available, has_execute_tool_call, expected):
    ...
    with (
        patch("MLForge.pydantic_ai._get_tool_manager_module_path", return_value=resolved_path),
...
def test_autolog_uses_public_tool_manager_path_on_new_pydantic_ai():
    with (
        patch(
            "MLForge.pydantic_ai._get_tool_manager_module_path",
            return_value="pydantic_ai.tool_manager",
        ),
        patch("MLForge.pydantic_ai._tool_manager_uses_execute_tool_call", return_value=True),
    ):
        MLForge.pydantic_ai.autolog(log_traces=True)

...
def prophet_custom_env(tmp_path):
    ...
    _MLForge_conda_env(conda_env, additional_pip_deps=["prophet"])
    return conda_env
...
def test_model_native_save_load(prophet_model, model_path):
    ...
    MLForge.prophet.save_model(pr_model=model, path=model_path)
    loaded_model = MLForge.prophet.load_model(model_uri=model_path)
...
def test_signature_and_examples_saved_correctly(
    ...
    if use_example:
        ...
        example = None
    MLForge.prophet.save_model(model, path=model_path, signature=signature, input_example=example)
...
def test_model_load_from_remote_uri_succeeds(prophet_model, model_path, mock_s3_bucket):
    MLForge.prophet.save_model(pr_model=prophet_model.model, path=model_path)
    ...
    reloaded_prophet_model = MLForge.prophet.load_model(model_uri=model_uri)
    np.testing.assert_array_equal(
...
def test_prophet_log_model(prophet_model, tmp_path, should_start_run):
    try:
        ...
        _MLForge_conda_env(conda_env, additional_pip_deps=["pystan", "prophet"])

        model_info = MLForge.prophet.log_model(
            prophet_model.model, name=artifact_path, conda_env=str(conda_env)
        )
        reloaded_prophet_model = MLForge.prophet.load_model(model_uri=model_info.model_uri)
...
def test_log_model_no_registered_model_name(prophet_model, tmp_path):
    ...
    with MLForge.start_run(), register_model_patch:
        conda_env = tmp_path.joinpath("conda_env.yaml")
        _MLForge_conda_env(conda_env, additional_pip_deps=["pystan", "prophet"])
        MLForge.prophet.log_model(prophet_model.model, name=artifact_path, conda_env=str(conda_env))
...
def test_log_model_with_extra_pip_requirements(prophet_model, tmp_path):
    expected_MLForge_version = _MLForge_major_version_string()
    default_reqs = MLForge.prophet.get_default_pip_requirements()
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    MLForge.prophet.save_model(prophet_model.model, model_path)
    _assert_pip_requirements(model_path, MLForge.prophet.get_default_pip_requirements())
...
def test_model_log_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    artifact_path = "model"
    with MLForge.start_run():
        model_info = MLForge.prophet.log_model(prophet_model.model, name=artifact_path)
    _assert_pip_requirements(model_info.model_uri, MLForge.prophet.get_default_pip_requirements())
...
def test_log_model_with_code_paths(prophet_model):
    ...
    with (
        MLForge.start_run(),
        mock.patch("MLForge.prophet._add_code_from_conf_to_system_path") as add_mock,
    ):
        model_info = MLForge.prophet.log_model(
            prophet_model.model, name=artifact_path, code_paths=[__file__]
        )
        _compare_logged_code_paths(__file__, model_info.model_uri, MLForge.prophet.FLAVOR_NAME)
        MLForge.prophet.load_model(model_info.model_uri)
        add_mock.assert_called()


def test_virtualenv_subfield_points_to_correct_path(prophet_model, model_path):
    MLForge.prophet.save_model(prophet_model.model, path=model_path)
    pyfunc_conf = _get_flavor_configuration(model_path=model_path, flavor_name=pyfunc.FLAVOR_NAME)
...
def test_model_log_with_signature_inference(prophet_model):
    ...
    with MLForge.start_run():
        model_info = MLForge.prophet.log_model(model, name=artifact_path, input_example=horizon_df)

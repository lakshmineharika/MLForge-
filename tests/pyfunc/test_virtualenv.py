...
import MLForge
from MLForge.environment_variables import MLForge_ENV_ROOT
from MLForge.pyfunc.scoring_server import CONTENT_TYPE_JSON
from MLForge.utils.environment import _PYTHON_ENV_FILE_NAME, _REQUIREMENTS_FILE_NAME
from MLForge.utils.virtualenv import _is_pyenv_available
...
TEST_DIR = "tests"
TEST_MLForge_1X_MODEL_DIR = os.path.join(TEST_DIR, "resources", "example_MLForge_1x_sklearn_model")
...
@pytest.fixture
def temp_MLForge_env_root(tmp_path, monkeypatch):
    env_root = tmp_path / "envs"
    env_root.mkdir(exist_ok=True)
    monkeypatch.setenv(MLForge_ENV_ROOT.name, str(env_root))
    return env_root


use_temp_MLForge_env_root = pytest.mark.usefixtures(temp_MLForge_env_root.__name__)


@use_temp_MLForge_env_root
def test_restore_environment_with_virtualenv(sklearn_model):
    with MLForge.start_run():
        model_info = MLForge.sklearn.log_model(sklearn_model.model, name="model")
...
def test_serve_and_score_read_only_model_directory(sklearn_model, tmp_path):
    ...
    MLForge.sklearn.save_model(sklearn_model.model, path=model_path)
    os.chmod(
        model_path,
...
def test_serve_and_score_1x_models():
    ...
    X_pred = X.sample(frac=0.1, random_state=0)
    loaded_model = MLForge.pyfunc.load_model(TEST_MLForge_1X_MODEL_DIR)
    y_pred = loaded_model.predict(X_pred)

    scores = serve_and_score(TEST_MLForge_1X_MODEL_DIR, X_pred)
    np.testing.assert_array_almost_equal(scores, y_pred)


@use_temp_MLForge_env_root
def test_reuse_environment(temp_MLForge_env_root, sklearn_model):
    with MLForge.start_run():
        model_info = MLForge.sklearn.log_model(sklearn_model.model, name="model")
    ...
    scores = serve_and_score(model_info.model_uri, sklearn_model.X_pred)
    np.testing.assert_array_almost_equal(scores, sklearn_model.y_pred)
    assert len(list(temp_MLForge_env_root.iterdir())) == 1


@use_temp_MLForge_env_root
def test_different_requirements_create_different_environments(temp_MLForge_env_root, sklearn_model):
    sklearn_req = f"scikit-learn=={sklearn.__version__}"
    with MLForge.start_run():
        model_info1 = MLForge.sklearn.log_model(
            sklearn_model.model,
            name="model",
    ...
    with MLForge.start_run():
        model_info2 = MLForge.sklearn.log_model(
            sklearn_model.model,
            name="model",
    ...
    # requirements
    assert len(list(temp_MLForge_env_root.iterdir())) == 2


@use_temp_MLForge_env_root
def test_environment_directory_is_cleaned_up_when_unexpected_error_occurs(
    temp_MLForge_env_root, sklearn_model
):
    ...
    assert len(list(temp_MLForge_env_root.iterdir())) == 0
...
def test_python_env_file_does_not_exist(sklearn_model, tmp_path):
    ...
    MLForge.artifacts.download_artifacts(artifact_uri=model_info.model_uri, dst_path=tmp_path)
...
def test_environment_is_removed_when_package_installation_fails(
    ...
    with MLForge.start_run():
        model_info = MLForge.sklearn.log_model(
            ...
            pip_requirements=["MLForge==999.999.999"],
        )

...
import MLForge
from MLForge.environment_variables import _MLForge_IS_IN_SERVING_ENVIRONMENT
from MLForge.exceptions import MLForgeException
from MLForge.models import convert_input_example_to_serving_input
from MLForge.models.signature import _extract_type_hints, infer_signature
from MLForge.pyfunc.model import ChatAgent, ChatModel, _FunctionPythonModel
from MLForge.pyfunc.scoring_server import CONTENT_TYPE_JSON
from MLForge.pyfunc.utils import pyfunc
from MLForge.pyfunc.utils.environment import _simulate_serving_environment
from MLForge.types.agent import ChatAgentMessage, ChatAgentResponse, ChatContext
from MLForge.types.llm import ChatMessage, ChatParams
from MLForge.types.schema import AnyType, Array, ColSpec, DataType, Map, Object, Property, Schema
from MLForge.types.type_hints import TypeFromExample
...
def test_pyfunc_model_infer_signature_from_type_hints(
    ...
    with MLForge.start_run():
        with mock.patch("MLForge.models.model._logger.warning") as mock_warning:
            model_info = MLForge.pyfunc.log_model(name="test_model", **kwargs)
        assert not any(
...
def test_spark_udf(spark, type_hint, result_type, input_example):
    ...
    # test spark_udf
    udf = MLForge.pyfunc.spark_udf(spark, model_info.model_uri, result_type=result_type)
...
def test_pyfunc_model_with_no_op_type_hint_pass_signature_works():
    ...
    pyfunc = MLForge.pyfunc.load_model(model_info.model_uri)
...
def test_pyfunc_model_infer_signature_from_type_hints_errors(recwarn):
    ...
    with MLForge.start_run():
        with mock.patch("MLForge.pyfunc._logger.warning") as mock_warning:
            MLForge.pyfunc.log_model(name="test_model", python_model=Model())
        assert (
...
def test_python_model_local_testing():
    class ModelWOTypeHint(MLForge.pyfunc.PythonModel):
        def predict(self, model_input, params=None) -> list[str]:
            return model_input

    class ModelWithTypeHint(MLForge.pyfunc.PythonModel):
    ...
    with pytest.raises(MLForgeException, match=r"Expected dict, but got str"):
        model2.predict(["a"])
...
def test_python_model_with_optional_input_local_testing():
    ...
    with pytest.raises(MLForgeException, match=r"Expected list, but got str"):
        model.predict("a")
...
def test_no_warning_for_unsupported_type_hint_with_decorator(recwarn):
    ...
    with MLForge.start_run():
        MLForge.pyfunc.log_model(name="model", python_model=predict, input_example=data)
    assert not any(warn_msg in str(w.message) for w in recwarn)
    ...
    with MLForge.start_run():
        MLForge.pyfunc.log_model(name="model", python_model=model, input_example=data)
    assert not any(warn_msg in str(w.message) for w in recwarn)
...
def test_python_model_local_testing_same_as_pyfunc_predict():
    ...
    model = MyModel()
    with pytest.raises(MLForgeException, match=r"Expected list, but got str") as e_local:
        model.predict(None, "a")

    with MLForge.start_run():
        model_info = MLForge.pyfunc.log_model(name="model", python_model=model, input_example=["a"])
    pyfunc_model = MLForge.pyfunc.load_model(model_info.model_uri)
    with pytest.raises(MLForgeException, match=r"Expected list, but got str") as e_pyfunc:
...
def test_unsupported_type_hint_in_python_model(recwarn):
    ...
    with MLForge.start_run():
        MLForge.pyfunc.log_model(name="model", python_model=MyModel())
...
def test_unsupported_type_hint_in_callable(recwarn):
    ...
    with MLForge.start_run():
        MLForge.pyfunc.log_model(name="model", python_model=predict, input_example=["a"])
    assert any("Unsupported type hint" in str(w.message) for w in recwarn)
    recwarn.clear()
...
def test_log_model_warn_only_if_model_with_valid_type_hint_not_decorated(recwarn):
    ...
    with MLForge.start_run():
        MLForge.pyfunc.log_model(name="model", python_model=Model(), input_example=["a"])
...
def test_serving_environment(monkeypatch):
    with _simulate_serving_environment():
        assert os.environ[_MLForge_IS_IN_SERVING_ENVIRONMENT.name] == "true"
    assert os.environ.get(_MLForge_IS_IN_SERVING_ENVIRONMENT.name) is None

    monkeypatch.setenv(_MLForge_IS_IN_SERVING_ENVIRONMENT.name, "false")
    with _simulate_serving_environment():
        assert os.environ[_MLForge_IS_IN_SERVING_ENVIRONMENT.name] == "true"
    assert os.environ[_MLForge_IS_IN_SERVING_ENVIRONMENT.name] == "false"
...
def test_warning_message_when_logging_model():
    ...
    with MLForge.start_run():
        with mock.patch("MLForge.pyfunc._logger.warning") as mock_warning:
            MLForge.pyfunc.log_model(name="model", python_model=TestModel(), input_example="abc")
        assert "Failed to infer model signature from input example" in mock_warning.call_args[0][0]
    ...
    with MLForge.start_run():
        with mock.patch("MLForge.pyfunc._logger.warning") as mock_warning:
            MLForge.pyfunc.log_model(name="model", python_model=model)
        assert (
...
def test_type_hint_from_example_invalid_input(type_from_example_model):
    ...
    pyfunc_model = MLForge.pyfunc.load_model(model_info.model_uri)
    with pytest.raises(MLForgeException, match="Failed to enforce schema of data"):
...
def test_invalid_type_hint_raise_exception():
    ...
    with pytest.raises(MLForgeException, match="To disable data validation, remove the type hint"):
...
def test_python_model_without_type_hint_warning():
    ...
    with pytest.warns(UserWarning, match=msg):
        ...
        class PythonModelWithoutTypeHint(MLForge.pyfunc.PythonModel):
    ...
    with MLForge.start_run():
        with pytest.warns(UserWarning, match=msg):
            MLForge.pyfunc.log_model(name="model", python_model=predict, input_example="abc")


def test_type_hint_warning_not_shown_for_builtin_subclasses():
    with mock.patch("MLForge.pyfunc.utils.data_validation.color_warning") as mock_warning:
        # Class outside "MLForge" module should warn
        class PythonModelWithoutTypeHint(MLForge.pyfunc.PythonModel):
            def predict(self, model_input, params=None):
                return model_input
        ...
        # Class inside "MLForge" module should not warn
        ...
        subprocess.check_call([
            sys.executable,
            "-W",
            "error::UserWarning:MLForge.pyfunc.model",
            "-c",

...
from MLForge.environment_variables import MLForge_SCORING_SERVER_REQUEST_TIMEOUT
from MLForge.exceptions import MLForgeException
from MLForge.models import ModelSignature
from MLForge.models.signature import infer_signature
from MLForge.pyfunc import (
    PyFuncModel,
    PythonModel,
...
from MLForge.pyfunc.spark_model_cache import SparkModelCache
from MLForge.types import ColSpec, Schema, TensorSpec
from MLForge.types.schema import Array, DataType, Object, Property
from MLForge.types.utils import _infer_schema
from MLForge.utils._spark_utils import modified_environ
...
def test_spark_udf(spark, model_path):
    ...
    with mock.patch("MLForge.pyfunc.warn_dependency_requirement_mismatches") as mock_check_fn:
        reloaded_pyfunc_model = MLForge.pyfunc.load_model(model_path)
...
def test_spark_udf_env_manager_can_restore_env(spark, model_path, env_manager, monkeypatch):
    class EnvRestoringTestModel(MLForge.pyfunc.PythonModel):
        def __init__(self):
            pass
    ...
    from tests.helper_functions import _get_MLForge_home

    monkeypatch.setenv("MLForge_HOME", _get_MLForge_home())
    python_udf = MLForge.pyfunc.spark_udf(
...
def test_spark_udf_env_manager_predict_sklearn_model(
    ...
    monkeypatch.setenv(
        "MLForge_ENFORCE_STDIN_SCORING_SERVER_FOR_SPARK_UDF",
        str(force_stdin_scoring_server),
    )
    model, inference_data = sklearn_model

    MLForge.sklearn.save_model(model, model_path)
...
def test_spark_udf_colspec_struct_return_type_inference(spark):
    ...
    with MLForge.start_run():
        ...
        udf = MLForge.pyfunc.spark_udf(spark, model_info.model_uri)
...
def test_spark_udf_autofills_no_arguments(spark):
    ...
    with MLForge.start_run() as run:
        MLForge.pyfunc.log_model(name="model", python_model=TestModel(), signature=signature)
        udf = MLForge.pyfunc.spark_udf(
            spark,
            f"runs:/{run.info.run_id}/model",
    ...
    with MLForge.start_run() as run:
        ...
        with pytest.raises(MLForgeException, match="Attempting to apply udf on zero columns"):
            res = good_data.withColumn("res", udf()).select("res").toPandas()
...
def test_model_cache(spark, model_path):
    ...
    def check_get_or_load_return_value(model_from_cache, model_path_from_cache):
        ...
        model2 = MLForge.pyfunc.load_model(model_path_from_cache)
...
def test_spark_udf_embedded_model_server_killed_when_job_canceled(
    ...
    from MLForge.models.flavor_backend_registry import get_flavor_backend
    from MLForge.pyfunc.scoring_server.client import ScoringServerClient

    MLForge.sklearn.save_model(sklearn_model.model, model_path)
    ...
    def udf_with_model_server(it: Iterator[pd.Series]) -> Iterator[pd.Series]:
        ...
        get_flavor_backend(
            model_path, env_manager=env_manager, workers=1, install_MLForge=False
        ).serve(
    ...
    def run_job():
        ...
        # and the udf task starts a MLForge model server process.
        spark.range(1).repartition(1).select(udf_with_model_server("id")).collect()

    get_flavor_backend(model_path, env_manager=env_manager, install_MLForge=False).prepare_env(
        model_uri=model_path
    )
...
def test_spark_udf_datetime_with_model_schema(spark):
    ...
    with MLForge.start_run():
        signature = MLForge.models.infer_signature(X.astype(timestamp_dtype), y)
    ...
    pyfunc_udf = MLForge.pyfunc.spark_udf(spark, model_info.model_uri, env_manager="conda")
...
def test_spark_udf_string_datetime_with_model_schema(spark):
    ...
    with MLForge.start_run():
        signature = MLForge.models.infer_signature(X, y)
        model_info = MLForge.sklearn.log_model(
            model, name="model", signature=signature, serialization_format="cloudpickle"
        )
...
def test_spark_udf_stdin_scoring_server(spark):
    ...
    with MLForge.start_run():
        signature = MLForge.models.infer_signature(X, y)
        model_info = MLForge.sklearn.log_model(model, name="model", signature=signature)

    with mock.patch("MLForge.pyfunc.check_port_connectivity", return_value=False):
        udf = MLForge.pyfunc.spark_udf(
            spark,
...
def test_spark_udf_return_nullable_array_field(spark):
    ...
    with MLForge.start_run():
        MLForge_info = MLForge.pyfunc.log_model(
            name="model",
            python_model=TestModel(),
        )
        udf = MLForge.pyfunc.spark_udf(
            spark,
            MLForge_info.model_uri,
...
def test_spark_udf_with_params(spark):
    ...
    signature = MLForge.models.infer_signature(["input"], params=test_params)
    spark_df = spark.createDataFrame(
        [
            ("input1",),
            ("input2",),
...
def test_spark_udf_with_params_with_errors(spark):
    ...
    with MLForge.start_run() as run:
        ...
        with pytest.raises(MLForgeException, match=r"Invalid 'spark_udf' result type"):
            MLForge.pyfunc.spark_udf(
                spark,
                f"runs:/{run.info.run_id}/model",
...
def test_spark_udf_compatible_with_MLForge_2_4_0(tmp_path, spark):
    """
    # Code for logging the model in MLForge 2.4.0
    import MLForge
    ...
    signature = MLForge.models.infer_signature(["input"])
    with MLForge.start_run() as run:
        MLForge.pyfunc.log_model(
            "model",
            python_model=TestModel(),
...
flavors:
  python_function:
    ...
    loader_module: MLForge.pyfunc.model
...
def test_spark_df_schema_inference_for_map_type(spark):
    ...
    with pytest.raises(
        MLForgeException, match=r"Please construct spark DataFrame with schema using StructType"
    ):
...
def test_spark_udf_structs_and_arrays(spark, tmp_path):
    ...
    MLForge.pyfunc.save_model(
        ...
        signature=MLForge.models.infer_signature(df),
    )

    udf = MLForge.pyfunc.spark_udf(spark=spark, model_uri=save_path, result_type="string")
...
def test_spark_udf_infer_return_type(spark, tmp_path):
    ...
    signature = MLForge.models.infer_signature(df, df)
    MLForge.pyfunc.save_model(
        path=tmp_path,
        python_model=MyModel(),
        signature=signature,
    ...
    udf = MLForge.pyfunc.spark_udf(spark=spark, model_uri=tmp_path)
...
def test_spark_udf_env_manager_with_invalid_pythonpath(
    ...
    with mock.patch("MLForge.utils.databricks_utils.is_in_databricks_runtime", return_value=True):
...
def test_build_model_env(spark, sklearn_model, model_path, tmp_path, monkeypatch):
    ...
    from MLForge.pyfunc.dbconnect_artifact_cache import extract_archive_to_dir
    ...
    with MLForge.start_run():
        model_info = MLForge.sklearn.log_model(
            ...
            pip_requirements=[
                ...
                # `build_model_env` doesn't support building env with dev version MLForge,
                # so add MLForge as a required dependency here.
                "MLForge",
            ],
        )
...
class CustomModelWithMLForgeConfig(MLForge.pyfunc.PythonModel):
    def predict(self, context, model_input, params=None):
...
def test_spark_udf_with_model_config(spark, model_path, monkeypatch, env_manager, use_stdin_serve):
    model = CustomModelWithMLForgeConfig()
    MLForge.pyfunc.save_model(
        model_path,
        python_model=model,
    ...
    with mock.patch("MLForge.pyfunc.check_port_connectivity", return_value=(not use_stdin_serve)):
        udf = MLForge.pyfunc.spark_udf(
            spark,
            model_path,
            result_type="long",
...
def test_spark_udf_model_server_timeout(spark, monkeypatch):
    monkeypatch.setenv("MLForge_SCORING_SERVER_REQUEST_TIMEOUT", "1")
    ...
    udf = MLForge.pyfunc.spark_udf(
        ...
        env_manager="virtualenv",
        extra_env={
            "MLForge_SCORING_SERVER_REQUEST_TIMEOUT": str(
                MLForge_SCORING_SERVER_REQUEST_TIMEOUT.get()
            )
        },
    )
    # Raised from MLForge.pyfunc.scoring_server.client.StdinScoringServerClient,
...
def test_spark_udf_preserve_model_output_type(spark, numpy_type, schema, value):
    ...
    signature = MLForge.models.infer_signature(numpy_type(value), numpy_type(value))
    with MLForge.start_run():
        model_info = MLForge.pyfunc.log_model(
            name="model",
            python_model=TestModel(),
...
def _make_sandbox_info(runtime_version):
    from MLForge.utils.databricks_utils import DBConnectUDFSandboxInfo
...
def test_spark_udf_dbconnect_version_gate_passes(runtime_version):
    ...
    with (
        mock.patch("MLForge.pyfunc.is_databricks_connect", return_value=True) as mock_dbconnect,
        mock.patch("MLForge.pyfunc.is_in_databricks_shared_cluster_runtime", return_value=False),
        mock.patch("MLForge.pyfunc.is_in_databricks_serverless_runtime", return_value=False),
        mock.patch("MLForge.pyfunc.is_in_databricks_runtime", return_value=True),
        mock.patch(
            "MLForge.pyfunc.get_dbconnect_udf_sandbox_info",
            return_value=_make_sandbox_info(runtime_version),
        ) as mock_sandbox,
        mock.patch("MLForge.pyfunc.get_nfs_cache_root_dir", side_effect=_GatePassed("gate passed")),
    ):
        spark = mock.MagicMock()
        spark.conf.get.return_value = "false"
        with pytest.raises(_GatePassed, match="gate passed"):
            MLForge.pyfunc.spark_udf(spark, "models:/m/1", env_manager="local")
...
def test_spark_udf_dbconnect_version_gate_rejects_old_runtime():
    with (
        ...
        spark = mock.MagicMock()
        spark.conf.get.return_value = "false"
        with pytest.raises(MLForgeException, match="requires Databricks runtime version >= 15.4"):
            MLForge.pyfunc.spark_udf(spark, "models:/m/1", env_manager="local")
        mock_dbconnect.assert_called_once()
        mock_sandbox.assert_called_once()

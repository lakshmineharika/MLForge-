...
from MLForge.models.utils import (
    _enforce_params_schema,
    _enforce_schema,
)
from MLForge.pyfunc import PyFuncModel
from MLForge.pyfunc.scoring_server import is_unified_llm_input
from MLForge.tracking.artifact_utils import _download_artifact_from_uri
from MLForge.types import ColSpec, DataType, ParamSchema, ParamSpec, Schema, TensorSpec
from MLForge.types.schema import AnyType, Array, Map, Object, Property
from MLForge.utils.proto_json_utils import dump_input_data
...
class PythonModelWithBasicParams(MLForge.pyfunc.PythonModel):
...
class PythonModelWithArrayParams(MLForge.pyfunc.PythonModel):
    def predict(self, context, model_input, params=None):
        assert isinstance(params, dict)
...
def test_column_schema_enforcement():
    ...
    with pytest.raises(MLForgeException, match=match_missing_inputs):
    ...
    # MLForge datetime type in input_schema does not encode precision, so add it for assertions
    ...
    with pytest.raises(MLForgeException, match=match_incompatible_inputs):
...
def test_tensor_multi_named_schema_enforcement():
    ...
    # test that missing column raises
    inp1 = inp.copy()
    with pytest.raises(MLForgeException, match="Model is missing inputs"):
    ...
    with pytest.raises(
        MLForgeException, match="dtype of input int32 does not match expected dtype uint64"
    ):
    ...
    with pytest.raises(
        MLForgeException, match=re.escape("Model is missing inputs ['a', 'b', 'c'].")
    ):
    ...
    inp7["a"] = np.array([])
    with pytest.raises(
        MLForgeException, match=re.escape("Shape of input (0,) does not match expected shape")
    ):
...
def test_missing_value_hint_is_displayed_when_it_should():
    ...
    with pytest.raises(MLForgeException, match=match) as ex:
        pyfunc_model.predict(pdf)
...
def test_column_schema_enforcement_no_col_names():
    ...
    with pytest.raises(MLForgeException, match="the provided value only has 2 inputs."):
        pyfunc_model.predict([[1.0, 2.0]])

    # Must provide the right types
    with pytest.raises(MLForgeException, match="Can not safely convert int64 to float64"):
        pyfunc_model.predict([[1, 2, 3]])

    # Can only provide data type that can be converted to dataframe...
    with pytest.raises(MLForgeException, match="Expected input to be DataFrame. Found: set"):
        pyfunc_model.predict({1, 2, 3})
...
def test_tensor_schema_enforcement_no_col_names():
    ...
    with pytest.raises(
        MLForgeException, match="dtype of input uint32 does not match expected dtype float32"
    ):
    ...
    with pytest.raises(
        MLForgeException, match=re.escape("Shape of input () does not match expected shape (-1, 3)")
    ):
...
def test_schema_enforcement_for_inputs_style_orientation_of_dataframe(orient):
    ...
    # Test Dict[str, <scalar>] (support added in MLForge 2.3.0)
    test_signature = {
        "inputs": '[{"name": "a", "type": "long"}, {"name": "b", "type": "string"}]',
        "outputs": '[{"name": "response", "type": "string"}]',
    }
    ...
    with pytest.raises(MLForgeException, match="This model contains a column-based"):
        _enforce_schema(data, signature.inputs)
    with pytest.raises(MLForgeException, match="Incompatible input types for column a. Can not"):
...
def test_schema_enforcement_for_optional_columns():
    ...
    with pytest.raises(MLForgeException, match="Incompatible input types for column d."):
...
def test_enforce_params_schema_errors():
    ...
    with pytest.raises(
        MLForgeException, match=r"Failed to validate type and shape for 'float_param'"
    ):
    ...
    with pytest.raises(
        MLForgeException, match=r"Failed to validate type and shape for 'float_array'"
    ):
        _enforce_params_schema(
    ...
    with pytest.raises(MLForgeException, match=r"Parameters must be a dictionary. Got type 'int'."):
    ...
    with pytest.raises(MLForgeException, match=r"received list with ndim 2"):
        _enforce_params_schema(
...
def test_enforce_params_schema_errors_with_model_with_params():
    ...
    loaded_model_with_params = MLForge.pyfunc.load_model(model_info.model_uri)
    with pytest.raises(MLForgeException, match=r"Parameters must be a dictionary. Got type 'list'"):
...
def test_param_spec_errors():
    ...
    with pytest.raises(MLForgeException, match=r"Failed to validate type and shape for 'a'"):
    ...
    with pytest.raises(MLForgeException, match=r"Binary type is not supported for parameters"):
        ParamSpec("a", DataType.binary, 1.0)
    with pytest.raises(MLForgeException, match=r"Failed to convert value"):
        ParamSpec("a", DataType.datetime, 1.0)
    with pytest.raises(MLForgeException, match=r"Failed to convert value"):
        ParamSpec("a", DataType.datetime, [1.0, 2.0], (-1,))
    with pytest.raises(MLForgeException, match=r"Failed to convert value to `DataType.datetime`"):
...
def test_python_model_serving_compatible(tmp_path):
    ...
    import MLForge
    from MLForge.models import infer_signature
...
{"data": [["input"]]}
    ...
    from MLForge.pyfunc.model import _FunctionPythonModel
...
def test_enforce_schema_with_arrays_in_python_model_predict(sample_params_with_arrays):
    ...
    with pytest.raises(MLForgeException, match=r"Failed to validate type and shape for 'int_array'"):
    ...
    with pytest.raises(
        MLForgeException, match=r"Failed to validate type and shape for 'double_array'"
    ):
...
def test_pyfunc_model_input_example_with_params(
    ...
    MLForge_model = Model.load(os.path.join(local_path, "MLmodel"))
    loaded_example = MLForge_model.load_input_example(local_path)
    ...
    for test_example in ["saved_example", "manual_example"]:
        if test_example == "saved_example":
            payload = MLForge_model.get_serving_input(local_path)
        else:
            if isinstance(example, pd.DataFrame):
...
def test_invalid_input_example_warn_when_model_logging():
    ...
    with mock.patch("MLForge.models.model._logger.warning") as mock_warning:
        with MLForge.start_run():
...
def test_input_example_validation_during_logging(
    ...
    from MLForge.models import validate_serving_input
    ...
    local_path = _download_artifact_from_uri(model_info.model_uri, output_path=tmp_path)
    serving_input_example = MLForge_model.get_serving_input(local_path)
    response = score_model_in_process(
        model_info.model_uri,
...
# `instances` is an invalid key for schema with MLForge < 2.9.0
...
# `inputs`` is an invalid key for schema with MLForge < 2.9.0

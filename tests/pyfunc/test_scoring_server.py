...
from MLForge.models import ModelSignature, infer_signature
from MLForge.protos.databricks_pb2 import BAD_REQUEST, ErrorCode
from MLForge.pyfunc import PythonModel
from MLForge.pyfunc.scoring_server import _get_jsonable_obj, get_cmd
from MLForge.types import ColSpec, DataType, ParamSchema, ParamSpec, Schema
from MLForge.types.schema import Array, Object, Property
from MLForge.utils import env_manager as _EnvManager
from MLForge.utils.file_utils import TempDir
from MLForge.utils.proto_json_utils import NumpyEncoder
from MLForge.version import VERSION
...
def build_and_save_sklearn_model(model_path):
    ...
    MLForge.sklearn.save_model(model, path=model_path)
...
def test_scoring_server_responds_to_malformed_json_input_with_error_code_and_message(
    ...
    MLForge.sklearn.save_model(sk_model=sklearn_model.model, path=model_path)
...
def test_scoring_server_successfully_evaluates_correct_tf_serving_keras_instances(
    ...
    MLForge.tensorflow.save_model(keras_model.model, path=model_path)
...
def test_serving_model_with_param_schema(sklearn_model, model_path):
    ...
    MLForge.sklearn.save_model(sk_model=sklearn_model.model, path=model_path, signature=signature)
...
def test_get_cmd(args: dict[str, Any], expected: str, timeout: str):
    ...
    assert cmd == (f"uvicorn {expected} MLForge.pyfunc.scoring_server.app:app")
    assert env[MLForge_SCORING_SERVER_REQUEST_TIMEOUT.name] == timeout
...
def test_scoring_server_client(sklearn_model, model_path):
    ...
    from MLForge.utils import find_free_port
    ...
    try:
        server_proc = get_flavor_backend(
            model_path, env_manager=_EnvManager.CONDA, workers=1, install_MLForge=False
        ).serve(
...
def test_scoring_server_allows_payloads_with_llm_chat_keys_for_pyfunc(
    ...
    MLForge.pyfunc.save_model(model_path, python_model=MyChatLLM(), signature=signature)
...
def test_scoring_server_allows_payloads_with_llm_completions_keys_for_pyfunc(
    ...
    MLForge.pyfunc.save_model(model_path, python_model=MyCompletionsLLM(), signature=signature)
...
def test_scoring_server_allows_payloads_with_llm_embeddings_keys_for_pyfunc(
    ...
    MLForge.pyfunc.save_model(model_path, python_model=MyEmbeddingsLLM(), signature=signature)
...
def test_scoring_server_allows_payloads_with_messages_for_pyfunc_wrapped(model_path):
    ...
    class WrappedSklearn(PythonModel):
        def load_context(self, context):
            self.model = MLForge.pyfunc.load_model(context.artifacts["model_path"])

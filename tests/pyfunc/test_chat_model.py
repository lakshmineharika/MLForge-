...
from MLForge.models.utils import load_serving_example
from MLForge.pyfunc.loaders.chat_model import _ChatModelPyfuncWrapper
from MLForge.tracing.constant import TraceTagKey
from MLForge.tracking.artifact_utils import _download_artifact_from_uri
from MLForge.types.llm import (
...
from MLForge.types.schema import ColSpec, DataType, Schema
...
class SimpleChatModel(MLForge.pyfunc.ChatModel):
    def predict(
...
class ChatModelWithContext(MLForge.pyfunc.ChatModel):
...
class ChatModelWithTrace(MLForge.pyfunc.ChatModel):
    @MLForge.trace
    def predict(
        self, context, messages: list[ChatMessage], params: ChatParams
    ) -> ChatCompletionResponse:
...
class ChatModelWithMetadata(MLForge.pyfunc.ChatModel):
    def predict(
        self, context, messages: list[ChatMessage], params: ChatParams
    ) -> ChatCompletionResponse:
        mock_response = get_mock_response(messages, params)
...
class ChatModelWithToolCalling(MLForge.pyfunc.ChatModel):
    def predict(
        self, context, messages: list[ChatMessage], params: ChatParams
    ) -> ChatCompletionResponse:
        tools = params.tools
...
def test_chat_model_save_load(tmp_path):
    ...
    MLForge.pyfunc.save_model(python_model=model, path=tmp_path)
...
def test_chat_model_save_throws_with_signature(tmp_path):
    ...
    with pytest.raises(MLForgeException, match="Please remove the `signature` parameter"):
...
def test_save_throws_on_invalid_output(tmp_path, ret):
    class BadChatModel(MLForge.pyfunc.ChatModel):
...
def test_chat_model_works_with_infer_signature_input_example(tmp_path):
    ...
    assert MLForge_model.load_input_example(local_path) == {
...
def test_chat_model_logs_default_metadata_task(tmp_path):
    ...
    with MLForge.start_run():
        model_info_with_override = MLForge.pyfunc.log_model(
...
def test_chat_model_without_context_in_predict():
    ...
    class Model(MLForge.pyfunc.ChatModel):

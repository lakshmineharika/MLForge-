...
class SimplePythonModel(MLForge.pyfunc.PythonModel):
...
def test_pyfunc_log_model_copies_uv_artifacts(tmp_uv_project, python_model, monkeypatch):
    ...
    with MLForge.start_run() as run:
        MLForge.pyfunc.log_model(name="model", python_model=python_model)
...
@requires_uv
def test_pyfunc_log_model_respects_MLForge_log_uv_files_env_var(
    tmp_uv_project, python_model, monkeypatch
):
    monkeypatch.chdir(tmp_uv_project)
    monkeypatch.setenv("MLForge_UV_AUTO_DETECT", "true")
...
def test_load_pyfunc_model_with_uv_artifacts_and_predict(tmp_uv_project, python_model, monkeypatch):
    ...
    with MLForge.start_run() as run:
        ...
        loaded_model = MLForge.pyfunc.load_model(model_uri)
...
def test_pyfunc_save_model_with_uv_project(tmp_uv_project, python_model, tmp_path, monkeypatch):
    ...
    MLForge.pyfunc.save_model(model_path, python_model=python_model)
...
def test_MLForge_log_uv_files_env_var_false_variants(
...
def test_MLForge_log_uv_files_env_var_true_variants(
    tmp_uv_project, python_model, monkeypatch, env_value
):
    monkeypatch.chdir(tmp_uv_project)
    monkeypatch.setenv("MLForge_UV_AUTO_DETECT", "true")
...
def test_export_uv_requirements_with_groups_real(uv_project_with_groups):
    from MLForge.utils.uv_utils import export_uv_requirements
...
def test_setup_uv_sync_environment_real(tmp_uv_project, tmp_path):
    from MLForge.utils.uv_utils import has_uv_lock_artifact, setup_uv_sync_environment
    ...
    assert 'name = "MLForge-model-env"' in pyproject_content
    assert 'requires-python = "==3.11.5"' in pyproject_content


@requires_uv
def test_extract_index_urls_from_real_uv_lock(tmp_uv_project):
    from MLForge.utils.uv_utils import extract_index_urls_from_uv_lock
...
def test_run_uv_sync_real(tmp_uv_project, tmp_path):
    from MLForge.utils.uv_utils import run_uv_sync

...
def pyfunc_custom_env_file(tmp_path):
    ...
    _MLForge_conda_env(
        ...
        additional_pip_deps=[
            ...
            "pytest",
            "cloudpickle",
            "-e " + os.path.dirname(MLForge.__path__[0]),
        ],
    )
    return conda_env
...
def test_model_save_load(sklearn_knn_model, iris_data, tmp_path, model_path):
    ...
    assert MLForge.pyfunc.FLAVOR_NAME in reloaded_model_config.flavors
    assert MLForge.pyfunc.PY_VERSION in reloaded_model_config.flavors[MLForge.pyfunc.FLAVOR_NAME]
    reloaded_model = MLForge.pyfunc.load_model(model_path)
...
def test_model_log_load(sklearn_knn_model, iris_data, tmp_path):
    ...
    with MLForge.start_run():
        ...
        pyfunc_model_path = _download_artifact_from_uri(
            f"runs:/{MLForge.active_run().info.run_id}/{pyfunc_artifact_path}"
        )

    model_config = Model.load(os.path.join(pyfunc_model_path, "MLmodel"))
    assert MLForge.pyfunc.FLAVOR_NAME in model_config.flavors
    assert MLForge.pyfunc.PY_VERSION in model_config.flavors[MLForge.pyfunc.FLAVOR_NAME]
    reloaded_model = MLForge.pyfunc.load_model(pyfunc_model_path)
...
def test_save_model_with_unsupported_argument_combinations_throws_exception(model_path):
    with pytest.raises(
        MLForgeException, match="Either `loader_module` or `python_model` must be specified"
    ):
        MLForge.pyfunc.save_model(path=model_path, data_path="/path/to/data")
...
def test_log_model_with_unsupported_argument_combinations_throws_exception():
    with (
        ...
        MLForge.pyfunc.log_model(name="pyfunc_model", data_path="/path/to/data")


def test_log_model_persists_specified_conda_env_file_in_MLForge_model_directory(
...
def test_log_model_persists_specified_conda_env_dict_in_MLForge_model_directory(
...
def test_log_model_persists_requirements_in_MLForge_model_directory(
    sklearn_knn_model, tmp_path, pyfunc_custom_env_dict
):
    sk_model_path = os.path.join(tmp_path, "knn.pkl")
    with open(sk_model_path, "wb") as f:
...
def test_log_model_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    _assert_pip_requirements(model_info.model_uri, MLForge.pyfunc.get_default_pip_requirements())
...
def test_streamable_model_save_load(tmp_path, model_path):
    ...
    loaded_pyfunc_model = MLForge.pyfunc.load_model(model_uri=model_path)
...
def test_log_loader_module_model_does_not_emit_pickle_warning(sklearn_knn_model, tmp_path):
    ...
    with MLForge.start_run(), mock.patch("MLForge.pyfunc._logger.warning") as mock_log_warning:
        MLForge.pyfunc.log_model(
            name="pyfunc_model",
            data_path=sk_model_path,
            loader_module=__name__,

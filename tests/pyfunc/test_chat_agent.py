...
from MLForge.pyfunc.loaders.chat_agent import _ChatAgentPyfuncWrapper
from MLForge.pyfunc.model import ChatAgent
from MLForge.tracing.constant import TraceTagKey
from MLForge.tracking.artifact_utils import _download_artifact_from_uri
from MLForge.types.agent import (
...
def test_chat_agent_works_with_infer_signature_input_example():
    ...
    loaded_input_example = MLForge_model.load_input_example(local_path)
...
def test_chat_agent_works_with_chat_agent_request_input_example():
    ...
    MLForge_model = Model.load(model_info.model_uri)
    local_path = _download_artifact_from_uri(model_info.model_uri)
    assert MLForge_model.load_input_example(local_path) == input_example_no_params
    ...
    MLForge_model = Model.load(model_info.model_uri)
    local_path = _download_artifact_from_uri(model_info.model_uri)
    assert MLForge_model.load_input_example(local_path) == input_example_with_params
...
def test_chat_agent_predict_wrapper():
    ...
    with pytest.raises(MLForgeException, match="Invalid dictionary input for a ChatAgent"):
...
def test_chat_agent_load_context_called_during_save(tmp_path):
    ...
    loaded_model = MLForge.pyfunc.load_model(save_path)

...
from MLForge.models.utils import _enforce_schema
from MLForge.types import ColSpec, DataType, Schema
...
def test_enforce_schema_spark_dataframe_incompatible_type(spark):
    ...
    df = spark.createDataFrame(data, spark_df_schema)
    with pytest.raises(MLForgeException, match="Incompatible input types"):
        _enforce_schema(df, input_schema)
...
def test_enforce_schema_spark_dataframe_incompatible_type_complex(spark):
    ...
    df = spark.createDataFrame(data, spark_df_schema)
    with pytest.raises(MLForgeException, match="Failed to enforce schema"):
        _enforce_schema(df, input_schema)

...
from MLForge.pyfunc import get_model_dependencies
...
scikit-learn==1.0.2"""
    ...
    with mock.patch("MLForge.pyfunc._logger.info") as mock_log_info:
        ...
        with mock.patch("MLForge.pyfunc._is_in_ipython_notebook", return_value=True):
            get_model_dependencies(model_path, format="pip")
            mock_log_info.assert_called_once_with(
                "To install the dependencies that were used to train the model, run the "
                f"following command: '%pip install -r {req_file}'."
    ...
    with pytest.raises(MLForgeException, match="Illegal format argument 'abc'"):
...
"""
    ...
    with pytest.raises(MLForgeException, match="No pip section found in conda.yaml file"):
        get_model_dependencies(model_path, format="pip")


def test_get_model_dependencies_with_model_version_uri():
    with MLForge.start_run():
        MLForge.sklearn.log_model(LinearRegression(), name="model", registered_model_name="linear")

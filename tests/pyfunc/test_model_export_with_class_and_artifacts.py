...
import MLForge.pyfunc.model
import MLForge.pyfunc.scoring_server as pyfunc_scoring_server
import MLForge.sklearn
from MLForge.entities import Trace
from MLForge.environment_variables import (
    MLForge_ALLOW_PICKLE_DESERIALIZATION,
    MLForge_LOG_MODEL_COMPRESSION,
    MLForge_RECORD_ENV_VARS_IN_MODEL_LOGGING,
)
from MLForge.exceptions import MLForgeException
from MLForge.models import Model, infer_signature
from MLForge.models.auth_policy import AuthPolicy, SystemAuthPolicy, UserAuthPolicy
from MLForge.models.dependencies_schemas import DependenciesSchemasType
from MLForge.models.model import _DATABRICKS_FS_LOADER_MODULE
from MLForge.models.resources import (
    DatabricksApp,
    DatabricksFunction,
...
from MLForge.models.utils import _read_example
from MLForge.pyfunc.context import Context, set_prediction_context
from MLForge.pyfunc.model import _load_pyfunc
...
from MLForge.tracking.artifact_utils import (
    _download_artifact_from_uri,
)
from MLForge.types.schema import ColSpec, Map, Schema
from MLForge.types.type_hints import _infer_schema_from_list_type_hint
from MLForge.utils.environment import _MLForge_conda_env
from MLForge.utils.file_utils import TempDir
from MLForge.utils.model_utils import _get_flavor_configuration
from MLForge.utils.requirements_utils import _get_installed_version
...
def get_model_class():
    ...
    class CustomSklearnModel(MLForge.pyfunc.PythonModel):
        ...
        def load_context(self, context):
            ...
            self.model = (
                MLForge.sklearn.load_model(model_uri=context.artifacts["sk_model"])
...
def _conda_env():
    # NB: We need MLForge as a dependency in the environment.
    return _MLForge_conda_env(
        additional_pip_deps=[
...
def test_model_save_load(sklearn_knn_model, main_scoped_model_class, iris_data, tmp_path):
    ...
    MLForge.sklearn.save_model(sk_model=sklearn_knn_model, path=sklearn_model_path)
    ...
    loaded_pyfunc_model = MLForge.pyfunc.load_model(model_uri=pyfunc_model_path)
...
@pytest.mark.skip(
    reason="In MLForge 3.0, `log_model` does not start a run. Consider removing this test."
)
def test_pyfunc_model_log_load_no_active_run(sklearn_knn_model, main_scoped_model_class, iris_data):
    ...
    with MLForge.start_run():
        MLForge.sklearn.log_model(sklearn_knn_model, name=sklearn_artifact_path)
        sklearn_model_uri = f"runs:/{MLForge.active_run().info.run_id}/{sklearn_artifact_path}"
    ...
    pyfunc_model_uri = f"runs:/{MLForge.active_run().info.run_id}/{pyfunc_artifact_path}"
    loaded_pyfunc_model = MLForge.pyfunc.load_model(model_uri=pyfunc_model_uri)
    np.testing.assert_array_equal(
        loaded_pyfunc_model.predict(iris_data[0]),
        test_predict(sk_model=sklearn_knn_model, model_input=iris_data[0]),
    )
...
def test_model_log_load(sklearn_knn_model, main_scoped_model_class, iris_data):
    ...
    with MLForge.start_run():
        sklearn_model_info = MLForge.sklearn.log_model(sklearn_knn_model, name=sklearn_artifact_path)
    ...
    with (
        MLForge.start_run(),
        mock.patch("MLForge.pyfunc._logger.warning") as mock_warning,
    ):
        pyfunc_model_info = MLForge.pyfunc.log_model(
            name=pyfunc_artifact_path,
    ...
    loaded_pyfunc_model = MLForge.pyfunc.load_model(model_uri=pyfunc_model_info.model_uri)
...
def test_python_model_predict_compatible_without_params(sklearn_knn_model, iris_data):
    class CustomSklearnModelWithoutParams(MLForge.pyfunc.PythonModel):
        def __init__(self, predict_fn):
            self.predict_fn = predict_fn
        ...
        def load_context(self, context):
            ...
            self.model = MLForge.sklearn.load_model(model_uri=context.artifacts["sk_model"])
    ...
    sklearn_artifact_path = "sk_model"
    with MLForge.start_run():
        model_info = MLForge.sklearn.log_model(sklearn_knn_model, name=sklearn_artifact_path)
    ...
    for signature in (None, signature_):
        for example in (None, example_):
            with TempDir() as tmp:
                ...
                MLForge_model = Model.load(path)
                if signature is not None:
                    assert MLForge_model.signature == signature
                elif example is None:
                    assert MLForge_model.signature is None
                if example is None:
                    assert MLForge_model.saved_input_example_info is None
                else:
                    np.testing.assert_array_equal(_read_example(MLForge_model, path), example)
...
def test_log_model_calls_register_model(sklearn_knn_model, main_scoped_model_class):
    with MLForge.start_run():
        with mock.patch(
            "MLForge.tracking._model_registry.fluent._register_model"
...
def test_model_load_from_remote_uri_succeeds(
    ...
    loaded_pyfunc_model = MLForge.pyfunc.load_model(model_uri=model_uri)
    np.testing.assert_array_equal(
        loaded_pyfunc_model.predict(iris_data[0]),
        test_predict(sk_model=sklearn_knn_model, model_input=iris_data[0]),
    )
...
def test_add_to_model_adds_specified_kwargs_to_mlmodel_configuration():
    ...
    assert MLForge.pyfunc.FLAVOR_NAME in model_config.flavors
    assert all(item in model_config.flavors[MLForge.pyfunc.FLAVOR_NAME] for item in custom_kwargs)
...
def test_save_model_persists_specified_conda_env_in_MLForge_model_directory(
    sklearn_knn_model, main_scoped_model_class, pyfunc_custom_env, tmp_path
):
    sklearn_model_path = os.path.join(tmp_path, "sklearn_model")
    MLForge.sklearn.save_model(
        sk_model=sklearn_knn_model,
        path=sklearn_model_path,
        serialization_format=MLForge.sklearn.SERIALIZATION_FORMAT_CLOUDPICKLE,
    )
    ...
    pyfunc_conf = _get_flavor_configuration(
        model_path=pyfunc_model_path, flavor_name=MLForge.pyfunc.FLAVOR_NAME
    )
    saved_conda_env_path = os.path.join(pyfunc_model_path, pyfunc_conf[MLForge.pyfunc.ENV]["conda"])
...
def test_save_model_persists_requirements_in_MLForge_model_directory(
    sklearn_knn_model, main_scoped_model_class, pyfunc_custom_env, tmp_path
):
    sklearn_model_path = os.path.join(tmp_path, "sklearn_model")
    MLForge.sklearn.save_model(
...
def test_log_model_with_pip_requirements(sklearn_knn_model, main_scoped_model_class, tmp_path):
    ...
    with MLForge.start_run():
        ...
        _assert_pip_requirements(
            model_info.model_uri,
            [expected_MLForge_version, "b", "-c constraints.txt"],
            ["a"],
            strict=True,
        )
...
def test_log_model_with_extra_pip_requirements(
    ...
    python_model = main_scoped_model_class(predict_fn=None)
    default_reqs = MLForge.pyfunc.get_default_pip_requirements()
    ...
    with MLForge.start_run():
        ...
        _assert_pip_requirements(
            model_info.model_uri,
            [expected_MLForge_version, *default_reqs, "a"],
        )
    ...
    with MLForge.start_run():
        ...
        _assert_pip_requirements(
            model_info.model_uri,
            [expected_MLForge_version, *default_reqs, "a", "b"],
        )
    ...
    with MLForge.start_run():
        ...
        _assert_pip_requirements(
            model_info.model_uri,
            [expected_MLForge_version, *default_reqs, "b", "-c constraints.txt"],
            ["a"],
        )


def test_log_model_persists_specified_conda_env_in_MLForge_model_directory(
...
def test_model_log_persists_requirements_in_MLForge_model_directory(
    sklearn_knn_model, main_scoped_model_class, pyfunc_custom_env
):
    sklearn_artifact_path = "sk_model"
    with MLForge.start_run():
...
def test_save_model_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    sklearn_model_path = os.path.join(tmp_path, "sklearn_model")
    MLForge.sklearn.save_model(sk_model=sklearn_logreg_model, path=sklearn_model_path)
    ...
    _assert_pip_requirements(pyfunc_model_path, MLForge.pyfunc.get_default_pip_requirements())
...
def test_log_model_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    _assert_pip_requirements(
        pyfunc_model_info.model_uri, MLForge.pyfunc.get_default_pip_requirements()
    )
...
def test_save_model_correctly_resolves_directory_artifact_with_nested_contents(
    ...
    class ArtifactValidationModel(MLForge.pyfunc.PythonModel):
        def predict(self, context, model_input, params=None):
    ...
    loaded_model = MLForge.pyfunc.load_model(model_uri=model_path)
...
def test_save_model_with_no_artifacts_does_not_produce_artifacts_dir(model_path):
    ...
    pyfunc_conf = _get_flavor_configuration(
        model_path=model_path, flavor_name=MLForge.pyfunc.FLAVOR_NAME
    )
    assert MLForge.pyfunc.model.CONFIG_KEY_ARTIFACTS not in pyfunc_conf
...
def test_save_model_with_python_model_argument_of_invalid_type_raises_exception(
    ...
    with pytest.raises(
        ...
        MLForge.pyfunc.save_model(path=os.path.join(tmp_path, "model1"), python_model=5)
    ...
    with pytest.raises(MLForgeException, match="The provided model path"):
        MLForge.pyfunc.save_model(
...
def test_save_model_with_unsupported_argument_combinations_throws_exception(model_path):
    ...
    with pytest.raises(
        ...
        MLForge.pyfunc.save_model(path=model_path, python_model=None, loader_module=None)
...
def test_log_model_with_unsupported_argument_combinations_throws_exception():
    ...
    with MLForge.start_run(), pytest.raises(MLForgeException, match=match):
        MLForge.pyfunc.log_model(
            name="pyfunc_model",
            artifacts={"artifact": "/path/to/artifact"},
            python_model=None,
    ...
    with (
        ...
        MLForge.pyfunc.log_model(name="pyfunc_model", python_model=None, loader_module=None)
...
def test_repr_can_be_called_without_run_id_or_artifact_path():
    ...
    assert "flavor: someFlavour" in MLForge.pyfunc.PyFuncModel(model_meta, model_impl).__repr__()
...
def test_load_model_with_differing_cloudpickle_version_at_micro_granularity_logs_warning(
    ...
    MLForge.pyfunc.save_model(path=model_path, python_model=TestModel())
    saver_cloudpickle_version = "0.5.8"
    model_config_path = os.path.join(model_path, "MLmodel")
    model_config = Model.load(model_config_path)
    model_config.flavors[MLForge.pyfunc.FLAVOR_NAME][
        MLForge.pyfunc.model.CONFIG_KEY_CLOUDPICKLE_VERSION
    ...
    with (
        mock.patch("MLForge.pyfunc._logger.warning") as warn_mock,
        ...
        MLForge.pyfunc.load_model(model_uri=model_path)
...
def test_load_model_with_missing_cloudpickle_version_logs_warning(model_path):
    ...
    model_config_path = os.path.join(model_path, "MLmodel")
    model_config = Model.load(model_config_path)
    del model_config.flavors[MLForge.pyfunc.FLAVOR_NAME][
        MLForge.pyfunc.model.CONFIG_KEY_CLOUDPICKLE_VERSION
    ]
    model_config.save(model_config_path)
    ...
    with mock.patch("MLForge.pyfunc._logger.warning") as warn_mock:
        warn_mock.side_effect = custom_warn
        MLForge.pyfunc.load_model(model_uri=model_path)
...
def test_load_cloudpickle_model_raises_when_pickle_deserialization_disallowed(
    ...
    MLForge.pyfunc.save_model(path=model_path, python_model=TestModel())
    monkeypatch.setenv(MLForge_ALLOW_PICKLE_DESERIALIZATION.name, "false")

    with pytest.raises(MLForgeException, match="Deserializing model using pickle is disallowed"):
        MLForge.pyfunc.load_model(model_uri=model_path)
...
def test_model_with_code_path_containing_main(tmp_path):
    ...
    with MLForge.start_run():
        model_info = MLForge.pyfunc.log_model(
            name="model",
            python_model=MLForge.pyfunc.model.PythonModel(),
...
def test_model_save_load_with_metadata(tmp_path):
    ...
    reloaded_model = MLForge.pyfunc.load_model(model_uri=pyfunc_model_path)
...
def test_model_log_with_metadata():
    ...
    reloaded_model = MLForge.pyfunc.load_model(model_uri=pyfunc_model_uri)
    assert reloaded_model.metadata.metadata["metadata_key"] == "metadata_value"


class SklearnModel(MLForge.pyfunc.PythonModel):
...
def test_dependency_inference_does_not_exclude_MLForge_dependencies(tmp_path):
    MLForge.pyfunc.save_model(
        path=tmp_path,
...
def test_functional_python_model_no_type_hints(tmp_path):
    ...
    MLForge.pyfunc.save_model(path=tmp_path, python_model=python_model, input_example=[{"a": "b"}])
...
def test_functional_python_model_list_invalid_example(tmp_path, input_example):
    with mock.patch("MLForge.models.signature._logger.warning") as mock_warning:
        MLForge.pyfunc.save_model(
...
def test_functional_python_model_list_dict_to_list_dict():
    ...
    pyfunc_model = MLForge.pyfunc.load_model(model_info.model_uri)
    assert pyfunc_model.predict([{"a": "x", "b": "y"}]) == [{"x": "a", "y": "b"}]


def test_list_dict_with_signature_override():
    class CustomModel(MLForge.pyfunc.PythonModel):
...
def test_functional_python_model_multiple_arguments(tmp_path):
    with pytest.raises(
        MLForgeException, match=r"must accept exactly one argument\. Found 2 arguments\."
    ):
        MLForge.pyfunc.save_model(path=tmp_path, python_model=multiple_arguments)
...
def test_functional_python_model_no_arguments(tmp_path):
    with pytest.raises(
        MLForgeException, match=r"must accept exactly one argument\. Found 0 arguments\."
    ):
        MLForge.pyfunc.save_model(path=tmp_path, python_model=no_arguments)
...
def test_functional_python_model_infer_requirements(tmp_path):
    MLForge.pyfunc.save_model(path=tmp_path, python_model=requires_sklearn, input_example=["a"])
...
def test_functional_python_model_throws_when_required_arguments_are_missing(tmp_path):
    ...
    with pytest.raises(MLForgeException, match="at least one of"):
        MLForge.pyfunc.save_model(path=tmp_path / uuid.uuid4().hex, python_model=requires_sklearn)


class AnnotatedPythonModel(MLForge.pyfunc.PythonModel):
...
def test_class_python_model_type_hints(tmp_path):
    MLForge.pyfunc.save_model(path=tmp_path, python_model=AnnotatedPythonModel())
    model = Model.load(tmp_path)
    ...
    model = MLForge.pyfunc.load_model(tmp_path)
...
def test_load_model_fails_for_feature_store_models(tmp_path):
    ...
    with pytest.raises(
        MLForgeException,
        match="Note: MLForge.pyfunc.load_model is not supported for Feature Store models",
    ):
        MLForge.pyfunc.load_model(f"runs:/{run.info.run_id}/model")
...
def test_streamable_model_save_load(iris_data, tmp_path):
    class StreamableModel(MLForge.pyfunc.PythonModel):
        def __init__(self):
            pass
...
def test_pyfunc_as_code_with_dependencies_store_dependencies_schemas_in_trace(
    ...
    monkeypatch.setenv("ENABLE_MLForge_TRACING", "true")
...
def test_model_pip_requirements_pin_numpy_when_pandas_included():
    ...
    with (
        MLForge.start_run(),
        mock.patch(
            "MLForge.utils.requirements_utils._get_installed_version",
    ...
    with MLForge.start_run():
        model_info = MLForge.pyfunc.log_model(name="model", python_model=TestModel())
        _assert_pip_requirements(
            model_info.model_uri,
            [expected_MLForge_version, f"cloudpickle=={cloudpickle.__version__}"],
            strict=True,
        )
...
def test_environment_variables_used_during_model_logging(monkeypatch):
    ...
    with MLForge.start_run():
        model_info = MLForge.pyfunc.log_model(name="model", python_model=MyModel())
    assert model_info.env_vars is None

    # disable logging by setting environment variable
    monkeypatch.setenv(MLForge_RECORD_ENV_VARS_IN_MODEL_LOGGING.name, "false")
    with MLForge.start_run():
        model_info = MLForge.pyfunc.log_model(
            name="model", python_model=MyModel(), input_example="data"
        )
...
def test_pyfunc_model_without_context_in_predict():
    ...
    with MLForge.start_run():
        model_info = MLForge.pyfunc.log_model(name="model", python_model=m, input_example="abc")
    pyfunc_model = MLForge.pyfunc.load_model(model_info.model_uri)
...
def test_pyfunc_model_input_example_with_signature():
    ...
    with MLForge.start_run():
        with pytest.warns(
            ...
            MLForge.pyfunc.log_model(name="model", python_model=Model(), signature=signature)

    with MLForge.start_run():
        with pytest.raises(
            MLForgeException, match=r"Input example does not match the model signature"
        ):
            MLForge.pyfunc.log_model(
...
def test_model_save_load_compression(
    ...
    monkeypatch.setenv(MLForge_LOG_MODEL_COMPRESSION.name, compression)
    sklearn_model_path = os.path.join(tmp_path, "sklearn_model")
    MLForge.sklearn.save_model(sk_model=sklearn_knn_model, path=sklearn_model_path)
...
class ExampleModel(MLForge.pyfunc.PythonModel):
    def predict(self, model_input: list[str]) -> list[str]:
        return model_input


def test_lock_model_requirements(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    monkeypatch.setenv("MLForge_LOCK_MODEL_DEPENDENCIES", "true")

    model_info = MLForge.pyfunc.log_model(name="model", python_model=ExampleModel())
    ...
    assert "MLForge==" in requirements_txt_contents

...
def test_spark_udf_spark_connect(spark):
    ...
    with MLForge.start_run():
        info = MLForge.sklearn.log_model(model, name="model")
    sdf = spark.createDataFrame(pd.DataFrame(X, columns=list("abcd")))
    udf = MLForge.pyfunc.spark_udf(spark, info.model_uri, env_manager="local")
...
def test_spark_udf_spark_connect_unsupported_env_manager(spark, tmp_path, env_manager):
    with pytest.raises(
        MLForge.MLForgeException,
        match=f"Environment manager {env_manager!r} is not supported",
    ):
        MLForge.pyfunc.spark_udf(spark, str(tmp_path), env_manager=env_manager)

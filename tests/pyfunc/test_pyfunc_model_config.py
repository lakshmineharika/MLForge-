...
class InferenceContextModel(MLForge.pyfunc.PythonModel):
    def predict(self, context, model_input, params=None):
...
def test_save_with_model_config(model_path, model_config):
    ...
    MLForge.pyfunc.save_model(model_path, python_model=model, model_config=model_config)
...
def test_save_with_model_config_path(model_path, model_config, model_config_path):
    model = InferenceContextModel()
    MLForge.pyfunc.save_model(model_path, python_model=model, model_config=model_config_path)
...
def test_override_model_config(model_path, model_config):
    ...
    MLForge.pyfunc.save_model(model_path, python_model=model, model_config=model_config)
    loaded_model = MLForge.pyfunc.load_model(model_uri=model_path, model_config=inference_override)
...
def test_override_model_config_path(tmp_path, model_path, model_config_path):
    ...
    MLForge.pyfunc.save_model(model_path, python_model=model, model_config=model_config_path)
    loaded_model = MLForge.pyfunc.load_model(model_uri=model_path, model_config=str(config_path))
...
def test_pyfunc_without_model_config(model_path, model_config):
    model = TestModel()
    MLForge.pyfunc.save_model(model_path, python_model=model)

    loaded_model = MLForge.pyfunc.load_model(model_uri=model_path, model_config=model_config)
...
def test_pyfunc_loader_without_model_config(model_path):
    MLForge.pyfunc.save_model(
        ...
        code_paths=[__file__],
        MLForge_model=Model(run_id="test", artifact_path="testtest"),
    )

    inference_override = {"invalid_key": 400}
    pyfunc_model = MLForge.pyfunc.load_model(model_path, model_config=inference_override)

...
# Only import model fixtures if when MLForge_RUN_SLOW_TESTS environment variable is set to true
if _MLForge_RUN_SLOW_TESTS.get():
    ...
    from tests.pyfunc.docker.conftest import (
        ...
        TEST_IMAGE_NAME,
        docker_client,
        save_model_with_latest_MLForge_version,
    )
...
pytestmark = pytest.mark.skipif(
    not _MLForge_RUN_SLOW_TESTS.get(),
    reason="Skip slow tests. Set MLForge_RUN_SLOW_TESTS environment variable to run them.",
)
...
def test_build_image_and_serve(flavor, request):
    ...
    backend.build_image(
        ...
        MLForge_home=MLForge_ROOT,  # Required to prevent installing dev version of MLForge from PyPI
    )
...
MLForge.models.set_model(RunnablePassthrough() | itemgetter("messages"))
"""

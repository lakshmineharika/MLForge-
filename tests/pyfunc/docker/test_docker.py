...
import MLForge
from MLForge.environment_variables import _MLForge_RUN_SLOW_TESTS
from MLForge.models import Model
from MLForge.models.docker_utils import build_image_from_context
from MLForge.models.flavor_backend_registry import get_flavor_backend
from MLForge.utils import PYTHON_VERSION
from MLForge.utils.env_manager import CONDA, LOCAL, VIRTUALENV
from MLForge.version import VERSION

from tests.pyfunc.docker.conftest import RESOURCE_DIR, get_released_MLForge_version


def _get_MLForge_install_specifier():
    if Version(VERSION).is_devrelease:
        return "https://github.com/MLForge/MLForge/archive/refs/heads/master.zip"
    return f"MLForge=={VERSION}"
...
def assert_dockerfiles_equal(actual_dockerfile_path: Path, expected_dockerfile_path: Path):
    ...
    expected_dockerfile = (
        ...
        .read_text()
        .replace("${{ MLForge_INSTALL }}", _get_MLForge_install_specifier())
...
def save_model(tmp_path):
    ...
    MLForge.sklearn.save_model(
        ...
        path=model_path,
        pip_requirements=[
            f"MLForge=={get_released_MLForge_version()}",
...
class Param:
    ...
    MLForge_home: str | None = None
    install_MLForge: bool = False
...
@pytest.mark.parametrize(
    ...
    [
        ...
        Param(install_MLForge=True, expected_dockerfile="Dockerfile_install_MLForge"),
        Param(MLForge_home=".", expected_dockerfile="Dockerfile_with_MLForge_home"),
...
def test_build_image(tmp_path, params):
    ...
    def _build_image_with_copy(context_dir, image_name):
        ...
        if _MLForge_RUN_SLOW_TESTS.get():
            for _ in range(3):
                try:
    ...
    with mock.patch(
        "MLForge.models.docker_utils.build_image_from_context",
        side_effect=_build_image_with_copy,
    ):
        backend.build_image(
            model_uri=model_uri,
            image_name="test_image",
            MLForge_home=params.MLForge_home,
            install_MLForge=params.install_MLForge,
        )

...
from MLForge.pyfunc.loaders.responses_agent import _ResponsesAgentPyfuncWrapper
from MLForge.pyfunc.model import _DEFAULT_RESPONSES_AGENT_METADATA_TASK, ResponsesAgent
from MLForge.types.responses import (
...
def test_responses_agent_predict(tmp_path):
    ...
    MLForge.pyfunc.save_model(python_model=model, path=model_path)
...
def test_responses_agent_throws_with_invalid_output(tmp_path):
    ...
    with pytest.raises(
        MLForgeException, match="Failed to save ResponsesAgent. Ensure your model's predict"
    ):
        MLForge.pyfunc.save_model(python_model=model, path=tmp_path)
...
def test_responses_agent_non_MLForge_decorators():
    ...
    # Both methods should get auto-traced since they don't have __MLForge_traced__

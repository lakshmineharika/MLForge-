...
def spacy_custom_env(tmp_path):
    ...
    _MLForge_conda_env(conda_env, additional_pip_deps=["pytest", "spacy"])
    return conda_env
...
def test_model_save_load(spacy_model_with_data, model_path):
    ...
    MLForge.spacy.save_model(spacy_model=spacy_model, path=model_path)
    loaded_model = MLForge.spacy.load_model(model_path)
        ...
        for example in (None, example_):
            with TempDir() as tmp:
                ...
                if signature is not None or example is None:
                    assert signature == MLForge_model.signature
                else:
                    # signature is inferred from input_example
                    assert MLForge_model.signature is not None
                if example is None:
...
def test_predict_df_with_wrong_shape(spacy_model_with_data, model_path):
    MLForge.spacy.save_model(spacy_model=spacy_model_with_data.model, path=model_path)
    pyfunc_loaded = MLForge.pyfunc.load_model(model_path)

    # Concatenating with itself to duplicate column and mess up input shape
    # then asserting n MLForgeException is raised
    with pytest.raises(MLForgeException, match="Shape of input dataframe must be"):
...
def test_model_log(spacy_model_with_data, tracking_uri_mock):
    spacy_model = spacy_model_with_data.model
    old_uri = MLForge.get_tracking_uri()
    ...
    for should_start_run in [False, True]:
        with TempDir(chdr=True, remove_on_exit=True):
            try:
                ...
                if should_start_run:
                    MLForge.start_run()
                model_info = MLForge.spacy.log_model(spacy_model, name=artifact_path)
                model_uri = model_info.model_uri
                assert model_info.model_uri == model_uri

                # Load model
                spacy_model_loaded = MLForge.spacy.load_model(model_uri=model_uri)
                assert all(
                    _predict(spacy_model, spacy_model_with_data.inference_data)
                ...
                MLForge.end_run()
                MLForge.set_tracking_uri(old_uri)
...
def test_save_model_with_pip_requirements(spacy_model_with_data, tmp_path):
    ...
    MLForge.spacy.save_model(spacy_model_with_data.model, tmpdir1, pip_requirements=str(req_file))
...
def test_save_model_with_extra_pip_requirements(spacy_model_with_data, tmp_path):
    expected_MLForge_version = _MLForge_major_version_string()
    default_reqs = MLForge.spacy.get_default_pip_requirements()
...
def test_model_save_accepts_conda_env_as_dict(spacy_model_with_data, model_path):
    conda_env = dict(MLForge.spacy.get_default_conda_env())
...
def test_model_log_persists_specified_conda_env_in_MLForge_model_directory(
    ...
    with MLForge.start_run():
        model_info = MLForge.spacy.log_model(
            spacy_model_with_data.model,
            name=artifact_path,
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    MLForge.spacy.save_model(spacy_model=spacy_model_with_data.model, path=model_path)
    _assert_pip_requirements(model_path, MLForge.spacy.get_default_pip_requirements())
...
def test_model_log_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    artifact_path = "model"
    with MLForge.start_run():
        model_info = MLForge.spacy.log_model(spacy_model_with_data.model, name=artifact_path)
    _assert_pip_requirements(model_info.model_uri, MLForge.spacy.get_default_pip_requirements())
...
def test_model_log_without_pyfunc_flavor():
    ...
    with MLForge.start_run():
        model_info = MLForge.spacy.log_model(nlp, name=artifact_path)
        model_path = _download_artifact_from_uri(model_info.model_uri)
...
def test_log_model_with_code_paths(spacy_model_with_data):
    ...
    with (
        MLForge.start_run(),
        mock.patch("MLForge.spacy._add_code_from_conf_to_system_path") as add_mock,
    ):
        model_info = MLForge.spacy.log_model(
            spacy_model_with_data.model, name=artifact_path, code_paths=[__file__]
        )
        _compare_logged_code_paths(__file__, model_info.model_uri, MLForge.spacy.FLAVOR_NAME)
        MLForge.spacy.load_model(model_info.model_uri)
...
def test_virtualenv_subfield_points_to_correct_path(spacy_model_with_data, model_path):
    MLForge.spacy.save_model(spacy_model_with_data.model, path=model_path)
    pyfunc_conf = _get_flavor_configuration(model_path=model_path, flavor_name=pyfunc.FLAVOR_NAME)

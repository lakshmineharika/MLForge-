...
from MLForge.pytorch._lightning_autolog import _get_optimizer_name
...
def pytorch_model_with_steps_logged(request):
    ...
    MLForge.pytorch.autolog(log_every_n_epoch=log_every_n_epoch, log_every_n_step=log_every_n_step)
    model = IrisClassification()
    dm = IrisDataModule()
    dm.setup(stage="fit")
    trainer = pl.Trainer(max_epochs=NUM_EPOCHS)
...
def test_pytorch_autolog_log_models_configuration(log_models):
    MLForge.pytorch.autolog(log_models=log_models)
    model = IrisClassification()
    dm = IrisDataModule()
    dm.setup(stage="fit")
    trainer = pl.Trainer(max_epochs=NUM_EPOCHS)
...
def test_pytorch_autolog_log_model_signature(use_ddp):
    ...
    trainer.fit(model, dm)
    logged_model = MLForge.last_logged_model()
    model_signature = MLForge.models.get_model_info(logged_model.model_uri).signature
...
def test_pytorch_autolog_log_model_signature_infer_fail():
    ...
    with (
        mock.patch(
            "MLForge.pytorch._lightning_autolog.infer_signature",
            side_effect=RuntimeError("infer-failure-XXYY"),
        ),
        mock.patch("MLForge.pytorch._lightning_autolog._logger.warning") as patched_warning,
    ):
        trainer.fit(model, dm)
    ...
    logged_model = MLForge.last_logged_model()
    assert MLForge.models.get_model_info(logged_model.model_uri).signature is None
...
def test_extra_tags_pytorch_autolog():
    MLForge.pytorch.autolog(extra_tags={"test_tag": "pytorch_autolog"})
    model = IrisClassification()
    dm = IrisDataModule()
    dm.setup(stage="fit")
    trainer = pl.Trainer(max_epochs=NUM_EPOCHS)
    ...
    assert run.data.tags[MLForge.utils.MLForge_tags.MLForge_AUTOLOGGING] == "pytorch"
...
def test_pytorch_autolog_raises_error_when_step_logging_unsupported():
    MLForge.pytorch.autolog(log_every_n_step=1)
    model = IrisClassification()
    dm = IrisDataModule()
    trainer = pl.Trainer(max_epochs=NUM_EPOCHS)
    with pytest.raises(
        MLForgeException, match="log_every_n_step is only supported for PyTorch-Lightning >= 1.1.0"
    ):
        trainer.fit(model, dm)


def test_pytorch_autolog_persists_manually_created_run():
    with MLForge.start_run() as manual_run:
        MLForge.pytorch.autolog()
        model = IrisClassification()
        dm = IrisDataModule()
        dm.setup(stage="fit")
        ...
        assert MLForge.active_run().info.run_id == manual_run.info.run_id
...
def test_pytorch_autolog_model_can_load_from_artifact(pytorch_model_with_callback):
    ...
    model = MLForge.pytorch.load_model(f"runs:/{run_id}/model")
...
def test_pytorch_autologging_supports_data_parallel_execution():
    ...
    assert MLForge.last_logged_model() is not None


def test_autolog_registering_model():
    registered_model_name = "test_autolog_registered_model"
    MLForge.pytorch.autolog(registered_model_name=registered_model_name)
    model = IrisClassification()
    dm = IrisDataModule()
    dm.setup(stage="fit")
    trainer = pl.Trainer(max_epochs=NUM_EPOCHS)
...
def test_automatic_checkpoint_per_epoch_callback():
    ...
    assert logged_metrics == MLForge.artifacts.load_dict(
...
def test_automatic_checkpoint_per_epoch_save_best_only_min_monitor_callback():
    ...
    run_id = run.info.run_id
    assert (
        MLForge.artifacts.load_dict(f"runs:/{run_id}/checkpoints/latest_checkpoint_metrics.json")[
            "epoch"
        ]
        == 0
    )
...
def test_autologging_disabled_for_forecasting_model_predict():
    ...
    last_run_id = MLForge.last_active_run().info.run_id
    deepar.predict(data)

    # assert `deepar.predict` does not trigger autologging (i.e. no new run is created)
    assert MLForge.last_active_run().info.run_id == last_run_id

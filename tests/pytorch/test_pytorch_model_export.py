...
import MLForge.pytorch
from MLForge import pyfunc
from MLForge.exceptions import MLForgeException
from MLForge.models import Model, ModelSignature
from MLForge.models.utils import _read_example, load_serving_example
from MLForge.pytorch import pickle_module as MLForge_pytorch_pickle_module
...
from MLForge.types.schema import DataType, Schema, TensorSpec
...
def pytorch_custom_env(tmp_path):
    ...
    _MLForge_conda_env(conda_env, additional_pip_deps=["pytorch", "torchvision", "pytest"])
    return conda_env
        ...
        for example in (None, example_):
            with TempDir() as tmp:
                ...
                if signature is None and example is None:
                    ...
                    assert MLForge_model.signature == iris_tensor_spec
                if example is None:
                    assert MLForge_model.saved_input_example_info is None
                else:
                    np.testing.assert_allclose(_read_example(MLForge_model, path), example)
...
def test_log_model(sequential_model, data, sequential_predicted):
    try:
        ...
        model_info = MLForge.pytorch.log_model(
            sequential_model, name=artifact_path, serialization_format="pickle"
        )

        sequential_model_loaded = MLForge.pytorch.load_model(model_uri=model_info.model_uri)
...
def test_raise_exception(sequential_model):
    with TempDir(chdr=True, remove_on_exit=True) as tmp:
        path = tmp.path("model")
        with pytest.raises(MLForgeException, match="No such artifact"):
            MLForge.pytorch.load_model(path)

        with pytest.raises(TypeError, match="Argument 'pytorch_model' should be a torch.nn.Module"):
            MLForge.pytorch.save_model([1, 2, 3], path)

        MLForge.pytorch.save_model(sequential_model, path, serialization_format="pickle")
        with pytest.raises(MLForgeException, match=f"Path '{os.path.abspath(path)}' already exists"):
            MLForge.pytorch.save_model(sequential_model, path, serialization_format="pickle")

        import sklearn.neighbors as knn

        from MLForge import sklearn
        ...
        with pytest.raises(MLForgeException, match='Model does not have the "pytorch" flavor'):
            MLForge.pytorch.load_model(path)


@pytest.mark.parametrize("scripted_model", [True, False])
def test_save_and_load_model(sequential_model, model_path, data, sequential_predicted):
    MLForge.pytorch.save_model(sequential_model, model_path, serialization_format="pickle")

    # Loading pytorch model
    sequential_model_loaded = MLForge.pytorch.load_model(model_path)
...
def test_load_model_from_remote_uri_succeeds(
    ...
    sequential_model_loaded = MLForge.pytorch.load_model(model_uri=model_uri)
    np.testing.assert_array_equal(_predict(sequential_model_loaded, data), sequential_predicted)


@pytest.mark.parametrize("scripted_model", [True, False])
def test_model_save_persists_specified_conda_env_in_MLForge_model_directory(
...
@pytest.mark.parametrize("scripted_model", [True, False])
def test_model_save_persists_requirements_in_MLForge_model_directory(
    sequential_model, model_path, pytorch_custom_env
):
    MLForge.pytorch.save_model(
        pytorch_model=sequential_model,
...
def test_save_model_with_pip_requirements(sequential_model, tmp_path):
    ...
    _assert_pip_requirements(tmpdir1, [expected_MLForge_version, "a"], strict=True)
    ...
    _assert_pip_requirements(tmpdir2, [expected_MLForge_version, "a", "b"], strict=True)
    ...
    _assert_pip_requirements(
        tmpdir3, [expected_MLForge_version, "b", "-c constraints.txt"], ["a"], strict=True
    )
...
def test_save_model_with_extra_pip_requirements(sequential_model, tmp_path):
    expected_MLForge_version = _MLForge_major_version_string()
    default_reqs = MLForge.pytorch.get_default_pip_requirements()
    ...
    _assert_pip_requirements(tmpdir1, [expected_MLForge_version, *default_reqs, "a"])
    ...
    _assert_pip_requirements(tmpdir2, [expected_MLForge_version, *default_reqs, "a", "b"])
    ...
    _assert_pip_requirements(
        tmpdir3, [expected_MLForge_version, *default_reqs, "b", "-c constraints.txt"], ["a"]
    )


@pytest.mark.parametrize("scripted_model", [True, False])
def test_model_save_accepts_conda_env_as_dict(sequential_model, model_path):
    conda_env = dict(MLForge.pytorch.get_default_conda_env())
...
@pytest.mark.parametrize("scripted_model", [True, False])
def test_model_log_persists_specified_conda_env_in_MLForge_model_directory(
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    _assert_pip_requirements(model_path, MLForge.pytorch.get_default_pip_requirements())
...
def test_model_log_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    _assert_pip_requirements(model_info.model_uri, MLForge.pytorch.get_default_pip_requirements())
...
def test_load_model_with_differing_pytorch_version_logs_warning(sequential_model, model_path):
    ...
    model_config.flavors[MLForge.pytorch.FLAVOR_NAME]["pytorch_version"] = saver_pytorch_version
    ...
    with (
        mock.patch("MLForge.pytorch._logger.warning") as warn_mock,
        mock.patch("torch.__version__", loader_pytorch_version),
    ):
        warn_mock.side_effect = custom_warn
        MLForge.pytorch.load_model(model_uri=model_path)
...
def test_pyfunc_model_serving_with_main_scoped_subclassed_model_and_custom_pickle_module(
    ...
    with MLForge.start_run():
        model_info = MLForge.pytorch.log_model(
            ...
            name="pytorch_model",
            pickle_module=MLForge_pytorch_pickle_module,
            input_example=data[0],
            serialization_format="pickle",
        )
...
def test_load_model_succeeds_with_dependencies_specified_via_code_paths(
    ...
    # `MLForge.pytorch.load_model`
    class TorchValidatorModel(pyfunc.PythonModel):
        def load_context(self, context):
            self.pytorch_model = MLForge.pytorch.load_model(context.artifacts["pytorch_model"])
    ...
    # constituent PyTorch model via `MLForge.pytorch.load_model`
    inference_payload = load_serving_example(model_info.model_uri)
    scoring_response = pyfunc_serve_and_score_model(
        model_uri=model_info.model_uri,
        data=inference_payload,
...
def test_load_pyfunc_succeeds_when_data_is_model_file_instead_of_directory(
    ...
    """
    This test verifies that PyTorch models saved in older versions of MLForge are loaded successfully
    by ``MLForge.pytorch.load_model``. The ``data`` path associated with these older models is
    ...
    assert MLForge.pytorch._SERIALIZED_TORCH_MODEL_FILE_NAME in os.listdir(model_data_path)
    pyfunc_conf[pyfunc.DATA] = os.path.join(
        model_data_path, MLForge.pytorch._SERIALIZED_TORCH_MODEL_FILE_NAME
    )
...
def test_load_model_allows_user_to_override_pickle_module_via_keyword_argument(
    ...
    with (
        ...
        MLForge.pytorch.load_model(model_uri=model_path, pickle_module=MLForge_pytorch_pickle_module)
        torch_load_mock.assert_called_with(mock.ANY, pickle_module=MLForge_pytorch_pickle_module)
        warn_mock.assert_any_call(mock.ANY, MLForge_pytorch_pickle_module.__name__, pickle.__name__)
...
def test_load_model_raises_exception_when_pickle_module_cannot_be_imported(
    ...
    model_data_path = os.path.join(model_path, pyfunc_conf[pyfunc.DATA])
    assert os.path.exists(model_data_path)
    assert MLForge.pytorch._PICKLE_MODULE_INFO_FILE_NAME in os.listdir(model_data_path)
    with open(
        os.path.join(model_data_path, MLForge.pytorch._PICKLE_MODULE_INFO_FILE_NAME), "w"
    ) as f:
...
def test_extra_files_log_model(create_extra_files, sequential_model):
    ...
    with MLForge.start_run():
        ...
        model_uri = "runs:/{run_id}/{model_path}".format(
            run_id=MLForge.active_run().info.run_id, model_path="models"
        )
...
def test_log_model_invalid_extra_file_path(sequential_model):
    with (
        MLForge.start_run(),
        pytest.raises(MLForgeException, match="No such artifact: 'non_existing_file.txt'"),
    ):
        MLForge.pytorch.log_model(
            sequential_model,
            name="models",
...
def test_save_state_dict(sequential_model, model_path, data):
    ...
    MLForge.pytorch.save_state_dict(state_dict, model_path)

    loaded_state_dict = MLForge.pytorch.load_state_dict(model_path)
...
def test_load_state_dict_disallows_pickle_deserialization(model_path, monkeypatch):
    model = get_sequential_model()
    MLForge.pytorch.save_state_dict(model.state_dict(), model_path)

    monkeypatch.setenv("MLForge_ALLOW_PICKLE_DESERIALIZATION", "false")
    with pytest.raises(MLForgeException, match="MLForge_ALLOW_PICKLE_DESERIALIZATION"):
        MLForge.pytorch.load_state_dict(model_path)
...
def test_save_state_dict_throws_for_invalid_object_type(not_state_dict, model_path):
    with pytest.raises(TypeError, match="Invalid object type for `state_dict`"):
        MLForge.pytorch.save_state_dict(not_state_dict, model_path)
...
def test_log_state_dict(sequential_model, data):
    ...
    state_dict = sequential_model.state_dict()
    with MLForge.start_run():
        MLForge.pytorch.log_state_dict(state_dict, artifact_path)
        state_dict_uri = MLForge.get_artifact_uri(artifact_path)

    loaded_state_dict = MLForge.pytorch.load_state_dict(state_dict_uri)
    assert state_dict_equal(loaded_state_dict, state_dict)
    model = get_sequential_model()
    model.load_state_dict(loaded_state_dict)
    np.testing.assert_array_almost_equal(
...
def test_log_model_with_code_paths(sequential_model):
    ...
    with (
        MLForge.start_run(),
        mock.patch("MLForge.pytorch._add_code_from_conf_to_system_path") as add_mock,
    ):
        model_info = MLForge.pytorch.log_model(
            sequential_model,
            name=artifact_path,
        ...
        _compare_logged_code_paths(__file__, model_info.model_uri, MLForge.pytorch.FLAVOR_NAME)
        MLForge.pytorch.load_model(model_info.model_uri)
        add_mock.assert_called()


def test_virtualenv_subfield_points_to_correct_path(model_path):
    model = get_sequential_model()
    MLForge.pytorch.save_model(model, path=model_path, serialization_format="pickle")
    pyfunc_conf = _get_flavor_configuration(model_path=model_path, flavor_name=pyfunc.FLAVOR_NAME)
...
def test_load_model_to_device(sequential_model):
    with mock.patch("MLForge.pytorch._load_model") as load_model_mock:
        with MLForge.start_run():
            model_info = MLForge.pytorch.log_model(
            ...
            MLForge.pyfunc.load_model(model_uri=model_info.model_uri, model_config=model_config)

            load_model_mock.assert_called_with(mock.ANY, **model_config)
            MLForge.pytorch.load_model(model_uri=model_info.model_uri, **model_config)
...
def test_passing_params_to_model(data):
    ...
    signature = MLForge.models.infer_signature(x, None, {"y": 1})
    with MLForge.start_run():
        model_info = MLForge.pytorch.log_model(
...
def test_exported_model_infer_dynamic_dim(tmp_path):
    ...
    loaded_model1 = MLForge.pytorch.load_model(save_path1)
    ...
    loaded_model2 = MLForge.pytorch.load_model(save_path2)
...
def test_load_exported_model_check_device_mismatch(sequential_model, model_path):
    ...
    MLForge.pytorch.load_model(model_path, device="cpu")

    with pytest.raises(
        MLForgeException,
        match="it can't be loaded on 'cuda' device.",
    ):
        MLForge.pytorch.load_model(model_path, device="cuda")
...
def test_save_and_load_exported_model_with_multi_inputs(model_path):
    ...
    model_loaded = MLForge.pytorch.load_model(model_path)
...
def test_exported_model_with_small_batch_size_input_example(tmp_path, batch_size):
    # Regression test for https://github.com/MLForge/MLForge/pull/24494: a batch-size-1
    ...
    MLForge.pytorch.save_model(model, model_path, input_example=input_example)
    assert (model_path / "data" / "model.pt2").exists()

    loaded_model = MLForge.pytorch.load_model(model_path)

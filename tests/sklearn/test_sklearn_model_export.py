...
from MLForge.entities.model_registry.model_version import ModelVersion, ModelVersionStatus
...
from MLForge.protos.databricks_pb2 import INVALID_PARAMETER_VALUE, ErrorCode
...
def sklearn_custom_env(tmp_path):
    ...
    _MLForge_conda_env(conda_env, additional_pip_deps=["scikit-learn", "pytest"])
    return conda_env


@pytest.mark.parametrize("serialization_format", MLForge.sklearn.SUPPORTED_SERIALIZATION_FORMATS)
def test_model_save_load(sklearn_logreg_model, model_path, serialization_format):
    from MLForge.utils.requirements_utils import _parse_requirements
    ...
    reloaded_model = MLForge.sklearn.load_model(model_uri=model_path)
    reloaded_pyfunc = pyfunc.load_model(model_uri=model_path)

    sklearn_conf = _get_flavor_configuration(
        model_path=model_path, flavor_name=MLForge.sklearn.FLAVOR_NAME
    )
    assert "serialization_format" in sklearn_conf
    assert sklearn_conf["serialization_format"] == serialization_format

    req_map = {
        MLForge.sklearn.SERIALIZATION_FORMAT_SKOPS: f"skops=={skops.__version__}",
        MLForge.sklearn.SERIALIZATION_FORMAT_CLOUDPICKLE: f"cloudpickle=={cloudpickle.__version__}",
    }
    ...
    if serialization_format != MLForge.sklearn.SERIALIZATION_FORMAT_PICKLE:
...
def test_model_skops_format_trusted_type(sklearn_knn_model, model_path):
    ...
    with pytest.raises(MLForgeException, match="The saved sklearn model references untrusted type"):
        MLForge.sklearn.save_model(
            sk_model=sk_model,
            path=model_path,
...
def test_log_model_skops_no_pip_requirements_warning(sklearn_logreg_model, recwarn):
    with MLForge.start_run():
        MLForge.sklearn.log_model(
            ...
            serialization_format=MLForge.sklearn.SERIALIZATION_FORMAT_SKOPS,
        )
...
def test_model_save_behavior_with_preexisting_folders(sklearn_logreg_model, tmp_path):
    ...
    MLForge.sklearn.save_model(sk_model=sklearn_logreg_model.model, path=sklearn_model_path)

    sklearn_model_path = tmp_path / "sklearn_model_filled_exists"
    sklearn_model_path.mkdir()
    (sklearn_model_path / "foo.txt").write_text("dummy content")
    with pytest.raises(MLForgeException, match="already exists and is not empty"):
        MLForge.sklearn.save_model(sk_model=sklearn_logreg_model.model, path=sklearn_model_path)
...
def test_model_load_from_remote_uri_succeeds(sklearn_logreg_model, model_path, mock_s3_bucket):
    MLForge.sklearn.save_model(sk_model=sklearn_logreg_model.model, path=model_path)
    ...
    reloaded_model = MLForge.sklearn.load_model(model_uri=model_uri)
    np.testing.assert_array_equal(
    ...
    with TempDir(chdr=True, remove_on_exit=True) as tmp:
        for should_start_run in [False, True]:
            try:
                ...
                _MLForge_conda_env(conda_env, additional_pip_deps=["scikit-learn"])
                ...
                reloaded_logsklearn_knn_model = MLForge.sklearn.load_model(
...
def test_log_model_calls_register_model(sklearn_logreg_model):
    ...
    with MLForge.start_run(), register_model_patch, TempDir(chdr=True, remove_on_exit=True) as tmp:
        conda_env = os.path.join(tmp.path(), "conda_env.yaml")
        _MLForge_conda_env(conda_env, additional_pip_deps=["scikit-learn"])
        model_info = MLForge.sklearn.log_model(
            sklearn_logreg_model.model,
...
def test_custom_transformer_can_be_saved_and_loaded_with_cloudpickle_format(
    ...
    reloaded_custom_transformer_model = MLForge.sklearn.load_model(
...
def test_log_model_with_extra_pip_requirements(sklearn_knn_model, tmp_path):
    expected_MLForge_version = _MLForge_major_version_string()
    default_reqs = MLForge.sklearn.get_default_pip_requirements(include_skops=True)
...
def test_model_save_accepts_conda_env_as_dict(sklearn_knn_model, model_path):
    conda_env = dict(MLForge.sklearn.get_default_conda_env())
...
def test_model_save_throws_exception_if_serialization_format_is_unrecognized(
    ...
    with pytest.raises(MLForgeException, match="Unrecognized serialization format") as exc:
        MLForge.sklearn.save_model(
            sk_model=sklearn_knn_model.model,
            path=model_path,
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    MLForge.sklearn.save_model(sk_model=sklearn_logreg_model.model, path=model_path)
    _assert_pip_requirements(
        model_path, MLForge.sklearn.get_default_pip_requirements(include_skops=True)
    )
...
def test_model_log_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    with MLForge.start_run():
        model_info = MLForge.sklearn.log_model(sklearn_logreg_model.model, name="model")

    _assert_pip_requirements(
        model_info.model_uri, MLForge.sklearn.get_default_pip_requirements(include_skops=True)
    )
...
def test_model_save_uses_skops_serialization_format_by_default(sklearn_logreg_model, model_path):
    ...
    assert "serialization_format" in sklearn_conf
    assert sklearn_conf["serialization_format"] == MLForge.sklearn.SERIALIZATION_FORMAT_SKOPS
...
def test_model_save_with_cloudpickle_format_adds_cloudpickle_to_conda_environment(
    ...
    assert "serialization_format" in sklearn_conf
    assert sklearn_conf["serialization_format"] == MLForge.sklearn.SERIALIZATION_FORMAT_CLOUDPICKLE
...
def test_model_save_without_cloudpickle_format_does_not_add_cloudpickle_to_conda_environment(
    ...
    non_cloudpickle_serialization_formats = list(MLForge.sklearn.SUPPORTED_SERIALIZATION_FORMATS)
    non_cloudpickle_serialization_formats.remove(MLForge.sklearn.SERIALIZATION_FORMAT_CLOUDPICKLE)
...
def test_load_pyfunc_succeeds_for_older_models_with_pyfunc_data_field(
    ...
    """
    This test verifies that scikit-learn models saved in older versions of MLForge are loaded
    successfully by ``MLForge.pyfunc.load_model``. These older models specify a pyfunc ``data``
    ...
    sklearn_conf = model_conf.flavors.get(MLForge.sklearn.FLAVOR_NAME)
...
def test_sklearn_compatible_with_MLForge_2_4_0(sklearn_knn_model, tmp_path):
...
"""
    ...
    assert Version(MLForge.__version__) > Version("2.4.0")
    model_uri = str(tmp_path)
    pyfunc_loaded = MLForge.pyfunc.load_model(model_uri)
    ...
    # Issues a warning if params are specified prior to MLForge support in 2.5.0
    with mock.patch("MLForge.models.utils._logger.warning") as mock_warning:
...
def test_log_model_with_code_paths(sklearn_knn_model):
    ...
    with (
        MLForge.start_run(),
        mock.patch("MLForge.sklearn._add_code_from_conf_to_system_path") as add_mock,
    ):
        model_info = MLForge.sklearn.log_model(
            sklearn_knn_model.model,
            name=artifact_path,
        ...
        _compare_logged_code_paths(__file__, model_info.model_uri, MLForge.sklearn.FLAVOR_NAME)
        MLForge.sklearn.load_model(model_uri=model_info.model_uri)
...
def test_virtualenv_subfield_points_to_correct_path(sklearn_logreg_model, model_path):
    MLForge.sklearn.save_model(sklearn_logreg_model.model, path=model_path)
    pyfunc_conf = _get_flavor_configuration(model_path=model_path, flavor_name=pyfunc.FLAVOR_NAME)
...
def test_model_size_bytes(sklearn_logreg_model, tmp_path):
    MLForge.sklearn.save_model(sklearn_logreg_model.model, path=tmp_path)
...
def test_pipeline_predict_proba(sklearn_logreg_model, model_path):
    ...
    MLForge.sklearn.save_model(sk_model=pipeline, path=model_path, pyfunc_predict_fn="predict_proba")

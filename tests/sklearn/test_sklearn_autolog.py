...
from MLForge.entities import RunStatus
...
from MLForge.sklearn.utils import (
    _get_arg_names,
...
from MLForge.types.utils import _infer_schema
from MLForge.utils import _truncate_dict
from MLForge.utils.autologging_utils import MLForgeAutologgingQueueingClient
...
def get_run(run_id):
    return MLForgeClient().get_run(run_id)


def get_run_data(run_id):
    client = MLForgeClient()
    data = client.get_run(run_id).data
    # Ignore tags MLForge logs by default (e.g. "MLForge.user")
    tags = {k: v for k, v in data.tags.items() if not k.startswith("MLForge.")}
    artifacts = [f.path for f in client.list_artifacts(run_id)]
    return data.params, data.metrics, tags, artifacts


def load_model_by_run_id(run_id):
    return MLForge.sklearn.load_model(f"runs:/{run_id}/{MODEL_DIR}")
...
def _get_model_uri(name: str = MODEL_DIR) -> str:
    ...
    """
    if uri := next(
        m.model_uri for m in MLForge.search_logged_models(output_format="list") if m.name == name
    ):
        return uri
...
def test_autolog_preserves_original_function_attributes():
    ...
    before = [get_cls_attrs(cls) for _, cls in MLForge.sklearn.utils._all_estimators()]
    MLForge.sklearn.autolog()
    after = [get_cls_attrs(cls) for _, cls in MLForge.sklearn.utils._all_estimators()]
...
def test_autolog_throws_error_with_negative_max_tuning_runs():
    with pytest.raises(
        MLForgeException, match="`max_tuning_runs` must be non-negative, instead got -1."
    ):
        MLForge.sklearn.autolog(max_tuning_runs=-1)
...
def test_autolog_max_tuning_runs_logs_info_correctly(max_tuning_runs, total_runs, output_statement):
    with mock.patch("MLForge.sklearn.utils._logger.info") as mock_info:
...
def test_autolog_does_not_terminate_active_run():
    ...
    assert MLForge.active_run() is not None
    MLForge.end_run()


def test_extra_tags_sklearn_autolog():
    MLForge.sklearn.autolog(extra_tags={"test_tag": "sklearn_autolog"})
    sklearn.cluster.KMeans().fit(*get_iris())
    assert MLForge.active_run() is None

    run = MLForge.last_active_run()
    assert run.data.tags["test_tag"] == "sklearn_autolog"
    assert run.data.tags[MLForge.utils.MLForge_tags.MLForge_AUTOLOGGING] == "sklearn"
...
def test_get_params_returns_dict_whose_key_or_value_exceeds_length_limit(long_params, messages):
    ...
    with (
        ...
        mock.patch("MLForge.utils._logger.warning") as mock_warning,
...
def test_autolog_terminates_run_when_active_run_does_not_exist_and_fit_fails():
    ...
    latest_run = MLForge.search_runs().iloc[0]
    assert MLForge.active_run() is None
...
def test_autolog_does_not_terminate_run_when_active_run_exists_and_fit_fails():
    ...
    assert MLForge.active_run() is not None
    assert MLForge.active_run() is run
    MLForge.end_run()
...
def test_autolog_emits_warning_message_when_score_fails():
    ...
    with MLForge.start_run(), mock.patch("MLForge.sklearn.utils._logger.warning") as mock_warning:
        model.fit(*get_iris())
...
def test_autolog_emits_warning_message_when_model_prediction_fails():
    ...
    with MLForge.start_run(), mock.patch("MLForge.sklearn.utils._logger.warning") as mock_warning:
        ...
        call_count_expected = 2 if MLForge.sklearn.utils._is_plotting_supported() else 1
...
def test_parameter_search_estimators_produce_expected_outputs(
    ...
    best_estimator = MLForge.sklearn.load_model(f"runs:/{run_id}/best_estimator")
    assert isinstance(best_estimator, sklearn.svm.SVC)
    cv_model = MLForge.sklearn.load_model(f"runs:/{run_id}/{MODEL_DIR}")
    ...
    cv_logged_model = MLForge.get_logged_model(cv_model_id)
    ...
    logged_best_model = MLForge.get_logged_model(best_estimator_conf.model_id)
    ...
    child_runs = client.search_runs(
        run.info.experiment_id, f"tags.`MLForge.parentRunId` = '{run_id}'"
    )
    ...
    # have a corresponding MLForge run with the expected data
    ...
    for _, result in cv_results_best_n_df.iterrows():
        ...
        search_filter = f"tags.`MLForge.parentRunId` = '{run_id}' and {params_search_clause}"
        ...
        assert child_run.data.tags.get(MLForge_AUTOLOGGING) == MLForge.sklearn.FLAVOR_NAME
...
def test_autolog_logs_signature_and_input_example(data_type):
    MLForge.sklearn.autolog(log_input_examples=True, log_model_signatures=True)
...
def test_autolog_metrics_input_example_and_signature_do_not_reflect_training_mutations():
    ...
    MLForge.sklearn.autolog(log_models=True, log_model_signatures=True, log_input_examples=True)
    ...
    metrics = get_run_data(MLForge.last_active_run().info.run_id)[1]
...
def test_autolog_does_not_throw_when_failing_to_sample_X():
    ...
    with MLForge.start_run(), mock.patch("MLForge.sklearn._logger.warning") as mock_warning:
...
def test_autolog_logs_signature_only_when_estimator_defines_predict():
    ...
    MLForge.sklearn.autolog(log_model_signatures=True)
...
def test_autolog_does_not_throw_when_predict_fails():
    ...
    with (
        ...
        mock.patch("MLForge.sklearn._logger.warning") as mock_warning,
    ):
        model = sklearn.linear_model.LinearRegression()
        model.fit(X, y)
...
def test_autolog_does_not_throw_when_infer_signature_fails():
    ...
    with (
        MLForge.start_run(),
        mock.patch("MLForge.models.infer_signature", side_effect=Exception("Failed")),
        mock.patch("MLForge.sklearn._logger.warning") as mock_warning,
    ):
        MLForge.sklearn.autolog(log_input_examples=True, log_model_signatures=True)
        model = sklearn.linear_model.LinearRegression()
...
def test_sklearn_autolog_log_models_configuration(log_models):
    ...
    with MLForge.start_run() as run:
        MLForge.sklearn.autolog(log_models=log_models)
        model = sklearn.linear_model.LinearRegression()
        model.fit(X, y)

    run_id = run.info.run_id
    _, _, _, artifacts = get_run_data(run_id)
    assert (MLForge.last_logged_model() is not None) == log_models
...
def test_sklearn_autolog_log_datasets_configuration(log_datasets):
    ...
    with MLForge.start_run() as run:
        MLForge.sklearn.autolog(log_datasets=log_datasets)
        model = sklearn.linear_model.LinearRegression()
        model.fit(X, y)
...
def test_sklearn_autolog_log_datasets_with_predict():
    ...
    with MLForge.start_run() as run:
        MLForge.sklearn.autolog(log_datasets=True)
        model = sklearn.linear_model.LinearRegression()
        model.fit(X, y)
        model.predict(X)
...
def test_sklearn_autolog_log_datasets_without_explicit_run():
    ...
    run_id = getattr(model, "_MLForge_run_id")
    client = MLForgeClient()
    dataset_inputs = client.get_run(run_id).inputs.dataset_inputs
...
def test_autolog_does_not_capture_runs_for_preprocessing_or_feature_manipulation_estimators():
    ...
    # Create a run using the MLForge client, which will be resumed via the fluent API,
    ...
    with MLForge.start_run(run_id=run_id):
        ...
        TfidfVectorizer().fit_transform([
            "MLForge is an end-to-end machine learning platform.",
            "MLForge enables me to systematize my ML experimentation",
        ])
...
def test_autolog_produces_expected_results_for_estimator_when_parent_also_defines_fit():
    """
    Test to prevent recurrences of https://github.com/MLForge/MLForge/issues/3574
    """
    MLForge.sklearn.autolog()
    ...
    # https://github.com/MLForge/MLForge/issues/3574 where ChildMod.fit() would either infinitely
    ...
    og_all_estimators = MLForge.sklearn.utils._all_estimators()
    new_all_estimators = og_all_estimators + [("ParentMod", ParentMod), ("ChildMod", ChildMod)]

    with mock.patch("MLForge.sklearn.utils._all_estimators", return_value=new_all_estimators):
        MLForge.sklearn.autolog()
...
@pytest.mark.parametrize("cross_val_func_name", MLForge.sklearn._apis_autologging_disabled)
def test_autolog_disabled_on_sklearn_cross_val_api(cross_val_func_name):
    ...
    exp_id = MLForge.tracking.fluent._get_experiment_id()
    runs_before = MLForge.search_runs([exp_id])
    cross_val_func(lasso, X, y, cv=3, **extra_params)
    runs_after = MLForge.search_runs([exp_id])
    assert len(runs_before) == len(runs_after)


def load_json_artifact(artifact_path):
    fpath = MLForge.get_artifact_uri(artifact_path).replace("file://", "")
...
@pytest.mark.parametrize("metric_name", MLForge.sklearn._get_metric_name_list())
...
def test_post_training_metric_autologging_patch_transform():
    ...
    with mock.patch(
        "MLForge.sklearn._AutologgingMetricsManager.register_prediction_input_dataset"
...
def test_nested_metric_call_is_disabled():
    ...
    with MLForge.start_run():
        with mock.patch(
            "MLForge.sklearn._AutologgingMetricsManager.log_post_training_metric"
...
def test_meta_estimator_disable_nested_post_training_autologging(scoring):
    ...
    with (
        mock.patch(
            "MLForge.sklearn._AutologgingMetricsManager.register_model"
        ) as mock_register_model,
        mock.patch(
            "MLForge.sklearn._AutologgingMetricsManager.is_metric_value_loggable"
...
def test_gen_metric_call_commands():
    ...
    cmd1 = MLForge.sklearn._AutologgingMetricsManager.gen_metric_call_command(
        None,
        metric_fn1,
    ...
    cmd2 = MLForge.sklearn._AutologgingMetricsManager.gen_metric_call_command(
        None, metric_fn1, *[data1, data2], **{"randarg1": "'xyz\"abc"}
    )
    assert cmd2 == "metric_fn1(a1=data1, b1=data2, randarg1='\\'xyz\"abc')"

    lr_model = sklearn.linear_model.LinearRegression()
    cmd3 = MLForge.sklearn._AutologgingMetricsManager.gen_metric_call_command(
...
def test_is_metrics_value_loggable():
    is_metric_value_loggable = MLForge.sklearn._AutologgingMetricsManager.is_metric_value_loggable
...
def test_log_post_training_metrics_configuration(log_models):
    ...
    for log_post_training_metrics in [True, False, True]:
        MLForge.sklearn.autolog(log_post_training_metrics=log_post_training_metrics)
        ...
        if log_models:
            logged_model = MLForge.last_logged_model()
...
def test_autolog_print_warning_if_custom_estimator_pickling_raise_error():
    ...
    with MLForge.start_run() as run, mock.patch("MLForge.sklearn._logger.warning") as mock_warning:
...
def test_autolog_registering_model():
    ...
    MLForge.sklearn.autolog(registered_model_name=registered_model_name)
    with MLForge.start_run():
        sklearn.cluster.KMeans().fit(*get_iris())

        registered_model = MLForgeClient().get_registered_model(registered_model_name)
        assert registered_model.name == registered_model_name


def test_autolog_pos_label_used_for_training_metric():
    MLForge.sklearn.autolog(pos_label=1)
    ...
    with MLForge.start_run() as run:
        ...
        with MLForgeAutologgingQueueingClient() as autologging_client:

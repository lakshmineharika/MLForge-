...
from MLForge.models import rag_signatures
from MLForge.models.utils import _enforce_tensor_spec
from MLForge.pyfunc import _parse_spark_datatype
from MLForge.types import DataType
from MLForge.types.schema import (
    AnyType,
    Array,
    ColSpec,
    Object,
...
from MLForge.types.utils import (
...
def test_col_spec():
    ...
    assert b1 != a1
    assert a1 == a2
    with pytest.raises(MLForgeException, match="Unsupported type 'unsupported'"):
        ColSpec("unsupported")
...
def test_tensor_spec():
    ...
    with pytest.raises(
        MLForgeException,
        match="MLForge does not support size information in flexible numpy data types",
    ):
...
def test_schema_creation():
    ...
    with pytest.raises(MLForgeException, match="Please choose one of"):
        Schema([TensorSpec(np.dtype("float64"), (-1,)), ColSpec("double")])

    # combination of named and unnamed inputs is not allowed
    with pytest.raises(
        MLForgeException, match="Creating Schema with a combination of named and unnamed inputs"
    ):
        Schema([
    ...
    with pytest.raises(
        MLForgeException, match="Creating Schema with multiple unnamed TensorSpecs is not supported"
    ):
        Schema([TensorSpec(np.dtype("double"), (-1,)), TensorSpec(np.dtype("double"), (-1,))])


def test_schema_creation_errors():
    with pytest.raises(MLForgeException, match=r"Creating Schema with empty inputs is not allowed."):
        Schema([])

    with pytest.raises(MLForgeException, match=r"Inputs of Schema must be a list, got type dict"):
...
def test_get_schema_type(dict_of_ndarrays):
    ...
    with pytest.raises(MLForgeException, match="TensorSpec only supports numpy types"):
        schema.pandas_types()
    with pytest.raises(MLForgeException, match="TensorSpec cannot be converted to spark dataframe"):
...
def test_schema_inference_on_pandas_series():
    ...
    with pytest.raises(MLForgeException, match="Unsupported numpy data type"):
...
def test_get_tensor_shape(dict_of_ndarrays):
    ...
    # Out of bounds
    with pytest.raises(
        MLForgeException, match="The specified variable_dimension 10 is out of bounds"
    ):
...
def test_schema_inference_on_list_with_errors():
    ...
    with pytest.raises(MLForgeException, match=re.escape(MULTIPLE_TYPES_ERROR_MSG)):
        _infer_schema(data)


def test_schema_inference_validating_dictionary_keys():
    for data in [{1.7: "a", "b": "c"}, {12.4: "c", "d": "e"}]:
        with pytest.raises(MLForgeException, match="The dictionary keys are not all strings."):
            _infer_schema(data)
...
def test_all_numpy_dtypes():
    ...
    for dtype in str_:
        ...
        test_dtype(np.array(["MLForge"], dtype=dtype), dtype)
        test_dtype(np.array(["MLForge is the best"], dtype=dtype), dtype)
...
def test_param_schema_find_duplicates():
    with pytest.raises(
        MLForgeException, match=re.escape("Duplicated parameters found in schema: ['param1']")
    ):
        ParamSchema([
    ...
    with pytest.raises(
        MLForgeException, match=re.escape("Duplicated parameters found in schema: ['param3']")
    ):
        ParamSchema([
            ParamSpec("param1", DataType.string, "default1", None),
            ParamSpec("param2", DataType.string, "default2", None),
...
def test_infer_param_schema():
    ...
    with pytest.raises(MLForgeException, match=r"Expected parameters to be dict, got list"):
    ...
    with pytest.raises(MLForgeException, match=r".*") as e:
        _infer_param_schema(test_parameters)
    assert e.match(r"Failed to infer schema for parameters: ")
    assert e.match(
        re.escape(
            "('b', (1, 2, 3), MLForgeException('Expected parameters "
            "to be 1D array or scalar, got tuple'))"
        )
    )
    assert e.match(
        re.escape(
            "('d', [[1, 2], [3, 4]], MLForgeException('Expected parameters "
            "to be 1D array or scalar, got 2D array'))"
        )
    )
    assert e.match(
        re.escape(
            "('e', [[[1, 2], [3], []]], MLForgeException('Expected parameters "
...
def test_infer_param_schema_with_errors():
    with pytest.raises(
        MLForgeException, match=r"Expected parameters to be 1D array or scalar, got Series"
    ):
        _infer_param_schema({"a": pd.Series([1, 2, 3])})


def test_object_construction_with_errors():
    with pytest.raises(MLForgeException, match=r"Expected properties to be a list, got type dict"):
        Object({"p1": Property("p1", DataType.string)})

    with pytest.raises(
        MLForgeException, match=r"Creating Object with empty properties is not allowed."
    ):
        Object([])
    ...
    with pytest.raises(MLForgeException, match=r"Expected values to be instance of Property"):
        Object(properties)
    ...
    with pytest.raises(MLForgeException, match=r"Found duplicated property names: `p2`"):
        Object(properties)
...
def test_property_from_dict_with_errors():
    ...
    with pytest.raises(
        MLForgeException, match=r"Expected properties to be a dictionary of Property JSON"
    ):
...
def test_object_from_dict_with_errors():
    ...
    with pytest.raises(
        MLForgeException, match=r"Type mismatch, Object expects `object` as the type"
    ):
...
def test_array_from_dict_with_errors():
    ...
    with pytest.raises(MLForgeException, match=r"Type mismatch, Array expects `array` as the type"):
        Array.from_json_dict(**{"type": "object", "items": "string"})

    with pytest.raises(MLForgeException, match=r"Expected items to be a dictionary of Object JSON"):
...
def test_schema_inference_with_empty_lists():
    ...
    # with version 2.7.1: https://github.com/MLForge/MLForge/blob/v2.7.1/MLForge/types/utils.py#L150
    data = []

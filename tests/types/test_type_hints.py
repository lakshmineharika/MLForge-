...
from MLForge.models.utils import PyFuncOutput, _enforce_schema
from MLForge.types.schema import AnyType, Array, ColSpec, DataType, Map, Object, Property, Schema
from MLForge.types.type_hints import (
...
def test_infer_schema_from_type_hints_errors():
    with pytest.raises(MLForgeException, match=r"Type hints must be wrapped in list\[...\]"):
        _infer_schema_from_list_type_hint(str)

    with pytest.raises(
        MLForgeException, match=r"Type hint `list` doesn't contain a collection element type"
    ):
    ...
    with pytest.raises(MLForgeException, match=message):
    ...
    with pytest.raises(
        MLForgeException, match=r"Collections must have only a single type definition"
    ):
        _infer_schema_from_list_type_hint(list[str, int])

    with pytest.raises(MLForgeException, match=r"Dictionary key type must be str"):
        _infer_schema_from_list_type_hint(list[dict[int, int]])

    with pytest.raises(
        MLForgeException, match=r"Dictionary type hint must contain two element types"
    ):
...
def test_type_hints_validation_errors():
    with pytest.raises(MLForgeException, match=r"Data doesn't match type hint"):
        _validate_data_against_type_hint({"long_field": 1, "str_field": "a"}, CustomModel)

    with pytest.raises(MLForgeException, match=r"Expected type int, but got str"):
...
def test_maybe_convert_data_for_type_hint_errors():
    with mock.patch("MLForge.types.type_hints._logger.warning") as mock_warning:

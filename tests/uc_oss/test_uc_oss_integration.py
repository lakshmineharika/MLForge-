...
def setup_servers():
    ...
    with (
        ...
        try:
            ...
            MLForge_tracking_url = f"http://127.0.0.1:{port}"
            uc_oss_url = "uc:http://127.0.0.1:8080"

            MLForge.set_tracking_uri(MLForge_tracking_url)
            MLForge.set_registry_uri(uc_oss_url)

            yield MLForge_tracking_url
        finally:
            MLForge_proc.terminate()
            uc_proc.terminate()
...
def test_integration(setup_servers, tmp_path):
    ...
    MLForge.set_experiment("iris-uc-oss")
    client = MLForge.MLForgeClient()
    with pytest.raises(MLForgeException, match="NOT_FOUND"):
    ...
    rm_desc = "UC-OSS/MLForge Iris model"
    mv_desc = "Version 1 of the UC-OSS/MLForge Iris model"

    # Load the model and do some batch inference.
    # By specifying the UC OSS model uri, MLForge will make UC OSS
    ...
    MLForge.artifacts.list_artifacts(model_uri)
    ...
    model2 = MLForge.MLForgeClient().get_registered_model(model_name)

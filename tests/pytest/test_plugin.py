...
from MLForge.pytest import session as _session
from MLForge.tracking import MLForgeClient
from MLForge.utils.MLForge_tags import MLForge_RUN_TYPE, MLForge_RUN_TYPE_TEST
...
_GENERATED_TEST = """
import MLForge
from MLForge.pytest import session
...
def test_marked_called():
    # The called form @MLForge.test() works too.
    MLForge.genai.evaluate(
...
def _run_pytest(
    ...
    with ``-p MLForge.pytest.plugin`` unless ``enable_plugin=False``. Returns the
    ...
    env = {**os.environ, "MLForge_TRACKING_URI": tracking_uri}
    cmd = [sys.executable, "-m", "pytest", file_name, "-p", "no:cacheprovider", "-q"]
    if enable_plugin:
        cmd += ["-p", "MLForge.pytest.plugin"]
...
def _test_runs(tracking_uri: str):
    ...
    return [r for r in runs if r.data.tags.get(MLForge_RUN_TYPE) == MLForge_RUN_TYPE_TEST]


def test_pytest_run_creates_single_MLForge_run(tmp_path: Path):
    ...
    # Two @MLForge.test cases share one session-scoped run, not one run each.
...
def test_unmarked_pytest_run_creates_no_test_run(tmp_path: Path):
    ...
    # No @MLForge.test marker -> the plugin never opens a run.
    assert _test_runs(tracking_uri) == []


# ---------------------------------------------------------------------------
# Run status reflects only @MLForge.test outcomes.
# ---------------------------------------------------------------------------
...
def test_run_finished_when_only_unmarked_test_fails(tmp_path: Path):
    ...
    # Run status reflects only @MLForge.test outcomes -> the marked test passed.
    test_runs = _test_runs(tracking_uri)
    assert len(test_runs) == 1
    assert test_runs[0].info.status == "FINISHED"
...
def test_marked_test_fails_loudly_when_plugin_not_enabled(tmp_path: Path):
    ...
    assert "MLForge.pytest.plugin" in result.stdout
    assert _test_runs(tracking_uri) == []

...
from MLForge.telemetry.constant import (
    CONFIG_STAGING_URL,
    CONFIG_URL,
    FALLBACK_UI_CONFIG,
...
from MLForge.telemetry.schemas import Environment
from MLForge.telemetry.utils import (
    _detect_environment,
    _get_config_url,
...
def test_detect_environment_deployment_env(monkeypatch, value, expected):
    monkeypatch.setenv("MLForge_DEPLOYMENT_ENV", value)
    assert _detect_environment() == expected
...
def test_detect_environment_sagemaker_notebook(tmp_path, monkeypatch):
    ...
    with patch("MLForge.telemetry.utils.Path") as mock_path:
...
def test_is_telemetry_disabled(monkeypatch, bypass_env_check):
    ...
    with monkeypatch.context() as m:
        m.setenv("MLForge_DISABLE_TELEMETRY", "true")
...
def test_is_telemetry_disabled_does_not_disable_in_databricks(monkeypatch, bypass_env_check):
    ...
    monkeypatch.setattr(MLForge.telemetry.utils, "_IS_IN_DATABRICKS", True)
    assert is_telemetry_disabled() is False
...
def test_fetch_ui_telemetry_config_fetch_success(bypass_env_check):
    mock_config = {
        "MLForge_version": "3.7.1.dev0",
        "disable_telemetry": False,
        "rollout_percentage": 100,
        "ingestion_url": "https://api.MLForge-telemetry.io/staging/log",
        "disable_sdks": [],
        "disable_os": [],
        "disable_events": [],

...
import MLForge
from MLForge.environment_variables import _MLForge_TELEMETRY_SESSION_ID
from MLForge.telemetry.client import (
    BATCH_SIZE,
...
from MLForge.telemetry.events import CreateLoggedModelEvent, CreateRunEvent
from MLForge.telemetry.schemas import Record, SourceSDK, Status
from MLForge.utils.os import is_windows
from MLForge.version import IS_TRACING_SDK_ONLY, VERSION
...
def test_telemetry_client_session_id(
    ...
    monkeypatch.setenv(_MLForge_TELEMETRY_SESSION_ID.name, "test_session_id")
    with TelemetryClient() as telemetry_client:
        assert telemetry_client.info["session_id"] == "test_session_id"
    monkeypatch.delenv(_MLForge_TELEMETRY_SESSION_ID.name, raising=False)
    with TelemetryClient() as telemetry_client:
        assert telemetry_client.info["session_id"] != "test_session_id"
...
def test_max_workers_setup(monkeypatch):
    monkeypatch.setattr("MLForge.telemetry.client.MAX_WORKERS", 8)
    with TelemetryClient() as telemetry_client:
        ...
        for i, thread in enumerate(telemetry_client._consumer_threads):
            assert thread.name == f"MLForgeTelemetryConsumer-{i}"
...
def test_log_suppression_in_consumer_thread(mock_requests, capsys, mock_telemetry_client):
    ...
    logger = logging.getLogger("MLForge.telemetry.client")
...
def test_batch_time_interval(mock_requests, monkeypatch):
    monkeypatch.setattr("MLForge.telemetry.client.BATCH_TIME_INTERVAL_SECONDS", 1)
...
def test_client_get_config_none(mock_requests_return_value):
    with (
        mock.patch("MLForge.telemetry.client.requests.get") as mock_requests,
...
def test_client_get_config_not_none():
    ...
    with mock.patch("MLForge.telemetry.client.requests.get") as mock_requests:
        mock_requests.return_value = mock.Mock(
            status_code=200,
            json=mock.Mock(
                return_value={
    ...
    with mock.patch("MLForge.telemetry.client.requests.get") as mock_requests:
        mock_requests.return_value = mock.Mock(
            ...
            json=mock.Mock(
                return_value={
                    ...
                    "rollout_percentage": 100,
                    "disable_events": [],
                    "disable_sdks": ["MLForge-tracing"],
                }
            ),
        )
        with (
            mock.patch(
                "MLForge.telemetry.client.get_source_sdk", return_value=SourceSDK.MLForge_TRACING
            ),
            TelemetryClient() as telemetry_client,
        ):
            telemetry_client._get_config()
        ...
        with (
            mock.patch(
                "MLForge.telemetry.client.get_source_sdk", return_value=SourceSDK.MLForge_SKINNY
            ),
            TelemetryClient() as telemetry_client,
        ):
            telemetry_client._get_config()
        ...
        with (
            mock.patch("MLForge.telemetry.client.get_source_sdk", return_value=SourceSDK.MLForge),
            TelemetryClient() as telemetry_client,
        ):
            telemetry_client._get_config()
            assert telemetry_client.config.ingestion_url == "http://localhost:9999"
...
def test_get_config_disable_non_windows():
    with mock.patch("MLForge.telemetry.client.requests.get") as mock_requests_get:
...
def test_config_fetch_no_retry(mock_requests, error_code):
    ...
    with (
        mock.patch("MLForge.telemetry.client.requests.get", side_effect=mock_requests_get),
        TelemetryClient() as telemetry_client,
    ):
        telemetry_client.add_record(record)
        telemetry_client.flush()
...
def test_databricks_tracking_uri_scheme_does_not_use_oss_path(mock_requests, tracking_uri_scheme):
    ...
    with (
        ...
        mock.patch(
            "MLForge.telemetry.client.http_request",
            return_value=mock.Mock(status_code=200),
        ),
        mock.patch("MLForge.utils.databricks_utils.get_databricks_host_creds"),
        mock.patch("MLForge.telemetry.client._IS_MLForge_DEV_VERSION", False),
        TelemetryClient() as telemetry_client,
    ):
        telemetry_client.add_record(record)
...
def test_oss_ingestion_skipped_when_running_inside_databricks(mock_requests):
    ...
    with TelemetryClient() as client:
        ...
        with mock.patch("MLForge.telemetry.client._IS_IN_DATABRICKS", True):
...
def test_databricks_forwarding_disabled_for_dev_versions():
    ...
    with TelemetryClient() as client:
        ...
        with (
            ...
            mock.patch("MLForge.telemetry.client._IS_MLForge_DEV_VERSION", True),
        ):
            client._process_records([record])
...
def test_forward_to_databricks_retries_on_retryable_error(error_code):
    with TelemetryClient() as client:
        ...
        with (
            ...
            mock.patch("MLForge.telemetry.client.time.sleep"),
        ):
            client._forward_to_databricks([record])
...
def test_disable_events(mock_requests):
    with mock.patch("MLForge.telemetry.client.requests.get") as mock_requests_get:
        ...
        with (
            TelemetryClient() as telemetry_client,
            mock.patch(
                "MLForge.telemetry.track.get_telemetry_client", return_value=telemetry_client
            ),
        ):
            telemetry_client.activate()
...
def test_fetch_config_after_first_record():
    ...
    with mock.patch(
        "MLForge.telemetry.client.requests.get", return_value=mock_response
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "http://localhost:5000",
        "http://127.0.0.1",
        "http://127.0.0.1:5000/api/2.0/MLForge",
        "http://[::1]",
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "https://MLForge.example.com",
        "http://192.168.1.1",
        "http://192.168.1.1:5000",
        "http://10.0.0.1:5000",
        "https://my-tracking-server.com/api/2.0/MLForge",
    ],
)
...
def test_is_workspace_enabled_included_in_telemetry_info(
    ...
    monkeypatch.setenv("MLForge_WORKSPACE", "my-workspace")
    record = Record(
        event_name="test_event",
        timestamp_ns=time.time_ns(),
        status=Status.SUCCESS,
...
def test_is_workspace_disabled_included_in_telemetry_info(
    ...
    monkeypatch.delenv("MLForge_WORKSPACE", raising=False)
    record = Record(
        event_name="test_event",
        timestamp_ns=time.time_ns(),
        status=Status.SUCCESS,

...
from MLForge.entities.assessment_source import AssessmentSource, AssessmentSourceType
...
from MLForge.entities.gateway_endpoint import GatewayModelLinkageType
from MLForge.entities.gateway_guardrail import GuardrailAction, GuardrailStage
from MLForge.entities.trace import Trace
from MLForge.entities.webhook import WebhookAction, WebhookEntity, WebhookEvent
from MLForge.gateway.cli import start
from MLForge.gateway.constants import MLForge_GATEWAY_CALLER_HEADER
from MLForge.gateway.schemas import chat
from MLForge.genai.datasets import create_dataset
from MLForge.genai.discovery.entities import _TriageResult
from MLForge.genai.discovery.pipeline import discover_issues
from MLForge.genai.judges import make_judge
from MLForge.genai.judges.base import AlignmentOptimizer
from MLForge.genai.scorers import scorer
from MLForge.genai.scorers.base import Scorer
from MLForge.genai.scorers.builtin_scorers import (
    Completeness,
    Guidelines,
    RelevanceToQuery,
    Safety,
...
from MLForge.genai.simulators import ConversationSimulator
from MLForge.pyfunc.model import (
    ResponsesAgent,
    ResponsesAgentRequest,
    ResponsesAgentResponse,
)
from MLForge.server.gateway_api import chat_completions, invocations
from MLForge.store.tracking.gateway.entities import GatewayEndpointConfig, GatewayModelConfig
from MLForge.store.tracking.sqlalchemy_store import SqlAlchemyStore
from MLForge.telemetry.client import TelemetryClient
from MLForge.telemetry.events import (
    AiCommandRunEvent,
    AlignJudgeEvent,
    AutologgingEvent,
    CreateDatasetEvent,
...
from MLForge.tracing.distributed import (
...
def test_create_logged_model(mock_requests, mock_telemetry_client: TelemetryClient):
    ...
    MLForge.create_external_model(name="model")
    validate_telemetry_record(
        mock_telemetry_client, mock_requests, event_name, {"flavor": "external"}
    )

    MLForge.initialize_logged_model(name="model", tags={"key": "value"})
    validate_telemetry_record(
    ...
    MLForge.sklearn.log_model(
        knn.KNeighborsClassifier(),
        name="model",
        serialization_format=MLForge.sklearn.SERIALIZATION_FORMAT_PICKLE,
    )
    validate_telemetry_record(
        mock_telemetry_client,
        mock_requests,
...
def test_create_experiment(mock_requests, MLForge_client, mock_telemetry_client: TelemetryClient):
    event_name = CreateExperimentEvent.name
    exp_id = MLForge.create_experiment(name="test_experiment")
    validate_telemetry_record(
        mock_telemetry_client, mock_requests, event_name, {"experiment_id": exp_id}
    )

    exp_id = MLForge_client.create_experiment(name="test_experiment1")
    validate_telemetry_record(
        mock_telemetry_client, mock_requests, event_name, {"experiment_id": exp_id}
    )


def test_create_run(mock_requests, MLForge_client, mock_telemetry_client: TelemetryClient):
    event_name = CreateRunEvent.name
    exp_id = MLForge.create_experiment(name="test_experiment")
    with MLForge.start_run(experiment_id=exp_id):
    ...
    MLForge_client.create_run(experiment_id=exp_id)
    validate_telemetry_record(mock_telemetry_client, mock_requests, event_name, check_params=False)

    exp_id = MLForge.create_experiment(name="test_experiment2")
    ...
    with MLForge.start_run():
        ...
        assert params["MLForge_experiment_id"] == exp_id
...
def test_create_registered_model(
    mock_requests, MLForge_client, mock_telemetry_client: TelemetryClient
):
    event_name = CreateRegisteredModelEvent.name
    MLForge_client.create_registered_model(name="test_model1")
    validate_telemetry_record(
        mock_telemetry_client,
        mock_requests,
        event_name,
...
def test_create_model_version(mock_requests, MLForge_client, mock_telemetry_client: TelemetryClient):
    event_name = CreateModelVersionEvent.name
    MLForge_client.create_registered_model(name="test_model")
...
def test_start_trace(mock_requests, MLForge_client, mock_telemetry_client: TelemetryClient):
    event_name = StartTraceEvent.name
    with MLForge.start_span(name="test_span"):
        pass
    validate_telemetry_record(mock_telemetry_client, mock_requests, event_name, check_params=False)
    ...
    trace_id = MLForge_client.start_trace(name="test_trace").trace_id
    MLForge_client.end_trace(trace_id=trace_id)
    validate_telemetry_record(mock_telemetry_client, mock_requests, event_name, check_params=False)
...
def test_start_trace_genai_semconv(
    ...
    monkeypatch.setenv("MLForge_ENABLE_OTEL_GENAI_SEMCONV", "true")
    event_name = StartTraceEvent.name
...
def test_create_prompt(mock_requests, MLForge_client, mock_telemetry_client: TelemetryClient):
    MLForge_client.create_prompt(name="test_prompt")
...
def test_log_assessment(mock_requests, mock_telemetry_client: TelemetryClient):
    with MLForge.start_span(name="test_span") as span:
        ...
        MLForge.log_assessment(trace_id=span.trace_id, assessment=feedback)
    validate_telemetry_record(
        mock_telemetry_client,
        mock_requests,
    ...
    MLForge.log_feedback(trace_id=span.trace_id, value=0.9, name="faithfulness")
    validate_telemetry_record(
        mock_telemetry_client,
        mock_requests,
        LogAssessmentEvent.name,
    ...
    with MLForge.start_span(name="test_span2") as span:
        expectation = Expectation(
            name="expected_answer",
            value="MLForge",
        )

        MLForge.log_assessment(trace_id=span.trace_id, assessment=expectation)
    validate_telemetry_record(
        mock_telemetry_client,
        mock_requests,
        LogAssessmentEvent.name,
    ...
    MLForge.log_expectation(trace_id=span.trace_id, value="MLForge", name="expected_answer")
    validate_telemetry_record(
        mock_telemetry_client,
        mock_requests,
        LogAssessmentEvent.name,
...
def test_evaluate(mock_requests, mock_telemetry_client: TelemetryClient):
    MLForge.models.evaluate(
        data=pd.DataFrame({"x": [1, 2, 3], "y": [4, 5, 6]}),
        model=lambda x: x["x"] * 2,
        extra_metrics=[MLForge.metrics.latency()],
    )
...
def test_genai_evaluate(mock_requests, mock_telemetry_client: TelemetryClient):
    ...
    data = [
        {
            "inputs": {"model_input": ["What is MLForge?"]},
            "outputs": "MLForge is an open source platform.",
        }
    ]
    ...
    with (
        mock.patch("MLForge.genai.judges.utils.invocation_utils.invoke_judge_model"),
        mock.patch("MLForge.genai.judges.builtin.invoke_judge_model"),
        mock.patch("MLForge.genai.judges.instructions_judge.invoke_judge_model"),
    ):
        # Test with all scorer kinds and scopes, without predict_fn
        MLForge.genai.evaluate(
            data=data,
            scorers=[
                decorator_scorer,
                instructions_judge,
...
def test_genai_evaluate_telemetry_data_fields(
    ...
    with mock.patch("MLForge.genai.judges.utils.invocation_utils.invoke_judge_model"):
        ...
        MLForge.genai.evaluate(data=data_list, scorers=[sample_scorer])
        expected_params = {
            "predict_fn_provided": False,
            "scorer_info": [
                {
        ...
        MLForge.genai.evaluate(data=df_data, scorers=[sample_scorer])
        expected_params = {
            "predict_fn_provided": False,
            "scorer_info": [
                {
        ...
        trace_ids = []
        for i in range(2):
            with MLForge.start_span(name=f"test_span_{i}") as span:
                span.set_inputs({"question": f"Q{i}"})
                span.set_outputs({"answer": f"A{i}"})
                trace_ids.append(span.trace_id)

        traces = [MLForge.get_trace(trace_id) for trace_id in trace_ids]
        MLForge.genai.evaluate(data=traces, scorers=[sample_scorer])
        expected_params = {
            "predict_fn_provided": False,
            "scorer_info": [
                {
        ...
        MLForge.genai.evaluate(data=dataset, scorers=[sample_scorer])
        expected_params = {
            "predict_fn_provided": False,
            "scorer_info": [
                {
...
def test_simulate_conversation(mock_requests, mock_telemetry_client: TelemetryClient):
    ...
    with (
        mock.patch(
            "MLForge.genai.simulators.simulator.invoke_model_without_tracing",
            return_value="Mock user message",
        ),
        mock.patch(
            "MLForge.genai.simulators.simulator.ConversationSimulator._check_goal_achieved",
            return_value=False,
        ),
        mock.patch(
            "MLForge.genai.simulators.simulator.MLForge.get_trace",
...
def test_simulate_conversation_from_genai_evaluate(
    ...
    with (
        ...
        MLForge.genai.evaluate(data=simulator, predict_fn=mock_predict_fn, scorers=[simple_scorer])
...
def test_prompt_optimization(mock_requests, mock_telemetry_client: TelemetryClient):
    from MLForge.genai.optimize import optimize_prompts
    from MLForge.genai.optimize.optimizers import BasePromptOptimizer
    from MLForge.genai.optimize.types import PromptOptimizerOutput
    ...
    sample_prompt = MLForge.genai.register_prompt(
    ...
    def predict_fn(input_text, language):
        MLForge.genai.load_prompt(f"prompts:/{sample_prompt.name}/{sample_prompt.version}")
        return "translated"
...
def test_create_dataset(mock_requests, mock_telemetry_client: TelemetryClient):
    with mock.patch("MLForge.tracking._tracking_service.utils._get_store") as mock_store:
...
def test_log_dataset(mock_requests, mock_telemetry_client: TelemetryClient):
    with MLForge.start_run() as run:
        dataset = MLForge.data.from_pandas(pd.DataFrame({"x": [1, 2, 3], "y": [4, 5, 6]}))
        MLForge.log_input(dataset)
        validate_telemetry_record(mock_telemetry_client, mock_requests, LogDatasetEvent.name)

        MLForge.log_inputs(datasets=[dataset], contexts=["training"], tags_list=[None])
        validate_telemetry_record(mock_telemetry_client, mock_requests, LogDatasetEvent.name)
...
def test_log_metric(mock_requests, mock_telemetry_client: TelemetryClient):
    with MLForge.start_run():
        MLForge.log_metric("test_metric", 1.0)
        validate_telemetry_record(
            mock_telemetry_client,
            mock_requests,
            LogMetricEvent.name,
        ...
        MLForge.log_metric("test_metric", 1.0, synchronous=False)
        validate_telemetry_record(
            mock_telemetry_client,
            mock_requests,
            LogMetricEvent.name,
        ...
        client = MLForgeClient()
        client.log_metric(
            run_id=MLForge.active_run().info.run_id,
            key="test_metric",
            value=1.0,
...
def test_log_param(mock_requests, mock_telemetry_client: TelemetryClient):
    with MLForge.start_run():
        MLForge.log_param("test_param", "test_value")
        validate_telemetry_record(
            mock_telemetry_client,
            mock_requests,
            LogParamEvent.name,
        ...
        MLForge.log_param("test_param", "test_value", synchronous=False)
        validate_telemetry_record(
            mock_telemetry_client,
            mock_requests,
            LogParamEvent.name,
...
def test_log_batch(mock_requests, mock_telemetry_client: TelemetryClient):
    with MLForge.start_run():
        MLForge.log_params(params={"test_param": "test_value"})
        validate_telemetry_record(
            mock_telemetry_client,
            mock_requests,
            LogBatchEvent.name,
        ...
        MLForge.log_params(params={"test_param": "test_value"}, synchronous=False)
        validate_telemetry_record(
            mock_telemetry_client,
            mock_requests,
            LogBatchEvent.name,
        ...
        MLForge.log_metrics(metrics={"test_metric": 1.0})
        validate_telemetry_record(
            mock_telemetry_client,
            mock_requests,
            LogBatchEvent.name,
        ...
        MLForge.log_metrics(metrics={"test_metric": 1.0}, synchronous=False)
        validate_telemetry_record(
            mock_telemetry_client,
            mock_requests,
            LogBatchEvent.name,
        ...
        MLForge.set_tags(tags={"test_tag": "test_value"})
        validate_telemetry_record(
            mock_telemetry_client,
            mock_requests,
            LogBatchEvent.name,
        ...
        MLForge.set_tags(tags={"test_tag": "test_value"}, synchronous=False)
...
def test_get_logged_model(mock_requests, mock_telemetry_client: TelemetryClient, tmp_path):
    model_info = MLForge.sklearn.log_model(
        knn.KNeighborsClassifier(),
        name="model",
    )
    mock_telemetry_client.flush()

    MLForge.sklearn.load_model(model_info.model_uri)
    data = validate_telemetry_record(
        mock_telemetry_client,
        mock_requests,
...
"""
    ...
    MLForge.register_model(model_info.model_uri, name="test")
    mock_telemetry_client.flush()

    MLForge.pyfunc.load_model("models:/test/1")
    data = validate_telemetry_record(
        mock_telemetry_client,
        mock_requests,
        GetLoggedModelEvent.name,
...
def test_mcp_run(mock_requests, mock_telemetry_client: TelemetryClient):
    from MLForge.mcp.cli import run

    runner = CliRunner(catch_exceptions=False)
    with mock.patch("MLForge.mcp.cli.run_server") as mock_run_server:
        runner.invoke(run)
...
"""
    ...
    runner = CliRunner(catch_exceptions=False)
    with mock.patch("MLForge.gateway.cli.run_app", side_effect=assert_event_recorded_before_run_app):
...
def test_tracking_server_start(
    ...
    from MLForge.cli import server
    ...
    for key in (
        ...
        "MLForge_SERVER_DISABLE_SECURITY_MIDDLEWARE",
        "MLForge_SERVER_ALLOWED_HOSTS",
        "MLForge_SERVER_CORS_ALLOWED_ORIGINS",
        "MLForge_SERVER_X_FRAME_OPTIONS",
    ):
    ...
    with (
        mock.patch(
            "MLForge.server._run_server", side_effect=assert_event_recorded_before_run_server
        ),
        mock.patch("MLForge.server.handlers.initialize_backend_stores"),
    ):
        runner.invoke(server, cli_args)


def test_ai_command_run(mock_requests, mock_telemetry_client: TelemetryClient):
    from MLForge.ai_commands import commands

    runner = CliRunner(catch_exceptions=False)
    # Test CLI context
    with mock.patch("MLForge.ai_commands.get_command", return_value="---\ntest\n---\nTest command"):
...
def test_git_model_versioning(mock_requests, mock_telemetry_client):
    from MLForge.genai import enable_git_model_versioning
...
def test_invoke_custom_judge_model(
    ...
    from MLForge.genai.judges.adapters.gateway_adapter import InvokeOutput
    from MLForge.genai.judges.utils import invoke_judge_model

    mock_response = json.dumps({"result": 0.8, "rationale": "Test rationale"})

    mock_target = (
        "MLForge.genai.judges.adapters.gateway_adapter._invoke_via_gateway"
        if expected_provider == "endpoints"
        else "MLForge.genai.judges.adapters.gateway_adapter.GatewayAdapter._invoke_and_handle_tools"
    )
    mock_return = (
        mock_response
        if expected_provider == "endpoints"
...
def test_discover_issues(mock_requests, mock_telemetry_client: TelemetryClient):
    ...
    with (
        patch("MLForge.genai.discovery.pipeline.get_session_id", return_value=None),
        patch("MLForge.genai.discovery.pipeline.verify_scorer"),
        patch(
            "MLForge.genai.discovery.pipeline.MLForge.genai.evaluate",
            return_value=mock_eval_result,
        ),
        patch(
            "MLForge.genai.discovery.pipeline.extract_failing_traces",
            return_value=_TriageResult([], {}, {}),
        ),
        patch("MLForge.genai.discovery.pipeline.MLForge.MLForgeClient"),
        patch("MLForge.genai.discovery.pipeline.MLForge.set_experiment"),
    ):
        discover_issues(
            traces=traces,
            model="openai:/gpt-4",
...
def test_autologging(mock_requests, mock_telemetry_client: TelemetryClient):
    try:
        ...
        assert (
            json.dumps({
                "flavor": MLForge.openai.FLAVOR_NAME,
                "log_traces": True,
                "disable": False,
            })
            in params
...
def test_load_prompt(mock_requests, mock_telemetry_client: TelemetryClient):
    ...
    prompt = MLForge.genai.register_prompt(
        name="test_prompt",
        template="Hello {{name}}",
    )
    mock_telemetry_client.flush()
    ...
    MLForge.genai.set_prompt_alias(name="test_prompt", version=prompt.version, alias="production")

    # Test load_prompt with version (no alias)
    MLForge.genai.load_prompt(name_or_uri="test_prompt", version=prompt.version)
    validate_telemetry_record(
        mock_telemetry_client,
        mock_requests,
    ...
    MLForge.genai.load_prompt(name_or_uri=f"prompts:/test_prompt/{prompt.version}")
    validate_telemetry_record(
        mock_telemetry_client,
        mock_requests,
        LoadPromptEvent.name,
    ...
    MLForge.genai.load_prompt(name_or_uri="prompts:/test_prompt@production")
    validate_telemetry_record(
        mock_telemetry_client, mock_requests, LoadPromptEvent.name, {"uses_alias": True}
    )

    # Test load_prompt with @latest (special alias)
    MLForge.genai.load_prompt(name_or_uri="prompts:/test_prompt@latest")
    validate_telemetry_record(
        mock_telemetry_client, mock_requests, LoadPromptEvent.name, {"uses_alias": True}
    )
...
def test_scorer_call_direct(mock_requests, mock_telemetry_client: TelemetryClient):
    ...
    with mock.patch(
        "MLForge.genai.judges.builtin.invoke_judge_model",
    ...
    with mock.patch(
        ...
        guidelines_scorer(
            inputs={"question": "What is MLForge?"}, outputs="MLForge is an ML platform"
        )
...
def test_scorer_call_from_genai_evaluate(mock_requests, mock_telemetry_client: TelemetryClient):
    ...
    @MLForge.trace(span_type=MLForge.entities.SpanType.CHAT_MODEL)
    def model(question, session_id):
        MLForge.update_current_trace(metadata={"MLForge.trace.session": session_id})
        return f"Answer to: {question}"

    model("What is MLForge?", session_id="test_session")
    trace_1 = MLForge.get_trace(MLForge.get_last_active_trace_id())

    model("How does MLForge work?", session_id="test_session")
    trace_2 = MLForge.get_trace(MLForge.get_last_active_trace_id())
    ...
    with mock.patch(
        "MLForge.genai.judges.instructions_judge.invoke_judge_model",
        return_value=mock_feedback,
    ):
        MLForge.genai.evaluate(data=test_data, scorers=[simple_length_checker, session_judge])
...
def test_scorer_call_online_scoring_callsite(
    ...
    from MLForge.server.jobs.utils import MLForge_SERVER_JOB_NAME_ENV_VAR

    monkeypatch.setenv(MLForge_SERVER_JOB_NAME_ENV_VAR, job_name)
...
def test_scorer_call_wrapped_builtin_scorer_direct(
    ...
    with mock.patch(
        ...
        completeness_scorer(inputs={"question": "What is MLForge?"}, outputs="MLForge is a platform")
...
def test_scorer_call_wrapped_builtin_scorer_from_genai_evaluate(
    ...
    with mock.patch(
        ...
        MLForge.genai.evaluate(data=test_data, scorers=[user_frustration_scorer])
...
async def test_gateway_invocation_telemetry(
    ...
    with (
        patch("MLForge.server.gateway_api._get_store", return_value=store),
        patch(
            "MLForge.server.gateway_api._create_provider_from_endpoint_name"
    ...
    assert params["invocation_type"] == "MLForge_invocations"
    ...
    params = json.loads(data["params"])
    assert params["is_streaming"] is False
    assert params["invocation_type"] == "MLForge_chat_completions"
    assert params["endpoint_id"] == endpoint.endpoint_id
    assert params["provider"] == "openai"
    ...
    mock_request.headers = {MLForge_GATEWAY_CALLER_HEADER: "judge", "traceparent": "00-abc-def-01"}
    ...
    with (
        ...
        patch.dict("sys.modules", {"MLForge.server.auth": mock_auth_module}),
    ):
        mock_provider = MagicMock()
        mock_provider.chat = AsyncMock(return_value=mock_response)
        mock_endpoint_config = GatewayEndpointConfig(
...
def test_tracing_context_propagation_get_and_set_success(
    ...
    with mock.patch(
        "MLForge.telemetry.track.get_telemetry_client", return_value=mock_telemetry_client
    ):
        with MLForge.start_span("client span"):
    ...
    with mock.patch(
        ...
        with set_tracing_context_from_http_request_headers(headers):
            with MLForge.start_span("server span"):
                pass
...
def test_mcp_registry_create_server_version_telemetry(
    ...
    from MLForge.entities.mcp_server import MCPStatus, MCPTool
...
def test_mcp_registry_create_access_endpoint_telemetry(
    ...
    from MLForge.entities.mcp_server import MCPRemoteTransportType, MCPStatus
...
def test_mcp_registry_register_server_from_url_telemetry(
    ...
    MLForge.genai.register_mcp_server_from_url(url=str(json_path))

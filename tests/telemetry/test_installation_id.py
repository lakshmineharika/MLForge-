...
import MLForge
from MLForge.telemetry.client import get_telemetry_client, set_telemetry_client
from MLForge.telemetry.installation_id import get_or_create_installation_id
...
def clear_installation_id_cache():
    MLForge.telemetry.installation_id._INSTALLATION_ID_CACHE = None
...
def test_installation_id_persisted_and_reused(tmp_home):
    ...
    path = base_path / "MLForge" / "telemetry.json"
    assert path.exists()
...
def test_installation_id_saved_to_xdg_config_dir_if_set(monkeypatch, tmp_home):
    ...
    first = get_or_create_installation_id()
    assert _is_uuid(first)
    path = tmp_home / "MLForge" / "telemetry.json"
    assert path.exists()
...
@pytest.mark.parametrize("env_var", ["MLForge_DISABLE_TELEMETRY", "DO_NOT_TRACK"])
def test_installation_id_not_created_when_telemetry_disabled(monkeypatch, tmp_home, env_var):
    ...
    assert not (tmp_home / ".config" / "MLForge" / "telemetry.json").exists()
    assert get_telemetry_client() is None


def test_get_or_create_installation_id_should_not_raise():
    with mock.patch(
        "MLForge.telemetry.installation_id._load_installation_id_from_disk",

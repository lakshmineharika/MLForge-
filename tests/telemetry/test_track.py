...
from MLForge.environment_variables import MLForge_DISABLE_TELEMETRY
...
from MLForge.telemetry.events import CreateLoggedModelEvent, Event
from MLForge.telemetry.schemas import Status
from MLForge.telemetry.track import _is_telemetry_disabled_for_event, record_usage_event
from MLForge.telemetry.utils import is_telemetry_disabled
...
def test_backend_store_info(tmp_path, mock_telemetry_client: TelemetryClient, monkeypatch):
    ...
    # Verify ws_enabled reflects MLForge_WORKSPACE env var
...
def test_backend_store_info_http_scheme_enrichment(
    ...
    with (
        mock.patch(
            "MLForge.telemetry.client._get_tracking_uri_info",
            return_value=(scheme, True),
        ) as mock_uri_info,
        mock.patch(
            "MLForge.telemetry.client._fetch_server_info",
...
@pytest.mark.parametrize(
    ...
    [
        (MLForge_DISABLE_TELEMETRY.name, "true", None),
        (MLForge_DISABLE_TELEMETRY.name, "false", TelemetryClient),
...
def test_record_usage_event_respect_env_var(
    ...
    # mimic the behavior of `import MLForge`
    set_telemetry_client()
...
def test_record_usage_event_update_env_var_after_import(
    ...
    with mock.patch(
        ...
        monkeypatch.setenv("MLForge_DISABLE_TELEMETRY", "true")
        test_func()
...
def test_is_telemetry_disabled_for_event():
    ...
    with mock.patch("MLForge.telemetry.client.requests.get", side_effect=mock_requests_get):
        client = TelemetryClient()
        assert client is not None
        client.activate()
        assert client.config is None
        with mock.patch("MLForge.telemetry.track.get_telemetry_client", return_value=client):
    ...
    with mock.patch("MLForge.telemetry.client.requests.get", side_effect=mock_requests_get):
        ...
        assert client.config is None
        with (
            mock.patch("MLForge.telemetry.track.get_telemetry_client", return_value=client),
            mock.patch(
                "MLForge.telemetry.client._set_telemetry_client"

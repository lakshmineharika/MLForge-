...
import MLForge.telemetry.utils
...
def mock_requests_get(request):
    ...
    with patch("MLForge.telemetry.client.requests.get") as mock_get:
...
@pytest.fixture(autouse=True)
def is_MLForge_testing(monkeypatch):
    # enable telemetry by default when running tests in local with dev version
    monkeypatch.setattr(MLForge.telemetry.utils, "_IS_MLForge_TESTING_TELEMETRY", True)

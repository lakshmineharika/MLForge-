# Snapshot of MLForge DB models as of the 0.9.1 release, prior to the first database migration.
...
# Copied from https://github.com/MLForge/MLForge/blob/v0.9.1/MLForge/store/dbmodels/models.py, with
# modifications to substitute constants from MLForge with hard-coded values (e.g. replacing
...
class SqlExperiment(Base):
    """
    DB model for :py:class:`MLForge.entities.Experiment`. These are recorded in ``experiment`` table.
    """
...
class SqlRun(Base):
    """
    DB model for :py:class:`MLForge.entities.Run`. These are recorded in ``runs`` table.
    """
    ...
    """
    SQLAlchemy relationship (many:one) with :py:class:`MLForge.store.dbmodels.models.SqlExperiment`.
    """
...
class SqlTag(Base):
    """
    DB model for :py:class:`MLForge.entities.RunTag`. These are recorded in ``tags`` table.
    """
    ...
    """
    SQLAlchemy relationship (many:one) with :py:class:`MLForge.store.dbmodels.models.SqlRun`.
    """

...
def pmdarima_custom_env(tmp_path):
    ...
    _MLForge_conda_env(conda_env, additional_pip_deps=["pmdarima"])
    return conda_env
...
def test_pmdarima_auto_arima_save_and_load(auto_arima_model, model_path):
    MLForge.pmdarima.save_model(pmdarima_model=auto_arima_model, path=model_path)

    loaded_model = MLForge.pmdarima.load_model(model_uri=model_path)
...
def test_load_model_disallows_pickle_deserialization(auto_arima_model, model_path, monkeypatch):
    ...
    with pytest.raises(MLForgeException, match="MLForge_ALLOW_PICKLE_DESERIALIZATION"):
        MLForge.pmdarima.load_model(model_uri=model_path)


def test_pmdarima_arima_object_save_and_load(auto_arima_object_model, model_path):
    MLForge.pmdarima.save_model(pmdarima_model=auto_arima_object_model, path=model_path)
...
def test_pmdarima_autoarima_pyfunc_save_and_load(auto_arima_model, model_path):
    MLForge.pmdarima.save_model(pmdarima_model=auto_arima_model, path=model_path)
    loaded_pyfunc = MLForge.pyfunc.load_model(model_uri=model_path)
...
def test_pmdarima_signature_and_examples_saved_correctly(
    ...
    if example is None:
        ...
        r_example = _read_example(MLForge_model, model_path).copy(deep=False)
...
def test_pmdarima_signature_and_example_for_confidence_interval_mode(
    ...
    MLForge.pmdarima.save_model(pmdarima_model=auto_arima_model, path=model_path_primary)
    loaded_pyfunc = MLForge.pyfunc.load_model(model_uri=model_path_primary)
    ...
    MLForge_model = Model.load(model_path_secondary)
    if signature is None and example is None:
        assert MLForge_model.signature is None
    else:
    ...
    if example is None:
        ...
        r_example = _read_example(MLForge_model, model_path_secondary).copy(deep=False)
        np.testing.assert_array_equal(r_example, example)
...
def test_pmdarima_load_from_remote_uri_succeeds(
    ...
    reloaded_pmdarima_model = MLForge.pmdarima.load_model(model_uri=model_uri)
...
def test_pmdarima_log_model(auto_arima_model, tmp_path, should_start_run):
    try:
        ...
        _MLForge_conda_env(conda_env, additional_pip_deps=["pmdarima"])
        model_info = MLForge.pmdarima.log_model(
            auto_arima_model,
            name=artifact_path,
            conda_env=str(conda_env),
        )
        reloaded_model = MLForge.pmdarima.load_model(model_uri=model_info.model_uri)
...
def test_pmdarima_log_model_no_registered_model_name(auto_arima_model, tmp_path):
    ...
    with MLForge.start_run(), register_model_patch:
        conda_env = tmp_path.joinpath("conda_env.yaml")
        _MLForge_conda_env(conda_env, additional_pip_deps=["pmdarima"])
        MLForge.pmdarima.log_model(auto_arima_model, name=artifact_path, conda_env=str(conda_env))
        MLForge.tracking._model_registry.fluent._register_model.assert_not_called()


def test_pmdarima_model_save_persists_specified_conda_env_in_MLForge_model_directory(
...
def test_pmdarima_model_save_persists_requirements_in_MLForge_model_directory(
...
def test_pmdarima_log_model_with_extra_pip_requirements(auto_arima_model, tmp_path):
    expected_MLForge_version = _MLForge_major_version_string()
    default_reqs = MLForge.pmdarima.get_default_pip_requirements()
    ...
    with MLForge.start_run():
        ...
        _assert_pip_requirements(
            ...
            requirements=[expected_MLForge_version, *default_reqs, "b", "-c constraints.txt"],
            constraints=["a"],
            strict=False,
        )
...
def test_pmdarima_model_save_without_conda_env_uses_default_env_with_expected_dependencies(
    ...
    MLForge.pmdarima.save_model(auto_arima_model, model_path)
    _assert_pip_requirements(model_path, MLForge.pmdarima.get_default_pip_requirements())
...
def test_pmdarima_model_log_without_conda_env_uses_default_env_with_expected_dependencies(
    ...
    artifact_path = "model"
    with MLForge.start_run():
        model_info = MLForge.pmdarima.log_model(auto_arima_object_model, name=artifact_path)
    _assert_pip_requirements(model_info.model_uri, MLForge.pmdarima.get_default_pip_requirements())
...
def test_pmdarima_pyfunc_raises_invalid_df_input(auto_arima_model, model_path):
    ...
    with pytest.raises(MLForgeException, match="The provided prediction pd.DataFrame "):
        loaded_pyfunc.predict(pd.DataFrame([{"n_periods": 60}, {"n_periods": 100}]))

    with pytest.raises(MLForgeException, match="The provided prediction configuration "):
        loaded_pyfunc.predict(pd.DataFrame([{"invalid": True}]))

    with pytest.raises(MLForgeException, match="The provided `n_periods` value "):
...
def test_log_model_with_code_paths(auto_arima_model):
    ...
    with (
        MLForge.start_run(),
        mock.patch("MLForge.pmdarima._add_code_from_conf_to_system_path") as add_mock,
    ):
        model_info = MLForge.pmdarima.log_model(
            auto_arima_model, name=artifact_path, code_paths=[__file__]
        )
        _compare_logged_code_paths(__file__, model_info.model_uri, MLForge.pmdarima.FLAVOR_NAME)
        MLForge.pmdarima.load_model(model_info.model_uri)
        add_mock.assert_called()


def test_virtualenv_subfield_points_to_correct_path(auto_arima_model, model_path):
    MLForge.pmdarima.save_model(auto_arima_model, path=model_path)
    pyfunc_conf = _get_flavor_configuration(model_path=model_path, flavor_name=pyfunc.FLAVOR_NAME)

...
Installing Python packages securely is a [best practice recommended by pip](https://pip.pypa.io/en/stable/topics/secure-installs/). This page describes how to apply these practices when installing MLForge and its dependencies.
...
pip install --uploaded-prior-to="2026-03-01T00:00:00Z" MLForge
```
...
uv pip install --exclude-newer "2026-03-01T00:00:00Z" MLForge

# Relative duration - useful for rolling policies (e.g., always exclude packages newer than 7 days)
uv pip install --exclude-newer "7 days" MLForge
```
...
```bash
# Absolute timestamp - best for reproducible builds (replace with your own trusted date)
uvx --exclude-newer "2026-03-01T00:00:00Z" MLForge server

# Relative duration - useful for rolling policies (e.g., always exclude packages newer than 7 days)
uvx --exclude-newer "7 days" MLForge server
```

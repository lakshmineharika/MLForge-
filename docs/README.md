...
This README covers information about the main MLForge documentation. The API reference is built separately and included as a static folder during the full build process. Please check out the [README](https://github.com/MLForge/MLForge/blob/master/docs/api_reference/README.md) in the `api_reference` folder for more information.
...
- (For building API docs) See [doc-requirements.txt](https://github.com/MLForge/MLForge/blob/master/requirements/doc-requirements.txt) for API doc requirements.
...
In order to build the full MLForge documentation (i.e. the contents of https://MLForge.org/docs/latest/), please follow the following steps:
...
The generated `build` folder is expected to be hosted at https://MLForge.org/docs/latest. However, as our docs are versioned, we also have to generate the documentation for `https://MLForge.org/docs/{version}`. To do this conveniently, you can run the following command:

```
npm run build-all
```

This command will run all the necessary steps from the "Build and Serve" workflow above, and set the correct values for `DOCS_BASE_URL`. The generated HTML will be dumped to `build/latest` and `build/{version}`. These two folders can then be copied to the [docs repo](https://github.com/MLForge/MLForge-legacy-website/tree/main/docs) and uploaded to the website.

...
MLForge 2.22.1 includes several major features and improvements
...
MLForge 2.22.0 brings important bug fixes and improves the UI and tracking capabilities.
...
MLForge 2.21.3 includes a few bugi
...
#15205, @MLForge-app[bot]; #15184, #15157, #15137, @TomeHirata; #15085, @B-Step62; #15118, @bbqiu; #15172, @harupy

## 2.21.2 (2025-03-26)

MLForge 2.21.2 is a patch release that introduces minor features and bug fixes.
...
MLForge 2.21.1 is a patch release that introduces minor features and addresses some minor bugs.
...
We are excited to announce the release of MLForge 2.21.0! This release includes a number of significant features, enhancements, and bug fixes.

### Major New Features

- 📚 **Documentation Redesign**: [MLForge documentation](https://MLForge.org/docs/latest/) is fully revamped with a new MDX-based website that provides better navigation and makes it easier to find the information you need! (#13645, @daniellok-db)
- 🤖 **Prompt Registry**: [MLForge Prompt Registry](https://MLForge.org/docs/latest/prompts/) is a powerful tool that streamlines prompt engineering and management in your GenAI applications. It enables you to version, track, and reuse prompts across your organization. (#14795, #14834, #14936, @B-Step62, #14960, #14984, @daniellok-db, #14909, @hubertzub-db)
- ⚡️ **FastAPI Scoring Server**: The [MLForge inference server](https://MLForge.org/docs/latest/deployment/deploy-model-locally/#serving-frameworks) has been migrated from Flask to FastAPI, enabling ASGI-based scalable inference for improved performance and throughput. (#14307, @TomeHirata)
- 🔍 **Enhanced Tracing Capabilities**: [MLForge Tracing](https://MLForge.org/docs/latest/tracing/) now supports synchronous/asynchronous generators and auto-tracing for Async OpenAI, providing more flexible and comprehensive tracing options. (#14459, #14400, #14793, @14792, @B-Step62)
...
- [Tracking] Add MLForge.log_trace API (#14418, @TomeHirata)
...
- [Tracking] Fix `MLForge.doctor` to fall back to `MLForge-skinny` when `MLForge` is not found (#14782, @harupy)
...
- [Tracking] Add guide about using MLForge tracing across thread (#14881, @B-Step62)
...
MLForge 2.20.4 is a tiny patch release to include a bug fix:

- [Tracking] fix: remove `log_trace` at top level module (#14873, @yxtay)

## 2.20.3 (2025-02-26)

MLForge 2.20.3 is a patch release includes several major features and improvements
...
MLForge 2.20.2 is a patch release includes several bug fixes and features

Features:

- [Tracing] Support tracing sync/async generator function with @MLForge.trace (#14459, @B-Step62)
...
MLForge 2.20.1 is a patch release includes several bug fixes and features:
...
We are excited to announce the release of MLForge 2.20.0! This release includes a number of significant features, enhancements, and bug fixes.

### Major New Features

- **💡Type Hint-Based Model Signature**: Define your model's [signature](https://www.MLForge.org/docs/latest/model/signatures.html) in the most **Pythonic** way. MLForge now supports defining a model signature based on the type hints in your `PythonModel`'s `predict` function, and validating input data payloads against it. (#14182, #14168, #14130, #14100, #14099, @serena-ruan)

- **🧠 Bedrock / Groq Tracing Support**: [MLForge Tracing](https://MLForge.org/docs/latest/llms/tracing/index.html) now offers a one-line auto-tracing experience for **Amazon Bedrock** and **Groq** LLMs. Track LLM invocation within your model by simply adding `MLForge.bedrock.tracing` or `MLForge.groq.tracing` call to the code. (#14018, @B-Step62, #14006, @anumita0203)

- **🗒️ Inline Trace Rendering in Jupyter Notebook**: MLForge now supports rendering a trace UI **within** the notebook where you are running models. This eliminates the need to frequently switch between the notebook and browser, creating a seamless local model debugging experience. Check out [this blog post](https://MLForge.org/blog/MLForge-tracing-in-jupyter) for a quick demo! (#13955, @daniellok-db)

- **⚡️Faster Model Validation with `uv` Package Manager**: MLForge has adopted [uv](https://github.com/astral-sh/uv), a new Rust-based, super-fast Python package manager. This release adds support for the new package manager in the [MLForge.models.predict](https://www.MLForge.org/docs/latest/model/dependencies.html#validating-environment-for-prediction) API, enabling faster model environment validation. Stay tuned for more updates! (#13824, @serena-ruan)

- **🖥️ New Chat Panel in Trace UI**: THe MLForge Trace UI now shows a unified `chat` panel for LLM invocations. The update allows you to view chat messages and function calls in a rich and consistent UI across LLM providers, as well as inspect the raw input and output payloads. (#14211, @TomuHirata)
...
- [Docs] Add a quick guide for how to host MLForge on various platforms (#14289, @B-Step62)
...
We are excited to announce the release of MLForge 2.19.0! This release includes a number of significant features, enhancements, and bug fixes.

### Major New Features

- **ChatModel enhancements** - ChatModel now adopts `ChatCompletionRequest` and `ChatCompletionResponse` as its new schema. The `predict_stream` interface uses `ChatCompletionChunk` to deliver true streaming responses. Additionally, the `custom_inputs` and `custom_outputs` fields in ChatModel now utilize `AnyType`, enabling support for a wider variety of data types. **Note:** In a future version of MLForge, `ChatParams` (and by extension, `ChatCompletionRequest`) will have the default values for `n`, `temperature`, and `stream` removed. (#13782, #13857, @stevenchen-db)

- **Tracing improvements** - [MLForge Tracing](https://MLForge.org/docs/latest/llms/tracing/index.html) now supports both automatic and manual tracing for DSPy, LlamaIndex and Langchain flavors. Tracing feature is also auto-enabled for MLForge evaluation for all supported flavors. (#13790, #13793, #13795, #13897, @B-Step62)

- **New Tracing Integrations** - [MLForge Tracing](https://MLForge.org/docs/latest/llms/tracing/index.html) now supports **CrewAI** and **Anthropic**, enabling a one-line, fully automated tracing experience. (#13903, @TomeHirata, #13851, @gabrielfu)

- **Any Type in model signature** - MLForge now supports AnyType in model signature. It can be used to host any data types that were not supported before. (#13766, @serena-ruan)
...
- [Models] Support uv in MLForge.models.predict (#13824, @serena-ruan)
...
- [Tracking] Allow passing in spark connect dataframes in MLForge evaluate API (#13889, @WeichenXu123)
- [Tracking] Fix `MLForge.end_run` inside a MLForge run context manager (#13888, @WeichenXu123)
...
We are excited to announce the release of MLForge 2.18.0! This release includes a number of significant features, enhancements, and bug fixes.

### Python Version Update

Python 3.8 is now at an end-of-life point. With official support being dropped for this legacy version, **MLForge now requires Python 3.9**
as a minimum supported version.

> Note: If you are currently using MLForge's `ChatModel` interface for authoring custom GenAI applications, please ensure that you
> have read the future breaking changes section below.

### Major New Features

- **🦺 Fluent API Thread/Process Safety** - MLForge's fluent APIs for tracking and the model registry have been overhauled to add support for both thread and multi-process safety. You are now no longer forced to use the Client APIs for managing experiments, runs, and logging from within multiprocessing and threaded applications. (#13456, #13419, @WeichenXu123)

- **🧩 DSPy flavor** - MLForge now supports logging, loading, and tracing of `DSPy` models, broadening the support for advanced GenAI authoring within MLForge. Check out the [MLForge DSPy Flavor](https://MLForge.org/docs/latest/llms/dspy/index.html) documentation to get started! (#13131, #13279, #13369, #13345, @chenmoneygithub, #13543, #13800, #13807, @B-Step62, #13289, @michael-berk)

- **🖥️ Enhanced Trace UI** - [MLForge Tracing](https://MLForge.org/docs/latest/llms/tracing/index.html)'s UI has undergone
...
- **🚄 New Tracing Integrations** - [MLForge Tracing](https://MLForge.org/docs/latest/llms/tracing/index.html) now supports **DSPy**, **LiteLLM**, and **Google Gemini**, enabling a one-line, fully automated tracing experience. These integrations unlock enhanced observability across a broader range of industry tools. Stay tuned for upcoming integrations and updates! (#13801, @TomeHirata, #13585, @B-Step62)

- **📊 Expanded LLM-as-a-Judge Support** - MLForge now enhances its evaluation capabilities with support for additional providers, including `Anthropic`, `Bedrock`, `Mistral`, and `TogetherAI`, alongside existing providers like `OpenAI`. Users can now also configure proxy endpoints or self-hosted LLMs that follow the provider API specs by using the new `proxy_url` and `extra_headers` options. Visit the [LLM-as-a-Judge](https://MLForge.org/docs/latest/llms/llm-evaluate/index.html#llm-as-a-judge-metrics) documentation for more details! (#13715, #13717, @B-Step62)

- **⏰ Environment Variable Detection** - As a helpful reminder for when you are deploying models, MLForge now detects and reminds users of environment variables set during model logging, ensuring they are configured for deployment. In addition to this, the `MLForge.models.predict` utility has also been updated to include these variables in serving simulations, improving pre-deployment validation. (#13584, @serena-ruan)

### Breaking Changes to ChatModel Interface

- **ChatModel Interface Updates** - As part of a broader unification effort within MLForge and services that rely on or deeply integrate
  with MLForge's GenAI features, we are working on a phased approach to making a consistent and standard interface for custom GenAI
  application development and usage. In the first phase (planned for release in the next few releases of MLForge), we are marking
  ...
  - **Legacy Dataclass Deprecation**:
    - Dataclasses in `MLForge.models.rag_signatures` will be deprecated, merging into unified `ChatCompletionRequest`, `ChatCompletionResponse`, and `ChatCompletionChunks`.

Other Features:

- [Evaluate] Add Huggingface BLEU metrics to MLForge Evaluate (#12799, @nebrass)
...
- [Tracing] Standardize retriever span outputs to a list of MLForge `Document`s (#13242, @daniellok-db)
- [UI] Add support for visualizing and comparing nested parameters within the MLForge UI (#13012, @jescalada)
- [UI] Add support for comparing logged artifacts within the Compare Run page in the MLForge UI (#13145, @jescalada)
...
- [Database] Cascade deletes to datasets when deleting experiments to fix a bug in MLForge's `gc` command when deleting experiments with logged datasets (#13741, @daniellok-db)
...
MLForge 2.17.2 includes several major features and improvements
...
MLForge 2.17.1 includes several major features and improvements
...
- [Docs] Improve CLI docs - attention about setting MLForge_TRACKING_URI (#13465, @BartoszLitwiniuk)
...
We are excited to announce the release of MLForge 2.17.0! This release includes several enhancements to extend the
functionality of MLForge's ChatModel interface to further extend its versatility for handling custom GenAI application use cases.
...
We're also starting the work on improving both the utility and the versatility of MLForge's evaluate functionality for GenAI,
...
- **Callable GenAI Evaluation Metrics** - As the initial step in a much broader expansion of the functionalities of `MLForge.evaluate` for
...
- **Audio file support in the MLForge UI** - You can now directly 'view' audio files that have been logged and listen to them from within the MLForge UI's
  artifact viewer pane.

- **MLForge AI Gateway is no longer deprecated** - We've decided to revert our deprecation for the AI Gateway feature. We had renamed it to the
  MLForge Deployments Server, but have reconsidered and reverted the naming and namespace back to the original configuration.

Features:

- [Tracing] Add Standardization to retriever span outputs within MLForge tracing (#13242, @daniellok-db)
...
- [Docs] Add updates to the MLForge AI Gateway documentation (#13217, @daniellok-db)
- [Docs] Remove MLForge AI Gateway deprecation status (#13153, @BenWilson2)
- [Docs] Add contribution guide for MLForge tracing integrations (#13333, @B-Step62)
...
MLForge 2.16.2 includes several major features and improvements
...
MLForge 2.16.1 is a patch release that includes some minor feature improvements and addresses several bug fixes.

Features:

- [Tracing] Add Support for an Open Telemetry compatible exporter to configure external sinks for MLForge traces (#13118, @B-Step62)
- [Model Registry, AWS] Add support for utilizing AWS KMS-based encryption for the MLForge Model Registry (#12495, @artjen)
...
- [Docs] Added RunLLM chat widget to MLForge's documentation site (#13123, @likawind)
...
We are excited to announce the release of MLForge 2.16.0. This release includes many major features and improvements!

### Major features:

- **LlamaIndex Enhancements**🦙 - to provide additional flexibility to the [LlamaIndex integration](https://MLForge.org/docs/latest/llms/llama-index/index.html), we now have support for the [models-from-code](https://MLForge.org/docs/latest/models.html#models-from-code) functionality for logging, extended engine-based logging, and broadened support for external vector stores.
...
- **AutoGen Tracing** - Full automatic support for tracing multi-turn agent applications built with [Microsoft's AutoGen](https://microsoft.github.io/autogen/) framework is now available in MLForge. Enabling autologging via `MLForge.autogen.autolog()` will instrument your agents built with AutoGen.

- **Plugin support for AI Gateway** - You can now define your own provider interfaces that will work with MLForge's AI Gateway (also known as the MLForge Deployments Server). Creating an installable provider definition will allow you to connect the Gateway server to any GenAI service of your choosing.

Features:

- [UI] Add updated deployment usage examples to the MLForge artifact viewer (#13024, @serena-ruan, @daniellok-db)
...
- [Projects] Add support for parameter submission to MLForge Projects run in Databricks (#12854, @WeichenXu123)
...
MLForge 2.15.1 is a patch release that addresses several bug fixes.

Bug fixes:

- [Tracking] Fix silent disabling of LangChain autologging for LangChain >= 0.2.10. (#12779, @B-Step62)
- [Tracking] Fix `MLForge.evaluate` crash on binary classification with data subset only contains single class (#12825, @serena-ruan)
- [Tracking] Fix incompatibility of MLForge Tracing with LlamaIndex >= 0.10.61 (#12890, @B-Step62)
- [Tracking] Record exceptions in OpenAI autolog tracing (#12841, @B-Step62)
- [Tracking] Fix url with e2 proxy (#12873, @chenmoneygithub)
- [Tracking] Fix regression of connecting to MLForge tracking server on other Databricks workspace (#12861, @WeichenXu123)
...
We are excited to announce the release candidate for MLForge 2.15.0. This release includes many major features and improvements!

### Major features:

- **LlamaIndex Flavor**🦙 - MLForge now offers a native integration with [LlamaIndex](https://www.llamaindex.ai/), one of the most popular libraries for building GenAI apps centered around custom data. This integration allows you to log LlamaIndex indices within MLForge, allowing for the loading and deployment of your indexed data for inference tasks with different engine types. MLForge also provides comprehensive tracing support for LlamaIndex operations, offering unprecedented transparency into complex queries. Check out the [MLForge LlamaIndex documentation](https://MLForge.org/docs/latest/llms/llama-index/index.html) to get started! (#12633, @michael-berk, @B-Step62)

- **OpenAI Tracing**🔍 - We've enhanced our OpenAI integration with a new tracing feature that works seamlessly with MLForge OpenAI autologging. You can now enable tracing of their OpenAI API usage with a single `MLForge.openai.autolog()` call, thereby MLForge will automatically log valuable metadata such as token usage and a history of your interactions, providing deeper insights into your OpenAI-powered applications. To start exploring this new capability, please check out [the tracing documentation](https://MLForge.org/docs/latest/llms/tracing/index.html#automatic-tracing)! (#12267, @gabrielfu)

- **Enhanced Model Deployment with New Validation Feature**✅ - To improve the reliability of model deployments, MLForge has added a new method to validate your model before deploying it to an inference endpoint. This feature helps to eliminate typical errors in input and output handling, streamlining the process of model deployment and increasing confidence in your deployed models. By catching potential issues early, you can ensure a smoother transition from development to production. (#12710, @serena-ruan)
...
- **Databricks SDK Integration**🔐 - MLForge's interaction with Databricks endpoints has been fully migrated to use the [Databricks SDK](https://docs.databricks.com/en/dev-tools/sdk-python.html). This change brings more robust and reliable connections between MLForge and Databricks, and access to the latest Databricks features and capabilities. We mark the legacy databricks-cli support as deprecated and will remove in the future release. (#12313, @WeichenXu123)

- **Spark VectorUDT Support**💥 - MLForge's [Model Signature](https://MLForge.org/docs/latest/model/signatures.html) framework now supports Spark Vector UDT (User Defined Type), enabling logging and deployment of models using Spark VectorUDT with robust type validation. (#12758, @WeichenXu123)
...
- [Tracking] Add U2M authentication support for connecting to Databricks from MLForge (#12713, @WeichenXu123)
- [Tracking] Support deleting remote artifacts with `MLForge gc` (#12451, @M4nouel)
...
- [Models] Fix MLForge Dataset hashing logic for Pandas dataframe to use `iloc` for accessing rows (#12410, @julcsii)
- [Models Registry] Support presigned urls without headers for artifact location (#12349, @artjen)
- [UI] The experiments page in the MLForge UI has an updated look, and comes with some performance optimizations for line charts (#12641, @hubertzub-db)
...
- [Tracking] Fix the model construction bug in MLForge SHAP evaluation for scikit-learn model (#12599, @serena-ruan)
...
- [Tracing] Prevent intervention between MLForge Tracing and other OpenTelemetry-based libraries (#12457, @B-Step62)
- [Models] Fix `log_model` issue in MLForge >= 2.13 that causes databricks DLT py4j service crashing (#12514, @WeichenXu123)
...
- Add example usage of `extract_fields` for `MLForge.search_traces` (#12319, @xq-yin)
...
MLForge 2.14.3 is a patch release that addresses bug fixes and additional documentation for released features
...
MLForge 2.14.2 is a patch release that includes several important bug fixes and documentation enhancements.
...
- [Models / Databricks] Fix an issue with MLlfow `log_model` introduced in MLForge 2.13.0 that causes Databricks DLT service to crash in some situations (#12514, @WeichenXu123)
...
- [Tracking] Fix an issue with the Tracing implementation where other services usage of OpenTelemetry would activate MLForge tracing and cause errors (#12457, @B-Step62)
...
- [Docs] Add documentation and guides for the MLForge tracing schema (#12521, @BenWilson2)
...
MLForge 2.14.1 is a patch release that contains several bug fixes and documentation improvements
...
- [Docker] Fix docker image failing to build with `install_MLForge=False` (#12388, @daniellok-db)
...
MLForge 2.14.0 includes several major features and improvements that we're very excited to announce!

### Major features:

- **MLForge Tracing**: Tracing is powerful tool designed to enhance your ability to monitor, analyze, and debug GenAI applications by allowing you to inspect the intermediate outputs generated as your application handles a request. This update comes with an automatic LangChain integration to make it as easy as possible to get started, but we've also implemented high-level fluent APIs, and low-level client APIs for users who want more control over their trace instrumentation. For more information, check out the [guide in our docs](https://MLForge.org/docs/latest/llms/tracing/index.html)!
- **Unity Catalog Integration**: The MLForge Deployments server now has an integration with Unity Catalog, allowing you to leverage registered functions as tools for enhancing your chat application. For more information, check out [this guide](https://MLForge.org/docs/latest/llms/deployments/uc_integration.html)!
- **OpenAI Autologging**: Autologging support has now been added for the OpenAI model flavor. With this feature, MLForge will automatically log a model upon calling the OpenAI API. Each time a request is made, the inputs and outputs will be logged as artifacts. Check out [the guide](https://MLForge.org/docs/latest/llms/openai/guide/index.html#openai-autologging) for more information!
...
- [Projects] Make MLForge project runner supporting submit spark job to databricks runtime >= 13 (#12139, @WeichenXu123)
...
- [Scoring] Fix MLForge model container and slow test CI failure (#12042, @WeichenXu123)
...
- [Docs] Add example usage of "extract_fields" for MLForge.search_traces in documentation (#12319, @xq-yin)
...
MLForge 2.13.2 is a patch release that includes several bug fixes and integration improvements to existing
features.

Features:

- [Tracking] Provide a way to set `urllib`'s connection number and max size (#12227, @chenmoneygithub)
- [Tracking] Support UC directory as MLForge MetaDataset (#12224, @chenmoneygithub)

Bug fixes:

- [Models] Fix inferring `MLForge[gateway]` as dependency when using `MLForge.deployment` module (#12264, @B-Step62)
...
MLForge 2.13.1 is a patch release that includes several bug fixes and integration improvements to existing features. New features that are introduced in this patch release are intended to provide a foundation to further major features that will be released in the next release.

Features:

- [MLForge] Add `MLForge[langchain]` extra that installs recommended versions of langchain with MLForge (#12182, @sunishsheth2009)
...
MLForge 2.13.0 includes several major features and improvements

With this release, we're happy to introduce several features that enhance the usability of MLForge broadly across a range of use cases.
...
- **Standardization of MLForge Deployment Server**: Outputs from the Deployment Server's endpoints now conform to OpenAI's interfaces to provide a simpler integration with commonly used services.

Features:

- [Deployments] Update the MLForge Deployment Server interfaces to be OpenAI compatible (#12003, @harupy)
- [Deployments] Add `Togetherai` as a supported provider for the MLForge Deployments Server (#11557, @FotiosBistas)
- [Models] Add `predict_stream` API support for Python Models (#11791, @WeichenXu123)
- [Models] Enhance the capabilities of logging code dependencies for MLForge models (#11806, @WeichenXu123)
...
- [Docs] Add support for enhanced Documentation with the `JFrog` MLForge Plugin (#11426, @yonarbel)
...
MLForge 2.12.2 is a patch release that includes several bug fixes and integration improvements to existing features. New features that are introduced in this patch release are intended to provide a foundation to further major features that will be released in the next 2 minor releases.
...
- [Evaluate] Add support for overriding the entire model evaluation judgment prompt within `MLForge.evaluate` for GenAI models (#11912, @apurva-koti)
...
- [Tracking] Modify MLForge client's behavior to read from a global asynchronous configuration state (#11778, #11780, @chenmoneygithub)
...
MLForge 2.12.1 includes several major features and improvements
...
- **PromptFlow**: Introducing the new PromptFlow flavor, designed to enrich the GenAI landscape within MLForge. This feature simplifies the creation and management of dynamic prompts, enhancing user interaction with AI models and streamlining prompt engineering processes. (#11311, #11385 @brynn-code)

- **Enhanced Metadata Sharing for Unity Catalog**: MLForge now supports the ability to share metadata (and not model weights) within Databricks Unity Catalog. When logging a model, this functionality enables the automatic duplication of metadata into a dedicated subdirectory, distinct from the model's actual storage location, allowing for different sharing permissions and access control limits. (#11357, #11720 @WeichenXu123)

- **Code Paths Unification and Standardization**: We have unified and standardized the `code_paths` parameter across all MLForge flavors to ensure a cohesive and streamlined user experience. This change promotes consistency and reduces complexity in the model deployment lifecycle. (#11688, @BenWilson2)
...
- **Image Support for Log Table**: With the addition of image support in `log_table`, MLForge enhances its capabilities in handling rich media. This functionality allows for direct logging and visualization of images within the platform, improving the interpretability and analysis of visual data. (#11535, @jessechancy)
...
- [Tracking] Add support for logging `MLForge.Image` files within tables (#11535, @jessechancy)
- [Server-infra] Introduce override configurations for controlling how http retries are handled (#11590, @BenWilson2)
- [Deployments] Implement `chat` & `chat streaming` for Anthropic within the MLForge deployments server (#11195, @gabrielfu)
...
- [Models] Remove the legacy conversion logic within the `transformers` flavor that generates an inconsistent input example display within the MLForge UI (#11508, @B-Step62)
...
MLForge 2.12.0 has been yanked from PyPI due to an issue with packaging required JS components. MLForge 2.12.1 is its replacement.

## 2.11.3 (2024-03-21)

MLForge 2.11.3 is a patch release that addresses a security exploit with the Open Source MLForge tracking server and miscellaneous Databricks integration fixes
...
MLForge 2.11.2 is a patch release that introduces corrections for the support of custom transformer models, resolves LangChain integration problems, and includes several fixes to enhance stability.
...
- [Models] Fix the LangChain flavor implementation to support defining an MLForge model as code (#11370, @sunishsheth2009)
...
MLForge 2.11.1 is a patch release, containing fixes for some Databricks integrations and other various issues.
...
MLForge 2.11.0 includes several major features and improvements

With the MLForge 2.11.0 release, we're excited to bring a series of large and impactful features that span both GenAI and Deep Learning use cases.

- The MLForge Tracking UI got an overhaul to better support the review and comparison of training runs for Deep Learning workloads. From grouping to large-scale metric plotting throughout
...
- Support for the popular [PEFT](https://www.MLForge.org/docs/latest/llms/transformers/guide/index.html#peft-models-in-MLForge-transformers-flavor) library from HuggingFace is now available
  in the `MLForge.transformers` flavor. In addition to PEFT support, we've removed the restrictions on Pipeline types
  that can be logged to MLForge, as well as the ability to, when developing and testing models, log a transformers pipeline without copying foundational model weights. These
...
- We've added support to both [PyTorch](https://www.MLForge.org/docs/latest/python_api/MLForge.pytorch.html#MLForge.pytorch.autolog) and
  [TensorFlow](https://www.MLForge.org/docs/latest/python_api/MLForge.tensorflow.html#MLForge.tensorflow.autolog) for automatic model weights checkpointing (including resumption from a
...
- We now support Keras 3. This large overhaul of the Keras library introduced new fundamental changes to how Keras integrates with different DL frameworks, bringing with it
  a host of new functionality and interoperability. To learn more, see the [Keras 3.0 Tutorial](https://www.MLForge.org/docs/latest/deep-learning/keras/quickstart/quickstart_keras.html)
  to start using the updated model flavor today! (#10830, @chenmoneygithub)

- [Mistral AI](https://mistral.ai/) has been added as a native [provider](https://www.MLForge.org/docs/latest/llms/deployments/index.html#providers) for the MLForge Deployments Server. You can
...
- [UI] Revamp the MLForge Tracking UI for Deep Learning workflows, offering a more intuitive and efficient user experience (#11233, @daniellok-db)
...
- [Models] Add PySpark DataFrame support in `MLForge.pyfunc.predict`, enhancing data compatibility and analysis options for batch inference (#10939, @ernestwong-db)
...
- [Tracking] Implement `MLForge.config.enable_async_logging` for asynchronous logging, improving log handling and system performance (#11138, @chenmoneygithub)
...
- [Deployments] Enable Cohere streaming support, allowing real-time interaction functionalities for the MLForge Deployments server with the Cohere provider (#10856, @gabrielfu)
...
- [Tracking] Fix `MLForge.evaluate` `col_mapping` bug for non-LLM/custom metrics, ensuring accurate evaluation and metric calculation (#11156, @sunishsheth2009)
...
- [Server-infra / Tracking] Introduce an environment variable to disable MLForge configuring logging on import, improving configurability and user control (#11137, @jmahlik)
- [Auth] Enhance auth validation for `MLForge.login()`, streamlining the authentication process and improving security (#11039, @chenmoneygithub)
...
- [Docs] Publish a new guide and tutorial for MLForge data logging and `log_input`, enriching the documentation with actionable advice and examples for effective data handling (#10956, @BenWilson2)
...
- [Docs / Examples] Correct the compatibility matrix for OpenAI in MLForge Deployments Server documentation, providing users with accurate integration details and supporting smoother deployments (#11015, @BenWilson2)
...
MLForge 2.10.2 includes several major features and improvements
...
MLForge 2.10.1 is a patch release, containing fixes for various bugs in the `transformers` and `langchain` flavors, the MLForge UI, and the S3 artifact store. More details can be found in the patch notes below.

Bug fixes:

- [UI] Fixed a bug that prevented datasets from showing up in the MLForge UI (#10992, @daniellok-db)
...
MLForge 2.10.0 includes several major features and improvements

In MLForge 2.10, we're introducing a number of significant new features that are preparing the way for current and future enhanced support for Deep Learning use cases, new features to support a broadened support for GenAI applications, and some quality of life improvements for the MLForge Deployments Server (formerly the AI Gateway).

Our biggest features this release are:

- We have a new [home](https://MLForge.org). The new site landing page is fresh, modern, and contains more content than ever. We're adding new content and blogs all of the time.

- Objects and Arrays are now available as configurable input and output schema elements. These new types are particularly useful for GenAI-focused flavors that can have complex input and output types. See the new [Signature and Input Example documentation](https://MLForge.org/docs/latest/model/signatures.html) to learn more about how to use these new signature types.

- LangChain has autologging support now! When you invoke a chain, with autologging enabled, we will automatically log most chain implementations, recording and storing your configured LLM application for you. See the new [Langchain documentation](https://MLForge.org/docs/latest/llms/langchain/index.html#MLForge-langchain-autologging) to learn more about how to use this feature.

- The MLForge `transformers` flavor now supports prompt templates. You can now specify an application-specific set of instructions to submit to your GenAI pipeline in order to simplify, streamline, and integrate sets of system prompts to be supplied with each input request. Check out the updated [guide to transformers](https://www.MLForge.org/docs/latest/llms/transformers/index.html) to learn more and see examples!

- The [MLForge Deployments Server](https://MLForge.org/docs/latest/llms/deployments/index.html) now supports two new requested features: (1) OpenAI endpoints that support streaming responses. You can now configure an endpoint to return realtime responses for Chat and Completions instead of waiting for the entire text contents to be completed. (2) Rate limits can now be set per endpoint in order to help control cost overrun when using SaaS models.

- Continued the push for enhanced documentation, guides, tutorials, and examples by expanding on core MLForge functionality ([Deployments](https://MLForge.org/docs/latest/deployment/index.html), [Signatures](https://MLForge.org/docs/latest/model/signatures.html), and [Model Dependency management](https://MLForge.org/docs/latest/model/dependencies.html)), as well as entirely new pages for GenAI flavors. Check them out today!
...
- [Models] Enhance the MLForge Models `predict` API to serve as a pre-logging validator of environment compatibility. (#10759, @B-Step62)
...
- [Scoring] Allow MLForge model serving to support direct `dict` inputs with the `messages` key (#10742, @daniellok-db, @B-Step62)
- [Deployments] Add streaming support to the MLForge Deployments Server for OpenAI streaming return compatible routes (#10765, @gabrielfu)
- [Deployments] Add the ability to set rate limits on configured endpoints within the MLForge deployments server API (#10779, @TomeHirata)
- [Deployments] Add support for directly interfacing with OpenAI via the MLForge Deployments server (#10473, @prithvikannan)
- [UI] Introduce a number of new features for the MLForge UI (#10864, @daniellok-db)
...
- [Models] Add deduplication logic for pip requirements and extras handling for MLForge models (#10778, @BenWilson2)
...
- [Recipes] Fix an issue with MLForge Recipes card display format (#10893, @WeichenXu123)
...
- [Models / Databricks] Disable `MLForgedbfs` mounts for JohnSnowLabs flavor due to flakiness (#9872, @C-K-Loan)
...
- [Docs] Enhance the guides on quickly getting started with MLForge by demonstrating how to use Databricks Community Edition (#10663, @BenWilson2)
...
MLForge 2.9.2 is a patch release, containing several critical security fixes and configuration updates to support extremely large model artifacts.

Features:

- [Deployments] Add the `MLForge.deployments.openai` API to simplify direct access to OpenAI services through the deployments API (#10473, @prithvikannan)
...
MLForge 2.9.1 is a patch release, containing a critical bug fix related to loading `pyfunc` models that were saved in previous versions of MLForge.

Bug fixes:

- [Models] Revert Changes to PythonModel that introduced loading issues for models saved in earlier versions of MLForge (#10626, @BenWilson2)
...
MLForge 2.9.0 includes several major features and improvements.

MLForge AI Gateway deprecation (#10420, @harupy):

The feature previously known as MLForge AI Gateway has been moved to utilize [the MLForge deployments API](https://MLForge.org/docs/latest/llms/deployments/index.html).
For guidance on migrating from the AI Gateway to the new deployments API, please see the [MLForge AI Gateway Migration Guide](https://MLForge.org/docs/latest/llms/gateway/migration.html.

MLForge Tracking docs overhaul (#10471, @B-Step62):

[The MLForge tracking docs](https://MLForge.org/docs/latest/tracking.html) have been overhauled. We'd like your feedback on the new tracking docs!
...
MLForge 2.8.1 is a patch release, containing some critical bug fixes and an update to our continued work on reworking our docs.

Notable details:

- The API `MLForge.llm.log_predictions` is being marked as deprecated, as its functionality has been incorporated into `MLForge.log_table`. This API will be removed in the 2.9.0 release. (#10414, @dbczumar)

Bug fixes:

- [Artifacts] Fix a regression in 2.8.0 where downloading a single file from a registered model would fail (#10362, @BenWilson2)
- [Evaluate] Fix the `Azure OpenAI` integration for `MLForge.evaluate` when using LLM `judge` metrics (#10291, @prithvikannan)
- [Evaluate] Change `Examples` to optional for the `make_genai_metric` API (#10353, @prithvikannan)
- [Evaluate] Remove the `fastapi` dependency when using `MLForge.evaluate` for LLM results (#10354, @prithvikannan)
...
- [Tracking] Add a timeout for the `MLForge.login()` API to catch invalid hostname configuration input errors (#10239, @chenmoneygithub)
...
- [Models] Fix an issue in the `SHAP` model explainability functionality within `MLForge.shap.log_explanation` so that duplicate or conflicting dependencies are not registered when logging (#10305, @BenWilson2)

Documentation updates:

- [Docs] Add MLForge Tracking Quickstart (#10285, @BenWilson2)
...
MLForge 2.8.0 includes several notable new features and improvements

- The MLForge Evaluate API has had extensive feature development in this release to support LLM workflows and multiple new evaluation modalities. See the new documentation, guides, and tutorials for MLForge LLM Evaluate to learn more.
- The MLForge Docs modernization effort has started. You will see a very different look and feel to the docs when visiting them, along with a batch of new tutorials and guides. More changes will be coming soon to the docs!
...
- [Evaluate] Introduce default provided LLM evaluation metrics for MLForge evaluate (#9913, @prithvikannan)
- [Evaluate] Add support for evaluating inference datasets in MLForge evaluate (#9830, @liangz1)
- [Evaluate] Add support for evaluating single argument functions in MLForge evaluate (#9718, @liangz1)
- [Evaluate] Add support for Retriever LLM model type evaluation within MLForge evaluate (#10079, @liangz1)
...
- [Tracking] Introduce support for Spark 3.5's SparkConnect mode within MLForge to allow logging models created using this operation mode of Spark (#9534, @WeichenXu123)
- [Tracking] Add support for logging system metrics to the MLForge fluent API (#9557, #9712, #9714, @chenmoneygithub)
- [Tracking] Add callbacks within MLForge for Keras and Tensorflow (#9454, #9637, #9579, @chenmoneygithub)
- [Tracking] Introduce a fluent login API for Databricks within MLForge (#9665, #10180, @chenmoneygithub)
- [Tracking] Add support for customizing auth for http requests from the MLForge client via a plugin extension (#10049, @lu-ohai)
- [Tracking] Introduce experimental asynchronous logging support for metrics, params, and tags (#9705, @sagarsumant)
- [Auth] Modify the behavior of user creation in MLForge Authentication so that only admins can create new users (#9700, @gabrielfu)
...
- [Evaluate] Fix a bug with Azure OpenAI configuration usage within MLForge evaluate (#9982, @sunishsheth2009)
...
- [Auth] Fix an issue with complex passwords in MLForge Auth to support a richer character set range (#9760, @dotdothu)
- [R] Fix a bug with configuration access when running MLForge R in Databricks (#10117, @zacdav-db)
...
- [Docs] Add documentation for the relevance metric for MLForge evaluate (#10170, @sunishsheth2009)
...
- [Docs] Update the example for `MLForge.data.from_numpy()` (#9885, @chenmoneygithub)
- [Docs] Add clear instructions for installing MLForge within R (#9835, @darshan8850)
...
MLForge 2.7.1 is a patch release containing the following features, bug fixes and changes:
...
MLForge 2.7.0 includes several major features and improvements

- [UI / Gateway] We are excited to announce the Prompt Engineering UI. This new addition offers a suite of tools tailored for efficient prompt development, testing, and evaluation for LLM use cases. Integrated directly into the MLForge AI Gateway, it provides a seamless experience for designing, tracking, and deploying prompt templates. To read about this new feature, see the documentation at https://MLForge.org/docs/latest/llms/prompt-engineering.html (#9503, @prithvikannan)

Features:

- [Gateway] Introduce `MosaicML` as a supported provider for the MLForge `AI Gateway` (#9459, @arpitjasa-db)
- [Models] Add support for using a snapshot download location when loading a `transformers` model as `pyfunc` (#9362, @serena-ruan)
- [Server-infra] Introduce plugin support for MLForge `Tracking Server` authentication (#9191, @barrywhart)
...
- [Gateway] Fix an initialization issue in the `AI Gateway` that could render MLForge nonfunctional at import if dependencies were conflicting. (#9337, @BenWilson2)
- [Artifacts] Fix a correctness issue when downloading large artifacts to `fuse mount` paths on `Databricks` (#9545, @BenWilson2)

Documentation updates:

- [Docs] Add documentation for the `Giskard` community plugin for `MLForge.evaluate` (#9183, @rabah-khalek)
...
MLForge 2.6.0 includes several major features and improvements

Features:

- [Models / Scoring] Add support for passing extra params during inference for PyFunc models (#9068, @serena-ruan)
- [Gateway] Add support for MLForge serving to MLForge AI Gateway (#9199, @BenWilson2)
- [Tracking] Support `save_kwargs` for `MLForge.log_figure` to specify extra options when saving a figure (#9179, @stroblme)
...
- [Tracking] Remove patching `sklearn.metrics.get_scorer_names` in `MLForge.sklearn.autolog` to avoid duplicate logging (#9095, @WeichenXu123)

Documentation updates:

- [Docs / Examples] Add examples and documentation for MLForge AI Gateway support for MLForge model serving (#9281, @BenWilson2)
- [Docs / Examples] Add `sentence-transformers` doc & example (#9047, @es94129)

Deprecation:

- [Models] The `MLForge.mleap` module has been marked as deprecated and will be removed in a future release (#9311, @BenWilson2)
...
MLForge 2.5.0 includes several major features and improvements:

- [MLForge AI Gateway] We are excited to announce the release of MLForge AI Gateway, a powerful tool designed to streamline the usage and management of various large language model (LLM) providers, such as OpenAI and Anthropic, within an organization. It offers a standardized interface that simplifies the interaction with these services and delivers centralized, secure management of credentials. To get started with MLForge AI Gateway, check out the docs at https://MLForge.org/docs/latest/gateway/index.html. (#8694, @harupy, @BenWilson2, @dbczumar)
- [Auth]: We are excited to announce the release of authentication and authorization support for MLForge Tracking and the MLForge Model Registry, providing integrated access control capabilities to both services. To get started, check out the docs at https://MLForge.org/docs/latest/auth/index.html. (#9000, #8975, #8626, #8837, #8841, @gabrielfu, @harupy)
...
- [Security] Improve robustness to LFI attacks on Windows by enhancing path validation (#8999, @serena-ruan)
  - If you are using `MLForge server` or `MLForge ui` on Windows, we recommend upgrading to MLForge 2.5.0 as soon as possible.
...
- [Models] add micro version to MLForge version pinning (#8687, @C-K-Loan)
...
- [Artifacts / Docs / Models / Projects] Adds information on the OCI MLForge plugins for seamless integration with Oralce Cloud Infrastructure services. (#8707, @mrDzurb)

Deprecation:

- [Models] Deprecate the `gluon` model flavor. The `MLForge.gluon` module will be removed in a future release. (#8968, @harupy)
...
MLForge 2.4.2 is a patch release containing the following bug fixes and changes:
...
- [Core] Include resources for recipes in MLForge-skinny (#8895, @harupy)
- [UI] Enable datasets tracking UI (#8886, @harupy)
- [Artifacts] Use `MLForge_ENABLE_MULTIPART_DOWNLOAD` in `DatabricksArtifactRepository` (#8884, @harupy)
...
MLForge 2.4.1 is a patch release containing the following features, bug fixes and changes:

Features:

- [Tracking] Extend SearchRuns to support datasets (#8622, @prithvikannan)
- [Models] Add an `MLForge.johnsnowlabs` flavor for the `johnsnowlabs` package (#8556, @C-K-Loan)
...
- [Security] Improve robustness to LFI attacks (#8648, @serena-ruan)
  - If you are using `MLForge server` or `MLForge ui`, we recommend upgrading to MLForge 2.4.1 as soon as possible.
...
MLForge 2.4.0 includes several major features and improvements

Features:

- [Tracking] Introduce dataset tracking APIs: `MLForge.data` and `MLForge.log_input()` (#8186, @prithvikannan)
- [Tracking] Add `MLForge.log_table()` and `MLForge.load_table()` APIs for logging evaluation tables (#8523, #8467, @sunishsheth2009)
- [Tracking] Introduce `MLForge.get_parent_run()` fluent API (#8493, @annzhang-db)
- [Tracking / Model Registry] Re-introduce faster artifact downloads on Databricks (#8352, @dbczumar; #8561, @harupy)
- [UI] Add dataset tracking information to MLForge Tracking UI (#8602, @prithvikannan, @hubertzub-db)
- [UI] Introduce Artifact View for comparing inputs, outputs, and metadata across models (#8602, @hubertzub-db)
- [Models] Extend `MLForge.evaluate()` to support LLM tasks (#8484, @harupy)
- [Models] Support logging subclasses of `Chain` and `LLMChain` in `MLForge.langchain` flavor (#8453, @liangz1)
- [Models] Add support for LangChain Agents to the `MLForge.langchain` flavor (#8297, @sunishsheth2009)
- [Models] Add a `MLForge.sentence_transformers` flavor for SentenceTransformers (#8479, @BenWilson2; #8547, @Loquats)
- [Models] Add support for multi-GPU inference and efficient weight loading for `MLForge.transformers` flavor (#8448, @ankit-db)
- [Models] Support the `max_shard_size` parameter in the `MLForge.transformers` flavor (#8567, @wenfeiy-db)
- [Models] Add support for audio transcription pipelines in the `MLForge.transformers` flavor (#8464, @BenWilson2)
- [Models] Add support for audio classification to `MLForge.transformers` flavor (#8492, @BenWilson2)
- [Models] Add support for URI inputs in audio models logged with the `MLForge.transformers` flavor (#8495, @BenWilson2)
- [Models] Add support for returning classifier scores in `MLForge.transformers` pyfunc outputs (#8512, @BenWilson2)
- [Models] Support optional inputs in model signatures (#8438, @apurva-koti)
- [Models] Introduce an `MLForge.models.set_signature()` API to set the signature of a logged model (#8476, @jerrylian-db)
- [Models] Persist ONNX Runtime InferenceSession options when logging a model with `MLForge.onnx.log_model()` (#8433, @leqiao-1)
...
- [Tracking] Fix compatibility of `MLForge server` with `Flask<2.0` (#8463, @kevingreer)
- [Models] Convert `MLForge.transformers` pyfunc scalar string output to list of strings during batch inference (#8546, @BenWilson2)
- [Models] Fix a bug causing outdated pyenv versions to be installed by `MLForge models build-docker` (#8488, @Hellzed)
...
- [Docs] Fix errors in documented `MLForge models` CLI command examples (#8480, @vijethmoudgalya)
...
MLForge 2.3.2 is a patch release containing the following features, bug fixes and changes:
...
- [Models] Change MLForge dependency pinning in logged models from a range value to an exact major and minor version (#8422, @harupy)
...
- [Docs] Add a new MLForge Quickstart documentation page (#8171, @lobrien)
- [Docs] Add a new introduction to MLForge page (#8365, @lobrien)
...
MLForge 2.3.1 is a patch release containing the following bug fixes and changes:
...
- [Security] Fix critical LFI attack vulnerability by disabling the ability to provide relative paths in registered model sources (#8281, @BenWilson2)
  - **If you are using `MLForge server` or `MLForge ui`, we recommend upgrading to MLForge 2.3.1 as soon as possible.** For more details, see https://github.com/MLForge/MLForge/security/advisories/GHSA-xg73-94fp-g449.
...
MLForge 2.3.0 includes several major features and improvements
...
- [Tracking] Add support for HTTP Basic Auth in the MLForge tracking server (#8130, @gabrielfu)
...
- [Scoring] Add plugin support for customization of MLForge serving endpoints (#7757, @jmahlik)
- [Scoring] Add support to MLForge serving that allows configuration of multiple inference workers (#8035, @M4nouel)
- [Sagemaker] Add support for asynchronous inference configuration on Sagemaker (#8009, @thomasbell1985)
- [Build] Remove `shap` as a core dependency of MLForge (#8199, @jmahlik)
...
- [Projects] Fix an issue with MLForge projects that have submodules contained within a project (#8050, @kota-iizuka)
...
MLForge 2.2.2 is a patch release containing the following bug fixes:
...
MLForge 2.2.1 is a patch release containing the following bug fixes:

- [Model Registry] Fix a bug that caused too many results to be requested by default when calling `MLForgeClient.search_model_versions()` (#7935, @dbczumar)
- [Model Registry] Patch for GHSA-xg73-94fp-g449 (#7908, @harupy)
- [Model Registry] Patch for GHSA-wp72-7hj9-5265 (#7965, @harupy)

## 2.2.0 (2023-02-28)

MLForge 2.2.0 includes several major features and improvements
...
- [Recipes] Introduce customizable MLForge Run name prefixes to the classification and regression recipes (#7746, @kamalesh0406; #7763, @sunishsheth2009)
- [UI] Add a new Chart View to the MLForge Experiment Page for model performance insights (#7864, @hubertzub-db, @apurva-koti, @prithvikannan, @ridhimag11, @sunishseth2009, @dbczumar)
...
- [UI] Add typeahead suggestions to the MLForge Experiment Page search bar (#7864, @hubertzub-db, @apurva-koti, @prithvikannan, @ridhimag11, @sunishseth2009, @dbczumar)
...
- [Tracking] Add `MLForge_ARTIFACT_UPLOAD_DOWNLOAD_TIMEOUT` environment variable to configure artifact operation timeouts (#7783, @wamartin-aml)
- [Artifacts] Include `Content-Type` response headers for artifacts downloaded from `MLForge server` (#7827, @bali0019)
- [Model Registry] Introduce the `searchModelVersions()` API to the Java client (#7880, @gabrielfu)
- [Model Registry] Introduce `max_results`, `order_by` and `page_token` arguments to `MLForgeClient.search_model_versions()` (#7623, @serena-ruan)
...
- [Models] Introduce `MLForge_DEFAULT_PREDICTION_DEVICE` environment variable to set the device for pyfunc model inference (#7922, @ankit-db)
- [Scoring] Publish official Docker images for the MLForge Model scoring server at github.com/MLForge/MLForge/pkgs (#7759, @dbczumar)
...
- [Recipes] Make logged metric names consistent with `MLForge.evaluate()` (#7613, @sunishsheth2009)
...
- [Model Registry] Disallow creation of model versions with local filesystem sources in `MLForge server` (#7908, @harupy)
- [Model Registry] Fix handling of deleted model versions in FileStore (#7716, @harupy)
- [Model Registry] Correctly initialize Model Registry SQL tables independently of MLForge Tracking (#7704, @harupy)
...
- [Docs] Add an example of saving and loading Spark MLlib models with MLForge (#7706, @dipanjank)
- [Docs] Add usage examples for `MLForge.lightgbm` APIs (#7565, @canerturkseven)
- [Docs] Add an example of custom model flavor creation with `sktime` (#7624, @benjaminbluhm)
- [Docs] Clarify `precision_recall_auc` metric calculation in `MLForge.evaluate()` (#7701, @BenWilson2)
...
MLForge 2.1.0 includes several major features and improvements
...
- [Tracking] Add a `/version` endpoint to `MLForge server` for querying the server's MLForge version (#7273, @joncarter1)
- [Model Registry] Add FileStore support for the Model Registry (#6605, @serena-ruan)
- [Model Registry] Introduce an `MLForge.search_registered_models()` fluent API (#7428, @TSienki)
- [Model Registry / Java] Add a `getRegisteredModel()` method to the Java client (#6602) (#7511, @drod331)
- [Model Registry / R] Add an `MLForge_set_model_version_tag()` method to the R client (#7401, @leeweijie)
...
- [Models] Pin the major version of MLForge in Models' `requirements.txt` and `conda.yaml` files (#7364, @BenWilson2)
- [Scoring] Extend `MLForge.pyfunc.spark_udf()` to support StructType results (#7527, @WeichenXu123)
- [Scoring] Extend TensorFlow and Keras Models to support multi-dimensional inputs with `MLForge.pyfunc.spark_udf()`(#7531, #7291, @WeichenXu123)
...
- [Tracking] Make the metric names produced by `MLForge.autolog()` consistent with `MLForge.evaluate()` (#7418, @wenfeiy-db)
...
- [Scoring] Fix a permissions issue encountered when using NFS during model scoring with `MLForge.pyfunc.spark_udf()` (#7427, @WeichenXu123)
...
The 2.0.1 version of MLForge is a major milestone release that focuses on simplifying the management of end-to-end MLOps workflows, providing new feature-rich functionality, and expanding upon the production-ready MLOps capabilities offered by MLForge.
This release contains several important breaking changes from the 1.x API, additional major features and improvements.

Features:

- [Recipes] MLForge Pipelines is now MLForge Recipes - a framework that enables data scientists to quickly develop high-quality models and deploy them to production
- [Recipes] Add support for classification models to MLForge Recipes (#7082, @bbarnes52)
...
- [Tracking] Update `MLForge.pyfunc.get_model_dependencies()` to download all referenced requirements files for specified models (#6733, @harupy)
- [Tracking] Add support for selecting the Keras model `save_format` used by `MLForge.tensorflow.autolog()` (#7123, @balvisio)
- [Models] Set `MLForge.evaluate()` status to stable as it is now a production-ready API
- [Models] Simplify APIs for specifying custom metrics and custom artifacts during model evaluation with `MLForge.evaluate()` (#7142, @harupy)
- [Models] Correctly infer the positive label for binary classification within `MLForge.evaluate()` (#7149, @dbczumar)
- [Models] Enable automated signature logging for `tensorflow` and `keras` models when `MLForge.tensorflow.autolog()` is enabled (#6678, @BenWilson2)
- [Models] Add support for native Keras and Tensorflow Core models within `MLForge.tensorflow` (#6530, @WeichenXu123)
- [Models] Add support for defining the `model_format` used by `MLForge.xgboost.save/log_model()` (#7068, @AvikantSrivastava)
...
- [Core] Support for Python 3.7 has been dropped. MLForge now requires Python >=3.8
- [Recipes] `MLForge.pipelines` APIs have been replaced with `MLForge.recipes`
...
- [Tracking] Remove deprecated `MLForgeClient.download_artifacts` API (#6537, @WeichenXu123)
- [Tracking] Change the behavior of environment variable handling for `MLForge_EXPERIMENT_NAME` such that the value is always used when creating an experiment (#6674, @BenWilson2)
- [Tracking] Update `MLForge server` to run in `--serve-artifacts` mode by default (#6502, @harupy)
- [Tracking] Update Experiment ID generation for the Filestore backend to enable threadsafe concurrency (#7070, @BenWilson2)
- [Tracking] Remove `dataset_name` and `on_data_{name | hash}` suffixes from `MLForge.evaluate()` metric keys (#7042, @harupy)
...
- [Models] Move Keras model logging APIs to the `MLForge.tensorflow` flavor and drop support for TensorFlow Estimators (#6530, @WeichenXu123)
- [Models] Remove deprecated `MLForge.sklearn.eval_and_log_metrics()` API in favor of `MLForge.evaluate()` API (#6520, @dbczumar)
- [Models] Require `MLForge.evaluate()` model inputs to be specified as URIs (#6670, @harupy)
- [Models] Drop support for returning custom metrics and artifacts from the same function when using `MLForge.evaluate()`, in favor of `custom_artifacts` (#7142, @harupy)
...
- [Scoring] Remove public APIs for `MLForge.sagemaker.deploy()` and `MLForge.sagemaker.delete()` in favor of MLForge deployments APIs, such as `MLForge deployments -t sagemaker` (#6650, @dbczumar)
- [Scoring] Rename input argument `df` to `inputs` in `MLForge.deployments.predict()` method (#6681, @BenWilson2)
- [Projects] Replace the `use_conda` argument with the `env_manager` argument within the `run` CLI command for MLForge Projects (#6654, @harupy)
- [Projects] Modify the MLForge Projects docker image build options by renaming `--skip-image-build` to `--build-image` with a default of `False` (#7011, @harupy)
- [Integrations/Azure] Remove deprecated `MLForge.azureml` modules from MLForge in favor of the `azure-MLForge` deployment plugin (#6691, @BenWilson2)
...
- [Tracking] Set the MLForge Run name correctly when specified as part of the `tags` argument to `MLForge.start_run()` (#7228, @Cokral)
- [Tracking] Fix an issue with conflicting MLForge Run name assignment if the `MLForge.runName` tag is set (#7138, @harupy)

...
MLForge 0.9.1 is a patch release on top of 0.9.0 containing mostly bug fixes and internal improvements. We have also included a one breaking API change in preparation for additions in MLForge 1.0 and later. This release also includes significant improvements to the Search API.
...
- [Tracking/Python] Reintroduces the parent_run_id argument to MLForgeClient.create_run. This API is planned for removal in MLForge 1.0 (#1137 @smurching)
- [Tracking/Python] Provide default implementations of AbstractStore log methods (#1051 @acroz)
- [R] (Released on CRAN as MLForge 0.9.0.1) Small bug fixes with R (#1123 @smurching; #1045, #1017, #1019, #1039, #1048, #1098, #1101, #1107, #1108, #1119 @tomasatdatabricks)
...
- Rebuilt MLForge JS assets to fix an issue where form input was broken in MLForge 0.9.0 (identified
...
- Support for running MLForge Projects in Docker containers. This allows you to include non-Python dependencies in their project environments and provides stronger isolation when running projects. See the [Projects documentation](https://MLForge.org/docs/latest/projects.html) for more information. (#555, @marcusrehm; #819, @mparkhe; #970, @dbczumar)
- Database stores for the MLForge Tracking Server. Support for a scalable and performant backend store was one of the top community requests. This feature enables you to connect to local or remote SQLAlchemy-compatible databases (currently supported flavors include MySQL, PostgreSQL, SQLite, and MS SQL) and is compatible with file backed store. See the [Tracking Store documentation](https://MLForge.org/docs/latest/tracking.html#storage) for more information. (#756, @AndersonReyes; #800, #844, #847, #848, #860, #868, #975, @mparkhe; #980, @dbczumar)
- Simplified custom Python model packaging. You can easily include custom preprocessing and postprocessing logic, as well as data dependencies in models with the `python_function` flavor using updated `MLForge.pyfunc` Python APIs. For more information, see the [Custom Python Models documentation](https://MLForge.org/docs/latest/models.html#custom-python-models). (#791, #792, #793, #830, #910, @dbczumar)
- Plugin systems allowing third party libraries to extend MLForge functionality. The [proposal document](https://gist.github.com/zblz/9e337a55a7ba73314890be68370fa69a) gives the full detail of the three main changes:

  - You can register additional providers of tracking stores using the `MLForge.tracking_store` entrypoint. (#881, @zblz)
  - You can register additional providers of artifact repositories using the `MLForge.artifact_repository` entrypoint. (#882, @mociarain)
  - The logic generating run metadata from the run context (e.g. `source_name`, `source_version`) has been refactored into an extendable system of run context providers. Plugins can register additional providers using the `MLForge.run_context_provider` entrypoint, which add to or overwrite tags set by the base library. (#913, #926, #930, #978, @acroz)
...
- [Models] PyTorch model persistence improvements to allow persisting definitions and dependencies outside the immediate scope:
  - Add a `code_paths` parameter to `MLForge.pytorch.save_model` and `MLForge.pytorch.log_model` to allow external module dependencies to be specified as paths to python files. (#842, @dbczumar)
  - Improve `MLForge.pytorch.save_model` to capture class definitions from notebooks and the `__main__` scope (#851, #861, @dbczumar)
...
- [Example] Correct missing tensorboardX module error in PyTorch example when running in MLForge Docker container (#809, @jimthompson5802)
...
MLForge 0.8.2 is a patch release on top of 0.8.1 containing only bug fixes and no breaking changes or features.

Bug fixes:

- [Python API] CloudPickle has been added to the set of MLForge library dependencies, fixing missing import errors when attempting to save models (#777, @tmielika)
- [Python API] Fixed a malformed logging call that prevented `MLForge.sagemaker.push_image_to_ecr()` invocations from succeeding (#784, @jackblandin)
...
MLForge 0.8.1 introduces several significant improvements:

- Improved UI responsiveness and load time, especially when displaying experiments containing hundreds to thousands of runs.
- Improved visualizations, including interactive scatter plots for MLForge run comparisons
- Expanded support for scoring Python models as Spark UDFs. For more information, see the [updated documentation for this feature](https://MLForge.org/docs/latest/models.html#export-a-python-function-model-as-an-apache-spark-udf).
...
- [API/CLI] Support for running MLForge projects from ZIP files (#759, @jmorefieldexpe)
...
- [Tracking] Include the Git repository URL as a tag when tracking an MLForge run within a Git repository (#741, @whiletruelearn, @mateiz)
...
- [UI] Extended CSRF support by allowing the MLForge UI server to specify a set of expected headers that clients should set when making AJAX requests (#733, @aarondav)

Bug fixes and documentation updates:

- [Python/Scoring] MLForge Python models that produce Pandas DataFrames can now be evaluated as Spark UDFs correctly. Spark UDF outputs containing multiple columns of primitive types are now supported (#719, @tomasatdatabricks)
...
- [Docs] New example demonstrating how the MLForge REST API can be used to create experiments and log run information (#750, kjahan)
- [Docs] R documentation has been updated for clarity and style consistency (#683, @stbof)
- [Docs] Added clarification about user setup requirements for executing remote MLForge runs on Databricks (#736, @andyk)

Small bug fixes and doc updates (#768, #715, @smurching; #728, dodysw; #730, mshr-h; #725, @kryptec; #769, #721, @dbczumar; #714, @stbof)

## 0.8.0 (2018-11-08)

MLForge 0.8.0 introduces several major features:
...
- Dramatically improved UI for comparing experiment run results:
  ...
  - Runs that are "nested" inside other runs (e.g., as part of a hyperparameter search or multistep workflow) now show up grouped by their parent run, and can be expanded or collapsed altogether. Runs can be nested by calling `MLForge.start_run` or `MLForge.run` while already within a run.
...
- [CLI] `MLForge sklearn serve` has been removed in favor of `MLForge pyfunc serve`, which takes the same arguments but works against any pyfunc model (#690, @dbczumar)

Features:

- [Scoring] pyfunc server and SageMaker now support the pandas "split" JSON format in addition to the "records" format. The split format allows the client to specify the order of columns, which is necessary for some model formats. We recommend switching client code over to use this new format (by sending the Content-Type header `application/json; format=pandas-split`), as it will become the default JSON format in MLForge 0.9.0. (#690, @dbczumar)
...
- [Server] Allow passing gunicorn options when starting MLForge server (#626, @mparkhe)
...
MLForge 0.7.0 introduces several major features:
...
- [Scoring] Support downloading MLForge scoring JAR from Maven during scoring container build (#507, @dbczumar)
...
MLForge 0.6.0 introduces several major features:
...
In addition to these features, there are a host of improvements and bugfixes to the REST API, Python API, tracking UI, and documentation. The [examples](https://github.com/MLForge/MLForge/tree/master/examples) subdirectory has also been revamped to make it easier to jump in, and examples demonstrating multistep workflows and hyperparameter tuning have been added.

Breaking changes:

We fixed a few inconsistencies in the the `MLForge.tracking` API, as introduced in 0.5.0:

- `MLForgeService` has been renamed `MLForgeClient` (#461, @mparkhe)
- You get an `MLForgeClient` by calling `MLForge.tracking.MLForgeClient()` (previously, this was `MLForge.tracking.get_service()`) (#461, @mparkhe)
- `MLForgeService.list_runs` was changed to `MLForgeService.list_run_infos` to reflect the information actually returned by the call. It now returns a `RunInfo` instead of a `Run` (#334, @aarondav)
- `MLForgeService.log_artifact` and `MLForgeService.log_artifacts` now take a `run_id` instead of `artifact_uri`. This now matches `list_artifacts` and `download_artifacts` (#444, @aarondav)

Features:

- Java client API added with support for the MLForge Tracking API (analogous to `MLForge.tracking`), allowing users to create and manage experiments, runs, and artifacts. The release includes a [usage example](https://github.com/MLForge/MLForge/blob/master/MLForge/java/client/src/main/java/org/MLForge/tracking/samples/QuickStartDriver.java>)and [Javadocs](https://MLForge.org/docs/latest/java_api/index.html). The client is published to Maven under `MLForge:MLForge` (#380, #394, #398, #409, #410, #430, #452, @aarondav)
...
- [API] Experiments can now be deleted and restored via REST API, Python Tracking API, and MLForge CLI (#340, #344, #367, @mparkhe)
- [API] Tags can now be set via a SetTag API, and they have been moved to `RunData` from `RunInfo` (#342, @aarondav)
- [API] Added `list_artifacts` and `download_artifacts` to `MLForgeService` to interact with a run's artifactory (#350, @andrewmchen)
- [API] Added `get_experiment_by_name` to Python Tracking API, and equivalent to Java API (#373, @vfdev-5)
- [API/Python] Version is now exposed via `MLForge.__version__`.
- [API/CLI] Added `MLForge artifacts` CLI to list, download, and upload to run artifact repositories (#391, @aarondav)
- [UI] Added icons to source names in MLForge Experiments UI (#381, @andrewmchen)
- [UI] Added support to view `.log` and `.tsv` files from MLForge artifacts UI (#393, @Shenggan; #433, @whiletruelearn)
- [UI] Run names can now be edited from within the MLForge UI (#382, @smurching)
- [Serving] Added `--host` option to `MLForge serve` to allow listening on non-local addresses (#401, @hamroune)
- [Serving/SageMaker] SageMaker serving takes an AWS region argument (#366, @dbczumar)
- [Python] Added environment variables to support providing HTTP auth (username, password, token) when talking to a remote MLForge tracking server (#402, @aarondav)
- [Python] Added support to override S3 endpoint for S3 artifactory (#451, @hamroune)
- MLForge nightly Python wheel and JAR snapshots are now available and linked from https://github.com/MLForge/MLForge (#352, @aarondav)

Bug fixes and documentation updates:

- [Python] `MLForge run` now logs default parameters, in addition to explicitly provided ones (#392, @mparkhe)
...
- [UI] Entry point name is now shown in MLForge UI (#345, @aarondav)
...
- [Python] Fixed an issue where any `MLForge run` failing printed an extraneous exception (#365, @smurching)
...
- [Docs] Added [comprehensive example](https://github.com/MLForge/MLForge/tree/master/examples/multistep_workflow) of doing a multistep workflow, chaining MLForge runs together and reusing results (#338, @aarondav)
- [Docs] Added [comprehensive example](https://github.com/MLForge/MLForge/tree/master/examples/hyperparam) of doing hyperparameter tuning (#368, @tomasatdatabricks)
- [Docs] Added code examples to `MLForge.keras` API (#341, @dmatrix)
...
MLForge 0.5.2 is a patch release on top of 0.5.1 containing only bug fixes and no breaking changes or features.

Bug fixes:

- Fix a bug with ECR client creation that caused `MLForge.sagemaker.deploy()` to fail when searching for a deployment Docker image (#366, @dbczumar)

## 0.5.1 (2018-08-23)

MLForge 0.5.1 is a patch release on top of 0.5.0 containing only bug fixes and no breaking changes or features.

Bug fixes:

- Fix `with MLForge.start_run() as run` to actually set `run` to the created Run (previously, it was None) (#322, @tomasatdatabricks)
...
- MLForge UI now appropriately shows entrypoint if it's not main (#345, @aarondav)
- Make Python API forward-compatible with newer server versions of protos (#348, @aarondav)
- Improved API docs (#305, #284, @smurching)

## 0.5.0 (2018-08-17)

MLForge 0.5.0 offers some major improvements, including Keras and PyTorch first-class support as models, SFTP support as an artifactory, a new scatterplot visualization to compare runs, and a more complete Python SDK for experiment and run management.

Breaking changes:

- The Tracking API has been split into two pieces, a "basic logging" API and a "tracking service" API. The "basic logging" API deals with logging metrics, parameters, and artifacts to the currently-active active run, and is accessible in `MLForge` (e.g., `MLForge.log_param`). The tracking service API allow managing experiments and runs (especially historical runs) and is available in `MLForge.tracking`. The tracking service API will look analogous to the upcoming R and Java Tracking Service SDKs. Please be aware of the following breaking changes:

  - `MLForge.tracking` no longer exposes the basic logging API, only `MLForge`. So, code that was written like `from MLForge.tracking import log_param` will have to be `from MLForge import log_param` (note that almost all examples were already doing this).
  - Access to the service API goes through the `MLForge.tracking.get_service()` function, which relies on the same tracking server set by either the environment variable `MLForge_TRACKING_URI` or by code with `MLForge.tracking.set_tracking_uri()`. So code that used to look like `MLForge.tracking.get_run()` will now have to do `MLForge.tracking.get_service().get_run()`. This does not apply to the basic logging API.
  - `MLForge.ActiveRun` has been converted into a lightweight wrapper around `MLForge.entities.Run` to enable the Python `with` syntax. This means that there are no longer any special methods on the object returned when calling `MLForge.start_run()`. These can be converted to the service API.

  - The Python entities returned by the tracking service API are now accessible in `MLForge.entities` directly. Where previously you may have used `MLForge.entities.experiment.Experiment`, you would now just use `MLForge.entities.Experiment`. The previous version still exists, but is deprecated and may be hidden in a future version.

- REST API endpoint `/ajax-api/2.0/preview/MLForge/artifacts/get` has been moved to `$static_prefix/get-artifact`. This change is coversioned in the JavaScript, so should not be noticeable unless you were calling the REST API directly (#293, @andremchen)
...
- Pyfunc serve/predict will activate conda environment stored in MLModel. This can be disabled by adding `--no-conda` to `MLForge pyfunc serve` or `MLForge pyfunc predict` (#225, @0wu)
- Python SDK formalized in `MLForge.tracking`. This includes adding SDK methods for `get_run`, `list_experiments`, `get_experiment`, and `set_terminated`. (#299, @aarondav)
- `MLForge run` can now be run against projects with no `conda.yaml` specified. By default, an empty conda environment will be created -- previously, it would just fail. You can still pass `--no-conda` to avoid entering a conda environment altogether (#218, @smurching)
...
- MLForge experiments REST API and `MLForge experiments create` now support providing `--artifact-location` (#232, @aarondav)
...
- MLForge client sends REST API calls using snake_case instead of camelCase field names (#232, @aarondav)
...
- [Projects] MLForge will use the conda installation directory given by the `$MLForge_CONDA_HOME`
  if specified (e.g. running conda commands by invoking `$MLForge_CONDA_HOME/bin/conda`), defaulting
...
- Add missing MLForge.version import to sagemaker module (#229, @dbczumar)
...
- [Projects] Removed the `use_temp_cwd` argument to `MLForge.projects.run()`
  (`--new-dir` flag in the `MLForge run` CLI). Runs of local projects now use the local project
...
- [Tracking] GCS artifact storage is now a pluggable dependency (no longer installed by default).
  ...
  (#202, @smurching)
- [Tracking] Clients running MLForge 0.4.0 and above require a server running MLForge 0.4.0
  or above, due to a fix that ensures clients no longer double-serialize JSON into strings when
  sending data to the server (#200, @aarondav). However, the MLForge 0.4.0 server remains
  backwards-compatible with older clients (#216, @aarondav)

Features:

- [Examples] Add a more advanced tracking example: using MLForge with PyTorch and TensorBoard (#203)
...
- [Server] Added --static-prefix option to serve UI from a specified prefix to MLForge UI and server (#116, @andrewmchen)
...
- [MLForge Server] Renamed `--artifact-root` parameter to `--default-artifact-root` in `MLForge server` to better reflect its purpose (#165, @aarondav)
...
- Support asychronous/parallel execution of MLForge runs (#82, @smurching)
- [SageMaker] Support for deleting, updating applications deployed via SageMaker (#145, @dbczumar)
- [SageMaker] Pushing the MLForge SageMaker container now includes the MLForge version that it was published with (#124, @sueann)
...
- Switch protobuf implementation to C, fixing a bug related to tensorflow/MLForge import ordering (issues #33 and #77, PR #74, @andrewmchen)
- Enable running MLForge server without git binary installed (#90, @aarondav)
- Fix Spark UDF support when running on multi-node clusters (#92, @aarondav)

## 0.2.0 (2018-06-27)

- Added `MLForge server` to provide a remote tracking server. This is akin to `MLForge ui` with new options:

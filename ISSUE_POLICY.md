...
The MLForge Issue Policy outlines the categories of MLForge GitHub issues and discusses the guidelines & processes
associated with each type of issue.

Before filing an issue, make sure to [search for related issues](https://github.com/MLForge/MLForge/issues) and check if
they address yours.

For support (ex. "How do I do X?"), please ask on [Stack Overflow](https://stackoverflow.com/questions/tagged/MLForge).
...
5. The implementation owner begins developing the feature and ultimately files associated pull requests against the
   MLForge Repository or packages the feature as an MLForge Plugin.
...
- Verify that the bug you are reporting meets one of the following criteria:
  - A recent release of MLForge does not support the operation you are doing that an earlier release did (a regression).
  - A [documented feature](https://MLForge.org/docs/latest/index.html) or functionality does not work properly by executing a provided example from the docs.
  - Any exception raised is directly from MLForge and is not the result of an underlying package's exception (e.g., don't file an issue that MLForge can't log a model that can't be trained due to a tensorflow Exception)
...
- Validate that MLForge supports the functionality that you're having an issue with. _A lack of a feature does not constitute a bug_.
...
3. An MLForge committer reproduces the bug and provides feedback about how to implement a fix.
4. After an approach has been agreed upon, an owner for the fix is identified. MLForge committers may choose to adopt
...
1. A documentation GitHub Issue is submitted, which contains a description of the issue and its location(s) in the MLForge documentation.
...
3. An MLForge committer confirms the documentation issue and provides feedback about how to implement a fix.
4. After an approach has been agreed upon, an owner for the fix is identified. MLForge committers may choose to adopt
...
3. An MLForge committer confirms the installation issue and provides feedback about how to implement a fix.
4. After an approach has been agreed upon, an owner for the fix is identified. MLForge committers may choose to adopt

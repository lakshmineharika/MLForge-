...
MLForge 3.14.0 includes several major features and improvements

### Major New Features

- **🚀 [One-command agent onboarding with `MLForge agent setup`](https://MLForge.org/docs/latest/genai/tracing/quickstart/)**: Install MLForge, set up tracing, and hand your favorite coding agent (Claude Code, OpenAI Codex, or OpenCode) the MLForge skills to instrument your app, all from a single command.
- **⚡ [Durable, low-latency tracing for Claude Code](https://MLForge.org/docs/latest/genai/tracing/integrations/listing/claude_code)**: Roll out Claude Code tracing across a team with confidence: a write-ahead-log keeps it from slowing the agent, overwhelming the tracking server, or losing traces on a network blip or crash.
- **📝 [Review Queues for traces](https://MLForge.org/docs/latest/genai/assessments/review-queues)**: Assign traces to reviewers (or agents) and collect structured feedback and ground-truth annotations in the UI, written straight back onto the trace so they are immediately usable for evaluation.
- **🗂️ [Revamped evaluation dataset UI](https://MLForge.org/docs/latest/genai/datasets/)**: Browse, inspect, edit, and bulk-manage evaluation dataset records directly in the UI, with click-through to the source trace.
- **🧪 [Pytest integration for regression testing](https://MLForge.org/docs/latest/genai/eval-monitor/regression-testing)**: Write GenAI regression tests as plain pytest functions with the `@MLForge.test` marker, gate them in CI, and review test history and per-assertion judge results in the UI.
- **🎛️ [LLM Playground](https://MLForge.org/docs/latest/genai/prompt-registry/playground)**: Iterate on prompts in the browser against your AI Gateway endpoints and Prompt Registry versions, with settings, tools, structured output, and template variables.

### Breaking Changes

- [Models] Change `MLForge.sklearn` `serialization_format` default from `cloudpickle` to `skops` (#23987, @copilot-swe-agent)
- [Models] Change `serialization_format` default to `"pt2"` for `MLForge.pytorch.log_model` and `MLForge.pytorch.save_model` (#23988, @copilot-swe-agent)
- [Models] Change `serialization_format` default to `"skops"` in `MLForge.lightgbm` `log_model`/`save_model` (#23986, @copilot-swe-agent)
...
- [Evaluation] [2/3] Add `EvaluationResult.passed`/`.reason` for `@MLForge.test` assertions (#23869, @B-Step62)
...
- [Tracing] Add `x-MLForge-run-id` support to OTLP trace ingestion (#23664, @sanatb187)
- [Evaluation / Tracing] [1/3] Add @MLForge.test pytest marker and assertion framework (#23864, @B-Step62)
- [UI] Improve review queue empty states with onboarding content (#23903, @B-Step62)
- [UI] Add `MLForge skills view/list` CLI (#23907, @joshuawong-db)
- [UI] Improve review queue list: flat layout, sortable columns, status filter (#23902, @B-Step62)
- [Tracing] Add `MLForge_WORKSPACE` support to OSS auth provider (#23927, @Nehanth)
- [Gateway] Add cached token pricing to Databricks model catalog (#23901, @TomeHirata)
- [Evaluation] Add `MLForge_GENAI_JUDGE_DEFAULT_MODEL` environment variable (#23860, @B-Step62)
- [Evaluation] Wire "Run judge(s)" submission in "Run Eval" in Evaluations Run page to POST /MLForge/genai/evaluate/invoke (#23781, @aaronteo-db)
- [Evaluation] Add rule-based built-in scorers: `RegexMatch`, `PIIDetection`, `ResponseLength` (#22571, @debu-sinha)
- [Tracing] Support Databricks backend in `MLForge agent setup` (#23783, @harupy)
- [Evaluation] Add POST /MLForge/genai/evaluate/invoke handler & job for UI-triggered eval runs (#23779, @aaronteo-db)
- [Tracing] [Claude Code] Support UC trace location via `MLForge_TRACE_LOCATION` (#23770, @B-Step62)
- [Tracing] [Codex] Support UC trace location via `MLForge_TRACE_LOCATION` (#23771, @B-Step62)
...
- [Tracing] Support mapping gen_ai.conversation.id to MLForge trace session (#23584, @SahilKumar75)
...
- [Tracing] Add MLForgeWalSpanExporter to hand traces off to the WAL daemon (#23641, @aaronteo-db)
...
- [Tracing] Fix TS SDK traces storage when MLForge server uses a local FS artifact root without MLForge-artifacts:// uri schema (#23992, @aaronteo-db)
...
- [Build] @MLForge/XXXX package root points to missing dist/index.js (#23874, @WeichenXu123)
...
- [Tracing / Tracking] Fix `MLForge.openai.autolog` span type resolution for `ChatCompletions` subclasses (#23759, @harupy)
- [Tracking] Fix `MLForge db upgrade` on a fresh database (#23752, @harupy)
...
- [Tracing / Tracking] Restore `MLForge.crewai` autolog on crewai 1.14.5 (#23682, @harupy)
...
- [Tracing] Extend `MLForge.sourceRun` metrics filter to cover post-hoc linked OTLP traces (#23591, @RudraDudhat2509)
- [Tracing] UI does not show Judge costs (#23586, @WeichenXu123)
- [Tracking] [Security] Register auth validator for /ajax-api/3.0/MLForge/get-trace-artifact (#23317, @B-Step62)
...
- [Docs / Evaluation] Add docs page for `@MLForge.test` pytest regression testing (#24011, @B-Step62)
...
- [Docs] Surface MLForge agent setup in docs (#23859, @joshuawong-db)
- [Docs] Document MLForge_STATIC_PREFIX behavior change in migration guide (#23851, @Sanket2329)
...
- [Docs / Models] Deprecate `validate_serving_input` in favor of `MLForge.models.predict` (#23376, @B-Step62)
...
MLForge 3.13.0 includes several major features and improvements

### Major New Features

- **🔐 [Role-Based Access Control & Admin UI](https://MLForge.org/docs/latest/self-hosting/security/role-based-access-control)**: A full RBAC system with reusable roles and workspace-scoped grants, plus a new web Admin UI for managing users, roles, and permissions on self-hosted MLForge.
- **🗄️ [Trace Retention & Auto Archival](https://MLForge.org/docs/latest/genai/tracing/observe-with-traces/archive-traces)**: Automatically move aged trace span data out of your SQL backend into object storage (e.g. S3) while keeping every trace fully readable in the UI and APIs.
- **🤖 [One-click observability & governance for coding agents](https://MLForge.org/docs/latest/genai/governance/ai-gateway/coding-agents/)**: Onboard Claude Code, OpenAI Codex, or Gemini CLI to the AI Gateway in one click for tracing, usage tracking, budgets, and guardrails.
- **✨ [New engines for MLForge Assistant](https://MLForge.org/docs/latest/genai/getting-started/try-assistant/)**: Run MLForge Assistant on a local Ollama model, the OpenAI Codex CLI, or any MLForge AI Gateway endpoint, in addition to Claude Code.
- **☸️ [Helm chart for Kubernetes](https://MLForge.org/docs/latest/self-hosting/kubernetes-helm)**: An official, production-ready Helm chart for deploying the MLForge tracking server to any Kubernetes cluster.
- **🌐 [Hermes Agent support](https://MLForge.org/docs/latest/genai/governance/ai-gateway/coding-agents/hermes-agent)**: Route the Hermes Agent runtime through the AI Gateway and capture its end-to-end traces in MLForge over OpenTelemetry.
- **🪵 [Span log levels](https://MLForge.org/docs/latest/genai/tracing/app-instrumentation/logging)**: Python-`logging`-style severity levels on spans, with a "Minimum log level" filter in the trace UI to hide low-level noise.
...
- MLServer is no longer available as a pyfunc serving backend. The previously deprecated `enable_mlserver` option has been removed, so `MLForge models serve` always uses the built-in scoring server. (#23356, @harupy)
- `MLForge autolog claude` no longer installs the old Python autolog hook; Claude Code tracing is now provided by the official Claude plugin, which must be installed separately. (#23339, @B-Step62)
...
- Pointing the tracking or model registry store at a local file-system path now raises an error by default; set `MLForge_ALLOW_FILE_STORE=true` to keep using a file-based store. (#22773, @harupy)

### Other Assorted Features & Improvements:

- [Gateway] Support AI Gateway as a backend of MLForge Assistant (#23559, @B-Step62)
...
- [Tracking / UI] Unified per-user permission APIs: `grant` / `revoke` / `get` / `list` under `/MLForge/users/permissions/*` (#23247, @PattaraS)
- [Evaluation / Tracing] Add `MLForge.genai.test_agent` for automated agent stress-testing (#22990, @serena-ruan)
...
- [Build] Add Helm charts for deploying MLForge to kubernetes cluster (#21973, @WeichenXu123)
- [Tracking] Fix Databricks unified auth support when MLForge_ENABLE_DB_SDK=true (#20599, @vb-dbrks)
...
- [Tracking] Forward MLForge client telemetry from inside Databricks workloads (#23483, @smoorjani)
...
- [Tracking] fix(tracking): return `<console>` for `MLForge.source.name` when `sys.argv[0]` is empty (#23352, @xodn348)
...
- [Tracing] Make `MLForge.get_trace` V4 retry policy configurable (#23443, @artjen)
...
- [Evaluation] Surface MLForge version mismatch when deserializing scorers (#23215, @smoorjani)
- [Tracing] Ship compiled `dist/` in @MLForge/MLForge-openclaw so `openclaw plugins install` works (#23220, @B-Step62)
...
- [Evaluation] [Security] Add `MLForge_ALLOW_PICKLE_DESERIALIZATION` guard to `PickleEvaluationArtifact` (#23183, @TomeHirata)
...
- [Models / Tracking] Add `MLForge_SKIP_PIP_REQUIREMENTS_CHECK` env var to bypass pip validation in air-gapped environments (#22920, @copilot-swe-agent)
...
- [Tracing] Preserve cache_read tokens in `@MLForge/claude-code` TypeScript plugin for cache observability (#22906, @dgokeeffe)
...
MLForge 3.12.0 includes several major features and improvements

### Major New Features

- **🖼️ Multimodal Tracing**: Users can now store multimodal content in tracing spans as artifact attachments instead of inline binary data. We've also patched the UI to support the new MLForge-attachment:// style URI, with rich rendering available for PDFs, audio, and images.
...
- **🛡️ Gateway guardrails**: You can now set guardrails on your gateway endpoints to prevent unsafe or non-compliant model inputs and outputs. Try it out in the MLForge UI!
...
- [Evaluation] Enable third-party scorer registration in OSS MLForge (#22634, @smoorjani)
- [] Add platform-side telemetry fields to Gateway invocation and budget events (#22557, @PattaraS)
- [Tracing] Auto-start SQL warehouse before V4/V5 MLForge tracing calls (#22798, @artjen)
...
- [Docs / Models] Add `MLForge.diffusers` flavor for diffusion model LoRA adapters (#22253, @Rasaboun)
...
- [Tracing] Add MLForge tracing spans to guardrail execution in gateway (#22581, @TomeHirata)
...
- [Tracing] Add @MLForge/vercel for better Vercel AI SDK tracing in Databricks UC (#22105, @dbrx-euirim)
...
- [] Add `X-MLForge-Gateway-Duration-Ms` and `X-MLForge-Gateway-Overhead-Duration-Ms` response headers (#22229, @PattaraS)
...
- [Build] Add `aiohttp` as a core dependency of `MLForge` (#22189, @TomeHirata)
...
- [Tracking] Add `MLForge db move-resources` command to move resources between workspaces (#21263, @mprahl)
...
- [Tracing / UI] Set `MLForge.message.format` on gateway passthrough spans to enable Chat tab (#22916, @TomeHirata)
...
- [Tracing] Fix exception propagation in `@MLForge.trace` within `@trace_disabled` context (#22501, @harupy)
...
- [Docs / Tracing] Enable async trace logging by default for OSS MLForge (#22304, @PattaraS)
...
- [Tracing / Tracking] Map user.id span attribute to MLForge.trace.user during OTel ingestion (#22150, @ajgassner)
...
- [Docs] Add Live Demo documentation page for demo.MLForge.org (#22840, @serena-ruan)
...
MLForge 3.11.1 includes several major features and improvements.

**Major New Features**:

- 🔍 **Automatic Issue Identification**: Automatically identify quality issues in your agent with AI! Use the new "Detect Issues" button in the traces table to analyze selected traces and surface potential problems across categories like correctness, safety, and performance. Issues are linked directly to traces for easy investigation and debugging. [Docs](https://MLForge.org/docs/latest/genai/eval-monitor/ai-insights/detect-issues) (#21431, #21204, #21165, #21163, #21161, @smoorjani, @serena-ruan)
- 💰 **Gateway Budget Alerts & Limits**: Control your AI Gateway spending with configurable budget policies! Set spending limits by time window (daily, weekly, or monthly), receive alerts before hitting limits, and prevent runaway costs with automatic request blocking. The new budget management UI lets you track spending, configure webhooks for notifications, and monitor violations across all your gateway endpoints. [Docs](https://MLForge.org/docs/latest/genai/governance/ai-gateway/budget-alerts-limits) (#21116, #21534, #21569, #21473, #21108, @TomeHirata, @copilot-swe-agent)
- 📊 **Trace Graph View**: Visualize complex trace hierarchies with an interactive graph view! Navigate multi-level trace structures, understand parent-child relationships at a glance, and debug complex systems more effectively with a visual representation of your trace topology. [Docs](https://MLForge.org/docs/latest/genai/tracing/observe-with-traces/ui) (#20607, @joelrobin18)
- 🌐 **Native OpenTelemetry GenAI Convention Support**: MLForge now natively supports the OpenTelemetry GenAI Semantic Conventions for trace export! When exporting traces via OTLP with `MLForge_ENABLE_OTEL_GENAI_SEMCONV` enabled, MLForge automatically translates them to follow the OTel GenAI semantic conventions, enabling seamless integration with OTel-compatible observability platforms while preserving GenAI-specific metadata. [Docs](https://MLForge.org/docs/latest/genai/tracing/opentelemetry/genai-semconv) (#21494, #21495, @B-Step62)
- 🔧 **OpenCode Tracing Integration**: Debug smarter with OpenCode CLI integration! Track and analyze code execution flows directly from your development workflow, making it easier to identify performance bottlenecks and trace issues back to specific code paths. [Docs](https://MLForge.org/docs/latest/genai/tracing/integrations/listing/opencode) (#20133, @joelrobin18)
- ⚡ **Native UV Support for Model Dependencies**: Automatic dependency inference now supports UV! MLForge automatically detects UV projects and captures exact, locked dependencies from your lockfile when logging models, ensuring reproducible environments. [Docs](https://MLForge.org/docs/latest/ml/model/dependencies) (#20344, #20935, @debu-sinha)
- 🔒 **Pickle-Free Model Serialization**: Enhance security with pickle-free model formats! MLForge now supports safer model serialization using `torch.export` and `skops` formats, with improved controls when `MLForge_ALLOW_PICKLE_DESERIALIZATION=False`. Comprehensive documentation guides you through migrating existing models to pickle-free formats for production deployments. [Docs](https://MLForge.org/docs/latest/ml/tracking/pickle-free-models) (#21404, #21188, #20774, @WeichenXu123)

**Breaking Changes**:

- ⚠️ **TypeScript SDK Package Renaming**: The MLForge TypeScript SDK packages have been renamed to use npm organization scoping. If you're using the TypeScript SDK, update your `package.json` dependencies and import statements: `MLForge-tracing` → `@MLForge/core`, `MLForge-openai` → `@MLForge/openai`, `MLForge-anthropic` → `@MLForge/anthropic`, `MLForge-gemini` → `@MLForge/gemini`. All packages are now at version `0.2.0`. (#20792, @B-Step62)
- Remove `MLForge_ENABLE_INCREMENTAL_SPAN_EXPORT` environment variable (#22182, @PattaraS)
...
- [Evaluation] Add pipelined predict-score execution for MLForge.genai.evaluate (#20940, @alkispoly-db)
...
- [UI] Add Structured Output (JSON Schema) Support to the MLForge Prompts UI (#21394, @kennyvoo)
...
- [Tracing] Add MLForge.tracing.context() API for injecting metadata/tags without wrapper spans (#21318, @B-Step62)
...
- [Docs / Tracing / Tracking] Add MLForge.otel.autolog() for OTEL-based tracing integrations (Langfuse, Arize/Phoenix) (#20954, @alkispoly-db)
...
- [Tracking] [MLForge Demo] Add server availability handling checks (#20349, @BenWilson2)
- [Tracking] [MLForge Demo] Add scorers demo (#20287, @BenWilson2)
...
- [Tracing / Tracking] Fix MLForge-skinny: guard numpy-dependent imports in MLForge.types (#22211, @Suraj-kumar00)
...
- [Tracking] Fix `NextMethod()` S3 dispatch error in R `MLForge_get_run_context` (#21957, @daniellok-db)
...
- [Scoring] Fix Starlette 1.0 compatibility in `MLForge/pyfunc/scoring_server/__init__.py` (#21908, @copilot-swe-agent)
...
- [Gateway] Change default MLForge_GATEWAY_BUDGET_REFRESH_INTERVAL from 60 to 600 seconds (#21565, @copilot-swe-agent)
- [Evaluation / Tracking] Fix scorer re-registration raising RESOURCE_ALREADY_EXISTS in auth layer (#21560, @harupy)
- [Tracking] Harden check when `MLForge_ALLOW_PICKLE_DESERIALIZATION` is disabled (#21404, @WeichenXu123)
...
- [Tracking] Fix: MLForge_ALLOW_PICKLE_DESERIALIZATION=False safety control is ineffective for pyfunc flavor (#21188, @WeichenXu123)
- [Tracing / UI] Fix Pydantic AI autologging: auto-enable instrumentation and fix Chat UI (#21278, @B-Step62)
- [Tracing] Fix span type not translated for OTel spans when MLForge SDK is active (#21307, @B-Step62)
...
- [Tracing / Tracking] Fix Claude Code autologging import collision with local MLForge folders (#21343, @smoorjani)
...
- [Docs] docs: clarify `uv` dependency management vs `MLForge_LOCK_MODEL_DEPENDENCIES`, add uv workspace limitation (#22312, @copilot-swe-agent)
...
- [Docs / Tracing] Fix `MLForge_ENABLE_ASYNC_TRACE_LOGGING` docs to reflect OSS default behavior (#21731, @copilot-swe-agent)
...
- [Docs] Add Guide: Deploy MLForge to Google Cloud (#21599, @WeichenXu123)
- [Docs] Add Guide: Deploy MLForge to Azure cloud (#21128, @WeichenXu123)
- [Docs / Tracing] Add Goose tracing integration documentation (#21190, @B-Step62)
- [Docs] Expand Koog integration doc (#21218, @B-Step62)
- [Docs / Tracing] Add 'Combine with MLForge SDK' section to OTel integration guides (#21298, @TomeHirata)
- [Docs] docs: add Budget Tracker Strategies guideline to AI Gateway budget page (#21633, @copilot-swe-agent)
- [Docs] Add tracking URI note to MLForge-skinny README (#21638, @harupy)
- [Docs] Add Guide: Deploy MLForge to AWS cloud (#20729, @WeichenXu123)
...
- [Docs / Prompts] Update prompt registry docs to use MLForge 3.x API examples (#21267, @copilot-swe-agent)
- [Docs] docs: Add single quotes to install commands with extras to prevent zsh errors (#21227, @mshavliuk)
- [Docs] Add Amazon Nova bedrock model examples for MLForge.metrics.genai (#21063, @ManasVardhan)
...
We're excited to announce MLForge 3.11.0rc0, which includes several notable updates:
...
- 🌐 **Native OpenTelemetry GenAI Convention Support**: MLForge now natively supports the OpenTelemetry GenAI Semantic Conventions for trace export! When exporting traces via OTLP with `MLForge_ENABLE_OTEL_GENAI_SEMCONV` enabled, MLForge automatically translates them to follow the OTel GenAI semantic conventions, enabling seamless integration with OTel-compatible observability platforms while preserving GenAI-specific metadata. (#21494, #21495, @B-Step62)
...
- ⚡ **UV Package Manager Support**: Automatic dependency inference now supports UV! MLForge automatically detects UV projects and captures exact, locked dependencies from your lockfile when logging models, ensuring reproducible environments. (#20344, #20935, @debu-sinha)
...
`pip install MLForge==3.11.0rc0`

## 3.10.1 (2026-03-05)

MLForge 3.10.1 is a patch release that contains some minor feature enhancements, bug fixes, and documentation updates.
...
- [Model Registry] Fix MLForgeClient.copy_model_version for the case that copy UC model across workspaces (#21212, @WeichenXu123)
...
We're excited to announce MLForge 3.10.0, which includes several notable updates:

**Major New Features**:

🏢 **Organization Support in MLForge Tracking Server**: MLForge now supports multi-workspace environments. Users can organize experiments, models, prompts, with a coarser level of unit and logically isolate them in a single tracking server. (#20702, #20657, @mprahl, @Gkrumbach07, @B-Step62)

💬 **Multi-turn Evaluation & Conversation Simulation**: MLForge now supports multi-turn evaluation, including evaluating existing conversations with session-level scorers and simulating conversations to test new versions of your agent, without the toil of regenerating conversations. Use the session-level scorers introduced in MLForge 3.8.0 and the brand new session UIs to evaluate the quality of your conversational agents and enable automatic scoring to monitor quality as traces are ingested. (#20243, #20377, #20289, @smoorjani)

💰 **Trace Cost Tracking**: Gain visibility into your LLM spending! MLForge now automatically extracts model information from LLM spans and calculates costs, with a new UI that renders model and cost data directly in your trace views. (#20327, #20330, @serena-ruan)
...
🎮 **MLForge Demo Experiment**: New to MLForge GenAI? With one click, launch a pre-populated demo and explore tracing, evaluation, and prompt management in action. No configuration, no code required. (#19994, #19995, #20046, #20047, #20048, #20162, @BenWilson2)
...
- [Docs] [MLForge Demo] Docs for GenAI Demo (#20240, @BenWilson2)
- [UI] Move Getting Started above experiments list and make collapsible (#20691, @B-Step62)
- [Model Registry / Tracking] Add MLForge `migrate-filestore` command (#20615, @harupy)
...
- [UI] [ MLForge Demo ] UI updates for MLForge Demo interfaces (#20162, @BenWilson2)
...
- [Tracing] Add per-decorator `sampling_ratio_override` parameter to `@MLForge.trace` (#19784, @harupy)
- [Evaluation / Tracking] Add `MLForge datasets list` CLI command (#20167, @alkispoly-db)
...
- [Tracking] [MLForge Demo] Add MLForge demo cli command (#20048, @BenWilson2)
- [Evaluation] Add an SDK for distillation from conversation to goal/persona (#20289, @smoorjani)
- [Tracing] Livekit Agents Integration in MLForge (#20439, @joelrobin18)
...
- [Tracking] [MLForge Demo] Add Prompt demo data (#20047, @BenWilson2)
- [Tracking] [MLForge Demo] Add Eval simulation data (#20046, @BenWilson2)
- [Tracking] [MLForge Demo] Add trace data for demo (#19995, @BenWilson2)
...
- [Tracking] [MLForge Demo] Base implementation for demo framework (#19994, @BenWilson2)
...
- [Prompts] [Prompt Optimization Backend PR #1] Wrap prompt optimize in MLForge job (#20001, @chenmoneygithub)
- [Tracking] Add --experiment-name option to MLForge experiments get command (#19929, @alkispoly-db)
...
- [Tracing] Add `MLForge_GATEWAY_RESOLVE_API_KEY_FROM_FILE` flag to prevent local file inclusion in API gateway (#20965, @TomeHirata)
...
- [Tracking] Fix: Do not emit pickle warning when user calls `MLForge.pyfunc.log_model` with `loader_module` param (#20727, @WeichenXu123)
...
- [Tracing] Fix type hints lost with @MLForge.trace decorator (#20648, @veeceey)
- [Evaluation] Use JSONAdapter for best-effort structured outputs in MemAlign predictions (#20679, @smoorjani)
- [Tracking] Fix `MLForge demo` URL to use experiment ID instead of name (#20678, @copilot-swe-agent)
...
- [Tracking] Ensure server-provided artifact root is reused on MLForgeClient calls (#19336, @mprahl)
- [UI] Fix trace selection not registering in SelectTracesModal (#20099, @joelrobin18)

Documentation updates:

- [Docs] Add documentation for `MLForge migrate-filestore` command (#20616, @harupy)
- [Docs] Document X-MLForge-WORKSPACE header for AI Gateway endpoints with workspace fallback behavior (#20984, @copilot-swe-agent)
...
- [Docs] Detailed guide for setting up SSO with MLForge-oidc-auth plugin (#20556, @WeichenXu123)
...
We're excited to announce MLForge 3.10.0rc0, which includes several notable updates:

**Major New Features**:

- 🏢 **Organization Support in MLForge Tracking Server**: MLForge now supports multi-workspace environments! You can organize your experiments and resources across different workspaces with a new landing page that lets you navigate between them seamlessly. (#20702, #20657, @Gkrumbach07, @B-Step62)
...
- 🎮 **MLForge Demo Experiment**: Get started with MLForge faster than ever! The new `MLForge demo` CLI command generates a fully-populated demo environment with sample traces, prompts, and evaluation data so you can explore MLForge's features hands-on without any setup. (#19994, #19995, #20046, #20047, #20048, #20162, @BenWilson2)
- 📊 **Gateway Usage Tracking**: Monitor your AI Gateway endpoints with detailed usage analytics. A new usage page shows request patterns and metrics, with trace ingestion that links gateway calls back to your experiments for end-to-end observability. (#20357, #20358, #20642, @TomeHirata)
...
`pip install MLForge==3.10.0rc0`

## 3.9.0 (2026-01-28)

We're excited to announce MLForge 3.9.0, which includes several notable updates:

**Major New Features**:

- 🔮 **MLForge Assistant**: Figuring out the next steps to debug your apps and agents can be challenging. We're excited to introduce the MLForge Assistant, an in-product chatbot that can help you identify, diagnose, and fix issues. The assistant is backed by Claude Code, and directly passes context from the MLForge UI to Claude. Click on the floating "Assistant" button in the bottom right of the MLForge UI to get started!
- 📈 **Trace Overview Dashboard**: You can now get insights into your agent's performance at a glance with the new "Overview" tab in GenAI experiments. Many pre-built statistics are available out of the box, including performance metrics (e.g. latency, request count), quality metrics (based on assessments), and tool call summaries. If there are any additional charts you'd like to see, please feel free to raise an issue in the MLForge repository!
- ✨ **AI Gateway**: We're revamping our AI Gateway feature! AI Gateway provides a unified interface for your API requests, allowing you to route queries to your LLM provider(s) of choice. In MLForge 3.9.0, the Gateway server is now located directly in the tracking server, so you don't need to spin up a new process. Additional features such as passthrough endpoints, traffic splits, and fallback models are also available, with more to come soon! For more detailed information, please take a look at the [docs](https://MLForge.org/docs/latest/genai/governance/ai-gateway/).
- 🔎 **Online Monitoring with LLM Judges**: Configure LLM judges to automatically run on your traces, without having to write a line of code! You can either use one of our [pre-defined judges](https://MLForge.org/docs/latest/genai/eval-monitor/scorers/llm-judge/predefined/), or provide your own prompt and instructions to create custom metrics. Head to the new "Judges" tab within the GenAI Experiment UI to get started.
...
- 🔗 **Distributed Tracing**: Trace context can now be propagated across different services and processes, allowing you to truly track request lifecycles from end to end. The related APIs are defined in the `MLForge.tracing.distributed` module (with more documentation to come soon).
...
- [Evaluation] Introduce a conversation simulator into `MLForge.genai` (#19614, @smoorjani)
- [Evaluation] Integrate conversation simulation into `MLForge.genai.evaluate` (#19760, @smoorjani)
...
- [UI] Merge MLForge Assistant branch (#20011, @B-Step62)
...
- [Prompts] Support metaprompting in `MLForge.genai.optimize_prompts()` (#19762, @chenmoneygithub)
...
- [Tracking] Support `MLForge.genai.to_predict_fn` for app invocation endpoints (#19779, @jennsun)
- [Tracking] Add `log_stream` API for logging binary streams as artifacts (#19104, @harupy)
- [Tracking] Add `import_checkpoints` API for databricks SGC Checkpointing with MLForge (#19839, @WeichenXu123)
...
- [Prompts] Fix the length check in `MLForge.genai.optimize_prompts()` (#19993, @chenmoneygithub)
- [UI] Fix trace selection not registering in SelectTracesModal (#20099, @joelrobin18)
- [UI] Fix LimitOverrunError in Assistant streaming (#20078, @joelrobin18)
- [Tracing] CC Token usage (#20022, @joelrobin18)
- [Gateway] Remove MLForge-specific `auth_mode` from `LiteLLMConfig` (#20059, @TomeHirata)
- [UI] Assistant UI fix for dark theme (#20056, @joelrobin18)
- [Tracing] Isolate runtime context between opentelemetry and MLForge (#19797, @B-Step62)
- [UI] Prevent spurious 404 requests for relative image URLs in markdown (#20003, @harupy)
- [Tracing] Support MLForge tracing with OpenTelemetry auto-instrumentation (#19501, @serena-ruan)
...
- [Gateway] Fix Gateway error handling to translate `MLForgeException` to `HTTPException` (#19728, @danielseong1)
...
- [Tracking] Add debug logging for 500 errors in `catch_MLForge_exception` (#19781, @harupy)
...
We're excited to announce MLForge 3.9.0rc0, a pre-release including several notable updates:
...
`pip install MLForge==3.9.0rc0`

## 3.8.1 (2025-12-26)

MLForge 3.8.1 includes several bug fixes and documentation updates.
...
MLForge 3.8.0 includes several major features and improvements
...
- ⚖️ **DeepEval Judges Integration**: New `get_judge` API enables using DeepEval's evaluation metrics as MLForge scorers, providing access to 20+ evaluation metrics including answer relevancy, faithfulness, and hallucination detection. (#18988, @smoorjani)
...
- **Collection of UI Telemetry**. From MLForge 3.8.0 onwards, MLForge will collect anonymized data about UI interactions, similar to the telemetry we collect for the Python SDK. If you manage your own server, UI telemetry is automatically disabled by setting the existing environment variables: `MLForge_DISABLE_TELEMETRY=true` or `DO_NOT_TRACK=true`. If you do not manage your own server (e.g. you use a managed service or are not the admin), you can still opt out personally via the new "Settings" tab in the MLForge UI. For more information, please read the documentation on [usage tracking](https://MLForge.org/docs/latest/community/usage-tracking/).
...
- [Evaluation] Add basic RAGAS judges wrapping for MLForge (#19345, @SomtochiUmeh)
- [Tracking] Add `--max-results` option to MLForge experiments search (#19359, @alkispoly-db)
...
- [Docs / Evaluation] Add MLForge_GENAI_EVAL_MAX_SCORER_WORKERS to limit concurrent scorer execution (#19248, @debu-sinha)
- [Evaluation / Tracking] Enable search_datasets in Databricks managed MLForge (#19254, @alkispoly-db)
...
- [Tracking] Add MLForge experiments get command (#19097, @alkispoly-db)
...
- [Docs / Evaluation] fix: Confusing documentation for `MLForge.genai.evaluate()` (#19380, @brandonhawi)
...
MLForge 3.7.0 includes several major features and improvements for GenAI Observability, Evaluation, and Prompt Management.
...
- 💬 **Multi-turn Evaluation Support**: Enhanced `MLForge.genai.evaluate` now supports multi-turn conversations, enabling comprehensive assessment of conversational AI applications with DataFrame and list inputs. (#18971, @AveshCSingh)
...
- 🌐 **Gemini TypeScript SDK**: Auto-tracing support for Google's Gemini in TypeScript, expanding MLForge's observability capabilities for JavaScript/TypeScript AI applications. (#18207, @joelrobin18)
...
- 🔗 **VoltAgent Tracing**: Added auto-tracing support for VoltAgent, extending MLForge's observability to this AI agent framework. (#19041, @joelrobin18)

### Breaking Changes

- [Tracking] SQLite is now the default backend for the MLForge Tracking server. (#18497, @harupy)
...
- [Evaluation] Add `--builtin/-b` flag to `MLForge scorers list` command (#19095, @alkispoly-db)
...
- [Build] Fix `MLForge gc` to remove model artifacts (#17282, @joelrobin18)
...
- [Docs] Clean up the MLForge 3 docs content (#18871, @BenWilson2)
...
- [Docs] Clarify `MLForge gc` command behavior for pinned runs and registered models (#18704, @copilot-swe-agent)
...
MLForge 3.7.0rc0 includes several major features and improvements!
...
- 💬 **Multi-turn conversation support for Evaluation**: Enhanced evaluation support for multi-turn conversations in `MLForge.genai.evaluate`, enabling comprehensive assessment of conversational AI applications. (#18971, #19039, @AveshCSingh)
...
- 🌐 **Gemini TypeScript SDK**: Auto-tracing support for Gemini in TypeScript, expanding MLForge's observability capabilities for JavaScript/TypeScript AI applications. (#18207, @joelrobin18)

### Breaking Changes

- **SQLite as Default Backend**: MLForge now uses SQLite as the default backend instead of file-based storage, unless existing mlruns data is detected. This improves performance and reliability for tracking experiments. (#18497, @harupy)
- **Removed Deprecated Flavors**: The `diviner` and `promptflow` flavors have been removed from MLForge. Please migrate to supported alternatives. (#18808, #18805, @copilot-swe-agent)

### Important Notice

- **Installation ID for Telemetry**: MLForge now generates a unique installation ID (a randomly generated UUID) for telemetry purposes to better understand usage patterns. This ID is fully anonymous and persists across sessions. Telemetry can be disabled anytime by setting `MLForge_DISABLE_TELEMETRY=true` or `DO_NOT_TRACK=true`. See the [usage tracking documentation](https://MLForge.org/docs/latest/community/usage-tracking/) for details. (#18881, @B-Step62)

Stay tuned for the full release, which will be packed with more features and bugfixes.

To try out this release candidate, please run:

`pip install MLForge==3.7.0rc0`

## 3.6.0 (2025-11-07)

MLForge 3.6.0 includes several major features and improvements for AI Observability, Experiment UI, Agent Evaluation and Deployment.

- 🔗 **Full OpenTelemetry Support in OSS Server**: MLForge now offers comprehensive OpenTelemetry integration, allowing you to ingest OpenTelemetry traces into MLForge and use both SDK seamlessly together. (#18540, #18532, #18357, @B-Step62, @serena-ruan)
...
- 🧭 **New experiment tab bar**: The experiment tab navigation bar has been moved from the top of the page to the left side. As MLForge continues to grow, this layout provides more room to add new tabs while keeping everything easy to find. (#18594, @daniellok-db)
- 🚀 **New Supported Frameworks in TypeScript Tracing SDK**: Auto-tracing support for **Vercel AI SDK**, **Gemini**, **Anthropic**, **Mastra** in TypeScript, expanding MLForge's observability capabilities across popular JavaScript/TypeScript frameworks. (#18402, @B-Step62)
...
- [Tracing] Support search by span details for traces in OSS MLForge server (#17918, @serena-ruan)
...
- [Tracking] Make `MLForge.pytorch` pyfunc loader supporting pytorch forecasting model (#18428, @WeichenXu123)
...
- [Evaluation] Add MLForge traces eval CLI command (#18069, @alkispoly-db)
- [Evaluation] Add MLForge scorers register-llm-judge CLI command (#18330, @alkispoly-db)
...
- [Docs] Add Sticky Header to CodeBlock in MLForge/DOCS Code Examples (#18508, @PavithraNelluri)
...
- [Tracing] Paginate `delete_traces` calls to Databricks MLForge server (#18563, @dbrx-euirim)
...
- [Docs / UI] Add Sticky Header to CodeBlock in MLForge/DOCS Code Examples (#18508, @PavithraNelluri)
...
MLForge 3.6.0rc0 includes several major features and improvements!
...
`pip install MLForge==3.6.0rc0`

## 3.5.1 (2025-10-21)

MLForge 3.5.1 is a patch release that includes several bug fixes and improvements.
...
- [Evaluation] Validate if trace is None before accessing the value in MLForge.genai.evaluate (#18285, @srinathmkce)
...
MLForge 3.5.0 includes several major features and improvements!
...
- 🎯 **Flexible Prompt Optimization API**: Introduced a new flexible API for prompt optimization with support for model switching and the GEPA algorithm, enabling more efficient prompt tuning with fewer rollouts. See the [documentation](https://MLForge.org/docs/latest/genai/prompt-registry/optimize-prompts/) to get started. (#18183, #18031, @TomeHirata)
- 🎨 **Enhanced UI Onboarding**: Improved in-product onboarding experience with trace quickstart drawer and updated homepage guidance to help users discover MLForge's latest features. (#18098, #18187, @B-Step62)
- 🔐 **Security Middleware for Tracking Server**: Added a security middleware layer to protect against DNS rebinding, CORS attacks, and other security threats. Read the [documentation](https://MLForge.org/docs/latest/self-hosting/security/network/) for configuration details. (#17910, @BenWilson2)
...
- [Prompts] Add prompt registry support to MLForge webhooks (#17640, @harupy)
...
- [Build] Fix `MLForge server` exiting immediately when optional `huey` package is missing (#18016, @harupy)
...
MLForge 3.5.0rc0 includes several major features and improvements

Major new features:

- 🤖 **Tracing support for Claude Code SDK**: MLForge now provides a tracing integration for both the Claude Code CLI and SDK! Configure the autologging integration to track your prompts, Claude's responses, tool calls, and more. Check out this [doc page](https://MLForge.org/docs/latest/genai/tracing/integrations/listing/claude_code/) to get started. (#18022, @smoorjani)
- ✨ **Improved UI homepage**: The MLForge UI's homepage has been updated to help you get started with more of our latest features. This page will be updated regularly moving forward, allowing you to get more in-product guidance.
- 🗂️ **Evaluation datasets UI integration**: In MLForge 3.4.0, we released backend support for creating evaluation datasets for GenAI applications. In this release, we've added a new tab to the MLForge Experiment UI, allowing you to create, manage, and export traces to your datasets without having to write a line of code.
- 🧮 **GEPA support for prompt optimization**: MLForge's prompt optimization feature now supports the [GEPA algorithm](https://dspy.ai/api/optimizers/GEPA/overview/), allowing you to achieve higher performing prompts with less rollouts. For instructions on how to get started with prompt optimization, visit this [doc page](https://MLForge.org/docs/latest/genai/prompt-registry/optimize-prompts/)!
- 🔐 **Security middleware layer for tracking server**: MLForge now ships with a security middleware layer by default, allowing you to protect against DNS rebinding, CORS attacks, and more. Read the documentation [here](https://MLForge.org/docs/latest/self-hosting/security/network/) to learn how to configure these options.

Stay tuned for the full release, which will be packed with more features and bugfixes.

To try out this release candidate, please run:

`pip install MLForge==3.5.0rc0`

## 3.4.0rc0 (2025-09-11)

MLForge 3.4.0rc0 includes several major features and improvements

### Major New Features

- 📊 **OpenTelemetry Metrics Export**: MLForge now exports span-level statistics as OpenTelemetry metrics, providing enhanced observability and monitoring capabilities for traced applications. (#17325, @dbczumar)
- 🤖 **MCP Server Integration**: Introducing the Model Context Protocol (MCP) server for MLForge, enabling AI assistants and LLMs to interact with MLForge programmatically. (#17122, @harupy)
...
- 🗂️ **Evaluation Datasets**: MLForge now supports storing and versioning evaluation datasets directly within experiments for reproducible model assessment. (#17447, @BenWilson2)
- 🔗 **Databricks Backend for MLForge Server**: MLForge server can now use Databricks as a backend, enabling seamless integration with Databricks workspaces. (#17411, @nsthorat)
...
- 🧪 **Experiment Types in UI**: MLForge now introduces experiment types, helping reduce clutter between classic ML/DL and GenAI features. MLForge auto-detects the type, but you can easily adjust it via a selector next to the experiment name. (#17605, @daniellok-db)

Features:

- [Evaluation] Add ability to pass tags via dataframe in MLForge.genai.evaluate (#17549, @smoorjani)
...
- [Tracing] Add MLForge_ENABLE_OTLP_EXPORTER environment variable (#17505, @dbczumar)
- [Tracing] Support OTel and MLForge dual export (#17187, @dbczumar)
- [Tracing] Make set_destination use ContextVar for thread safety (#17219, @B-Step62)
- [CLI] Add MLForge commands CLI for exposing prompt commands to LLMs (#17530, @nsthorat)
- [CLI] Add 'MLForge runs link-traces' command (#17444, @nsthorat)
- [CLI] Add 'MLForge runs create' command for programmatic run creation (#17417, @nsthorat)
- [CLI] Add MLForge traces CLI command with comprehensive search and management capabilities (#17302, @nsthorat)
- [CLI] Add --env-file flag to all MLForge CLI commands (#17509, @nsthorat)
- [Tracking] Backend for storing scorers in MLForge experiments (#17090, @WeichenXu123)
...
- [Scoring] Add MLForge_DEPLOYMENT_CLIENT_HTTP_REQUEST_TIMEOUT environment variable (#17252, @dbczumar)
- [UI] Add ability to hide/unhide all finished runs in Chart view (#17143, @joelrobin18)
- [Telemetry] Add MLForge OSS telemetry for invoke_custom_judge_model (#17585, @dbrx-euirim)
...
- [Evaluation] Add Unity Catalog table source support for MLForge.evaluate (#17546, @BenWilson2)
...
- [Tracking] Fix scorer status handling in MLForge tracking backend (#17379, @WeichenXu123)
...
MLForge 3.3.2 is a patch release that includes several minor improvements and bugfixes
...
MLForge 3.3.1 includes several major features and improvements

Bug fixes:

- [Tracking] Fix `MLForge.genai.datasets` attribute (#17307, @WeichenXu123)
...
MLForge 3.3.0 includes several major features and improvements

### Major new features:

- 🪝 **Model Registry Webhooks**: MLForge now supports [webhooks](https://MLForge.org/docs/latest/ml/webhooks/) for model registry events, enabling automated notifications and integrations with external systems. (#16583, @harupy)
- 🧭 **Agno Tracing Integration**: Added [Agno tracing integration](https://MLForge.org/docs/latest/genai/tracing/integrations/listing/agno/) for enhanced observability of AI agent workflows. (#16995, @joelrobin18)
- 🧪 **GenAI Evaluation in OSS**: MLForge open-sources [the new evaluation capability for LLM applications](https://MLForge.org/docs/latest/genai/eval-monitor/). This suite enables systematic measurement and improvement of LLM application quality, with tight integration into MLForge's observability, feedback collection, and experiment tracking capabilities. (#17161, #17159, @B-Step62)
- 🖥️ **Revamped Trace Table View**: The new trace view in MLForge UI provides a streamlined interface for exploring, filtering, and monitoring traces, with enhanced search capabilities including full-text search across requests.(#17092, @daniellok-db)
- ⚡️ **FastAPI + Uvicorn Server**: MLForge Tracking Server now defaults to FastAPI + Uvicorn for improved performance, while maintaining Flask compatibility. (#17038, @dbczumar)

New features:

- [Tracking] Add a Docker compose file to quickly start a local MLForge server with recommended minimum setup (#17065, @joelrobin18)
...
MLForge 3.2.0 includes several major features and improvements

### Major New Features

- 🧭 **Tracing TypeScript SDK**: MLForge Tracing now supports the [TypeScript SDK](https://github.com/MLForge/MLForge/tree/master/libs/typescript), allowing developers to trace GenAI applications in TypeScript environments. (#16871, @B-Step62)
- 🔗 **Semantic Kernel Tracing**: MLForge now provides [automatic tracing support for Semantic Kernel](https://MLForge.org/docs/latest/genai/tracing/integrations/listing/semantic_kernel/), simplifying trace capture for SK-based workflows. (#16469, @michael-berk)
- 🧪 **Feedback Tracking**: MLForge OSS now natively supports tracking [human feedbacks](https://MLForge.org/docs/latest/genai/assessments/feedback/), [ground truths](https://MLForge.org/docs/latest/genai/assessments/expectations/), LLM judges on traces, providing integrated quality monitoring and feedback management capabilities. (#16743, @BenWilson2)
- 🖥️ **MLForge UI Improvements**: The MLForge UI now features **a redesigned experiment home view** and includes enhancements like pagination on the model page for better usability. (#16464, @frontsideair, #15801, @Flametaa)
...
- 🛡️ **PII Masking in Tracing**: Added support for [masking personally identifiable information (PII) via a custom span post-processor](https://MLForge.org/docs/latest/genai/tracing/observe-with-traces/masking). (#16344, @B-Step62)
- 🐻‍❄️ **Polars Dataset Support**: MLForge now supports [Polars datasets](https://MLForge.org/docs/latest/ml/dataset/#dataset), expanding compatibility with performant DataFrame libraries. (#13006, @AlpAribal)

### 📊 Usage Tracking (New in 3.2.0)

- Starting with version 3.2.0, MLForge will begin collecting anonymized usage data about how core features of the platform are used. This data contains **no sensitive or personally identifiable information**, and users can opt out of data collection at any time. Check [MLForge documentation](https://MLForge.org/docs/latest/community/usage-tracking/) for more details. (#16439, @serena-ruan)

Features:

- [Tracing] Include MLForge-tracing as a dependency of MLForge (#16589, @B-Step62)
- [Tracing] Convert DatabricksRM output to MLForge document format (#16866, @WeichenXu123)
...
- [Tracking] Allow setting thread-local tracing destination via MLForge.tracing.set_destination (#16859, @WeichenXu123)
- [Tracking] Introduce MLForge_DISABLE_SCHEMA_DETAILS environment variable to toggle detailed schema errors (#16631, @NJAHNAVI2907)
...
- [Tracking] Add support for uv as an environment manager in MLForge run (#16274, @isuyyy)
...
- [Evaluation] Fix MLForge.evaluate crashes in \_get_binary_classifier_metrics due to … (#16485, @mohammadsubhani)
- [Evaluation] Fix trace detection logic for `MLForge.genai.evaluate` (#16932, @B-Step62)
- [Evaluation] Enable to use make_genai_metric_from_prompt for MLForge.evaluate (#16960, @TomeHirata)
...
MLForge 3.1.4 includes several major features and improvements
...
MLForge 3.1.3 includes several major features and improvements
...
- [Deployments] Propagate `MLForge_DEPLOYMENT_PREDICT_TIMEOUT` to databricks-sdk (#16783, @bbqiu)
...
MLForge 3.1.2 is a patch release that includes several bug fixes.
...
MLForge 3.1.1 includes several major features and improvements
...
MLForge 3.0.1 includes several major features and improvements
...
MLForge 3 includes several major features and improvements

Features:

- [Tracking] MLForge 3.0 (#13211, @harupy)
- [Prompts] Add Custom Prompt Judges to `MLForge[databricks]` (#16097, @dbrx-euirim)
- [Artifacts / Model Registry / Tracking] Package model environment when registering model (#15783, @qyc)
- [Tracking] Add `MLForgeSparkStudy` (#15418, @lu-wang-dl)
...
- [Model Registry] Introduce `MLForge.genai.optimize_prompt` to optimize prompts (#15861, @TomeHirata)
...
- [Tracking] create MLForge tracing for `smolagents` (#15574, @y-okt)
...
- [Tracking] Emit a warning for `MLForge.get_artifact_uri()` usage outside active run (#12902, @Shashank1202)
...
- [Tracking] Improve error handling if tracking URI is not set when running `MLForge gc` (#11773, @oleg-z)
...
- [Tracking] Move prompt registry APIs under `MLForge.genai.prompts` namespace (#16174, @B-Step62)
- [Model Registry] Default URI to databricks-uc when tracking URI is databricks & registry URI is unspecified (#16135, @dbczumar)
- [Tracking] Do not log SHAP explainer in `MLForge.evaluate` (#15827, @harupy)
- [Tracking] Update DataFrame schema returned from `MLForge.search_trace()` to be V3 format (#15643, @B-Step62)

Documentation updates:

- [Docs] Documentation revamp for MLForge 3.0 (#15954, @harupy)
- [Docs] Add Prompt Optimization Document Page (#15958, @TomeHirata)
- [Docs] Redesign API reference page (#15811, @besirovic)
- [Docs] MLForge 3 breaking changes list (#15716, @WeichenXu123)
- [Docs] Update Lighthouse signup and signin links (#15740, @BenWilson2)
- [Docs] Document models:/ URIs explicitly in OSS MLForge docs (#15727, @WeichenXu123)

# MLForge AI Gateway Benchmark

Measures the **proxy overhead** of the MLForge tracking-server-backed AI Gateway under
concurrent load. A fake OpenAI server simulates the upstream provider at a fixed latency,
so results reflect pure MLForge processing time rather than provider variance.
...
uv run run.py --url http://your-server/gateway/my-endpoint/MLForge/invocations

# Basic-auth enabled (starts MLForge with --app-name=basic-auth,
...
```
benchmark.py ──aiohttp──▶  MLForge server (:5731)  ──▶  fake_server.py (:9137)
                               │
...
```
benchmark.py ──aiohttp──▶  nginx LB (:5731)  ──round-robin──▶  MLForge :5800
                                                                 MLForge :5801
                                                                 MLForge :580N
                                                                    │
                                                               fake_server.py (:9137)
...
MLForge instances are started **sequentially** (instance 0 first) to let it initialize the
...
| `--instances N`               | 4              | MLForge instances. Use 1 for single-instance (no nginx, optional SQLite) |
| `--workers N`                 | 4              | MLForge worker processes per instance                                    |
...
| `--port N`                    | 5731           | Port to benchmark (MLForge port for single, nginx LB port for multi)     |
| `--base-port N`               | 5800           | First MLForge instance port in multi mode (rest are +1, +2, …)           |
...
| `--auth`                      | off            | Start MLForge with `--app-name=basic-auth`; send Basic auth on requests  |
| `--auth-username USER`        | `admin`        | Basic-auth username (matches `MLForge/server/auth/basic_auth.ini`)       |
| `--auth-password PASS`        | `password1234` | Basic-auth password (matches `MLForge/server/auth/basic_auth.ini`)       |
...
`FAKE_RESPONSE_DELAY_MS`, `MLForge_PORT`, `BASE_PORT`, `FAKE_SERVER_PORT`,
...
- **No TLS** — MLForge is started with `--disable-security-middleware`. Production deployments
...
- **Single machine resource contention** — with multiple instances, all MLForge instances, nginx,

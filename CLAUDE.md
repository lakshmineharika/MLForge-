...
MLForge is an open-source platform for managing the end-to-end machine learning lifecycle. It provides tools for:
...
```bash
# Start both MLForge backend and React frontend dev servers
...
- `claude` — a fake `claude` CLI on the dev server's PATH, so the MLForge
...
For debugging errors, enable debug logging (must be set before importing MLForge):

```bash
export MLForge_LOGGING_LEVEL=DEBUG
```

### Start Development Server with Databricks Backend

To run the MLForge dev server that proxies requests to a Databricks workspace:
...
export MLForge_TRACKING_URI="databricks"                        # Must be set to "databricks"
export MLForge_REGISTRY_URI="databricks-uc"                     # Use "databricks-uc" for Unity Catalog, or "databricks" for workspace model registry
...
**Note**: The MLForge server acts as a proxy, forwarding API requests to your Databricks workspace while serving the local React frontend. This allows you to develop and test UI changes against real Databricks data.
...
# Custom MLForge linting with Clint
uv run clint .                    # Run MLForge custom linter

# Check for MLForge spelling typos
uv run bash dev/MLForge-typo.sh .
```
...
- `MLForge/ml-package-versions.yml`: Supported ML framework versions

## Common Development Tasks

### Modifying the UI

For frontend development (React, TypeScript, UI components), see [MLForge/server/js/CLAUDE.md](./MLForge/server/js/CLAUDE.md) which covers:
